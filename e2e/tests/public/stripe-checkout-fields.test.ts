...
test.describe('Ghost Public - Stripe Checkout Fields', () => {
  ...
  async function readMember(
    request: APIRequestContext,
    email: string,
  ): Promise<MemberResponse['members'][number]> {
    const found = await request.get(
      `/ghost/api/admin/members/?search=${encodeURIComponent(email)}`,
    );
  ...
  test("collects a publisher's questions and a delivery address, and saves both on the member", async ({
    page,
    stripe,
  }) => {
    ...
    expect(session?.request.shipping_address_collection).toEqual({
      allowed_countries: ['GB', 'IE'],
    });
  ...
  test('refuses to complete a checkout with data it never collected', async ({ page, stripe }) => {
    ...
    await expect(
      stripe!.completeLatestSubscriptionCheckout({
