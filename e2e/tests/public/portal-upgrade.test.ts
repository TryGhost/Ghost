...
test.describe('Ghost Public - Portal Upgrade', () => {
  ...
  test('free member upgrades to paid via portal - portal shows billing info', async ({
    page,
    stripe,
  }) => {
  ...
  test('comped member upgrades to paid via portal - portal shows billing info', async ({
    page,
    stripe,
  }) => {
    const memberFactory = createMemberFactory(page.request);
  ...
  test('paid member changes plan in portal - subscription switches between monthly and yearly', async ({
    page,
    stripe,
  }) => {
    const tier = await createPaidPortalTier(page.request, {
