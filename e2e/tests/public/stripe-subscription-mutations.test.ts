...
function getLatestCheckoutSuccessUrl(
  stripeCheckoutCount: { response: { success_url: string } }[],
): string {
...
test.describe('Ghost Public - Stripe Subscription Mutations', () => {
  ...
  test('paid member subscription update via ghost - switches the fake stripe price', async ({
    page,
    stripe,
  }) => {
        ...
        () => {
          return stripe!
            ...
            .filter(
              (item) =>
                item.product ===
                stripe!.getProducts().find((product) => product.name === tier.name)?.id,
            ).length;
        },
        { timeout: 10000 },
      )
    ...
    const response = await page
      .context()
      .request.put(`/members/api/subscriptions/${subscription!.id}/`, {
        data: {
          identity,
          tierId: tier.id,
    ...
    const updatedSubscription = stripe!
      .getSubscriptions()
