...
test.describe('Ghost Public - Stripe Offer Checkout', () => {
  ...
  test('offer checkout creates a fake stripe coupon - redeemed offer is linked to the subscription', async ({
    page,
    stripe,
  }) => {
    ...
    const createdMember = await stripe!.completeLatestSubscriptionCheckout({
      name: 'Offer Member',
    });
