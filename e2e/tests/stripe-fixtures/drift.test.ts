...
import {buildCheckoutSession, buildCoupon, buildCustomer, buildPaymentMethod, buildPrice, buildProduct, buildSubscription} from '@/helpers/services/stripe/builders';
import {expect, test} from '@playwright/test';
import {fileURLToPath} from 'node:url';

const fixtureDir = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '../../helpers/services/stripe/fixtures');
...
test.describe('Stripe fixtures - builder shapes match captured Stripe responses', () => {
    const cases = [
        ...
        {name: 'customer', built: buildCustomer({email: 'a@b.test', name: 'A B'}), captured: 'customer'},
        {name: 'subscription', built: buildSubscription({customerId: 'cus_x'}), captured: 'subscription.paid'},
        {name: 'checkout session', built: buildCheckoutSession().response, captured: 'checkout_session.subscription'}
    ];
    ...
    for (const {name, built, captured} of cases) {
        test(`${name} - invents no field Stripe does not return`, () => {
            ...
            expect(invented, `${name} builder emits keys Stripe does not: ${invented.join(', ')}`).toEqual([]);
        });
    }
    ...
    const consumed: Array<{object: string; path: string; readAt: string}> = [
        {object: 'subscription', path: 'plan.nickname', readAt: 'member-repository isComplimentaryPlanNickname'},
        ...
        {object: 'subscription', path: 'status', readAt: 'member-repository isActiveSubscriptionStatus'},
        {object: 'subscription', path: 'current_period_end', readAt: 'member-repository linkSubscription'},
        {object: 'subscription', path: 'cancel_at_period_end', readAt: 'member-repository linkSubscription'},
        {object: 'subscription', path: 'items.data.0.price.id', readAt: 'member-repository linkSubscription'},
    ...
    for (const {object, path: fieldPath, readAt} of consumed) {
        test(`${object}.${fieldPath} - built and captured, read by ${readAt}`, () => {
            expect(resolve(builtByObject[object], fieldPath), `builder omits ${object}.${fieldPath}`)
                .not.toBeUndefined();
            expect(resolve(fixture(capturedByObject[object]), fieldPath), `Stripe does not return ${object}.${fieldPath}, so this entry is wrong`)
                .not.toBeUndefined();
        });
    }
});
...
test.describe('Stripe fixtures - completed checkout carries what we read', () => {
    ...
    test('shipping address parts match the ones our address type declares', () => {
        expect(Object.keys(completed.shipping?.address ?? {}).sort())
            .toEqual(['city', 'country', 'line1', 'line2', 'postal_code', 'state']);
    });
