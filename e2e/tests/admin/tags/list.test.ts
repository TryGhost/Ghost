...
test.describe('Ghost Admin - Tags', () => {
    ...
    test('lists public and internal tags separately', async ({page}) => {
        ...
        expect(await tagsPage.activeTab).toHaveText('Public tags');

        expect(await tagsPage.tagList).toBeVisible();
        expect(await tagsPage.tagListRow.first()).toContainText('Public Tag Name');
        expect(await tagsPage.tagListRow.first()).toContainText('Public Tag description');
        expect(await tagsPage.tagListRow).toHaveCount(2);

        // Can switch to internal tags
        await tagsPage.selectTab('Internal tags');
        expect(await tagsPage.activeTab).toHaveText('Internal tags');

        expect(await tagsPage.tagList).toBeVisible();
        expect(await tagsPage.tagListRow).toContainText('Internal Tag Name');
        expect(await tagsPage.tagListRow).toContainText('Internal Tag description');
        expect(await tagsPage.tagListRow).toHaveCount(1);
    ...
    test('create new tag', async ({page}) => {
        ...
        await expect(page.url()).toContain('/ghost/#/tags/new');
    });
    ...
    test('loads tags on scroll with pagination', async ({page}) => {
        ...
        await tagsPage.mockTagsResponse(async (request) => {
            ...
            const pageSize = 20;
            const pages = 3;
            const total = pages * (pageSize - 0.5);
