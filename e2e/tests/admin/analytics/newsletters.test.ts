...
test.describe('Ghost Admin - Newsletters', () => {
  ...
  test('empty average open rate and click rate card', async () => {
    ...
    await expect(newslettersPage.newslettersCard).toContainText(
      'No newsletters in the last 30 days',
    );
  ...
  test('empty top newsletters card', async () => {
    await expect(newslettersPage.topNewslettersCard).toContainText(
