...
test.describe('Ghost Admin - Growth', () => {
  ...
  test('empty top content card - posts and pages', async () => {
    await expect(growthPage.topContent.contentCard).toContainText(
      'Which posts or pages drove the most growth in the last 30 days',
    );
  ...
  test('empty top content card - posts', async () => {
    ...
    await expect(growthPage.topContent.contentCard).toContainText(
      'Which posts drove the most growth in the last 30 days',
    );
    await expect(growthPage.topContent.contentCard).toContainText('No conversions');
  });
  ...
  test('empty top content card - pages', async () => {
    ...
    await expect(growthPage.topContent.contentCard).toContainText(
      'Which pages drove the most growth in the last 30 days',
    );
    await expect(growthPage.topContent.contentCard).toContainText('No conversions');
  });
  ...
  test('empty top content card - sources', async () => {
    ...
    await expect(growthPage.topContent.contentCard).toContainText(
      'Which sources drove the most growth in the last 30 days',
    );
    await expect(growthPage.topContent.contentCard).toContainText('No conversions');
  });
});
