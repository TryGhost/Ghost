    ...
    .poll(
      async () => {
        ...
        const filter = encodeURIComponent(
          `data.created_at:<'${now}'+type:automated_email_sent_event+data.member_id:'${memberId}'`,
        );
        const response = await request.get(
          `/ghost/api/admin/members/events/?filter=${filter}&limit=5`,
        );
...
test.describe('Ghost Admin - Member Activity Events', () => {
  ...
  test('welcome email event appears in member activity feed', async ({ page }) => {
    ...
    const welcomeEmailEvent = memberDetailsPage.getActivityEventByText(
      /received welcome email \(Free\)/i,
    );
