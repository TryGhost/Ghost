...
async function completeSignupViaMagicLink(
  emailClient: EmailClient,
  page: Page,
  emailAddress: string,
) {
...
async function expectWelcomeEmailCount(
  emailClient: EmailClient,
  emailAddress: string,
...
async function getFreeWelcomeEmail(page: Page): Promise<AutomatedEmail> {
  ...
  const freeWelcomeEmail = data.automated_emails.find(
    (email) => email.slug === 'member-welcome-email-free',
  );
...
async function restoreWelcomeEmail(page: Page, automatedEmail: AutomatedEmail): Promise<void> {
  ...
  const response = await page.request.put(
    `/ghost/api/admin/automated_emails/${automatedEmail.id}/`,
    {
      data: {
        automated_emails: [automatedEmail],
      },
...
test.describe('Ghost Admin - Member Welcome Emails', () => {
  ...
  test('free signup sends welcome email after enabling it', async ({ page, browser, baseURL }) => {
    ...
    await expect(welcomeEmailsSection.freeWelcomeEmailToggle).toHaveAttribute(
      'aria-checked',
      'true',
    );
  ...
  test('free welcome email preview - renders edited subject and body', async ({ page }) => {
    ...
    const previewResponse = page.waitForResponse(
      (response) =>
        response.url().includes('/ghost/api/admin/automated_emails/') &&
  ...
  test('free welcome email preview - preserves subject tokens when edited and saved', async ({
    page,
    browser,
    baseURL,
  }) => {
