...
test.describe('Ghost Admin - Portal Settings', () => {
  ...
  test('shows free tier toggle in the Portal settings modal when Stripe is disconnected', async ({
    page,
  }) => {
  ...
  test.describe('paid signup links', () => {
    ...
    test('monthly signup link opens fake stripe checkout - navigates to checkout', async ({
      page,
    }) => {
      ...
      await settingsPage.goto();
      await settingsPage.portalSection.openCustomizeModal();
      const portalUrl = await settingsPage.portalSection.getPaidSignupLinkForTier(
        tier.name,
        tier.id,
        'monthly',
      );
    ...
    test('yearly signup link opens fake stripe checkout - navigates to checkout', async ({
      page,
    }) => {
      const tier = await createPaidPortalTier(page.request, {
