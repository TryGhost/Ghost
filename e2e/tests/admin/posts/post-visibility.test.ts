...
async function createAuthenticatedPublicPage(
  browser: Browser,
  baseURL: string,
  member: Member,
): Promise<{ context: BrowserContext; page: Page; postPage: PostPage }> {
...
test.describe('Ghost Admin - Post Visibility', () => {
  ...
  test('paid-members-only post shows paid subscriber gate', async ({ page }) => {
    ...
    const publicPage = new PostPage(page);
    await publicPage.gotoPost(post.slug);
    await expect(publicPage.accessCtaHeading).toHaveText(
  ...
  test.describe('specific tier visibility', () => {
    ...
    test('only allows selected tier members', async ({ page, browser }) => {
      ...
      const disallowedSession = await createAuthenticatedPublicPage(
        browser,
        baseURL,
        disallowedMember,
      );
