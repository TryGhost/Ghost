...
test.describe('Ghost Admin - Member activity', () => {
  ...
  test('follows a member from all activity to their profile and preserves activity filters', async ({
    ...
    await expect(page.getByRole('heading', { name: 'Member activity', exact: true })).toBeVisible();
    ...
    await expect(page.getByRole('heading', { name: 'Activity Target', exact: true })).toBeVisible();
