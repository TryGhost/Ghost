...
test.describe('Ghost Admin - Members List', () => {
  ...
  test('add yourself from empty state - creates a member for the current user', async ({
    page,
    ghostAccountOwner,
  }) => {
    ...
    await expect(membersPage.getMemberByName(ghostAccountOwner.name)).toContainText(
      ghostAccountOwner.email,
    );
  });
