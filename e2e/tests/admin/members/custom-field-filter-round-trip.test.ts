...
test.describe('Ghost Admin - Filter members by custom fields', () => {
  ...
  test('a custom field filter can be built, saved as a segment, and reopened intact', async ({
    ...
    const matchingMember = await memberFactory.create({
      name: `Ghost Employee ${stamp}`,
      email: `ghost-${stamp}@example.com`,
    });
    await memberFactory.create({
      name: `Acme Employee ${stamp}`,
      email: `acme-${stamp}@example.com`,
    });
  ...
  test('an is-set custom field filter matches members that have a value', async ({ page }) => {
    ...
    const withValue = await memberFactory.create({
      name: `Reachable ${stamp}`,
      email: `reachable-${stamp}@example.com`,
    });
    await memberFactory.create({
      name: `Unreachable ${stamp}`,
      email: `unreachable-${stamp}@example.com`,
    });
