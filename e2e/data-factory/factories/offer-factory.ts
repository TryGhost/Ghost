...
export class OfferFactory extends Factory<OfferCreateInput, AdminOffer> {
  ...
  build(options: Partial<OfferCreateInput> = {}): AdminOffer {
    ...
    const code =
      options.code ?? `${generateSlug(name)}-${faker.string.alphanumeric(6).toLowerCase()}`;
  ...
  async create(options: Partial<OfferCreateInput> = {}): Promise<AdminOffer> {
    if (!this.request) {
      throw new Error(
        'Cannot create without an HTTP client. Use createOfferFactory() for persisted test data access.',
      );
    }
  ...
  async update(id: string, input: OfferUpdateInput): Promise<AdminOffer> {
    if (!this.request) {
      throw new Error(
        'Cannot update without an HTTP client. Use createOfferFactory() for persisted test data access.',
      );
    }
  ...
  async getOffers(): Promise<AdminOffer[]> {
    if (!this.request) {
      throw new Error(
        'Cannot fetch offers without an HTTP client. Use createOfferFactory() for persisted test data access.',
      );
    }
  ...
  async getById(id: string): Promise<AdminOffer> {
    if (!this.request) {
      throw new Error(
        'Cannot fetch an offer without an HTTP client. Use createOfferFactory() for persisted test data access.',
      );
    }
  ...
  private async extractFirstOfferOrThrow(
    action: string,
    status: number,
