...
export class TwoFactorAuthPage {
    ...
    constructor(page: Page, emailService: IEmailService, emailContext: string) {
        this.page = page;
    ...
    async complete2FA() {
        ...
        logging.info(`Completing 2FA using email context: ${this.emailContext}`);
        ...
        logging.info('2FA Email received. Attempting to extract code...');
        ...
        if (!twoFactorCode) {
            ...
            logging.error(`Email body snippet for failed 2FA code extraction: ${bodySnippet}...`);
            throw new Error('Could not extract 2FA code from email for 2FA step.');
        }
        logging.info(`Extracted 2FA code: ${twoFactorCode}`);
