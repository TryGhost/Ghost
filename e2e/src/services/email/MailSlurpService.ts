...
export class MailSlurpService implements IEmailService {
    ...
    async waitForLatestEmail(inboxId: string, timeoutMs: number = 30000, unreadOnly: boolean = true): Promise<Email> {
        ...
        logging.info(`Waiting for email in MailSlurp inbox ${inboxId}...`);
    ...
    extractVerificationCode(emailBody: string): string | null {
        ...
        if (!codeMatch || !codeMatch[0]) {
            logging.error('Could not find 6-digit OTP in email body:', emailBody);
            return null;
        }
        return codeMatch[0];
    }
