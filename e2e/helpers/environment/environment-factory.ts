 ...
 * Detects by checking for the ghost_dev network and healthy MySQL container.
 */
export async function isDevEnvironmentAvailable(): Promise<boolean> {
    ...
    try {
        ...
        if (containers.length === 0 || !containers[0].Status.includes('healthy')) {
            debug('Dev environment not available: MySQL not healthy');
            return false;
        }
