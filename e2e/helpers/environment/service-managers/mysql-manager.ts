...
export class MySQLManager {
  ...
  async setupTestDatabase(
    ...
    try {
      ...
      throw error instanceof Error
        ? error
        : new Error(`Failed to setup test database: ${String(error)}`);
    }
  }
  ...
  async createDatabase(databaseName: string): Promise<void> {
    ...
    await this.exec(
      'mysql -uroot -proot -e "CREATE DATABASE IF NOT EXISTS \\`' + databaseName + '\\`;"',
    );
  ...
  async createSnapshot(
    sourceDatabase: string = 'ghost_testing',
    outputPath: string = '/tmp/dump.sql',
  ): Promise<void> {
  ...
  async restoreDatabaseFromSnapshot(
    database: string,
    snapshotPath: string = '/tmp/dump.sql',
  ): Promise<void> {
  ...
  async updateStripeSettings(
    ...
    const command =
      'mysql -uroot -proot -e "INSERT INTO \\`' +
      database +
      '\\`.settings ' +
      '(id, \\`group\\`, \\`key\\`, value, type, flags, created_at, updated_at) VALUES ' +
      "(SUBSTRING(REPLACE(UUID(), '-', ''), 1, 24), 'members', 'stripe_secret_key', '" +
      escapedSecretKey +
      "', 'string', NULL, NOW(), NOW()), " +
      "(SUBSTRING(REPLACE(UUID(), '-', ''), 1, 24), 'members', 'stripe_publishable_key', '" +
      escapedPublishableKey +
      "', 'string', NULL, NOW(), NOW()) " +
