...
export class DevGhostManager {
        ...
        const config: ContainerCreateOptions = {
            ...
            HostConfig: {
                Binds: [
                    ...
                    // Mount shared-config volume created by ghost-dev Compose project
                    // Compose prefixes volumes with project name, so it's ghost-dev_shared-config
    ...
    async runMigrations(database: string): Promise<void> {
        ...
        if (result.StatusCode !== 0) {
            // Try to get logs before the container is auto-removed
            try {
