...
export class GhostManager {
  ...
  async setup(database?: string): Promise<void> {
    ...
    this.gatewayContainer = await this.getOrCreateContainer(gatewayName, () =>
      this.createGatewayContainer(gatewayName, ghostName),
    );
    this.ghostContainer = await this.getOrCreateContainer(ghostName, () =>
      this.createGhostContainer(ghostName, database),
    );
    ...
    const config: ContainerCreateOptions = {
      ...
      Healthcheck: {
        ...
        Test: [
          ...
          'node',
          '-e',
          `fetch('http://localhost:${TEST_ENVIRONMENT.ghost.port}',{redirect:'manual'}).then(r=>process.exit(r.status<500?0:1)).catch(()=>process.exit(1))`,
        ],
    ...
    try {
      ...
      for (const [index, result] of results.entries()) {
        if (result.status === 'rejected') {
          debug(
            'cleanupAllContainers: failed to remove container',
            containers[index]?.Id,
            result.reason,
          );
        }
  ...
  private async probeHostReadiness(): Promise<boolean> {
    ...
    try {
      const response = await fetch(
        `http://localhost:${this.getGatewayPort()}/ghost/api/admin/authentication/setup`,
        {
          method: 'GET',
      ...
      const body = (await response.json().catch(() => null)) as {
        setup?: Array<{ status?: unknown }>;
      } | null;

      return (
        response.ok && Array.isArray(body?.setup) && typeof body.setup[0]?.status === 'boolean'
      );
    } catch (error) {
