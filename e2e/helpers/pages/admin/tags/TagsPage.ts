...
export class TagsPage extends AdminPage {
    ...
    constructor(page: Page) {
        ...
        // XXX: This is the only way I have found to reliably selecting the table rows 
        // without also targeting the header or any of the placeholder or spacer rows.
        this.tagListRow = this.tagList.locator('tr:not([aria-hidden]):not(:first-child)');
    ...
    async selectTab(tabText: string) {
        ...
        await tab.click();
        await this.tabs.locator('[data-state="active"]');
    }
