...
export class MembersListPage extends AdminPage {
  ...
  async addFilter(fieldName: string, value: string): Promise<void> {
    ...
    const placeholder =
      membersSel.textFilterFields[fieldName as keyof typeof membersSel.textFilterFields];
    if (placeholder) {
  ...
  async addSearchableFilter(
    ...
    await this.page.getByRole('option', { name: fieldName, exact: true }).click();
    await this.page
      .getByPlaceholder(`Search ${fieldName.toLowerCase()}...`)
      .pressSequentially(searchText);
