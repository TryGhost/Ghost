 ...
 * section.
 */
const menuAction = (page: Page, name: string) =>
  page.getByRole('menuitem', { name }).or(page.getByRole('button', { name }));
...
export class MemberDetailsPage extends AdminPage {
  ...
  constructor(page: Page) {
    ...
    this.disableCommentingConfirmButton = this.disableCommentingModal.getByRole('button', {
      name: 'Disable commenting',
    });
    this.disableCommentingCancelButton = this.disableCommentingModal.getByRole('button', {
      name: 'Cancel',
    });
    this.hideCommentsCheckbox = this.disableCommentingModal.getByRole('switch', {
      name: 'Hide all previous comments',
    });
   ...
   */
  async setCompositeCustomFieldValue(
    fieldName: string,
