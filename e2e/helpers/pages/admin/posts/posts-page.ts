...
export class PostsPage extends AdminPage {
  ...
  constructor(
    page: Page,
    { implementation = 'ember' }: { implementation?: PostsListImplementation } = {},
  ) {
    super(page);
  ...
  async selectOrder(orderName: string): Promise<void> {
    if (this.implementation !== 'ember') {
      ...
      // first needs it.
      throw new Error(
        'selectOrder drives the Ember sort dropdown; the React list sorts via its sort menu',
      );
    }
