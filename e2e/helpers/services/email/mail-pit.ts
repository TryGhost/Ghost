...
export interface EmailClient {
  ...
  search(
    queryOptions: Partial<EmailSearchQuery>,
...
export class MailPit implements EmailClient {
  ...
  async getMessages(limit: number = 50): Promise<EmailMessage[]> {
    const response = await this.executeApiCall(`messages?limit=${limit}`, 'fetch messages');

    return this.parseMessagesFromResponse(response);
  }

  async getMessageDetailed(message: EmailMessage): Promise<EmailMessageDetailed> {
    const response = await this.executeApiCall(`message/${message.ID}`, 'fetch message');
  ...
  async searchByQuery(
    options: Partial<EmailSearchQuery>,
    limit: number = 50,
  ): Promise<EmailMessage[]> {
