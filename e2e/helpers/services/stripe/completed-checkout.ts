...
function capturedSession(): Record<string, unknown> {
  return JSON.parse(
    fs.readFileSync(path.resolve(fixtureDir, 'checkout_session.completed.json'), 'utf8'),
  );
}
...
function answeredQuestions(
  ...
  const requested =
    (
      session.request as {
        custom_fields?: Array<{ key: string; label?: { custom?: string }; optional?: boolean }>;
      }
    ).custom_fields ?? [];
...
function shippingBlock(
  session: RecordedStripeCheckoutSession,
  given: CollectedCheckoutInput['shipping'],
  ...
  if (given !== undefined && given !== null && !collecting) {
    throw new Error(
      'The checkout never asked for a shipping address, so a member could not have given one.',
    );
  }
  ...
  return {
    ...
    address: Object.fromEntries(
      Object.keys(capturedShipping.address).map((part) => [
        part,
        address[part as keyof typeof address] ?? null,
      ]),
    ),
    carrier: null,
    phone: null,
...
export function buildCollectedCheckoutCompletedEvent(opts: {
  ...
  if (collected.taxId && !asks(session, 'tax_id_collection')) {
    throw new Error(
      'The checkout never asked for a tax number, so a member could not have given one.',
    );
  }
  if (collected.phone && !asks(session, 'phone_number_collection')) {
    throw new Error(
      'The checkout never asked for a phone number, so a member could not have given one.',
    );
  }
