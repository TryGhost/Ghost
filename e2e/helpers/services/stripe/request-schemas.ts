...
const bool = (fallback: boolean) => z.preprocess(
...
export const CreatePriceSchema = z.object({
    ...
    currency: z.string().transform(value => value.toLowerCase()).catch('usd'),
    unit_amount: num,
    recurring: z.object({interval: z.enum(PRICE_INTERVALS).optional().catch(undefined)})
        .optional()
        .catch(undefined),
    custom_unit_amount: z.preprocess(
...
export const CreateCheckoutSessionSchema = z.object({
    ...
    discounts: list(z.object({coupon: str}))
        .transform(items => items.map(item => ({coupon: item.coupon ?? ''})).filter(item => item.coupon))
        .optional()
        .catch(undefined),
    line_items: list(z.object({
        price: str,
        ...
        .transform(items => items.map((item) => {
            ...
            return {
                price: item.price,
                ...(currency && unitAmount !== undefined ? {price_data: {currency, unit_amount: unitAmount}} : {}),
    ...
    subscription_data: z.object({
        ...
        .transform(data => ({
            ...
            ...(typeof data.trial_period_days === 'number' ? {trial_period_days: data.trial_period_days} : {}),
    ...
    invoice_creation: z.object({
        ...
        .transform((data) => {
            ...
            return {
                ...
                ...(Object.keys(invoiceMetadata).length > 0 ? {invoice_data: {metadata: invoiceMetadata}} : {})
            };
        })
        .optional()
        .catch(undefined),
    custom_fields: list(z.object({
        key: z.string().catch(''),
        ...
        .transform(fields => fields.map(field => ({
            key: field.key,
            type: field.type,
    ...
    shipping_address_collection: z.object({allowed_countries: list(z.string())})
        .optional()
        .catch(undefined),
    tax_id_collection: z.object({enabled: bool(false)}).optional().catch(undefined),
    phone_number_collection: z.object({enabled: bool(false)}).optional().catch(undefined)
});

export const UpdateSubscriptionSchema = z.object({
    items: list(z.object({id: str, price: str}))
