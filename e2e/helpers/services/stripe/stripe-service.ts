...
export class StripeTestService {
  ...
  async completeLatestDonationCheckout(
    opts: {
      amount?: number;
  ...
  async completeLatestGiftCheckout(
    opts: {
      email?: string;
      name?: string;
    } = {},
    ...
    const session = this.getCheckoutSessions()
      .filter((item) => {
        return (
          item.response.mode === 'payment' &&
          (Boolean(item.response.metadata.ghost_gift_id) ||
            item.response.metadata.ghost_gift === 'true')
        );
      })
      .at(-1);
    ...
    if (!inlinePrice || !email) {
      throw new Error(
        `Gift checkout session ${session.response.id} is missing price or customer data`,
      );
    }
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `checkout.session.completed gift webhook failed (${response.status}): ${body}`,
      );
    }
  }

  async createPaidMemberViaWebhooks(opts: {
    email: string;
    name: string;
  }): Promise<CreatedPaidMember> {
    ...
    debug(
      'Seeded server with customer=%s, subscription=%s, paymentMethod=%s',
      customer.id,
      subscription.id,
      paymentMethod.id,
    );
  ...
  async sendInvoicePaymentSucceeded(opts: {
    subscription: StripeSubscription;
    amount?: number;
  }): Promise<void> {
  ...
  private async completeDonationCheckout(opts: {
    ...
    if (
      ...
      throw new Error(
        `Checkout session ${opts.sessionId} does not include a valid donation amount`,
      );
    }
  ...
  private resolveCheckoutCustomer(
    ...
    const existingCustomer = this.getCustomers().find((customer) => {
      return (
        customer.id === session.response.customer || (email ? customer.email === email : false)
      );
    });
  ...
  private async sendCheckoutSessionCompletedWebhook(
    ...
    if (!checkoutResponse.ok) {
      ...
      throw new Error(
        `checkout.session.completed webhook failed (${checkoutResponse.status}): ${body}`,
      );
    }
  }
  ...
  private async sendSubscriptionCreatedWebhook(subscription: StripeSubscription): Promise<void> {
    ...
    if (!subscriptionResponse.ok) {
      ...
      throw new Error(
        `customer.subscription.created webhook failed (${subscriptionResponse.status}): ${body}`,
      );
    }
  }
