...
export type StripeCoupon = Omit<
  Pick<
    ...
    | 'percent_off'
  >,
  'amount_off' | 'currency' | 'duration_in_months' | 'name' | 'percent_off'
> & {
...
export type StripeDiscount = Omit<
  Pick<Stripe.Discount, 'coupon' | 'end' | 'id' | 'object' | 'start'>,
  'coupon' | 'end'
> & {
  coupon: StripeCoupon;
  end: number | null;
...
export type StripePaymentMethod = Omit<
  Pick<Stripe.PaymentMethod, 'billing_details' | 'card' | 'id' | 'object' | 'type'>,
  'billing_details' | 'card' | 'type'
> & {
...
export type StripeSubscription = Omit<
  Pick<
    ...
    | 'trial_start'
  >,
  'customer' | 'default_payment_method' | 'discount' | 'items' | 'metadata'
> & {
  customer: string;
...
export type StripeCustomer = Omit<
  Pick<Stripe.Customer, 'email' | 'id' | 'name' | 'object'>,
  'email' | 'name'
> & {
  email: string;
  name: string;
...
export type StripeCheckoutSessionResponse = Omit<
  Pick<
    ...
    | 'url'
  >,
  'customer' | 'customer_email' | 'metadata' | 'url'
> & {
...
export function buildDiscount(opts: { coupon: StripeCoupon; start?: number }): StripeDiscount {
  const start = opts.start ?? Math.floor(Date.now() / 1000);
  let end: number | null = null;

  if (
    opts.coupon.duration === 'repeating' &&
    typeof opts.coupon.duration_in_months === 'number' &&
    opts.coupon.duration_in_months > 0
  ) {
...
export function buildSubscription(opts: {
  ...
  const trialDays =
    typeof opts.trialDays === 'number' && opts.trialDays > 0 ? opts.trialDays : null;
  const status = opts.status ?? (trialDays ? 'trialing' : 'active');
  const trialEnd = trialDays ? startDate + 60 * 60 * 24 * trialDays : null;
  const currentPeriodEnd =
    trialDays && status === 'trialing' && trialEnd ? trialEnd : startDate + 60 * 60 * 24 * 31;
