...
} from './fake-checkout-page-renderer';
  ...
  protected setupRoutes(): void {
    ...
    this.app.get('/v1/products/:id', (req, res) => {
      ...
      if (!product) {
        ...
        res
          .status(404)
          .json({ error: { type: 'invalid_request_error', message: 'No such product' } });
        return;
      }
    ...
    this.app.get('/v1/prices/:id', (req, res) => {
      ...
      if (!price) {
        ...
        res
          .status(404)
          .json({ error: { type: 'invalid_request_error', message: 'No such price' } });
        return;
      }
    ...
    this.app.get('/v1/customers/:id', (req, res) => {
      ...
      this.debug(
        `Returning customer: ${customerId} with ${customerSubscriptions.length} subscription(s)`,
      );
    ...
    this.app.post('/v1/coupons', (req, res) => {
      ...
      const hasAmountOff =
        amountOffRaw !== undefined && amountOffRaw !== null && amountOffRaw !== '';
      const hasPercentOff =
        percentOffRaw !== undefined && percentOffRaw !== null && percentOffRaw !== '';
      ...
      if (hasAmountOff && hasPercentOff) {
        return badRequest('You cannot provide both percent_off and amount_off.', 'percent_off');
      }
      ...
      if (duration === 'repeating') {
        ...
        if (
          typeof parsedDurationInMonths !== 'number' ||
          !Number.isInteger(parsedDurationInMonths) ||
          parsedDurationInMonths <= 0
        ) {
          return badRequest(
    ...
    this.app.get('/v1/subscriptions/:id', (req, res) => {
      ...
      if (!subscription) {
        ...
        res
          .status(404)
          .json({ error: { type: 'invalid_request_error', message: 'No such subscription' } });
        return;
      }
      ...
      if (
        expand.includes('default_payment_method') &&
      ...
      this.debug(
        `Returning subscription: ${subscriptionId} (expand: ${expand.join(', ') || 'none'})`,
      );
      res.status(200).json(response);
    });
    ...
    this.app.post('/v1/subscriptions/:id', (req, res) => {
      ...
      if (defaultPaymentMethod !== undefined) {
        if (defaultPaymentMethod !== '' && !this.paymentMethods.has(defaultPaymentMethod)) {
          this.debug(
            `Cannot update subscription ${subscriptionId} with missing payment method: ${defaultPaymentMethod}`,
          );
          res
            .status(400)
            .json({ error: { type: 'invalid_request_error', message: 'No such payment method' } });
          return;
        }
      ...
      if (itemUpdates.length > 0) {
        ...
        if (missingPriceId) {
          this.debug(
            `Cannot update subscription ${subscriptionId} with missing price: ${missingPriceId}`,
          );
          res
            .status(400)
            .json({ error: { type: 'invalid_request_error', message: 'No such price' } });
    ...
    this.app.post('/v1/checkout/sessions', (req, res) => {
      ...
      if (missingCouponId) {
        ...
        res
          .status(400)
          .json({ error: { type: 'invalid_request_error', message: 'No such coupon' } });
        return;
      }
  ...
  private validateCheckoutSessionRequest(body: Record<string, unknown>): string | null {
    ...
    if ('customer_update' in body) {
      if (body.customer_update === '') {
        return (
          "You passed an empty string for 'customer_update'. We assume empty values are an " +
