...
export type PortalSignupOfferInput = Pick<
  OfferCreateInput,
  'amount' | 'duration' | 'duration_in_months' | 'type'
> & {
  codePrefix: string;
...
export async function completeOfferSignupViaPortal(
  page: Page,
  stripe: StripeTestService,
  opts?: { emailAddress?: string; name?: string },
...
export async function redeemOfferViaPortal(
  page: Page,
  stripe: StripeTestService,
  opts?: {
    emailAddress?: string;
