...
export async function completeDonationViaFakeCheckout(page: Page, stripe: StripeTestService, opts: CompleteDonationOptions = {}): Promise<void> {
    ...
    await expect.poll(async () => {
        const candidates = await mailpit.search({subject: opts.name ?? email}, {timeoutMs: null});
        const messages = await Promise.all(candidates.map(candidate => mailpit.getMessageDetailed(candidate)));

        return messages.some(message => message.HTML.includes(email));
    }, {timeout: 10000, message: `No donation notification email found for ${email}`}).toBe(true);
