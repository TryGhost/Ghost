...
export async function provision(stripe: Stripe): Promise<{product: Stripe.Product; prices: Stripe.Price[]; coupon: Stripe.Coupon}> {
    const product = await findIn(
        stripe.products.list({limit: 100, active: true}) as unknown as AsyncIterable<Stripe.Product>,
    ...
    for (const {nickname, amount, interval} of PRICES) {
        ...
        const found = await findIn(
            stripe.prices.list({product: product.id, limit: 100}) as unknown as AsyncIterable<Stripe.Price>,
            price => price.active
        ...
        prices.push(found ?? await stripe.prices.create({
            product: product.id,
            active: true,
            nickname,
            currency: 'usd',
