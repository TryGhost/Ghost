...
LocalFileStore.prototype.save = function (image, targetDir) {
    ...
    targetDir = targetDir || this.getTargetDir(paths.imagesPath);
    ...
    return this.getUniqueFileName(this, image, targetDir).then(function (filename) {
        ...
        var fullUrl = (paths.subdir) + '/' + paths.imagesRelPath + '/' +
            path.relative(paths.imagesPath, targetFilename).replace(new RegExp('\\' + path.sep, 'g'), '/');
        return fullUrl;
    }).catch(function (e) {
        errors.logError(e);
...
LocalFileStore.prototype.serve = function () {
    ...
    return express['static'](config.get('paths:imagesPath'), {maxAge: utils.ONE_YEAR_MS});
};
