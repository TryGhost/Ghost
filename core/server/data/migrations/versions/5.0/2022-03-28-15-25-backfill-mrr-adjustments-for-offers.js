...
module.exports = createTransactionalMigration(
    async function up(knex) {
        ...
        function calculateMRR(event, redemption) {
            if (redemption && event.interval !== redemption.discount_interval) {
                logging.error('Found invalid price & redemption pair');
                return calculateMRR(event);
            }

            if (!redemption) {
                return event.interval === 'year' ? event.amount / 12 : event.amount;
            }

            if (redemption.discount_type === 'percent') {
                return calculateMRR({
                    interval: event.interval,
                    amount: event.amount * (100 - redemption.discount_amount) / 100
                });
            }

            return calculateMRR({
                interval: event.interval,
                amount: event.amount - redemption.discount_amount
            });
        }
