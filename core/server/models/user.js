    ...
    */
                ...
                return bcryptCompare(object.password, user.get('password')).then(function then(matched) {
                    if (!matched) {
                        return Promise.resolve(self.setWarning(user, {validate: false})).then(function then(remaining) {
                            s = (remaining > 1) ? 's' : '';
                            return Promise.reject(new errors.UnauthorizedError(i18n.t('errors.models.user.incorrectPassword', {remaining: remaining, s: s})));
                            ...
                            return Promise.reject(new errors.UnauthorizedError(i18n.t('errors.models.user..incorrectPassword2')));
                        });
                    }
    ...
    changePassword: function changePassword(object, options) {
        ...
        return self.forge({id: userId}).fetch({require: true}).then(function then(_user) {
            ...
            if (!matched) {
                return Promise.reject(new errors.ValidationError(i18n.t('errors.models.user.incorrectPassword2')));
            }
