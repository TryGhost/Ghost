...
module.exports = {
    ...
    exportCSV: {
        ...
        validation: {},
        async query(frame) {
            const hasFilter = frame.options.limit !== 'all' || frame.options.filter || frame.options.search;
            ...
            if (hasFilter) {
                ...
                frame.options.columns = ['id'];
                pagination = await models.Member.findPage(frame.options);
                ids = pagination.data.map(d => d.id);
            }
            ...
            for (const row of rows) {
                const product_ids = row.products?.split(',') ?? [];
                ...
                const labels = labelIds.map(id => {
                    const label = allLabels.find(product => product.id === id)
                    return {
                        name: label.get('name')
                    }
                })
