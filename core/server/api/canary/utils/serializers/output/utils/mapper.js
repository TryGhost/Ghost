...
const metaSchema = require('../../../../../../data/schema').tables.meta;
...
const mapPost = (model, frame) => {
    ...
    let metaAttrs = _.keys(_.omit(metaSchema, ['id', 'post_id']));
    _(metaAttrs).filter((k) => {
        return (!frame.options.columns || (frame.options.columns && frame.options.columns.includes(k)));
    }).each((attr) => {
        jsonModel[attr] = _.get(jsonModel.meta, attr) || null;
    });
    delete jsonModel.meta;
