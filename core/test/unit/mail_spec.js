...
describe('Mail', function () {
    ...
    it('should fall back to [blog.title] <ghost@[blog.url]> as from address', function () {
        ...
        // Strip Port
        config.set('url', 'http://default.com:2368/');
        config.set(' mail', {from: null});
        config.set('theme', {title: 'Test'});
        mailer.from().should.equal('"Test" <ghost@default.com>');
    });
    ...
    it('should attach blog title if from or fromaddress are only email addresses', function () {
        ...
        config.set('mail', {from: 'from@default.com', fromaddress: 'fa@default.com'});
        config.set('theme', {title: 'Test'});
        mailer.from().should.equal('"Test" <from@default.com>');

        // only from set
        config.set('mail', {from: 'from@default.com', fromaddress: null});
        config.set('theme', {title: 'Test'});
        mailer.from().should.equal('"Test" <from@default.com>');

        // only fromaddress set
        config.set('mail', {from: null, fromaddress: 'fa@default.com'});
        config.set('theme', {title: 'Test'});
    ...
    it('should ignore theme title if from address is Title <email@address.com> format', function () {
        // from and fromaddress are both set
        config.set('mail', {from: '"R2D2" <from@default.com>', fromaddress: '"C3PO" <fa@default.com>'});
        config.set('theme', {title: 'Test'});
        mailer.from().should.equal('"R2D2" <from@default.com>');

        // only from set
        config.set('mail', {from: '"R2D2" <from@default.com>', fromaddress: null});
        config.set('theme', {title: 'Test'});
        mailer.from().should.equal('"R2D2" <from@default.com>');

        // only fromaddress set
        config.set('mail', {from: null, fromaddress: '"C3PO" <fa@default.com>'});
        config.set('theme', {title: 'Test'});
