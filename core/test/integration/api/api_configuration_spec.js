...
describe('Configuration API', function () {
    ...
    it('can read basic config and get all expected properties', function (done) {
        ConfigurationAPI.read().then(function (response) {
            ...
            props.should.have.property('newsletter').which.is.an.Object().with.properties('type', 'value');

            // Check a few values
            props.blogUrl.should.have.property('value', 'http://127.0.0.1:2369');
            props.fileStorage.should.have.property('value', true);

            JSON.parse(props.newsletter.value).status.should.eql('disabled');
            should.not.exist(JSON.parse(props.newsletter.value).rrule);
            should.not.exist(JSON.parse(props.newsletter.value).lastExecutedAt);
