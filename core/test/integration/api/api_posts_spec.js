...
describe('Post API', function () {
    ...
    beforeEach(function () {
        return Promise.mapSeries(testUtils.DataGenerator.forKnex.posts, function (post) {
            return models.Post.add(post, {context: {internal:true}});
        });
    });

    beforeEach(function () {
        return Promise.mapSeries(testUtils.DataGenerator.forKnex.tags, function (tag) {
            return models.Tag.add(tag, {context: {internal:true}});
        });
    });

    beforeEach(function () {
        return db.knex('posts_tags').insert(testUtils.DataGenerator.forKnex.posts_tags);
    });
    ...
    describe('Browse', function () {
        ...
        it('can fetch featured posts for user 1', function () {
            return PostAPI.browse({context: {user: 1}, filter: 'featured:true'}).then(function (results) {
                should.exist(results.posts);
        ...
        it('can fetch featured posts for user 2', function () {
            return PostAPI.browse({context: {user: 2}, filter: 'featured:true'}).then(function (results) {
                should.exist(results.posts);
                results.posts.length.should.eql(4);
                results.posts[0].featured.should.eql(true);
            });
        ...
        it('can exclude featured posts for user 1', function () {
            return PostAPI.browse({context: {user: 1}, status: 'all', filter: 'featured:false'}).then(function (results) {
                should.exist(results.posts);
        ...
        it('can include author', function () {
        ...
        it('can fetch all posts for a tag', function () {
            return PostAPI.browse({context: {user: 1}, status: 'all', filter: 'tags:kitchen-sink', include: 'tags'}).then(function (results) {
        ...
        it('can include author and be case insensitive', function () {
            return PostAPI.browse({context: {user: 1}, status: 'all', include: 'Author'}).then(function (results) {
                should.exist(results.posts);
                should.exist(results.posts[0].author.name);
                results.posts[0].author.name.should.eql('Joe Bloggs');
        ...
        it('can include author and ignore space in include', function () {
            return PostAPI.browse({context: {user: 1}, status: 'all', include: ' author'}).then(function (results) {
                should.exist(results.posts);
                should.exist(results.posts[0].author.name);
                results.posts[0].author.name.should.eql('Joe Bloggs');
        ...
        it('can fetch all posts for an author', function () {
            return PostAPI.browse({context: {user: 1}, status: 'all', filter: 'author:joe-bloggs', include: 'author'}).then(function (results) {
                should.exist(results.posts);
                results.posts.length.should.eql(6);
        ...
        it.skip('cannot fetch all posts for an author with invalid slug', function () {
            return PostAPI.browse({filter: 'author:invalid!'}).then(function () {
        ...
        it('with context.user can fetch multiple fields', function () {
            return PostAPI.browse({context: {user: 1}, status: 'all', limit: 5, fields: 'slug,published_at'}).then(function (results) {
                should.exist(results.posts);
        ...
        it('with context.user can fetch a field and not return invalid field', function () {
            return PostAPI.browse({context: {user: 1}, status: 'all', limit: 5, fields: 'foo,title'}).then(function (results) {
                var objectKeys;
                should.exist(results.posts);
        ...
        it('can order posts using asc', function () {
            ...
            return PostAPI.browse({context: {user: 1}, status: 'all', order: 'title asc', fields: 'title'}).then(function (results) {
                should.exist(results.posts);
        ...
        it('can order posts using desc', function () {
            ...
            return PostAPI.browse({context: {user: 1}, status: 'all', order: 'title DESC', fields: 'title'}).then(function (results) {
                should.exist(results.posts);
        ...
        it('can order posts and filter disallowed attributes', function () {
            ...
            return PostAPI.browse({context: {user: 1}, status: 'all', order: 'bunny DESC, title ASC', fields: 'title'}).then(function (results) {
                should.exist(results.posts);
    ...
    describe('Read', function () {
        ...
        it('can fetch post with by id', function () {
            return PostAPI.read({context: {user: 1}, id: 2, status: 'all'}).then(function (results) {
                should.exist(results.posts);
        ...
        it('can include tags', function () {
            return PostAPI.read({context: {user: 1}, id: 3, include: 'tags'}).then(function (results) {
        ...
        it('can include author', function () {
            return PostAPI.read({context: {user: 1}, id: 2, include: 'author'}).then(function (results) {
                should.exist(results.posts[0].author.name);
                results.posts[0].author.name.should.eql('Joe Bloggs');
            });
        });

        it('can include next post', function () {
            return PostAPI.read({context: {user: 1}, id: 3, include: 'next'}).then(function (results) {
                should.exist(results.posts[0].next.slug);
                results.posts[0].next.slug.should.eql('not-so-short-bit-complex');
            });
        });

        it('can include next post with author and tags', function () {
            return PostAPI.read({context: {user: 1}, id: 3, include: 'next,next.tags,next.author'}).then(function (results) {
                should.exist(results.posts[0].next.slug);
                results.posts[0].next.slug.should.eql('not-so-short-bit-complex');
        ...
        it('can include next post with just tags', function () {
            return PostAPI.read({context: {user: 1}, id: 2, include: 'next,next.tags'}).then(function (results) {
                should.exist(results.posts[0].next.slug);
        ...
        it('can include previous post', function () {
            return PostAPI.read({context: {user: 1}, id: 3, include: 'previous'}).then(function (results) {
                should.exist(results.posts[0].previous.slug);
                results.posts[0].previous.slug.should.eql('ghostly-kitchen-sink');
            });
        });

        it('can include previous post with author and tags', function () {
            return PostAPI.read({context: {user: 1}, id: 3, include: 'previous,previous.author,previous.tags'}).then(function (results) {
                should.exist(results.posts[0].previous.slug);
                results.posts[0].previous.slug.should.eql('ghostly-kitchen-sink');
        ...
        it('can include previous post with just author', function () {
            return PostAPI.read({context: {user: 1}, id: 3, include: 'previous,previous.author'}).then(function (results) {
                should.exist(results.posts[0].previous.slug);
    ...
    describe('Edit', function () {
        it('can edit own post', function () {
            return PostAPI.edit({posts:[{status: 'test'}]}, {context: {user: 1}, id: 1}).then(function (results) {
                should.exist(results.posts);
            });
        });

        it('cannot edit others post', function () {
            return testUtils.fixtures.insertOne('users', 'createUser', 4).then(function (result) {
                return PostAPI.edit({posts: [{status: 'test'}]}, {context: {user: result[0]}, id: 1}).catch(function (err) {
                    should.exist(err);
        ...
        it('should update post & not delete tags with `tags` not included', function () {
            var options = {context: {user: 1}, id: 1},
                includeOptions = {include: 'tags'},
                startTags;

            // Step 1, fetch a post from the API with tags
            return PostAPI.read(_.extend({}, options, includeOptions)).then(function (results) {
        ...
        it('should update post & not delete tags with `tags` set to undefined', function () {
            var options = {context: {user: 1}, id: 1},
                includeOptions = {include: 'tags'},
                startTags;
        ...
        it('should update post & not delete tags with `tags` set to null', function () {
            var options = {context: {user: 1}, id: 1},
                includeOptions = {include: 'tags'},
                startTags;
        ...
        it('should update post & should delete tags with `tags` set to []', function () {
            var options = {context: {user: 1}, id: 1},
                includeOptions = {include: 'tags'};
