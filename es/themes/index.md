---
layout: themes
meta_title: Cómo crear temas para Ghost - Documentación de Ghost
meta_description: Una guía detallada para crear temas para la plataforma de blog Ghost. Todo lo que necesitas saber para crear temas para Ghost.
heading: Temas para Ghost
subheading: Crea tus propios temas para Ghost
chapter: themes
---

{% raw %}

## Cambia el Tema <a id="switching-theme"></a>

Los temas de Ghost se encuentran en <code class="path">content/themes/</code>

Si quieres usar un tema diferente al predeterminado (Casper), visita el [marketplace gallery](http://marketplace.ghost.org/) donde se encuentra una variedad de temas personalizados. Descarga el tema que te guste, descomprime el archivo y copialo a <code class="path">content/themes</code> junto a Casper.

Si quieres crear tu propio tema, recomendamos copiar el directorio casper, cambia el nombre y edita las plantillas a tu estilo y preferencia.

Para cambiar al tema que acabas de agregar:

1.  Reinicia Ghost. Por ahora, Ghost no se dará cuenta que agregaste una carpeta a <code class="path">content/themes</code> por lo tanto, lo debes reiniciar primero
2.  Ingresa a tu administración de Ghost y navega a <code class="path">/ghost/settings/general/</code>
3.  Selecciona el nombre de tu tema en menu 'Theme'
4.  Haz click a 'Save'
5.  Visita el frontend de tu blog y con ve con maravilla tu nuevo tema

<p class="note">**Nota:** Si usas Ghost Pro, y no Ghost descargado a tu propio servidor o hosting, para cambiar tema, debes ir a tu página <a href="https://ghost.org/blogs/">blog management</a> y haz click a "edit" al lado del nombre de tu blog.</p>


##  ¿Qué es Handlebars? <a id="what-is-handlebars"></a>

[Handlebars](http://handlebarsjs.com/) es el lenguaje para crear plantillas en Ghost.

> Handlebars ofrece el poder necesario para construir plantillas semánticas efectivamente sin frustraciones.

Si buscas empezar a crear tu propio tema, es probable que primero quieras familiarizarte con la sintaxis de Handlebars. Lee la [documentación de handlebars documentation](http://handlebarsjs.com/expressions.html), o ve este [tutorial de Treehouse](http://blog.teamtreehouse.com/getting-started-with-handlebars-js) – puedes saltar la primera sección sobre instalación y uso (ya lo hicimos) y empieza con ‘Basic Expressions’.

Ghost también usa una biblioteca adicional llamada `express-hbs` que añade [funciones adicionales](https://github.com/barc/express-hbs#syntax) a Handlebars el cual Ghost usa bastante, como [layouts](#default-layout) y [partials](#partials).

## Sobre los temas de Ghost <a id="about"></a>

Los temas de Ghost tienen por objeto ser fáciles de desarrollar y mantener. Proponen una significante separación entre las plantillas (el HTML) y cualquier lógica (JavaScript). Handlebars no tiene (casi) lógica y propone esta separación, usando un mecanismo útil para que la lógica para visualizar contenido quede separada y autónoma. Esta separación ayuda a facilitar la colaboración entre los diseñadores y desarrolladores al crear temas.

Las plantillas Handlebars son jerárquicas (una plantilla puede extender otra plantilla) y también apoyan las plantillas parciales. Ghost utiliza estas características para reducir la duplicación de código y mantener las plantillas individuales principalmente para hacer una sola cosa, y hacerla bien. Un tema bien estructurado será fácil de mantener y conservando los componentes separadamente los hace más fácil de reutilizar entre varios temas.
