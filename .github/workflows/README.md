...
- **Never pass a secret to a local action (`uses: ./...`) on a `pull_request` run.** The workflow file comes from the base branch, but a local action's definition is loaded from the checked-out workspace, so a PR authors the code that receives the input. A guard _inside_ the action is too late — the secret is already an input. Gate it at the caller:
