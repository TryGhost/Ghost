...
const REVIEW_TOOL = {
  ...
  description:
    'Submit the translation review with per-line inline comments and an overall verdict.',
  input_schema: {
    type: 'object',
    properties: {
      verdict: {
        ...
        enum: ['ok', 'questions'],
        description:
          '"ok" if you have no concerns. "questions" if you have at least one comment to raise.',
      },
      overall: {
        type: 'string',
        description:
          'A short, warm one-paragraph note to the translator. Do not repeat anything raised as an inline comment. Finish with "Thank you!" translated into the target language.',
      },
      comments: {
        type: 'array',
...
export async function analyzePR(
  prNumber,
  { octokit, anthropic, owner, repo, model = DEFAULT_MODEL },
) {
  ...
  if (i18nFiles.length > MAX_FILES) {
    console.log(
      `PR exceeds file cap (files=${i18nFiles.length}/${MAX_FILES}). Skipping model call.`,
    );
  ...
  if (validKeys.size > MAX_ADDED_LINES) {
    console.log(
      `PR exceeds line cap (lines=${validKeys.size}/${MAX_ADDED_LINES}). Skipping model call.`,
    );
    return buildSkippedReview({
      pr,
      filesCount: eligibleFiles.length,
      linesCount: validKeys.size,
      reason: 'lines',
    });
  }

  // Both caps passed — now make the network calls we need for the review.
  const contextJson = await fetchFile(
    octokit,
    owner,
    repo,
  ...
  const toolUse = response.content.find(
    (c) => c.type === 'tool_use' && c.name === 'post_translation_review',
  );
...
function buildUserMessage(pr, contextJson, fileChanges) {
  ...
  if (contextJson) {
    sections.push(
      `Translator context (\`packages/i18n/locales/context.json\`):\n\`\`\`json\n${contextJson}\n\`\`\``,
    );
  }
  for (const fc of fileChanges) {
    ...
    if (fc.currentContent) {
      parts.push(
        `Current file content (for tone/consistency context):\n\`\`\`json\n${fc.currentContent}\n\`\`\``,
      );
    }
  ...
  sections.push(
    'When you call `post_translation_review`, reference each inline comment by its exact `filename` and `position` from the list above.',
  );
