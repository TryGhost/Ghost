...
describe('ci-plan', () => {
    ...
    it('runs comprehensive product validation for CI workflow changes', () => {
        const outputs = plan({
        ...
        assert.strictEqual(outputs.changed_ci, 'true');
        assert.strictEqual(outputs.changed_core, 'true');
        assert.strictEqual(outputs.run_e2e, 'true');
        assert.strictEqual(outputs.run_coverage, 'false');
        assert.notDeepStrictEqual(includeMatrix(outputs.unit_test_matrix), []);
        assert.notDeepStrictEqual(includeMatrix(outputs.core_db_matrix), []);
        assert.notDeepStrictEqual(includeMatrix(outputs.app_acceptance_matrix), []);
        assert.notDeepStrictEqual(includeMatrix(outputs.e2e_matrix), []);
    });
});
