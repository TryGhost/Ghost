...
Scheduled and offloaded jobs registered through the legacy Bree-based service
run in worker threads, so they must initialize their own dependencies and
cannot rely on the main Ghost process's memory. Jobs migrated to the
class-based service (token cleanup, gift cleanup, gift reminders, update
checks, media inlining, content imports, webmentions, newsletter sending) run
...
scheduling service from boot. Handlers should only route the rehydrated
payload to an initialized service method. The queue options below are part of
this API.
...
- Site and member imports, which run as inline jobs.
