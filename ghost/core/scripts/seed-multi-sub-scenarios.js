...
const SCENARIOS = [
  {
    ...
    subs: [
      ...
      {
        id: 'sarah_cancelled_recent',
        status: 'canceled',
        daysAgo: 7,
        tier: 'default-product',
        mrr: 0,
      },
    ],
...
async function ensureProductAndPrice() {
  ...
  if (!stripeProduct) {
    throw new Error(
      `Could not find a Stripe product for ${product.slug}. Run \`pnpm reset:data\` first.`,
    );
  }
...
async function deleteExistingScenarioMembers() {
  ...
  await knex('members_stripe_customers_subscriptions')
    .whereIn(
      'customer_id',
      knex('members_stripe_customers').whereIn('member_id', ids).select('customer_id'),
    )
    .del();
...
async function seedScenario(scenario, { stripePrice }) {
  ...
  await knex('members').insert({
    ...
    status: scenario.subs.some((s) => ACTIVE_STATUSES.includes(s.status))
      ? 'paid'
      : scenario.subs.length > 0
        ? 'free'
        : 'comped',
    created_at: iso(now),
    updated_at: iso(now),
...
async function summarise() {
  ...
  for (const scenario of SCENARIOS) {
    ...
    const lookup = tableExists
      ...
      : null;
    const resolvedSub = lookup
      ? await knex('members_stripe_customers_subscriptions')
          .where({ id: lookup.subscription_id })
          .first()
      : null;
...
async function main() {
  ...
  console.log(
    `Using stripe price: ${stripePrice.stripe_price_id} (${stripePrice.nickname || 'Monthly'})`,
  );
