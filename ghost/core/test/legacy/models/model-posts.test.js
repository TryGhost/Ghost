  ...
  describe('Single author posts', function () {
    ...
    describe('edit', function () {
      ...
      it('[failure] multiple edits in one transaction', function () {
        ...
        return models.Base.transaction(function (txn) {
          ...
          return models.Post.edit(
            data,
            _.merge({ id: testUtils.DataGenerator.Content.posts[3].id }, options),
          )
            .then(function () {
              return models.Post.edit(
                data,
                _.merge({ id: testUtils.DataGenerator.Content.posts[5].id }, options),
              );
            })
            .then(function () {
              // force rollback
      ...
      it('draft -> scheduled: expect update of published_at', async function () {
        ...
        assert.equal(
          moment(edited.attributes.published_at)
            .startOf('seconds')
    ...
    describe('add', function () {
      ...
      it('can add, defaults are all correct', async function () {
        ...
        assert.equal(
          createdPost.relations.authors.models[0].id,
        ...
        assert.equal(
          publishedPost.get('published_by'),
          testUtils.DataGenerator.Content.users[0].id,
        );
      ...
      it('can add, with previous published_at date', async function () {
        ...
        assert.equal(
          new Date(newPost.get('published_at')).getTime(),
          previousPublishedAtDate.getTime(),
        );
      ...
      it('add multiple authors', async function () {
        ...
        assertExists(newPost);
        assert.equal(
          newPost.toJSON().primary_author.id,
      ...
      it('it stores urls as transform-ready and reads as absolute', async function () {
        ...
        assert.equal(
          createdPost.get('html'),
          `<p><a href="${siteUrl}/test">Testing</a></p><figure class="kg-card kg-image-card"><img src="${siteUrl}/content/images/card.jpg" class="kg-image" alt="" loading="lazy"></figure>`,
        );
        assert(createdPost.get('plaintext').includes('Testing'));
        assert.equal(
          createdPost.get('custom_excerpt'),
          `Testing <a href="${siteUrl}/internal">links</a> in custom excerpts`,
        );
        assert.equal(
          createdPost.get('codeinjection_head'),
          `<script src="${siteUrl}/assets/head.js"></script>`,
        );
        assert.equal(
          createdPost.get('codeinjection_foot'),
          `<script src="${siteUrl}/assets/foot.js"></script>`,
        );
        ...
        assert.equal(
          ...
          `${siteUrl.replace('http:', 'https:')}/https-internal`,
        );
        assert.equal(
          updatedPost.get('feature_image'),
          `${siteUrl}/content/images/updated_feature.png`,
        );
        ...
        assert.equal(
          knexPost.html,
          '<p><a href="__GHOST_URL__/test">Testing</a></p><figure class="kg-card kg-image-card"><img src="__GHOST_URL__/content/images/card.jpg" class="kg-image" alt="" loading="lazy"></figure>',
        );
        assert(knexPost.plaintext.includes('Testing'));
        assert.equal(
          knexPost.custom_excerpt,
          'Testing <a href="__GHOST_URL__/internal">links</a> in custom excerpts',
        );
        assert.equal(
          knexPost.codeinjection_head,
          '<script src="__GHOST_URL__/assets/head.js"></script>',
        );
        assert.equal(
          knexPost.codeinjection_foot,
          '<script src="__GHOST_URL__/assets/foot.js"></script>',
        );
        assert.equal(knexPost.feature_image, '__GHOST_URL__/content/images/updated_feature.png');
        assert.equal(
          knexPost.canonical_url,
          `${siteUrl.replace('http:', 'https:')}/https-internal`,
        );
      });
      ...
      it('stores lexical as transform-ready and reads as absolute', async function () {
        ...
        assert.equal(
          createdPost.get('lexical'),
          `{"root":{"children":[{"type":"image","src":"${siteUrl}/content/images/card.jpg"},{"children":[{"children":[{"detail":0,"format":0,"mode":"normal","style":"","text":"local link","type":"text","version":1}],"direction":"ltr","format":"","indent":0,"type":"link","version":1,"rel":null,"target":null,"url":"${siteUrl}/local"}],"direction":"ltr","format":"","indent":0,"type":"paragraph","version":1},{"children":[{"children":[{"detail":0,"format":0,"mode":"normal","style":"","text":"external link","type":"text","version":1}],"direction":"ltr","format":"","indent":0,"type":"link","version":1,"rel":null,"target":null,"url":"https://example.com/external"}],"direction":"ltr","format":"","indent":0,"type":"paragraph","version":1}],"direction":"ltr","format":"","indent":0,"type":"root","version":1}}`,
          'Post.add result',
        );
      ...
      describe('URL transformations without CDN config', function () {
        ...
        describe('Mobiledoc', function () {
          ...
          it('transforms all media card URLs to absolute site URLs', function () {
            // Image card
            const imageCard = mobiledoc.cards.find(
              (card) => card[0] === 'image' && card[1].src.includes('inline.jpg'),
            );
            ...
            // File card
            const fileCard = mobiledoc.cards.find(
              (card) => card[0] === 'file' && card[1].src.includes('document.pdf'),
            );
            assert.equal(fileCard[1].src, `${siteUrl}/content/files/document.pdf`);

            // Video card
            const videoCard = mobiledoc.cards.find(
              (card) =>
                card[0] === 'video' &&
                card[1].src.includes('video.mp4') &&
                !card[1].src.includes('snippet'),
            );
            ...
            // Audio card
            const audioCard = mobiledoc.cards.find(
              (card) =>
                card[0] === 'audio' &&
                card[1].src.includes('audio.mp3') &&
                !card[1].src.includes('snippet'),
            );
          ...
          it('transforms inserted snippet URLs to absolute site URLs', function () {
            // Snippet image
            const snippetImage = mobiledoc.cards.find(
              (card) => card[0] === 'image' && card[1].src.includes('snippet-inline'),
            );
            assert.equal(snippetImage[1].src, `${siteUrl}/content/images/snippet-inline.jpg`);

            // Snippet file
            const snippetFile = mobiledoc.cards.find(
              (card) => card[0] === 'file' && card[1].src.includes('snippet-document'),
            );
            assert.equal(snippetFile[1].src, `${siteUrl}/content/files/snippet-document.pdf`);

            // Snippet video
            const snippetVideo = mobiledoc.cards.find(
              (card) => card[0] === 'video' && card[1].src.includes('snippet-video'),
            );
            assert.equal(snippetVideo[1].src, `${siteUrl}/content/media/snippet-video.mp4`);
            assert.equal(
              snippetVideo[1].thumbnailSrc,
              `${siteUrl}/content/images/snippet-video-thumb.jpg`,
            );

            // Snippet audio
            const snippetAudio = mobiledoc.cards.find(
              (card) => card[0] === 'audio' && card[1].src.includes('snippet-audio'),
            );
            assert.equal(snippetAudio[1].src, `${siteUrl}/content/media/snippet-audio.mp3`);
            assert.equal(
              snippetAudio[1].thumbnailSrc,
    ...
    describe('destroy', function () {
      ...
      it('published post', async function () {
        ...
        const postsTags = await ghostBookshelf.knex
          .select()
          .table('posts_tags')
          .where('post_id', firstItemData.id);
    ...
    it("can't edit dates and authors of existing tag", function () {
      ...
      return new Promise((resolve) => {
        ...
        .then(function (updatedPost) {
          ...
          assert.equal(
            updatedAtFormat,
            moment(postJSON.tags[0].updated_at).format('YYYY-MM-DD HH:mm:ss'),
          );
          assert.notEqual(
            updatedAtFormat,
            moment(newJSON.tags[0].updated_at).format('YYYY-MM-DD HH:mm:ss'),
          );

          createdAtFormat = moment(updatedPost.tags[0].created_at).format('YYYY-MM-DD HH:mm:ss');
          assert.equal(
            createdAtFormat,
            moment(postJSON.tags[0].created_at).format('YYYY-MM-DD HH:mm:ss'),
          );
          assert.notEqual(
            createdAtFormat,
            moment(newJSON.tags[0].created_at).format('YYYY-MM-DD HH:mm:ss'),
          );
        });
    });
