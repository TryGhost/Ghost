...
describe('Members Importer API', function () {
  ...
  it('Can import CSV with minimum one field and labels', function () {
    ...
    return request
      ...
      .then((res) => {
        ...
        const importedMember1 = jsonResponse.members.find(
          (m) => m.email === 'member+labels_1@example.com',
        );
        ...
        const importedMember2 = jsonResponse.members.find(
          (m) => m.email === 'member+labels_2@example.com',
        );
  ...
  it('Can import CSV with mapped fields', function () {
    return request
      ...
      .attach(
        'membersfile',
        path.join(__dirname, '/../../../utils/fixtures/csv/members-with-mappings.csv'),
      )
      .set('Origin', config.get('url'))
      .expect('Content-Type', /json/)
      .expect('Cache-Control', testUtils.cacheRules.private)
        ...
        return request
          .get(
            localUtils.API.getApiQuery(
              `members/?search=${encodeURIComponent('member+mapped_1@example.com')}`,
            ),
          )
          .set('Origin', config.get('url'))
          .expect('Content-Type', /json/)
  ...
  it('Can import CSV with labels and provide additional labels', function () {
    return request
      ...
      .attach(
        'membersfile',
        path.join(__dirname, '/../../../utils/fixtures/csv/valid-members-defaults.csv'),
      )
      .set('Origin', config.get('url'))
      .expect('Content-Type', /json/)
      .expect('Cache-Control', testUtils.cacheRules.private)
      ...
      .then((res) => {
        ...
        const defaultMember1 = jsonResponse.members.find(
          (member) => member.email === 'member+defaults_1@example.com',
        );
        ...
        const defaultMember2 = jsonResponse.members.find(
          (member) => member.email === 'member+defaults_2@example.com',
        );
  ...
  it('Runs imports with stripe_customer_id as background job', function () {
    return request
      ...
      .attach(
        'membersfile',
        path.join(__dirname, '/../../../utils/fixtures/csv/members-with-stripe-ids.csv'),
      )
      .set('Origin', config.get('url'))
      .expect('Content-Type', /json/)
      .expect('Cache-Control', testUtils.cacheRules.private)
  ...
  it('Fails to import member with invalid values', function () {
    return request
      ...
      .attach(
        'membersfile',
        path.join(__dirname, '/../../../utils/fixtures/csv/members-invalid-values.csv'),
      )
      .set('Origin', config.get('url'))
      .expect('Content-Type', /json/)
      .expect('Cache-Control', testUtils.cacheRules.private)
  ...
  it('Can import members with host emailVerification limits', async function () {
    try {
      ...
      const res = await request
        ...
        .attach(
          'membersfile',
          path.join(__dirname, '/../../../utils/fixtures/csv/valid-members-import-large-501.csv'),
        )
        .set('Origin', config.get('url'))
        .expect('Content-Type', /json/)
        .expect('Cache-Control', testUtils.cacheRules.private)
      ...
      assert.equal(
        settingsCache.get('email_verification_required'),
        true,
        'Email verification should now be required',
      );
      ...
      const secondImport = await request
        ...
        .attach(
          'membersfile',
          path.join(__dirname, '/../../../utils/fixtures/csv/valid-members-import-large.csv'),
        )
        .set('Origin', config.get('url'))
        .expect('Content-Type', /json/)
        .expect('Cache-Control', testUtils.cacheRules.private)
      ...
      assert.equal(
        settingsCache.get('email_verification_required'),
        true,
        'Email verification should still be required',
      );

      // Don't send another email
      assert.equal(
        receivedWebhookRequests.length,
        1,
        'Expected no further webhook requests after second import',
      );
    } finally {
