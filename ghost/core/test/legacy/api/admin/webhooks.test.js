...
describe('Webhooks API', function () {
  ...
  it('Can create a webhook using integration', function () {
    ...
    return request
      .post(localUtils.API.getApiQuery('webhooks/'))
      .set(
        'Authorization',
        `Ghost ${localUtils.getValidAdminToken('/admin/', testUtils.DataGenerator.Content.api_keys[0])}`,
      )
      ...
      .then((res) => {
        ...
        assert.equal(
          jsonResponse.webhooks[0].target_url,
          'http://example.com/webhooks/test/extra/canary',
        );
        assert.equal(
          jsonResponse.webhooks[0].integration_id,
          testUtils.DataGenerator.Content.api_keys[0].integration_id,
        );
  ...
  it("Integration cannot edit or delete other integration's webhook", function () {
    ...
    return Promise.resolve()
      ...
      .then(({ body }) => {
        ...
        return request
          .put(localUtils.API.getApiQuery(`webhooks/${createdWebhook.id}/`))
          .set(
            'Authorization',
            `Ghost ${localUtils.getValidAdminToken('/admin/', testUtils.DataGenerator.Content.api_keys[0])}`,
          )
      ...
      .then(() => {
        return request
          .del(localUtils.API.getApiQuery(`webhooks/${createdWebhook.id}/`))
          .set(
            'Authorization',
            `Ghost ${localUtils.getValidAdminToken('/admin/', testUtils.DataGenerator.Content.api_keys[0])}`,
          )
  ...
  it('Integration editing non-existing webhook returns 404', function () {
    return request
      .put(localUtils.API.getApiQuery(`webhooks/5f27d0287c75da744d8615da/`))
      .set(
        'Authorization',
        `Ghost ${localUtils.getValidAdminToken('/admin/', testUtils.DataGenerator.Content.api_keys[0])}`,
      )
  ...
  it('Integration deleting non-existing webhook returns 404', function () {
    return request
      .delete(localUtils.API.getApiQuery(`webhooks/5f27d0287c75da744d8615db/`))
      .set(
        'Authorization',
        `Ghost ${localUtils.getValidAdminToken('/admin/', testUtils.DataGenerator.Content.api_keys[0])}`,
      )
  ...
  it('Cannot edit webhooks using content api keys', function () {
    ...
    return request
      ...
      .set(
        'Authorization',
        `Ghost ${localUtils.getValidAdminToken('/admin/', testUtils.DataGenerator.Content.api_keys[1])}`,
      )
      .send({ webhooks: [webhookData] })
      .expect('Content-Type', /json/)
      .expect('Cache-Control', testUtils.cacheRules.private)
