...
describe('Authentication API', function () {
  ...
  describe('Blog setup', function () {
    ...
    it('complete setup', async function () {
      ...
      assert.equal(
        description,
        'Custom Site Description on Setup &mdash; great for everyone',
        'The site description should have been set',
      );
      ...
      assert(
        tierWithDefaultProductName === undefined,
        'The default Tier should have had a name change',
      );
      ...
      const defaultNewsletter = newsletterBody.newsletters.find(
        (x) => x.slug === 'default-newsletter',
      );
      const newsletterWithDefaultName = newsletterBody.newsletters.find(
        (x) => x.name === 'Default Newsletter',
      );

      assert(
        defaultNewsletter.name === 'a test blog',
        'The default newsletter should have had a name change',
      );

      assert(
        newsletterWithDefaultName === undefined,
        'The default newsletter should have had a name change',
      );
    });

    it('is setup? yes', async function () {
      await agent.get('authentication/setup').matchBodySnapshot().matchHeaderSnapshot({
        'content-version': anyContentVersion,
        etag: anyEtag,
      });
    });
    ...
    it('only returns status when setup is complete', async function () {
      const { body } = await agent.get('authentication/setup').expectStatus(200);
    ...
    it('only returns status when setup is complete even with config values set', async function () {
      ...
      try {
        ...
        assert.equal(
          setup.email,
          undefined,
          'Email should not be returned after setup is complete',
        );
        assert.equal(setup.name, undefined, 'Name should not be returned after setup is complete');
      } finally {
  ...
  describe('Reset authentication', function () {
    ...
    it('rotates every api key, locks every user, kills every session, and sends no proactive emails', async function () {
      ...
      for (const key of apiKeysAfter) {
        assert.notEqual(
          key.secret,
          apiKeysBefore[key.id],
          `Secret for key ${key.id} should have changed`,
        );
      }
      ...
      for (const user of users) {
        assert.equal(
          user.get('status'),
          'locked',
          `Status should be locked for user ${user.get('email')}`,
        );
      }
