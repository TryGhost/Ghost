    ...
    describe('Ensure re-registering redirects works', function () {
      it('no redirects file exists', function () {
        ...
        return startGhost({
          ...
          .then(() => {
            return request.get('/my-old-blog-post/').expect(404);
          })
          .then(() => {
            // Provide a redirects file in the root directory of the content test folder
            fs.writeFileSync(
              path.join(config.get('paths:contentPath'), 'redirects-init.json'),
              JSON.stringify([
                {
                  from: 'k',
                  to: 'l',
          ...
          .then((res) => {
            ...
            return request.get('/k/').expect(302);
          })
          .then((response) => {
            ...
            const fileContent = fs.readFileSync(
              path.join(contentFolder, 'data', 'redirects.json'),
              'utf-8',
            );
            assert.equal(
              fileContent,
          ...
          .then(() => {
            ...
            fs.writeFileSync(
              path.join(config.get('paths:contentPath'), 'redirects.yaml'),
              '302:\n  c: d',
            );
          })
          .then(() => {
          ...
          .then(() => {
            return request.get('/c/').expect(302);
          })
          .then((response) => {
            ...
            assert.deepEqual(JSON.parse(fileContent), [{ from: 'c', to: 'd', permanent: false }]);
