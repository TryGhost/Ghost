...
describe('Posts API', function () {
  ...
  describe('Browse', function () {
    it('fields & formats combined', async function () {
      await request
        .get(localUtils.API.getApiQuery('posts/?formats=mobiledoc,html&fields=id,title'))
      ...
      await request
        .get(
          localUtils.API.getApiQuery(
            'posts/?formats=mobiledoc,html,plaintext&fields=id,title,primary_tag,doesnotexist&include=authors,tags,email',
          ),
        )
        .set('Origin', config.get('url'))
        .expect('Content-Type', /json/)
    ...
    it('can filter by fields coming from posts_meta table non null meta_description', async function () {
      await request
        .get(localUtils.API.getApiQuery(`posts/?filter=meta_description:-null`))
        .set('Origin', config.get('url'))
        .expect('Content-Type', /json/)
        .expect('Cache-Control', testUtils.cacheRules.private)
        .expect(200)
    ...
    it('can filter by fields coming from posts_meta table by value', async function () {
      await request
        .get(
          localUtils.API.getApiQuery(
            `posts/?filter=meta_description:'meta description for short and sweet'`,
          ),
        )
        .set('Origin', config.get('url'))
        .expect('Content-Type', /json/)
        ...
        .expect(function (res) {
          ...
          assert.equal(
            jsonResponse.posts[0].meta_description,
            'meta description for short and sweet',
          );
    ...
    it('can order by fields coming from posts_meta table', async function () {
      await request
        .get(localUtils.API.getApiQuery('posts/?order=meta_description%20ASC'))
        .set('Origin', config.get('url'))
        .expect('Content-Type', /json/)
        .expect('Cache-Control', testUtils.cacheRules.private)
        .expect(200)
        .expect(function (res) {
          ...
          assert.equal(
            jsonResponse.posts[14].meta_description,
            'meta description for short and sweet',
          );
    ...
    it('can order by email open rate', async function () {
      ...
      await request
        .get(localUtils.API.getApiQuery('posts/?order=email.open_rate%20DESC'))
        .set('Origin', config.get('url'))
        .expect('Content-Type', /json/)
        .expect('Cache-Control', testUtils.cacheRules.private)
        .expect(200)
      ...
      await request
        .get(localUtils.API.getApiQuery('posts/?order=email.open_rate%20ASC'))
        .set('Origin', config.get('url'))
        .expect('Content-Type', /json/)
        .expect('Cache-Control', testUtils.cacheRules.private)
        .expect(200)
  ...
  describe('Read', function () {
    it("can't retrieve non existent post", async function () {
      await request
        .get(localUtils.API.getApiQuery(`posts/${ObjectId().toHexString()}/`))
        .set('Origin', config.get('url'))
    ...
    it('throws a 400 when a non-existing field is requested', async function () {
      await request
        .get(
          localUtils.API.getApiQuery(
            `posts/slug/${testUtils.DataGenerator.Content.posts[0].slug}/?fields=tags`,
          ),
        )
        .set('Origin', config.get('url'))
        .expect('Content-Type', /json/)
  ...
  describe('Edit', function () {
      ...
      return request
        ...
        .then((res) => {
          return request
            .put(
              localUtils.API.getApiQuery(
                'posts/' + testUtils.DataGenerator.Content.posts[0].id + '/',
              ),
            )
            .set('Origin', config.get('url'))
            .send({
    ...
    it('publishes a post with email_only and sends email to all', async function () {
      ...
      const publishedRes = await request
        .put(
          localUtils.API.getApiQuery(
            `posts/${res.body.posts[0].id}/?newsletter=${defaultNewsletterSlug}`,
          ),
        )
        .set('Origin', config.get('url'))
        .send({
    ...
    it('publishes a post while setting email_only flag sends an email to paid', async function () {
      ...
      const publishedRes = await request
        .put(
          localUtils.API.getApiQuery(
            `posts/${res.body.posts[0].id}/?email_segment=status:-free&newsletter=${defaultNewsletterSlug}`,
          ),
        )
        .set('Origin', config.get('url'))
        .send({
    ...
    it('only send an email to paid subscribed members of the selected newsletter', async function () {
      ...
      const publishedRes = await request
        .put(
          localUtils.API.getApiQuery(
            `posts/${res.body.posts[0].id}/?email_segment=status:-free&newsletter=${secondNewsletterSlug}`,
          ),
        )
        .set('Origin', config.get('url'))
        .send({
    ...
    it('read-only value do not cause errors when edited', function () {
      return request
        ...
        .then((res) => {
          ...
          assert.equal(
            res.body.posts[0].plaintext,
            testUtils.DataGenerator.Content.posts[0].plaintext,
          );
        });
    });
            ...
            .put(
              localUtils.API.getApiQuery(
                'posts/' +
                  ...
                  '/?source=html&formats=html,plaintext',
              ),
            )
            .set('Origin', config.get('url'))
            .send({
    ...
    it('update dates', function () {
      ...
      return request
        .put(
          localUtils.API.getApiQuery('posts/' + testUtils.DataGenerator.Content.posts[0].id + '/'),
        )
        .set('Origin', config.get('url'))
        .send({ posts: [post] })
        .expect('Content-Type', /json/)
      ...
      return request
        ...
        .then((res) => {
          ...
          return request
            .put(
              localUtils.API.getApiQuery(
                'posts/' + testUtils.DataGenerator.Content.posts[7].id + '/',
              ),
            )
            .set('Origin', config.get('url'))
            .send({
      ...
      return request
        ...
        .then((res) => {
          ...
          return request
            .put(
              localUtils.API.getApiQuery(
                'posts/' + testUtils.DataGenerator.Content.posts[3].id + '/',
              ),
            )
            .set('Origin', config.get('url'))
            .send({
        ...
        .then((res) => {
          ...
          assert.equal(
            ...
            '/email/d52c42ae-2755-455c-80ec-70b2ec55c903/',
          );
        });
    });
    ...
    it('errors with invalid email segment', function () {
      return request
        ...
        .then((res) => {
          return request
            .put(
              `${localUtils.API.getApiQuery(`posts/${res.body.posts[0].id}/`)}?newsletter=${secondNewsletterSlug}&email_segment=not-a-filter`,
            )
            .set('Origin', config.get('url'))
            .send({
              posts: [
