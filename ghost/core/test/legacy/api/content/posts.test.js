...
describe('api/endpoints/content/posts', function () {
    ...
    const res = await request
      .get(
        localUtils.API.getApiQuery(
          `posts/?key=${validKey}&filter=authors.password:'${hashedPassword}'`,
        ),
      )
  ...
  it('can not filter posts by author.email or authors.email', async function () {
    ...
    const res = await request
      .get(localUtils.API.getApiQuery(`posts/?key=${validKey}&filter=authors.email:'${userEmail}'`))
      .set('Origin', testUtils.API.getURL())
      .expect('Content-Type', /json/)
      .expect('Cache-Control', testUtils.cacheRules.public)
      .expect(200);
  ...
  it('can not filter posts by authors.password.x (3-segment bypass)', async function () {
    ...
    try {
      const res = await request
        .get(
          localUtils.API.getApiQuery(
            `posts/?key=${validKey}&filter=authors.password.x:'${hashedPassword}'`,
          ),
        )
        .set('Origin', testUtils.API.getURL())
        .expect('Content-Type', /json/)
  ...
  it('browse posts', async function () {
    await request
      .get(localUtils.API.getApiQuery(`posts/?key=${validKey}`))
      .set('Origin', testUtils.API.getURL())
      .expect('Content-Type', /json/)
      .expect('Cache-Control', testUtils.cacheRules.public)
      .expect(200)
  ...
  it('browse posts with related authors/tags also returns primary_author/primary_tag', async function () {
    await request
      .get(localUtils.API.getApiQuery(`posts/?key=${validKey}&include=authors,tags`))
      .set('Origin', testUtils.API.getURL())
      .expect('Content-Type', /json/)
      .expect('Cache-Control', testUtils.cacheRules.public)
      .expect(200)
  ...
  it('browse posts with unsupported "page" filter returns a request validation error', function () {
    return request
      .get(localUtils.API.getApiQuery(`posts/?key=${validKey}&filter=page:true,featured:true`))
      .expect('Content-Type', /json/)
  ...
  it('browse posts with published and draft status, should not return drafts', async function () {
    await request
      .get(
        localUtils.API.getApiQuery(`posts/?key=${validKey}&filter=status:published,status:draft`),
      )
      .expect('Content-Type', /json/)
      .expect('Cache-Control', testUtils.cacheRules.public)
      .expect(200)
  ...
  it('browse posts with slug filter, should order in slug order', function () {
    return request
      .get(
        localUtils.API.getApiQuery(
          `posts/?key=${validKey}&filter=slug:[write,ghostly-kitchen-sink,grow]`,
        ),
      )
      .expect('Content-Type', /json/)
      .expect('Cache-Control', testUtils.cacheRules.public)
  ...
  it('browse posts with slug filter should order taking order parameter into account', function () {
    return request
      .get(
        localUtils.API.getApiQuery(
          `posts/?key=${validKey}&order=slug%20DESC&filter=slug:[write,ghostly-kitchen-sink,grow]`,
        ),
      )
      .expect('Content-Type', /json/)
      .expect('Cache-Control', testUtils.cacheRules.public)
    ...
    await request
      ...
      .expect(function (res) {
        ...
        assert.equal(
          ...
          `http://localhost:9999/ghost/api/content/posts/?key=${validKey}`,
        );
        assertExists(res.headers['access-control-allow-origin']);
        assert.equal(res.headers['x-cache-invalidate'], undefined);
      });
  ...
  it("can't read page", function () {
    return request
      .get(
        localUtils.API.getApiQuery(
          `posts/${testUtils.DataGenerator.Content.posts[5].id}/?key=${validKey}`,
        ),
      )
      .set('Origin', testUtils.API.getURL())
      .expect('Content-Type', /json/)
  ...
  it('can read post with fields', function () {
    const complexPostId = testUtils.DataGenerator.Content.posts.find(
      (p) => p.slug === 'not-so-short-bit-complex',
    ).id;

    return request
      .get(
        localUtils.API.getApiQuery(
          `posts/${complexPostId}/?key=${validKey}&fields=title,slug,excerpt&formats=plaintext`,
        ),
      )
      .set('Origin', testUtils.API.getURL())
      .expect('Content-Type', /json/)
      ...
      .expect(200)
      .then((res) => {
        localUtils.API.checkResponse(res.body.posts[0], 'post', null, null, [
          'id',
          'title',
          'slug',
          'excerpt',
  ...
  describe('content gating', function () {
    ...
    it('public post fields are always visible', function () {
      return request
        .get(
          localUtils.API.getApiQuery(
            `posts/${publicPost.id}/?key=${validKey}&fields=slug,html,plaintext&formats=html,plaintext`,
          ),
        )
        .set('Origin', testUtils.API.getURL())
        .expect('Content-Type', /json/)
    ...
    it('cannot read members only post plaintext', function () {
      return request
        .get(
          localUtils.API.getApiQuery(
            `posts/${membersPost.id}/?key=${validKey}&formats=html,plaintext&fields=html,plaintext`,
          ),
        )
        .set('Origin', testUtils.API.getURL())
        .expect('Content-Type', /json/)
    ...
    it('can read "free" html and plaintext content of members post when using paywall card', function () {
      return request
        .get(
          localUtils.API.getApiQuery(
            `posts/${membersPostWithPaywallCard.id}/?key=${validKey}&formats=html,plaintext`,
          ),
        )
        .set('Origin', testUtils.API.getURL())
        .expect('Content-Type', /json/)
    ...
    it('cannot browse members only posts content', function () {
      return request
        ...
        .then((res) => {
          ...
          const membersOnlySlugs = ['thou-shalt-not-be-seen', 'thou-shalt-be-paid-for'];

          const freeToSeeSlugs = ['free-to-see', 'thou-shalt-have-a-taste', 'sell'];
