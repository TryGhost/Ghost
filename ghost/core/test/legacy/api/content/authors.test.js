...
describe('Authors Content API', function () {
  ...
  it('can not filter authors by password', async function () {
    ...
    const res = await request
      .get(
        localUtils.API.getApiQuery(`authors/?key=${validKey}&filter=password:'${hashedPassword}'`),
      )
    ...
    const res = await request
      .get(
        localUtils.API.getApiQuery(
          `authors/?key=${validKey}&filter=users.password:'${hashedPassword}'`,
        ),
      )
      .set('Origin', testUtils.API.getURL())
      .expect('Content-Type', /json/)
  ...
  it('can not filter authors by table-qualified email', async function () {
    ...
    const res = await request
      .get(localUtils.API.getApiQuery(`authors/?key=${validKey}&filter=users.email:'${userEmail}'`))
      .set('Origin', testUtils.API.getURL())
      .expect('Content-Type', /json/)
      .expect('Cache-Control', testUtils.cacheRules.public)
      .expect(200);
  ...
  it('can not filter authors by email', async function () {
    ...
    const res = await request
      .get(localUtils.API.getApiQuery(`authors/?key=${validKey}&filter=email:'${userEmail}'`))
      .set('Origin', testUtils.API.getURL())
      .expect('Content-Type', /json/)
      .expect('Cache-Control', testUtils.cacheRules.public)
      .expect(200);
  ...
  it('can read authors with fields', function () {
    return request
      .get(
        localUtils.API.getApiQuery(
          `authors/${DataGenerator.Content.users[0].id}/?key=${validKey}&fields=name`,
        ),
      )
      .set('Origin', testUtils.API.getURL())
      .expect('Content-Type', /json/)
  ...
  it('browse authors with slug filter, should order in slug order', function () {
    return request
      .get(
        localUtils.API.getApiQuery(
          `authors/?key=${validKey}&filter=slug:[joe-bloggs,ghost,slimer-mcectoplasm]`,
        ),
      )
      .expect('Content-Type', /json/)
      .expect('Cache-Control', testUtils.cacheRules.public)
