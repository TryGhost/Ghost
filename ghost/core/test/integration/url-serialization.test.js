...
const ROUTES = [
  {
    identifier: 'primary-tag-collection',
    filter: null,
    resourceType: 'posts',
...
const FIELD_SHAPES = [
  'url',
  'id,slug,url',
  'title,custom_excerpt,html,url',
  'title,url,custom_excerpt,html',
  null,
];
...
describe('Integration: Content API URL serialization (primary_tag permalinks)', function () {
  ...
  beforeAll(async function () {
    ...
    urlService.reset();
    ROUTES.forEach((r) =>
      urlService.onRouterAddedType(r.identifier, r.filter, r.resourceType, r.permalink),
    );
  ...
  describe('getRoutableResources', function () {
    ...
    it("applies each type's routing gate", async function () {
      ...
      assert.ok(
        tags.some((t) => t.slug === 'business'),
        'public tags with posts must be routable',
      );
  ...
  describe('a resource missing what the routing config reads', function () {
    ...
    it('routes a fully-loaded resource', async function () {
      ...
      assert.equal(
        filtered.getUrlForResource({ ...post.toJSON(), type: 'posts' }),
        '/news/public-first/',
      );
    });
