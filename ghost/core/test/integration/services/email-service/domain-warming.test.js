...
describe('Domain Warming Integration Tests', function () {
  ...
  afterEach(async function () {
    ...
    const postIds = await db.knex('posts').where('title', 'like', 'Test Post%').pluck('id');

    if (postIds.length > 0) {
      // Get email IDs for these posts
      const emailIds = await db.knex('emails').whereIn('post_id', postIds).pluck('id');
  ...
  describe('Domain warming progression', function () {
    it('sends first email with warmup limit of 200 from custom domain', async function () {
      ...
      assert.equal(
        customDomainCount,
        totalCount,
        'All emails should use custom domain when total < warmup limit',
      );
      assert.equal(
        fallbackDomainCount,
        0,
        'No emails should use fallback domain when total < warmup limit',
      );
    ...
    it('increases custom domain limit on subsequent day', async function () {
      ...
      assert.equal(
        customDomainCount,
        expectedLimit,
        `Should send ${expectedLimit} emails from custom domain on day 2`,
      );
    });
    ...
    it('does not increase limit when sending multiple emails on same day', async function () {
      ...
      assert.equal(
        email2.get('csd_email_count'),
        csdCount1,
        'CSD count should not increase on same day',
      );
    });
    ...
    it('respects total email count when it is less than warmup limit', async function () {
      ...
      if (totalCount <= 200) {
        assert.equal(
          csdCount,
          totalCount,
          'All emails should use custom domain when total < initial limit',
        );
      }
    });
    ...
    it('does not warm up when fallback domain and address are not set', async function () {
      ...
      assert.equal(
        uniqueValues.length,
        1,
        'All batches should use same domain when warmup disabled',
      );
      assert.equal(
        uniqueValues[0],
        false,
        'All batches should use primary domain when warmup disabled',
      );
    });
    ...
    it.runIf(isMySQL)('handles maximum limit scenarios', { timeout: 60000 }, async function () {
      ...
      for (let day = 0; day < 5; day++) {
        ...
        if (previousCsdCount > 0) {
          assert.ok(
            csdCount >= previousCsdCount,
            `Day ${day}: Should not decrease from previous day`,
          );
        }
