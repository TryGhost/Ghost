...
describe('Email tier segmentation (recipient partition)', function () {
  ...
  async function membersForSegment(segment) {
    const rows = await models.Member.getFilteredCollectionQuery({
  ...
  it('partitions recipients of a tier-restricted post into access and no-access segments', async function () {
    ...
    assert.deepEqual(
      accessMembers,
      new Set([members.gold.get('email'), members.both.get('email')]),
    ...
    assert.deepEqual(
      noAccessMembers,
      new Set([members.silver.get('email'), members.free.get('email')]),
    ...
    assert.equal(
      union.size,
      4,
      'every seeded recipient must land in exactly one segment (no drops)',
    );
  });
});
