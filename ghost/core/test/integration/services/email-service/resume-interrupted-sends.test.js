...
describe('Resume interrupted sends', function () {
  ...
  it('resumes a pending batch and skips an already-submitted batch', async function () {
    ...
    assert.equal(
      emailModel.get('status'),
      'submitted',
      'email should re-promote to submitted after resume',
    );
    ...
    assert.equal(
      refreshedA.get('status'),
      'submitted',
      'already-submitted batch remains submitted',
    );
    assert.equal(
      refreshedB.get('status'),
      'submitted',
      'previously-pending batch is now submitted',
    );
  ...
  it('marks email as failed when an orphan submitting batch is encountered', async function () {
    ...
    assert.equal(
      emailModel.get('status'),
      'failed',
      'email should promote to failed when an orphan submitting batch is present',
    );
    ...
    const orphanLogs = errorLog
      ...
      .filter(
        (call) =>
          ...
          /is stuck in status=submitting \(orphan from a crashed worker\)/.test(call.args[0]),
      );
    assert.ok(
      orphanLogs.length > 0,
      'expected the "orphan from a crashed worker" guard to log an error',
    );
    ...
    assert.equal(
      refreshedB.get('status'),
      'submitting',
      'orphan submitting batch is preserved for operator review',
    );
  ...
  it('marks email as failed when the parent post is no longer published', async function () {
    ...
    assert.equal(
      emailModel.get('status'),
      'failed',
      'email should be flipped to failed when post is no longer published',
    );
    sinon.assert.notCalled(mailgunStub);
  });
});
