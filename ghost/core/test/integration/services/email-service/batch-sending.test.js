...
const mobileDocExample =
  '{"version":"0.3.1","atoms":[],"cards":[],"markups":[],"sections":[[1,"p",[[0,[],0,"Hello world"]]]],"ghostVersion":"4.0"}';
const mobileDocWithPaywall =
  '{"version":"0.3.1","markups":[],"atoms":[],"cards":[["paywall",{}]],"sections":[[1,"p",[[0,[],0,"Free content"]]],[10,0],[1,"p",[[0,[],0,"Members content"]]]]}';
const mobileDocWithFreeMemberOnly =
  '{"version":"0.3.1","atoms":[],"cards":[["email-cta",{"showButton":false,"showDividers":true,"segment":"status:free","alignment":"left","html":"<p>This is for free members only</p>"}]],"markups":[],"sections":[[1,"p",[[0,[],0,"Hello world"]]],[10,0],[1,"p",[[0,[],0,"Bye."]]]],"ghostVersion":"4.0"}';
const mobileDocWithPaidMemberOnly =
  '{"version":"0.3.1","atoms":[],"cards":[["email-cta",{"showButton":false,"showDividers":true,"segment":"status:-free","alignment":"left","html":"<p>This is for paid members only</p>"}]],"markups":[],"sections":[[1,"p",[[0,[],0,"Hello world"]]],[10,0],[1,"p",[[0,[],0,"Bye."]]]],"ghostVersion":"4.0"}';
const mobileDocWithPaidAndFreeMemberOnly =
  '{"version":"0.3.1","atoms":[],"cards":[["email-cta",{"showButton":false,"showDividers":true,"segment":"status:free","alignment":"left","html":"<p>This is for free members only</p>"}],["email-cta",{"showButton":false,"showDividers":true,"segment":"status:-free","alignment":"left","html":"<p>This is for paid members only</p>"}]],"markups":[],"sections":[[1,"p",[[0,[],0,"Hello world"]]],[10,0],[10,1],[1,"p",[[0,[],0,"Bye."]]]],"ghostVersion":"4.0"}';
const mobileDocWithFreeMemberOnlyAndPaywall =
  '{"version":"0.3.1","atoms":[],"cards":[["email-cta",{"showButton":false,"showDividers":true,"segment":"status:free","alignment":"left","html":"<p>This is for free members only</p>"}],["paywall",{}]],"markups":[],"sections":[[1,"p",[[0,[],0,"Hello world"]]],[10,0],[1,"p",[[0,[],0,"Bye."]]],[10,1],[1,"p",[[0,[],0,"This is after the paywall."]]]],"ghostVersion":"4.0"}';
const mobileDocWithReplacements =
  '{"version":"0.3.1","atoms":[],"cards":[["email",{"html":"<p>Hey {first_name, \\"there\\"}, Hey {first_name},</p>"}]],"markups":[],"sections":[[1,"p",[[0,[],0,"Hello {first_name},"]]],[10,0]],"ghostVersion":"4.0"}';
...
async function testEmailBatches(settings, email_recipient_filter, expectedBatches) {
  ...
  assert.equal(
    emailModel.get('email_count'),
    expectedTotal,
    'This email should have an email_count of ' + expectedTotal + ' recipients',
  );
  ...
  for (const expectedBatch of expectedBatches) {
    ...
    const index = remainingBatches.findIndex(
      (b) => b.get('member_segment') === expectedBatch.segment,
    );
    ...
    const emailRecipientsFirstBatch = await models.EmailRecipient.findAll({
      filter: `email_id:'${emailModel.id}'+batch_id:'${firstBatch.id}'`,
    });
...
describe('Batch sending tests', function () {
  ...
  afterEach(async function () {
    ...
    const leaked = (await models.Member.findAll()).models.filter(
      (member) => !seededMemberIds.has(member.id),
    );
  ...
  it("Doesn't include members created after the email in the batches", async function () {
    ...
    const emailRecipients2 = await models.EmailRecipient.findAll({
      filter: `email_id:'${emailModel2.id}'`,
    });
  ...
  it('One failed batch marks the email as failed and allows for a retry', async function () {
    ...
    for (const batch of batches.models) {
      ...
      // Did we create recipients?
      const batchRecipients = await models.EmailRecipient.findAll({
        filter: `email_id:'${emailModel.id}'+batch_id:'${batch.id}'`,
      });
    ...
    const skipLogs = infoLog
      .getCalls()
      .filter((call) => /already submitted on a prior run; skipping/.test(call.args[0]));
    infoLog.restore();
    assert.ok(
      skipLogs.length > 0,
      'expected at least one "already submitted on a prior run; skipping" info log',
    );
  ...
  it('Cannot send an email if verification is required', async function () {
    ...
    assert.equal(
      ...
      false,
      'This test requires email verification to be disabled initially',
    );
    ...
    await agent
      .put(`posts/${id}/?newsletter=${newsletterSlug}`)
  ...
  describe('Analytics', function () {
    it('Adds link tracking to all links in a post', async function () {
      ...
      const links = await linkRedirectRepository.getAll({
        filter: "post_id:'" + emailModel.get('post_id') + "'",
      });
      ...
      for (const el of $('a').toArray()) {
        ...
        if (href.includes('/unsubscribe/?uuid')) {
          assert(
            href.includes('?uuid=' + memberUuid),
            'Subscribe link need to contain uuid, got ' + href,
          );
          continue;
        }
        ...
        if (href.includes('https://ghost.org/?via=pbg-newsletter')) {
          assert(
            !href.includes('?m=' + memberUuid),
            'Powererd by Ghost link should not be tracked',
          );
          continue;
        }
        ...
        await frontendAgent.get(path).expect('Location', link.to.href).expect(302);

        // Wait for the link clicks to be processed
        await DomainEvents.allSettled();

        const clickEvent = await linkClickRepository.getAll({
          member_id: member.id,
  ...
  describe('Replacements', function () {
    it('Does replace with and without fallback in both plaintext and html for member without name', async function () {
      ...
      assert.match(
        html,
        /<a href="http:\/\/127.0.0.1:\d+\/unsubscribe\/\?uuid=[a-z0-9-]+&key=[a-z0-9]+&newsletter=[a-z0-9-]+"/,
        'Unsubscribe link not found in html',
      );
      ...
      assert.match(
        plaintext,
        /\[http:\/\/127.0.0.1:\d+\/unsubscribe\/\?uuid=[a-z0-9-]+&key=[a-z0-9]+&newsletter=[a-z0-9-]+\]/,
        'Unsubscribe link not found in plaintext',
      );
  ...
  describe('Newsletter settings', function () {
    ...
    it('Shows 2 comment buttons for published posts without feedback enabled', async function () {
      ...
      const defaultNewsletter = await getDefaultNewsletter();
      assert(
        defaultNewsletter.get('show_comment_cta'),
        'show_comment_cta should be true for this test',
      );
      assert(
        !defaultNewsletter.get('feedback_enabled'),
        'feedback_enabled should be off for this test',
      );
      ...
      assert.equal(
        html.match(/#ghost-comments/g).length,
        1,
        'Every email should have 1 button to comments',
      );
      await matchEmailSnapshot();
    });
    ...
    it('Shows share button when enabled in newsletter for public, members, paid, and tiers posts', async function () {
      ...
      try {
        ...
        await models.Newsletter.edit(
          { show_share_button: originalShowShareButton },
          { id: defaultNewsletter.id },
        );
      }
    });
    ...
    it('Shows subscription details box for free members', async function () {
      ...
      // Currently the link is not present in plaintext version (because no text)
      assert.equal(
        html.match(/#\/portal\/account/g).length,
        1,
        'Subscription details box should contain a link to the account page',
      );
    ...
    it('Shows subscription details box for trialing member', async function () {
      ...
      assert.match(
        plaintext,
        /You are receiving this because you are a trialing subscriber to Ghost\. Your free trial ends on \d+ \w+ \d+, at which time you will be charged the regular price\. You can always cancel before then\./,
      );
    ...
    it('Shows subscription details box for paid member', async function () {
      ...
      assert.match(
        plaintext,
        /You are receiving this because you are a paid subscriber to Ghost\. Your subscription will renew on \d+ \w+ \d+\./,
      );
    ...
    it('Shows subscription details box for canceled paid member', async function () {
      ...
      assert.match(
        plaintext,
        /You are receiving this because you are a paid subscriber to Ghost\. Your subscription has been canceled and will expire on \d+ \w+ \d+\. You can resume your subscription via your account settings\./,
      );
    ...
    it('Shows 3 latest posts', async function () {
      ...
      assert.equal(
        html.match(/This is the main post title/g).length,
        2,
        'Should only contain the title two times',
      ); // otherwise post is in last 3 posts
