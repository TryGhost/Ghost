...
describe('welcome email automations poll', function () {
  ...
  it('loads and sends emails, then marks their jobs finished', async function () {
    ...
    sinon.assert.calledWith(
      options.memberWelcomeEmailService.api.send,
      sinon.match({
        member: {
          name: 'Member One',
          email: 'member1@example.com',
  ...
  it('requests retry if email send fails', async function () {
    ...
    assert.ok(
      enqueuedDrift < 2000,
      `enqueueAnotherPollAt should be ~RETRY_DELAY_MS after pollStart (drift: ${enqueuedDrift}ms)`,
    );
    ...
    const readyAt =
      updatedRun.ready_at instanceof Date
    ...
    assert.ok(
      readyAtDrift < 2000,
      `ready_at should be ~RETRY_DELAY_MS after pollStart (drift: ${readyAtDrift}ms)`,
    );
  });
  ...
  it('dispatches another poll event when batch size reaches limit', async function () {
    ...
    sinon.assert.calledOnceWithExactly(
      options.enqueueAnotherPollAt,
      sinon.match((date) => date instanceof Date && date <= new Date() && date >= beforePoll),
    );
  });
});
