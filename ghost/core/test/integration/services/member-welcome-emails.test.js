...
describe('Member Welcome Emails Integration', function () {
  ...
  afterEach(async function () {
    ...
    if (defaultNewsletterSenderState) {
      await db.knex('newsletters').where('id', defaultNewsletterSenderState.id).update({
    ...
    await db.knex('members').del();
    await db.knex('email_design_settings').where('id', defaultEmailDesignSettingId).update({
      sender_name: null,
      sender_email: null,
  ...
  describe('Member creation with welcome emails', function () {
    ...
    it('creates legacy automation run when automations labs flag is disabled', async function () {
      await db.knex.transaction(async (trx) => {
        ...
        const runs = await trx('welcome_email_automation_runs').where('member_id', member.id);
  ...
  describe('Sending welcome emails', function () {
    ...
    it('uses configured sender and reply-to when sending member welcome email', async function () {
      ...
      await db.knex('newsletters').where('id', defaultNewsletter.id).update({
        sender_name: 'Scott',
    ...
    it('prefers sender details from email design settings for member welcome emails', async function () {
      ...
      const welcomeEmailAutomation = await db
        .knex('automations')
        .where('slug', MEMBER_WELCOME_EMAIL_SLUGS.free)
        .first('id');
    ...
    it('uses newsletter sender details for member welcome emails when email design sender details are empty', async function () {
      ...
      sinon.assert.calledOnceWithExactly(
        mailService.GhostMailer.prototype.send,
        sinon.match({
    ...
    it('adds the configured bulk email Mailgun tag to automation emails', async function () {
      sinon.stub(config, 'get').callThrough().withArgs('bulkEmail:mailgun:tag').returns('blog-123');
    ...
    it('adds an updates & announcements unsubscribe link and one-click headers when automations is enabled', async function () {
      ...
      assert.match(
        message.html,
        /\/unsubscribe\/\?uuid=99999999-9999-4999-8999-999999999999&amp;key=[a-f0-9]+&amp;updatesandannouncements=1/,
      );
      assert.match(
        recipientData['automation-member@example.com'].list_unsubscribe,
        /^http.*\/unsubscribe\/\?uuid=99999999-9999-4999-8999-999999999999&key=[a-f0-9]+&updatesandannouncements=1$/,
      );
    });
