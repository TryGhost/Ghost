...
async function resolvedSubscriptionId(memberId: string): Promise<string | null> {
  const row = await db.knex('members_resolved_subscription').where({ member_id: memberId }).first();
...
describe('Subscription resolution priority', function () {
  ...
  it('treats trialing, past_due, and unpaid as active alongside active', async function () {
    ...
    for (const activeStatus of ['trialing', 'past_due', 'unpaid']) {
      ...
      assert.equal(
        resolved,
        recentActiveEquivalent.get('id'),
        `${activeStatus} sub should resolve as current over a more recent canceled sub`,
      );
