...
function assertFromAddress(from, replyTo) {
  ...
  while (emailMockReceiver.getSentEmail(i)) {
    ...
    assert.equal(
      email.from,
      from,
      `From address (${email.from}) of ${i + 1}th email (${email.subject}) does not match ${from}`,
    );

    if (!replyTo) {
      assert(
        email.replyTo === null || email.replyTo === undefined,
        `Unexpected reply-to address (${email.replyTo}) of ${i + 1}th email (${email.subject}), expected none`,
      );
    } else {
      assert.equal(
        email.replyTo,
...
async function assertFromAddressNewsletter(aFrom, aReplyTo) {
  ...
  if (!aReplyTo) {
    assert(
      ...
      `Unexpected reply-to address (${replyTo}), expected none`,
    );
  } else {
...
describe('Email addresses', function () {
  ...
  describe('Custom sending domain', function () {
    ...
    it('[NEWSLETTER] Does not allow to send a newsletter from any email address (instead uses mail.from), but allows reply-to to be set', async function () {
      ...
      await assertFromAddressNewsletter(
        '"Anything Possible" <default@sendingdomain.com>',
        '"Anything Possible" <anything@possible.com>',
      );
    });
    ...
    it('[NEWSLETTER] Does allow to set the replyTo address to any address', async function () {
      ...
      await sendNewsletter();
      await assertFromAddressNewsletter(
        '"Anything Possible" <anything@sendingdomain.com>',
        'anything@possible.com',
      );
    });
  ...
  describe('Self-hosted', function () {
    ...
    it('[NEWSLETTER] Uses site title as default sender name', async function () {
      ...
      await sendNewsletter();
      await assertFromAddressNewsletter(
        '"Example Site" <default@sendingdomain.com>',
        '"Example Site" <default@sendingdomain.com>',
      );
    });
  });
