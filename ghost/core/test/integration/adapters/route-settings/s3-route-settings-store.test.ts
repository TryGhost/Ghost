...
const canonicalKey = (tenantPrefix = ''): string =>
  [tenantPrefix, STATIC_PREFIX, CANONICAL_FILENAME].filter(Boolean).join('/');
...
const backupKeyPattern = (tenantPrefix = ''): RegExp =>
  new RegExp(
  ...
  function () {
    ...
    describe('get', function () {
      ...
      it('throws InternalServerError when the bundled default file is missing on the empty state', async function () {
        const store = createStore({
          defaultSettingsBasePath: path.join(REAL_DEFAULTS_PATH, 'does-not-exist'),
        });
    ...
    describe('tenantPrefix scoping', function () {
      ...
      it('isolates tenants sharing the same bucket', async function () {
        ...
        assert.deepEqual((await storeB.get()).routes, [
          { type: 'template', path: '/b/', templates: ['b'] },
        ]);
        assert.deepEqual((await listObjectKeys(adminClient, bucket)).sort(), [
          canonicalKey('tenant-a'),
          canonicalKey('tenant-b'),
        ]);
      });
    });
    ...
    describe('replace: timestamped backups', function () {
      ...
      it('creates a new backup on every overwrite', { timeout: 15000 }, async function () {
        ...
        assert.equal(
          backupKeys.length,
          2,
          `expected 2 timestamped backups, got: ${keys.join(', ')}`,
        );
        assert.ok(
          keys.includes(canonicalKey()),
          `expected canonical ${canonicalKey()}, got: ${keys.join(', ')}`,
        );
      });
...
describe('Integration: S3RouteSettingsStore without a live bucket', function () {
  ...
  it('throws IncorrectUsageError when constructed with invalid config', function () {
    assert.throws(() => new S3RouteSettingsStore({ staticFileURLPrefix: STATIC_PREFIX } as never), {
  ...
  it('validate() accepts valid options and rejects invalid ones', function () {
    ...
    assert.throws(() => S3RouteSettingsStore.validate({} as never), {
      errorType: 'IncorrectUsageError',
    });
  });
  ...
  it('reports a non-NotFound error raised by the existence check on replace', async function () {
    ...
    await assert.rejects(
      store.replace(fromYaml(SAMPLE_YAML)),
