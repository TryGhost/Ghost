...
describe('Importer', function () {
  ...
  describe('Empty database (except of owner user), general tests', function () {
    ...
    it('ensure return structure', function () {
      return dataImporter
        .doImport(exportedBodyV2().db[0], importOptions)
    ...
    it('cares about invalid dates and date formats', function () {
      ...
      return dataImporter.doImport(exportData, importOptions).then(function (importResult) {
        ...
        assert.equal(
          importResult.problems[0].message,
        ...
        assert.equal(
          moment(importResult.data.posts[0].updated_at).format(),
          '2013-10-18T23:58:44Z',
        );
        assert.equal(
          moment(importResult.data.posts[0].published_at).format(),
          '2013-12-29T11:58:30Z',
        );
    ...
    it('warning that theme was not imported', function () {
      ...
      return dataImporter
        ...
        .then(function (importResult) {
          ...
          assert.equal(
            importResult.problems[0].message,
            'Theme not imported, please upload in Settings - Design',
          );
          return models.Settings.findOne(
            _.merge({ key: 'active_theme' }, testUtils.context.internal),
          );
        })
    ...
    it('removes duplicate tags and updates associations', function () {
      ...
      exportData.data.posts_tags = [
        testUtils.DataGenerator.forKnex.createPostsTags(
          exportData.data.posts[0].id,
          exportData.data.tags[0].id,
        ),
        testUtils.DataGenerator.forKnex.createPostsTags(
          exportData.data.posts[0].id,
          exportData.data.tags[1].id,
        ),
      ];
    ...
    it('removes broken tags from post (not in db, not in file)', function () {
      ...
      return dataImporter
        .doImport(exportData, importOptions)
        .then(function () {
          return Promise.all([models.Post.findPage({ withRelated: ['tags'] })]);
        })
        .then(function (data) {
    ...
    it('import owner, ensure original owner stays as is', function () {
      ...
      exportData.data.roles_users[0] = [
        testUtils.DataGenerator.forKnex.createUsersRoles(
          exportData.data.users[0].id,
          testUtils.DataGenerator.Content.roles[3].id,
        ),
      ];
      ...
      return dataImporter
        ...
        .then(function () {
          return models.User.findAll(
            Object.assign({ withRelated: ['roles'] }, testUtils.context.internal),
          );
        })
        .then(function (result) {
          ...
          assert.equal(
            result.models[0].get('email'),
            testUtils.DataGenerator.Content.users[0].email,
          );
    ...
    it('handles validation errors nicely', function () {
      ...
      exportData.data.users[1] = testUtils.DataGenerator.forKnex.createUser({
        email:
          'thisisareallylongemailaddressIamhappytobeusingacharactercounterbutIhavealongwaytogoyetImeanserioulsywhohasemailaddressthislongthereisnowaythiswillpassvalidationsonghost100andisarealedgecase',
      });
      ...
      return dataImporter
        ...
        .catch(function (err) {
          ...
          assert.equal(
            err.message,
            'Value in [users.bio] exceeds maximum length of 250 characters.',
          );
          ...
          assert.equal(
            response[2].message,
            'Value in [tags.meta_title] exceeds maximum length of 300 characters.',
          );
          ...
          assert.equal(
            response[5].message,
            'Value in [posts.title] exceeds maximum length of 255 characters.',
          );
        });
    });
    ...
    it('does import data with invalid user references', function () {
      ...
      return dataImporter
        ...
        .then(function (importedData) {
          ...
          assert.equal(
            importedData.problems[0].message,
            'Entry was imported, but we were not able to ' +
    ...
    it('invalid uuid', function () {
      ...
      exportData.data.posts[3] = testUtils.DataGenerator.forKnex.createPost({
        uuid: "hey-i'm-not-a-uuid-at-all",
        slug: 'post4',
      });
      ...
      return dataImporter
        ...
        .then(function (result) {
          assert.equal(
            validator.isUUID(result.data[0].toJSON().uuid),
            true,
            'Old Ghost UUID NOT fixed',
          );
          assert.equal(
            validator.isUUID(result.data[1].toJSON().uuid),
            true,
            'Empty UUID NOT fixed',
          );
          assert.equal(
            validator.isUUID(result.data[2].toJSON().uuid),
            true,
            'Missing UUID NOT fixed',
          );
          assert.equal(
            validator.isUUID(result.data[3].toJSON().uuid),
            true,
            'Malformed UUID NOT fixed',
          );
        });
    });
    ...
    it('ensure post tag order is correct', function () {
      ...
      exportData.data.posts[0] = testUtils.DataGenerator.forKnex.createPost({
        slug: 'post1',
        published_at: moment().add(3, 'day').toDate(),
      });
      exportData.data.posts[1] = testUtils.DataGenerator.forKnex.createPost({
        slug: 'post2',
      ...
      exportData.data.posts[2] = testUtils.DataGenerator.forKnex.createPost({
        slug: 'post3',
        published_at: moment().add(2, 'day').toDate(),
      });
      ...
      exportData.data.posts_tags = [
        ...
        testUtils.DataGenerator.forKnex.createPostsTags(
          exportData.data.posts[2].id,
          exportData.data.tags[2].id,
        ),
        testUtils.DataGenerator.forKnex.createPostsTags(
          exportData.data.posts[2].id,
          exportData.data.tags[0].id,
      ...
      return dataImporter
        ...
        .then(function () {
          return Promise.all([
            models.Post.findPage(
              Object.assign({ withRelated: ['tags'] }, testUtils.context.internal),
            ),
    ...
    it('import multiple users, tags, posts', function () {
      ...
      exportData.data.roles_users = [
        ...
        testUtils.DataGenerator.forKnex.createUsersRoles(
          exportData.data.users[2].id,
          exportData.data.roles[2].id,
        ),
        testUtils.DataGenerator.forKnex.createUsersRoles(
          exportData.data.users[3].id,
          exportData.data.roles[4].id,
        ),
      ];
      ...
      return dataImporter
        ...
        .then(function (result) {
          ...
          assert.equal(
            users[1].created_at.toISOString(),
            moment(exportData.data.users[0].created_at).startOf('seconds').toISOString(),
          );
          assert.equal(
            users[2].created_at.toISOString(),
            moment(exportData.data.users[1].created_at).startOf('seconds').toISOString(),
          );
          assert.equal(
            users[3].created_at.toISOString(),
            moment(exportData.data.users[2].created_at).startOf('seconds').toISOString(),
          );
          assert.equal(
            users[4].created_at.toISOString(),
            moment(exportData.data.users[3].created_at).startOf('seconds').toISOString(),
          );

          assert.equal(
            users[1].updated_at.toISOString(),
            moment(exportData.data.users[0].updated_at).startOf('seconds').toISOString(),
          );
          assert.equal(
            users[2].updated_at.toISOString(),
            moment(exportData.data.users[1].updated_at).startOf('seconds').toISOString(),
          );
          assert.equal(
            users[3].updated_at.toISOString(),
            moment(exportData.data.users[2].updated_at).startOf('seconds').toISOString(),
          );
          assert.equal(
            users[4].updated_at.toISOString(),
            moment(exportData.data.users[3].updated_at).startOf('seconds').toISOString(),
          );
    ...
    it('does import settings with string booleans', function () {
      ...
      return dataImporter
        ...
        .then(function (imported) {
          assert.equal(imported.problems.length, 0);
          return models.Settings.findOne(
            _.merge({ key: 'portal_button' }, testUtils.context.internal),
          );
        })
        .then(function (result) {
          assert.equal(result.attributes.value, true);
          return models.Settings.findOne(
            _.merge({ key: 'is_private' }, testUtils.context.internal),
          );
        })
        .then(function (result) {
          assert.equal(result.attributes.value, false);

          return db.knex('settings').where('key', 'portal_button');
        })
        .then(function (result) {
          assert.equal(result[0].value, 'true');

          return db.knex('settings').where('key', 'is_private');
        })
        .then(function (result) {
    ...
    it('import comment_id', function () {
      ...
      return dataImporter
        .doImport(exportData, importOptions)
        .then(function () {
          return Promise.all([models.Post.findPage(testUtils.context.internal)]);
        })
        .then(function (result) {
          const posts = result[0].data.map((model) => model.toJSON());
    ...
    it('ensure authors are imported correctly', function () {
      ...
      exportData.data.users[4] = testUtils.DataGenerator.forKnex.createUser({
        email: 'user4@ghost.org',
        slug: 'user4',
      });
      ...
      exportData.data.posts_authors = [
        testUtils.DataGenerator.forKnex.createPostsAuthors(
          exportData.data.posts[0].id,
          exportData.data.users[1].id,
          0,
        ),
        testUtils.DataGenerator.forKnex.createPostsAuthors(
          exportData.data.posts[0].id,
      ...
      return dataImporter
        ...
        .then(function (imported) {
          ...
          return Promise.all([
            models.Post.findPage(
              Object.assign({ withRelated: ['authors'] }, testUtils.context.internal),
            ),
            models.User.findPage(testUtils.context.internal),
          ]);
        })
    ...
    it('import 1.0 Koenig post format', function () {
      ...
      return dataImporter
        .doImport(exportData, importOptions)
        .then(function () {
          return Promise.all([models.Post.findPage(options)]);
        })
        .then(function (result) {
          ...
          assert.equal(
            posts[0].lexical,
            '{"root":{"children":[{"type":"image","src":"source","cardWidth":"wide"},{"type":"markdown","markdown":"# Post Content"}],"direction":null,"format":"","indent":0,"type":"root","version":1}}',
          );
          assert.equal(
            posts[0].html,
    ...
    it('import 2.0 Koenig post format', function () {
      ...
      return dataImporter
        ...
        .then(function (result) {
          ...
          assert.equal(
            posts[0].lexical,
            '{"root":{"children":[{"type":"markdown","markdown":"## Post Content"},{"type":"image","src":"source2","cardWidth":"not-wide"}],"direction":null,"format":"","indent":0,"type":"root","version":1}}',
          );
          assert.equal(
            posts[0].html,
  ...
  describe('Existing database', function () {
    ...
    it('import multiple users, tags, posts', function () {
      ...
      return dataImporter
        .doImport(exportData, clonedImportOptions)
        .then(function () {
          return Promise.all([
            models.Post.findPage(postOptions),
            models.Tag.findPage(tagOptions),
        ...
        .then(function (result) {
          ...
          assert.equal(
            users.length,
            exportData.data.users.length + testUtils.DataGenerator.forKnex.users.length,
