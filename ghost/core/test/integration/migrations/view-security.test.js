...
describe('Migrations - view security', function () {
  ...
  it('ships members_resolved_subscription with SQL SECURITY INVOKER', async function () {
  ...
  it('the 6.50 migration converts an existing DEFINER view to INVOKER', async function () {
    await db.knex.raw(
      'CREATE OR REPLACE SQL SECURITY DEFINER VIEW `members_resolved_subscription` AS ' +
        views.members_resolved_subscription,
    );
    assert.equal(
      await securityType('members_resolved_subscription'),
      'DEFINER',
      'precondition: view should be DEFINER before the migration',
    );
  ...
  it('recovers a view whose DEFINER account is absent', async function () {
    const viewName = 'ber3756_view_security_demo';
    const viewSql = 'SELECT 1 AS one';

    try {
      await db.knex.raw(
        'CREATE OR REPLACE DEFINER=`ghost_absent`@`10.255.255.1` SQL SECURITY DEFINER VIEW ?? AS ' +
          viewSql,
        [viewName],
      );
