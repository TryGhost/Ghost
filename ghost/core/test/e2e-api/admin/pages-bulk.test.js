...
describe('Pages Bulk API', function () {
  ...
  describe('Edit', function () {
    it('Can feature multiple pages', async function () {
      ...
      const changedPages = await models.Post.findPage({
        filter: forcePageFilter(filter),
        limit: 1,
        status: 'all',
      });
      ...
      assert.equal(
        ...
        amount,
        `Expect all matching pages (${amount}) to be changed`,
      );
    ...
    it('Can add a single tag to pages', async function () {
      ...
      assert.equal(
        response.body.bulk.meta.stats.successful,
        amount,
        `Expect all matching pages (${amount}) to be changed, got ${response.body.bulk.meta.stats.successful} instead`,
      );
      ...
      for (const page of pages) {
        ...
        assert(
          ...
          `Expect page ${page.id} to have tag ${tag.id}`,
        );
      }
    });
    ...
    it('Can add multiple tags to pages and create new tags', async function () {
      ...
      for (const page of pages) {
        ...
        assert(
          ...
          `Expect page ${page.id} to have new tag ${newTagModel.id}`,
        );
      }
    });
    ...
    it('Can unpublish pages', async function () {
      ...
      assert.equal(
        response.body.bulk.meta.stats.successful,
        amount,
        `Expect all matching pages (${amount}) to be unpublished, got ${response.body.bulk.meta.stats.successful} instead`,
      );
  ...
  describe('Delete', function () {
    ...
    it('Can delete pages that match a tag', async function () {
      ...
      assert.equal(
        response.body.bulk.meta.stats.successful,
        amount,
        `Expect all matching pages (${amount}) to be deleted, got ${response.body.bulk.meta.stats.successful} instead`,
      );
      ...
      assert.equal(
        ...
        0,
        `Expect all matching pages (${amount}) to be deleted`,
      );
    });
