...
describe('Members import — custom fields', function () {
  ...
  async function importCSV(csv: string, mapping?: Record<string, string>): Promise<any> {
    const csvPath = path.join(
      os.tmpdir(),
      `members-import-cf-${Date.now()}-${Math.random().toString(16).slice(2)}.csv`,
    );
    fs.writeFileSync(csvPath, csv);
    try {
      let req = (request.post(localUtils.API.getApiQuery('members/upload/')) as any).set(
        'Origin',
        config.get('url'),
      );
