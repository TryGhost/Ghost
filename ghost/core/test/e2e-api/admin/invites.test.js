...
describe('Invites API', function () {
  ...
  describe('As Owner', function () {
    ...
    it('Can fetch all invites', async function () {
      const res = await request
        .get(localUtils.API.getApiQuery('invites/'))
    ...
    it('Can read an invitation by id', async function () {
      const res = await request
        .get(
          localUtils.API.getApiQuery(`invites/${testUtils.DataGenerator.forKnex.invites[0].id}/`),
        )
        .set('Origin', config.get('url'))
        .expect('Content-Type', /json/)
        .expect('Cache-Control', testUtils.cacheRules.private)
    ...
    it('Can add a new invite', async function () {
      const res = await request
        ...
        .send({
          invites: [
            { email: 'test@example.com', role_id: testUtils.getExistingData().roles[1].id },
          ],
        })
        .expect('Content-Type', /json/)
        .expect('Cache-Control', testUtils.cacheRules.private)
      ...
      assert.equal(
        ...
        `/ghost/api/admin/invites/${res.body.invites[0].id}/`,
      );
    });
    ...
    it('Cannot destroy an non-existent invite', async function () {
      await request
        .del(localUtils.API.getApiQuery(`invites/abcd1234abcd1234abcd1234/`))
        .set('Origin', config.get('url'))
        .expect('Cache-Control', testUtils.cacheRules.private)
        .expect(404)
        .expect((res) => {
          assert.equal(
            ...
            'Resource not found error, cannot delete invite.',
          );
        });
  ...
  describe('As Admin Integration', function () {
    ...
    it('Can add a new invite by API Key with the Contributor Role', async function () {
      const roleId = testUtils
        .getExistingData()
        .roles.find((role) => role.name === 'Contributor').id;
      const res = await request
        .post(localUtils.API.getApiQuery('invites/'))
        .set('Authorization', `Ghost ${localUtils.getValidAdminToken('/admin/')}`)
        .send({
    ...
    it('Can add a new invite by API Key with the Super Editor Role', async function () {
      const roleId = testUtils
        .getExistingData()
        .roles.find((role) => role.name === 'Super Editor').id;
      const res = await request
        .post(localUtils.API.getApiQuery('invites/'))
        .set('Authorization', `Ghost ${localUtils.getValidAdminToken('/admin/')}`)
        .send({
    ...
    it('Can not add a new invite by API Key with the Administrator Role', async function () {
      const roleId = testUtils
        .getExistingData()
        .roles.find((role) => role.name === 'Administrator').id;
      await request
        .post(localUtils.API.getApiQuery('invites/'))
        .set('Authorization', `Ghost ${localUtils.getValidAdminToken('/admin/')}`)
        .send({
