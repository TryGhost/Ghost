...
describe('Members filtering, every operator admin can emit', function () {
    ...
    beforeAll(async function () {
        ...
        async function createMember(member: Record<string, unknown>) {
            await agent.post('members/').body({members: [member]}).expectStatus(201);
        }

        await createMember({email: ALICE, name: 'Alice Anderson', labels: [{name: 'vip'}], newsletters: [{id: newsletterId}]});
        await createMember({email: BOB, name: 'Bob Brown', labels: [{name: 'vip'}], newsletters: []});
        ...
        await setColumns(CAROL, {status: 'comped', email_count: 10, created_at: '2025-01-01 00:00:00', email_disabled: true});
    });
    ...
    describe('native — a date', function () {
        matrix([
            ...
            {what: 'created on or before', nql: "created_at:<='2024-09-01T23:59:59.999Z'", expect: [ALICE, BOB]},
            {what: 'created after', nql: "created_at:>'2024-09-01T00:00:00.000Z'", expect: [CAROL]},
            {what: 'created on or after', nql: "created_at:>='2024-03-01T00:00:00.000Z'", expect: [BOB, CAROL]}
        ]);
    });
    ...
    describe('derived — one newsletter, by slug', function () {
        it('subscribed to a named newsletter', async function () {
            assert.deepEqual(await browse(`(newsletters.slug:${newsletterSlug}+email_disabled:0)`), [ALICE]);
        });

        it('unsubscribed from a named newsletter', async function () {
            assert.deepEqual(await browse(`(newsletters.slug:-${newsletterSlug},email_disabled:1)`), [BOB, CAROL].sort());
        });
    });
    ...
    describe('native — a text value containing the anchor characters', function () {
        ...
        beforeAll(async function () {
            for (const [email, name] of [
                ...
                await agent.post('members/').body({members: [{email, name}]}).expectStatus(201);
            }
        });
    ...
    describe('combinations, as a publisher builds them', function () {
        matrix([
            ...
            {what: 'a derived and a date', nql: "label:[vip]+created_at:<'2024-03-01T00:00:00.000Z'", expect: [ALICE]},
            {what: 'a filter matching nobody still answers', nql: "status:paid+name:~'Zeta'", expect: []}
        ]);
