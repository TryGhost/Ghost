...
describe('Actions API', function () {
  ...
  it('Can request actions for resource', async function () {
    ...
    const res2 = await request
      .get(
        localUtils.API.getApiQuery(
          `actions/?filter=${encodeURIComponent(`resource_id:'${postId}'`)}&include=actor`,
        ),
      )
      .set('Origin', config.get('url'))
      .expect('Content-Type', /json/)
    ...
    assert.equal(
      res2.body.actions[0].actor.image,
      testUtils.DataGenerator.Content.users[0].profile_image,
    );
    ...
    assert.equal(
      res4.body.actions[0].actor.image,
      testUtils.DataGenerator.Content.users[0].profile_image,
    );
    ...
    assert.equal(
      res5.body.actions[0].actor.name,
      testUtils.DataGenerator.Content.integrations[0].name,
    );
    assert.equal(
      res5.body.actions[0].actor.slug,
      testUtils.DataGenerator.Content.integrations[0].slug,
    );
  });
  ...
  it('Can request security actions with resource included', async function () {
    ...
    const res = await request
      .get(
        localUtils.API.getApiQuery(
          `actions/?filter=${encodeURIComponent(`id:'${action.id}'`)}&include=actor,resource`,
        ),
      )
      .set('Origin', config.get('url'))
      .expect('Content-Type', /json/)
