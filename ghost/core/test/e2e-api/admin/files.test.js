...
describe('Files API', function () {
  ...
  it('Can upload a file', async function () {
    const res = await request
      .post(localUtils.API.getApiQuery('files/upload'))
    ...
    assert.match(
      new URL(res.body.files[0].url).pathname,
      /\/content\/files\/\d+\/\d+\/loadingcat_square\.gif/,
    );
  ...
  it('Passes the content type to the storage adapter when uploading a PDF', async function () {
    ...
    assert.equal(
      fileArg.type,
      'application/pdf',
      'save() should receive the correct content type for PDF files',
    );
  });
  ...
  it('Derives content type from file extension, ignoring client-provided MIME type', async function () {
    ...
    assert.equal(
      fileArg.type,
      'application/pdf',
      'save() should use extension-derived type, not client-provided text/html',
    );
  });
  ...
  it('Passes the content type to the storage adapter when uploading a JSON file', async function () {
    ...
    assert.match(
      new URL(res.body.files[0].url).pathname,
      /\/content\/files\/\d+\/\d+\/redirects\.json/,
    );
    files.push(new URL(res.body.files[0].url).pathname);
    ...
    assert.equal(
      fileArg.type,
      'application/json',
      'save() should receive the correct content type for JSON files',
    );
  });
  ...
  it('Stores non-browser-renderable files with application/octet-stream', async function () {
    ...
    try {
      ...
      assert.match(
        new URL(res.body.files[0].url).pathname,
        /\/content\/files\/\d+\/\d+\/spreadsheet\.xlsx/,
      );
      files.push(new URL(res.body.files[0].url).pathname);
    ...
    try {
      for (const filename of ['package.rpm', 'package.deb']) {
        ...
        assert.equal(
          fileArg.type,
          'application/octet-stream',
          `${filename} should be stored as application/octet-stream`,
        );
      }
    } finally {
      fs.removeSync(tmpDir);
