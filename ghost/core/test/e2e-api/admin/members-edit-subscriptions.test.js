...
describe('Members API: edit subscriptions', function () {
  ...
  beforeEach(function () {
    sinon
      ...
      .returns('http://domain.com/unsubscribe/?uuid=memberuuid&key=abc123dontstealme'); // member uuid changes with every test run
  ...
  it('Comp product is removed when a comped member upgrades to a paid subscription', async function () {
    ...
    member = await models.Member.findOne(
      { email },
      { require: true, withRelated: ['products', 'stripeCustomers', 'stripeSubscriptions'] },
    );
    ...
    assert.equal(
      member.related('products').length,
      1,
      'This member should have one product (paid only)',
    );
    assert.deepEqual(
      member
        .related('products')
        .models.map((m) => m.get('slug'))
        .sort(),
      ['default-product'],
    );
  ...
  it('Can cancel a subscription for a member with duplicate customers', async function () {
    ...
    assert.equal(
      member.related('products').length,
      1,
      'This member should only have one remaning product',
    );
    ...
    assert.equal(
      member.related('products').length,
      0,
      'This member should only have no remaning products',
    );
  });
  ...
  it('Can update a subscription for a member with duplicate subscriptions', async function () {
    ...
    assert.equal(
      member.related('products').length,
      2,
      'This member should still have two products',
    );
  ...
  it('Can recover member products when we cancel a subscription', async function () {
    ...
    await models.Member.edit(
      {
        products: member
          .related('products')
          .models.filter((p) => p.get('slug') !== 'default-product'),
      },
      { id: member.id },
    );
    ...
    assert.equal(
      member.related('products').length,
      1,
      'This member should have one product after the update',
    );
    ...
    assert.equal(
      member.related('products').length,
      1,
      'This member should still have the other product that was wrongfully removed in the past',
    );
    assert.equal(
      member.related('products').models[0].get('slug'),
      'default-product',
      'This member should still have the other product that was wrongfully removed in the past',
    );
  ...
  it('Can recover member products when we update a subscription', async function () {
    ...
    assert.equal(
      member.related('products').length,
      2,
      'Should readd the product that was wrongfully removed in the past',
    );
  });
