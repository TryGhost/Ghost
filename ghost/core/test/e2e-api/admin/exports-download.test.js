...
describe('Exports API — download', function () {
  ...
  it('Can download a full site export zip', async function () {
    ...
    assert.match(
      ...
      /^Attachment; filename="[A-Za-z0-9._-]*ghost\.export\.\d{4}-\d{2}-\d{2}\.zip"$/,
    );
    ...
    assert.ok(
      !settingKeys.includes('stripe_connect_secret_key'),
      'blocklisted settings must not be exported',
    );
    ...
    assert.ok(
      analyticsRows >= EXTRA_POST_COUNT,
      `expected at least ${EXTRA_POST_COUNT} analytics rows, got ${analyticsRows}`,
    );
