...
describe('Admin API - Max Limit Cap', function () {
        ...
        posts: [
          {
            ...
            mobiledoc:
              '{"version":"0.3.1","atoms":[],"cards":[],"markups":[],"sections":[[1,"p",[[0,[],0,"Test content for email"]]]]}',
          },
        ],
      })
      .expectStatus(201);
  ...
  async function createAdditionalPosts() {
    ...
    const { body: postsCheck } = await agent.get('posts/').expectStatus(200);
    ...
    if (postsCheck.meta.pagination.total < 10) {
      ...
      for (let i = existingCount; i < 10; i++) {
        additionalPosts.push({
          ...
          mobiledoc:
            '{"version":"0.3.1","atoms":[],"cards":[],"markups":[],"sections":[[1,"p",[[0,[],0,"Test content"]]]]',
  ...
  describe('Posts API', function () {
    it(`should cap limit to ${MAX_LIMIT} when limit exceeds max`, async function () {
      const { body } = await agent.get('posts/?limit=10').expectStatus(200);
    ...
    it(`should cap limit to ${MAX_LIMIT} when limit is "all"`, async function () {
      const { body } = await agent.get('posts/?limit=all').expectStatus(200);
    ...
    it('should respect smaller limits', async function () {
      const { body } = await agent.get('posts/?limit=3').expectStatus(200);
    ...
    it('should allow large limits for export endpoint', async function () {
      ...
      const { text } = await agent.get('posts/export/?limit=1000').expectStatus(200);
  ...
  describe('Members API', function () {
    it(`should cap limit to ${MAX_LIMIT} when limit exceeds max`, async function () {
      const { body } = await agent.get('members/?limit=10').expectStatus(200);
    ...
    it('should cap limit to 5 when limit is "all"', async function () {
      const { body } = await agent.get('members/?limit=all').expectStatus(200);
  ...
  describe('Tags API', function () {
    it(`should cap limit to ${MAX_LIMIT} when limit exceeds max`, async function () {
      const { body } = await agent.get('tags/?limit=10').expectStatus(200);
  ...
  describe('Pages API', function () {
    it(`should cap limit to ${MAX_LIMIT} when limit exceeds max`, async function () {
      const { body } = await agent.get('pages/?limit=10').expectStatus(200);
  ...
  describe('Exception Endpoints', function () {
    it('should bypass limit cap for emails batches endpoint', async function () {
      ...
      const { body: batchesBody } = await agent
        .get(`emails/${testEmail.id}/batches/?limit=10`)
        .expectStatus(200);
    ...
    it('should bypass limit cap for emails recipient-failures endpoint', async function () {
      ...
      const { body: failuresBody } = await agent
        .get(`emails/${testEmail.id}/recipient-failures/?limit=10`)
        .expectStatus(200);
    ...
    it('should allow "all" for exception endpoints', async function () {
      ...
      const { body: batchesBody } = await agent
        .get(`emails/${testEmail.id}/batches/?limit=all`)
        .expectStatus(200);
      ...
      const { body: failuresBody } = await agent
        .get(`emails/${testEmail.id}/recipient-failures/?limit=all`)
        .expectStatus(200);
  ...
  describe('Edge Cases', function () {
    it('should handle non-numeric limit values', async function () {
      const { body } = await agent.get('posts/?limit=invalid').expectStatus(200);
    ...
    it('should handle limit=0', async function () {
      const { body } = await agent.get('posts/?limit=0').expectStatus(200);
