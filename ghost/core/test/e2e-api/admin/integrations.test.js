...
describe('Integrations API', function () {
  ...
  it('Can browse all integrations', async function () {
    const res = await request
      .get(localUtils.API.getApiQuery(`integrations/`))
    ...
    // There is no enforced order for integrations.
    ...
    const selfServeMigrationIntegration = _.find(res.body.integrations, {
    ...
    await request
      .get(
        localUtils.API.getApiQuery(
          `integrations/${testUtils.DataGenerator.Content.integrations[1].id}/`,
        ),
      )
      .set('Origin', config.get('url'))
      .expect('Content-Type', /json/)
  ...
  it('Can successfully create a single integration with auto generated content and admin api key', async function () {
    const res = await request
      .post(localUtils.API.getApiQuery('integrations/'))
      .set('Origin', config.get('url'))
      .send({
        integrations: [
          {
    ...
    assert.equal(
      ...
      `/ghost/api/admin/integrations/${res.body.integrations[0].id}/`,
    );
  });
  ...
  it('Can successfully get a created integration', async function () {
    ...
    const res2 = await request
      .get(localUtils.API.getApiQuery(`integrations/${createdIntegration.id}/`))
      .set('Origin', config.get('url'))
      .expect('Content-Type', /json/)
      .expect('Cache-Control', testUtils.cacheRules.private)
      .expect(200);
  ...
  it('Can successfully get *all* created integrations with api_keys', async function () {
    ...
    const res = await request
      .get(localUtils.API.getApiQuery(`integrations/?include=api_keys&limit=all`))
      .set('Origin', config.get('url'))
      .expect('Content-Type', /json/)
      .expect('Cache-Control', testUtils.cacheRules.private)
      .expect(200);
      ...
      if (integration.api_keys.length) {
        integration.api_keys.forEach((apiKey) => {
          ...
          if (apiKey.type === 'content') {
            assert.equal(
              apiKey.secret.split(':').length,
              1,
              `${integration.name} api key secret should have correct key format without ":"`,
            );
          } else if (apiKey.type === 'admin') {
            assert.equal(
              apiKey.secret.split(':').length,
              2,
              `${integration.name} api key secret should have correct key format with ":"`,
            );
          }
        });
      }
  ...
  it('Can successfully edit a created integration', async function () {
    ...
    await request
      .put(localUtils.API.getApiQuery(`integrations/${createdIntegration.id}/`))
      .set('Origin', config.get('url'))
      .send({
        integrations: [
          {
  ...
  it('Can successfully refresh an integration api key', async function () {
    const res = await request
      .post(localUtils.API.getApiQuery('integrations/?include=api_keys'))
      .set('Origin', config.get('url'))
      .send({
        integrations: [
          {
    ...
    await request
      .post(
        localUtils.API.getApiQuery(
          `integrations/${createdIntegration.id}/api_key/${adminApiKey.id}/refresh`,
        ),
      )
      .set('Origin', config.get('url'))
      .send({
    ...
    const res2 = await request
      .get(localUtils.API.getApiQuery(`integrations/${createdIntegration.id}/?include=api_keys`))
      .set('Origin', config.get('url'))
      .expect('Content-Type', /json/)
      .expect('Cache-Control', testUtils.cacheRules.private)
      .expect(200);
    ...
    const res3 = await request
      .get(
        localUtils.API.getApiQuery(`actions/?filter=resource_id:'${adminApiKey.id}'&include=actor`),
      )
      .set('Origin', config.get('url'))
      .expect('Content-Type', /json/)
      .expect('Cache-Control', testUtils.cacheRules.private)
  ...
  it('Can successfully add and delete a created integrations webhooks', async function () {
    ...
    const res2 = await request
      .get(localUtils.API.getApiQuery(`integrations/${createdIntegration.id}/?include=webhooks`))
      .set('Origin', config.get('url'))
      .expect('Content-Type', /json/)
      .expect('Cache-Control', testUtils.cacheRules.private)
      .expect(200);
  ...
  it('Can successfully delete a created integration', async function () {
    ...
    await request
      .del(localUtils.API.getApiQuery(`integrations/${createdIntegration.id}/`))
      .set('Origin', config.get('url'))
      .expect(204)
      .expect(({ body }) => {
