...
describe('Tiers API', function () {
  ...
  it('Can read Tiers', async function () {
    ...
    await agent.get(`/tiers/${tier.id}/`).expectStatus(200);
  });
  ...
  it('Can edit visibility', async function () {
    ...
    } = await agent.get('/tiers/?filter=type:paid&limit=1');
    ...
    assert(
      updatedTier.visibility === visibility,
      `The visibility of the Tier should have been updated to ${visibility}`,
    );
  });
  ...
  it('Can edit tier properties and relations', async function () {
    let {
      ...
      .get('/tiers/?filter=type:paid&limit=1')
      .expectStatus(200)
      .matchHeaderSnapshot({
