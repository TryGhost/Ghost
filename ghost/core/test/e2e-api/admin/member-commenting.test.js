...
describe('Member Commenting API', function () {
  ...
  describe('Member API responses with commenting data', function () {
    it('New members have can_comment: true by default', async function () {
      const { body } = await agent.get(`members/${member.id}`).expectStatus(200);
  ...
  describe('Actions/audit log', function () {
    ...
    it('Enable action is logged with the staff member as actor', async function () {
      ...
      // Then enable
      await agent.post(`members/${member.id}/commenting/enable`).expectStatus(200);
...
describe('Member Commenting Service Behavior', function () {
  ...
  it('Member with expired disable has can_comment: true', async function () {
    ...
    const { body } = await adminAgent.get(`members/${member.id}`).expectStatus(200);
  ...
  it('Member with commenting enabled has can_comment: true again', async function () {
    ...
    // Enable commenting
    response = await adminAgent.post(`members/${member.id}/commenting/enable`).expectStatus(200);
...
describe('Member with Commenting Disabled - Comment Restriction', function () {
  ...
  it('Member with commenting disabled cannot unlike a comment', async function () {
    ...
    await membersAgent.post(`/api/comments/${comment.id}/like/`).expectStatus(204);
  ...
  it('Disabling with hide_comments hides member comments from the public API', async function () {
    ...
    const commentsBefore = beforeResponse.body.comments.filter((c) => c.member?.id === member.id);
    ...
    const afterResponse = await membersAgent.get(`/api/comments/post/${postId}/`).expectStatus(200);

    const commentsAfter = afterResponse.body.comments.filter((c) => c.member?.id === member.id);
  ...
  it('Disabling without hide_comments keeps member comments visible', async function () {
    ...
    const response = await membersAgent.get(`/api/comments/post/${postId}/`).expectStatus(200);

    const memberComments = response.body.comments.filter((c) => c.member?.id === member.id);
  ...
  it('Member with commenting re-enabled can comment again', async function () {
    ...
    // Enable commenting
    await adminAgent.post(`members/${member.id}/commenting/enable`).expectStatus(200);
