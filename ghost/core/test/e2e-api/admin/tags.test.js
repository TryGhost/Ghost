...
describe('Tag API', function () {
    ...
    const res = await request
      .get(
        localUtils.API.getApiQuery(
          `tags/${testUtils.getExistingData().tags[0].id}/?include=count.posts`,
        ),
      )
      .set('Origin', config.get('url'))
      .expect('Content-Type', /json/)
  ...
  it('Counts posts, not posts_tags rows', async function () {
    ...
    const existing = await models.Base.knex('posts_tags').where('tag_id', tagId).select('post_id');
    ...
    try {
      assert.equal(
        await readCount(),
        before,
        'Expect duplicate posts_tags rows not to inflate count.posts',
      );
    } finally {
      await models.Base.knex('posts_tags')
        .whereIn(
          'id',
          redundant.map((r) => r.id),
        )
        .del();
    }
  });
  ...
  it('Can add a tag', async function () {
    ...
    assert.equal(
      ...
      `/ghost/api/admin/tags/${res.body.tags[0].id}/`,
    );
  });
  ...
  it('Can edit a tag', async function () {
    const res = await request
      ...
      .send({
        tags: [
          Object.assign({}, testUtils.getExistingData().tags[0], {
            description: 'hey ho ab ins klo',
          }),
        ],
      })
      .expect('Content-Type', /json/)
