...
const { anyObjectId, anyErrorId, anyISODateTime, anyContentVersion, anyLocationFor, anyEtag } =
  matchers;
...
const {
  ...
  ClickEvent,
  SubscribeEvent,
} = require('../../../core/server/services/recommendations/service');
...
async function addClicksAndSubscribers({ memberId }) {
  const recommendations = await recommendationsService.repository.getAll({
...
describe('Recommendations Admin API', function () {
  ...
  describe('browse', function () {
    ...
    it('Can request pages', async function () {
      ...
      const { body: page2 } = await agent
        .get('recommendations/?page=2&limit=10')
        .expectStatus(200)
        .matchHeaderSnapshot({
          'content-version': anyContentVersion,
          etag: anyEtag,
    ...
    it('Can include only clicks', async function () {
      ...
      const { body: page1 } = await agent
        .get('recommendations/?include=count.clicks')
        .expectStatus(200)
        .matchHeaderSnapshot({
          'content-version': anyContentVersion,
          etag: anyEtag,
    ...
    it('Can include only subscribers', async function () {
      ...
      const { body: page1 } = await agent
        .get('recommendations/?include=count.subscribers')
        .expectStatus(200)
        .matchHeaderSnapshot({
          'content-version': anyContentVersion,
          etag: anyEtag,
    ...
    it('Can include click and subscribe counts and order by clicks+subscribe count', async function () {
      ...
      const { body: page1 } = await agent
        .get(
          'recommendations/?include=count.clicks,count.subscribers&order=' +
            encodeURIComponent('count.clicks desc, count.subscribers asc'),
        )
        .expectStatus(200)
        .matchHeaderSnapshot({
          'content-version': anyContentVersion,
    ...
    it('can fetch recommendations filtered by an exact title', async function () {
      ...
      const { body } = await agent
        .get(`recommendations/?filter=title:'Recommendation 1'`)
        .expectStatus(200);
    ...
    it('can fetch recommendations filtered by a partial URL', async function () {
      ...
      const { body } = await agent
        .get(`recommendations/?filter=url:~'recommendation1.com'`)
        .expectStatus(200);
  ...
  describe('read', function () {
    it('can get a recommendation by ID', async function () {
      ...
      assert.equal(
        body.recommendations[0].featured_image,
        'https://recommendation1.com/featured.jpg',
      );
  ...
  describe('edit', function () {
    ...
    it('Can edit some fields of a recommendation without changing others', async function () {
      ...
      assert.equal(
        body.recommendations[0].featured_image,
        'https://recommendation0.com/featured.jpg',
      );
      ...
      await agent
        ...
        .body({
          recommendations: [
            {
              title:
                'Recommendation 3 with the same hostname and pathname, but with different protocol, www, query params and hash fragement',
  ...
  describe('check', function () {
    ...
    it('Returns nullified values if site fails to fetch', async function () {
      nock('https://dogpictures.com').get('/').reply(404);
