...
describe('Posts API', function () {
  ...
  it('Can retrieve all posts', async function () {
    ...
    assert.match(
      new URL(jsonResponse.posts[0].url).pathname,
      /\/p\/[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}/,
    );
    assert.equal(new URL(jsonResponse.posts[2].url).pathname, '/welcome/');
    assert.equal(
      new URL(jsonResponse.posts[13].feature_image).pathname,
      '/content/images/2018/hey.jpg',
    );
    ...
    assert.equal(
      jsonResponse.posts[14].newsletter.id,
      testUtils.DataGenerator.Content.newsletters[0].id,
    );
    ...
    const res = await request
      .get(
        localUtils.API.getApiQuery(
          'posts/?formats=plaintext,mobiledoc,lexical&limit=3&order=title%20ASC',
        ),
      )
      .set('Origin', config.get('url'))
      .expect('Content-Type', /json/)
  ...
  it('Can include single relation', async function () {
    const res = await request
      .get(localUtils.API.getApiQuery('posts/?include=tags'))
      .set('Origin', config.get('url'))
      .expect('Content-Type', /json/)
      .expect('Cache-Control', testUtils.cacheRules.private)
      .expect(200);
    ...
    localUtils.API.checkResponse(jsonResponse, 'posts');
    assert.equal(jsonResponse.posts.length, 15);
    localUtils.API.checkResponse(jsonResponse.posts[0], 'post', null, [
      'authors',
      'primary_author',
      'email',
      'tiers',
  ...
  it('Can filter posts', async function () {
    const res = await request
      .get(localUtils.API.getApiQuery('posts/?filter=featured:true'))
      .set('Origin', config.get('url'))
      .expect('Content-Type', /json/)
      .expect('Cache-Control', testUtils.cacheRules.private)
      .expect(200);
  ...
  it('Returns a validation error when unknown filter key is used', async function () {
    ...
    await request
      .get(localUtils.API.getApiQuery('posts/?filter=page:true'))
      .set('Origin', config.get('url'))
      .expect('Content-Type', /json/)
      .expect('Cache-Control', testUtils.cacheRules.private)
      .expect(400);
  ...
  it('Can paginate posts', async function () {
    const res = await request
      .get(localUtils.API.getApiQuery('posts/?page=2'))
      .set('Origin', config.get('url'))
      .expect('Content-Type', /json/)
      .expect('Cache-Control', testUtils.cacheRules.private)
      .expect(200);
  ...
  it('Can request a post by id', async function () {
    ...
    // Check if the newsletter relation is loaded by default and newsletter_id is not returned
    assert.equal(
      jsonResponse.posts[0].newsletter.id,
      testUtils.DataGenerator.Content.newsletters[0].id,
    );
    assert.equal(jsonResponse.posts[0].newsletter_id, undefined);
  });
  ...
  it('Can retrieve a post by slug', async function () {
    const res = await request
      .get(localUtils.API.getApiQuery('posts/slug/welcome/'))
      .set('Origin', config.get('url'))
      .expect('Content-Type', /json/)
      .expect('Cache-Control', testUtils.cacheRules.private)
      .expect(200);
      ...
      .get(
        localUtils.API.getApiQuery(
          'posts/' +
            ...
            '/?include=authors,tags,email,tiers,newsletter,post_revisions',
        ),
      )
      .set('Origin', config.get('url'))
      .expect('Content-Type', /json/)
  ...
  it('Can add a post', async function () {
    ...
    assert.match(
      new URL(res.body.posts[0].url).pathname,
      /\/p\/[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}/,
    );
    assert.equal(res.headers['x-cache-invalidate'], undefined);
    ...
    assert.equal(
      ...
      `/ghost/api/admin/posts/${res.body.posts[0].id}/`,
    );
  ...
  it('Can update and force re-render', async function () {
    ...
    const res2 = await request
      .put(
        localUtils.API.getApiQuery(
          'posts/' +
            testUtils.DataGenerator.Content.posts[3].id +
            '/?force_rerender=true&formats=mobiledoc,lexical,html',
        ),
      )
      .set('Origin', config.get('url'))
      .send({ posts: [post] })
  ...
  it(`Can't change the newsletter of a post from the post body`, async function () {
    ...
    assert.equal(
      modelBefore.get('newsletter_id'),
      null,
      'This test requires the initial post to not have a newsletter',
    );
  ...
  it('Cannot send to an archived newsletter', async function () {
    ...
    assert.equal(
      testUtils.DataGenerator.Content.newsletters[2].status,
      'archived',
      'This test expects an archived newsletter in the test fixtures',
    );
  ...
  it('Does not change post status when email sending fails', async function () {
    ...
    const checkCanSendEmailStub = sinon.stub(emailService.service, 'checkCanSendEmail').rejects(
    ...
    assert.equal(
      model.get('status'),
      'scheduled',
      'Post should remain scheduled when email sending fails',
    );
  ...
  it('Can publish an email_only post with free filter', async function () {
    ...
    const publishedRes = await request
      .put(
        localUtils.API.getApiQuery(
          'posts/' + id + '/?newsletter=' + newsletterSlug + '&email_segment=status%3Afree',
        ),
      )
      .set('Origin', config.get('url'))
      .send({ posts: [updatedPost] })
  ...
  it('Can publish a scheduled post with custom email segment', async function () {
    ...
    const scheduledRes = await request
      .put(
        localUtils.API.getApiQuery(
          'posts/' + id + '/?newsletter=' + newsletterSlug + '&email_segment=status:free',
        ),
      )
      .set('Origin', config.get('url'))
      .send({ posts: [updatedPost] })
  ...
  it('Cannot schedule an email only post without newsletter reference', async function () {
    ...
    await request
      ...
      .expect((result) => {
        ...
        assert.equal(
          result.body.errors[0].context,
          'Scheduling an email requires a newsletter reference.',
        );
      });
  });
  ...
  it("Can't change the newsletter once it has been sent", async function () {
    ...
    const res2 = await request
      .put(
        localUtils.API.getApiQuery(
          'posts/' + id + '/?email_segment=status:-free&newsletter=' + newsletterSlug,
        ),
      )
      .set('Origin', config.get('url'))
      .send({ posts: [updatedPost] })
  ...
  it('Cannot get post via pages endpoint', async function () {
    await request
      .get(localUtils.API.getApiQuery(`pages/${testUtils.DataGenerator.Content.posts[3].id}/`))
      .set('Origin', config.get('url'))
      .expect('Content-Type', /json/)
      .expect('Cache-Control', testUtils.cacheRules.private)
      .expect(404);
  ...
  it('Cannot update post via pages endpoint', async function () {
    ...
    await request
      .put(localUtils.API.getApiQuery('pages/' + testUtils.DataGenerator.Content.posts[3].id))
      .set('Origin', config.get('url'))
      .send({ pages: [post] })
      .expect('Content-Type', /json/)
      .expect('Cache-Control', testUtils.cacheRules.private)
