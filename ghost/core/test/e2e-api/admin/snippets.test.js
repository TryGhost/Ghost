...
const { anyContentVersion, anyEtag, anyLocationFor, anyObjectId, anyISODateTime, anyErrorId } =
  matchers;
  ...
  describe('URL transformations', function () {
    ...
    it('Can read Mobiledoc snippet with all URLs as absolute site URLs', async function () {
      ...
      assert.equal(
        ...
        `${siteUrl}/content/images/snippet-inline.jpg`,
      );
      assert.equal(
        mobiledoc.cards.find((c) => c[0] === 'file')[1].src,
        `${siteUrl}/content/files/snippet-document.pdf`,
      );
      assert.equal(
        mobiledoc.cards.find((c) => c[0] === 'video')[1].src,
        `${siteUrl}/content/media/snippet-video.mp4`,
      );
      assert.equal(
        mobiledoc.cards.find((c) => c[0] === 'audio')[1].src,
        `${siteUrl}/content/media/snippet-audio.mp3`,
      );
    ...
    it('Can read Mobiledoc snippet with CDN URLs when configured', async function () {
      ...
      assert.equal(
        mobiledoc.cards.find((c) => c[0] === 'image')[1].src,
        `${cdnUrl}/content/images/snippet-inline.jpg`,
      );
      assert.equal(
        mobiledoc.cards.find((c) => c[0] === 'file')[1].src,
        `${cdnUrl}/content/files/snippet-document.pdf`,
      );
      assert.equal(
        mobiledoc.cards.find((c) => c[0] === 'video')[1].src,
        `${cdnUrl}/content/media/snippet-video.mp4`,
      );
      assert.equal(
        mobiledoc.cards.find((c) => c[0] === 'audio')[1].src,
        `${cdnUrl}/content/media/snippet-audio.mp3`,
      );
      // Video/audio thumbnails use CDN URL
      assert.equal(
        mobiledoc.cards.find((c) => c[0] === 'video')[1].thumbnailSrc,
        `${cdnUrl}/content/images/snippet-video-thumb.jpg`,
      );
      assert.equal(
        mobiledoc.cards.find((c) => c[0] === 'audio')[1].thumbnailSrc,
        `${cdnUrl}/content/images/snippet-audio-thumb.jpg`,
      );
      assert(!snippet.mobiledoc.includes('__GHOST_URL__'));
    });
