...
describe(`Admin Comments API`, function () {
  ...
  describe('Hide', function () {
    it('Can hide comments', async function () {
      const commentToHide = await dbFns.addComment({
      ...
      } = await getMemberComments(`/api/comments/${commentToHide.id}/`);
      ...
      assert.equal(
        ...
        `/api/members/comments/post/${postId}/, /api/members/comments/${commentToHide.id}/`,
      );

      const {
        body: {
          comments: [afterHiding],
        },
      } = await getMemberComments(`/api/comments/${commentToHide.id}/`);
  ...
  describe('Pin', function () {
    it('Can pin and unpin top-level comments', async function () {
      ...
      assert.equal(
        pinRes.headers['x-cache-invalidate'],
        `/api/members/comments/post/${postId}/, /api/members/comments/${commentToPin.id}/`,
      );
      ...
      assert.equal(
        unpinRes.headers['x-cache-invalidate'],
        `/api/members/comments/post/${postId}/, /api/members/comments/${commentToPin.id}/`,
      );
    ...
    it('Cannot pin deleted comments', async function () {
      ...
      await adminApi
        .put(`comments/${deletedComment.id}/`)
        .body({
          comments: [
            {
  ...
  describe('browse by post', function () {
    ...
    it('orders pinned comments first for front-end staff browsing', async function () {
      ...
      const res = await adminApi
        .get('/comments/post/' + postId + '/?order=' + encodeURIComponent('created_at asc'))
        .expectStatus(200);

      assert.deepEqual(
        res.body.comments.map((comment) => comment.id),
        [pinnedNewer.id, pinnedOlder.id, unpinnedOlder.id, unpinnedNewer.id],
      );
    });
    ...
    it('preserves pinned state when fields are requested', async function () {
      ...
      const res = await adminApi
        .get('/comments/post/' + postId + '/?fields=id,pinned')
        .expectStatus(200);
    ...
    it('includes deleted reply tombstones when they have visible descendants', async function () {
      ...
      assert.deepEqual(
        ...
        [deletedReply.get('id'), hiddenReply.get('id')],
      );
    ...
    it('reply endpoint paginates deleted tombstones before the visible descendant that makes them displayable', async function () {
      ...
      assert.deepEqual(
        firstPage.body.comments.map((reply) => reply.id),
        [deletedReply.get('id')],
      );
      ...
      assert.deepEqual(
        secondPage.body.comments.map((reply) => reply.id),
        [hiddenReply.get('id')],
      );
    ...
    it('reply endpoint excludes deleted leaf replies while keeping visible siblings', async function () {
      ...
      assert.deepEqual(
        replies.map((reply) => reply.id),
        [hiddenReply.get('id'), publishedReply.get('id')],
      );
  ...
  describe('Logged in member gets own likes via admin api', function () {
      ...
      await membersApi
        ...
        .matchHeaderSnapshot({
          ...
          'x-cache-invalidate': stringMatching(
            new RegExp(
              '/api/members/comments/post/[0-9a-f]{24}/, /api/members/comments/[0-9a-f]{24}/$',
            ),
          ),
        })
        .expectEmptyBody();
    ...
    it('can get comment liked status by impersonating member via admin browse route', async function () {
      ...
      const res = await adminApi.get(
        `/comments/post/${post.id}/?impersonate_member_uuid=${fixtureManager.get('members', 1).uuid}`,
      );
    ...
    it('can get comment liked status by impersonating member via admin get by comment id read route', async function () {
      const res = await adminApi.get(
        `/comments/${comment.get('id')}/?impersonate_member_uuid=${fixtureManager.get('members', 1).uuid}`,
      );
      assert.equal(res.body.comments[0].liked, true);
    });
    ...
    it('can get comment liked status by impersonating member via admin get by comment replies route', async function () {
      ...
      const res = await adminApi.get(
        `/comments/${parent.get('id')}/replies/?impersonate_member_uuid=${fixtureManager.get('members', 1).uuid}`,
      );
      assert.equal(res.body.comments[0].liked, true);
    });
  });
  ...
  describe('Add comments via Admin API', function () {
    ...
    it('Ignores in_reply_to_id when no parent_id is specified', async function () {
      ...
      assert.equal(
        comment.html,
        '<p>This is a top-level comment that incorrectly references another comment</p>',
      );
      assert.equal(comment.status, 'published');
  ...
  describe('Browse All', function () {
    ...
    it('Returns only top-level with include_nested=false', async function () {
      ...
      await adminApi
        .get('/comments/?include_nested=false')
        .expectStatus(200)
        .matchBodySnapshot({
    ...
    it('Can filter by status', async function () {
      ...
      await adminApi
        .get('/comments/?filter=' + encodeURIComponent('status:hidden'))
        .expectStatus(200)
        .matchBodySnapshot({
          comments: [commentMatcher],
        });
    ...
    it('Can filter by member_id', async function () {
      ...
      await adminApi
        .get(
          '/comments/?filter=' +
            encodeURIComponent(`member_id:'${fixtureManager.get('members', 0).id}'`),
        )
        .expectStatus(200)
        .matchBodySnapshot({
          comments: [commentMatcher],
    ...
    it('Keeps requested ordering for pinned comments', async function () {
      ...
      const res = await adminApi
        .get('/comments/?order=' + encodeURIComponent('created_at asc'))
        .expectStatus(200);

      assert.deepEqual(
        res.body.comments.map((comment) => comment.id),
        [unpinnedOlder.id, pinnedOlder.id, pinnedNewer.id, unpinnedNewer.id],
      );
      assert.deepEqual(
        res.body.comments.map((comment) => comment.pinned),
    ...
    it('Can filter for highly reported comments using count.reports:>=2', async function () {
      ...
      const res = await adminApi.get(
        '/comments/?filter=' + encodeURIComponent('count.reports:>=2'),
      );
      assert.equal(res.body.comments.length, 1);
  ...
  describe('Comment Reports', function () {
    ...
    it('Can browse reporters for a comment', async function () {
      ...
      await adminApi
        .get(`/comments/${comment.id}/reports/`)
        .expectStatus(200)
        .matchBodySnapshot({
    ...
    it('Returns empty list for comment with no reports', async function () {
      ...
      await adminApi.get(`/comments/${comment.id}/reports/`).expectStatus(200).matchBodySnapshot({
        comment_reports: [],
      });
    });
    ...
    it('Returns 404 for non-existent comment', async function () {
      ...
      await adminApi
        .get(`/comments/${fakeCommentId}/reports/`)
        .expectStatus(404)
        .matchBodySnapshot({
          errors: [
            {
  ...
  describe('Comment Likes', function () {
    ...
    it('Returns empty list for comment with no likes', async function () {
      ...
      await adminApi.get(`/comments/${comment.id}/likes/`).expectStatus(200).matchBodySnapshot({
        comment_likes: [],
      });
    });
    ...
    it('Returns 404 for non-existent comment', async function () {
      ...
      await adminApi
        .get(`/comments/${fakeCommentId}/likes/`)
        .expectStatus(404)
        .matchBodySnapshot({
          errors: [
            {
  ...
  describe('Comment Dislikes', function () {
    ...
    it('Can browse comment dislikes', async function () {
      ...
      await adminApi
        .get(`/comments/${comment.id}/dislikes/`)
        .expectStatus(200)
        .matchBodySnapshot({
