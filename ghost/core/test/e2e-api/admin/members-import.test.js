...
describe('Members import', function () {
  ...
  beforeAll(async function () {
    ...
    assert.ok(
      subscribeOnSignupCount > 0,
      'needs at least one subscribe_on_signup newsletter fixture',
    );
  });
  ...
  it('imports nothing from a column the mapping empties', async function () {
    ...
    assert.ok(
      !member.name,
      `the emptied column did not fill the name, got ${JSON.stringify(member.name)}`,
    );
  ...
  it('keeps a form label with a comma whole but splits a row label on commas', async function () {
    ...
    assert.ok(
      names1.includes('Bristol, UK'),
      `form label kept whole, got ${JSON.stringify(names1)}`,
    );
  ...
  it('rejects a gift row that also specifies a tier', async function () {
    ...
    assert.match(
      ...
      /Cannot specify both gift_id and import_tier/,
    );
  });
  ...
  it('rejects a gift row that also specifies a complimentary plan', async function () {
    ...
    assert.match(
      res.body.meta.stats.invalid[0].error,
      /Cannot specify both gift_id and complimentary_plan/,
    );
  });
  ...
  it('applies newsletter subscription rules on re-import', async function () {
    ...
    assert.equal(
      neverHad.newsletters.length,
      0,
      'no newsletters → not re-subscribed by a later true',
    );

    const kept = await findMember('member+nl_c@example.com');
    assertExists(kept);
    assert.equal(
      kept.newsletters.length,
      subscribeOnSignupCount,
      'still subscribed → newsletters kept',
    );
  });
  ...
  it('defers over the threshold and emails a completion report', async function () {
    ...
    assert.ok(
      email.attachments[0].content.includes('not-a-valid-email'),
      'error report lists the failed row',
    );
  });
});
