...
describe('Search-scoped automation counts API', () => {
  ...
  it('returns chart buckets in the requested timezone, including all-time search', async () => {
    await seed(3);
    const tb = mockCounts([
      {
        ...zero,
    ...
    const response = await read({ include_entries: 'true', timezone: 'America/New_York' });
    assert.deepEqual(response.meta.entry_buckets, {
      version: 1,
    ...
    assert.equal(tb.requests[0].include_entries, 'true');
    assert.equal(tb.requests[0].timezone, 'America/New_York');
    assert.equal(tb.requests[0].date_from, undefined);
  });
  it('rejects invalid chart options and chart cursors reused without their timezone or capability', async () => {
    await read({ include_entries: 'yes' }, 422);
    await read({ include_entries: 'true', search: '' }, 422);
    const scope = countCursorScope(automationId, site, 'anna', {}, 'America/New_York');
    const token = encodeCountCursor(
      { scope, after: id(1), upper: id(99), counts: zero, expires: Date.now() + 900000 },
      settingsCache.get('admin_session_secret'),
    );
    await read({ include_entries: 'true', timezone: 'UTC', cursor: token }, 422);
    await read({ timezone: 'America/New_York', cursor: token }, 422);
  });
  ...
  it('returns only completed totals across multiple signed continuations and retries', async () => {
    ...
    assert.ok(tb.requests.every((p) => p.run_ids.split(',').filter(Boolean).length <= 30000));
  });
  it.each(['%', '_', '!', '\\', 'current@'])(
