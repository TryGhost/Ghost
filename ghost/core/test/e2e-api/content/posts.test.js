...
const { anyArray, anyContentVersion, anyErrorId, anyEtag, anyUuid, anyISODateTimeWithTZ } =
  matchers;
...
const postMatcherShallowIncludes = Object.assign({}, postMatcher, {
  tags: anyArray,
  authors: anyArray,
});
...
describe('Posts Content API', function () {
  ...
  it('Can request posts', async function () {
    ...
    assert.match(
      res.body.posts[9].html,
      new RegExp(`<a href="${escapedUrl}/about#nowhere" title="Relative URL`),
    );
    assert.equal(res.body.posts[11].slug, 'ghostly-kitchen-sink');
    assert.match(
      res.body.posts[11].html,
      new RegExp(`<img src="${escapedUrl}/content/images/lol.jpg"`),
    );
  });
  ...
  it('Can filter posts by tag', async function () {
    const res = await agent
      .get('posts/?filter=tag:kitchen-sink,featured:true&include=tags')
      .expectStatus(200)
      .matchHeaderSnapshot({
        'content-version': anyContentVersion,
        etag: anyEtag,
      ...
      .matchBodySnapshot({
        posts: new Array(4).fill(postMatcher),
      });
  ...
  it('Can filter posts by authors', async function () {
    ...
    assert.equal(
      jsonResponse.posts[0].slug,
      'welcome',
      'The API orders by number of matched authors, then by published_at desc, then by id desc',
    );
  ...
  it('Can request the url of a single post with a narrowed fields list', async function () {
    ...
    const { body } = await agent.get('posts/slug/welcome/?fields=title,url').expectStatus(200);
  ...
  it('Can include relations', async function () {
    await agent
      ...
      .matchBodySnapshot({
        posts: new Array(13).fill(postMatcherShallowIncludes),
      });
  });
  ...
  it('Can filter by published date', async function () {
    ...
    const res = await agent
      ...
      .matchBodySnapshot({
        posts: new Array(1).fill(postMatcher),
      });
  ...
  it('Can include only paid tier for paid post', async function () {
    ...
    const paidPostRes = await agent.get(`posts/${paidPost.id}/?include=tiers`).expectStatus(200);
  ...
  it('Can include specific tier for post with tiers visibility', async function () {
    ...
    const tiersPostRes = await agent.get(`posts/${tiersPost.id}/?include=tiers`).expectStatus(200);
  ...
  it('Can use post excerpt as field', async function () {
    await agent.get(`posts/?fields=excerpt`).expectStatus(200).matchBodySnapshot();
  });

  it('Can use post plaintext as field', async function () {
    await agent.get(`posts/?fields=plaintext`).expectStatus(200).matchBodySnapshot();
  });
  ...
  it('Adds ?ref tags', async function () {
    const post = await models.Post.add(
      {
        ...
        mobiledoc: JSON.stringify({
          ...
          atoms: [],
          cards: [
            ['html', { html: '<a href="https://example.com">Link</a><a href="invalid">Test</a>' }],
          ],
          markups: [],
          sections: [
            [10, 0],
    ...
    let response = await agent.get(`posts/${post.id}/`).expectStatus(200);
    assert(
      response.body.posts[0].html.includes(
        '<a href="https://example.com/?ref=127.0.0.1">Link</a><a href="invalid">Test</a>',
      ),
      'Html not expected (should contain ?ref): ' + response.body.posts[0].html,
    );

    // Disable outbound link tracking
    mockManager.mockSetting('outbound_link_tagging', false);
    response = await agent.get(`posts/${post.id}/`).expectStatus(200);
    assert(
      response.body.posts[0].html.includes(
        '<a href="https://example.com">Link</a><a href="invalid">Test</a>',
      ),
      'Html not expected: ' + response.body.posts[0].html,
    );
  });
  ...
  it('Strips out gated blocks not viewable by anonymous viewers ', async function () {
            ...
            children: [
              {
                type: 'html',
                version: 1,
                html: '<p>Visible to free/paid members</p>',
                visibility: {
              ...
              },
              {
                type: 'html',
                version: 1,
                html: '<p>Visible to anonymous viewers</p>',
                visibility: {
    ...
    const response = await agent.get(`posts/${post.id}/`).expectStatus(200);
