...
describe('Settings Content API', function () {
  ...
  it('Includes GA labs flags in settings response', async function () {
    const { body } = await agent.get('settings/').expectStatus(200);
    ...
    for (const key of labs.GA_KEYS) {
      assert.equal(
        settingsLabs[key],
        true,
        `Expected labs.${key} to be true in Content API settings`,
      );
    }
    ...
    for (const [key, value] of Object.entries(allLabs)) {
      assert.equal(
        settingsLabs[key],
        value,
        `Expected labs.${key} to be ${value} in Content API settings`,
      );
    }
  });
});
