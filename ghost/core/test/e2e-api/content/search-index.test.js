...
describe('Search Index Content API', function () {
  ...
  describe('fetchPosts', function () {
    ...
    it('should return a list of posts', async function () {
      ...
      assert.equal(
        post.plaintext,
        undefined,
        'plaintext field should be not included in the response',
      );
      assert.equal(
        post.mobiledoc,
        undefined,
        'mobiledoc field should be not included in the response',
      );
  ...
  describe('fetchAuthors', function () {
    ...
    it('should return a list of authors', async function () {
      await agent
        ...
        .matchBodySnapshot({
          authors: new Array(2).fill(searchIndexAuthorMatcher),
        });
    });
  });
  ...
  describe('fetchTags', function () {
    ...
    it('does not return internal tags', async function () {
      ...
      assert.equal(
        tags.body.tags.find((tag) => tag.id === internalTag.id),
        undefined,
        'should not include internal tag',
      );
    });
  });
});
