...
describe('Gift Subscriptions', function () {
  ...
  describe('Purchase a gift', function () {
    it('Can purchase a gift as an anonymous visitor', async function () {
      ...
      const pendingGift = await models.Gift.findOne(
      ...
      assert.ok(
        getCheckoutGiftToken(checkoutSession),
        'Should have a gift token in the success URL',
      );
    ...
    it('waits for delayed payment success before completing a gift', async function () {
      ...
      await stripeMocker.sendWebhook({
        ...
        data: {
          object: { ...baseSession, payment_status: 'paid', payment_intent: 'pi_delayed_gift' },
        },
      });
      await DomainEvents.allSettled();
      await gift.refresh();
    ...
    it('traces a customized 3-month gift from checkout through redemption and member access', async function () {
      ...
      const redeemer = await models.Member.findOne(
        { email: redeemerEmail },
        {
          require: true,
    ...
    it('Includes gift token in the purchase success URL', async function () {
      ...
      assert.ok(
        successUrl.includes('stripe=gift-purchase-success'),
        'Success URL should contain stripe=gift-purchase-success',
      );
      assert.ok(
        successUrl.includes(`gift_token=${getCheckoutGiftToken(checkoutSession)}`),
        'Success URL should contain the gift token',
      );
    });
  ...
  describe('Check if a gift is redeemable', function () {
    ...
    it('returns 400 when the logged-in member already has a paid subscription', async function () {
      ...
      const { body } = await agent.get(`/api/gifts/${gift.get('token')}/redeem/`).expectStatus(400);
  ...
  describe('Redeem a gift', function () {
    describe('Logged-in member (immediate gift redemption)', function () {
      ...
      it('only redeems a gift once when two members redeem concurrently', async function () {
        ...
        const firstMember = await models.Member.findOne(
          { email: firstEmail },
          { require: true, withRelated: ['products'] },
        );
        const secondMember = await models.Member.findOne(
          { email: secondEmail },
          { require: true, withRelated: ['products'] },
        );
        const giftMembers = [firstMember, secondMember].filter(
          (member) => member.get('status') === 'gift',
        );
        const freeMembers = [firstMember, secondMember].filter(
          (member) => member.get('status') === 'free',
        );
    ...
    describe('Anonymous visitor (gift redemption via magic link)', function () {
      describe('New member', function () {
        it('signs up and redeems a gift during magic link confirmation', async function () {
          ...
          assert.equal(
            existingMember,
            null,
            'Member should not exist before magic link confirmation',
          );
          ...
          try {
            ...
            await models.WelcomeEmailAutomatedEmail.add({
              ...
              lexical: JSON.stringify({
                root: { children: [{ type: 'paragraph', children: [{ text: 'Welcome' }] }] },
              }),
            ...
            await models.WelcomeEmailAutomatedEmail.add({
              ...
              lexical: JSON.stringify({
                root: { children: [{ type: 'paragraph', children: [{ text: 'Welcome paid' }] }] },
              }),
              email_design_setting_id: emailDesignSetting.id,
            });
            ...
            const res = await membersAgent
              .get(`/?token=${token}&action=subscribe&r=${encodeURIComponent(redirectUrl.href)}`)
              .expectStatus(302)
            ...
            assert.equal(
              welcomeRuns.length,
              1,
              'Should enqueue exactly one welcome email automation run for gift member',
            );
            assert.equal(
      ...
      describe('Existing member', function () {
        it('signs in and redeems a gift during magic link confirmation', async function () {
          ...
          // Create member first
          const existingMember = await models.Member.add({
            email,
          ...
          try {
            ...
            assert.equal(
              runsBefore.length,
              0,
              'Existing free member should have no welcome email runs before redemption',
            );
            ...
            assert.equal(
              welcomeRuns.length,
              1,
              'Should enqueue exactly one welcome email automation run for an existing free member redeeming a gift',
            );
            assert.equal(
              welcomeRuns.models[0].get('welcome_email_automation_id'),
              paidWelcomeAutomation.id,
      ...
      it('fails gift redemption on a second magic link exchange attempt', async function () {
        ...
        try {
          ...
          const firstRes = await membersAgent
            .get(`/?token=${firstToken}&action=subscribe&r=${encodeURIComponent(redirectUrl.href)}`)
            .expectStatus(302)
            .expectHeader('Set-Cookie', /members-ssr.*/);
          ...
          const secondRes = await membersAgent
            .get(
              `/?token=${secondToken}&action=subscribe&r=${encodeURIComponent(redirectUrl.href)}`,
            )
            .expectStatus(302);
  ...
  describe('Continue a gift subscription as paid subscription', function () {
    ...
    async function setupGiftMember({
      cadence = 'year',
      duration = 1,
      consumesInDays = 30,
      tierId,
      ...
      const member = await membersService.api.members.create({
        email,
        name: `Gift Continuer ${continueSequence}`,
      });
      const identityToken = await membersService.api.getMemberIdentityToken(
    ...
    it('applies remaining gift days as trial and continues to checkout on the same tier/cadence', async function () {
      const { identityToken, gift, paidTier } = await setupGiftMember({
        cadence: 'month',
        duration: 3,
        consumesInDays: 30,
      });
      ...
      assert.ok(
        ...
        `Trial days should be approximately 30, got ${trialDays}`,
      );
    ...
    it('marks the gift as consumed once the paid subscription activates', async function () {
      ...
      const giftAfterActivation = await models.Gift.findOne(
    ...
    it('does not enqueue a second paid welcome email when a gift member upgrades to paid', async function () {
      ...
      try {
        ...
        const memberAfterActivation = await models.Member.findOne(
          { id: member.id },
          { require: true },
        );
  ...
  describe('Read a member with an active gift subscription', function () {
    it('returns gift price details and correct nickname on the member subscription', async function () {
      ...
      assert.equal(
        ...
        1,
        'Gift members should receive a single synthetic subscription',
      );
