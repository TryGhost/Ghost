...
describe('Members API - Member Offers', function () {
  ...
  describe('POST /members/api/member/offers', function () {
    it('returns 401 when not authenticated', async function () {
      await membersAgent.post('/api/member/offers').body({}).expectStatus(401);
    });
    ...
    it('returns null-tier retention offers for any paid member', async function () {
      const member = await models.Member.findOne(
        { email: 'paid@test.com' },
        {
          withRelated: ['stripeSubscriptions', 'stripeSubscriptions.stripePrice'],
        },
      );
    ...
    it('returns retention offers if subscription has an expired discount', async function () {
      ...
      try {
        ...
        // Clean up
        await subscription.save(
          { offer_id: null, discount_start: null, discount_end: null },
          { patch: true },
        );
