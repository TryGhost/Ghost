...
describe('Members Signin', function () {
  ...
  it('Will redirect to the paid welcome page for signup-paid', async function () {
    ...
    await membersAgent
      .get(`/?token=${token}&action=signup-paid`)
      .expectStatus(302)
  ...
  it('Will redirect to the free welcome page for subscribe', async function () {
    ...
    await membersAgent
      .get(`/?token=${token}&action=subscribe`)
      .expectStatus(302)
      .expectHeader('Location', /\/welcome-free\/\?success=true&action=signup$/)
      .expectHeader('Set-Cookie', /members-ssr.*/);
  });
  ...
  describe('Validity Period', function () {
    ...
    it('Expires a token after 10 minutes of first usage', async function () {
      ...
      assert.equal(
        model.get('first_used_at').getTime(),
        startDate.getTime(),
        'first_used_at should be set after first usage',
      );
      assert.equal(
        model.get('updated_at').getTime(),
        startDate.getTime(),
        'updated_at should be set on changes',
      );
      ...
      assert.equal(
        model.get('first_used_at').getTime(),
        startDate.getTime(),
        'first_used_at should not be changed on second usage',
      );
      ...
      assert.ok(
        updatedAtDrift < 60 * 1000,
        `updated_at should be ~5min after start (drift: ${updatedAtDrift}ms)`,
      );
      ...
      assert.equal(
        model.get('first_used_at').getTime(),
        startDate.getTime(),
        'first_used_at should not be changed',
      );
      assert.equal(
        model.get('updated_at').getTime(),
