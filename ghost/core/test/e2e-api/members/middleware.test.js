...
describe('Comments API', function () {
  ...
  describe('when not authenticated but enabled', function () {
    it('can not get member data', async function () {
      await membersAgent.get(`/api/member/`).expectStatus(204).expectEmptyBody();
    });
    ...
    it('can update comment notifications', async function () {
      ...
      assert.equal(
        member.get('enable_comment_notifications'),
        true,
        'This test requires the initial value to be true',
      );
    ...
    it('can update updates & announcements preference', async function () {
      ...
      assert.equal(
        member.get('enable_updates_and_announcements'),
        null,
        'This test requires the initial value to be null',
      );
  ...
  describe('when authenticated', function () {
    ...
    it("can remove a member's email from the suppression list", async function () {
      ...
      // remove suppression
      await membersAgent.delete(`/api/member/suppression`).expectStatus(204).expectEmptyBody();
