...
async function getSubscription(subscriptionId) {
  ...
  return await models['StripeCustomerSubscription']
...
async function assertMemberEvents({ eventType, memberId, asserts }) {
  ...
  assert.equal(
    events.length,
    asserts.length,
    `Only ${asserts.length} ${eventType} should have been added.`,
  );
}
...
describe('Members API', function () {
  ...
  describe('Handling the end of subscriptions', function () {
    ...
    it('Handles cancellation of paid subscriptions at the end of the billing cycle', async function () {
      ...
      assert.equal(
        initialMember.attribution.referrer_medium,
        'Ghost Admin',
        'The member should have been created via Ghost Admin',
      );
      ...
      assert.equal(
        memberEditedEvents.length,
        1,
        'A cancellation should emit exactly one member.edited',
      );
      assert.ok(
        memberEditedEvents[0]._changed.stripeSubscriptions,
      ...
      assert.equal(
        webhookBody.member.previous.subscriptions[0].cancel_at_period_end,
        false,
      ...
      assert.equal(
        webhookBody.member.current.subscriptions[0].price.amount,
        500,
        'The webhook payload subscriptions should include the price',
      );
      assert.equal(
        webhookBody.member.previous.subscriptions[0].price.amount,
        500,
    ...
    it('Includes every subscription in the previous payload when one of several is cancelled', async function () {
      ...
      const initialMember = await createMemberFromStripe();
      assert.equal(
        initialMember.subscriptions.length,
        2,
        'The member should start with two subscriptions',
      );
      ...
      assert.deepEqual(
        previousSubs.map((s) => s.id).sort(),
        currentSubs.map((s) => s.id).sort(),
        'previous and current must describe the same set of subscriptions',
      );

      assert.equal(
        currentSubs.find((s) => s.id === second_subscription_id).cancel_at_period_end,
        true,
        'current must show the cancelled subscription as cancelling',
      );
      assert.equal(
        currentSubs.find((s) => s.id === first_subscription_id).cancel_at_period_end,
        false,
        'current must show the untouched subscription unchanged',
      );
      assert.equal(
        previousSubs.find((s) => s.id === second_subscription_id).cancel_at_period_end,
        false,
        'previous must show the changed subscription in its prior state',
      );
      assert.equal(
        previousSubs.find((s) => s.id === first_subscription_id).cancel_at_period_end,
        false,
    ...
    it('Does not emit member.edited when a subscription update leaves cancel_at_period_end unchanged', async function () {
      ...
      assert.equal(
        initialMember.subscriptions.length,
        1,
        'The member must start with one linked subscription',
      );
      ...
      assert.equal(
        Math.floor(subscriptionRow.get('current_period_end').getTime() / 1000),
        renewedPeriodEnd,
    ...
    it('Can create a comlimentary subscription after canceling a paid subscription', async function () {
      ...
      assert.equal(
        ...
        2,
        'The member should have two subscriptions',
      );
  ...
  describe('checkout.session.completed', function () {
    ...
    it('Will create a member if one does not exist', async function () {
      ...
      await assertSubscription(member.subscriptions[0].id, {
        ...
        current_period_end: new Date(
          Math.floor(beforeNow / 1000) * 1000 + 60 * 60 * 24 * 31 * 1000,
        ),
        mrr: 500,
      });
    ...
    describe('Fields collected on the checkout page', function () {
      ...
      it('stops collecting for a tier that has turned it off, while another still does', async function () {
        ...
        await adminAgent.put(`/tiers/${secondId}/checkout_config/`).body({
          tiers_checkout_config: [
            {
              shipping: {
                collect: true,
    ...
    it('Will create a member with default newsletter subscriptions', async function () {
      ...
      {
        // ensure member didn't already exist
        const { body } = await adminAgent.get(
          '/members/?search=checkout-newsletter-default-test@email.com',
        );
        assert.equal(body.members.length, 0, 'A member already existed');
      }
    ...
    it('Will create a member with signup newsletter preference', async function () {
...

      {
        // ensure member didn't already exist
        const { body } = await adminAgent.get(
          '/members/?search=checkout-newsletter-test@email.com',
        );
        assert.equal(body.members.length, 0, 'A member already existed');
      }
      ...
      assert.equal(member.subscriptions.length, 1, 'The member should have a single subscription');
      assert.equal(
        member.newsletters.length,
        0,
        'The member should not have any newsletter subscription',
      );
    });
    ...
    it('Cancels Stripe-backed complimentary subscription when comped member completes a paid checkout', async function () {
      ...
      assert.equal(
        initialMember.subscriptions.length,
        1,
        'The member should have one Stripe subscription',
      );
    ...
    it('Removes Ghost-only comp tier when comped member completes a paid checkout', async function () {
      ...
      assert.equal(
        updatedMember.subscriptions.length,
        1,
        'The member should have one subscription',
      );
      assert.equal(
        updatedMember.subscriptions[0].status,
        'active',
        'The paid subscription should be active',
      );
    });
    ...
    it('Member becomes free (not comped) when paid subscription is cancelled after upgrading from comp', async function () {
      ...
      assert.equal(
        cancelledMember.status,
        'free',
        'The member should be free after cancellation, not comped',
      );
      assert.equal(cancelledMember.tiers.length, 0, 'The member should have no tiers');
      assert.equal(
        cancelledMember.subscriptions.length,
        1,
        'The member should have one subscription',
      );
      assert.equal(
        cancelledMember.subscriptions[0].status,
        'canceled',
  ...
  describe('Discounts', function () {
    ...
    it('Supports creating an offer from a Stripe coupon', async function () {
      ...
      assert.notEqual(
        createdOffer,
        null,
        `An offer should now have been created with coupon ID: ${stripeCouponId}`,
      );
      assert.equal(
        createdOffer.get('code'),
        stripeCouponId,
        'Offer code should match Stripe coupon ID',
      );
      assert.equal(
        createdOffer.get('active'),
        false,
        'Imported offer should be archived (not active)',
      );
      assert.equal(
        createdOffer.get('name'),
        '20% off forever (stripe-coupon-id)',
        'Offer name should be auto-generated from coupon',
      );
  ...
  describe('Member attribution', function () {
    ...
    it('The customer.subscription.created webhook should set the attribution metadata', async function () {
      ...
      set(subscription, {
        ...
        cancel_at_period_end: false,
        metadata: {
          // set the attribution on the subscription
          attribution_id: attribution.id,
          attribution_url: attribution.url,
          attribution_type: attribution.type,
        },
    ...
    it('Returns subscription created attributions in activity feed', async function () {
      ...
      await adminAgent
        ...
        .expect(({ body }) => {
          assert.equal(
            body.events.find((e) => e.type !== 'subscription_event'),
            undefined,
          );
