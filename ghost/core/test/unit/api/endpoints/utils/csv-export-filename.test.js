...
} = require('../../../../../core/server/api/endpoints/utils/csv-export-filename');
...
describe('Unit: api/endpoints/utils/csv-export-filename', function () {
  ...
  it('prefixes the slugified site title', function () {
    ...
    assert.match(
      getCSVExportFileName('members'),
      /^my-publication\.ghost\.members\.\d{4}-\d{2}-\d{2}\.csv$/,
    );
  });
  ...
  it('uses the given export type', function () {
    ...
    assert.match(
      ...
      /^my-publication\.ghost\.analytics\.\d{4}-\d{2}-\d{2}\.csv$/,
    );
  });
  ...
  it('transliterates non-ASCII titles via slugify', function () {
    ...
    assert.match(
      getCSVExportFileName('members'),
      /^cafe-munchen\.ghost\.members\.\d{4}-\d{2}-\d{2}\.csv$/,
    );
  });
