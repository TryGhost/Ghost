...
describe('Unit: endpoints/utils/serializers/output/utils/url', function () {
    ...
    describe('forPost', function () {
        ...
        it('routes pages through the pages router type', function () {
            // The pages mapper delegates to the posts mapper, which calls
            // forPost. Regression: a previous version hardcoded
            // `type: 'posts'`, which would route page records against the
            // posts router in lazy mode (eager mode is unaffected because
            // the facade only reads `id` in its eager fallback).
            const page = {id: 'page-id', slug: 'about', type: 'page'};

            urlUtil.forPost(page.id, page, {options: {}});
