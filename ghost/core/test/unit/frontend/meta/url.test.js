...
describe('getUrl', function () {
  ...
  it('should return url for a post', function () {
    ...
    urlServiceGetUrlForResourceStub
      .withArgs(sinon.match({ id: post.id, type: 'posts' }), {
        absolute: undefined,
  ...
  describe('Content-API-serialized posts (status stripped by the serializer)', function () {
    it('defaults status to published so the lazy URL service can evaluate its base filter', function () {
      ...
      urlServiceGetUrlForResourceStub
        .withArgs(sinon.match({ id: post.id, type: 'posts', status: 'published' }))
        .returns('post url');
    ...
    it('keeps the real status when present', function () {
      ...
      urlServiceGetUrlForResourceStub
        .withArgs(sinon.match({ id: post.id, type: 'posts', status: 'draft' }))
        .returns('/404/');
      urlUtilsUrlForStub
        .withArgs({ relativeUrl: '/p/' + post.uuid + '/' }, null, undefined)
        .returns('preview url');
  ...
  it('should return url for a tag', function () {
    ...
    urlServiceGetUrlForResourceStub
      .withArgs(sinon.match({ id: tag.id, type: 'tags' }), {
        absolute: undefined,
        withSubdirectory: true,
      })
      .returns('tag url');
  ...
  it('should return url for a author', function () {
    ...
    urlServiceGetUrlForResourceStub
      .withArgs(sinon.match({ id: author.id, type: 'authors' }), {
        absolute: undefined,
        withSubdirectory: true,
      })
      .returns('author url');
  ...
  it('should return url for a nav', function () {
    ...
    urlUtilsUrlForStub.withArgs('nav', { nav: data }, undefined).returns('nav url');
  ...
  it('should return absolute url for a nav', function () {
    ...
    urlUtilsUrlForStub.withArgs('nav', { nav: data }, true).returns('absolute nav url');
