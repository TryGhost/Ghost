...
describe('{{ghost_head}} helper', function () {
  ...
  beforeEach(function () {
    ...
    routingRegistryGetRssUrlStub = sinon
      .stub(routing.registry, 'getRssUrl')
      .returns('http://localhost:65530/rss/');
  ...
  describe('accent_color', function () {
    ...
    it('includes style tag in design preview when announcement bar renders nothing', async function () {
      ...
      const rendered = await testGhostHead({
        hash: { exclude: 'card_assets,comment_counts' },
        ...testUtils.createHbsResponse({
          templateOptions,
          locals: {
            relativeUrl: '/',
    ...
    it('does not override code injection', async function () {
      getStub
        .withArgs('codeinjection_head')
        .returns('<style>:root {--ghost-accent-color: #site-code-injection}</style>');

      const renderObject = {
        post: Object.assign({}, posts[1], {
          codeinjection_head: '<style>:root {--ghost-accent-color: #post-code-injection}</style>',
        }),
      };
  ...
  describe('respects values from excludes: ', function () {
    ...
    it('loads the admin toolbar script with public site and post metadata', async function () {
      ...
      assert.match(
        rendered,
        /data-ghost-admin-toolbar="http:\/\/(?:localhost:65530|127\.0\.0\.1:\d+)\/ghost\/"/,
      );
    ...
    it('does not expose the superseded on-site editor for either builder flag', async function () {
      ...
      const toolbarTag = rendered.match(
        /<script defer src="[^"]*admin-toolbar\.min\.js"[^>]*><\/script>/,
      )?.[0];
