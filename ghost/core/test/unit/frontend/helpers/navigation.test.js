...
describe('{{navigation}} helper', function () {
  ...
  it('should throw errors on invalid data', function () {
    ...
    assert.throws(runHelperThunk(optionsData), {
      message: 'navigation data is not an object or is a function',
    });
    ...
    assert.throws(runHelperThunk(optionsData), {
      message: 'Invalid value, Url and Label must be strings',
    });
  ...
  it('filters navigation items by member visibility', function () {
    const hasLabel = (rendered, label) =>
      rendered.string.includes(`<span class="nav-label">${label}</span>`);
  ...
  describe('when navigationIcons lab is disabled', function () {
    ...
    it('renders the same markup as before the flag existed', function () {
      ...
      assert.equal(
        ...
        '<ul class="nav">\n    <li class="nav-foo"><a href="' +
          configUtils.config.get('url') +
          '/foo">Foo</a></li>\n</ul>',
      );
    });
  });
...
describe('{{navigation}} helper with custom template', function () {
  ...
  describe('using compile', function () {
    ...
    it('can render both primary and secondary nav in order', function () {
      assert.equal(
        compile('{{navigation}}{{navigation type="secondary"}}').with({}),
        '\n\n\n\nPrime time!\n\n    <a href="">Foo</a>\n\n\n\n\nJeremy Bearimy baby!\n\n    <a href="">Fighters</a>\n',
      );
    });
    ...
    it('can render both primary and secondary nav in reverse order', function () {
      assert.equal(
        compile('{{navigation type="secondary"}}{{navigation}}').with({}),
        '\n\n\n\nJeremy Bearimy baby!\n\n    <a href="">Fighters</a>\n\n\n\n\nPrime time!\n\n    <a href="">Foo</a>\n',
      );
    });
  });
