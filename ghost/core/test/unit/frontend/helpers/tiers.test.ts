...
import tiersHelper from '../../../../core/frontend/helpers/tiers';

describe('{{tiers}} helper', function () {
  it('can return string with tiers', function () {
    const tiers = [{ name: 'tier 1' }, { name: 'tier 2' }, { name: 'tier 3' }];
  ...
  it('can use a different final separator', function () {
    ...
    const rendered = tiersHelper.call(
      { tiers: tiers },
      { hash: { lastSeparator: ' as well as ' } },
    );
    assertExists(rendered);
  ...
  it('can add a prefix and suffix to multiple tiers', function () {
    ...
    const rendered = tiersHelper.call(
      { tiers: tiers },
      { hash: { prefix: 'on ', suffix: ' products' } },
    );
    assertExists(rendered);
