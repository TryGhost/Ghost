...
describe('{{#foreach}} helper', function () {
  ...
  describe('(compile)', function () {
    ...
    const arrayHashWithVis = {
      posts: [
        { title: 'first', visibility: 'members', slug: 'first', html: '' },
        { title: 'second' },
        { title: 'third' },
        { title: 'fourth' },
        { title: 'fifth' },
    ...
    it('object and @key', function () {
      ...
      const expected =
        '<ul><li>first first</li><li>second second</li><li>third third</li><li>fourth fourth</li><li>fifth fifth</li></ul>';
    ...
    it('@index', function () {
      ...
      const expected =
        '<ul><li>0 first</li><li>1 second</li><li>2 third</li><li>3 fourth</li><li>4 fifth</li></ul>';
    ...
    it('@number', function () {
      const templateString =
        '<ul>{{#foreach posts}}<li>{{@number}} {{title}}</li>{{/foreach}}</ul>';
      const expected =
        '<ul><li>1 first</li><li>2 second</li><li>3 third</li><li>4 fourth</li><li>5 fifth</li></ul>';
    ...
    it('nested @index', function () {
      const templateString =
        '{{#foreach goodbyes}}{{@index}}. {{text}}! {{#foreach ../goodbyes}}{{@index}} {{/foreach}}After {{@index}} {{/foreach}}{{@index}}cruel {{world}}!';
      const expected =
    ...
    it('array block params', function () {
      const templateString =
        '{{#foreach goodbyes as |value index|}}{{index}}. {{value.text}}! {{#foreach ../goodbyes as |childValue childIndex|}} {{index}} {{childIndex}}{{/foreach}} After {{index}} {{/foreach}}{{index}}cruel {{world}}!';
      const expected =
        '0. goodbye!  0 0 0 1 0 2 After 0 1. Goodbye!  1 0 1 1 1 2 After 1 2. GOODBYE!  2 0 2 1 2 2 After 2 cruel world!';
    ...
    it('object block params', function () {
      const templateString =
        '{{#foreach goodbyes as |value index|}}{{index}}. {{value.text}}! {{#foreach ../goodbyes as |childValue childIndex|}} {{index}} {{childIndex}}{{/foreach}} After {{index}} {{/foreach}}{{index}}cruel {{world}}!';
      const expected =
        'foo. goodbye!  foo foo foo bar foo baz After foo bar. Goodbye!  bar foo bar bar bar baz After bar baz. GOODBYE!  baz foo baz bar baz baz After baz cruel world!';
    ...
    it('@first', function () {
      const templateString =
        '{{#foreach goodbyes}}{{#if @first}}{{text}}! {{/if}}{{/foreach}}cruel {{world}}!';
    ...
    it('nested @first', function () {
      const templateString =
        '{{#foreach goodbyes}}({{#if @first}}{{text}}! {{/if}}{{#foreach ../goodbyes}}{{#if @first}}{{text}}!{{/if}}{{/foreach}}{{#if @first}} {{text}}!{{/if}}) {{/foreach}}cruel {{world}}!';
    ...
    it('@last', function () {
      const templateString =
        '{{#foreach goodbyes}}{{#if @last}}{{text}}! {{/if}}{{/foreach}}cruel {{world}}!';
      const expected = 'GOODBYE! cruel world!';
    ...
    it('nested @last', function () {
      const templateString =
        '{{#foreach goodbyes}}({{#if @last}}{{text}}! {{/if}}{{#foreach ../goodbyes}}{{#if @last}}{{text}}!{{/if}}{{/foreach}}{{#if @last}} {{text}}!{{/if}}) {{/foreach}}cruel {{world}}!';
      const expected = '(GOODBYE!) (GOODBYE!) (GOODBYE! GOODBYE! GOODBYE!) cruel world!';
    ...
    it('@last in foreach with limit', function () {
      const templateString =
        '{{#foreach goodbyes limit="2"}}{{#if @last}}{{text}}! {{/if}}{{/foreach}}cruel {{world}}!';
      const expected = 'Goodbye! cruel world!';
    ...
    it('foreach with limit 1', function () {
      const templateString =
        '<ul>{{#foreach posts limit="1"}}<li>{{title}}</li>{{else}}not this{{/foreach}}</ul>';
    ...
    it('foreach with limit 3', function () {
      const templateString =
        '<ul>{{#foreach posts limit="3"}}<li>{{title}}</li>{{else}}not this{{/foreach}}</ul>';
    ...
    it('foreach with newsletters with members visibility', function () {
      ...
      const templateString =
        '<ul>{{#foreach newsletters}}<li>{{name}}</li>{{else}}not this{{/foreach}}</ul>';
      const expected = '<ul><li>first</li><li>second</li><li>third</li></ul>';
    ...
    it('foreach with from 2', function () {
      const templateString =
        '<ul>{{#foreach posts from="2"}}<li>{{title}}</li>{{else}}not this{{/foreach}}</ul>';
    ...
    it('foreach with to 4', function () {
      const templateString =
        '<ul>{{#foreach posts to="4"}}<li>{{title}}</li>{{else}}not this{{/foreach}}</ul>';
    ...
    it('foreach with from 2 and to 3', function () {
      const templateString =
        '<ul>{{#foreach posts from="2" to="3"}}<li>{{title}}</li>{{else}}not this{{/foreach}}</ul>';
    ...
    it('foreach with from 3 and limit 2', function () {
      const templateString =
        '<ul>{{#foreach posts from="3" limit="2"}}<li>{{title}}</li>{{else}}not this{{/foreach}}</ul>';
    ...
    it('foreach with from 2, to 5 and limit 3', function () {
      const templateString =
        '<ul>{{#foreach posts from="2" to="5" limit="3"}}<li>{{title}}</li>{{else}}not this{{/foreach}}</ul>';
    ...
    it('@first in foreach with from 2 and to 4', function () {
      const templateString =
        '<ul>{{#foreach posts from="2" to="4"}}{{#if @first}}<li>{{title}}</li>{{/if}}{{/foreach}}</ul>';
    ...
    it('@last in foreach with from 2 and to 4', function () {
      const templateString =
        '<ul>{{#foreach posts from="2" to="4"}}{{#if @last}}<li>{{title}}</li>{{/if}}{{/foreach}}</ul>';
    ...
    it('@last in foreach with from 4', function () {
      const templateString =
        '<ul>{{#foreach posts from="4"}}{{#if @last}}<li>{{title}}</li>{{/if}}{{/foreach}}</ul>';
    ...
    it('@first in foreach with from 4', function () {
      const templateString =
        '<ul>{{#foreach posts from="4"}}{{#if @first}}<li>{{title}}</li>{{/if}}{{/foreach}}</ul>';
      const expected = '<ul><li>fourth</li></ul>';
    ...
    it('@last in foreach with to 4', function () {
      const templateString =
        '<ul>{{#foreach posts to="4"}}{{#if @last}}<li>{{title}}</li>{{/if}}{{/foreach}}</ul>';
      const expected = '<ul><li>fourth</li></ul>';
    ...
    it('@first in foreach with to 4', function () {
      const templateString =
        '<ul>{{#foreach posts to="4"}}{{#if @first}}<li>{{title}}</li>{{/if}}{{/foreach}}</ul>';
      const expected = '<ul><li>first</li></ul>';
    ...
    it('@last in foreach with from 4 and limit 3', function () {
      const templateString =
        '<ul>{{#foreach posts from="4" limit="3"}}{{#if @last}}<li>{{title}}</li>{{/if}}{{/foreach}}</ul>';
      const expected = '<ul><li>fifth</li></ul>';
    ...
    it('@first in foreach with from 4 and limit 3', function () {
      const templateString =
        '<ul>{{#foreach posts from="4" limit="3"}}{{#if @first}}<li>{{title}}</li>{{/if}}{{/foreach}}</ul>';
      const expected = '<ul><li>fourth</li></ul>';
    ...
    it('@last in foreach with to 4 and limit 3', function () {
      const templateString =
        '<ul>{{#foreach posts to="4" limit="3"}}{{#if @last}}<li>{{title}}</li>{{/if}}{{/foreach}}</ul>';
    ...
    it('@first in foreach with to 4 and limit 3', function () {
      const templateString =
        '<ul>{{#foreach posts to="4" limit="3"}}{{#if @first}}<li>{{title}}</li>{{/if}}{{/foreach}}</ul>';
      const expected = '<ul><li>first</li></ul>';
    ...
    describe('Internal Tags', function () {
      ...
      it('will not output internal tags', function () {
        ...
        const expected =
          '<ul><li>0 first</li><li>1 second</li><li>2 fourth</li><li>3 fifth</li></ul>';
      ...
      it('should still correctly apply from & limit tags', function () {
        const templateString =
          '<ul>{{#foreach tags from="2" limit="2"}}<li>{{@index}} {{name}}</li>{{/foreach}}</ul>';
      ...
      it('should output all tags with visibility="all"', function () {
        const templateString =
          '<ul>{{#foreach tags visibility="all"}}<li>{{@index}} {{name}}</li>{{/foreach}}</ul>';
        const expected =
          '<ul><li>0 first</li><li>1 second</li><li>2 third</li><li>3 fourth</li><li>4 fifth</li></ul>';
      ...
      it('should output all tags with visibility property set with visibility="public,internal"', function () {
        const templateString =
          '<ul>{{#foreach tags visibility="public,internal"}}<li>{{@index}} {{name}}</li>{{/foreach}}</ul>';
        const expected =
          '<ul><li>0 first</li><li>1 second</li><li>2 third</li><li>3 fourth</li><li>4 fifth</li></ul>';
      ...
      it('should output all tags with visibility="internal"', function () {
        const templateString =
          '<ul>{{#foreach tags visibility="internal"}}<li>{{@index}} {{name}}</li>{{/foreach}}</ul>';
