...
describe('{{#social_accounts}} helper', function () {
  ...
  it('exposes type, href, username, and name per iteration', function () {
    const out = compile(
      `{{#social_accounts @site}}{{type}}={{href}}|{{name}}|{{username}};{{/social_accounts}}`,
    ).with({}, { data: { site: { twitter: 'testuser-tw', linkedin: 'testuser-li' } } });
    assert.equal(
      out,
      'x=https://x.com/testuser-tw|X|testuser-tw;linkedin=https://www.linkedin.com/in/testuser-li|LinkedIn|testuser-li;',
    );
  });
  ...
  it('uses `href` (not `url`) for the link to avoid collision with the {{url}} helper', function () {
    ...
    try {
      const out = compile(`{{#social_accounts @site}}{{href}};{{/social_accounts}}`).with(
        {},
        { data: { site: { twitter: 'me' } } },
      );
  ...
  it('skips platforms without a username', function () {
    const out = compile(`{{#social_accounts @site}}{{type}},{{/social_accounts}}`).with(
      {},
      { data: { site: { twitter: 'me', instagram: 'me-ig' } } },
    );
    assert.equal(out, 'x,instagram,');
  });

  it('renders the {{else}} block when nothing is set', function () {
    const out = compile(`{{#social_accounts @site}}link{{else}}none{{/social_accounts}}`).with(
      {},
      { data: { site: {} } },
    );
  ...
  it('returns an empty string when nothing is set and no else block is provided', function () {
    const out = compile(`{{#social_accounts @site}}link{{/social_accounts}}`).with(
      {},
      { data: { site: {} } },
    );
    assert.equal(out, '');
  ...
  it('iterates over a passed author object', function () {
    const out = compile(
      `{{#social_accounts author}}{{type}}={{username}};{{/social_accounts}}`,
  ...
  it('iterates over the current context with `this`', function () {
    const out = compile(`{{#social_accounts this}}{{type}}={{username}};{{/social_accounts}}`).with(
      { twitter: 'ctx-tw', mastodon: 'mastodon.social/@me' },
    );
  ...
  it('exposes @first, @last, @index iteration vars', function () {
    const out = compile(
      `{{#social_accounts @site}}{{@index}}:{{type}}{{#if @first}}!{{/if}}{{#if @last}}*{{/if}} {{/social_accounts}}`,
  ...
  it('throws an IncorrectUsageError when called with no source', function () {
    assert.throws(
      ...
      (err) =>
        err instanceof errors.IncorrectUsageError && /requires a source argument/.test(err.message),
    );
  });
  ...
  it('throws an IncorrectUsageError when used as an inline helper', function () {
    assert.throws(
      ...
      (err) =>
        err instanceof errors.IncorrectUsageError &&
        /must be used as a block helper/.test(err.message),
    );
  });

  it('renders nothing when source is undefined (variable not in scope)', function () {
    const out = compile(`{{#social_accounts missingVar}}x{{else}}none{{/social_accounts}}`).with(
      {},
    );
    assert.equal(out, 'none');
  });
  ...
  it('emits type "x" (not "twitter") for the X/Twitter platform', function () {
    const out = compile(`{{#social_accounts @site}}{{type}}={{name}};{{/social_accounts}}`).with(
      {},
      { data: { site: { twitter: 'ghost' } } },
    );
  ...
  it('emits type "x" when iterating an author inside {{#author}}', function () {
    const out = compile(
      `{{#author}}{{#social_accounts this}}{{type}}{{/social_accounts}}{{/author}}`,
