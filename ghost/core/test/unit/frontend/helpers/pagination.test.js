...
describe('{{pagination}} helper', function () {
  ...
  it('validates values', function () {
    ...
    assert.throws(
      runErrorTest({
        pagination: { page: 3, prev: true, next: null, limit: 15, total: 8, pages: 3 },
      }),
      { message: 'Invalid value, Next/Prev must be a number' },
    );
    assert.throws(
    ...
    assert.throws(runErrorTest({ pagination: { limit: 15, total: 8, pages: 3 } }), {
      message: 'All values must be defined for page, pages, limit and total',
    });
    assert.throws(runErrorTest({ pagination: { page: 3, total: 8, pages: 3 } }), {
      message: 'All values must be defined for page, pages, limit and total',
    });
    assert.throws(runErrorTest({ pagination: { page: 3, limit: 15, pages: 3 } }), {
      message: 'All values must be defined for page, pages, limit and total',
    });
    assert.throws(runErrorTest({ pagination: { page: 3, limit: 15, total: 8 } }), {
      message: 'All values must be defined for page, pages, limit and total',
    });

    assert.throws(
      runErrorTest({
        pagination: { page: null, prev: null, next: null, limit: 15, total: 8, pages: 3 },
      }),
      { message: 'Invalid value, check page, pages, limit and total are numbers' },
    );
    assert.throws(
      runErrorTest({
        pagination: { page: 1, prev: null, next: null, limit: null, total: 8, pages: 3 },
      }),
      { message: 'Invalid value, check page, pages, limit and total are numbers' },
    );
    assert.throws(
      runErrorTest({
        pagination: { page: 1, prev: null, next: null, limit: 15, total: null, pages: 3 },
      }),
      { message: 'Invalid value, check page, pages, limit and total are numbers' },
    );
    assert.throws(
      runErrorTest({
        pagination: { page: 1, prev: null, next: null, limit: 15, total: 8, pages: null },
      }),
      { message: 'Invalid value, check page, pages, limit and total are numbers' },
    );
  });
