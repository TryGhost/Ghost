...
describe('Private Controller', function () {
  ...
  beforeEach(function () {
    ...
    defaultPath = path.join(
      ...
      '/core/frontend/apps/private-blogging/lib/views/private.hbs',
    );
...
describe('private.hbs template translation', function () {
  const privateViewPath = path.join(
    __dirname,
    '../../../../../core/frontend/apps/private-blogging/lib/views/private.hbs',
  );
  ...
  i18nImplementations.forEach(({ name, useNewTranslation }) => {
    describe(`with ${name}`, function () {
      ...
      it('renders the site icon above the site title when an icon exists', function () {
        ...
        assertExists(html);
        assert(
          /<header>[\s\S]*<img class="site-icon"[\s\S]*<h1>\s*Test\s*<\/h1>[\s\S]*<\/header>/.test(
            html,
          ),
        );
      });
      ...
      it('renders the site description below the site title when present', function () {
        const context = {
          site: {
            title: 'Test',
            description: 'A private publication',
            url: 'http://test.local',
            locale: 'en',
            admin_url: 'http://test.local/ghost/',
          },
        ...
        assertExists(html);
        assert(
          /<h1>\s*Test\s*<\/h1>[\s\S]*<p class="gh-private-description">A private publication<\/p>/.test(
            html,
          ),
        );
      });
      ...
      it('renders the signup form when self signup is enabled', function () {
        ...
        assertExists(html);
        assert(
          /<form class="gh-private-signup-form gh-signin"[^>]*data-ghost-private-subscribe-form[^>]*data-members-form="subscribe"/.test(
            html,
          ),
        );
      ...
      it('uses the site accent color for the signup button when available', function () {
        ...
        assert(
          html.includes(
            'style="--gh-private-accent-color: #FFFFFF; --gh-private-accent-color-rgba: rgba(255, 255, 255, 0.25); --gh-private-accent-contrast-color: #000000;"',
          ),
        );
      });
      ...
      it('renders English strings when locale is en', function () {
        ...
        assert(
          /<div class="gh-private-footer-links">[\s\S]*<a class="gh-private-powered" href="https:\/\/ghost\.org"[\s\S]*Powered by Ghost[\s\S]*\|[\s\S]*<a class="gh-private-trigger"[^>]*data-ghost-private-trigger[\s\S]*Enter access code[\s\S]*\|[\s\S]*<a href="https?:\/\/[^"]+">Site owner login<\/a>/.test(
            html,
          ),
        );
        assert(html.includes('id="access"'));
