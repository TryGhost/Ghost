...
describe('private.js', function () {
  ...
  beforeAll(function () {
    scriptContent = fs.readFileSync(
      path.join(__dirname, '../../../../core/frontend/public/private.js'),
      'utf8',
    );
  });
  ...
  it('submits the subscribe request using the expected payload shape', async function () {
    ...
    env.window.sessionStorage.setItem(
      'ghost-history',
      JSON.stringify([{ path: '/welcome', refSource: 'ghost' }]),
    );
    ...
    assert.equal(
      env.window.fetch.firstCall.args[0],
      'https://example.com/members/api/integrity-token/',
    );
  ...
  it('normalizes rate-limit errors by type rather than exact message', async function () {
    ...
    env.window.fetch.onSecondCall().resolves({
      ...
      json: async () => ({
        errors: [
          {
            message: 'Too many sign-in attempts try again in 14 minutes',
