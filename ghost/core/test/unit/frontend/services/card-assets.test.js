...
const expectedHash = (content) =>
  crypto.createHash('sha256').update(content).digest('base64url').substring(0, 16);

describe('Card Asset Manifest Builder', function () {
  let testDir, manifestPath, manifest, ungroupedCssManifest, shippedCssManifest;
  ...
  beforeAll(async function () {
    ...
    await Promise.all([
      ...
      fs.writeFile(
        path.join(ungroupedCssDir, 'headerz.css'),
        '.kg-header-card.kg-v2 { color: green; }',
      ),
    ]);
  ...
  it('maps header v2 CSS to the public header card name', function () {
    ...
    assert.ok(
      manifest.css.header.indexOf('.kg-header-card{') <
        manifest.css.header.indexOf('.kg-header-card.kg-v2{'),
    );
  });
  ...
  it('does not expose a versioned header card name from the shipped CSS', function () {
    assert.deepEqual(
      Object.keys(shippedCssManifest).filter((name) => name.startsWith('header')),
      ['header'],
    );
  });
...
describe('Card Asset Service', function () {
  let testDir, manifestPath;

  const service = (config) =>
    new CardAssetService(
      Object.assign({ manifest: manifestPath }, config === undefined ? {} : { config }),
    );
  ...
  describe('Bundling', function () {
    ...
    it('terminates js chunks so they cannot run into each other', function () {
      ...
      assert.equal(
        cardAssets.getBundle('js').content,
        `${MANIFEST.js.audio};\n${MANIFEST.js.gallery}`,
      );
    });
    ...
    it('produces a different hash for a different selection', function () {
      assert.notEqual(
        service(true).getHash('css'),
        service({ exclude: ['bookmark'] }).getHash('css'),
      );
    });
