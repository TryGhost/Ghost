...
describe('templates', function () {
  ...
  describe('[private fn] getEntriesTemplateHierarchy', function () {
    ...
    it('should return custom templates even if the collection is index', function () {
      assert.deepEqual(
        templateHelpers.getEntriesTemplateHierarchy({ name: 'index', templates: ['something'] }),
        ['something', 'index'],
      );
    });

    it('should return collection name', function () {
      assert.deepEqual(templateHelpers.getEntriesTemplateHierarchy({ name: 'podcast' }), [
        'podcast',
        'index',
      ]);
    });
    ...
    it('should return custom templates', function () {
      assert.deepEqual(
        templateHelpers.getEntriesTemplateHierarchy({ name: 'podcast', templates: ['mozart'] }),
        ['mozart', 'podcast', 'index'],
      );
    });

    it('should return just index if collection name is index even if slug is set', function () {
      assert.deepEqual(
        templateHelpers.getEntriesTemplateHierarchy(
          { name: 'index', slugTemplate: true },
          { slugParam: 'test' },
        ),
        ['index'],
      );
    ...
    it('should return collection, index if collection has name', function () {
      assert.deepEqual(templateHelpers.getEntriesTemplateHierarchy({ name: 'tag' }), [
        'tag',
        'index',
      ]);
    });
    ...
    it('should return front, collection-slug, collection, index if name, slugParam+slugTemplate & frontPageTemplate+pageParam=1 is set', function () {
      assert.deepEqual(
        templateHelpers.getEntriesTemplateHierarchy(
          ...
          { page: 1, path: '/', slugParam: 'test' },
        ),
        ['front-tag', 'tag-test', 'tag', 'index'],
      );
    });
  ...
  describe('[private fn] pickTemplate', function () {
    ...
    it('returns fallback if there is no active_theme', function () {
      ...
      assert.equal(
        templateHelpers.pickTemplate(['tag-test', 'tag', 'index'], 'fallback'),
        'fallback',
      );
      assert.equal(
        templateHelpers.pickTemplate(['page-my-post', 'page', 'post'], 'fallback'),
        'fallback',
      );
    });
    ...
    it('returns fallback if active_theme has no templates', function () {
      ...
      assert.equal(
        templateHelpers.pickTemplate(['page-about', 'page', 'post'], 'fallback'),
        'fallback',
      );
    });
    ...
    describe('with many templates', function () {
      ...
      it('returns first matching template', function () {
        ...
        assert.equal(
          templateHelpers.pickTemplate(['page-magic', 'page', 'post'], 'fallback'),
          'page',
        );
  ...
  describe('[private fn] getTemplateForEntries', function () {
    ...
    describe('without tag templates', function () {
      ...
      it('will return correct view for a tag', function () {
        const view = templateHelpers.getTemplateForEntries(
          { name: 'tag', slugTemplate: true },
          { slugParam: 'development' },
        );
        assertExists(view);
  ...
  describe('fn: setTemplate', function () {
    ...
    beforeEach(function () {
      ...
      stubs.getTemplateForEntry = sinon
        .stub(templates, 'getTemplateForEntry')
        .returns('testFromEntry');
      stubs.getTemplateForEntries = sinon
        .stub(templates, 'getTemplateForEntries')
        .returns('testFromEntries');
      stubs.getTemplateForError = sinon
        .stub(templates, 'getTemplateForError')
        .returns('testFromError');
    });
    ...
    it('calls getTemplateForEntries for type collection', function () {
      ...
      sinon.assert.calledWith(stubs.getTemplateForEntries, {
        testCollection: 'test',
        type: 'collection',
      });
    });
