...
describe('Unit: frontend/services/llms/service', function () {
  ...
  it('builds llms.txt with markdown URLs, stable page ordering, and truncated descriptions', async function () {
      ...
      posts: [
        {
          ...
          title: 'Recent Post',
          slug: 'recent-post',
          custom_excerpt: longDescription,
          type: 'post',
        },
        {
          id: 'post-b',
    ...
    assert.match(
      llmsTxt,
      /## Pages[\s\S]*\[A Page\]\(https:\/\/example\.com\/about\.md\)[\s\S]*\[B Page\]\(https:\/\/example\.com\/contact\.md\)/m,
    );
    assert.match(
      llmsTxt,
      /\[Recent Post\]\(https:\/\/example\.com\/2026\/04\/recent-post\.md\) - A{299}…/,
    );
  ...
  it('computes fresh output on every call (no cache)', async function () {
    ...
    assert.ok(
      callCount >= 4,
      `Expected at least 4 browse calls (2 per getLlmsTxt), got ${callCount}`,
    );
  });
  ...
  it('fetches index content through the public Posts/Pages API, not the model layer', async function () {
    ...
    assert.equal(
      postCall.options.fields,
      'id,title,slug,custom_excerpt,featured,published_at,url,visibility',
    );
  ...
  it('includes the .md discoverability line and llms-full link in llms.txt', async function () {
    ...
    assert.match(
      llmsTxt,
      /## Optional[\s\S]*\[Full content of pages and posts\]\(http:\/\/127\.0\.0\.1:\d+\/llms-full\.txt\)/m,
    );
  });
  ...
  it('bounds the llms.txt index to the size budget', async function () {
    ...
    assert.ok(
      Buffer.byteLength(llmsTxt, 'utf8') <= 50 * 1024,
      `expected <= 50KB, got ${Buffer.byteLength(llmsTxt, 'utf8')}`,
    );
