...
describe('UNIT: services/routing/middleware/page-param', function () {
  ...
  it('success', async function () {
    const { body } = await request(app).get('/blog/page/2/').expect(200);
    assert.equal(body.page, 2);
  });

  it('redirect for /page/1/', async function () {
    await request(app).get('/blog/page/1/').expect(301).expect('Location', '/blog/');
  });

  it('404 for /page/0/', async function () {
    await request(app).get('/blog/page/0/').expect(404).expect({ errorType: 'NotFoundError' });
  });
  ...
  it('redirect for /rss/page/1/', async function () {
    await request(app).get('/blog/rss/page/1/').expect(301).expect('Location', '/blog/rss/');
  });
});
