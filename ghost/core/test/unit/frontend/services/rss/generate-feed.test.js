  ...
  describe('without subdirectory', function () {
    ...
    it('defaults status to published on the URL lookup (the serializer strips it)', async function () {
      ...
      sinon.assert.calledWith(
        ...
        sinon.match({ id: post.id, type: 'posts', status: 'published' }),
        { absolute: true },
      );
    });
    ...
    it('should get the item tags correct', async function () {
      ...
      _.each(data.posts, function (post) {
        routerManagerGetUrlForResourceStub
          .withArgs(sinon.match({ id: post.id }), { absolute: true })
          .returns('http://my-ghost-blog.com/' + post.slug + '/');
      });
    ...
    it('should only return visible tags', async function () {
      ...
      assert.match(
        xmlData,
        /<content:encoded><!\[CDATA\[<!--kg-card-begin: markdown--><h2 id="testing">testing<\/h2>\n/,
      );
