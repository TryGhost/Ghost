...
describe('Checks', function () {
  ...
  it('isNewsletter', function () {
    ...
    assert.equal(
      checks.isNewsletter({ name: 'Test', visibility: 'members', subscribe_on_signup: true }),
      true,
    );
    assert.equal(
      checks.isNewsletter({ name: 'Test', visibility: 'paid', subscribe_on_signup: false }),
      true,
    );
  });
  ...
  it('isTag', function () {
    ...
    assert.equal(
      checks.isTag({ name: 'Test', slug: 'test', description: '', feature_image: '' }),
      true,
    );
  });
  ...
  it('isUser', function () {
    ...
    assert.equal(
      checks.isUser({ bio: 'Test', website: 'test', profile_image: '', location: '' }),
      true,
    );
  });
