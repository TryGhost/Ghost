...
describe('Email verification flow', function () {
    ...
    it('Uses custom message when emailVerificationNeededMessage is provided', async function () {
        const emailStub = sinon.stub().resolves(null);
        const settingsStub = sinon.stub().resolves(null);
        const trigger = new VerificationTrigger({
            Settings: {
            ...
            isVerificationRequired: () => false,
            sendVerificationEmail: emailStub,
            emailVerificationNeededMessage: 'Custom verification message for managed host'
        });
        ...
        try {
            ...
            assert.equal(e.message, 'Custom verification message for managed host');
            assert.equal(e.code, 'EMAIL_VERIFICATION_NEEDED');
        }
    });
