...
describe('buildPostData', function () {
    ...
    it('slugs the title with the standard rules, not the importing-mode pass', function () {
        const data = buildPostData(row({title: 'A post with a comma, in its title'}), htmlToLexical, TAGS);
    ...
    it('dates the whole post from the CSV date: published_at, created_at and updated_at', function () {
        const data = buildPostData(row({title: 'T', published_at: '2025-01-01T00:00:00.000Z'}), htmlToLexical, TAGS);
    ...
    it('fills every unspecified field with the import defaults', function () {
        ...
        assert.equal(data.visibility, 'public', 'explicit: the model would read the default_content_visibility setting');
        assert.deepEqual(data.tags, TAGS.map(name => ({name})));
    ...
    const skipsWith = (cells: Record<string, string>, reason: string | RegExp) => {
        assert.throws(() => buildPostData(row(cells), htmlToLexical, TAGS), (error: unknown) => {
    ...
    it('accepts a non-ISO date format that new Date can parse', function () {
        const data = buildPostData(row({title: 'T', published_at: 'May 1, 2024'}), htmlToLexical, TAGS);
