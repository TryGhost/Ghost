...
describe('UNIT: services/route-settings/validation (via parseRouteSettings)', function () {
  ...
  describe('route validation', function () {
    ...
    it('throws on channel route with wrong-typed rss', function () {
      throwsValidation(() =>
        parse({
          routes: {
            '/featured/': { controller: 'channel', filter: 'featured:true', rss: 'notabool' },
          },
        }),
      );
    });
  ...
  describe('error messages', function () {
      ...
      assert.equal(
        messageFor({
          routes: {
            '/x/': {
              template: 'x',
              data: { episodes: { type: 'browse', resource: 'posts', limit: [1, 2] } },
            },
          },
        }),
    ...
    it('names the reserved data key that needs wrapping', function () {
      assert.equal(
        messageFor({
          routes: { '/x/': { template: 'x', data: { resource: 'posts', type: 'browse' } } },
        }),
    ...
    it('describes what the author wrote', function () {
      ...
      const found = (value: unknown) =>
        messageFor({ routes: { '/x/': { controller: 'channel', rss: value } } }).replace(
          /^.*but /,
          '',
        );
      ...
      assert.match(
        messageFor({ routes: { '/x/': { controller: 'channel', filter: true } } }),
        /but true was provided\.$/,
      );
    });
    ...
    describe('paths', function () {
      const cases: [string, unknown, string][] = [
        [
          'route missing a leading slash',
          { routes: { 'about/': 'about' } },
          'The following definition "routes[\'about/\']" is invalid: "about/" is missing a leading slash. Please use e.g. /about/.',
        ],
        [
          'route missing a trailing slash',
          { routes: { '/about': 'about' } },
          'The following definition "routes[\'/about\']" is invalid: "/about" is missing a trailing slash. Please use e.g. /about/.',
        ],
        [
          'collection missing a leading slash',
          { collections: { 'blog/': { permalink: '/{slug}/' } } },
          'The following definition "collections[\'blog/\']" is invalid: "blog/" is missing a leading slash. Please use e.g. /blog/.',
        ],
        [
          'collection missing a trailing slash',
          { collections: { '/blog': { permalink: '/{slug}/' } } },
          'The following definition "collections[\'/blog\']" is invalid: "/blog" is missing a trailing slash. Please use e.g. /blog/.',
        ],
        [
          'permalink missing a leading slash',
          { collections: { '/blog/': { permalink: '{slug}/' } } },
          'The following definition "collections[\'/blog/\'].permalink" is invalid: "{slug}/" is missing a leading slash. Please use e.g. /{slug}/.',
        ],
        [
          'permalink missing a trailing slash',
          { collections: { '/blog/': { permalink: '/{slug}' } } },
          'The following definition "collections[\'/blog/\'].permalink" is invalid: "/{slug}" is missing a trailing slash. Please use e.g. /{slug}/.',
        ],
        [
          'permalink using :param',
          { collections: { '/blog/': { permalink: '/:slug/' } } },
          'The following definition "collections[\'/blog/\'].permalink" is invalid: "/:slug/" uses the :param notation. Please use "/{slug}/".',
        ],
        [
          'taxonomy missing a leading slash',
          { taxonomies: { tag: 'tag/{slug}/' } },
          'The following definition "taxonomies.tag" is invalid: "tag/{slug}/" is missing a leading slash. Please use e.g. /tag/{slug}/.',
        ],
        [
          'taxonomy using :param',
          { taxonomies: { author: '/author/:slug/' } },
          'The following definition "taxonomies.author" is invalid: "/author/:slug/" uses the :param notation. Please use "/author/{slug}/".',
        ],
      ];
    ...
    it('points at the docs for schema failures and at an example for data failures', function () {
      ...
      assert.equal(
        ...
        'e.g. data: tag.recipes',
      );
      assert.match(
        helpFor({ routes: { '/x/': { template: 'x', data: { e: { resource: 'posts' } } } } }) ?? '',
        /^e\.g\.\n data:/,
      );
    });
  });
  ...
  describe('domain error codes', function () {
    ...
    const cases: Array<[string, unknown, string]> = [
      ...
      [
        'collection path missing a slash',
        { collections: { 'blog/': { permalink: '/{slug}/' } } },
        'INVALID_PATH',
      ],
      [
        'collection permalink using :param',
        { collections: { '/blog/': { permalink: '/:slug/' } } },
        'INVALID_PERMALINK',
      ],
      [
        'collection missing a permalink',
        { collections: { '/blog/': { template: 'index' } } },
        'INVALID_PERMALINK',
      ],
      [
        'route with no template, data or content_type',
        { routes: { '/about/': {} } },
        'INVALID_TEMPLATE',
      ],
      [
      ...
      [
        'taxonomy permalink using :param',
        { taxonomies: { tag: '/tag/:slug/' } },
        'INVALID_PERMALINK',
      ],
      ...
      [
        'collection permalink left empty',
        { collections: { '/blog/': { permalink: null } } },
        'INVALID_PERMALINK',
      ],
      [
        'collection permalink of the wrong type',
        { collections: { '/blog/': { permalink: 42 } } },
        'INVALID_PERMALINK',
      ],
      ...
      [
        'unsupported longform data resource',
        {
          routes: {
            '/x/': {
              template: 'x',
              data: { thing: { type: 'read', resource: 'widgets', slug: 'a' } },
            },
          },
        },
        'INVALID_RESOURCE',
      ...
      [
        'unsupported shorthand data resource',
        { routes: { '/x/': { template: 'x', data: 'widget.foo' } } },
        'INVALID_RESOURCE',
      ],
      [
        'reserved data key',
        { routes: { '/x/': { template: 'x', data: { slug: 'tag.recipes' } } } },
        'INVALID_DATA',
      ],
      [
        'data provided as a list',
        { routes: { '/x/': { template: 'x', data: ['a', 'b'] } } },
        'INVALID_DATA',
      ],
