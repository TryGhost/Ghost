...
import {
  parseRouteSettings,
  serializeRouteSettings,
} from '../../../../../core/server/services/route-settings/route-settings-parser';
  ...
  describe('parseRouteSettings', function () {
    ...
    describe('routes', function () {
      ...
      it('parses object route with template string', function () {
        ...
        assert.deepEqual(result.routes, [{ type: 'template', path: '/me/', templates: ['me'] }]);
      });
    ...
    describe('collections', function () {
      it('parses basic collection', function () {
        ...
        assert.deepEqual(result.collections, [{ path: '/', permalink: '/{slug}/', templates: [] }]);
      });
    ...
    describe('legacy YAML coercions', function () {
      ...
      it('treats empty optional scalars as unset in routes', function () {
        ...
        assert.deepEqual(result.routes, [{ type: 'channel', path: '/podcast/', templates: [] }]);
      });
      ...
      it('treats an empty template as unset in collections', function () {
        const result = parse({
          collections: {
            '/anime/': { permalink: '/anime/{slug}/', template: null, filter: 'primary_tag:anime' },
          },
        });
        ...
        assert.deepEqual(result.collections, [
          {
            path: '/anime/',
            permalink: '/anime/{slug}/',
            templates: [],
            filter: 'primary_tag:anime',
          },
        ]);
      });
      ...
      it('treats an empty content_type as unset', function () {
        ...
        assert.deepEqual(result.routes, [{ type: 'template', path: '/rss/', templates: ['rss'] }]);
      });
    });
  ...
  describe('determinism', function () {
    ...
    const complexConfig = {
      routes: {
        ...
        '/reader/': {
          template: 'reader',
          data: { entry: { type: 'read', resource: 'posts', slug: 'welcome', redirect: false } },
        },
      },
      collections: {
