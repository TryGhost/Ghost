...
function createSettingsMock({ setDirect, setConnect }) {
  ...
  getStub
    .withArgs('stripe_connect_publishable_key')
    .returns(setConnect ? 'connect_publishable' : null);
...
describe('Settings Helpers', function () {
  ...
  describe('createUnsubscribeUrl', function () {
    ...
    const memberUuidHash = crypto
      .createHmac('sha256', mockValidationKey)
      .update(`${memberUuid}`)
      .digest('hex');
    let fakeSettings;
    ...
    it('returns a url that can be used to unsubscribe a member for a given newsletter', function () {
      ...
      assert.equal(
        url,
        `http://domain.com/unsubscribe/?uuid=memberuuid&key=${memberUuidHash}&newsletter=newsletteruuid`,
      );
    });
    ...
    it('returns a url that can be used to unsubscribe a member from comments', function () {
      ...
      assert.equal(
        url,
        `http://domain.com/unsubscribe/?uuid=memberuuid&key=${memberUuidHash}&comments=1`,
      );
    });
    ...
    it('returns a url that can be used to unsubscribe a member from updates & announcements', function () {
      ...
      assert.equal(
        url,
        `http://domain.com/unsubscribe/?uuid=memberuuid&key=${memberUuidHash}&updatesandannouncements=1`,
      );
    });
  });
  ...
  describe('getAllBlockedEmailDomains', function () {
    ...
    it('returns an array of unique domains', function () {
      fakeSettings.get
        .withArgs('blocked_email_domains')
        .returns(['domain.com', 'settingsdomain.com']);
      configUtils.set({
        spam: {
    ...
    it('normalises the list of email domains and filters out invalid domains', function () {
      fakeSettings.get
        .withArgs('blocked_email_domains')
        .returns(['Domain.com', '@example.com', 'hello@spam.xyz', 'bar', '']);
      configUtils.set({
        spam: {
