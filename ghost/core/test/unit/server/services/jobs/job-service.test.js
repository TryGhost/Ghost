...
describe('JobService', function () {
  ...
  it('does not let worker status logging failures escape', function () {
    ...
    assert.doesNotThrow(() =>
      workerMessageHandler({ name: 'clean-tokens', message: 'execution started' }),
    );
  });
  ...
  it('does not let worker failure logging failures escape either job manager', function () {
    ...
    assert.doesNotThrow(() =>
      workerErrorHandler(new Error('Job failed'), { name: 'clean-tokens' }),
    );

    require(mentionsJobServicePath);
    assert.doesNotThrow(() =>
      workerErrorHandler(new Error('Job failed'), { name: 'send-webmentions' }),
    );
  });
});
