...
describe('Email analytics queries', function () {
    ...
    describe('getJobMetadata', function () {
        ...
        it('returns empty metadata for missing, null, or invalid metadata', async function () {
            await insertJob();
            assert.deepEqual(await queries.getJobMetadata('email-analytics-scheduled'), {begin: null, end: null});

            await knex('jobs').where('name', 'email-analytics-scheduled').update({metadata: 'invalid-json'});
            assert.deepEqual(await queries.getJobMetadata('email-analytics-scheduled'), {begin: null, end: null});
        });
    });
