...
describe('GiftEmailAnalyticsBatchProcessor', function () {
    it('maps Mailgun delivery and failure events to latest gift outcomes without opens', async function () {
        ...
        sinon.assert.calledWithExactly(giftDeliveryService.recordOutcome, sinon.match({
    ...
    it('treats failed events as temporary unless Mailgun marks them permanent', async function () {
        ...
        await processor.processBatch([
            {type: 'failed', providerId: 'missing-severity', timestamp: new Date('2026-08-05T12:00:00.000Z')},
            {type: 'failed', severity: 'unexpected', providerId: 'unexpected-severity', timestamp: new Date('2026-08-05T12:01:00.000Z')},
            {type: 'failed', severity: 'permanent', providerId: 'permanent', timestamp: new Date('2026-08-05T12:02:00.000Z')}
        ], result, {});
