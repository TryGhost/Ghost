...
describe('LastSeenAtUpdater', function () {
  ...
  describe('subscribe', function () {
    it('Calls updateLastSeenAt on MemberPageViewEvents', async function () {
      ...
      DomainEvents.dispatch(
        MemberPageViewEvent.create(
          { memberId: '1', memberLastSeenAt: previousLastSeen, url: '/' },
          now.toDate(),
        ),
      );
    ...
    it('Catches errors in updateLastSeenAtWithoutKnownLastSeen on EmailOpenedEvents', async function () {
      ...
      assert(
        true,
        'The LastSeenAtUpdater should catch errors in updateLastSeenAtWithoutKnownLastSeen',
      );
    });
    ...
    it("Doesn't fire on other events", async function () {
      ...
      updater.subscribe(DomainEvents);
      DomainEvents.dispatch(
        MemberSubscribeEvent.create({ memberId: '1', source: 'api' }, new Date()),
      );
      await DomainEvents.allSettled();
      assert(
        spy.notCalled,
        'The LastSeenAtUpdater should never fire on MemberSubscribeEvent events.',
      );
    });
    ...
    describe('Disable via config', function () {
      it('MemberLinkClickEvent should not be fired when disabled', async function () {
        ...
        assert(
          updater.updateLastSeenAt.notCalled,
          'The LastSeenAtUpdater should not attempt a member update when disabled',
        );
      });
      ...
      it('MemberLinkClickEvent should be fired when enabled/empty', async function () {
        ...
        assert(
          updater.updateLastSeenAt.calledOnce,
          'The LastSeenAtUpdater should attempt a member update when not disabled',
        );
      });
    });
  });
  ...
  describe('updateLastSeenAt', function () {
    it('works correctly on another timezone (not updating last_seen_at)', async function () {
      ...
      assert(
        saveStub.notCalled,
        'The LastSeenAtUpdater should attempt a member update when the new timestamp is within the same day in the publication timezone.',
      );
    });
    ...
    it("Doesn't update when last_seen_at is too recent", async function () {
      ...
      assert(
        saveStub.notCalled,
        "The LastSeenAtUpdater should't update a member when the previous last_seen_at is close to the event timestamp.",
      );
    });
    ...
    it('avoids a race condition when updating last_seen_at', async function () {
      ...
      getStub
        .onSecondCall()
        .resolves({ get: () => now.toDate(), save: saveStub, refresh: refreshStub });
      ...
      assert(
        saveStub.calledOnce,
        `The LastSeenAtUpdater should attempt a member update only once, but was called ${saveStub.callCount} times`,
      );
      assert(
        saveStub.calledOnceWithExactly(
          sinon.match({ last_seen_at: now.tz('utc').format('YYYY-MM-DD HH:mm:ss') }),
      ...
      assert(
        events.emit.calledOnceWithExactly('member.edited', sinon.match.any),
  ...
  describe('cachedUpdateLastSeenAt', function () {
    ...
    it('only calls updateLastSeenAt once per day per member', async function () {
      ...
      assert(
        updater.updateLastSeenAt.calledOnce,
        'The LastSeenAtUpdater should only update a member once per day.',
      );
      clock.restore();
    });
    ...
    it('calls updateLastSeenAt again if it fails the first time', async function () {
      ...
      assert(
        updater.updateLastSeenAt.calledTwice,
        'The LastSeenAtUpdater should attempt to update a member again if the first update fails.',
      );
      clock.restore();
    });
  });
