...
describe('LinkClickTrackingService', function () {
  ...
  describe('addRedirectToUrl', function () {
    it('Creates a redirect', async function () {
      ...
      const addRedirect = sinon.stub().resolves({
        link_id: linkId,
      ...
      const updatedUrl = await service.addRedirectToUrl(
  ...
  describe('addTrackingToUrl', function () {
    it('Creates a redirect', async function () {
      ...
      const postId = new ObjectID().toHexString();
      const updatedUrl = await service.addTrackingToUrl(
        new URL('https://example.com/destination'),
        { id: postId },
        '123',
      );
  ...
  describe('addAutomationTrackingToUrl', function () {
    it('reuses the revision redirect and appends the member UUID', async function () {
      ...
      assert.equal(
        updatedUrl.href,
        'https://example.com/r/uniqueslug?m=00000000-0000-4000-8000-000000000001&step=run-step-id',
      );
  ...
  describe('bulkEdit', function () {
    ...
    [
      ...
      {
        name: 'a quote normalised out of the path',
        url: 'https://example.com/say"hi?ref=Test-newsletter',
      },
      {
        name: 'a backslash normalised out of the path',
        url: 'https://example.com/path\\?ref=Test-newsletter',
      },
    ...
    it('preserves hash when appending attribution params', async function () {
      ...
      assert.equal(
        updateData.to,
        'https://ghost.test/path?foo=1&ref=Test-newsletter&attribution_type=post&attribution_id=1#section',
      );
    });
  });
});
