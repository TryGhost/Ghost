    ...
    it('sanitizes notification HTML before returning it for storage', function () {
      ...
      const { notificationsToAdd } = notificationsSvc.add({
        notifications: [
          {
            message:
              '<p>Keep <strong>formatting</strong> and <a href="https://ghost.org" onclick="alert(1)">safe links</a>.</p><script>alert(1)</script>',
          },
        ],
      });

      assert.equal(
        notificationsToAdd[0].message,
        '<p>Keep <strong>formatting</strong> and <a href="https://ghost.org" target="_blank" rel="noopener noreferrer">safe links</a>.</p>',
      );
    });
        ...
        get: sinon.fake.returns([
          {
            custom: true,
            message:
              '<p>Keep <em>formatting</em>.</p><img src=x onerror="alert(1)"><script>alert(1)</script>',
