...
const StripeDepositAddressProvider = require('../../../../../core/server/services/machine-payments/stripe-deposit-address-provider');

describe('Unit: server/services/machine-payments/stripe-deposit-address-provider', function () {
    afterEach(function () {
        sinon.restore();
    });
    ...
    it('creates a fresh deposit address for each payment challenge', async function () {
        sinon.stub(settingsHelpers, 'getActiveStripeKeys').returns({
            publicKey: 'pk_test_active',
            secretKey: 'sk_test_active'
        });

        const createPaymentIntent = sinon.stub();
        createPaymentIntent.onFirstCall().resolves({
            next_action: {
                crypto_display_details: {
                    deposit_addresses: {
                        base: {
        ...
        createPaymentIntent.onSecondCall().resolves({
            next_action: {
                crypto_display_details: {
                    deposit_addresses: {
                        base: {
        ...
        sinon.assert.calledTwice(createPaymentIntent);
    });

    it('creates distinct payment intents for concurrent challenges', async function () {
        sinon.stub(settingsHelpers, 'getActiveStripeKeys').returns({
            publicKey: 'pk_test_active',
            secretKey: 'sk_test_active'
        });
