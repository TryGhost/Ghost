...
describe('Unit: server/services/machine-payments/service', function () {
    ...
    it('delegates paid content lookup to the server-side provider', async function () {
        const paidEntry = {id: 'post-id', visibility: 'paid'};
        paidContentProvider.get.withArgs('posts', 'post-id').resolves(paidEntry);

        assert.equal(await service.getPaidContent('posts', 'post-id'), paidEntry);
    });
    ...
    it('fails closed when llms.txt is disabled', async function () {
        settingsCache.get.withArgs('llms_enabled').returns(false);
