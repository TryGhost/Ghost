...
describe('UNIT: RedirectsService', function () {
  ...
  describe('activate', function () {
    it('clears existing redirects, then loads each one from the store', async function () {
      ...
      sinon.assert.calledWithExactly(redirectManager.addRedirect.firstCall, '/a', '/b', {
        permanent: true,
      });
      sinon.assert.calledWithExactly(redirectManager.addRedirect.secondCall, '/c', '/d', {
        permanent: false,
      });
      assert.ok(
    ...
    it('skips and logs invalid individual redirects without crashing', async function () {
      ...
      sinon.assert.calledTwice(redirectManager.addRedirect);
      sinon.assert.calledWithExactly(redirectManager.addRedirect.firstCall, '/ok-1', '/x', {
        permanent: true,
      });
      sinon.assert.calledWithExactly(redirectManager.addRedirect.secondCall, '/ok-2', '/z', {
        permanent: false,
      });
