...
describe('Comments Service: CommentsService', function () {
  ...
  function createClassInstance({ commentsEnabled = 'all', commentStatus = 'published' } = {}) {
    ...
    const models = {
      ...
      Comment: {
        findOne: sinon.stub().callsFake(async (data, options = {}) => {
          ...
          if (
            options.filter &&
            options.filter.includes('status:') &&
            !options.filter.includes(commentStatus)
          ) {
            return null;
          }
    ...
    const instance = new CommentsService({
      ...
      settingsHelpers: {},
      urlService: { getRequiredRelations: sinon.stub().returns([]) },
      urlUtils: {},
      contentGating: {
        BLOCK_ACCESS: 'block',
  ...
  describe('comment votes on non-public comments', function () {
    async function assertVoteActionRejects(status, run) {
      ...
      await assert.rejects(() => run(instance), errors.NotFoundError);
  ...
  describe('reportComment', function () {
    it('creates a report for a published comment', async function () {
      ...
      await instance.reportComment('comment-id', reporter, {
        columns: ['id'],
        context: { member: reporter },
      });
    ...
    it('does not report hidden or deleted comments', async function () {
      for (const status of ['hidden', 'deleted']) {
        ...
        await assert.rejects(
          () =>
            instance.reportComment('comment-id', reporter, {
              columns: ['id'],
              context: { member: reporter },
            }),
  ...
  describe('getCommentByID', function () {
    ...
    it('returns hidden comments to member-facing direct reads', async function () {
      ...
      const result = await instance.getCommentByID('comment-id', {
        columns: ['id'],
        context: { member: { id: 'member-id' } },
      });
  ...
  describe('sendNewCommentNotifications', function () {
    function setupNotificationTest({
      ...
      parentNotified = parentStatus === 'published',
    }) {
      const { instance, models } = createClassInstance();
      ...
      const commentsById = {
        'parent-id': buildCommentModel({
          id: 'parent-id',
          member_id: parentMemberId,
          status: parentStatus,
        }),
        'in-reply-to-id': buildCommentModel({
          id: 'in-reply-to-id',
          member_id: inReplyToMemberId,
          status: 'published',
        }),
      };
    ...
    it('sends a single email when the same member authored the parent and in_reply_to comments', async function () {
      ...
      sinon.assert.calledWithMatch(instance.emails.notifyParentCommentAuthor, reply, {
        type: 'parent',
      });
    });
  ...
  describe('editCommentContent', function () {
    it('checks ownership before returning an unchanged comment', async function () {
      ...
      await assert.rejects(
        () =>
          instance.editCommentContent('comment-id', 'member-id', undefined, {
            context: { member: { id: 'member-id' } },
          }),
  ...
  describe('getAdminAllComments', function () {
    it('loads dislike counts for admin moderation', async function () {
      ...
      sinon.assert.calledWith(
        models.Comment.findPage,
        sinon.match({
          withRelated: sinon.match(
            (value) => value.includes('count.dislikes') && value.includes('count.net_score'),
          ),
        }),
      );
    });
