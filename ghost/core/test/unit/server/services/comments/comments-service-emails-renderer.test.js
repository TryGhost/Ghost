...
describe('Comments Service Email Renderer', function () {
    describe('renderEmail Template with different locales', function () {
        ...
        it('should render html and text templates in English for unknown locale', async function () {
            // arrange
            const i18n = i18nLib('xx', 'ghost');
            const renderer = new CommentsServiceEmailRenderer({t: i18n.t});

            const templateData = {
                postTitle: 'Other Post',
                siteUrl: 'https://ghost.abc'
            };
            ...
            // assert
            should(result.html).containEql('Hey there,</p>');
            should(result.html).containEql('This message was sent from <a class="small" href="https://ghost.abc"');
            should(result.text).containEql('Hey there,');
            should(result.text).containEql('Someone just replied to your comment on Other Post.');
        });
    });
});
