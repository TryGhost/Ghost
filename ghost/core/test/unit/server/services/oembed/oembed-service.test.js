...
describe('oembed-service', function () {
  ...
  describe('known provider', function () {
    ...
    it('should return a ValidationError if upstream 401s', async function () {
      nock('https://www.youtube.com').get('/oembed').query(true).reply(401);
    ...
    it('should return a ValidationError if upstream 403s', async function () {
      nock('https://www.youtube.com').get('/oembed').query(true).reply(403);
    ...
    it('should return a ValidationError if upstream 404s', async function () {
      nock('https://www.youtube.com').get('/oembed').query(true).reply(404);
    ...
    it('should return a ValidationError if upstream 500s', async function () {
      nock('https://www.youtube.com').get('/oembed').query(true).reply(500);
  ...
  describe('fetchOembedDataFromUrl', function () {
    ...
    it('extracts Amazon product metadata via the metascraper-amazon ruleset', async function () {
      ...
      assert.equal(
        ...
        'https://m.media-amazon.com/images/I/example-hires.jpg',
      );
    });
    ...
    it('does not apply Amazon rules to .co hosts that merely end in "a.co"', async function () {
      ...
      const response = await oembedService.fetchOembedDataFromUrl(
        'https://www.rangemedia.co/some-post',
        'bookmark',
      );
    ...
    it('should return a bookmark response when the oembed endpoint returns a link type', async function () {
      nock('https://www.example.com')
        ...
        .reply(
          200,
          `<html><head><link type="application/json+oembed" href="https://www.example.com/oembed"><title>Example</title></head></html>`,
        );

      nock('https://www.example.com').get('/oembed').query(true).reply(200, {
        type: 'link',
        version: '1.0',
        title: 'Test Title',
        author_name: 'Test Author',
    ...
    it('enriches other allowlisted provider bookmarks without discarding page metadata', async function () {
      const knownProviderStub = sinon.stub(oembedService, 'knownProvider').resolves({
    ...
    it('uses bookmark enrichment when the provider page request fails', async function () {
      ...
      nock('https://www.youtube.com').get('/watch').query({ v: 'blocked' }).reply(403);
    ...
    it('falls back to page metadata when bookmark enrichment times out', async function () {
      nock('https://www.youtube.com').get('/oembed').query(true).delayConnection(500).reply(200, {
    ...
    it('does not process missing bookmark images', async function () {
      ...
      try {
        const response = await oembedService.fetchBookmarkData(
          'https://www.example.com',
    ...
    it('builds a YouTube bookmark card from allowlisted oEmbed provider metadata (#24741)', async function () {
      ...
      try {
        ...
        assert.equal(
          ...
          "What happens in the European Space Agency's Mission Control?",
        );
    ...
    it('converts YT live URLs to watch URLs', async function () {
      ...
      await oembedService.fetchOembedDataFromUrl(
        'https://www.youtube.com/live/1234?param=existing',
      );
    });
  ...
  describe('processImageFromUrl', function () {
      ...
      const service = new OembedService({
        ...
        imageStore: {
          getSanitizedFileName: sinon
            .stub()
            .throws(new Error('getSanitizedFileName should not be called')),
    ...
    it('stores downloaded bookmark assets via image storage and returns the adapter URL', async function () {
      ...
      const storedUrl = await service.processImageFromUrl(
        'https://example.com/sample.png?token=abc',
        'thumbnail',
      );
      ...
      assert.equal(
        storedUrl,
        'https://storage.ghost.is/c/6f/a3/site/content/images/thumbnail/sample-x.png',
      );
      sinon.assert.calledOnce(getSanitizedFileName);
      sinon.assert.calledOnce(saveRaw);
      assert.match(
        saveRaw.firstCall.args[1],
        new RegExp(`^thumbnail/sample-${UUID_RE.source}\\.png$`),
      );
    });
    ...
    it('writes a fresh key on every call, even for identical bytes (ONC-1788)', async function () {
      ...
      assert.match(
        saveRaw.secondCall.args[1],
        new RegExp(`^icon/favicon-${UUID_RE.source}\\.ico$`),
      );
    ...
    describe('SVG bookmark images', function () {
      ...
      it('stores an SVG icon as a PNG', async function () {
        ...
        await buildService(saveRaw, SVG).processImageFromUrl(
      ...
      it('bounds the output in both dimensions', async function () {
        ...
        const saveRaw = sinon.stub().resolves('/stored');
        const tall =
          '<svg xmlns="http://www.w3.org/2000/svg" width="1000" height="8000"><rect width="1000" height="8000"/></svg>';

        await buildService(saveRaw, tall).processImageFromUrl(
          'https://example.com/favicon.svg',
          'thumbnail',
        );
      ...
      it('converts a namespace-prefixed <svg:svg> root', async function () {
        const saveRaw = sinon.stub().resolves('/stored');
        const prefixed =
          '<svg:svg xmlns:svg="http://www.w3.org/2000/svg" width="16" height="16"><svg:circle cx="8" cy="8" r="7"/></svg:svg>';

        await buildService(saveRaw, prefixed).processImageFromUrl(
          'https://example.com/favicon.ico',
          'icon',
        );
      ...
      it('converts anything served under an .svg name, whatever its case', async function () {
        ...
        await buildService(saveRaw, padded).processImageFromUrl(
          'https://example.com/favicon.SVG',
          'icon',
        );
      ...
      it('rejects a gzipped SVG rather than inflating it', async function () {
        ...
        await assert.rejects(
          () =>
            buildService(saveRaw, zlib.gzipSync(Buffer.from(SVG))).processImageFromUrl(
              'https://example.com/favicon.svgz',
              'icon',
            ),
      ...
      it('rejects an SVG too large to convert', async function () {
        ...
        await assert.rejects(
          () =>
            buildService(saveRaw, huge).processImageFromUrl(
              'https://example.com/favicon.svg',
              'icon',
            ),
          { message: /too large or compressed/ },
      ...
      it('rejects markup that is not a convertible image', async function () {
        ...
        const service = buildService(
          saveRaw,
          '<!doctype html><html><body><svg><h1>404</h1></body></html>',
        );

        await assert.rejects(() =>
          service.processImageFromUrl('https://example.com/favicon.svg', 'icon'),
        );
        sinon.assert.notCalled(saveRaw);
      });
  ...
  describe('fetchBookmarkData favicon selection', function () {
    ...
    const buildHtml = (links) =>
      `<html><head><title>Test Page</title>${links}</head><body></body></html>`;

    const getIcon = (links, type) =>
      service
