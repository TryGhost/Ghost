...
describe('RemoteFlagsService', function () {
  ...
  describe('refresh - happy path', function () {
    it('fetches, resolves and applies a manifest, and logs remote_flags.applied', async function () {
      ...
      const appliedLog = logInfo
        .getCalls()
        .find((c) => c.args[0]?.system?.event === 'remote_flags.applied');
  ...
  describe('refresh - 304 not modified', function () {
    it('keeps last-known-good and does not re-log on 304', async function () {
      ...
      await service.refresh();
      const appliedCountAfterFirst = logInfo
        .getCalls()
        .filter((c) => c.args[0]?.system?.event === 'remote_flags.applied').length;
      applyOverrides.resetHistory();

      await service.refresh();

      assert.equal(applyOverrides.called, false, 'should not re-apply on 304');
      const appliedCountAfterSecond = logInfo
        .getCalls()
        .filter((c) => c.args[0]?.system?.event === 'remote_flags.applied').length;
      assert.equal(
        appliedCountAfterSecond,
        appliedCountAfterFirst,
        'should not re-log applied on 304',
      );
    });
  });
  ...
  describe('refresh - fail open', function () {
    ...
    it('keeps last-known-good and warns on a network error', async function () {
      ...
      assert.ok(
        logWarn.getCalls().some((c) => c.args[0]?.system?.event === 'remote_flags.fetch_failed'),
      );
    });
    ...
    it('keeps last-known-good and warns when the body is not valid JSON', async function () {
      ...
      request
        .onSecondCall()
        .resolves({ statusCode: 200, body: '<html>not json</html>', headers: { etag: '"v2"' } });
      ...
      assert.equal(applyOverrides.called, false);
      assert.ok(
        logWarn.getCalls().some((c) => c.args[0]?.system?.event === 'remote_flags.parse_failed'),
      );
    });
  ...
  describe('change detection', function () {
    it('applies every successful fetch but logs applied only when the resolved set changes', async function () {
      ...
      const appliedLogs = logInfo
        .getCalls()
        .filter((c) => c.args[0]?.system?.event === 'remote_flags.applied');
  ...
  describe('review hardening - etag, ordering, re-entrancy', function () {
    it('does not re-log applied when the manifest only reorders its keys', async function () {
      ...
      request
        .onFirstCall()
        .resolves(ok({ flagA: true, commentModeration: false }, { etag: '"v1"' }));
      request
        .onSecondCall()
        .resolves(ok({ commentModeration: false, flagA: true }, { etag: '"v2"' }));
    ...
    it('does not commit the ETag when applying fails, so the next poll retries instead of getting a 304', async function () {
      ...
      assert.equal(
        (request.secondCall.args[1].headers || {})['if-none-match'],
        undefined,
        'must not send a stale ETag for an unapplied manifest',
      );
      assert.equal(applyOverrides.callCount, 2);
      assert.ok(
        logWarn.getCalls().some((c) => c.args[0]?.system?.event === 'remote_flags.apply_failed'),
      );
    });
    ...
    it('treats a 3xx as an unexpected status and keeps last-known-good', async function () {
      ...
      request
        .onSecondCall()
        .resolves({ statusCode: 301, body: '', headers: { location: '/elsewhere' } });
      ...
      assert.ok(
        logWarn
          .getCalls()
          .some((c) => c.args[0]?.system?.event === 'remote_flags.fetch_bad_status'),
      );
    });
