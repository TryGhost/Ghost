...
const {
  GiftEmailService,
} = require('../../../../../core/server/services/gifts/gift-email-service');
...
describe('GiftEmailService', function () {
  ...
  it('falls back to site domain when site title is undefined', async function () {
    ...
    sinon.assert.calledWith(
      transactionalMailer.send,
      sinon.match.has('text', sinon.match('membership to example.com')),
    );
  });
  ...
  it('escapes user-controlled values containing HTML in the purchase confirmation HTML', async function () {
    ...
    sinon.assert.match(
      msg.html,
      sinon.match((value) => !value.includes('<script>alert(1)</script>')),
    );
    sinon.assert.match(
      msg.html,
      sinon.match((value) => !value.includes('<img src=x onerror=alert(1)>')),
    );
    sinon.assert.match(
      msg.html,
      sinon.match((value) => !value.includes('buyer">@example.com')),
    );
