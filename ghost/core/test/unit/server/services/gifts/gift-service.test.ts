...
import {
  GiftService,
  type GiftPurchaseData,
} from '../../../../../core/server/services/gifts/gift-service';
import { Gift } from '../../../../../core/server/services/gifts/gift';
import type {
  FindPendingReminderOptions,
  GiftRepository,
} from '../../../../../core/server/services/gifts/gift-bookshelf-repository';
...
describe('GiftService', function () {
  type GiftRepositoryStub = {
    ...
    getById: sinon.SinonStub<
      Parameters<GiftRepository['getById']>,
      ReturnType<GiftRepository['getById']>
    >;
    getByToken: sinon.SinonStub<
      Parameters<GiftRepository['getByToken']>,
      ReturnType<GiftRepository['getByToken']>
    >;
    getByPaymentIntentId: sinon.SinonStub<[string], Promise<Gift | null>>;
    getActiveByMember: sinon.SinonStub<
      Parameters<GiftRepository['getActiveByMember']>,
      ReturnType<GiftRepository['getActiveByMember']>
    >;
    getActiveByMembers: sinon.SinonStub<
      Parameters<GiftRepository['getActiveByMembers']>,
      ReturnType<GiftRepository['getActiveByMembers']>
    >;
    ...
    deleteAbandonedCheckouts: sinon.SinonStub<
      Parameters<GiftRepository['deleteAbandonedCheckouts']>,
      ReturnType<GiftRepository['deleteAbandonedCheckouts']>
    >;
    browsePurchaseEvents: sinon.SinonStub<
      Parameters<GiftRepository['browsePurchaseEvents']>,
      ReturnType<GiftRepository['browsePurchaseEvents']>
    >;
    browseRedemptionEvents: sinon.SinonStub<
      Parameters<GiftRepository['browseRedemptionEvents']>,
      ReturnType<GiftRepository['browseRedemptionEvents']>
    >;
  ...
  beforeEach(function () {
    giftRepository = {
      ...
      getById: sinon
        .stub<Parameters<GiftRepository['getById']>, ReturnType<GiftRepository['getById']>>()
        .resolves(null),
      getByToken: sinon
        .stub<Parameters<GiftRepository['getByToken']>, ReturnType<GiftRepository['getByToken']>>()
        .resolves(null),
      getByPaymentIntentId: sinon.stub<[string], Promise<Gift | null>>().resolves(null),
      getActiveByMember: sinon
        .stub<
          Parameters<GiftRepository['getActiveByMember']>,
          ReturnType<GiftRepository['getActiveByMember']>
        >()
        .resolves(null),
      getActiveByMembers: sinon
        .stub<
          Parameters<GiftRepository['getActiveByMembers']>,
          ReturnType<GiftRepository['getActiveByMembers']>
        >()
      ...
      deleteAbandonedCheckouts: sinon
        .stub<
          Parameters<GiftRepository['deleteAbandonedCheckouts']>,
          ReturnType<GiftRepository['deleteAbandonedCheckouts']>
        >()
        .resolves(0),
      browsePurchaseEvents: sinon
        .stub<
          Parameters<GiftRepository['browsePurchaseEvents']>,
          ReturnType<GiftRepository['browsePurchaseEvents']>
        >()
      ...
      transaction: sinon
        .stub<Parameters<GiftRepository['transaction']>, Promise<unknown>>()
  ...
  describe('completePurchase', function () {
    ...
    it('refuses to complete a pending gift when no buyer email is available', async function () {
      ...
      await assert.rejects(
        ...
        (err: unknown) =>
          ...
          (err as { property?: string }).property === 'buyerEmail',
      );
    ...
    it('keeps legacy completion tolerant of Stripe email formats', async function () {
      ...
      assert.equal(
        giftRepository.create.firstCall.firstArg.buyerEmail,
        'unusual..local@example.com',
      );
    });
      ...
      await assert.rejects(
        () =>
          service.completePurchase({
            ...purchaseData,
            duration: '3months',
          } as unknown as GiftPurchaseData),
        (err: any) => {
  ...
  describe('processConsumed', function () {
    ...
    it('handles multiple gifts for different members', async function () {
      ...
      giftRepository.getByToken
        .withArgs('gift-1', { transacting, forUpdate: true })
        .resolves(gift1)
        .withArgs('gift-2', { transacting, forUpdate: true })
        .resolves(gift2);
      memberRepository.get
        .withArgs({ id: 'member_1' }, { transacting, forUpdate: true })
        .resolves({
          id: 'member_1',
          get: sinon.stub().withArgs('status').returns('gift'),
        })
        .withArgs({ id: 'member_2' }, { transacting, forUpdate: true })
        .resolves({
          id: 'member_2',
          get: sinon.stub().withArgs('status').returns('gift'),
        });
  ...
  describe('processExpired', function () {
    ...
    it('marks purchased gifts past their expiry as expired', async function () {
      ...
      sinon.assert.calledOnceWithExactly(giftDeliveryService.cancelPendingForGift, gift.token, {
        transacting,
      });
    });
  ...
  describe('processReminders', function () {
    ...
    it('sends the reminder, marks the gift as reminded, and returns counts', async function () {
      ...
      sinon.assert.calledWithExactly(giftRepository.getByToken.secondCall, gift.token, {
        transacting,
        forUpdate: true,
      });
    ...
    it('continues processing the batch when one gift fails', async function () {
      ...
      giftEmailService.sendReminder
        .onFirstCall()
        .rejects(new Error('Transient SMTP error'))
        .onSecondCall()
        .resolves(undefined);
  ...
  describe('redeem', function () {
    ...
    it('uses an external transaction when provided instead of creating its own', async function () {
      ...
      sinon.assert.calledOnceWithExactly(
        memberRepository.get,
        { id: 'member_1' },
        { transacting: externalTrx, forUpdate: true },
      );
      sinon.assert.calledOnceWithExactly(
        memberRepository.update,
        {
      ...
      sinon.assert.calledOnceWithExactly(giftRepository.update, redeemed, {
        transacting: externalTrx,
      });
      assert.equal(redemption.token, 'gift-token');
    });
  ...
  describe('handlePaymentRefund', function () {
    it('saves a refunded gift and returns true', async function () {
      ...
      sinon.assert.calledOnceWithExactly(giftDeliveryService.cancelPendingForGift, saved.token, {
        transacting,
      });
    });
    ...
    it('throws when member downgrade fails', async function () {
      ...
      const service = createService();
      await assert.rejects(() => service.handlePaymentRefund({ paymentIntentId: 'pi_456' }), {
  ...
  describe('reassignRedeemer', function () {
    ...
    it('updates the member and the gift when the gift is reassignable', async function () {
      ...
      assert.deepEqual(
        memberUpdateData.products[0].expiry_at,
    ...
    it('throws BadRequestError with already-assigned message when the redeemer is set', async function () {
      ...
      await assert.rejects(
        ...
        (err: Error) =>
          err instanceof errors.BadRequestError && /already assigned/.test(err.message),
      );
      sinon.assert.notCalled(memberRepository.update);
    });
    ...
    it('throws BadRequestError with not-reassignable message for a consumed gift', async function () {
      ...
      await assert.rejects(
        service.reassignRedeemer({ giftId: 'gift_id_1', memberId: 'member_new' }),
        (err: Error) =>
          err instanceof errors.BadRequestError && /reassignable status/.test(err.message),
      );
    });
    ...
    it('throws InternalServerError when a redeemed gift has no consumesAt', async function () {
      ...
      await assert.rejects(
        service.reassignRedeemer({ giftId: 'gift_id_1', memberId: 'member_new' }),
        (err: Error) =>
          err instanceof errors.BadRequestError && /"consumes at" date/.test(err.message),
      );
      sinon.assert.notCalled(memberRepository.update);
      sinon.assert.notCalled(giftRepository.update);
    });
    ...
    it('uses the caller-supplied transaction if provided and skips opening a new one', async function () {
      ...
      const service = createService();
      await service.reassignRedeemer({
        giftId: 'gift_id_1',
        memberId: 'member_new',
        transacting: outerTransacting,
      });
    ...
    it('throws BadRequestError when the destination member already has an active paid subscription', async function () {
      ...
      await assert.rejects(
        service.reassignRedeemer({ giftId: 'gift_id_1', memberId: 'member_new' }),
        (err: Error) =>
          err instanceof errors.BadRequestError && /active subscription/.test(err.message),
      );
      sinon.assert.notCalled(memberRepository.update);
      sinon.assert.notCalled(giftRepository.update);
    });
    ...
    it('throws BadRequestError when the destination member already has a different active gift attached', async function () {
      ...
      giftRepository.getActiveByMember.resolves(
        buildGift({
      ...
      await assert.rejects(
        service.reassignRedeemer({ giftId: 'gift_id_1', memberId: 'member_new' }),
        (err: Error) =>
          err instanceof errors.BadRequestError && /different active gift/.test(err.message),
      );
      sinon.assert.notCalled(memberRepository.update);
      sinon.assert.notCalled(giftRepository.update);
    });
