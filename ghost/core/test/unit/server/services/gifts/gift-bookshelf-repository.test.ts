...
type TestGiftBookshelfModel = Omit<GiftBookshelfModel, 'findPage'> &
  Partial<Pick<GiftBookshelfModel, 'findPage'>>;
...
describe('GiftBookshelfRepository', function () {
  ...
  describe('getActiveByMembers', function () {
    ...
    it('queries with an NQL filter for redeemed status and the supplied member ids', async function () {
      ...
      assert.equal(callArg.filter, `redeemer_member_id:['member_1','member_2']+status:redeemed`);
    });
