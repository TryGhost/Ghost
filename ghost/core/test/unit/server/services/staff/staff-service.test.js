...
function testCommonMailData({ mailStub, getEmailAlertUsersStub }) {
  ...
  sinon.assert.calledWith(
    ...
    sinon.match.has(
      'html',
      sinon.match('https://admin.ghost.example/#/settings/staff/ghost/email-notifications'),
    ),
  );
}
  ...
  describe('email notifications:', function () {
    ...
    describe('subscribeEvents', function () {
      ...
      it('listens to events', async function () {
        ...
        DomainEvents.dispatch(
          SubscriptionActivatedEvent.create({
            source: 'member',
            memberId: 'member-1',
    ...
    describe('notifyPaidSubscriptionStart', function () {
      ...
      it('sends paid subscription start alert with attribution', async function () {
        ...
        await service.emails.notifyPaidSubscriptionStarted(
          { member, offer: null, tier, subscription, attribution },
          options,
        );
      ...
      it('sends paid subscription start alert without offer', async function () {
        await service.emails.notifyPaidSubscriptionStarted(
          { member, offer: null, tier, subscription },
          options,
        );
        ...
        assert.equal(mailStub.calledWith(sinon.match.has('html', sinon.match('Offer'))), false);
      });
      ...
      it('sends paid subscription start alert without member name', async function () {
        ...
        await service.emails.notifyPaidSubscriptionStarted(
          { member: memberData, offer: null, tier, subscription },
          options,
        );
      ...
      it('sends paid subscription start alert with percent offer - first payment', async function () {
        await service.emails.notifyPaidSubscriptionStarted(
          { member, offer, tier, subscription },
          options,
        );
        ...
        sinon.assert.calledWith(
          ...
          sinon.match.has(
            'html',
            sinon.match('Test Tier: $50.00/month - Offer: Half price - 50% off, first payment'),
          ),
        );
      });
      ...
      it('sends paid subscription start alert with gift-derived trial period', async function () {
        ...
        await service.emails.notifyPaidSubscriptionStarted(
          { member, offer: null, tier, subscription: trialSubscription },
          options,
        );
        ...
        sinon.assert.calledWith(
          mailStub,
          sinon.match.has('html', sinon.match('Test Tier: $50.00/month - 30 days free')),
        );
      });
      ...
      it('does not show inline trial on tier line when a trial offer is present', async function () {
        ...
        await service.emails.notifyPaidSubscriptionStarted(
          { member, offer, tier, subscription: trialSubscription },
          options,
        );
    ...
    describe('notifyPaidSubscriptionCancel', function () {
      ...
      it('sends paid subscription cancel notification when sub is canceled at the end of billing period', async function () {
        await service.emails.notifyPaidSubscriptionCanceled(
          {
            member,
            tier,
            subscription: {
      ...
      it('sends paid subscription cancel alert when sub is canceled without reason', async function () {
        await service.emails.notifyPaidSubscriptionCanceled(
          { member, tier, subscription, expiryAt, cancelNow },
          options,
        );
        ...
        assert.equal(
          mailStub.calledWith(sinon.match.has('html', sinon.match('Cancellation reason'))),
          false,
        );
      });
      ...
      it('sends paid subscription cancel alert when subscription is canceled immediately', async function () {
        ...
        assert.equal(
          mailStub.calledWith(sinon.match.has('html', sinon.match('Canceled on'))),
          false,
        );
      ...
      it('send Members milestone email', async function () {
        ...
        sinon.assert.calledWith(
          ...
          sinon.match.has(
            'html',
            sinon.match(
              'src="https://static.ghost.org/v5.0.0/images/milestone-email-members-25k.png" width="580" align="center"',
            ),
          ),
        );
      ...
      it('send ARR milestone email', async function () {
        ...
        sinon.assert.calledWith(
          ...
          sinon.match.has(
            'html',
            sinon.match(
              'src="https://static.ghost.org/v5.0.0/images/milestone-email-usd-500k.png" width="580" height="348" align="center"',
            ),
          ),
        );
        ...
        sinon.assert.calledWith(
          ...
          sinon.match.has(
            'html',
            sinon.match(
              '<strong>Ghost Site</strong> is now generating <strong>$500,000</strong> in annual recurring revenue. Congratulations &mdash; this is a significant milestone.',
            ),
          ),
        );
    ...
    describe('notifyGiftPurchased', function () {
      it('sends gift email with correct subject', async function () {
        ...
        sinon.assert.calledWith(
          mailStub,
          sinon.match.has('subject', sinon.match('Gift subscription purchased: $60.00 from Alice')),
        );
      });
      ...
      it('falls back to email when name is null', async function () {
        ...
        sinon.assert.calledWith(
          mailStub,
          sinon.match.has('subject', sinon.match('from anon@example.com')),
        );
      });
    ...
    describe('notifyGiftSubscriptionStarted', function () {
      it('sends gift subscription email with correct subject', async function () {
        ...
        sinon.assert.calledWith(
          mailStub,
          sinon.match.has('subject', sinon.match('🎁 Gift subscription redeemed: Jamie')),
        );
      });
      ...
      it('includes the tier and cadence in HTML and plain text', async function () {
        ...
        sinon.assert.calledWith(
          mailStub,
          sinon.match.has('text', sinon.match('Tier: Premium • 1 year')),
        );
      });
      ...
      it('pluralises the cadence when duration is greater than 1', async function () {
        ...
        sinon.assert.calledWith(
          mailStub,
          sinon.match.has('text', sinon.match('Tier: Premium • 3 months')),
        );
      });
      ...
      it('includes the buyer email in HTML and plain text', async function () {
        ...
        sinon.assert.calledWith(
          mailStub,
          sinon.match.has('html', sinon.match('gifter@example.com')),
        );
        sinon.assert.calledWith(
          mailStub,
          sinon.match.has('text', sinon.match('Gifted by: gifter@example.com')),
        );
      });
    });
  });
