...
describe('register-job-handlers', function () {
  ...
  let giftService: { cleanup: sinon.SinonStub; sendReminders: sinon.SinonStub };
  ...
  beforeEach(function () {
    ...
    giftService = { cleanup: sinon.stub().resolves(), sendReminders: sinon.stub().resolves() };
  ...
  it('runs send-gift-reminders with the injected gift service', async function () {
    ...
    assert.ok(giftService.sendReminders.calledOnce);
  });
  ...
  it('propagates send-gift-reminders failures', async function () {
    ...
    giftService.sendReminders.rejects(error);
    const sendGiftRemindersHandler = handlerFor('send-gift-reminders');
