...
describe('ReferrersStatsService', function () {
  describe('getReferrerHistory', function () {
    ...
    it('Responds with correct data', async function () {
      ...
      assert.deepEqual(
        results.data.map((result) => result.date),
        expectedDates,
      );
  ...
  describe('getTopSourcesWithRange', function () {
    ...
    it('should properly deduplicate members who converted from free to paid', async function () {
      ...
      // Query for January
      const result = await stats.getTopSourcesWithRange({
      ...
      assert.equal(
        googleStats.signups,
        2,
        'Google should have 2 signups (excluding only member who converted in same window)',
      );
      assert.equal(
        googleStats.paid_conversions,
        1,
        'Google should have 1 paid conversion in January',
      );

      // Query for February to see member_2's conversion
      const febResult = await stats.getTopSourcesWithRange({
      ...
      assert.equal(
        febGoogleStats.paid_conversions,
        1,
        'Google should have 1 paid conversion in February',
      );
    });
      ...
      await db('members_created_events').insert([
        {
          member_id: 'jan_1',
          referrer_source: 'Google',
          created_at: moment('2024-01-15').utc().format('YYYY-MM-DD HH:mm:ss'),
        },
        {
          member_id: 'jan_2',
          referrer_source: 'Google',
          created_at: moment('2024-01-20').utc().format('YYYY-MM-DD HH:mm:ss'),
        },
        {
          member_id: 'feb_1',
          referrer_source: 'Google',
          created_at: moment('2024-02-15').utc().format('YYYY-MM-DD HH:mm:ss'),
        },
        {
          member_id: 'feb_2',
          referrer_source: 'Google',
          created_at: moment('2024-02-20').utc().format('YYYY-MM-DD HH:mm:ss'),
        },
      ]);

      const janResult = await stats.getTopSourcesWithRange({
        date_from: '2024-01-01',
        date_to: '2024-01-31',
        timezone: 'UTC',
      });
    ...
    it('should sort results correctly', async function () {
      ...
      const signupsResult = await stats.getTopSourcesWithRange({
        date_from: '2024-01-01',
        date_to: '2024-01-31',
        timezone: 'UTC',
        order: 'signups desc',
      ...
      assert.equal(
        signupsResult.data[0].source,
        'Twitter',
        'Twitter should be first when sorted by signups',
      );
      ...
      const sourceResult = await stats.getTopSourcesWithRange({
        date_from: '2024-01-01',
        date_to: '2024-01-31',
        timezone: 'UTC',
        order: 'source desc',
      ...
      assert.equal(
        sourceResult.data[0].source,
        'Twitter',
        'Sources should be sorted alphabetically descending',
      );
    });
    ...
    it('should handle members with same source converting on different dates', async function () {
      ...
      assert.equal(
        janGoogle.signups,
        1,
        'January should count this member as signup (conversion happened later)',
      );
    ...
    it('should handle members who sign up and convert with different sources', async function () {
      ...
      if (googleStats) {
        assert.equal(
          googleStats.signups,
          0,
          'Google should have 0 signups (member converted in same window)',
        );
