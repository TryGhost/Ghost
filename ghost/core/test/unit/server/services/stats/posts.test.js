...
describe('PostsStatsService', function () {
  ...
  async function _createPaidConversionEvent(
    postId,
    memberId,
    subscriptionId,
    mrr,
  ...
  async function _createPaidSignup(
    postId,
    mrr,
    referrerSource,
    memberId = null,
    ...
    await _createFreeSignupEvent(postId, finalMemberId, referrerSource);
    await _createPaidConversionEvent(
      postId,
      finalMemberId,
      finalSubscriptionId,
      mrr,
  ...
  async function _createPaidConversion(
    signupPostId,
    conversionPostId,
    mrr,
    referrerSource,
  ...
  describe('getTopPosts', function () {
    ...
    it('correctly ranks posts by free_members', async function () {
      ...
      const expectedResults = [
        {
          ...
          post_id: 'post1',
          attribution_type: 'post',
          attribution_id: 'post1',
          published_at: result.data[0].published_at,
          post_type: 'post',
          url_exists: true,
        },
        {
          attribution_url: '/post-2/',
          title: 'Post 2',
          free_members: 1,
          paid_members: 1,
          mrr: 500,
          post_id: 'post2',
          attribution_type: 'post',
          attribution_id: 'post2',
          published_at: result.data[1].published_at,
          post_type: 'post',
          url_exists: true,
        },
        {
          attribution_url: '/post-3/',
          title: 'Post 3',
          free_members: 0,
          paid_members: 1,
          mrr: 1000,
          post_id: 'post3',
          attribution_type: 'post',
          attribution_id: 'post3',
          published_at: result.data[2].published_at,
          post_type: 'post',
          url_exists: true,
        },
      ];
      ...
      assert.deepEqual(
        result.data,
        expectedResults,
        'Results should match expected order and counts for free_members desc',
      );
    });
    ...
    it('correctly ranks posts by paid_members', async function () {
      ...
      const expectedResults = [
        {
          ...
          attribution_type: 'post',
          attribution_id: 'post2',
          published_at: result.data.find((p) => p.post_id === 'post2').published_at,
          post_type: 'post',
          url_exists: true,
        },
        {
          ...
          attribution_type: 'post',
          attribution_id: 'post1',
          published_at: result.data.find((p) => p.post_id === 'post1').published_at,
          post_type: 'post',
          url_exists: true,
        },
        {
          ...
          attribution_type: 'post',
          attribution_id: 'post3',
          published_at: result.data.find((p) => p.post_id === 'post3').published_at,
          post_type: 'post',
          url_exists: true,
        },
        {
          attribution_url: '/post-4/',
          title: 'Post 4',
          free_members: 1,
          paid_members: 0,
          mrr: 0,
          post_id: 'post4',
          attribution_type: 'post',
          attribution_id: 'post4',
          published_at: result.data.find((p) => p.post_id === 'post4').published_at,
          post_type: 'post',
          url_exists: true,
        },
      ];
      ...
      assert.deepEqual(
        sortedResults,
        expectedResults,
        'Results should match expected order and counts for paid_members desc',
      );
    });
    ...
    it('correctly ranks posts by mrr', async function () {
      ...
      assert.deepEqual(
        sortedResults,
        expectedResults,
        'Results should match expected order and counts for mrr desc',
      );
    });
    ...
    it('properly filters by date range', async function () {
      ...
      assert.equal(
        lastFifteenDaysResult.data.length,
        1,
        'Should return only posts with events in date range',
      );
      ...
      assert.equal(
        lastThirtyDaysResult.data.length,
        2,
        'Should return both posts when date range includes both events',
      );
  ...
  describe('getReferrersForPost', function () {
    ...
    it('respects the limit parameter', async function () {
      ...
      assert.equal(
        returnedSources.every((source) => validSources.includes(source)),
        true,
  ...
  describe('getGrowthStatsForPost', function () {
    it('returns growth stats for a post', async function () {
      ...
      const expectedResults = [{ post_id: 'post1', free_members: 2, paid_members: 1, mrr: 500 }];
    ...
    it('returns growth stats for a page', async function () {
      ...
      const expectedResults = [{ post_id: 'page1', free_members: 2, paid_members: 1, mrr: 500 }];
  ...
  describe('getTopPostsViews', function () {
    it('returns latest posts with zero views when no Tinybird client exists', async function () {
      ...
      result.data.forEach((post) => {
        ...
        assert.ok(
          ...
          'Post should have a published_at timestamp',
        );
      });
      ...
      for (let i = 1; i < result.data.length; i++) {
        assert.ok(
          result.data[i - 1].published_at >= result.data[i].published_at,
          'Posts should be ordered by published_at desc',
        );
      }
    });
    ...
    it('counts free and paid members separately without deduplication', async function () {
      const mockTinybirdClient = {
        fetch: (endpoint) => {
          if (endpoint === 'api_top_pages') {
            return Promise.resolve([{ post_uuid: 'uuid1', visits: 1000 }]);
          }
          return Promise.resolve([]);
        },
      };
    ...
    it('returns properly formatted multiple authors with single commas', async function () {
      ...
      assert.equal(
        authorsString,
        'Alice Johnson, Bob Wilson, Carol Smith',
        'Authors should be formatted as "Alice Johnson, Bob Wilson, Carol Smith"',
      );
    });
    ...
    it('filters out pages when no Tinybird client exists (fallback)', async function () {
      ...
      result.data.forEach((item) => {
        assert.ok(
          ['post1', 'post2'].includes(item.post_id),
          `Should only return posts, but got ${item.post_id}`,
        );
        assert.ok(
          ['Test Post 1', 'Test Post 2'].includes(item.title),
          `Should only return post titles, but got ${item.title}`,
        );
      });
    });
    ...
    it('filters out pages when backfilling additional posts', async function () {
      ...
      result.data.forEach((item) => {
        assert.ok(
          ['post1', 'post2', 'post3'].includes(item.post_id),
          `Should only return posts, but got ${item.post_id}`,
        );
        assert.ok(
          ['Test Post 1', 'Test Post 2', 'Test Post 3'].includes(item.title),
          `Should only return post titles, but got ${item.title}`,
        );
      });
  ...
  describe('getNewsletterSubscriberStats', function () {
    ...
    it('should return cumulative values instead of deltas', async function () {
      ...
      await _createNewsletterSubscription(
        newsletterId,
        'member1',
        true,
      ...
      assert.equal(
        stats.values[2].value,
        2,
        'Day 3: cumulative total of 2 (one unsubscribe, one resubscribe)',
      );
    });
    ...
    it('should exclude email_disabled members', async function () {
      ...
      assert.equal(
        stats.values[1].value,
        1,
        'Should stay 1 on second day (member2 excluded due to email_disabled)',
      );
    });
    ...
    it('should handle multiple events on same day', async function () {
      ...
      assert.equal(
        stats.values[0].value,
        2,
        'Net change for the day: +3 -1 = +2, starting from 0 = 2',
      );
    });
    ...
    it('should respect date filters', async function () {
      ...
      assert.equal(
        stats.values[0].value,
        2,
        'Day 1 (Jan 4): Should show 2 (starting value before Jan 5 event)',
      );
  ...
  describe('_fillMissingDates', function () {
    it('fills in all missing dates in the range', function () {
      ...
      const result = service._fillMissingDates(sparseValues, '2024-01-01', '2024-01-05', 'UTC', 5);
    ...
    it('uses startingValue when no events exist at the beginning', function () {
      const sparseValues = [{ date: '2024-01-03', value: 20 }];
    ...
    it('handles empty values array', function () {
      const result = service._fillMissingDates([], '2024-01-01', '2024-01-03', 'UTC', 50);
    ...
    it('returns values as-is when no date range provided', function () {
      const sparseValues = [{ date: '2024-01-01', value: 10 }];

      const result = service._fillMissingDates(sparseValues, null, null, 'UTC', 0);

      assert.deepEqual(
        result,
        sparseValues,
        'Should return input values unchanged when no date range',
      );
    });

    it('returns empty array when values is null and no dates', function () {
      const result = service._fillMissingDates(null, null, null, 'UTC', 0);

      assert.deepEqual(result, []);
    });

    it('handles single day range', function () {
      const sparseValues = [{ date: '2024-01-01', value: 25 }];

      const result = service._fillMissingDates(sparseValues, '2024-01-01', '2024-01-01', 'UTC', 10);
    ...
    it('respects timezone when parsing dates', function () {
      const sparseValues = [{ date: '2024-01-02', value: 30 }];
