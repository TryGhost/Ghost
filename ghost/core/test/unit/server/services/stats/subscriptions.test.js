...
describe('SubscriptionStatsService', function () {
  describe('getSubscriptionHistory', function () {
    ...
    const createEvent =
      (type) =>
      (id, attr = {}) =>
        Object.assign({ tier: null, cadence: null, mrr: null }, attr, { id, type });
    ...
    it('Correctly handles upgrades', async function () {
      ...
      // Check totals
      assert(
        (result.meta.totals.find(
          (item) => item.tier === 'basic' && item.cadence === 'month',
        ).count = 1),
      );
      assert(
        (result.meta.totals.find((item) => item.tier === 'basic' && item.cadence === 'year').count =
          1),
      );
      assert(
        (result.meta.totals.find(
          (item) => item.tier === 'beyond' && item.cadence === 'month',
        ).count = 1),
      );
      assert(
        (result.meta.totals.find(
          (item) => item.tier === 'beyond' && item.cadence === 'year',
        ).count = 3),
      );
      ...
      const finder = (tier, cadence, date) => (resultItem) => {
        return (
          resultItem.tier === tier && resultItem.cadence === cadence && resultItem.date === date
        );
      };
