...
describe('UNIT > Settings BREAD Service:', function () {
  ...
  describe('read', function () {
    it('returns the setting ', async function () {
      const defaultSettingsManager = new SettingsBreadService({
        SettingsModel: {},
        settingsCache: {
          get: sinon.stub().withArgs('version_notifications', { resolve: false }).returns({
    ...
    it('throws an error when reading unknown setting', async function () {
      const defaultSettingsManager = new SettingsBreadService({
        SettingsModel: {},
        settingsCache: {
          get: sinon.stub().withArgs('unknown_setting', { resolve: false }).returns(null),
        },
        mail,
        urlUtils,
        singleUseTokenProvider: {},
  ...
  describe('edit', function () {
    function createLabsService({ automations = false } = {}) {
      ...
      return new SettingsBreadService({
        ...
        settingsCache: {
          get: sinon.stub().withArgs('labs', { resolve: false }).returns(labsSetting),
        },
        mail,
        urlUtils,
        singleUseTokenProvider: {},
  ...
  describe('publicSiteAccess limit', function () {
    ...
    function createService({ isDisabled = false } = {}) {
      return new SettingsBreadService({
        SettingsModel: {
          async edit(changes) {
            return changes.map((change) => ({
              toJSON: () => ({ ...siteSettings[change.key], ...change }),
            }));
          },
        },
        settingsCache: {
    ...
    describe('edit', function () {
      ...
      it('rejects external attempts to change password when the limit is disabled', async function () {
        ...
        await assert.rejects(
          service.edit(
            [{ key: 'password', value: 'attacker-chosen-code' }],
            { context: { user: 'test' } },
            null,
          ),
          /Site visibility and access code cannot be changed/,
  ...
  describe('verifyKeyUpdate', function () {
    ...
    it('can not set other fields', async function () {
      ...
      await assert.rejects(
        defaultSettingsManager.verifyKeyUpdate('test'),
        /Not allowed to update this setting key via tokens/,
      );
    });
  });
});
