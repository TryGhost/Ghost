...
describe('Users service', function () {
  describe('lockAll', function () {
    ...
    it('rotates passwords on suspended users while preserving suspension', async function () {
      ...
      assert.equal(
        suspended.passwordRotated,
        true,
        'suspended users still get their password rotated so a compromised credential cannot survive a future unsuspend',
      );
    });
            ...
            knex: (tableName) => {
              if (tableName === 'posts_authors') {
                return {
                  ...
                  select() {
                    return Promise.resolve(userPostIds.map((post_id) => ({ post_id })));
                  },
                };
              }
              ...
              if (tableName === 'posts_tags') {
                return {
                  ...
                  select() {
                    return Promise.resolve(alreadyTaggedPostIds.map((post_id) => ({ post_id })));
                  },
                  insert(rows) {
