...
async function validateHtml(html) {
  ...
  assert.equal(
    report.valid,
    true,
    'Expected valid HTML without warnings, got errors:\n' + parsedErrors.join('\n\n'),
  );
}
...
describe('Email renderer', function () {
  ...
  describe('buildReplacementDefinitions', function () {
    ...
    it('returns manage_account_url', function () {
      ...
      assert.equal(
        replacements[0].getValue(member),
        'http://example.com/subdirectory/#/portal/account',
      );
    });
    ...
    it('returns status_text', function () {
      ...
      assert.equal(
        replacements[0].getValue(member),
        'Your free trial ends on 13 March 2050, at which time you will be charged the regular price. You can always cancel before then.',
      );
    });
    ...
    it('supports combination of multiple fallback values', function () {
      const html =
        'Hey %%{first_name, "there"}%%, %%{first_name, "member"}%% %%{first_name}%% %%{first_name, "there"}%%';
      const replacements = emailRenderer.buildReplacementDefinitions({
        html,
        newsletterUuid: newsletter.get('uuid'),
      });
    ...
    it('handles members uuid and key', function () {
      ...
      const memberHmac = crypto
        .createHmac('sha256', getMembersValidationKey())
        .update(member.uuid)
        .digest('hex');
  ...
  describe('getMemberStatusText', function () {
    ...
    it('Returns for canceled paid member', function () {
      ...
      assert.equal(
        result,
        'Your subscription has been canceled and will expire on 13 March 2023. You can resume your subscription via your account settings.',
      );
    });
    ...
    it('Returns for a paid member without subscriptions', function () {
      ...
      assert.equal(
        result,
        'Your subscription has been canceled and will expire on 13 March 2050. You can resume your subscription via your account settings.',
      );
    });
  ...
  describe('renderBody', function () {
    ...
    it('Converts a mobiledoc-only post to lexical before rendering', async function () {
      ...
      const convertedFromMobiledoc = renderersStub.lexical.render.getCalls().some((call) => {
        ...
        return (
          typeof lexicalArg === 'string' &&
          lexicalArg.includes('"type":"root"') &&
          lexicalArg.includes('Legacy mobiledoc content')
        );
      });
      assert.ok(
        convertedFromMobiledoc,
        'mobiledoc content is converted to lexical and passed to the renderer',
      );

      // the mobiledoc renderer stub must never be touched - it no longer exists in the renderers
      assert.equal(
        renderersStub.mobiledoc.render.called,
        false,
        'the mobiledoc renderer is not used',
      );
    ...
    for (const locale of ['fa', 'ar', 'he', 'ur']) {
      it(`Renders RTL <html> attributes for ${locale}`, async function () {
        ...
        assert.match(
          response.html,
          new RegExp(`<html lang="${locale}" dir="rtl">`),
          `expected rtl <html> for ${locale}`,
        );
        assert.match(
          response.html,
          /direction:\s*rtl/,
          `expected direction: rtl in body for ${locale}`,
        );
        assert.match(
          response.html,
          /class="feedback-buttons-container" dir="rtl"/,
          `expected feedback buttons dir="rtl" for ${locale}`,
        );
      });
    }
    ...
    it('returns feedback buttons and unsubscribe links', async function () {
      ...
      assert.deepEqual(
        response.replacements.map((r) => r.id),
        ['uuid', 'key', 'unsubscribe_url', 'list_unsubscribe'],
      );
    ...
    for (const visibility of ['public', 'members', 'paid', 'tiers']) {
      it(`includes share links for posts with ${visibility} visibility`, async function () {
        ...
        assert(
          response.html.includes('href="http://example.com/#/share"'),
          `Expected share link for "${visibility}" visibility`,
        );
        assert(
          response.html.includes('>Share</p>'),
          `Expected share button text for "${visibility}" visibility`,
        );
      });
    }
    ...
    it('excludes preheader spacing characters from plaintext', async function () {
      ...
      assert(
        !response.plaintext.includes(FIGURE_SPACE),
        'plaintext should not contain preheader figure space',
      );
      assert(
        !response.plaintext.includes(COMBINING_GRAPHEME_JOINER),
        'plaintext should not contain preheader combining grapheme joiner',
      );
      assert(
        !response.plaintext.includes(SOFT_HYPHEN),
        'plaintext should not contain preheader soft hyphen',
      );
    });
    ...
    it('tracks links containing %%{uuid}%% and preserves placeholder in destination', async function () {
      ...
      const transistorCall = addTrackingToUrlStub
        .getCalls()
        .find((call) => call.args[0].href.includes('transistor.fm'));
      ...
      const hasPlaceholder = href.includes('%%{uuid}%%') || href.includes('%25%25%7Buuid%7D%25%25');
    ...
    it('removes data-gh-segment and renders paywall', async function () {
      ...
      assert.equal(
        response.html.match(/Test Post/g).length,
        3,
        'Should contain the post name 3 times: in the title element, the preheader and in the post title section',
      );
    ...
    it('paywalls a tiers post for the no-access segment and not for the access segment', async function () {
      ...
      const preview = await emailRenderer.renderBody(
        post,
        newsletter,
        "status:-free+(product:'gold')",
        {},
    ...
    it('renders tier-gated content according to a specific tier segment', async function () {
      ...
      const silver = await emailRenderer.renderBody(
        post,
        newsletter,
        "status:-free+product:'silver'",
        {},
      ...
      const bronze = await emailRenderer.renderBody(
        post,
        newsletter,
        "status:-free+product:'bronze'",
        {},
      );
    ...
    it('does not paywall a tier segment on a paid-visibility post', async function () {
      ...
      const tierSegment = await emailRenderer.renderBody(
        post,
        newsletter,
        "status:-free+product:'gold'",
        {},
    ...
    it('should output valid HTML and escape HTML characters in mobiledoc', async function () {
      ...
      assert.equal(
        response.html.includes('<span>Footer content with valid HTML</span>'),
        true,
        'Should include footer content without escaping',
      );
    ...
    it('does not replace img height and width with auto from css', async function () {
      ...
      renderedPost =
        '<p>This is the post.</p><figure class="kg-card kg-image-card"><img src="__GHOST_URL__/content/images/2023/07/audio-sample_thumb.png" class="kg-image" alt loading="lazy" width="248" height="248"></figure><p>Theres an image!</p>';
      ...
      assert.equal(
        response.html.includes('width="248" height="248"'),
        true,
        'Should not replace img height and width with auto from css',
      );
      assert.equal(
        response.html.includes('width="auto" height="auto"'),
        false,
        'Should not replace img height and width with auto from css',
      );
    });
    ...
    describe('show excerpt', function () {
      it('is rendered when enabled and customExcerpt is present', async function () {
        const post = createModel(
          Object.assign({}, basePost, { custom_excerpt: 'This is an excerpt' }),
        );
        const newsletter = createModel({
          show_post_title_section: true,
          show_excerpt: true,
        ...
        assert.equal(
          response.html.match(/This is an excerpt/g).length,
          2,
          'Excerpt should appear twice (preheader and excerpt section)',
        );
      });
      ...
      it('is not rendered when disabled and customExcerpt is present', async function () {
        ...
        assert.equal(
          response.html.match(/This is an excerpt/g).length,
          1,
          'Subtitle should only appear once (preheader, excerpt section skipped)',
        );
    ...
    const testLexicalRenderDesignOptions = async function ({ expectedObject }) {
      ...
      sinon.assert.calledWithMatch(renderersStub.lexical.render, post.get('lexical'), {
        target: 'email',
        postUrl: 'http://example.com',
    ...
    it('does not entity-encode interpolated values in the t helper', async function () {
      ...
      for (const entity of forbiddenInPlaintext) {
        assert.equal(
          response.plaintext.includes(entity),
          false,
          `Plaintext should not contain ${entity}`,
        );
      }
      ...
      assert.ok(
        response.html.includes('O&#39;Brien'),
        'HTML should contain single-encoded apostrophe',
      );
      assert.ok(
        response.html.includes('&amp; Co.'),
        'HTML should contain single-encoded ampersand',
      );

      // Publication date flows through {{t '{date}'}} (double-brace) —
      // the actual symptom path in the issue.
      assert.ok(
        response.html.includes('19/03/2026'),
        'HTML should contain the literal slash-separated date',
      );
      assert.ok(
        response.plaintext.includes('19/03/2026'),
        'Plaintext should contain the literal slash-separated date',
      );

      assert.ok(
        response.plaintext.includes("Author/Name O'Brien & Co."),
        'Plaintext should fully decode to raw characters',
      );
    });
  });
  ...
  describe('getTemplateData', function () {
            ...
            findAll: [
              {
                ...
                custom_excerpt:
                  'Super long custom excerpt. Super long custom excerpt. Super long custom excerpt. Super long custom excerpt. Super long custom excerpt.',
    ...
    async function templateDataWithSettings(settingsObj) {
      ...
      const data = await emailRenderer.getTemplateData({
        post,
        newsletter,
        html,
        addPaywall: false,
    ...
    it('Uses the correct post title color', async function () {
      ...
      for (const test of tests) {
        ...
        assert.equal(
          data.postTitleColor,
          test.expected,
          `Failed for post_title_color input: ${test.input}`,
        );
      }
    });
    ...
    it('includes share URL for posts with public, members, paid, and tiers visibility', async function () {
      ...
      for (const visibility of ['public', 'members', 'paid', 'tiers']) {
        ...
        assert.equal(
          data.post.shareUrl,
          'http://example.com/#/share',
          `Expected share URL for "${visibility}" visibility`,
        );
      }
    });
    ...
    it('uses newsletter font styles', async function () {
      ...
      assert.deepEqual(data.classes, {
        ...
        excerpt:
          'post-excerpt post-excerpt-no-feature-image post-excerpt-serif-sans post-excerpt-left',
    ...
    it('has correct excerpt classes for serif title+body', async function () {
      ...
      assert.equal(
        ...
        'post-excerpt post-excerpt-no-feature-image post-excerpt-serif-serif post-excerpt-left',
      );
    });
    ...
    it('latestPosts can be enabled', async function () {
      ...
      assert.deepEqual(data.latestPosts, [
        {
          excerpt:
            'Super long custom excerpt. Super long custom excerpt. Super long custom excerpt. Super lo<span class="desktop-only">ng custom excerpt. Super long </span>…',
          title: 'Test Post 1',
    ...
    async function testDataProperty(
      newsletterSettings,
      property,
      expectedValue,
      options = { labsEnabled: false },
    ) {
    ...
    async function testButtonColor(
      settingValue,
      expectedValue,
      expectedTextValue,
      otherSettings = {},
    ...
    async function testHeaderBackgroundColor(
      settingValue,
      expectedValue,
      expectedIsDarkValue,
      otherSettings = {},
    ...
    [
      ...
      it(`sets headerBackgroundColor/headerBackgroundIsDark to correct values for ${settingValue}${otherSettings ? ` with ${JSON.stringify(otherSettings)}` : ''}`, async function () {
        await testHeaderBackgroundColor(
          settingValue,
          expectedValue,
          expectedIsDarkValue,
          otherSettings,
  ...
  describe('truncateHTML', function () {
    it('works correctly', async function () {
      const emailRenderer = new EmailRenderer({});
      assert.equal(
        emailRenderer.truncateHtml('This is a short one', 10, 5),
  ...
  describe('limitImageWidth', function () {
    it('Limits width of local images', async function () {
      ...
      const response = await emailRenderer.limitImageWidth(
        'http://your-blog.com/content/images/2017/01/02/example.png',
      );
      assert.equal(response.width, 600);
      assert.equal(response.height, 300);
      assert.equal(
        response.href,
        'http://your-blog.com/content/images/size/w1200/2017/01/02/example.png',
      );
    });
    ...
    it('Limits width and height of local images', async function () {
      ...
      assert.equal(
        response.href,
        'http://your-blog.com/content/images/size/w1200h1200/2017/01/02/example.png',
      );
    });
    ...
    it('Limits width of CDN content images', async function () {
      ...
      const response = await emailRenderer.limitImageWidth(
        'https://storage.ghost.is/c/6f/a3/test/content/images/2026/02/example.png',
      );
      assert.equal(response.width, 600);
      assert.equal(response.height, 300);
      assert.equal(
        response.href,
        'https://storage.ghost.is/c/6f/a3/test/content/images/size/w1200/2026/02/example.png',
      );
    });
    ...
    it('Does not rewrite external content/images URLs', async function () {
      ...
      const response = await emailRenderer.limitImageWidth(
        'https://example.com/content/images/example.png',
      );
      assert.equal(response.width, 600);
      assert.equal(response.height, 300);
    ...
    it('Does not double-rewrite already-sized CDN image URLs', async function () {
      ...
      const response = await emailRenderer.limitImageWidth(
        'https://storage.ghost.is/c/6f/a3/test/content/images/size/w600/2026/02/example.png',
      );
      assert.equal(response.width, 600);
      assert.equal(response.height, 300);
      assert.equal(
    ...
    it('Limits width of unsplash images', async function () {
      ...
      assert.equal(
        response.href,
        'https://images.unsplash.com/photo-1657816793628-191deb91e20f?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=MnwxMTc3M3wwfDF8YWxsfDJ8fHx8fHwyfHwxNjU3ODkzNjU5&ixlib=rb-1.2.1&q=80&w=1200',
      );
    });
    ...
    it('Limits width and height of unsplash images', async function () {
      ...
      assert.equal(
        response.href,
        'https://images.unsplash.com/photo-1657816793628-191deb91e20f?crop=entropy&cs=tinysrgb&fit=crop&fm=jpg&ixid=MnwxMTc3M3wwfDF8YWxsfDJ8fHx8fHwyfHwxNjU3ODkzNjU5&ixlib=rb-1.2.1&q=80&w=1200&h=1200',
      );
    });
    ...
    it('Uses cached image dimensions on cache hit', async function () {
      const underlyingFetch = sinon
        .stub()
        .callsFake(() => Promise.resolve({ width: 2000, height: 1000 }));
      const cacheStore = new InMemoryCache();
      // Pre-populate cache
      cacheStore.set('https://example.com/image.png', {
    ...
    it('Returns default dimensions when fetch fails', async function () {
      ...
      const underlyingFetch = sinon.stub().rejects(
        new ghosterrors.InternalServerError({
