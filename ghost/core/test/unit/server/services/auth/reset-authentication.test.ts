 ...
 */
function buildAuthDomain({
  apiKeysToRotate,
  usersToLock,
  currentKey,
}: {
  apiKeysToRotate: number;
  usersToLock: number;
  currentKey: { id: string; secret: string };
}) {
  const recorded = {
...
describe('resetAuthentication', function () {
  it('rotates keys, locks users, writes audit row with counts, returns counts', async function () {
    const env = buildAuthDomain({
      apiKeysToRotate: 4,
      usersToLock: 3,
      currentKey: { id: 'k', secret: 'old' },
    });
  ...
  it('asks the scheduler adapter to reschedule with the pre-rotation key', async function () {
    const env = buildAuthDomain({
      apiKeysToRotate: 1,
      usersToLock: 1,
      currentKey: { id: 'k', secret: 'pre-rotation' },
    });
    let observed: { id: string; secret: string } | undefined;

    await resetAuthentication({
      schedulerAdapter: {
        rescheduleAll: async (opts) => {
