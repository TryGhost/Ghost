...
describe('Update Check', function () {
  ...
  describe('Notifications', function () {
    ...
    it('should send an email for critical notification', async function () {
      ...
      assert.equal(
        emailSendStub.args[0][0].subject,
        'Action required: Critical alert from Ghost instance http://127.0.0.1:2369',
      );
  ...
  describe('Response shapes', function () {
    ...
    it('handles a bare-array response', async function () {
      nock('https://updates.ghost.org')
        .get('/')
        .query(true)
        .reply(200, JSON.stringify([releaseNotification, customNotification]), {
          'Content-Type': 'application/json',
        });
  ...
  describe('Error handling', function () {
    it('logs an error when error', function () {
      ...
      assert.equal(
        logging.error.args[0][0].err.context,
