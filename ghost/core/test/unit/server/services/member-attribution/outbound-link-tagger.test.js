...
describe('OutboundLinkTagger', function () {
  ...
  describe('addToUrl', function () {
    ...
    it('uses sluggified newsletter name for internal urls', async function () {
      ...
      assert.equal(
        updatedUrl.toString(),
        'https://example.com/?ref=used-newsletter-name-newsletter',
      );
    });
    ...
    it('does not add ref to blocked domains', async function () {
      ...
      const urlFour = new URL('https://example.substack.com/p/hello');
      const updatedUrlFour = await service.addToUrl(urlFour);

      assert.equal(updatedUrlFour.toString(), 'https://example.substack.com/p/hello');
    });
    ...
    it('preserves existing query param encoding when appending ref', async function () {
      ...
      const url = new URL(
        'https://adserver.example.com/jump?iu=/12345678/Ad_Unit_1&sz=300x250&c=[TIMESTAMP]',
      );
      const updatedUrl = await service.addToUrl(url);

      assert.equal(
        updatedUrl.toString(),
        'https://adserver.example.com/jump?iu=/12345678/Ad_Unit_1&sz=300x250&c=[TIMESTAMP]&ref=blog.com',
      );
    });
  ...
  describe('addToHtml', function () {
    it('adds refs to external links', async function () {
      ...
      const html = await service.addToHtml(
        '<a href="https://example.com/test-site">Hello world</a><a href="https://other.com/test/">Hello world</a>',
      );
      assert.equal(
        html,
        '<a href="https://example.com/test-site?ref=blog.com">Hello world</a><a href="https://other.com/test/?ref=blog.com">Hello world</a>',
      );
    });
    ...
    it('does not add refs to anchors', async function () {
      ...
      const html = await service.addToHtml(
        '<a href="#test">Hello world</a><a href="#">Hello world</a>',
      );
