...
describe('automation run cursors', function () {
  ...
  for (const [label, other] of [
    [
      'date range',
      { date_from: '2026-09-01', date_to: '2026-10-01', timezone: 'America/New_York' },
    ],
...
it('continues the same entry window, but rejects changed dates or timezone', () => {
  const dated = {
    ...scope,
  ...
  const cursor = encodeRunCursor(dated, position);
  assert.deepEqual(decodeRunCursor(cursor, dated), position);
  for (const change of [
    { date_from: '2026-09-02' },
    { date_to: '2026-09-30' },
    { timezone: 'UTC' },
  ]) {
    assert.throws(() => decodeRunCursor(cursor, { ...dated, ...change }), /does not match/);
  }
});
