...
describe('StripeAPI', function () {
  ...
  describe('createCheckoutSession', function () {
    ...
    it('Sends card as payment method if labs flag not enabled', async function () {
      ...
      assert.deepEqual(
        mockStripe.checkout.sessions.create.firstCall.firstArg.payment_method_types,
        ['card'],
      );
    });
    ...
    it('explicitly disables Managed Payments', async function () {
      ...
      assert.deepEqual(mockStripe.checkout.sessions.create.firstCall.firstArg.managed_payments, {
        enabled: false,
      });
    });
    ...
    it('sets valid trialDays', async function () {
      ...
      assert.equal(
        mockStripe.checkout.sessions.create.firstCall.firstArg.subscription_data.trial_from_plan,
        undefined,
      );
      assertExists(
        mockStripe.checkout.sessions.create.firstCall.firstArg.subscription_data.trial_period_days,
      );
      assert.equal(
        mockStripe.checkout.sessions.create.firstCall.firstArg.subscription_data.trial_period_days,
        12,
    ...
    it('passes customer ID successfully to Stripe', async function () {
      ...
      assert.equal(
        mockStripe.checkout.sessions.create.firstCall.firstArg.customer,
        'cust_mock_123456',
      );
    });
    ...
    it('passes email if no customer object provided', async function () {
      ...
      assert.equal(
        mockStripe.checkout.sessions.create.firstCall.firstArg.customer_email,
        'foo@example.com',
      );
    });
              ...
              data: [
                {
                  ...
                  subscriptions: {
                    data: [{ current_period_end: 1000 }, { current_period_end: 9000 }],
                  },
                },
                {
  ...
  describe('createCheckoutSession automatic tax flag', function () {
    ...
    it('createCheckoutSession adds customer_update if automatic tax flag is enabled and customer is not undefined', async function () {
      ...
      assert.deepEqual(mockStripe.checkout.sessions.create.firstCall.firstArg.customer_update, {
        address: 'auto',
        name: 'auto',
      });
    });
    ...
    it('createCheckoutSession adds tax_id_collection when automatic tax flag is enabled', async function () {
      ...
      assert.deepEqual(mockStripe.checkout.sessions.create.firstCall.firstArg.tax_id_collection, {
        enabled: true,
      });
    });
    ...
    it('createCheckoutSession does not add tax_id_collection when automatic tax flag is disabled', async function () {
      ...
      assert.equal(
        mockStripe.checkout.sessions.create.firstCall.firstArg.tax_id_collection,
        undefined,
      );
    });
  });
  ...
  describe('createGiftCheckoutSession', function () {
    ...
    it('creates a session with mode payment and inline price_data', async function () {
      ...
      assert.equal(
        args.line_items[0].price_data.product_data.name,
        'Gift subscription — Pro (1 year)',
      );
      assert.deepEqual(args.metadata, { ghost_gift_id: 'gift_123' });
      assert.deepEqual(mockStripe.checkout.sessions.create.firstCall.args[1], {
        idempotencyKey: 'gift_123',
      });
    });
    ...
    it('uses 1 month label for monthly cadence', async function () {
      ...
      assert.equal(
        args.line_items[0].price_data.product_data.name,
        'Gift subscription — Basic (1 month)',
      );
    });
    ...
    it('pluralises cadence label when duration is greater than 1', async function () {
      ...
      assert.equal(
        args.line_items[0].price_data.product_data.name,
        'Gift subscription — Pro (3 months)',
      );
    });
