...
describe('AdapterManager', function () {
  ...
  it('Loads registered adapters in the order defined by the paths', function () {
    const pathsToAdapters = ['first/path', 'second/path', 'third/path'];

    const loadAdapterFromPath = sinon.stub();
    loadAdapterFromPath.withArgs('first/path/mail/incomplete').returns(IncompleteMailAdapter);
    loadAdapterFromPath.withArgs('second/path/mail/custom').returns(CustomMailAdapter);
    loadAdapterFromPath.withArgs('third/path/mail/default').returns(DefaultMailAdapter);
    loadAdapterFromPath.withArgs('first/path/mail/broken').throwsException('SHIT_GOT_REAL');
  ...
  it('Reads adapter type from the adapter name divided with a colon (adapter:feature syntax)', function () {
    ...
    loadAdapterFromPath.withArgs('/path/mail/custom').returns(CustomMailAdapter);
    loadAdapterFromPath.withArgs('/path/mail/default').returns(DefaultMailAdapter);
  ...
  describe('init', function () {
    ...
    it('aggregates failures from multiple adapters into one error', function () {
      ...
      assert.throws(
        () => adapterManager.init(),
        (err: Error & { errorDetails?: unknown }) => {
...
`;
    ...
    it('falls back to directory resolution when the manifest has no name', function () {
      const pkgDir = writeAdapterPackage('unnamed-adapter', {
        exports: { '.': './dist/index.js' },
      });
    ...
    describe('resolveAdapterEntryPoint', function () {
      it('returns non-absolute paths untouched', function () {
        ...
        assert.equal(
          resolveAdapterEntryPoint('some-node-module-adapter'),
          'some-node-module-adapter',
        );
        assert.equal(
          resolveAdapterEntryPoint('relative/path/mail/custom'),
          'relative/path/mail/custom',
        );
      });
