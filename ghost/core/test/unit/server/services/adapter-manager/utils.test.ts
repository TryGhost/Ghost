...
describe('Adapter Manager: utils', function () {
  describe('normalizeAdapterConfig', function () {
    ...
    it('keeps an explicitly configured S3RouteSettingsStore default settings path', () => {
      ...
      const normalizedConfig = normalizeAdapterConfig(conf);
      assert.equal(
        normalizedConfig['route-settings'].S3RouteSettingsStore.defaultSettingsBasePath,
        '/custom/defaults',
      );
    });
    ...
    it('re-resolves route-settings paths when the content path changes', () => {
      ...
      assert.notEqual(
        after['route-settings'].FileStore.basePath,
        before['route-settings'].FileStore.basePath,
      );
    });
  });
  ...
  describe('resolveAdapterOptions', function () {
    ...
    it('combines configurations from shared adapter and feature adapter instances', function () {
      ...
      const { adapterClassName: secondadapterClassName, adapterConfig: secondAdapterConfig } =
        resolveAdapterOptions(secondaryadapterClassName, adapterServiceConfig);
