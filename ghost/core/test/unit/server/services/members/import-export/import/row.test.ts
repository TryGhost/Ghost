...
describe('member import row schema', function () {
  it('coerces a present subscribed cell leniently: true unless it is literally false', function () {
    ...
    assert.equal(
      memberImportRowSchema.parse({ subscribed: '' }).subscribed,
      true,
      'an empty cell in a present column coerces to true',
    );
  });
  ...
  it('coerces a present complimentary_plan cell: false unless it is literally true', function () {
    assert.equal(
      memberImportRowSchema.parse({ complimentary_plan: 'true' }).complimentary_plan,
      true,
    );
    assert.equal(
      memberImportRowSchema.parse({ complimentary_plan: 'false' }).complimentary_plan,
      false,
    );
    assert.equal(
      memberImportRowSchema.parse({ complimentary_plan: 'yes' }).complimentary_plan,
      false,
    );
    assert.equal(
      memberImportRowSchema.parse({ complimentary_plan: '' }).complimentary_plan,
      false,
      'an empty cell in a present column coerces to false',
    );
  });
  ...
  it('splits the labels cell into label objects', function () {
    assert.deepEqual(memberImportRowSchema.parse({ labels: 'vip,premium' }).labels, [
      { name: 'vip' },
      { name: 'premium' },
    ]);
    assert.deepEqual(memberImportRowSchema.parse({ labels: '' }).labels, []);
    assert.deepEqual(
      memberImportRowSchema.parse({}).labels,
      [],
      'omitted labels is an empty array',
    );
  });
