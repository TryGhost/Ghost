...
const MembersCSVImporterStripeUtils = require('../../../../../../../core/server/services/members/import-export/import/stripe-utils');
...
describe('MembersCSVImporterStripeUtils', function () {
   ...
   * @returns {Object}
   */
  const getStripeApiServiceStub = ({ configured = true, resolveCustomer = true } = {}) => {
  ...
  const getProductRepositoryStub = ({
    ...
    productRepositoryStub.get
      .withArgs({ id: PRODUCT_ID }, { ...OPTIONS, withRelated: ['stripePrices', 'stripeProducts'] })
  ...
  describe('forceSubscriptionToProduct', function () {
    ...
    it('rejects when the Stripe customer has no existing subscription', async function () {
      ...
      await assert.rejects(
        ...
        ),
        {
          name: 'DataImportError',
          message:
            'Cannot update subscription when customer does not have an existing subscription',
        },
      );
    });
    ...
    it("creates a new price on the Stripe product matching the Stripe customer's existing subscription and updates the subscription", async function () {
      ...
      stripeAPIServiceStub.createPrice
        .withArgs({
    ...
    it('archives the newly-created price if updating the subscription fails (#22115)', async function () {
      ...
      assert.equal(
        stripeAPIServiceStub.updatePrice.calledWithExactly(NEW_STRIPE_PRICE_ID, { active: false }),
        true,
      );
    });
  ...
  describe('archivePrice', function () {
    it('archives a Stripe price', async function () {
      ...
      sinon.assert.calledOnce(stripeAPIServiceStub.updatePrice);
      assert.equal(
        stripeAPIServiceStub.updatePrice.calledWithExactly(stripePriceId, { active: false }),
        true,
      );
    });
  });
