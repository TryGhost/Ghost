...
describe('MemberBreadService', function () {
  ...
  describe('add', function () {
    ...
    it('passes context to linkStripeCustomer when stripe_customer_id is provided', async function () {
      ...
      assert.equal(
        linkStripeCustomerStub.calledOnce,
        true,
        'linkStripeCustomer should be called once',
      );
      ...
      assert.ok(
        linkStripeCustomerOptions.context,
        'context should be passed to linkStripeCustomer',
      );
    ...
    it('passes context to linkStripeCustomer when transacting and stripe_customer_id are both provided', async function () {
      ...
      assert.equal(
        linkStripeCustomerOptions.transacting,
        mockTransaction,
        'transacting should be passed',
      );
  ...
  describe('edit', function () {
    ...
    it('does not create a complimentary subscription when editing an already-comped member (#25735)', async function () {
      const { service, setComplimentarySubscription } = createService({
  ...
  describe('read', function () {
    ...
    beforeEach(function () {
      ...
      memberRepositoryStub.get
        .withArgs({ id: MEMBER_ID }, { withRelated: DEFAULT_RELATIONS })
    ...
    it('does not eager-load the embedded email when email_recipients is included', async function () {
      ...
      assert.ok(
        passedWithRelated.includes('email_recipients'),
        'email_recipients itself should still be eager-loaded',
      );
      assert.ok(
        !passedWithRelated.includes('email_recipients.email'),
        'email_recipients.email must not be eager-loaded',
      );
      assert.ok(
        !passedWithRelated.some(
          (item) => typeof item === 'object' && item !== null && 'email_recipients.email' in item,
        ),
        'email_recipients.email must not be eager-loaded via object form either',
      );
    });
    ...
    it('returns a member with subscriptions that only have a price', async function () {
      ...
      assert.deepEqual(member.subscriptions, [subscriptionsJSON[0]]);
    });
    ...
    it('returns a member with a gift subscription', async function () {
      ...
      memberModelStub.related.withArgs('stripeSubscriptions').returns([]);
      ...
      const giftServiceStub = {
        service: {
          getMemberPresentations: sinon
            .stub()
            .resolves(new Map([[MEMBER_ID, { cadence: 'month', currency: 'eur', amount: 1500 }]])),
        },
      };
      ...
      sinon.assert.calledOnceWithExactly(giftServiceStub.service.getMemberPresentations, [
        MEMBER_ID,
      ]);
    ...
    it('returns a member with suppression data', async function () {
      emailSuppressionListStub.getSuppressionData.withArgs(memberModelJSON.email).resolves({
        suppressed: true,
        info: 'bounce',
      });
    ...
    it('returns a member with an unsubscribe url', async function () {
      ...
      assert.equal(
        ...
        `https://example.com/unsubscribe/?uuid=${MEMBER_UUID}&key=456`,
      );
    });
    ...
    it('returns member with next_payment amount for subscriptions without discount', async function () {
      ...
      assert.equal(member.subscriptions.length, 1);
      assert.ok(
        member.subscriptions[0].next_payment !== undefined,
        'next_payment should be defined',
      );
    ...
    it('returns member with next_payment amount for subscriptions with discount', async function () {
      ...
      const offersAPIStub = {
        ...
        getRedeemedOfferIdsForSubscriptions: sinon
          .stub()
          .resolves([{ subscription_id: '1', offer_id: offerId }]),
      };
