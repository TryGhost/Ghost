...
describe('hasActiveOffer', function () {
    ...
    function createOffer(overrides: Partial<OfferDTO> = {}): Pick<OfferDTO, 'duration' | 'redemption_type' | 'duration_in_months'> {
        return {
            duration: 'once',
    ...
    it('returns false for retention offers without discount dates (no legacy support)', async function () {
        ...
        const offersAPI = createOffersAPI(createOffer({duration: 'repeating', duration_in_months: 3, redemption_type: 'retention'} as const));
