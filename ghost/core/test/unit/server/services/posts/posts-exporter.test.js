...
describe('PostsExporter', function () {
  ...
  describe('export', function () {
    ...
    it('Can export posts', async function () {
      const posts = await Array.fromAsync(await exporter.export({}));
    ...
    it('Streams exports in stable pages when limit is greater than one batch', async function () {
      ...
      assert.deepEqual(
        exportedPosts.map((exportedPost) => exportedPost.id),
        posts.slice(0, 75).map((data) => data.id),
      );
      assert.deepEqual(
        findPageCalls.map((call) => call.limit),
        [50, 50],
      );
      assert.deepEqual(
        findPageCalls.map((call) => call.page),
        [1, 2],
      );
    ...
    it('Keeps custom ordered stream exports stable across page boundaries', async function () {
      ...
      models.Post = {
        findPage: async (options) => {
          ...
          const hasStableOrder = options.order
            .split(',')
            .some((orderPart) => orderPart.trim().toLowerCase().startsWith('id '));
          const orderedPosts =
            options.page === 2 && !hasStableOrder
              ? [...posts.slice(0, 49), posts[50], posts[49], ...posts.slice(51)]
              : posts;
          const start = (options.page - 1) * options.limit;
      ...
      const exportedPosts = await Array.fromAsync(
    ...
    it('Preserves the default export limit when no limit is supplied', async function () {
      ...
      const exportedPosts = await Array.fromAsync(await exporter.export({}));

      assert.equal(exportedPosts.length, 15);
      assert.deepEqual(
        exportedPosts.map((exportedPost) => exportedPost.id),
        posts.slice(0, 15).map((data) => data.id),
      );
    });
    ...
    it('Falls back to the default export limit when an invalid limit is supplied', async function () {
      ...
      const exportedPosts = await Array.fromAsync(await exporter.export({ limit: 'invalid' }));
  ...
  describe('mapPostStatus', function () {
    ...
    it('Returns draft', function () {
      assert.equal(exporter.mapPostStatus('draft', false), 'draft');
    });

    it('Returns scheduled', function () {
      assert.equal(exporter.mapPostStatus('scheduled', false), 'scheduled');
    });

    it('Returns emailed only', function () {
      assert.equal(exporter.mapPostStatus('sent', false), 'emailed only');
    });

    it('Returns published and emailed', function () {
      assert.equal(exporter.mapPostStatus('published', true), 'published and emailed');
    });

    it('Returns published only', function () {
      assert.equal(exporter.mapPostStatus('published', false), 'published only');
    });

    it('Returns unsupported', function () {
      assert.equal(exporter.mapPostStatus('unsupported', false), 'unsupported');
    });
  });
  ...
  describe('postAccessToString', function () {
    ...
    it('Returns members', function () {
      ...
      assert.equal(access, 'Members-only');
    });
    ...
    it('Returns paid', function () {
      ...
      assert.equal(access, 'Paid members-only');
    });
    ...
    it('Returns empty tiers', function () {
      ...
      assert.equal(access, 'Specific tiers: none');
    });
    ...
    it('Returns multiple tiers', function () {
      ...
      assert.equal(access, 'Specific tiers: Silver, Gold');
    });
    ...
    it('Returns unsupported', function () {
      ...
      assert.equal(access, 'unsupported');
    });
  });
  ...
  describe('humanReadableEmailRecipientFilter', function () {
    ...
    it('Returns all', function () {
      assert.equal(exporter.humanReadableEmailRecipientFilter('all'), 'All subscribers');
    });

    it('Returns empty', function () {
      assert.equal(exporter.humanReadableEmailRecipientFilter(''), '');
    });
    ...
    it('Returns status', function () {
      assert.equal(exporter.humanReadableEmailRecipientFilter('status:free'), 'Free subscribers');

      assert.equal(exporter.humanReadableEmailRecipientFilter('status:-free'), 'Paid subscribers');

      assert.equal(exporter.humanReadableEmailRecipientFilter('status:paid'), 'Paid subscribers');
      ...
      assert.equal(exporter.humanReadableEmailRecipientFilter('status:-paid'), 'Free subscribers');
    });
