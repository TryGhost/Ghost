...
const {
  EmailAddressService,
} = require('../../../../../core/server/services/email-address/email-address-service');
...
describe('NewslettersService', function () {
  ...
  describe('read', function () {
    ...
    it('throws a not found error if not found', async function () {
      ...
      await assert.rejects(newsletterService.read({ slug: 'my-slug' }), {
  ...
  describe('add', function () {
    ...
    beforeEach(function () {
      ...
      addStub = sinon
        .stub(Newsletter, 'add')
        .returns({ get: getStub, id: 'test', subscribeMembersById: subscribeStub });
      fetchMembersStub = sinon.stub(Member, 'fetchAllSubscribed').returns([]);
      getNextAvailableSortOrderStub = sinon
        .stub(Newsletter, 'getNextAvailableSortOrder')
        .returns(1);
      findOneStub = sinon
        .stub(Newsletter, 'findOne')
        .returns({ get: getStub, id: 'test', subscribeMembersById: subscribeStub });
    });
    ...
    it('will try to find existing members when opt_in_existing is provided', async function () {
      ...
      sinon.assert.calledOnceWithExactly(
        findOneStub,
        { id: 'test' },
        { opt_in_existing: true, transacting: options.transacting, require: true },
      );
    });
