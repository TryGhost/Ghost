...
describe('VerificationWebhookService', function () {
  ...
  it('returns false when webhookType is missing', async function () {
    ...
    assert.equal(
      warnMessage,
      'Verification webhook is not configured because webhookType is missing.',
    );
  });
  ...
  it('sends a POST request with the signed webhook payload', async function () {
    ...
    assert.equal(
      infoMessage,
      'Triggering verification webhook to "https://test-webhook-receiver.com"',
    );
  ...
  it('logs a sanitized webhook URL when delivery fails', async function () {
    ...
    await assert.rejects(
      service.sendVerificationWebhook({
        amountTriggered: 1001,
        threshold: 1000,
        method: 'import',
      }),
    ...
    assert.equal(
      errorMessage,
      'Failed to send verification webhook to "https://test-webhook-receiver.com": Webhook failed',
    );
  });
});
