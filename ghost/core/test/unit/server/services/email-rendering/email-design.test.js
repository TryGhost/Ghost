...
describe('getEmailDesign', function () {
  ...
  describe('buttonColor', function () {
    ...
    it('returns the accent color when input is invalid', function () {
      for (const buttonColor of INVALID_HEX_COLORS.filter(
        (color) => color !== 'accent' && color !== null,
      )) {
  ...
  describe('headerBackgroundColor', function () {
    ...
    it('returns null when input is invalid', function () {
      for (const headerBackgroundColor of INVALID_HEX_COLORS.filter(
        (color) => color !== 'accent',
      )) {
        const result = getEmailDesign({ ...baseSettings, headerBackgroundColor });
  ...
  describe('linkColor', function () {
    ...
    it('returns the accent color when input is invalid', function () {
      for (const linkColor of INVALID_HEX_COLORS.filter(
        (color) => color !== 'accent' && color !== null,
      )) {
  ...
  describe('postTitleColor', function () {
    ...
    it('returns the background contrast color when input is invalid and the header background color is not set', function () {
      assertColorsEqual(
        getEmailDesign({ ...baseSettings, backgroundColor: '#333', postTitleColor: 'invalid' })
          .postTitleColor,
        '#ffffff',
      );
      assertColorsEqual(
        getEmailDesign({ ...baseSettings, backgroundColor: '#ddd', postTitleColor: 'invalid' })
          .postTitleColor,
        '#000000',
      );
    });
  ...
  describe('titleWeight', function () {
    it('maps title font weight settings to numeric CSS values', function () {
      assert.equal(
        getEmailDesign({ ...baseSettings, titleFontWeight: 'normal' }).titleWeight,
        '400',
      );
      assert.equal(
        getEmailDesign({ ...baseSettings, titleFontWeight: 'medium' }).titleWeight,
        '500',
      );
      assert.equal(
        getEmailDesign({ ...baseSettings, titleFontWeight: 'semibold' }).titleWeight,
        '600',
      );
