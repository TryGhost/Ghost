...
describe('UnsafeData', function () {
  ...
  describe('nullable', function () {
    it('Returns data if not null', function () {
      const data = new UnsafeData({ foo: 'bar' });
      assert.equal(data.nullable, data);
    });

    it('Returns proxy if null', function () {
      const data = new UnsafeData(null);
      assert.notEqual(data.nullable, data);

      assert.equal(data.nullable.string, null);
      assert.equal(data.nullable.boolean, null);
      assert.equal(data.nullable.number, null);
      assert.equal(data.nullable.integer, null);
      assert.equal(data.nullable.url, null);
      assert.equal(data.nullable.enum(['foo', 'bar']), null);
      const n = data.nullable;
      assert.equal(n.key('test'), n);
      assert.equal(data.nullable.key('test').string, null);
      assert.equal(data.nullable.optionalKey('test').string, null);
      assert.equal(data.nullable.array, null);
      assert.equal(data.nullable.index(0).string, null);
    });
  });
  ...
  describe('boolean', function () {
    ...
    it('Returns if false', function () {
      ...
      assert.equal(data.boolean, false);
    });
  ...
  describe('number', function () {
    ...
    it('Converts from float string', function () {
      ...
      assert.equal(data.number, 15.33);
    });
  ...
  describe('enum', function () {
    it('Returns if a valid value', function () {
      ...
      assert.equal(data.enum(['foo', 'bar']), 'foo');
    });

    it('Works for numbers too', function () {
      const data = new UnsafeData(5);
      assert.equal(data.enum([5, 8]), 5);
    });
  ...
  describe('array', function () {
    it('Returns if an array', function () {
      const data = new UnsafeData(['foo', 'bar']);
      assert.equal(data.array[0].string, 'foo');
      assert.equal(data.array[1].string, 'bar');
    });

    it('Extends context', function () {
      const data = new UnsafeData(['foo', 'bar'], { field: ['baz'] });
      assert.deepEqual(data.array[0], new UnsafeData('foo', { field: ['baz', '0'] }));
      assert.deepEqual(data.array[1], new UnsafeData('bar', { field: ['baz', '1'] }));
    });
  ...
  describe('index', function () {
    it('Returns if an array', function () {
      const data = new UnsafeData(['foo', 'bar']);
      assert.equal(data.index(0).string, 'foo');
    });

    it('Extends context', function () {
      const data = new UnsafeData(['foo', 'bar'], { field: ['baz'] });
      assert.deepEqual(data.index(0), new UnsafeData('foo', { field: ['baz', '0'] }));
      assert.deepEqual(data.index(1), new UnsafeData('bar', { field: ['baz', '1'] }));
    });
