...
import {
  ...
  ModelClass,
  ModelInstance,
} from '../../../../../../core/server/services/recommendations/service/bookshelf-repository';
...
class Model implements ModelClass<string> {
  ...
  findOne(
    data: { id: string },
    options?: { require?: boolean | undefined } | undefined,
  ): Promise<ModelInstance<string> | null> {
  ...
  fetchAll(): Promise<ModelInstance<string>[]> {
    ...
    return Promise.resolve(
      sorted.slice(this.offset ?? 0, (this.offset ?? 0) + (this.limit ?? sorted.length)),
    );
  }
