...
const {
  matchPermalink,
  toLookupParams,
} = require('../../../../../core/server/services/url/permalink-matcher');
...
describe('permalink-matcher', function () {
  ...
  describe('multi-segment templates', function () {
    it('captures multiple placeholders', function () {
      assert.deepEqual(captured('/:primary_tag/:slug/', '/podcast/hello/'), {
        primary_tag: 'podcast',
        slug: 'hello',
      });
    });
    ...
    it('captures date-based placeholders', function () {
      assert.deepEqual(captured('/:year/:month/:slug/', '/2026/04/hello/'), {
        year: '2026',
        month: '04',
        slug: 'hello',
      });
  ...
  describe('multi-token segments (#28076)', function () {
    it('captures hyphen-separated params within one segment', function () {
      assert.deepEqual(captured('/:year-:month-:day-:slug/', '/2026-04-15-hello/'), {
        year: '2026',
        month: '04',
        day: '15',
        slug: 'hello',
    ...
    it('does not let an earlier param consume a later one across hyphens', function () {
      ...
      assert.deepEqual(captured('/:slug-:primary_tag/', '/hello-podcast/'), {
        slug: 'hello',
        primary_tag: 'podcast',
      });
    });
  ...
  describe('value-format pre-filter', function () {
    ...
    it('matches when an :id segment is a valid ObjectId', function () {
      assert.deepEqual(captured('/:id/', '/0123456789abcdef01234567/'), {
  ...
  describe('toLookupParams', function () {
    it('narrows to slug when present', function () {
      assert.deepEqual(toLookupParams({ slug: 'hello', primary_tag: 'podcast' }), {
        slug: 'hello',
      });
    });

    it('narrows to id when present', function () {
      assert.deepEqual(toLookupParams({ id: '0123456789abcdef01234567' }), {
        id: '0123456789abcdef01234567',
      });
    });

    it('prefers id over slug', function () {
      assert.deepEqual(toLookupParams({ id: '0123456789abcdef01234567', slug: 'hello' }), {
        id: '0123456789abcdef01234567',
      });
    });
  });
