...
} = require('../../../../../core/server/services/url/routable-resources');
...
describe('Unit: services/url/routable-resources', function () {
  ...
  it('applies the routing gates for each type', async function () {
    ...
    sinon.assert.calledWith(
      fetchAll,
      sinon.match({ modelName: 'Post', filter: 'status:published+type:post' }),
    );
    sinon.assert.calledWith(
      fetchAll,
      sinon.match({ modelName: 'Post', filter: 'status:published+type:page' }),
    );
