...
describe('Unit: services/url/Urls', function () {
    ...
    it('fn: add', function () {
        ...
        assert(eventsToRemember['url.added']);
        ...
        assert(eventsToRemember['url.added'].resource);
        assert(eventsToRemember['url.added'].resource.data);
    ...
    it('fn: getByResourceId', function () {
        ...
        assert(urls.getByResourceId('object-id-2').generatorId);
        assert.equal(urls.getByResourceId('object-id-2').generatorId, 1);
    });

    it('fn: getByGeneratorId', function () {
        assert.deepEqual(urls.getByGeneratorId(2).length, 2);
    });

    it('fn: getByUrl', function () {
        assert.deepEqual(urls.getByUrl('/something/').length, 1);
    });
