...
const mockWebmentionMetadata = {
  async fetch() {
    return {
      ...
      excerpt:
        'How many times have you woken up and almost cancelled your church plans? Well this breakfast is about to change everything, a hearty, faith restoring egg dish that will get your tastebuds in a twist.',
      author: 'Dr Egg Man',
...
describe('MentionsAPI', function () {
  ...
  it('Can list mentions in descending order', async function () {
    ...
    assert(
      page.data[0].id === mentionTwo.id,
      'First mention should be the second one in descending order',
    );
    assert(
      page.data[1].id === mentionOne.id,
      'Second mention should be the first one in descending order',
    );
  });
  ...
  it('Can list mentions in ascending order', async function () {
    ...
    assert(page.meta.pagination.total === 2);
    assert(
      page.data[0].id === mentionOne.id,
      'First mention should be the first one in ascending order',
    );
    assert(
      page.data[1].id === mentionTwo.id,
      'Second mention should be the second one in ascending order',
    );
  });
  ...
  it('Can update recommendations', async function () {
    ...
    assert.equal(
      ...
      1,
      'Mention should be deleted after 3rd consecutive hard failure',
    );
  });
    ...
    const api = new MentionsAPI({
      ...
      webmentionMetadata: {
        fetch: sinon
          .stub()
          .onFirstCall()
          .resolves(mockWebmentionMetadata.fetch())
          .onSecondCall()
          .rejects(transientErr),
      },
    });
  ...
  it('Successful fetch after failures resets the failure count', async function () {
    ...
    assert.equal(
      page.meta.pagination.total,
      1,
      'Mention should survive because success reset the failure count',
    );
  });
    ...
    const api = new MentionsAPI({
      ...
      webmentionMetadata: {
        fetch: sinon
          ...
          .resolves(mockWebmentionMetadata.fetch())
          .onSecondCall()
          .resolves({ ...(await mockWebmentionMetadata.fetch()), body: 'test' })
          .onThirdCall()
          .resolves(mockWebmentionMetadata.fetch()),
      },
    });
