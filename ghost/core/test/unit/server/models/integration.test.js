...
describe('Unit: models/integration', function () {
  ...
  describe('permittedOptions', function () {
    ...
    beforeEach(function () {
      ...
      sinon.stub(Base.Model, 'permittedOptions').returns(basePermittedOptionsReturnVal);
    });
  ...
  describe('getApiKeyBySlug', function () {
    ...
    it('returns the matching API key as a flat DTO', async function () {
      ...
      assert.equal(
        queries[0].sql,
        'select `api_keys`.`id`, `api_keys`.`secret` from `api_keys` inner join `integrations` on `api_keys`.`integration_id` = `integrations`.`id` where `integrations`.`slug` = ? and `api_keys`.`type` = ? limit ?',
      );
