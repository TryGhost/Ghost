...
describe('Unit: models/member', function () {
  ...
  describe('toJSON', function () {
    ...
    it('avatar_image: generates gravatar url', function () {
      ...
      assert.equal(
        json.avatar_image,
        `https://www.gravatar.com/avatar/55502f40dc8b7c769880b10874abc9d0?s=250&r=g&d=blank`,
      );
    });
  ...
  describe('onFetchingCollection: deep offset pagination', function () {
    ...
    it('rewrites deep-offset browse queries to a deferred join', function () {
      ...
      assert.match(
        sql,
        /inner join \(select `members`\.`id` as `deep_page_id` from `members` where `members`\.`status` = 'free' order by created_at DESC, id DESC limit 100 offset 10300\) as `deep_page`/,
      );
    ...
    it('rewrites deep-offset queries whose filters contain subquery joins', function () {
      ...
      assert.ok(
        qb.toString().includes('deep_page'),
        'filtered deep pages should still be rewritten',
      );
    });
    ...
    it('leaves non-browse query shapes untouched', function () {
      const unsafe = [
        db('members')
          .innerJoin('members_labels', 'members.id', 'members_labels.member_id')
          .limit(100)
          .offset(10300),
