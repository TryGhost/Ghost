...
} = require('../../../../core/server/models/welcome-email-automated-email');
...
describe('Unit: models/welcome-email-automated-email', function () {
  ...
  describe('parse', function () {
    it('transforms __GHOST_URL__ to absolute URL in lexical field', function () {
      ...
      const result = model.parse({
        id: '123',
        lexical:
          '{"root":{"children":[{"type":"paragraph","children":[{"type":"link","url":"__GHOST_URL__/test"}]}]}}',
      });
  ...
  describe('onCreating', function () {
    it('assigns the default email design setting when not provided', async function () {
      const model = WelcomeEmailAutomatedEmail.forge();
      const baseOnCreating = sinon
        .stub(Object.getPrototypeOf(WelcomeEmailAutomatedEmail.prototype), 'onCreating')
        .resolves();
      const findOne = sinon
        .stub(EmailDesignSetting, 'findOne')
        .resolves(EmailDesignSetting.forge({ id: 'default-setting-id' }));
    ...
    it('keeps the provided email design setting id', async function () {
      const model = WelcomeEmailAutomatedEmail.forge({
        email_design_setting_id: 'custom-setting-id',
      });
      const baseOnCreating = sinon
        .stub(Object.getPrototypeOf(WelcomeEmailAutomatedEmail.prototype), 'onCreating')
        .resolves();
    ...
    it('throws when the default email design setting is missing', async function () {
      ...
      await assert.rejects(() => model.onCreating(model, {}, {}), {
