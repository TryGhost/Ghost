    ...
    describe('filter', function () {
        it('generates correct query for - filter: tags: [photo, video] + id: -{id},limit of: 3, with related: tags', function () {
            ...
            return models.Post.findPage({
                ...
                const ast1 = parser.astify(queries[0].sql);
                const expectedAst1 = parser.astify('select count(distinct posts.id) as aggregate from `posts` where ((`posts`.`id` != ? and `posts`.`id` in (select `posts_tags`.`post_id` from `posts_tags` inner join `tags` on `tags`.`id` = `posts_tags`.`tag_id` where `tags`.`slug` in (?, ?))) and (`posts`.`type` = ? and `posts`.`status` = ?))');
                should(ast1).deep.equal(expectedAst1);
        ...
        it('generates correct query for - filter: published_at:>\'2015-07-20\', limit of: 5, with related: tags', function () {
            ...
            return models.Post.findPage({
                ...
                const parser = new Parser();
                const ast1 = parser.astify(queries[0].sql);
                const expectedAst1 = parser.astify('select count(distinct posts.id) as aggregate from `posts` where (`posts`.`published_at` > ? and (`posts`.`type` = ? and `posts`.`status` = ?))');
                should(ast1).deep.equal(expectedAst1);
                queries[0].bindings.should.eql([
                    '2015-07-20',
                    'post',
            ...
            it('generates correct query for - filter: primary_tag:photo, with related: tags', function () {
                ...
                return models.Post.findPage({
                    ...
                    queries.length.should.eql(2);
                        const parser = new Parser();
                        const ast1 = parser.astify(queries[0].sql);
                        const expectedAst1 = parser.astify('select count(distinct posts.id) as aggregate from `posts` where ((`posts`.`id` in (select `posts_tags`.`post_id` from `posts_tags` inner join `tags` on `tags`.`id` = `posts_tags`.`tag_id` and `posts_tags`.`sort_order` = 0 where `tags`.`slug` = ? and `tags`.`visibility` = ?)) and (`posts`.`type` = ? and `posts`.`status` = ?))');
                        should(ast1).deep.equal(expectedAst1);
                    queries[0].bindings.should.eql([
                        'photo',
                        'public',
