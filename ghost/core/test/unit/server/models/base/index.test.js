...
describe('Models: base', function () {
  ...
  describe('generateSlug', function () {
    ...
    it('default', function () {
      ...
      return Base.Model.generateSlug(Model, 'My-Slug', options).then((slug) => {
    ...
    it('slug exists but it does not exist for the id', function () {
      ...
      return Base.Model.generateSlug(Model, 'My-Slug', { modelId: 'incorrect-model-id' }).then(
        (slug) => {
    ...
    it('slug exists but it exists for the id', function () {
      ...
      return Base.Model.generateSlug(Model, 'My-Slug', { modelId: 'correct-model-id' }).then(
        (slug) => {
          assert.equal(slug, 'my-slug');
        },
      );
    ...
    it('too long', function () {
      ...
      return Base.Model.generateSlug(Model, slug, options).then((generatedSlug) => {
    ...
    it('contains invisible unicode', function () {
      ...
      return Base.Model.generateSlug(Model, 'abc\u0008', options).then((slug) => {
  ...
  describe('sanitizeData', function () {
    it('date is invalid', function () {
      ...
      try {
        Base.Model.sanitizeData.bind({ prototype: { tableName: 'posts' } })(data);
      } catch (err) {
