...
import imageModule from '../../../../../core/server/web/gift-preview/image';

const giftPreviewPath = path.dirname(
  require.resolve('../../../../../core/server/web/gift-preview/image'),
);
  ...
  describe('generateGiftPreviewImage', function () {
    ...
    it('returns the same cached result for concurrent calls with same params', async function () {
      ...
      const results = await Promise.all(
        Array.from({ length: 10 }, () => imageModule.generateGiftPreviewImage(params)),
      );
