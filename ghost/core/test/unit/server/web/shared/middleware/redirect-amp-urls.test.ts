...
import { redirectAmpUrls } from '../../../../../../core/server/web/shared/middleware/redirect-amp-urls';
...
describe('Middleware: redirectAmpUrls', function () {
  ...
  describe('AMP URLs', function () {
    const cases = [
      ...
      [
        '/ghost/blog/2023/post-title/amp/?utm_source=test',
        '/ghost/blog/2023/post-title/?utm_source=test',
      ],
    ...
    it.each(cases)('redirects %s to %s', async (path, location) => {
      await request(createApp()).get(path).expect(301).expect('Location', location);
    });
  });
});
