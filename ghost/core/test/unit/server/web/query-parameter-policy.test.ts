...
describe('Query parameter policy', function () {
    ...
    it.each([
        ...
        ['an unsupported schema version', {...validPolicy(), schemaVersion: 2}, /Unsupported schema version/],
        ...
        ['an empty parameter name', {...validPolicy(), public: [{name: '', reason: 'Empty'}]}, /Parameter names must be nonempty/],
        ['an invalid parameter name', {...validPolicy(), public: [{name: 'not valid&', reason: 'Invalid'}]}, /must not contain whitespace or query-string separators/],
        ['an empty reason', {...validPolicy(), public: [{name: 'gift', reason: '   '}]}, /Parameter reasons must be nonempty/],
        ['a duplicate public name', {...validPolicy(), public: Array(2).fill({name: 'gift', reason: 'Gift link'})}, /Duplicate parameter name.*gift/],
        ['a duplicate Content API name', {...validPolicy(), contentApi: Array(2).fill({name: 'filter', reason: 'Filtering'})}, /Duplicate parameter name.*filter/]
    ...
    it('exports the validated canonical policy deterministically', async function () {
        ...
        try {
            ...
            assert.deepEqual(JSON.parse(await readFile(outputPath, 'utf8')), JSON.parse(await readFile(manifestPath, 'utf8')));
        } finally {
