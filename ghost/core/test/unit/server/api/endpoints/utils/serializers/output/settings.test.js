  ...
  describe('icon rewrite (Admin API)', function () {
    ...
    it('rewrites CDN icon URL with /size/w256h256/', function () {
      const models = [
        {
          key: 'icon',
          value: 'https://storage.ghost.is/c/6f/a3/site/content/images/2026/02/icon.png',
          group: 'site',
        },
      ];
      const frame = {
      ...
      assert.equal(
        icon.value,
        'https://storage.ghost.is/c/6f/a3/site/content/images/size/w256h256/2026/02/icon.png',
      );
    });

    it('does not modify settings without icon', function () {
      const models = [{ key: 'title', value: 'Test Blog', group: 'site' }];
      const frame = {
        apiType: 'admin',
        options: {},
        response: null,
    ...
    it('does not modify null icon value', function () {
      const models = [{ key: 'icon', value: null, group: 'site' }];
      const frame = {
        apiType: 'admin',
        options: {},
        response: null,
