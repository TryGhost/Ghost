...
} from '../../../../../core/server/adapters/storage/S3Storage';
...
describe('S3Storage', function () {
    ...
    it('throws when multipart chunk size is below the 5 MiB minimum', function () {
      assert.throws(
        () =>
          S3Storage.validate({
            ...baseOptions,
            multipartChunkSizeBytes: MIN_MULTIPART_CHUNK_SIZE - 1,
          }),
        /at least 5 MiB/,
      );
    });
    ...
    it('throws when multipart byte counts are not integers', function () {
      ...
      assert.throws(
        () =>
          S3Storage.validate({
            ...baseOptions,
            multipartChunkSizeBytes: MIN_MULTIPART_CHUNK_SIZE + 0.5,
          }),
  ...
  it('uploads files with prefix and returns cdn url', async function () {
    ...
    assert.equal(
      url,
      'https://cdn.example.com/configurable/prefix/content/files/2024/06/test-image.jpg',
    );
  ...
  it('saves raw buffers relative to storagePath', async function () {
    ...
    assert.equal(
      url,
      'https://cdn.example.com/configurable/prefix/content/files/thumbnails/raw-image.jpg',
    );
    sinon.assert.calledOnce(sendStub);
    const command = sendStub.firstCall.args[0] as PutObjectCommand;
  ...
  it('exists throws when fileName is empty', async function () {
    ...
    await assert.rejects(storage.exists('', '2024/06'), /requires a non-empty fileName/);
  });

  it('delete throws when fileName is empty', async function () {
    const { storage } = createStorage();

    await assert.rejects(storage.delete('', '2024/06'), /requires a non-empty fileName/);
  });
  ...
  it('urlToPath throws when relative segments resolve outside storagePath', function () {
    ...
    assert.throws(() => {
      storage.urlToPath(
        'https://cdn.example.com/configurable/prefix/content/files/2024/06/../../../../../../etc/passwd',
      );
    }, /not a valid URL/);
  });
  ...
  it('read() throws when no path is given', async function () {
    ...
    await assert.rejects(storage.read(), /requires a non-empty path/);
  });
  ...
  describe('absolute and pre-prefixed paths are normalised before building keys', function () {
    ...
    it('strips an absolute filesystem prefix that contains the storagePath segment', function () {
      ...
      const fromAbsolute = (storage as any).buildKey(
        '/var/lib/ghost/content/files/2024/06/image.jpg',
      );
    ...
    it('exists() probes the same key for absolute, pre-prefixed and relative inputs', async function () {
      ...
      const keys = sendStub
        .getCalls()
        .map((call) => (call.args[0] as InstanceType<typeof HeadObjectCmd>).input.Key);
      assert.equal(keys.length, 3);
      assert.ok(
        keys.every((k) => k === 'configurable/prefix/content/files/2024/06/image.jpg'),
  ...
  describe('Multipart Upload', function () {
    ...
    it('uses simple upload for files below threshold', async function () {
      ...
      assert.equal(
        url,
        'https://cdn.example.com/configurable/prefix/content/files/2024/06/small-file.mp4',
      );
      sinon.assert.calledOnce(sendStub);
    ...
    it('uses multipart upload for files at or above threshold', async function () {
      ...
      sinon
        ...
        .returns(createMockReadStream(fileContent) as unknown as fs.ReadStream);
      ...
      assert.equal(
        url,
        'https://cdn.example.com/configurable/prefix/content/files/2024/06/large-file.mp4',
      );
    ...
    it('throws error when UploadPart returns no ETag', async function () {
      ...
      const abortCall = sendStub
        .getCalls()
        .find((call) => call.args[0] instanceof AbortMultipartUploadCommand);
    ...
    it('aborts multipart upload when part upload fails with S3 error', async function () {
      ...
      assert.equal(abortCall.args[0].input.UploadId, 'test-upload-id');
      assert.equal(
        abortCall.args[0].input.Key,
        'configurable/prefix/content/files/2024/06/large-file.mp4',
      );
    });
    ...
    it('handles file exactly at threshold using multipart', async function () {
      ...
      const createCall = sendStub
        .getCalls()
        .find((call) => call.args[0] instanceof CreateMultipartUploadCommand);
    ...
    it('respects custom multipartThreshold and partSize options', async function () {
      ...
      const uploadPartCalls = sendStub
        .getCalls()
        .filter((call) => call.args[0] instanceof UploadPartCommand);
