...
describe('Adapter Cache Redis', function () {
  ...
  describe('get', function () {
    ...
    describe('concurrent cache miss coalescing', function () {
      ...
      it('propagates fetchData rejection to all concurrent callers', async function () {
        ...
        const [v1, v2] = await Promise.all([cache.get(KEY, fetchData), cache.get(KEY, fetchData)]);
      ...
      it('allows retry after a coalesced fetch rejection', async function () {
        ...
        await Promise.all([cache.get(KEY, fetchData), cache.get(KEY, fetchData)]);
        assert.equal(fetchData.callCount, 1);
      ...
      it('does not coalesce fetches across a prefix_hash cycle (reset)', async function () {
        ...
        let resolveSlow;
        const slowFetch = sinon.stub().returns(
          new Promise((resolve) => {
            resolveSlow = resolve;
          }),
        );
        ...
        assert.equal(
          fastFetch.callCount,
          1,
          'post-reset caller must not join pre-reset in-flight fetch',
        );
    ...
    it('returns the cached value when background refresh fails', async function () {
      ...
      sinon.assert.calledOnce(logging.error);
      assert.equal(
        logging.error.firstCall.args[0].message,
        'There was an error refreshing cache data in the background',
      );
  ...
  describe('metrics', function () {
    ...
    it('emits cache-error (but not cache-miss) when a redis get rejects in timeout mode', async function () {
      ...
      assert.equal(
        metricCall('cache-miss'),
        undefined,
        'redis error is not also counted as a miss',
      );
    });
