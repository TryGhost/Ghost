...
describe('UNIT: adapters/lib/s3/errors', function () {
    ...
    describe('toS3RequestError', function () {
        it('names the resource, S3 code and operation in the message', function () {
            const err = toS3RequestError(serviceException('AccessDenied'), {...OPTIONS, operation: 'GetObject'});
        ...
        it('says "save" rather than "read" for the write operations', function () {
            const put = toS3RequestError(serviceException('AccessDenied'), {...OPTIONS, operation: 'PutObject'});
            const copy = toS3RequestError(serviceException('AccessDenied'), {...OPTIONS, operation: 'CopyObject'});
            const head = toS3RequestError(serviceException('AccessDenied'), {...OPTIONS, operation: 'HeadObject'});
            ...
            assert.match(head.message, /^Could not read routes\.yaml from storage/);
        });

        it('reports the bucket, key and S3 request metadata as details', function () {
            const err = toS3RequestError(serviceException('AccessDenied'), {...OPTIONS, operation: 'PutObject'});
        ...
        it('suggests a next step for the failures an operator can act on', function () {
            assert.match(
                String(toS3RequestError(serviceException('AccessDenied'), {...OPTIONS, operation: 'PutObject'}).help),
                /credentials/
            );
            assert.match(
                String(toS3RequestError(serviceException('NoSuchBucket'), {...OPTIONS, operation: 'PutObject'}).help),
                /bucket name, region and endpoint/
            );
            assert.match(
                String(toS3RequestError(serviceException('SlowDown'), {...OPTIONS, operation: 'PutObject'}).help),
                /temporarily unavailable/
            );
        });

        it('offers no guess for an unrecognised code', function () {
            const err = toS3RequestError(serviceException('SomeBrandNewCode'), {...OPTIONS, operation: 'PutObject'});
        ...
        it('falls back to the errno when the failure never reached S3', function () {
            ...
            const err = toS3RequestError(connectionRefused, {...OPTIONS, operation: 'GetObject'});
        ...
        it('prefers the errno over an AggregateError name', function () {
            ...
            const err = toS3RequestError(aggregate, {...OPTIONS, operation: 'PutObject'});
        ...
        it('recovers the code from the status when the SDK could not name the failure', function () {
            ...
            const err = toS3RequestError(headForbidden, {...OPTIONS, operation: 'HeadObject'});
        ...
        it('reports UnknownError when there is nothing to go on', function () {
            const err = toS3RequestError(new Error('something went wrong'), {...OPTIONS, operation: 'GetObject'});

            assert.equal(err.errorDetails.s3ErrorCode, 'UnknownError');
        });

        it('does not throw on a non-object rejection', function () {
            const err = toS3RequestError(undefined, {...OPTIONS, operation: 'GetObject'});
        ...
        it('does not pick up help from Object.prototype', function () {
            const err = toS3RequestError(Object.assign(new Error('odd'), {name: 'constructor'}), {...OPTIONS, operation: 'GetObject'});

            assert.equal(err.help, undefined);
        });

        it('leaves the S3 metadata fields undefined when the SDK reported none', function () {
            const err = toS3RequestError(new Error('no metadata'), {...OPTIONS, operation: 'GetObject'});
        ...
        it('keeps the origin stack without keeping the SDK exception', function () {
            ...
            const err = toS3RequestError(sdkError, {...OPTIONS, operation: 'PutObject'});
