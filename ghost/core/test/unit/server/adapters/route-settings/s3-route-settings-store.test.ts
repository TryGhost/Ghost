...
const REAL_DEFAULTS_PATH = path.join(
  __dirname,
...
const putCommands = (sent: S3Command[]): PutObjectCommand[] =>
  sent.filter((c): c is PutObjectCommand => c instanceof PutObjectCommand);
...
describe('UNIT: S3RouteSettingsStore', function () {
  ...
  describe('constructor validation', function () {
    ...
    it('throws when only accessKeyId is provided', function () {
      assert.throws(() => new S3RouteSettingsStore({ ...validConfig, accessKeyId: 'AKIA' }), {
    ...
    it('throws when only secretAccessKey is provided', function () {
      assert.throws(() => new S3RouteSettingsStore({ ...validConfig, secretAccessKey: 'shh' }), {
        errorType: 'IncorrectUsageError',
        message: /accessKeyId.*secretAccessKey/,
      });
    });
    ...
    it('throws when sessionToken is provided without the credential pair', function () {
      assert.throws(() => new S3RouteSettingsStore({ ...validConfig, sessionToken: 'session' }), {
        errorType: 'IncorrectUsageError',
        message: /accessKeyId.*secretAccessKey/,
      });
    });
    ...
    it('accepts a tenantPrefix without throwing', function () {
      assert.doesNotThrow(
        () => new S3RouteSettingsStore({ ...validConfig, tenantPrefix: 'tenant-abc' }),
      );
    });
    ...
    it('accepts a complete credential pair without throwing', function () {
      assert.doesNotThrow(
        () =>
          new S3RouteSettingsStore({ ...validConfig, accessKeyId: 'AKIA', secretAccessKey: 'shh' }),
      );
    });
  });
  ...
  describe('behaviour (stubbed S3 client)', function () {
    ...
    describe('replace', function () {
      ...
      it('writes under the tenant prefix when configured', async function () {
        ...
        await createStore(fake.client, { tenantPrefix: 'tenant-abc' }).replace(
          fromYaml(SAMPLE_YAML),
        );
      ...
      it('reports a non-NotFound existence-check error and does not overwrite', async function () {
        ...
        await assert.rejects(
          createStore(client).replace(fromYaml(SAMPLE_YAML)),
    ...
    describe('get', function () {
      ...
      it('reports an error the API error handler can clone', async function () {
        const sdkException = Object.assign(new Error('Access Denied'), {
          name: 'AccessDenied',
