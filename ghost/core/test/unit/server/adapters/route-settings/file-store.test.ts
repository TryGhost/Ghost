...
describe('UNIT: route-settings FileStore', function () {
  ...
  const createStore = (overrides: Partial<ConstructorParameters<typeof FileStore>[0]> = {}) =>
    new FileStore({
      basePath,
  ...
  describe('adapter contract', function () {
    ...
    it('throws IncorrectUsageError when constructed without the required paths', function () {
      assert.throws(
        () => new FileStore({} as ConstructorParameters<typeof FileStore>[0]),
  ...
  describe('get', function () {
    ...
    it('throws ValidationError for structurally invalid settings', async function () {
      await fs.writeFile(
        path.join(basePath, 'routes.yaml'),
        'routes:\n  no-slashes: about\n',
        'utf8',
      );
