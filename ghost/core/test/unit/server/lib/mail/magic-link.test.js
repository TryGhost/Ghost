...
const MagicLink = require('../../../../../core/server/lib/mail/magic-link');
  ...
  describe('#sendMagicLink', function () {
    ...
    it('Sends an email to the user with a link generated from getSigninURL(token, type)', async function () {
      ...
      sinon.assert.calledWithExactly(
        options.getSigninURL.firstCall,
        token,
        'blazeit',
        'https://whatever.com',
      );
      ...
      assert.equal(
        options.transporter.sendMail.firstCall.args[0].subject,
        options.getSubject.firstCall.returnValue,
      );
      assert.equal(
        options.transporter.sendMail.firstCall.args[0].text,
        options.getText.firstCall.returnValue,
      );
      assert.equal(
        options.transporter.sendMail.firstCall.args[0].html,
        options.getHTML.firstCall.returnValue,
      );
    });
  });
  ...
  describe('#sendMagicLink with OTC', function () {
    ...
    it('should include OTC when includeOTC is true', async function () {
      ...
      sinon.assert.calledWith(
        mockSingleUseTokenProvider.deriveOTC,
        'test-token-ref-123',
        'mock-token',
      );
