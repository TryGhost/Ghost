...
describe('getInboxLinks', function () {
  ...
  it('handles Google emails', async function () {
    const emails = ['example@gmail.com', 'example@googlemail.com', 'example@google.com'];
    await Promise.all(
  ...
  it('handles Microsoft emails', async function () {
    ...
    await Promise.all(
      emails.map(async (recipient) => {
        ...
        assert.equal(
          result?.desktop,
          `https://outlook.live.com/mail/?login_hint=${encodeURIComponent(recipient)}`,
        );
        assert(result?.android.startsWith('intent:'));
  ...
  it('handles Proton emails', async function () {
    const emails = ['example@proton.me', 'example@pm.me', 'example@protonmail.com'];
    await Promise.all(
      emails.map(async (recipient) => {
        const result = await getInboxLinks({
          recipient,
  ...
  it('handles iCloud emails', async function () {
    const emails = ['example@icloud.com', 'example@me.com', 'example@mac.com'];
    await Promise.all(
      emails.map(async (recipient) => {
        const result = await getInboxLinks({
          recipient,
