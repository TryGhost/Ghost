...
const LocalStorageBase =
  require('../../../../../core/server/adapters/storage/LocalStorageBase').default;
  ...
  describe('getImageSizeFromUrl', function () {
    ...
    it('[success] should returns largest image value for .ico files', async function () {
      ...
      const requestMock = nock('https://super-website.com')
        ...
        .replyWithFile(
          200,
          path.join(__dirname, '../../../../utils/fixtures/images/favicon_multi_sizes.ico'),
        );
    ...
    it('[success] should return image dimensions asset path images', async function () {
      ...
      const imageSize = createImageSize({
        urlUtils: {
          ...
          urlJoin: function () {
            if (
              [...arguments].join('') === 'http://myblog.com///assets/img/logo.png?v=d30c3d1e41'
            ) {
    ...
    it('[success] should switch to local file storage if available', async function () {
      ...
      const requestMock = nock('http://myblog.com').get('/content/images/favicon.png').reply(200, {
      ...
      const imageSize = createImageSize({
        ...
        storageUtils: {
          isLocalImage: () => true,
          getLocalImagesStoragePath: (imageUrl) =>
            path.join(storagePath, imageUrl.replace(/.*\//, '')),
        },
        urlUtils: {
          urlFor: urlForStub,
          getSubdir: urlGetSubdirStub,
    ...
    it('[failure] will handle responses timing out', async function () {
      const url =
        'https://static.wixstatic.com/media/355241_d31358572a2542c5a44738ddcb59e7ea.jpg_256';
    ...
    it('[failure] returns error if `image-size` module throws error', async function () {
      ...
      const requestMock = nock('https://static.wixstatic.com').get('/media/nope.cur').reply(404);
        ...
        fetchExternal: (requestUrl) => {
          if (requestUrl === url) {
            return Promise.resolve({
              body: Buffer.from(
                '2c be a4 40 f7 87 73 1e 57 2c c1 e4 0d 79 03 95 42 f0 42 2e 41 95 27 c9 5c 35 a7 71 2c 09 5a 57 d3 04 1e 83 03 28 07 96 b0 c8 88 65 07 7a d1 d6 63 50'.replace(
                  / /g,
                  '',
                ),
                'hex',
  ...
  describe('getImageSizeFromStoragePath', function () {
    ...
    it('[failure] returns error if storage adapter errors', async function () {
      ...
      const imageSize = createImageSize({
        ...
        storageUtils: createFixtureStorageUtils(),
        urlUtils: createLocalUrlUtils('http://myblog.com/content/images/not-existing-image.png'),
        fetchExternal: () => Promise.reject(new Error('request should not be used')),
      });
    ...
    it('[failure] rejects traversal outside the image storage root', async function () {
      ...
      const imageSize = createImageSize({
        ...
        storageUtils: {
          isLocalImage: () => true,
          getLocalImagesStoragePath: (imageUrl) =>
            imageUrl.replace('http://myblog.com/content/images', ''),
        },
        validator: {},
        urlUtils: {
      ...
      await assert.rejects(imageSize.getImageSizeFromStoragePath(url), {
    ...
    it('[failure] returns error if `image-size` module throws error', async function () {
      ...
      const imageSize = createImageSize({
        imageStore: {
          read: () => {
            return Promise.resolve(
              Buffer.from('<svg xmlns="http://www.w3.org/2000/svg viewBox="0 0 100 100>/svg>'),
            );
          },
        },
        storageUtils: {
