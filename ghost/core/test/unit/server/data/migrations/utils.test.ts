...
describe('migrations/utils', function () {
  describe('createTransactionalMigration', function () {
    it('Accepts two functions and creates a transactional migration from them', function () {
      ...
      assert(
        up.calledOnceWith(config.transacting),
        'up function should be called with transacting',
      );
      assert(
        down.calledOnceWith(config.transacting),
        'down function should be called with transacting',
      );
    });
  });
  ...
  describe('combineTransactionalMigrations', function () {
    it('Accepts a number of migrations and returns a single migration', async function () {
      ...
      assert(
        combinedMigration.config.transaction,
        'Migration config should set transaction to true',
      );
      ...
      await upResult;
      assert(
        upA.calledOnceWith(config.transacting),
        'first up fn should be called with transacting',
      );
      assert(
        upB.calledOnceWith(config.transacting),
        'second up fn should be called with transacting',
      );
      ...
      await downResult;
      assert(
        downA.calledOnceWith(config.transacting),
        'first down fn should be called with transacting',
      );
      assert(
        downB.calledOnceWith(config.transacting),
        'second down fn should be called with transacting',
      );
      assert(
        downB.calledBefore(downA),
        'Migration down method should call child down methods in reverse order',
      );
    });
    ...
    it('Waits for each migration to finish before executing the next', async function () {
      ...
      assert(
        !downA.called,
        'penultimate down fn should not be called until last down fn has resolved',
      );
      await downDeferredB.resolve();
      assert(
        downA.calledOnce,
        'penultimate down fn should be called once last down fn has resolved',
      );
    });
    ...
    it('Stops execution if a migration errors, and propagates error', async function () {
      ...
      try {
        ...
        assert(
          !downA.called,
          'penultimate down fn should not be called if last down fn has rejected',
        );
      }
...
describe('migrations/utils/permissions', function () {
  describe('addPermission', function () {
    it('Accepts a name, action and object and returns a migration which adds a permission to the database', async function () {
      ...
      assert(
        insertedPermissionsAfterUp.length === 1,
        'The permission was inserted into the database',
      );
  ...
  describe('addPermissionWithRoles', function () {
    it('Accepts addPermission config and a list of roles, and creates the permission, linking it to roles', async function () {
      ...
      assert(
        ...
        'The permission was attached to the other role.',
      );
      ...
      assert(
        insertedPermissionsAfterDown.length === 0,
        'The permission was removed from the database',
      );
      ...
      assert(
        !permissionAttachedToOtherRoleAfterDown,
        'The permission was removed from the other role.',
      );
    });
  });
});
...
describe('migrations/utils/schema nullable functions', function () {
  describe('createSetNullableMigration', function () {
    it('Sets a not-nullable column to nullable', async function () {
      ...
      const migration = utils.createSetNullableMigration(
        'test_nullable_migration',
        'not_nullable_col',
      );

      assert.equal(
        migration.config.transaction,
        expectsTransaction,
        'createSetNullableMigration is transactional on MySQL but not on SQLite',
      );

      // Verify initial state - column should be not nullable
      const isNullableInitial = await checkColumnNullable(
        'test_nullable_migration',
        'not_nullable_col',
        knex,
      );
      ...
      // Test down migration
      await runDownMigration();
      const isNullableAfterDown = await checkColumnNullable(
        'test_nullable_migration',
        'not_nullable_col',
        knex,
      );
  ...
  describe('createDropNullableMigration', function () {
    it('Drops nullable from a nullable column', async function () {
      ...
      const migration = utils.createDropNullableMigration(
        'test_nullable_migration',
        'nullable_col',
      );
      ...
      assert.equal(
        migration.config.transaction,
        expectsTransaction,
        'createDropNullableMigration is transactional on MySQL but not on SQLite',
      );
      ...
      const isNotNullableAfter = await checkColumnNullable(
        'test_nullable_migration',
        'nullable_col',
        knex,
      );
    ...
    it('Skips dropping nullable when column is already not nullable', async function () {
      ...
      const isNotNullableInitial = await checkColumnNullable(
        'test_nullable_migration',
        'not_nullable_col',
        knex,
      );
  ...
  describe('helper functions', function () {
    it('Nullable status detection works correctly', async function () {
      ...
      const nullableResult = await checkColumnNullable(
        'test_nullable_migration',
        'nullable_col',
        knex,
      );
      const notNullableResult = await checkColumnNullable(
        'test_nullable_migration',
        'not_nullable_col',
        knex,
      );
