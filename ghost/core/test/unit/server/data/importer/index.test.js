...
const ImageHandler = Object.assign(
  {},
  require('../../../../../core/server/data/importer/handlers/image'),
);
...
describe('Importer', function () {
  ...
  describe('ImportManager', function () {
    ...
    it('gets the correct extensions', function () {
      ...
      assert.equal(ImportManager.getExtensions().length, 107);
    ...
    it('globs extensions correctly', function () {
      const extGlob =
        '+(.jpg|.jpeg|.gif|.png|.svg|.svgz|.ico|.webp|.mp4|.webm|.ogv|.mp3|.wav|.ogg|.m4a|.7z|.aac|.ai|.aif|.aiff|.apkg|.arw|.avif|.avi|.blend|.bmp|.bz2|.cr2|.cr3|.css|.csv|.deb|.dng|.doc|.docx|.eot|.epub|.eps|.fb2|.fbx|.flac|.geojson|.glb|.gltf|.gpx|.gz|.heic|.heif|.html|.ics|.ipynb|.js|.json|.jsonld|.key|.kml|.m4v|.md|.mid|.midi|.mkv|.mobi|.mov|.nef|.np3|.numbers|.obj|.odp|.ods|.odt|.opus|.orf|.otf|.pages|.paprikarecipes|.pdf|.ppt|.pptx|.psd|.py|.raf|.rar|.rpm|.rtf|.rw2|.skp|.stl|.tar|.tgz|.tif|.tiff|.toml|.tsv|.ttf|.txt|.vcf|.woff|.woff2|.xcf|.xls|.xlsb|.xlsm|.xlsx|.xml|.yaml|.zip|.markdown)';
      ...
      assert.equal(
        ...
        '+(.jpg|.jpeg|.gif|.png|.svg|.svgz|.ico|.webp)',
      );
    ...
    describe('loadFile', function () {
      ...
      it('has same result for zips and files', async function () {
        ...
        getFileSpy.returns([]);
        getFileSpy
          .withArgs(JSONHandler, sinon.match.string)
      ...
      describe('Validate Zip', function () {
        ...
        it('accepts a zip without a base directory', function () {
          const testDir = path.resolve(
            __dirname,
            '../../../../utils/fixtures/import/zips/zip-without-base-dir',
          );
        ...
        it('accepts a zip with an image directory', function () {
          const testDir = path.resolve(
            __dirname,
            '../../../../utils/fixtures/import/zips/zip-image-dir',
          );
        ...
        it('accepts a zip with a content directory', function () {
          const testDir = path.resolve(
            __dirname,
            '../../../../utils/fixtures/import/zips/zip-content-dir',
          );
        ...
        it('accepts a zip with a content/images directory', function () {
          const testDir = path.resolve(
            __dirname,
            '../../../../utils/fixtures/import/zips/zip-content-images-subdir',
          );
        ...
        it('accepts a zip with a media directory', function () {
          const testDir = path.resolve(
            __dirname,
            '../../../../utils/fixtures/import/zips/zip-media-dir',
          );
        ...
        it('accepts a zip with a files directory', function () {
          const testDir = path.resolve(
            __dirname,
            '../../../../utils/fixtures/import/zips/zip-files-dir',
          );
        ...
        it('accepts a zip with uppercase image extensions', function () {
          const testDir = path.resolve(
            __dirname,
            '../../../../utils/fixtures/import/zips/zip-uppercase-extensions',
          );
        ...
        it('fails a zip with two base directories', function () {
          const testDir = path.resolve(
            __dirname,
            '../../../../utils/fixtures/import/zips/zip-with-double-base-dir',
          );

          assert.throws(
            ImportManager.isValidZip.bind(ImportManager, testDir),
        ...
        it('fails a zip with no content', function () {
          const testDir = path.resolve(
            __dirname,
            '../../../../utils/fixtures/import/zips/zip-invalid',
          );
      ...
      describe('Zip behavior', function () {
        it('can call extract and error correctly', async function () {
          ...
          await assert.rejects(
            ImportManager.extractZip(
              path.resolve(__dirname, '../../../../utils/fixtures/import/zips/zip-with-base-dir'),
            ),
            (err) => {
  ...
  describe('JSONHandler', function () {
    ...
    it('correctly errors when given a bad db api wrapper', async function () {
      ...
      await assert.rejects(JSONHandler.loadFile(file), (response) => {
  ...
  describe('MarkdownHandler', function () {
    ...
    it('does convert a markdown file into a post object', async function () {
      ...
      assert.equal(
        moment.utc(result.data.posts[0].created_at).format('DD MM YY HH:mm'),
        '19 12 14 12:00',
      );
    ...
    it('can import a published post', async function () {
      ...
      assert.equal(
        moment.utc(result.data.posts[0].published_at).format('DD MM YY HH:mm'),
        '19 12 14 12:00',
      );
      assert(!('image' in result.data.posts[0]));
    });
    ...
    it('can import multiple files', async function () {
      ...
      assert.equal(
        moment.utc(result.data.posts[one].published_at).format('DD MM YY HH:mm'),
        '19 12 14 12:00',
      );
  ...
  describe('DataImporter', function () {
    ...
    it('rejects with a DataImportError carrying the importer errors when the import fails', async function () {
      ...
      await assert.rejects(
        DataImporter.doImport({ meta: { version: '4.0.0' }, data: {} }, {}),
        (err) => {
