...
describe('Data Generator', function () {
  ...
  it('Can import automation actions and revisions', async function () {
    ...
    const actions = await db
      .select('id', 'automation_id', 'created_at', 'type')
    ...
    for (const action of actions) {
      ...
      if (action.type === 'wait') {
        ...
        assert.ok(actionRevisions.every((revision) => revision.email_lexical !== null));
        assert.ok(
          actionRevisions.every(
            (revision) => revision.email_design_setting_id === '000000000000000000000001',
          ),
        );
      }
    }
    ...
    const bySourceAndTarget = (first, second) => {
      return (
        first.source_action_id.localeCompare(second.source_action_id) ||
        first.target_action_id.localeCompare(second.target_action_id)
      );
    };
  ...
  it('Can import automation runs and run steps', async function () {
    ...
    const runs = await db
      .select('id', 'automation_id', 'member_id', 'member_email')
    ...
    const actionCountsByAutomation = new Map(
      [...Map.groupBy(actions, (action) => action.automation_id)].map(
        ([automationId, automationActions]) => [automationId, automationActions.length],
      ),
    );
    const revisionsById = new Map(revisions.map((revision) => [revision.id, revision]));
    const latestRevisionIds = new Set(
      [...Map.groupBy(revisions, (revision) => revision.action_id).values()].map(
        (actionRevisions) => {
          return actionRevisions.toSorted(
            (first, second) =>
    ...
    const fullPathExtraSteps = [...actionCountsByAutomation.values()].reduce(
      (total, actionCount) => total + actionCount - 1,
      0,
    );
  ...
  it('Can import explicit offer redemptions', async function () {
    ...
    assert.equal(
      new Set(
        redemptions.map((redemption) => `${redemption.subscription_id}:${redemption.offer_id}`),
      ).size,
      10,
    );
  });
...
describe('Importer', function () {
  ...
  it('Should update products to reference price ids', async function () {
    ...
    const results = await db
      .select('id', 'name', 'monthly_price_id', 'yearly_price_id')
      .from('products');
  ...
  it('Clamps redemption timestamps to the subscription window', function () {
    ...
    assert.equal(
      subscriptionState.lastRedeemedAt.valueOf(),
