...
describe('Service', function () {
  ...
  describe('copySettingsBetweenThemes()', function () {
    const settingsForTheme = async (themeName) => {
      ...
      return collection
        .toJSON()
        .map(({ theme, key, type, value }) => ({ theme, key, type, value }));
    };
    ...
    it('copies settings rows to the new theme name', async function () {
      ...
      const destinationCheck = model.findAll
        .getCalls()
        .find((call) => call.firstArg.filter === `theme:'test-edited'`);
    ...
    it('leaves no partial copy behind when the copy fails part-way', async function () {
      ...
      await assert.rejects(
        service.copySettingsBetweenThemes('test', 'test-edited'),
        /second add failed/,
      );
