...
function checkResponseValue(jsonResponse, expectedProperties) {
  ...
  assert.equal(
    providedProperties.length,
    expectedProperties.length,
    'provided properties length does not match expected properties length',
  );
}
...
function checkResponse(
  ...
  let checkProperties = options.public
    ? this.expectedProperties[objectType].public || this.expectedProperties[objectType]
    : this.expectedProperties[objectType].default || this.expectedProperties[objectType];

  checkProperties = onlyProperties ? onlyProperties : checkProperties;
  checkProperties = additionalProperties
    ? checkProperties.concat(additionalProperties)
    : checkProperties;
        ...
        if (res.statusCode === 302) {
          ...
          return reject(
            new errors.InternalServerError({
              message:
                'Ghost is redirecting, do you have an instance already running on port 2369?',
            }),
          );
