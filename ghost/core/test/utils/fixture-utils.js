...
const {
  Queries: EmailAnalyticsQueries,
} = require('../../core/server/services/email-analytics/lib/queries');
...
const fixtures = {
  ...
  insertPostsAndTags: async function insertPostsAndTags() {
    await Promise.all(
      DataGenerator.forKnex.tags.map((tag) => {
    ...
    for (const post of _.cloneDeep(DataGenerator.forKnex.posts)) {
      ...
      let postAuthorsRelations = _.filter(DataGenerator.forKnex.posts_authors, {
        post_id: post.id,
      });
    ...
    await Promise.all(
      DataGenerator.forKnex.posts_meta.map((postMeta) => {
  ...
  insertExtraPosts: function insertExtraPosts(max) {
    ...
    return models.User.getOwnerUser(context.internal).then(function (ownerUser) {
      return Promise.all(
        posts.map((post, index) => {
    ...
    const gatedPosts = [
      {
        ...
        lexical: DataGenerator.markdownToLexical(
          'Before paywall\n\n<!--members-only-->\n\nAfter paywall',
        ),
        visibility: 'paid',
        ...
        return Promise.all(
          posts.slice(0, max).map((post) => {
            ...
            return models.Post.edit(
              {
                tags: post.tags.concat([
                  _.find(DataGenerator.Content.tags, { id: injectionTagId }),
                ]),
              },
  ...
  insertRoles: function insertRoles() {
    return Promise.all(
      DataGenerator.forKnex.roles.map((role) => {
  ...
  changeOwnerUserStatus: function changeOwnerUserStatus(options) {
    return models.User.getOwnerUser(context.internal).then(function (user) {
      return models.User.edit(
        { status: options.status },
        _.merge({ id: user.id }, context.internal),
      );
    });
  },
  ...
  createUsersWithRoles: function createUsersWithRoles() {
    return Promise.all(
      ...
      return Promise.all(
        _.cloneDeep(DataGenerator.forKnex.users).map((user) => {
          let userRolesRelations = _.filter(DataGenerator.forKnex.roles_users, {
            user_id: user.id,
          });
  ...
  createExtraUsers: function createExtraUsers() {
    ...
    extraUsers = _.map(extraUsers, function (user) {
      return DataGenerator.forKnex.createUser(
        _.extend({}, user, {
  ...
  permissionsFor: function permissionsFor(obj) {
    let permsToInsert = _.cloneDeep(
      fixtureManager.findModelFixtures('Permission', { object_type: obj }).entries,
    );
      ...
      if (perms[obj]) {
        if (perms[obj] === 'all') {
          _.each(actions, function (action) {
            ...
            permissionsRoles[action.permissionId].push(
              _.find(DataGenerator.Content.roles, { id: roles[role] }),
            );
          });
        } else {
          _.each(perms[obj], function (action) {
            ...
            permissionsRoles[_.find(actions, { type: action }).permissionId].push(
              _.find(DataGenerator.Content.roles, { id: roles[role] }),
            );
          });
        }
  ...
  insertInvites: function insertInvites() {
    return Promise.all(
      DataGenerator.forKnex.invites.map((invite) => {
  ...
  insertWebhooks: function insertWebhooks() {
    return Promise.all(
      DataGenerator.forKnex.webhooks.map((webhook) => {
        return models.Webhook.add(webhook, context.internal);
      }),
    );
  },
  ...
  insertIntegrations: function insertIntegrations() {
    return Promise.all(
      DataGenerator.forKnex.integrations.map((integration) => {
  ...
  insertApiKeys: function insertApiKeys() {
    return Promise.all(
      DataGenerator.forKnex.api_keys.map((api_key) => {
  ...
  insertLinks: function insertLinks() {
    return Promise.all(
      DataGenerator.forKnex.links.map((link) => {
  ...
  insertMentions: function insertMentions() {
    return Promise.all(
      DataGenerator.forKnex.mentions.map((mention) => {
  ...
  insertEmails: function insertEmails() {
    return Promise.all(
      DataGenerator.forKnex.emails.map((email) => {
  ...
  insertExtraTiers: async function insertExtraTiers() {
    ...
    const extraTier2 = DataGenerator.forKnex.createProduct({
      id: '6834edbcd2e19501e29e9537',
      slug: 'silver',
      name: 'Silver',
    });
  ...
  insertMembersAndLabelsAndProducts: async function insertMembersAndLabelsAndProducts(
    newsletters = false,
  ) {
    await Promise.all(
      DataGenerator.forKnex.labels.map((label) => {
    ...
    for (const productFixture of testProductFixtures) {
      if (productFixture.id) {
        ...
        // Used to update the core fixtures
    ...
    await Promise.all([
      ...
      (async () => {
        for (const member of _.cloneDeep(DataGenerator.forKnex.members)) {
          let memberLabelRelations = _.filter(DataGenerator.forKnex.members_labels, {
            member_id: member.id,
          });
          ...
          if (newsletters) {
            let memberNewsletterRelations = _.filter(DataGenerator.forKnex.members_newsletters, {
              member_id: member.id,
            });
            memberNewsletterRelations = _.map(
              memberNewsletterRelations,
              function (memberNewsletterRelation) {
                return _.find(DataGenerator.forKnex.newsletters, {
                  id: memberNewsletterRelation.newsletter_id,
                });
              },
            );
    ...
    const defaultProduct = await models.Product.findOne(
    ...
    for (const member of members) {
      for (const subscription of member.subscriptions) {
        await models.Member.edit(
          {
            products: member.products.concat({
  ...
  insertNewsletters: async function insertNewsletters() {
    return Promise.all(
      DataGenerator.forKnex.newsletters.map((newsletter) => {
  ...
  insertRedirects: async function insertClicks() {
    await Promise.all(
      DataGenerator.forKnex.redirects.map((click) => {
  ...
  insertClicks: async function insertClicks() {
    await Promise.all(
      DataGenerator.forKnex.members_click_events.map((click) => {
  ...
  insertFeedback: async function insertFeedback() {
    await Promise.all(
      DataGenerator.forKnex.members_feedback.map((feedback) => {
  ...
  insertSnippets: function insertSnippets() {
    return Promise.all(
      DataGenerator.forKnex.snippets.map((snippet) => {
  ...
  insertCustomThemeSettings: function insertCustomThemeSettings() {
    return Promise.all(
      DataGenerator.forKnex.custom_theme_settings.map((setting) => {
  ...
  async enableAllLabsFeatures() {
    const labsValue = Object.fromEntries(
      labsService.WRITABLE_KEYS_ALLOWLIST.map((key) => [key, true]),
    );
