...
const limitService = require('../../core/server/services/limits').default;
  ...
  Object.keys(matchers).forEach((key) => {
    ...
    if (value instanceof RegExp) {
      assert.match(
        spyCall.args[0][key],
        value,
        `Expected Email ${emailCount} to have ${key} that matches ${value}, got ${spyCall.args[0][key]}`,
      );
      return;
    }
    ...
    assert.equal(
      spyCall.args[0][key],
      value,
      `Expected Email ${emailCount} to have ${key} of ${value}`,
    );
  });
...
const sentEmailCount = (expectedCount) => {
  ...
  assert.equal(
    mocks.mail.callCount,
    expectedCount,
    `Expected ${expectedCount} emails to be sent, but ${mocks.mail.callCount} were sent.`,
  );
};
