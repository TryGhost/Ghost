...
const exposeFixtures = async () => {
  ...
  return Promise.all(Object.values(fixturePromises))
    .then((results) => {
...
const startGhost = async (options) => {
  ...
  debug(
    `[e2e-utils] Accumulated start time across ${totalBoots} boots is ${totalStartTime / 1000}s (average = ${averageBootTime}ms)`,
  );
  return ghostServer;
};
