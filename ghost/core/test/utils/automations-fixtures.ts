...
const TEST_AUTOMATION_SLUGS = [MEMBER_WELCOME_EMAIL_SLUGS.free, MEMBER_WELCOME_EMAIL_SLUGS.paid];
...
const timestamp = (offset: number): string =>
  moment(new Date(Date.UTC(2026, 0, 1, 0, 0, offset))).format('YYYY-MM-DD HH:mm:ss');
...
export async function setupAutomationsFixture(): Promise<void> {
  ...
  await db
    .knex('automation_action_edges')
    .insert([...buildLinearEdges(freeActions), ...buildLinearEdges(paidActions)]);
}
...
export async function cleanupAutomationsFixture(): Promise<void> {
  ...
  if (automationIds.length === 0) {
    await db.knex('email_design_settings').where('id', TEST_EMAIL_DESIGN_SETTING_ID).del();
    return;
  }

  const actionIds: string[] = await db
    .knex('automation_actions')
    .whereIn('automation_id', automationIds)
    .pluck('id');
  const runIds: string[] = await db
    .knex('automation_runs')
    .whereIn('automation_id', automationIds)
    .pluck('id');

  if (runIds.length > 0) {
    const runStepIds: string[] = await db
      .knex('automation_run_steps')
      .whereIn('automation_run_id', runIds)
      .pluck('id');

    await db.knex('automated_email_recipients').whereIn('automation_run_step_id', runStepIds).del();

    await db.knex('automation_run_steps').whereIn('automation_run_id', runIds).del();
    await db.knex('automation_runs').whereIn('id', runIds).del();
  }

  if (actionIds.length > 0) {
    const revisionIds: string[] = await db
      .knex('automation_action_revisions')
    ...
    if (revisionIds.length > 0) {
      await db
        ...
        .del();
      const redirectIds: string[] = await db
        .knex('redirects')
        .whereIn('automation_action_revision_id', revisionIds)
        .pluck('id');
      if (redirectIds.length > 0) {
        await db.knex('members_click_events').whereIn('redirect_id', redirectIds).del();
        await db.knex('redirects').whereIn('id', redirectIds).del();
      }
    }
    ...
    await db
      ...
      .del();
    await db.knex('automation_action_revisions').whereIn('action_id', actionIds).del();
    await db.knex('automation_actions').whereIn('id', actionIds).del();
  }

  await db.knex('automations').whereIn('id', automationIds).del();
  await db.knex('email_design_settings').where('id', TEST_EMAIL_DESIGN_SETTING_ID).del();
}
