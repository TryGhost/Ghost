...
describe('Front-end members behavior', function () {
  ...
  describe('Member routes', function () {
    it('should error serving webhook endpoint without any parameters', async function () {
      await request.post('/members/webhooks/stripe').expect(400);
    });

    it('should fail processing a webhook endpoint with stripe header', async function () {
      await request
        .post('/members/webhooks/stripe')
    ...
    it('should return no content for invalid token passed in session', async function () {
      await request.get('/members/api/session').expect(204);
    });

    it('should return no content for invalid token passed in entitlements', async function () {
      await request.get('/members/api/entitlements').expect(204);
    });
    ...
    it('returns an entitlement token for a valid member session', async function () {
      ...
      const res = await request.get('/members/api/entitlements').expect(200);
    ...
    it('should error for invalid member token on member data endpoint', async function () {
      await request.get('/members/api/member').expect(204);
    });

    it('should error for invalid data on member magic link endpoint', async function () {
      await request.post('/members/api/send-magic-link').expect(400);
    });

    it('should error for invalid data on members create checkout session endpoint', async function () {
      await request.post('/members/api/create-stripe-checkout-session').expect(400);
    });
    ...
    it('should error for invalid data on members create update session endpoint', async function () {
      await request.post('/members/api/create-stripe-update-session').expect(400);
    });
    ...
    it('should error for invalid subscription id on members create update session endpoint', async function () {
      ...
      await request
        .post('/members/api/create-stripe-update-session')
        .send({
          identity: token,
    ...
    it('should error for invalid data on members subscription endpoint', async function () {
      await request.put('/members/api/subscriptions/123').expect(400);
    });
    ...
    describe('Newsletters', function () {
      ...
      it('should error for fetching member newsletters with missing uuid', async function () {
        await request.get('/members/api/member/newsletters').expect(401);
      });

      it('should error for fetching member newsletters with invalid uuid', async function () {
        await request.get('/members/api/member/newsletters?uuid=abc').expect(401);
      });

      it('should error for updating member newsletters with missing uuid', async function () {
        await request.put('/members/api/member/newsletters').expect(401);
      });

      it('should error for updating member newsletters with invalid uuid', async function () {
        await request.put('/members/api/member/newsletters?uuid=abc').expect(401);
      });
      ...
      it('should 401 for GET member newsletters with a mismatched hmac key', async function () {
        sinon.stub(settingsHelpers, 'getMembersValidationKey').returns('test');
        await request.get('/members/api/member/newsletters?uuid=abc&key=blah').expect(401);
      });

      it('should 401 for PUT member newsletters with a mismatched hmac key', async function () {
        sinon.stub(settingsHelpers, 'getMembersValidationKey').returns('test');
        await request.put('/members/api/member/newsletters?uuid=abc&key=blah').expect(401);
      });
      ...
      it('should fetch and update member newsletters with valid uuid', async function () {
        ...
        const res = await request
          .put(`/members/api/member/newsletters?uuid=${memberUUID}&key=${memberHmac}`)
          .send({
            newsletters: [],
          })
          .expect(200);
        ...
        const resRestored = await request
          .put(`/members/api/member/newsletters?uuid=${memberUUID}&key=${memberHmac}`)
          .send({
    ...
    it('should serve theme 404 on members endpoint', async function () {
      await request.get('/members/').expect(404).expect('Content-Type', 'text/html; charset=utf-8');
    });

    it('should redirect invalid token on members endpoint', async function () {
      await request
        .get('/members/?token=abc&action=signup')
        .expect(302)
  ...
  describe('Unsubscribe', function () {
    ...
    it('should pass through an optional newsletter param', async function () {
      await request
        .get('/unsubscribe/?uuid=XXX&newsletter=YYY')
        .expect(302)
    ...
    it('should pass through an optional key param', async function () {
      await request
        .get('/unsubscribe/?uuid=XXX&key=YYY')
        .expect(302)
    ...
    it('should pass through an optional updates & announcements param', async function () {
      await request
        .get('/unsubscribe/?uuid=XXX&updatesandannouncements=1')
        .expect(302)
        .expect(
          'Location',
          `${config.get('url')}/?uuid=XXX&updatesandannouncements=1&action=unsubscribe`,
        );
    });

    it('should reject when missing a uuid', async function () {
      await request.get('/unsubscribe/').expect(400);
    });
    ...
    it('should do an actual unsubscribe on POST', async function () {
      ...
      await request.post(`/unsubscribe/?uuid=${memberUUID}&key=${memberHmac}`).expect(201);
      ...
      assert.equal(
        getJsonResponse.newsletters.length,
        0,
        'Sending a POST request to the unsubscribe endpoint should unsubscribe the member',
      );
    });
    ...
    it('should only do a partial unsubscribe on POST', async function () {
      ...
      const member = await createMember({
        ...
        newsletters: [{ id: newsletterId }, { id: newsletter2Id }],
      });
      ...
      await request
        .post(`/unsubscribe/?uuid=${memberUUID}&newsletter=${newsletter2Uuid}&key=${memberHmac}`)
        .expect(201);
      ...
      assert.equal(
        getJsonResponse.newsletters.length,
        1,
        'Sending a POST request to the unsubscribe endpoint should unsubscribe the member from that specific newsletter',
      );
    });
    ...
    it('should unsubscribe from comment notifications on POST', async function () {
      ...
      await request
        .post(`/unsubscribe/?uuid=${memberUUID}&key=${memberHmac}&comments=1`)
        .expect(201);

      const updatedMember = await members.api.members.get(
        { id: member.id },
    ...
    it('should unsubscribe from updates & announcements on POST', async function () {
      ...
      await request
        .post(`/unsubscribe/?uuid=${memberUUID}&key=${memberHmac}&updatesandannouncements=1`)
        .expect(201);
      ...
      assert.equal(
        updatedMember.related('newsletters').models.length,
        1,
        'newsletters should be untouched',
      );
    });
    ...
    it('unsubscribe post works with x-www-form-urlencoded', async function () {
      ...
      await request
        .post(`/unsubscribe/?uuid=${memberUUID}&key=${memberHmac}`)
        .type('form')
  ...
  describe('Content gating', function () {
    ...
    beforeAll(function () {
      ...
      paidPostWithPaywallCardEmailOnly = testUtils.DataGenerator.forKnex.createPost({
        ...
        mobiledoc:
          '{"version":"0.3.1","markups":[],"atoms":[],"cards":[["paywall",{}]],"sections":[[1,"p",[[0,[],0,"Before paywall"]]],[10,0],[1,"p",[[0,[],0,"After paywall"]]]]}',
    ...
    describe('as non-member', function () {
      ...
      it('cannot read members post content', async function () {
        await request.get('/thou-shalt-not-be-seen/').expect(200).expect(assertContentIsAbsent);
      });

      it('cannot read paid post content', async function () {
        await request.get('/thou-shalt-be-paid-for/').expect(200).expect(assertContentIsAbsent);
      });

      it('cannot read label-only post content', async function () {
        await request.get('/thou-must-be-labelled-vip/').expect(200).expect(assertContentIsAbsent);
      });
      ...
      it("doesn't generate a MemberPageView event", async function () {
        ...
        assert(
          spy.notCalled,
          "A page view from a non-member shouldn't generate a MemberPageViewEvent event",
        );
      });
    ...
    describe('log out', function () {
      ...
      it('clears both ghost-members-ssr and ghost-members-ssr.sig cookies when signature is invalid', async function () {
        ...
        const mainCookieCleared = setCookieHeaders.some(
          (cookie) =>
            cookie.startsWith('ghost-members-ssr=') &&
            cookie.includes('expires=Thu, 01 Jan 1970 00:00:00 GMT'),
        );

        const sigCookieCleared = setCookieHeaders.some(
          (cookie) =>
            cookie.startsWith('ghost-members-ssr.sig=') &&
            cookie.includes('expires=Thu, 01 Jan 1970 00:00:00 GMT'),
        );

        assert.ok(
          sigCookieCleared,
          'ghost-members-ssr.sig cookie should be cleared with expiry in the past',
        );
        assert.ok(
          mainCookieCleared,
          'ghost-members-ssr cookie should be cleared with expiry in the past',
        );
      });

      it('an invalid token causes a set-cookie logout when requesting the identity', async function () {
        // Check logged in
        await request.get('/members/api/member').expect(200);
    ...
    describe('as free member', function () {
      ...
      it('can read members post content', async function () {
        await request.get('/thou-shalt-not-be-seen/').expect(200).expect(assertContentIsPresent);
      });
    ...
    describe('as free member with vip label', function () {
      ...
      it('generates a MemberPageView event', async function () {
        ...
        assert.equal(
          ...
          null,
          "The member shouldn't have a `last_seen_at` property set before this test.",
        );
        ...
        assert(
          spy.calledOnce,
          'A page view from a member should generate a MemberPageViewEvent event',
        );
        member = await models.Member.findOne({ email });
        assert.notEqual(
          member.get('last_seen_at'),
          null,
          'The member should have a `last_seen_at` property after having visited a page while logged-in.',
        );
      });
    });
    ...
    describe('as paid member', function () {
      ...
      it('can fetch member data', async function () {
        const res = await request.get('/members/api/member').expect(200);
      ...
      it('can read paid post content', async function () {
        await request.get('/thou-shalt-be-paid-for/').expect(200).expect(assertContentIsPresent);
      });
