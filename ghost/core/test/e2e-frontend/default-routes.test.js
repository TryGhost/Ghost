...
describe('Default Frontend routing', function () {
  ...
  describe('Error', function () {
    ...
    it('should 404 for unknown file', async function () {
      await request
        .get('/content/images/some/file/that/doesnt-exist.jpg')
        .expect('Cache-Control', testUtils.cacheRules.noCache)
        .expect(404)
    ...
    it('/welcome/ should respond with valid HTML', async function () {
      await request
        ...
        .expect((res) => {
          ...
          assert.match(
            res.text,
            /<h1[^>]*?>Start here for a quick overview of everything you need to know<\/h1>/,
          );
    ...
    it('/rss/ should serve an RSS feed', async function () {
      await request
        ...
        .expect((res) => {
          assert.match(
            res.text,
            /<!\[CDATA\[Start here for a quick overview of everything you need to know\]\]>/,
          );
          assert(!res.text.includes('__GHOST_URL__'));
        });
    });
  ...
  describe('Private Blogging', function () {
    ...
    it('should still serve private RSS feed', async function () {
      await request
        .get(`/${settingsCache.get('public_hash')}/rss/`)
        .expect(200)
    ...
    it('should still serve private tag RSS feed', async function () {
      await request
        .get(`/tag/getting-started/${settingsCache.get('public_hash')}/rss/`)
        .expect(200)
        .expect('Cache-Control', testUtils.cacheRules.private)
        .expect('Content-Type', 'application/rss+xml; charset=utf-8')
        .expect(assertCorrectFrontendHeaders)
    ...
    it('should redirect, NOT 404 for private tag RSS feed with extra path', async function () {
      await request
        .get(`/tag/getting-started/${settingsCache.get('public_hash')}/rss/hack/`)
        .expect(
          'Location',
          `/private/?r=%2Ftag%2Fgetting-started%2F${settingsCache.get('public_hash')}%2Frss%2Fhack%2F`,
        )
        .expect(302)
        .expect(assertCorrectFrontendHeaders);
    });
    ...
    it('should redirect, NOT 404 for unknown private RSS feed', async function () {
      ...
      await request
        .get(`/hack/${settingsCache.get('public_hash')}/rss/`)
