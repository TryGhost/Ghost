...
describe('Front-end gift links', function () {
  ...
  beforeAll(async function () {
    ...
    const lexical = testUtils.DataGenerator.markdownToLexical(
      'Before paywall\n\n<!--members-only-->\n\nAfter paywall',
    );
  ...
  it('paywalls the paid post for an anonymous visitor on the canonical URL', async function () {
    const res = await request.get(`/${slug}/`).expect(200).expect(assertLocked);

    // No gift → no toast on the canonical URL.
    assert.doesNotMatch(
      res.text,
      /gh-gift-toast/,
      'gift toast must not appear on the canonical URL',
    );
    // The bare URL has no ?gift, so it isn't no-store bypassed — it stays cacheable.
    assert.doesNotMatch(
      res.headers['cache-control'] || '',
      /no-store/,
      'the bare URL is cacheable',
    );
  });

  it('unlocks the full post for an anonymous visitor with a valid ?gift token', async function () {
    const res = await request.get(`/${slug}/?gift=${token}`).expect(200).expect(assertUnlocked);
    ...
    assert.match(
      res.text,
      /gifted access to this paid post/,
      'toast announces the gift and its paid access level',
    );
  });

  it('varies the toast copy by access level: a members-only post reads "members-only"', async function () {
    const res = await request.get(`/${membersSlug}/?gift=${membersToken}`).expect(200);

    assert.match(res.text, /id="gh-gift-toast"/, 'default gift toast renders on a gift view');
    assert.match(
      res.text,
      /gifted access to this members-only post/,
      'toast reflects the members-only access level',
    );
  });

  it('301s to the canonical URL, dropping ?gift, when the token is invalid', async function () {
    const res = await request.get(`/${slug}/?gift=not-a-real-token`).redirects(0).expect(301);
  ...
  it('unlocks a paid page with a valid ?gift token on its canonical URL', async function () {
    ...
    assert.match(
      res.text,
      /gifted access to this paid page/,
      'toast reflects that the gifted entry is a page',
    );
  });

  it('301s a paid page, dropping ?gift, when the token is invalid', async function () {
    const res = await request.get(`/${pageSlug}/?gift=not-a-real-token`).redirects(0).expect(301);
  ...
  it('does not unlock a different post: a token for post A 301s away from post B', async function () {
    ...
    const res = await request.get(`/${otherSlug}/?gift=${token}`).redirects(0).expect(301);
  ...
  it('strips a malformed (non-string) ?gift param without looping', async function () {
    ...
    assert.doesNotMatch(
      ...
      /\?/,
      'the malformed param is stripped, leaving no query string',
    );
  });
  ...
  it('does not expose the synthesized member to template rendering (@member stays anonymous)', async function () {
    ...
    assert.match(
      res.text,
      /gh-test-member-state">anonymous</,
      '@member must stay anonymous on a gift view',
    );
  });
  ...
  it('resolves kg-gated-block visibility as a paid member on a gift view', async function () {
    ...
    assert.doesNotMatch(
      res.text,
      /Anonymous gated content/,
      'nonMember block must be stripped on a gift view',
    );
  });
  ...
  it('resolves kg-gated-block visibility as anonymous on the canonical URL', async function () {
    ...
    const res = await request.get(`/${gatedBlocksSlug}/`).expect(200).expect(assertLocked);

    assert.match(
      res.text,
      /Anonymous gated content/,
      'nonMember block renders for an anonymous read',
    );
    assert.doesNotMatch(
      res.text,
      /Paid-member gated content/,
      'paid-segment block must not render without access',
    );
  });
  ...
  describe('markdown variants', function () {
    ...
    it('serves the gated markdown variant on the clean .md URL', async function () {
      ...
      assert.doesNotMatch(
        res.text,
        /Paid-member gated content/,
        'gated blocks stay stripped for the anonymous markdown read',
      );
    });
  });
  ...
  describe('the gift_link analytics dimension', function () {
    ...
    it('emits the verified token as tb_gift_link on a valid gift read', async function () {
      const res = await request.get(`/${slug}/?gift=${token}`).expect(200);
    ...
    it('emits an empty tb_gift_link on the canonical (non-gift) URL', async function () {
      const res = await request.get(`/${slug}/`).expect(200);
    ...
    it('keeps an invalid token out of analytics — the 301 strips it and the canonical render carries none', async function () {
      ...
      const res = await request.get(`/${slug}/?gift=not-a-real-token`).redirects(1).expect(200);

      assert.match(res.text, /tb_gift_link=""/, 'the canonical render carries no gift dimension');
      assert.ok(
        !res.text.includes('not-a-real-token'),
        'the forged token never appears in the response',
      );
    });
    ...
    it('keeps a token for another post out of that post’s analytics', async function () {
      ...
      const res = await request.get(`/${otherSlug}/?gift=${token}`).redirects(1).expect(200);
