...
const { allEmailJobsSettled } = require('../utils/batch-email-utils');
...
describe('Click Tracking', function () {
  ...
  it('Full test', async function () {
    ...
    } = await agent.post('/posts/?source=html', {
      body: {
        posts: [
          {
            title: 'My Newsletter',
    ...
    } = await agent.put(`/posts/${draft.id}/?newsletter=${newsletterSlug}`, {
      body: {
        posts: [
          {
    ...
    const {
      body: { links },
    } = await agent.get(`/links/?filter=${encodeURIComponent(`post_id:'${post.id}'`)}`);
    ...
    for (const link of links) {
      ...
      if (isInternal) {
        ...
        assert(
          redirectedToUrl.searchParams.get('attribution_id'),
          'attribution_id should be present on internal redirects',
        );
        assert(
          redirectedToUrl.searchParams.get('attribution_type'),
          'attribution_type should be present on internal redirects',
        );
      } else {
        ...
        assert(
          !redirectedToUrl.searchParams.get('attribution_id'),
          'attribution_id should not be present on internal redirects',
        );
        assert(
          !redirectedToUrl.searchParams.get('attribution_type'),
          'attribution_type should not be present on internal redirects',
        );
      }
    ...
    const {
      body: {
        members: [memberWhoClicked],
      },
    } = await agent.get(`/members/${memberToClickLink.id}`);
    assert(memberWhoClicked.last_seen_at !== null, 'last_seen_at should be set after a click');
    assert(
      new Date(memberWhoClicked.last_seen_at).getTime() > 0,
      'last_seen_at should be a valid date',
    );
