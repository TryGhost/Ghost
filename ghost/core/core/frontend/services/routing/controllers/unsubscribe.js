...
module.exports = async function unsubscribeController(req, res) {
  ...
  if (req.method === 'POST') {
    logging.info(
      '[List-Unsubscribe] Received POST unsubscribe for ' +
        query.uuid +
        ', newsletter: ' +
        (query.newsletter ?? 'null') +
        ', comments: ' +
        (query.comments ?? 'false') +
        ', updates & announcements: ' +
        (query.updatesandannouncements ?? 'false'),
    );
    ...
    try {
      if (!query.key) {
        logging.warn(
          '[List-Unsubscribe] Unsubscribe failed due to missing verification key for ' + query.uuid,
        );
      ...
      const member = await members.api.members.get(
        { uuid: query.uuid },
      ...
      if (member) {
        if (query.comments) {
          ...
          const filteredNewsletters = query.newsletter
            ? member
                .related('newsletters')
                .models.filter((n) => n.get('uuid') !== query.newsletter)
                .map((n) => ({ id: n.id }))
            : [];
          await members.api.members.update(
            {
