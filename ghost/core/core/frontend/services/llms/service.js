...
function createLlmsService({
  ...
  async function getLlmsFullTxt() {
    ...
    const header = [buildHeader(), LLMS_FULL_TXT_INTRO, MARKDOWN_DISCOVERABILITY, ''].join('\n');
    ...
    if (!wasTruncated) {
      const postResult = await appendBoundedSectionPaginated(output, 'Posts', 'post', {
        maxEntries: LLMS_FULL_TXT_POST_LIMIT,
      });
  ...
  function buildOptionalSection() {
    ...
    optionalLinks.push(
      `- [Full content of pages and posts](${urlUtils.urlFor({ relativeUrl: '/llms-full.txt' }, true)})`,
    );
