...
function createAddonBlockRuntimeMiddleware() {
  return function serveAddonBlockRuntime(req, res) {
    ...
    const install = installs.find(
      (candidate) => candidate?.enabled === true && candidate.handle === handle,
    );
    const block = Array.isArray(install?.editor?.blocks)
      ? install.editor.blocks.find(
          (candidate) => candidate?.name === blockName && candidate.hydrate === true,
        )
      : undefined;
