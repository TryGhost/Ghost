...
function sign(expiry, nonce, secret) {
  return crypto.createHmac('sha256', secret).update(`${expiry}:${nonce}`).digest('hex');
}
...
function adminToolbarMiddleware(req, res, next) {
  ...
  res.locals.staffFrontendToolsEnabled = toolbarSuppressed
    ? false
    : hasValidToken(getCookieValue(req));
