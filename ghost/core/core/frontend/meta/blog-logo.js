...
function getBlogLogo() {
    ...
    if (settingsCache.get('logo')) {
        ...
        logging.info('[IMAGE-CDN-TEST] getBlogLogo -> using site logo ' + JSON.stringify({logoSetting: settingsCache.get('logo'), resolvedUrl: logo.url}));
    } else {
        ...
        logging.info('[IMAGE-CDN-TEST] getBlogLogo -> fallback to icon ' + JSON.stringify({resolvedUrl: logo.url}));
    }
