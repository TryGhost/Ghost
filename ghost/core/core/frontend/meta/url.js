...
function getUrl(data, absolute) {
  if (checks.isPost(data)) {
    ...
    if (typeof data.url === 'string') {
      ...
      return absolute
        ? urlUtils.relativeToAbsolute(data.url)
        : urlUtils.absoluteToRelative(data.url);
    }
    ...
    const postResource = {
      status: 'published',
      ...data,
      type: checks.isPage(data) ? 'pages' : 'posts',
    };
    ...
    return urlService.getUrlForResource(postResource, {
      absolute: absolute,
