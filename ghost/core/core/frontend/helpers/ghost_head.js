...
function getMarkdownAlternateLink({ context, post, canonicalUrl }) {
  ...
  try {
    return (
      ...
      escapeExpression(getMarkdownUrl(canonicalUrl)) +
      '">'
    );
  } catch (err) {
    logging.warn(err);
...
function getAddonBlocksHelper(dataRoot) {
  const resources = [
    dataRoot.post,
    dataRoot.page,
    ...(Array.isArray(dataRoot.posts) ? dataRoot.posts : []),
  ];
  const hasAddonBlock = resources.some(
    (resource) => typeof resource?.html === 'string' && resource.html.includes('kg-addon-card'),
  );
  if (!hasAddonBlock) {
    return '';
  }
...
module.exports = async function ghost_head(options) {
  ...
  try {
    ...
    if (options.data.site.accent_color) {
      ...
      const existingScriptIndex = _.findLastIndex(
        head,
        (str) => typeof str === 'string' && /<\/(style|script)>/.test(str),
      );
