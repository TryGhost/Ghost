...
(function () {
  ...
  const footerLinks = document.querySelector(
    config.footerLinksSelector || '.gh-private-trigger-wrap',
  );
  ...
  const subscribeFeedback =
    subscribeForm && subscribeForm.querySelector('[data-ghost-private-subscribe-feedback]');
  ...
  const messages = {
    ...
    restrictedDomain:
      config.restrictedDomainMessage || 'Signups from this email domain are currently restricted.',
    tooManyAttempts: config.tooManyAttemptsMessage || 'Too many sign-up attempts, try again later',
  };
  const successResetDelay = Number.isFinite(config.successResetDelay)
    ? config.successResetDelay
    : 1000;
  ...
  subscribeForm.addEventListener('submit', async function (event) {
    ...
    try {
      ...
      const newsletterInputs = subscribeForm.querySelectorAll(
      ...
      const checkableNewsletterInputs = subscribeForm.querySelectorAll(
