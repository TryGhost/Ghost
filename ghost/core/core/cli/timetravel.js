...
module.exports = class TimeTravel extends Command {
  ...
  async handle(argv = {}) {
    ...
    if (!argv.offset) {
      const datumPoint = await knex(datum.table).max(datum.column, { as: datum.column }).first();
      ...
      argv.offset = Math.floor(
        DateTime.utc().diff(DateTime.fromJSDate(datumPoint[datum.column]), 'days').toObject().days,
      );
    }
    ...
    for (const table in dateTimeFields) {
      for (const column of dateTimeFields[table]) {
        ...
        await db(table).update(column, db.raw(`DATE_ADD(${column}, interval ${dateOffset} day)`));
