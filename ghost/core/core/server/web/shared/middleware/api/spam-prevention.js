...
// Per-staff-user limiter for editor presence POSTs (/enter, /leave).
// Generous because legitimate use fires on every editor navigation:
// 600 requests / hour / user is roughly 1 every 6s, comfortably above
// expected editorial usage but tight enough to cap a runaway client.
const presenceBlock = () => {
    const ExpressBrute = require('express-brute');
...
const checkoutSessionGlobal = () => {
  ...
  if (!checkoutSessionGlobalInstance) {
    checkoutSessionGlobalInstance = new ExpressBrute(
      store,
      extend(
        {
          attachResetToRequest: true,
...
const checkoutSessionEmail = () => {
  ...
  if (!checkoutSessionEmailInstance) {
    checkoutSessionEmailInstance = new ExpressBrute(
      store,
      extend(
        {
          attachResetToRequest: true,
...
const otcVerificationEnumeration = () => {
  ...
  if (!otcVerificationEnumerationInstance) {
    otcVerificationEnumerationInstance = new ExpressBrute(
      store,
      extend(
        {
          attachResetToRequest: false,
...
const otcVerification = () => {
  ...
  if (!otcVerificationInstance) {
    otcVerificationInstance = new ExpressBrute(
      store,
      extend(
        {
          attachResetToRequest: false,
