  ...
  return new Promise((resolve: () => void) => {
    const connect = (options: ConnectOptions) => {
      ...
      client.on('error', (err: Error) => {
        logging.warn(
          `Can't connect to the bootstrap socket (${socketAddress.host} ${socketAddress.port}) ${getErrorCode(err)}.`,
        );
