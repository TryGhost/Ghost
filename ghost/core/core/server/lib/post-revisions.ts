...
export class PostRevisions {
  ...
  shouldGenerateRevision(
    ...
    const isUnpublished =
      options && options.newStatus === 'draft' && options.olderStatus === 'published';
    if (isUnpublished) {
    ...
    if (
      lexicalHasChanged ||
      titleHasChanged ||
      featuredImagedHasChanged ||
      customExcerptHasChanged
    ) {
  ...
  async getRevisions(
    ...
    if (updatedRevisions.length > this.config.max_revisions) {
      return updatedRevisions.slice(
        updatedRevisions.length - this.config.max_revisions,
  ...
  async removeAuthorFromRevisions(authorId: string, options: any): Promise<void> {
    ...
    const revisionIds = revisions.toJSON().map(({ id }: { id: string }) => id);
