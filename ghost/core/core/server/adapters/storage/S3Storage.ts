...
const messages = {
  ...
  multipartUploadReadFailed:
    'There was an error uploading the file. The file may have been modified or removed during upload.',
  ...
  multipartChunkSizeTooSmall:
    'S3Storage multipartChunkSizeBytes must be at least 5 MiB (5242880 bytes)',
  ...
  partialCredentials:
    'S3Storage requires both accessKeyId and secretAccessKey when either is provided',
};
...
const configSchema = z
  .object({
    ...
    multipartChunkSizeBytes: z
      .number({ error: tpl(messages.missingMultipartChunkSize) })
...
export default class S3Storage extends StorageBase {
  ...
  async save(file: UploadFile, targetDir?: string): Promise<string> {
    ...
    if (stats.size >= this.multipartUploadThresholdBytes) {
      logging.info(
        `Large file, using multipart upload: file=${key} size=${stats.size} threshold=${this.multipartUploadThresholdBytes}`,
      );
      return await this.uploadMultipart(file, key);
    }

    logging.info(
      `Small file, using simple upload: file=${key} size=${stats.size} threshold=${this.multipartUploadThresholdBytes}`,
    );
  ...
  private buildKey(relativePath: string): string {
    ...
    const pathWithStorage = path.posix.join(
      this.storagePath,
      this.toCanonicalRelativePath(relativePath),
    );

    if (
      !pathWithStorage.startsWith(this.storagePath + '/') &&
  ...
  private toCanonicalRelativePath(input: string): string {
    return (
      this.fromAbsoluteFilesystemPath(input) ??
