  ...
  Bookshelf.Model = Bookshelf.Model.extend(
    {
      ...
      getAction(event, options) {
        ...
        const previousPrimaryName =
          this.previous('title') || this.previous('name') || this.previous('email');
    ...
    },
    {
      ...
      async addActions(event, ids, options) {
        if (ids.length === 1) {
          ...
          const model = await this.findOne(
            { [options.column ?? 'id']: ids[0] },
            { require: true, transacting: options.transacting, context: { internal: true } },
          );
      ...
      getBulkAction(event, count, options) {
        ...
        if (
          this.getBulkActionExtraContext &&
          typeof this.getBulkActionExtraContext === 'function'
        ) {
          context = {
            ...context,
