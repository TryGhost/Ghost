...
User = ghostBookshelf.Model.extend(
  ...
  },
  {
    ...
    findOne: function findOne(dataToClone, unfilteredOptions) {
      ...
      if (options.columns) {
        options.columns = options.columns.filter((col) =>
          this.prototype.permittedAttributes().includes(col),
        );
      }
    ...
    add: function add(dataToClone, unfilteredOptions) {
      ...
      function getAuthorRole() {
        return ghostBookshelf
          .model('Role')
          .findOne({ name: 'Author' }, { transacting: options.transacting })
      ...
      return ghostBookshelf.Model.add
        ...
        .then(function (_roles) {
          ...
          if (typeof roles[0] === 'string' && !ObjectId.isValid(roles[0])) {
            ...
            return Promise.all(rolePromises).then((roleModels) => {
    ...
    permissible: async function permissible(
      ...
      if (
        typeof userModelOrId === 'object' &&
        !(typeof userModelOrId.related('roles') === 'object')
      ) {
