...
function getDisplayableCommentIdsQuery(excludedStatuses, { parentId, parentIds, postId } = {}) {
  ...
  const scopeFilter = {
    sql: [
      scopedParentIds?.length
        ? `AND comments.parent_id IN (${scopedParentIds.map(() => '?').join(',')})`
        : '',
      postId ? 'AND comments.post_id = ?' : '',
    ]
      .filter(Boolean)
      .join('\n              '),
    bindings: [...(scopedParentIds?.length ? scopedParentIds : []), ...(postId ? [postId] : [])],
  };
    ...
    applyCustomQuery(options) {
      ...
      this.query((qb) => {
        if (options.browseAll) {
          ...
          qb.whereIn(
            'comments.id',
            getDisplayableCommentIdsQuery(excludedStatuses, {
        ...
        if (options.reportCount !== undefined) {
          const subquery =
            '(SELECT COUNT(*) FROM comment_reports WHERE comment_reports.comment_id = comments.id)';
      ...
      if (repliesOptionIndex > -1) {
        ...
        withRelated[repliesOptionIndex] = {
          replies: (qb) => {
            qb.whereIn(
              'comments.id',
              getDisplayableCommentIdsQuery(excludedStatuses, { parentIds }),
            );
          },
        };
      }
    ...
    async findPage(options) {
      ...
      if (result.data.length > 0 && relationsToLoadInBatch.length > 0) {
        ...
        await collection.load(
          relationsToLoadInBatch,
          _.omit(options, 'withRelated', 'columns', 'selectRaw'),
        );
      }
        ...
        direct_replies(modelOrCollection, options) {
          ...
          modelOrCollection.query(
            ...
            ghostBookshelf.knex.raw(
              ...
              [...excludedCommentStatuses, ...excludedCommentStatuses],
            ),
          );
        },
