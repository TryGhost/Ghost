...
Post = ghostBookshelf.Model.extend(
  {
    ...
    handleAttachedModels: function handleAttachedModels(model) {
      ...
      model.related('tags').once('detaching', function detachingTags(collection, tag) {
        model
          .related('tags')
          .once('detached', function detachedTags(detachedCollection, response, options) {
      ...
      model.related('tags').once('attaching', function tagsAttaching(collection, tags) {
        model
          .related('tags')
          .once('attached', function tagsAttached(detachedCollection, response, options) {
            tags.forEach((tag) => {
      ...
      model.related('authors').once('detaching', function authorsDetaching(collection, author) {
        model
          .related('authors')
          .once('detached', function authorsDetached(detachedCollection, response, options) {
      ...
      model.related('authors').once('attaching', function authorsAttaching(collection, authors) {
        model
          .related('authors')
          .once('attached', function authorsAttached(detachedCollection, response, options) {
    ...
    onSaving: async function onSaving(model, attrs, options) {
      ...
      if (newStatus === 'scheduled') {
        if (!publishedAt) {
          ...
          moment(publishedAt).isBefore(
            moment().add(config.get('times').cannotScheduleAPostBeforeInMinutes, 'minutes'),
          ) &&
          !options.importing &&
        ...
        loopTags: for (const tag of this.get('tags')) {
          ...
          for (i = 0; i < tagsToSave.length; i = i + 1) {
            if (
              tagsToSave[i].name &&
              tag.name &&
              tagsToSave[i].name.toLocaleLowerCase() === tag.name.toLocaleLowerCase()
            ) {
              continue loopTags;
            }
          }
      ...
      if (
        ...
        try {
          this.set(
            'html',
            await lexicalLib.render(this.get('lexical'), { transacting: options.transacting }),
          );
        } catch (err) {
          throw new errors.ValidationError({
      ...
      if ((newStatus === 'published' || newStatus === 'sent') && this.hasChanged('status')) {
        ...
        if (this.hasChanged('published_by') && !options.importing) {
          this.set(
            'published_by',
            this.previous('published_by') ? String(this.previous('published_by')) : null,
          );
        }
      }
      ...
      if (!model.get('mobiledoc') && !options.importing && !options.migrating) {
        ...
        const authorExists = await ghostBookshelf
          .model('User')
          .findOne({ id: authorId }, { transacting: options.transacting });
        if (!authorExists) {
          authorId = (
            await ghostBookshelf.model('User').getOwnerUser({ transacting: options.transacting })
          ).get('id');
        }
        ops.push(async function updateRevisions() {
          const revisionModels = await ghostBookshelf.model('PostRevision').findAll(
            Object.assign(
              {
          ...
          const revisions = revisionModels
            .toJSON()
  ...
  },
  {
    ...
    edit: function edit(data, unfilteredOptions) {
      ...
      const editPost = () => {
        ...
        return ghostBookshelf.Model.edit.call(this, data, options).then((post) => {
          return this.findOne(
            {
              status: 'all',
              id: options.id,
    ...
    add: function add(data, unfilteredOptions) {
      ...
      const addPost = () => {
        return ghostBookshelf.Model.add.call(this, data, options).then((post) => {
          return this.findOne(
            {
              status: 'all',
              id: post.id,
