...
class DockerDatabaseUtils {
  ...
  async init() {
    ...
    try {
      ...
      console.log(
        `Database connection initialized (${this.options.host}:${this.options.port}/${this.options.database})`,
      );
    } catch (error) {
  ...
  async getPostsWithDetails(options = {}) {
    ...
    let query = this.knex('posts').select(['uuid', 'slug', 'type', 'published_at', 'status']);
  ...
  async getSiteUuid() {
    ...
    try {
      const result = await this.knex('settings').where('key', 'site_uuid').select('value').first();
