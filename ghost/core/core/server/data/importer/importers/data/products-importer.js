    ...
    _.each(this.dataToImport, (row) => {
      ...
      if (!row.monthly_price || !row.currency) {
        const monthlyStripePrice =
          _.find(this.requiredFromFile.stripe_prices, { id: row.monthly_price_id }) ||
          _.find(this.requiredExistingData.stripe_prices, { id: row.monthly_price_id });
      ...
      if (!row.yearly_price || !row.currency) {
        const yearlyStripePrice =
          _.find(this.requiredFromFile.stripe_prices, { id: row.yearly_price_id }) ||
          _.find(this.requiredExistingData.stripe_prices, { id: row.yearly_price_id });
