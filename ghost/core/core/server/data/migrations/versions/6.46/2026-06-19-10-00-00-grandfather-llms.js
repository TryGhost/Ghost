...
module.exports = createTransactionalMigration(
  async function up(knex) {
    const setting = await knex('settings').where({ key: 'llms_enabled' }).first();
    ...
    if (setting.value !== 'true') {
      logging.info(
        'llms_enabled setting is not the previous default of "true", skipping migration',
      );
      return;
    }

    logging.info('Disabling llms_enabled for existing site');
    await knex('settings').where({ key: 'llms_enabled' }).update({ value: 'false' });
  },
  async function down() {
