...
module.exports = createTransactionalMigration(
  async function up(knex) {
    ...
    for (const row of rows) {
      ...
      if (!existingSettings) {
        logging.warn(
          `Skipping welcome email ${row.id} sender details backfill - email design settings row ${row.email_design_setting_id} does not exist`,
        );
        continue;
      }
      ...
      if (!Object.keys(attrs).length) {
        logging.info(
          `Skipping welcome email ${row.id} sender details backfill - no missing sender details to copy`,
        );
        continue;
      }

      logging.info(
        `Backfilling welcome email ${row.id} sender details to email design settings row ${row.email_design_setting_id}`,
      );

      await knex(TARGET_TABLE).where({ id: row.email_design_setting_id }).update(attrs);
    }
  },
