...
module.exports = createTransactionalMigration(
  async function up(knex) {
    ...
    if (!recipientsTableExists) {
      logging.warn(
        'Skipping foreign key migration - automated_email_recipients table does not exist',
      );
      return;
    }
    ...
    if (!newTableExists) {
      logging.warn(
        'Skipping foreign key migration - welcome_email_automated_emails table does not exist',
      );
      return;
    }
  ...
  async function down(knex) {
    ...
    if (!recipientsTableExists) {
      logging.warn(
        'Skipping foreign key rollback - automated_email_recipients table does not exist',
      );
      return;
    }
    ...
    if (!newTableExists) {
      logging.warn(
        'Skipping foreign key rollback - welcome_email_automated_emails table does not exist',
      );
      return;
    }
