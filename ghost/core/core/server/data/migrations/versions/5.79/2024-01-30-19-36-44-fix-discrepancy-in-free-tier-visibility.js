...
module.exports = createTransactionalMigration(
  async function up(knex) {
    const portalPlansRaw = await knex('settings')
      .select('value')
    ...
    if (!portalPlansRaw || !freeTier) {
      logging.warn(
        'Could not read `portal_plans` setting or `visibility` of the free tier - skipping migration',
      );
      return;
    }
    ...
    if (portalPlans.includes('free') && freeTier.visibility === 'none') {
      logging.info(
        '`portal_plans` setting contains "free", but free tier is not visible - updating free tier visibility to public',
      );
    ...
    if (!portalPlans.includes('free') && freeTier.visibility === 'public') {
      logging.info(
        '`portal_plans` setting does not contain "free", but free tier is visible - updating free tier visibility to none',
      );
