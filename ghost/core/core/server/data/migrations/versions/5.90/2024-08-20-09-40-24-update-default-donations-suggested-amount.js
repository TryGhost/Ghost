  ...
  async function up(knex) {
    try {
      ...
      if (existingSuggestedAmount.value !== '0') {
        logging.info(
          'donations_suggested_amount setting does not have previous default of 0, skipping migration',
        );
        return;
      }
      ...
      await knex('settings').where({ key: 'donations_suggested_amount' }).update({ value: '500' });
    } catch (error) {
