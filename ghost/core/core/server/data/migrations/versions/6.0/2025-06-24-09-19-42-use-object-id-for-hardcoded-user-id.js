...
module.exports = createTransactionalMigration(
  async function up(knex) {
    ...
    const currentUserWithHardcodedId = await knex('users')
      .where('id', LEGACY_HARDCODED_USER_ID)
      .first();
    ...
    logging.info(
      `Cloned existing user with updated properties - ID: ${newId}, email: ${temporaryEmail}, slug: ${temporarySlug}`,
    );
    ...
    await knex('permissions_users')
      .where('user_id', LEGACY_HARDCODED_USER_ID)
    ...
    await knex('posts_authors')
      .where('author_id', LEGACY_HARDCODED_USER_ID)
    ...
    await knex('posts')
      .where('published_by', LEGACY_HARDCODED_USER_ID)
      .update({ published_by: newId });
