...
module.exports = createTransactionalMigration(
  async function up(knex) {
    ...
    if (!existingLatestCollection) {
      ...
      const latestPostsRows = await knex('posts').select('id').where({
        type: 'post',
      });
    ...
    if (!existingFeaturedCollection) {
      ...
      const featuredPostsRows = await knex('posts').select('id').where({
        featured: true,
        type: 'post',
      });
  ...
  async function down(knex) {
    ...
    if (existingLatestCollection) {
      logging.info(
        `Deleting collection_posts for latest collection: ${existingLatestCollection.id}`,
      );
    ...
    if (existingFeaturedCollection) {
      logging.info(
        `Deleting collection_posts for featured collection: ${existingFeaturedCollection.id}`,
      );
