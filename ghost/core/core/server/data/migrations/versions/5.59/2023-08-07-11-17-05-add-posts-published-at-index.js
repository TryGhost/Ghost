...
module.exports = createNonTransactionalMigration(
  async function up(knex) {
    ...
    if (DatabaseInfo.isSQLite(knex)) {
      const result = await knex.raw(
        `select * from sqlite_master where type = 'index' and tbl_name = 'posts' and name = '${INDEX_NAME}'`,
      );
    ...
    if (hasIndex) {
      logging.info(
        `Skipping creation of index ${INDEX_NAME} on posts for published_at - already exists`,
      );
      return;
    }
  ...
  async function down(knex) {
    ...
    if (missingIndex) {
      logging.info(
        `Skipping drop of index ${INDEX_NAME} on posts for published_at - does not exist`,
      );
      return;
    }
