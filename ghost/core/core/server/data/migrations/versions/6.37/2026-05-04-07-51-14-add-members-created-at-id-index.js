...
async function hasIndex(knex) {
  if (DatabaseInfo.isSQLite(knex)) {
    const result = await knex.raw(
      `select * from sqlite_master where type = 'index' and tbl_name = 'members' and name = '${INDEX_NAME}'`,
    );
...
module.exports = createNonTransactionalMigration(
  async function up(knex) {
    if (await hasIndex(knex)) {
      logging.info(
        `Skipping creation of index ${INDEX_NAME} on members for created_at, id - already exists`,
      );
      return;
    }
  ...
  async function down(knex) {
    if (!(await hasIndex(knex))) {
      logging.info(
        `Skipping drop of index ${INDEX_NAME} on members for created_at, id - does not exist`,
      );
      return;
    }
