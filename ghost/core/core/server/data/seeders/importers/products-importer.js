...
class ProductsImporter extends TableImporter {
   ...
   */
  async finalise() {
    const stripeProducts = await this.transaction
      .select('id', 'product_id', 'stripe_product_id')
      .from('stripe_products');
    const stripePrices = await this.transaction
      .select('id', 'stripe_product_id', 'interval')
      .from('stripe_prices');
    ...
    for (const { id } of products) {
      ...
      const monthlyPrice = stripePrices.find((p) => {
        return p.stripe_product_id === stripeProduct.stripe_product_id && p.interval === 'monthly';
      });
      const yearlyPrice = stripePrices.find((p) => {
        return p.stripe_product_id === stripeProduct.stripe_product_id && p.interval === 'yearly';
      });
