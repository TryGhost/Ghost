...
> {
  ...
  async importForEach(
    ...
    debug(
      `${this.name} generated ${data.length} records in ${Date.now() - now}ms (${settingReferenceModel}ms setting reference model)`,
    );
  ...
  async batchInsert(data: T[]): Promise<void> {
    ...
    if (data.length > 5000 && !process.env.DISABLE_FAST_IMPORT) {
      ...
      const [result] = await this.transaction.raw(
        `LOAD DATA LOCAL INFILE '${filePath}' INTO TABLE \`${this.name}\` FIELDS TERMINATED BY ',' ENCLOSED BY '"' IGNORE 1 LINES (${Object.keys(
          data[0],
        )
          .map((d) => '`' + d + '`')
          .join(',')});`,
      ...
      if (result.affectedRows !== data.length) {
        ...
        logging.warn(
          `CSV import warning: expected ${data.length} imported rows, got ${result.affectedRows}.`,
        );
      }
    } else {
      await this.knex
        .batchInsert(this.name, data as GeneratedModel[])
