...
class MembersSubscriptionCreatedEventsImporter extends TableImporter {
  ...
  async import(quantity) {
    ...
    while (true) {
      const membersStripeCustomersSubscriptions = await this.transaction
      ...
      const membersStripeCustomers = await this.transaction
        ...
        .whereIn(
          'customer_id',
          membersStripeCustomersSubscriptions.map((subscription) => subscription.customer_id),
        );
  ...
  generate() {
    ...
    if (luck(30)) {
      const createdAt = fromDatabaseDate(this.model.created_at);
      const post = this.posts.find(
        (p) => p.visibility === 'public' && fromDatabaseDate(p.published_at) < createdAt,
      );
      if (post) {
        attribution = {
