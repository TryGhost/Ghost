...
class MembersStripeCustomersImporter extends TableImporter {
  ...
  generate() {
    if (this.model.status !== 'paid') {
      ...
      const daysSinceMemberCreated = Math.floor(
        (new Date() - fromDatabaseDate(this.model.created_at)) / (1000 * 60 * 60 * 24),
      );
      const shouldHaveStripeCustomer =
        faker.number.int({ min: 0, max: 100 }) <
        Math.max(Math.min(daysSinceMemberCreated / 60, 15), 2);
