...
class MembersStripeCustomersSubscriptionsImporter extends TableImporter {
  ...
  async import() {
    ...
    this.products = await this.transaction
      ...
      .from('products')
      .whereNot('type', 'free');
    this.stripeProducts = await this.transaction
      .select('id', 'product_id', 'stripe_product_id')
      .from('stripe_products');
    this.stripePrices = await this.transaction
      .select(
        'id',
        'nickname',
        'stripe_product_id',
    ...
    while (true) {
      const membersStripeCustomers = await this.transaction
      ...
      this.members = await this.transaction
        ...
        .whereIn(
          'id',
          membersStripeCustomers.map((m) => m.member_id),
        );
      ...
      const membersProducts = await this.transaction
  ...
  generate() {
    ...
    let ghostProduct = memberProduct
      ? this.products.find((product) => product.id === memberProduct.product_id)
      : null;
    ...
    const stripeProduct = this.stripeProducts.find(
      (product) => product.product_id === ghostProduct.id,
    );
    ...
    const status = createValid
      ? faker.helpers.arrayElement(validStatusses)
      : faker.helpers.arrayElement(invalidStatusses);
