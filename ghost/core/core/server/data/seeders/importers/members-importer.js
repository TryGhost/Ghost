...
class MembersImporter extends TableImporter {
  ...
  async import(quantity = this.defaultQuantity) {
    ...
    debug(
      `${this.name} generated ${this.timestamps.length} timestamps in ${Date.now() - generateNow}ms`,
    );
  ...
  async finalise() {
    ...
    for (const [memberId, emailInfo] of Object.entries(memberData)) {
      ...
      await this.transaction('members')
        .update({
  ...
  generate() {
    ...
    return {
      ...
      email: `${name
        .replace(' ', '.')
        .replace(/[^a-zA-Z0-9]/g, '')
        .toLowerCase()}${faker.number.int({ min: 0, max: 999999 })}@example.com`,
      ...
      last_seen_at: luck(40)
        ? toDatabaseDate(faker.date.recent({ days: 7 }))
        : toDatabaseDate(faker.date.between({ from: timestamp, to: new Date() })),
