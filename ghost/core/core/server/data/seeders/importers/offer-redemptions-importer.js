...
class OfferRedemptionsImporter extends TableImporter {
  ...
  async import(quantity = this.defaultQuantity) {
    ...
    const [offers, stripeProducts, membersStripeCustomers, subscriptions] = await Promise.all([
      ...
      this.transaction
        .select('id', 'customer_id', 'plan_id', 'plan_interval', 'created_at', 'current_period_end')
        .from('members_stripe_customers_subscriptions'),
    ]);

    const productIdByStripeProductId = new Map(
      stripeProducts.map((stripeProduct) => [
        stripeProduct.stripe_product_id,
        stripeProduct.product_id,
      ]),
    );
    const memberIdByCustomerId = new Map(
      membersStripeCustomers.map((customer) => [customer.customer_id, customer.member_id]),
    );
    ...
    for (const subscription of subscriptions) {
      ...
      this.subscriptionPool.push({
        ...
        subscriptionCreatedAt: fromDatabaseDate(subscription.created_at),
  ...
  generate() {
    ...
    if (subscriptionState.availableOffers.length === 0) {
      this.subscriptionPool[subscriptionIndex] =
        this.subscriptionPool[this.subscriptionPool.length - 1];
