...
class EmailRecipientsImporter extends TableImporter {
  ...
  async import(quantity) {
    ...
    this.emailBatches = await this.transaction
      .select('id', 'email_id', 'updated_at')
      .from('email_batches')
      .orderBy('email_id');
    const members = await this.transaction.select('id', 'uuid', 'email', 'name').from('members');
    this.membersSubscribeEvents = await this.transaction
      .select('id', 'newsletter_id', 'created_at', 'member_id')
      .from('members_subscribe_events')
      .orderBy('created_at', 'asc'); // Order required for better performance in setReferencedModel
    ...
    for (const memberSubscribeEvent of this.membersSubscribeEvents) {
      ...
      this.membersSubscribeEventsByNewsletterId
        .get(memberSubscribeEvent.newsletter_id)
        .push(memberSubscribeEvent);

      if (
        !this.membersSubscribeEventsCreatedAtsByNewsletterId.has(memberSubscribeEvent.newsletter_id)
      ) {
        this.membersSubscribeEventsCreatedAtsByNewsletterId.set(
          memberSubscribeEvent.newsletter_id,
          [],
        );
      }

      this.membersSubscribeEventsCreatedAtsByNewsletterId
        .get(memberSubscribeEvent.newsletter_id)
  ...
  setReferencedModel(model) {
    ...
    const lastIndex = findFirstHigherIndex(
      this.membersSubscribeEventsCreatedAtsByNewsletterId.get(this.model.newsletter_id),
      this.batchUpdatedAt,
    );

    this.membersList = this.membersSubscribeEventsByNewsletterId
      .get(this.model.newsletter_id)
      .slice(0, Math.max(0, lastIndex - 1))
    ...
    this.emailMeta = {
      ...
      openedCount: Math.max(
        0,
        this.emailMeta.openedCount - Math.max(0, offset - this.emailMeta.failedCount),
      ),
      deliveredCount: Math.max(
        0,
        this.emailMeta.deliveredCount -
          Math.max(0, offset - this.emailMeta.failedCount - this.emailMeta.openedCount),
      ),
    };
  }
  ...
  generate() {
    ...
    timestamp = new Date(
      timestamp.getTime() - this.eventStartTimeUsed.getTime() + this.batchUpdatedAt.getTime(),
    );
