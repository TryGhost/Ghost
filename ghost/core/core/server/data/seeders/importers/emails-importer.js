...
class EmailsImporter extends TableImporter {
  ...
  generate() {
    ...
    const publishedAt = fromDatabaseDate(this.model.published_at);
    ...
    const recipientCount = this.membersSubscribeEvents
      ...
      .filter((entry) => fromDatabaseDate(entry.created_at) < timestamp).length;
    ...
    return {
      ...
      submitted_at: toDatabaseDate(timestamp),
