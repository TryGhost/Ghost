...
class DataGenerator {
    ...
    for (const table of tables) {
      ...
      if (table === 'roles_users') {
        await transaction(table)
          .del()
          .whereNot('user_id', ownerUser?.user_id || null);
      } else if (table === 'users') {
        // Avoid deleting the admin user
        await transaction(table)
          .del()
          .whereNot('id', ownerUser?.user_id || null);
      } else {
  ...
  async importData() {
    ...
    if (this.useTransaction) {
      ...
      const transactionConfig = DatabaseInfo.isSQLite(this.knex)
        ? {}
  ...
  async #run(transaction) {
    ...
    if (await transaction.schema.hasTable('members_current_subscription')) {
      try {
        ...
        const resolved = await transaction('members_resolved_subscription').select(
          'member_id',
          'subscription_id',
        );
