...
module.exports = async (model, frame, options = {}) => {
    ...
    _(metaAttrs).filter((k) => {
        ...
        // The `posts_meta` table is lazily created, so values can be undefined even when the
        // schema defines a default. Preserve explicit falsey values from the row.
        const defaultValue = Object.prototype.hasOwnProperty.call(postsMetaSchema[attr], 'defaultTo') ? postsMetaSchema[attr].defaultTo : null;
        const value = _.get(jsonModel.posts_meta, attr);

        jsonModel[attr] = _.isUndefined(value) ? defaultValue : value;
    });
