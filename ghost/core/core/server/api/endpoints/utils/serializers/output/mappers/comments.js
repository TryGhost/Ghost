...
const postFields = ['id', 'uuid', 'title', 'url', 'feature_image'];

const countFields = ['replies', 'direct_replies', 'likes'];

const countFieldsAdmin = ['replies', 'direct_replies', 'likes', 'dislikes', 'reports'];
...
const commentMapper = (model, frame) => {
  ...
  const replyToComment =
    jsonModel.in_reply_to ||
    (!isPublicRequest && jsonModel.parent_id && !jsonModel.in_reply_to_id && jsonModel.parent);

  if (
    replyToComment &&
    (replyToComment.status === 'published' ||
      (!isPublicRequest && replyToComment.status === 'hidden'))
  ) {
  ...
  const response = _.pick(
    jsonModel,
    fields ? commentFields.filter((field) => fields.has(field)) : commentFields,
  );
