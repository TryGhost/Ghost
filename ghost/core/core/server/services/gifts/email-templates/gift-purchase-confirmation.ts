...
export function renderText(data: GiftPurchaseConfirmationData, t: Translate): string {
    const heading = data.gift.recipientEmail && data.gift.deliveryDate ? t('Your gift is scheduled') : data.gift.recipientEmail ? t('Your gift is on its way') : t('Your gift is ready');
