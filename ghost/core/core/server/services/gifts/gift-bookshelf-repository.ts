...
type BookshelfModel<T> = {
  ...
  ): Promise<BookshelfDocument<T> | null>;
  findAll(options?: BookshelfFindOptions): Promise<BookshelfCollection<T>>;
  findPage(
    options: GiftEventQueryOptions,
  ): Promise<{ data: GiftEventBookshelfDocument[]; meta: GiftEventPage['meta'] }>;
};
...
export class GiftBookshelfRepository implements GiftRepository {
  ...
  async findPendingReminder({
    ...
    reminderFloorMs,
    transacting,
  }: FindPendingReminderOptions): Promise<Gift[]> {
  ...
  private async browseEvents({
    ...
    const replaceCustomFilter = (existingFilter: ParsedNqlFilter): ParsedNqlFilter =>
      replaceFilters(existingFilter, {
        custom: filter,
      });
