...
export interface GiftRepository {
    ...
    recordDeliveryOutcome(data: {providerMessageId: string; outcome: GiftDeliveryOutcome; timestamp: Date; diagnostics: string | null}): Promise<boolean>;
...
export class GiftBookshelfRepository implements GiftRepository {
    ...
    async recordDeliveryOutcome({providerMessageId, outcome, timestamp, diagnostics}: {providerMessageId: string; outcome: GiftDeliveryOutcome; timestamp: Date; diagnostics: string | null}): Promise<boolean> {
        return this.transaction(async (transacting) => {
            const updated = await transacting('gifts')
                ...
                .update({
                    ...
                    delivery_outcome_at: timestamp,
                    delivery_outcome_diagnostics: diagnostics
                });
