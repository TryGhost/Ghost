...
export async function init(options: GiftServiceInitOptions): Promise<void> {
  ...
  const {
    Base: BaseModel,
    Gift: GiftModel,
    GiftDelivery: GiftDeliveryModel,
    MemberStripeCustomer: StripeCustomerModel,
  } = require('../../models');
  ...
  DomainEvents.subscribe(
    ...
    async (event: { data: { memberId: string } }) => {
      try {
...
export async function recoverPendingDeliveries(): Promise<void> {
  ...
  try {
    ...
    if (sentCount + skippedCount + failedCount > 0) {
      logging.info(
        `Gift delivery recovery: ${sentCount} sent, ${skippedCount} skipped, ${failedCount} failed`,
      );
    }
  } catch (err) {
