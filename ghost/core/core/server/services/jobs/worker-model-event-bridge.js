...
class WorkerModelEventBridge {
  ...
  async handle(message, meta = {}) {
    ...
    if (validationError) {
      this.logging.warn(
        `Ignoring invalid worker model event from job ${meta.jobName || 'unknown'}: ${validationError}`,
      );
      return false;
    }
  ...
  async emitModelEvent(message) {
    ...
    try {
      ...
      if (isNotFoundError(err)) {
        this.logging.warn(
          `Could not emit worker model event ${message.eventName}: ${message.model} ${message.id} was not found`,
        );
        return false;
      }
