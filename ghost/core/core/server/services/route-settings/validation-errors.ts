...
export type RouteSettingsErrorCode =
  (typeof ROUTE_SETTINGS_ERROR_CODES)[keyof typeof ROUTE_SETTINGS_ERROR_CODES];
...
const CONTAINER_EXPECTATIONS: Record<string, { subject: string; expectation: string }> = {
  routes: {
    subject: 'a route',
    expectation: 'a template name, or a map of route options (e.g. template, controller, data)',
  },
  collections: {
    subject: 'a collection',
    expectation: 'a map of collection options (e.g. permalink, template, filter)',
  },
};
...
function describeExpectation(
...
): { subject: string; expectation: string } | null {
  if (path.length === 0) {
  ...
  if (path.length === 2 && section === 'taxonomies') {
    return {
      subject: `the ${String(path[1])} taxonomy`,
      expectation: `a permalink route (e.g. ${String(path[1])}: /${String(path[1])}/{slug}/)`,
    };
  }
...
export function validationError(
  ...
  {
    help,
    code = ROUTE_SETTINGS_ERROR_CODES.INVALID_ROUTE_SETTINGS,
  }: { help?: string; code?: RouteSettingsErrorCode } = {},
...
export function toValidationError(
  ...
  const resolvedCode =
    code ?? codeForPath(path) ?? ROUTE_SETTINGS_ERROR_CODES.INVALID_ROUTE_SETTINGS;

  const expectation = describeExpectation(path);
  if (!expectation) {
    return validationError(formatLocation(path), issue.message, {
      help: DOCS_URL,
      code: resolvedCode,
    });
  }
  ...
  const listProblem = Array.isArray(provided)
    ? lookup(LIST_ENTRY_PROBLEMS, fieldKey(path))
    : undefined;
  const found = listProblem ?? `${describeValue(provided)} was provided`;

  return validationError(
    formatLocation(path),
    `${expectation.subject} must be ${expectation.expectation}, but ${found}.`,
    { help: DOCS_URL, code: resolvedCode },
  );
}
