...
const messages = {
  ...
  authorDeprecatedError:
    '"author" is reserved. Please choose a different name. We recommend not using author.',
};
...
const OptionalBooleanField = z
  ...
  .transform((v) => v ?? undefined);
const LimitField = z
  .union([z.number(), z.literal('all'), z.string().regex(/^\d+$/).transform(Number)])
  .nullish()
  .transform((v) => v ?? undefined);
...
const DataLongFormEntrySchema = z.discriminatedUnion('type', [
  DataReadEntrySchema,
  DataBrowseEntrySchema,
]);

const DATA_ENTRY_HELP =
  'e.g.\n data:\n  my-tag:\n    resource: tags\n    type: read\n    slug: recipes\n';
    ...
    if (!result.success) {
      ...
      if (issue.code === 'invalid_union' && issue.path[0] === 'type') {
        if (record.type === undefined) {
          throw validationError(at, 'type is required. Please use read or browse.', {
            help: DATA_ENTRY_HELP,
            code: ROUTE_SETTINGS_ERROR_CODES.INVALID_DATA,
          });
        }
        throw validationError(
          at,
          `type "${record.type}" is not supported. Please use read or browse.`,
      ...
      if (issue.path.includes('resource')) {
        if (record.resource === undefined) {
          throw validationError(
            at,
            `resource is required. Please use ${humanList(VALID_LONGFORM_RESOURCES)}.`,
            { help: DATA_ENTRY_HELP, code: ROUTE_SETTINGS_ERROR_CODES.INVALID_RESOURCE },
          );
        }
        throw validationError(
          at,
          `resource "${record.resource}" is not supported. Please use ${humanList(VALID_LONGFORM_RESOURCES)}.`,
          { help: DATA_ENTRY_HELP, code: ROUTE_SETTINGS_ERROR_CODES.INVALID_RESOURCE },
        );
      }
      if (issue.path.includes('slug') && !record.slug) {
        throw validationError(at, 'slug is required for read data entries.', {
          help: DATA_ENTRY_HELP,
          code: ROUTE_SETTINGS_ERROR_CODES.INVALID_DATA,
        });
      }
...
function parseRouteData(data: unknown, path: PathSegment[]): RouteData {
  ...
  for (const key of Object.keys(record)) {
    if (RESERVED_DATA_KEYS.includes(key)) {
      throw validationError(formatLocation([...path, key]), tpl(messages.badDataError, { key }), {
...
const routeObjectSchema = (path: PathSegment[]) =>
  z
    .object({
...
const collectionValueSchema = (path: PathSegment[]) =>
  z
    .object({
...
function validatePath(
  ...
  example: string,
  {
    allowParamNotation = false,
    code = ROUTE_SETTINGS_ERROR_CODES.INVALID_PATH,
  }: { allowParamNotation?: boolean; code?: RouteSettingsErrorCode } = {},
): void {
  const at = formatLocation(path);
  ...
  if (!value.startsWith('/')) {
    throw validationError(
      at,
      `"${value}" is missing a leading slash. Please use e.g. ${example}.`,
      { code },
    );
  }
  if (!value.endsWith('/')) {
    throw validationError(
      at,
      `"${value}" is missing a trailing slash. Please use e.g. ${example}.`,
      { code },
    );
  }
  if (!allowParamNotation && /\/:\w+/.test(value)) {
    ...
    throw validationError(
      at,
      `"${value}" uses the :param notation. Please use "${value.replace(/\/:(\w+)/g, '/{$1}')}".`,
      { code },
    );
  }
}
  ...
  if (rawRoutes) {
    for (const [path, value] of Object.entries(rawRoutes)) {
      ...
      if (value === null || value === undefined) {
        throw validationError(
          formatLocation(routeLocation),
          'Please define a template, e.g. /about/: about.',
          { code: ROUTE_SETTINGS_ERROR_CODES.INVALID_TEMPLATE },
        );
      }
  ...
  if (rawCollections) {
    for (const [path, value] of Object.entries(rawCollections)) {
      ...
      if (!collection.permalink) {
        throw validationError(
          formatLocation(collectionLocation),
          'Please define a permalink route, e.g. permalink: /{slug}/.',
          { code: ROUTE_SETTINGS_ERROR_CODES.INVALID_PERMALINK },
        );
      }
  ...
  if (rawTaxonomies) {
    for (const [key, value] of Object.entries(rawTaxonomies)) {
      ...
      if (!['tag', 'author'].includes(key)) {
        throw validationError(
          formatLocation(taxonomyLocation),
          'Unknown taxonomy. Please use tag or author.',
          { code: ROUTE_SETTINGS_ERROR_CODES.INVALID_TAXONOMY },
        );
      }
      if (!value) {
        throw validationError(
          formatLocation(taxonomyLocation),
          `Please define a taxonomy permalink route, e.g. ${key}: /${key}/{slug}/.`,
          { code: ROUTE_SETTINGS_ERROR_CODES.INVALID_PERMALINK },
        );
      }
