...
export class RecommendationService {
      ...
      setTimeout(
        async () => {
          try {
            ...
            logging.error(
              '[Recommendations] Failed to update all recommendations metadata on boot',
              e,
            );
          }
        },
  ...
  async updateAllRecommendationsMetadata() {
    ...
    for (const recommendation of recommendations) {
      try {
        ...
        logging.error(
          '[Recommendations] Failed to save updated metadata for recommendation ' +
  ...
  private sendMentionToRecommendation(recommendation: Recommendation) {
    this.mentionSendingService
      .sendAll({
        url: this.wellknownService.getURL(),
        links: [recommendation.url],
      })
      .catch((err) => {
  ...
  async _updateRecommendationMetadata(recommendation: Recommendation) {
    ...
    try {
      ...
      logging.error(
        '[Recommendations] Failed to update metadata for recommendation ' +
          recommendation.url.toString(),
        e,
      );
    }
  ...
  async editRecommendation(
    id: string,
    recommendationEdit: Partial<Recommendation>,
