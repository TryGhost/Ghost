...
export type ModelInstance<T> = {
  ...
  save(
    properties: object,
    options?: { autoRefresh?: boolean; method?: 'update' | 'insert' },
...
export abstract class BookshelfRepository<IDType, T extends Entity<IDType>> {
  ...
  async #fetchAll(options: AllOptions<T> = {}): Promise<T[]> {
    ...
    return (await Promise.all(models.map((model) => this.modelToEntity(model)))).filter(
      (entity) => !!entity,
    ) as T[];
  }

  async getAll({ filter, order, include }: Omit<AllOptions<T>, 'page' | 'limit'> = {}): Promise<
    T[]
  > {
  ...
  async getPage({
    ...
    limit,
    include,
  }: AllOptions<T> & Required<Pick<AllOptions<T>, 'page' | 'limit'>>): Promise<T[]> {
    return this.#fetchAll({
      filter,
      order,
      page,
  ...
  async getGroupedCount<K extends keyof T>({
    filter,
    groupBy,
  }: {
    filter?: string;
    groupBy: K;
  }): Promise<({ count: number } & Record<K, T[K]>)[]> {
