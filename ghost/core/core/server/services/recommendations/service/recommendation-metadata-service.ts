...
type OEmbedService = {
  fetchOembedDataFromUrl<Type extends string>(
    url: string,
    type: Type,
    options?: { timeout?: number | { request: number } },
...
export class RecommendationMetadataService {
  ...
  async #fetchJSON(url: URL, options?: { timeout?: number | { request: number } }) {
    ...
    try {
      const timeout =
        typeof options?.timeout === 'number' ? { request: options.timeout } : options?.timeout;
  ...
  async fetch(
    url: URL,
    options: { timeout: number | { request: number } } = { timeout: { request: 5000 } },
