...
type RecommendationModelClass<T> = ModelClass<T> & {
  findOne: (
    data: RecommendationFindOneData<T>,
    options?: { require?: boolean },
  ) => Promise<ModelInstance<T> | null>;
};

export class BookshelfRecommendationRepository
  extends BookshelfRepository<string, Recommendation>
  implements RecommendationRepository
{
  ...
  applyCustomQuery(query: Knex.QueryBuilder, options: AllOptions<Recommendation>) {
    ...
    if (
      options.include?.includes('clickCount') ||
      ...
      query.select((knex: Knex.QueryBuilder) => {
        knex
          ...
          .where(
            'recommendation_click_events.recommendation_id',
            knex.client.raw('recommendations.id'),
          )
          .as('count__clicks');
      });
    }
    ...
    if (
      options.include?.includes('subscriberCount') ||
      ...
      query.select((knex: Knex.QueryBuilder) => {
        knex
          ...
          .where(
            'recommendation_subscribe_events.recommendation_id',
            knex.client.raw('recommendations.id'),
          )
