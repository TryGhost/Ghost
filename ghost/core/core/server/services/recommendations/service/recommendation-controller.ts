...
export class RecommendationController {
  ...
  #stringToOrder(str?: string) {
    ...
    for (const [index, part] of parts.entries()) {
      ...
      const directionData = new UnsafeData(trimmed.split(' ')[1]?.trim() ?? 'desc', {
  ...
  async browse(frame: Frame) {
    ...
    const include =
      options.optionalKey('withRelated')?.array.map((item) => {
        return RecommendationIncludesMap[
          item.enum(
    ...
    return {
      data: recommendations.map((entity) => {
        const d = {
          ...
          count:
            entity.clickCount !== undefined || entity.subscriberCount !== undefined
              ? {
