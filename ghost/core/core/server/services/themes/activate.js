...
module.exports.loadAndActivate = async (themeName, options = {}) => {
  ...
  try {
    ...
    if (!validate.canActivate(checkedTheme)) {
      logging.error(
        validate.getThemeValidationError('activeThemeHasFatalErrors', themeName, checkedTheme),
      );
    } else if (checkedTheme.results.error.length) {
      // CASE: inform that the theme has errors, but not fatal (theme still works)
      logging.warn(
        validate.getThemeValidationError('activeThemeHasErrors', themeName, checkedTheme),
      );
    }
