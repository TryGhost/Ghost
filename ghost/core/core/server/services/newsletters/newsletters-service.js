...
class NewslettersService {
  ...
  async edit(attrs, options) {
    ...
    const { cleanedAttrs, emailsToVerify } = await this.prepAttrsForEmailVerification(
      attrs,
      originalNewsletter,
    );
  ...
  async prepAttrsForEmailVerification(attrs, newsletter) {
    ...
    const emailProperties = [
      {
        ...
        type: 'from',
        emptyable: true,
        error: messages.senderEmailNotAllowed,
      },
    ];
    ...
    if (cleanedAttrs.sender_reply_to !== undefined) {
      if (!['newsletter', 'support'].includes(cleanedAttrs.sender_reply_to)) {
        emailProperties.push({
          ...
          type: 'replyTo',
          emptyable: false,
          error: messages.replyToNotAllowed,
        });
      }
    }
    ...
    const didChangeReplyTo =
      newsletter &&
      attrs.sender_reply_to !== undefined &&
      newsletter.get('sender_reply_to') !== attrs.sender_reply_to;
    const didChangeSenderEmail =
      newsletter &&
      attrs.sender_email !== undefined &&
      newsletter.get('sender_email') !== attrs.sender_email;
    if (didChangeReplyTo && !didChangeSenderEmail && newsletter.get('sender_email')) {
      const validated = this.emailAddressService.service.validate(
        newsletter.get('sender_email'),
        'from',
      );
      if (!validated.allowed) {
        logging.info(
          `Resetting sender_email for newsletter ${newsletter.id} because it became invalid`,
        );
