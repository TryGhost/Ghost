...
module.exports = class MailgunClient {
  ...
  async send(message, recipientData, replacements) {
    ...
    try {
      ...
      if (
        Object.keys(recipientData)[0] &&
        recipientData[Object.keys(recipientData)[0]].list_unsubscribe
      ) {
        messageData['h:List-Unsubscribe'] =
  ...
  async #fetchEventsFromDomain(
    ...
    try {
      ...
      while (events.length !== 0) {
        ...
        debug(
          `[MailgunClient fetchEventsFromDomain ${domain}]: starting fetching next page ${nextPageId}`,
        );
        ...
        debug(
          `[MailgunClient fetchEventsFromDomain ${domain}]: finished fetching next page with ${events.length} events`,
        );
      }
      ...
      error:
        ...
        typeof (event['delivery-status'].message || event['delivery-status'].description) ===
          ...
          ? {
              ...
              enhancedCode:
                event['delivery-status']['enhanced-code']?.toString()?.substring(0, 50) ?? null,
            }
          : null,
    };
  }
