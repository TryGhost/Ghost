...
class EmailEventProcessor {
  ...
  async getRecipient(emailIdentification, recipientCache) {
    ...
    // With the mailgun_message_id and email address we can look for the EmailRecipient
    const emailId =
      emailIdentification.emailId ?? (await this.getEmailId(emailIdentification.providerId));
    if (!emailId) {
      // Invalid
      return;
    }
  ...
  async batchGetRecipients(emailIdentifications) {
    ...
    const providerIds = [
      ...new Set(
        emailIdentifications.filter((e) => e.providerId && !e.emailId).map((e) => e.providerId),
      ),
    ];
    ...
    for (const identification of emailIdentifications) {
      const emailId =
        identification.emailId ?? this.providerIdEmailIdMap[identification.providerId];
    ...
    recipientQuery.where(function () {
      for (const lookup of lookups) {
        this.orWhere(function () {
          this.where('member_email', lookup.email).andWhere('email_id', lookup.emailId);
        });
      }
    });
