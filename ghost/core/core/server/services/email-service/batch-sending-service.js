...
class BatchSendingService {
   ...
   * Resolves once the queue has accepted the job, not once the email is sent. The
   * in-memory backend drops a dispatch after shutdown without rejecting, so a resolved
   * promise means the queue took the job, not that it will run.
   * @param {Email} email
