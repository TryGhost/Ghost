...
export class DomainWarmingService {
  ...
  constructor(dependencies: { models: { Email: EmailModel }; config: ConfigService }) {
  ...
  async #getDaysSinceFirstEmail(): Promise<number> {
    ...
    return Math.floor(
      (Date.now() - new Date(res.data[0].get('created_at') as string).getTime()) /
        (1000 * 60 * 60 * 24),
    );
  }
