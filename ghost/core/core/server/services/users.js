    ...
    async assignTagToUserPosts({ id, context, transacting }) {
        ...
        if (!createdTag) {
            ...
            const taggedPostIds = await this.models.Base.knex("posts_tags")
                ...
                .where("tag_id", tagId)
                .select("post_id");
            const taggedPostIdSet = new Set(
                taggedPostIds.map((post) => post.post_id),
            );
            usersPostIds = usersPostIds.filter(
                (postId) => !taggedPostIdSet.has(postId),
            );
        }
        ...
        await this.models.Post.addActions("edited", usersPostIds, {
            transacting,
            context,
        });
