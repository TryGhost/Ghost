...
// Bookshelf models are loosely typed; we only describe the methods this
// service actually uses. `any`/`unknown` here matches the wider codebase's
// pragmatic treatment of Ghost's untyped model layer.
interface Models {
    GiftLink: {
        findOne(data: Record<string, unknown>, options?: Record<string, unknown>): Promise<GiftLinkModel | null>;
        add(data: Record<string, unknown>, options?: Record<string, unknown>): Promise<GiftLinkModel>;
    };
    Post: {
        findOne(data: Record<string, unknown>, options?: Record<string, unknown>): Promise<GiftLinkModel | null>;
    };
    Base: {
        transaction<T>(operation: (transacting: unknown) => Promise<T>): Promise<T>;
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        knex: ((table: string) => any) & {raw(sql: string): unknown};
    };
}
...
class GiftLinksService {
     ...
     * @returns {Promise<T>}
     */
    async withTransaction<T>(options: Options, operation: (opts: Options) => Promise<T>): Promise<T> {
     ...
     */
    async getActive(postId: string, options: Options = {}): Promise<GiftLinkModel | null> {
     ...
     * @returns {Promise<object|null>}
     */
    async getActiveByToken(token: string, options: Options = {}): Promise<GiftLinkModel | null> {
        if (!token) {
            return null;
        }
        return this.models.GiftLink.findOne(
     ...
     */
    async ensure(postId: string, options: Options = {}): Promise<GiftLinkModel> {
     ...
     */
    async reset(postId: string, options: Options = {}): Promise<GiftLinkModel> {
        return this.withTransaction(options, async (opts) => {
            await this.models.Post.findOne({id: postId, status: 'all'}, {...opts, forUpdate: true, require: true});
     ...
     * @private
     */
    async #deactivateActive(where: Record<string, unknown>, opts: Options): Promise<number> {
