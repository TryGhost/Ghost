...
interface GiftLinksApi {
    getActiveByToken(token: string): Promise<GiftLinkModel | null>;
...
async function loadGiftLink(req: Request, res: Response, next: NextFunction) {
    ...
    const giftLinksService = require('./') as {api: GiftLinksApi};
 ...
 */
function countGiftRead(req: Request, res: Response, giftLink: GiftLink | null) {
    if (!giftLink) {
        return;
    }
    ...
    try {
        ...
        // `recordRead` runs — so the read would never be counted. Setting it on
