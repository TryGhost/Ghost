...
const messages = {
  offerNotFoundAfterDuplicateError:
    'Tried to create duplicate offer for the Stripe coupon {couponId}, but could not find offer in database',
};
...
class OffersAPI {
  ...
  async createOffer(data, options = {}) {
    return this.repository.createTransaction(async (transaction) => {
      ...
      if (offer.redemptionType.value === 'retention' && offer.status.value === 'active') {
        await this.archiveActiveRetentionOffers(offer.id, offer.cadence.value, saveOptions);
      }
  ...
  async updateOffer(data, options = {}) {
    return await this.repository.createTransaction(async (transaction) => {
      ...
      if (offer.redemptionType.value === 'retention' && offer.status.value === 'active') {
        await this.archiveActiveRetentionOffers(offer.id, offer.cadence.value, updateOptions);
      }
  ...
  async listOffersAvailableToSubscription({ subscriptionId, tierId, cadence, redemptionType }) {
    debug(
      `listOffersAvailableToSubscription: subscriptionId=${subscriptionId}, tierId=${tierId}, cadence=${cadence}, redemptionType=${redemptionType}`,
    );
    ...
    return await this.repository.createTransaction(async (transaction) => {
      ...
      let available = allOffers.filter(
        (offer) =>
          (offer.tier === null || offer.tier.id === tierId) && offer.cadence.value === cadence,
      );
      debug(`listOffersAvailableToSubscription: ${available.length} offers match tier and cadence`);

      if (available.length === 0) {
        const tierIds = [
          ...new Set(allOffers.filter((o) => o.tier !== null).map((o) => o.tier.id)),
        ];
        const cadences = [...new Set(allOffers.map((o) => o.cadence.value))];
        debug(
          `listOffersAvailableToSubscription: no offers match - available tiers: [${tierIds.join(', ')}], available cadences: [${cadences.join(', ')}]`,
        );
      ...
      if (redemptionType) {
        ...
        debug(
          `listOffersAvailableToSubscription: ${available.length} offers match redemption type (filtered ${beforeFilter - available.length})`,
        );
      }
      ...
      if (beforeTrialFilter > available.length) {
        debug(
          `listOffersAvailableToSubscription: filtered out ${beforeTrialFilter - available.length} trial offers`,
        );
      }
      ...
      if (redeemedOfferIds.length > 0) {
        debug(
          `listOffersAvailableToSubscription: filtered ${beforeRedeemedFilter - available.length} already-redeemed offers`,
        );
      }
  ...
  async getRedeemedOfferIdsForSubscriptions({ subscriptionIds }) {
    ...
    return await this.repository.createTransaction(async (transaction) => {
      return await this.repository.getRedeemedOfferIdsForSubscriptions(subscriptionIds, {
        transacting: transaction,
      });
    });
  }
