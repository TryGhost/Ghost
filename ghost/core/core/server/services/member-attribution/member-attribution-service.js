...
class MemberAttributionService {
  ...
  async getMemberCreatedAttribution(memberId) {
    ...
    const memberCreatedEvent = await this.models.MemberCreatedEvent.where('member_id', memberId)
  ...
  async getSubscriptionCreatedAttribution(subscriptionId) {
    ...
    // so order deterministically to avoid non-deterministic attribution resolution.
    const subscriptionCreatedEvent = await this.models.SubscriptionCreatedEvent.where(
      'subscription_id',
      subscriptionId,
    )
      .query((qb) => qb.orderBy('created_at', 'desc').orderBy('id', 'desc'))
