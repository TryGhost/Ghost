...
class TwitterOEmbedProvider {
  ...
  async canSupportRequest(url) {
    return (
      (url.host === 'twitter.com' || url.host === 'x.com') && TWITTER_PATH_REGEX.test(url.pathname)
    );
  }
  ...
  async getOEmbedData(url, externalRequest) {
    if (url.host === 'x.com') {
      // api is still at twitter.com... also not certain how people are getting x urls because twitter currently redirects every x host to twitter
