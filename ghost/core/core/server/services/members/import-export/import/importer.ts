...
export interface MembersRepository {
  ...
  linkStripeCustomer(
    data: { customer_id: string; member_id: string },
    options: object,
  ): Promise<void>;
...
class MembersCSVImporter {
  ...
  async importInline(
    ...
    return this.importRows(
      rows,
      buildImportLabelName(this._getTimezone()),
      request.extraLabels ?? [],
      verificationTrigger,
    );
  }
  ...
  private async writeRows(rows: MemberImportRow[], prepared: PreparedRun): Promise<WrittenRows> {
    ...
    const cloneGlobalLabels = (): Label[] =>
      prepared.globalLabels.map((label) => ({ ...label })).filter((label) => label.name);
    ...
    for (const row of rows) {
      ...
      try {
        ...
        const createdAt =
          row.created_at && moment(row.created_at).isAfter(moment())
            ? moment().toDate()
            : row.created_at;
        ...
        const existingMember = row.email
          ? await this._members.get(
              { email: row.email },
              { ...options, withRelated: ['labels', 'newsletters'] },
            )
          : null;
        let member;
        if (existingMember) {
          ...
          member = await this._members.create(
            memberValues,
            Object.assign({}, options, { context: CREATE_CONTEXT }),
          );
        }
        ...
        if (row.stripe_customer_id) {
          ...
          if (stripeCustomerId) {
            if (row.import_tier) {
              const { isNewStripePrice, stripePriceId } =
                await this._stripe.forceStripeSubscriptionToProduct(
                  {
            ...
            await this._members.linkStripeCustomer(
              { customer_id: stripeCustomerId, member_id: member.id },
              options,
            );
          }
        ...
        const reasons = errorList
          .map((e) =>
            typeof e === 'object' && e !== null && 'message' in e ? e.message : undefined,
          )
...
function wrapGiftError(error: unknown) {
  ...
  const message =
    (err && typeof err.message === 'string' && err.message) || tpl(messages.giftReassignFailed);
