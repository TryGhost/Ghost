...
module.exports = class EventRepository {
  ...
  async getSubscriptionEvents(options = {}, filter) {
    ...
    const data = models.map((model) => {
      ...
      const d = {
        ...
        attribution:
          model.get('type') === 'created' && subscriptionCreatedEvent && subscriptionCreatedEvent.id
            ? this._memberAttributionService.getEventAttribution(subscriptionCreatedEvent)
            : null,
        signup:
          model.get('type') === 'created' &&
          subscriptionCreatedEvent &&
          subscriptionCreatedEvent.id &&
          subscriptionCreatedEvent.related('memberCreatedEvent') &&
          subscriptionCreatedEvent.related('memberCreatedEvent').id
            ? true
            : false,
        tierName,
      };
  ...
  async getAutomatedEmailSentEvents(options = {}, filter) {
    ...
    const { automatedEmailLabels, automationActionRevisionLabels } =
  ...
  async getAutomatedEmailSentEventLabels(models) {
    const automatedEmailIds = [
      ...new Set(models.map((model) => model.get('automated_email_id')).filter(Boolean)),
    ];
    const automationActionRevisionIds = [
      ...new Set(models.map((model) => model.get('automation_action_revision_id')).filter(Boolean)),
    ];

    const [automatedEmailRows, automationActionRevisionRows] = await Promise.all([
      automatedEmailIds.length
        ? this._knex('welcome_email_automated_emails as email')
            .select(
              'email.id as id',
            ...
            .innerJoin(
              ...
              'automation.id',
              'email.welcome_email_automation_id',
            )
            .whereIn('email.id', automatedEmailIds)
        : [],
      automationActionRevisionIds.length
        ? this._knex('automation_action_revisions as revision')
            .select(
              'revision.id as id',
              'automation.slug as slug',
              'automation.name as name',
    ...
    return {
      ...
      automationActionRevisionLabels: new Map(
        automationActionRevisionRows.map((row) => [row.id, row]),
      ),
    };
  }
