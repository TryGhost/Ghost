...
module.exports = class RouterController {
  ...
  async _getSubscriptionCheckoutData(body) {
    ...
    if (!offerId && !tierId) {
      logging.error(
        '[RouterController._getSubscriptionCheckoutData] Expected offerId or tierId, received none',
      );
    ...
    if (offerId && tierId) {
      logging.error(
        '[RouterController._getSubscriptionCheckoutData] Expected offerId or tierId, received both',
      );
      throw new BadRequestError({
        message: tpl(messages.badRequest),
    ...
    if (tierId && !cadence) {
      logging.error(
        '[RouterController._getSubscriptionCheckoutData] Expected cadence to be "month" or "year", received ',
        cadence,
      );
      throw new BadRequestError({
        message: tpl(messages.badRequest),
  ...
  async _createSubscriptionCheckoutSession(options) {
    ...
    /** @type {SignupContext} */
    let ghostSignupContext =
      options.isAuthenticated && member
        ? SIGNUP_CONTEXTS.ALREADY_AUTHENTICATED
        : SIGNUP_CONTEXTS.NEEDS_MAGIC_LINK_EMAIL;
  ...
  async verifyOTC(req, res) {
    ...
    const redirectUrl = this._magicLinkService.getSigninURL(
      tokenValue,
      'signin',
      referrer,
      otcVerificationHash,
  ...
  async _createHashFromOTCAndToken(otc, token) {
    ...
    const hash = this._magicLinkService.tokenProvider.createOTCVerificationHash(
      otc,
      token,
      timestamp,
    );
  ...
  async _validateNewsletters(requestedNewsletters) {
    ...
    const requestedNewsletterNamesFilter = requestedNewsletterNames.map(
      (newsletter) => `'${newsletter.replace(/("|')/g, '\\$1')}'`,
    );
    ...
    if (matchedNewsletters.length !== requestedNewsletterNames.length) {
      ...
      const invalidNewsletterNames = requestedNewsletterNames.filter(
        (newsletter) => !validNewsletterNames.includes(newsletter),
      );
