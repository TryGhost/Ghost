...
(async () => {
  ...
  if (expiredCompedRows?.length) {
    ...
    deletedExpiredSubs = await db.knex('members_products').whereIn('id', rowIds).del();
    ...
    updatedMembers = await db.knex('members').whereIn('id', updateMemberIds).update({
      status: 'free',
      updated_at: now,
    });
  ...
  debug(
    `Removed ${deletedExpiredSubs} expired subscriptions, updated ${updatedMembers} members in ${cleanupEndDate.valueOf() - cleanupStartDate.valueOf()}ms`,
  );

  if (parentPort) {
    parentPort.postMessage(
      `completed in ${cleanupEndDate.valueOf() - cleanupStartDate.valueOf()}ms: removed ${deletedExpiredSubs} expired subscriptions and updated ${updatedMembers} members`,
    );
