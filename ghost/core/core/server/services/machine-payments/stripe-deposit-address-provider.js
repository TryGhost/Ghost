...
class StripeDepositAddressProvider {
    ...
    async getAddress({amount, currency, network, paymentHeader, request}) {
        ...
        return await this.#createAddress({amount, currency, network, stripe});
    }
    ...
    async #createAddress({amount, currency, network, stripe}) {
        ...
        this.cache.set(this.#getAddressCacheKey({address, cacheNamespace: this.stripeCacheNamespace}), true);
        return address;
    }
