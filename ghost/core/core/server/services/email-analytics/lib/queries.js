...
/** @typedef {'email-analytics-latest-opened'|'email-analytics-latest-others'|'email-analytics-missing'|'email-analytics-scheduled'|'email-analytics-automation-latest-opened'|'email-analytics-automation-latest-others'|'email-analytics-automation-missing'|'email-analytics-automation-scheduled'} EmailAnalyticsJobName */
...
module.exports = {
     ...
     * @param {EmailAnalyticsEventSource} eventSource - Recipient table and timestamp columns used to seed a new job.
     * @returns {Promise<Date|null>} The timestamp of the last seen event, or null if no events are found.
     */
    async getLastEventTimestamp(jobName, events, eventSource) {
        ...
        if (lastJobRunTimestamp) {
            ...
            debug(`Job data not found for ${jobName}, using ${eventSource.tableName} data`);
            logging.info(`Job data not found for ${jobName}, using ${eventSource.tableName} data`);

            for (const event of events) {
                const columnName = eventSource.eventColumns[event];
                if (!columnName) {
                    continue;
                }

                const row = await db.knex(eventSource.tableName)
