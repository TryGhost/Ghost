...
class CommentsServiceEmails {
    ...
    async notifyPostAuthors(comment) {
        ...
        for (const author of post.related('authors')) {
            ...
            const {html, text} = await this.commentsServiceEmailRenderer.renderEmailTemplate('new-comment', templateData);
    ...
    async notifyParentCommentAuthor(reply, {type = 'parent'} = {}) {
        ...
        const {html, text} = await this.commentsServiceEmailRenderer.renderEmailTemplate('new-comment-reply', templateData);
