...
module.exports = class CommentsController {
  ...
  async adminBrowseAll(frame) {
    ...
    const { filter, reportCount, mongoTransformer } = this.#extractReportCountFilter(
  ...
  async adminEdit(frame) {
    ...
    const result = await this.service.moderateComment(frame.options.id, updates, frame.options);
