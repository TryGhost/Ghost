...
const COMMENT_STATUSES_REPLY_PARENT = [
...
class CommentsService {
  ...
  async #fetchCommentByID(
    id,
    options = {},
    { requiredColumns = [], statuses = [], forUpdate = false } = {},
  ) {
  ...
  async #getPublishedCommentForAction(
    id,
    options = {},
    requiredColumns = [],
    { forUpdate = false } = {},
  ...
  async #getMemberCommentVotes({ commentId, memberId, score }, options) {
    ...
    collection.query((qb) => {
      qb.where('comment_likes.comment_id', commentId).where('comment_likes.member_id', memberId);
  ...
  async #setCommentVote(commentId, member, targetScore, alreadyMessage, options = {}) {
    ...
    return await this.#withTransaction(options, async (transactionOptions) => {
      ...
      const alreadyHasSingleTargetVote =
        votes.length === 1 && Number(votes[0].get('score')) === targetScore;
  ...
  async likeComment(commentId, member, options = {}) {
    ...
    return await this.#setCommentVote(
      commentId,
      member,
      COMMENT_LIKE_SCORE,
      messages.alreadyLiked,
      options,
    );
  }
  ...
  async unlikeComment(commentId, member, options = {}) {
    ...
    await this.#clearCommentVote(
      commentId,
      member,
      COMMENT_LIKE_SCORE,
      messages.likeNotFound,
      options,
    );
  }
  ...
  async dislikeComment(commentId, member, options = {}) {
    ...
    return await this.#setCommentVote(
      commentId,
      member,
      COMMENT_DISLIKE_SCORE,
      messages.alreadyDisliked,
      options,
    );
  }
  ...
  async getAdminAllComments({
    ...
    const postUrlRelations = this.urlService
      .getRequiredRelations()
      .map((relation) => `post.${relation}`);
    const withRelated = [
      'member',
      'post',
      ...postUrlRelations,
  ...
  async moderateComment(id, data, options) {
    ...
    if (Object.prototype.hasOwnProperty.call(data, 'pinned')) {
      ...
      if (data.pinned) {
        ...
        if (
          existingComment.get('status') === COMMENT_STATUS_DELETED ||
  ...
  async deleteComment(id, member, options) {
    this.checkEnabled();
    const existingComment = await this.#getPublishedCommentForAction(
      id,
      options,
      OWNERSHIP_REQUIRED_COLUMNS,
    );
