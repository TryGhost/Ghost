...
class LastSeenAtUpdater {
   ...
   */
  constructor({ services: { settingsCache }, getMembersApi, db, events, lastSeenAtCache, config }) {
    if (!getMembersApi) {
  ...
  subscribe(domainEvents) {
    domainEvents.subscribe(MemberPageViewEvent, async (event) => {
      try {
        await this.cachedUpdateLastSeenAt(
          event.data.memberId,
    ...
    const shouldUpdateForClickTracking =
      !this._config || this._config.get('backgroundJobs:clickTrackingLastSeenAtUpdater') !== false;
  ...
  async updateLastSeenAtWithoutKnownLastSeen(memberId, timestamp) {
    ...
    await this._db
      ...
      .where('id', '=', memberId)
      .andWhere((builder) =>
        builder.where('last_seen_at', '<', startOfDayInSiteTimezone).orWhereNull('last_seen_at'),
      )
      .update({
  ...
  async updateLastSeenAt(memberId, memberLastSeenAt, timestamp) {
    ...
    if (
      ...
      moment(moment.utc(timestamp).tz(timezone).startOf('day')).isAfter(memberLastSeenAt)
    ) {
      try {
        ...
        await this._db.knex.transaction(async (trx) => {
          ...
          const currentMember = await membersApi.members.get(
            { id: memberId },
            { require: true, transacting: trx, forUpdate: true },
          );
          const currentMemberLastSeenAt = currentMember.get('last_seen_at');
          if (
            currentMemberLastSeenAt === null ||
            moment(moment.utc(timestamp).tz(timezone).startOf('day')).isAfter(
              currentMemberLastSeenAt,
            )
          ) {
            const memberToUpdate = await currentMember.refresh({
              transacting: trx,
              forUpdate: false,
            ...
            const updatedMember = await memberToUpdate.save(
              ...
              { transacting: trx, patch: true, method: 'update' },
            );
  ...
  async updateLastCommentedAt(memberId, timestamp) {
    ...
    if (
      ...
      moment(moment.utc(timestamp).tz(timezone).startOf('day')).isAfter(memberLastCommentedAt)
    ) {
