...
export class ActivityPubService {
  ...
  async checkWebhookState(expectedWebhooks: ExpectedWebhook[], integration: { id: string }) {
    ...
    for (const expectedWebhook of expectedWebhooks) {
      const foundWebhook = webhooks.find((webhook) => {
        return (
          webhook.event === expectedWebhook.event &&
          webhook.target_url === expectedWebhook.target_url.href &&
          webhook.secret === expectedWebhook.secret
        );
      });
      if (!foundWebhook) {
        this.logging.error(
          `Could not find webhook for ${expectedWebhook.event} ${expectedWebhook.target_url}`,
        );
        return false;
      }
    }
  ...
  async removeWebhooks() {
    ...
    await this.knex.del().from('webhooks').where('integration_id', '=', integration.id);
  }
  ...
  async initialiseWebhooks() {
    ...
    await this.knex.insert(webhooksToInsert).into('webhooks');
  }
