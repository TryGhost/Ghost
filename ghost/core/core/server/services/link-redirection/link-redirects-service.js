...
class LinkRedirectsService {
  ...
  async getOrAddAutomationRedirect(automationActionRevisionId, to) {
    const existing = await this.#linkRedirectRepository.getByAutomationActionRevisionAndURL(
      automationActionRevisionId,
      to,
    );
    if (existing) {
    ...
    try {
      ...
      const winner = await this.#linkRedirectRepository.getByAutomationActionRevisionAndURL(
        automationActionRevisionId,
        to,
      );
      if (winner) {
