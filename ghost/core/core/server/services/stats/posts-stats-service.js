...
class PostsStatsService {
    ...
    try {
      ...
      const results = await this.knex
        ...
        .leftJoin('posts as p', function () {
          this.on('url_metadata.attribution_id', 'p.id').andOnIn('url_metadata.attribution_type', [
            'post',
            'page',
          ]);
        })
        .whereRaw(
          '(COALESCE(free_attr.free_members, 0) > 0 OR COALESCE(paid_attr.paid_members, 0) > 0 OR COALESCE(mrr_attr.mrr, 0) > 0)',
        )
    ...
    return Promise.all(
      results.map(async (row) => {
        ...
        return {
          ...
          mrr: row.mrr,
          post_type:
            row.attribution_type === 'post'
              ? 'post'
              : row.attribution_type === 'page'
                ? 'page'
                : null,
  ...
  async getReferrersForPost(postId, options = {}) {
    try {
      ...
      const allSources = this.knex
        .select('source', 'referrer_url')
        .from(membersCreatedSources.as('sources1'))
        .union(function () {
          this.select('source', 'referrer_url').from(membersSubscriptionSources.as('sources2'));
        });

      // Create the final CTE that we'll use to get all referrers
      const allReferrersCTE = this.knex
        .select('source')
      ...
      const results = await query.orderBy(orderField, orderDirection).limit(limit);
      ...
      const [freeMembers, paidMembers, mrr] = await Promise.all([
        this.knex('members_created_events as mce')
          ...
          .leftJoin('members_subscription_created_events as msce', function () {
            this.on('mce.member_id', '=', 'msce.member_id').andOn(
              'mce.attribution_id',
              '=',
              'msce.attribution_id',
            );
  ...
  _buildFreeMembersSubquery(options, groupByUrl = false) {
    ...
    let subquery = knex('members_created_events as mce')
      ...
      .leftJoin('members_subscription_created_events as msce', function () {
        this.on('mce.member_id', '=', 'msce.member_id').andOn(
          joinCondition,
          '=',
          groupByUrl ? 'msce.attribution_url' : 'msce.attribution_id',
        );
        ...
        if (options.post_type === 'page') {
          ...
          this.andOn(function () {
            this.on('msce.attribution_type', '=', knex.raw('?', ['post'])).orOn(
              'msce.attribution_type',
              '=',
              knex.raw('?', ['page']),
            );
          });
    ...
    if (!groupByUrl) {
      ...
      if (options.post_type === 'page') {
        ...
        subquery = subquery.whereIn('mce.attribution_type', [
          'post',
          'page',
          'url',
          'tag',
  ...
  _buildPaidMembersSubquery(options, groupByUrl = false) {
    ...
    if (!groupByUrl) {
      ...
      if (options.post_type === 'page') {
        ...
        // Include all types that can drive conversions
        subquery = subquery.whereIn('msce.attribution_type', [
          'post',
          'page',
          'url',
          'tag',
  ...
  async getTopPostsViews(options) {
    try {
      ...
      const postUuids = [
        ...new Set(viewsData.filter((row) => row.post_uuid).map((row) => row.post_uuid)),
      ];
      ...
      if (viewsData.length > 0) {
        ...
        const posts = await this.knex('posts as p')
          ...
          .orderByRaw('CASE WHEN p.uuid IN (?) THEN 0 ELSE 1 END', [
            postUuids.length > 0 ? postUuids : ['none'],
          ])
        ...
        const authorsData =
          postIds.length > 0
            ? await this.knex('posts_authors as pa')
        ...
        const [memberAttributionCounts, clickCounts] = await Promise.all([
          this._getMemberAttributionCounts(
        ...
        postsWithViews = viewsData
          .map((row) => {
            ...
            const attributionCount = memberAttributionCounts.find(
              (ac) => ac.post_id === post.post_id,
            );
            const memberCount = attributionCount
              ? attributionCount.free_members + attributionCount.paid_members
              : 0;
            ...
            return {
              ...
              feature_image: post.feature_image
                ? urlUtils.transformReadyToAbsolute(post.feature_image)
                : post.feature_image,
              status: post.status,
      ...
      if (remainingCount > 0) {
        ...
        if (additionalPosts.length > 0) {
          [additionalMemberAttributionCounts, additionalClickCounts] = await Promise.all([
            this._getMemberAttributionCounts(
              additionalPosts.map((p) => p.post_id),
              options,
            ),
      ...
      const additionalPostsWithZeroViews = additionalPosts.map((post) => {
        // Find the member attribution count for this post
        const attributionCount = additionalMemberAttributionCounts.find(
          (ac) => ac.post_id === post.post_id,
        );
        const memberCount = attributionCount
          ? attributionCount.free_members + attributionCount.paid_members
  ...
  async _getMemberAttributionCounts(postIds, options = {}) {
    ...
    try {
      ...
      const [freeResults, paidResults] = await Promise.all([freeMembersQuery, paidMembersQuery]);
