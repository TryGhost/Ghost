...
function frontendTemplate(node, document, options) {
    ...
    // Build embed URL with member UUID
    const baseUrl = `https://partner.transistor.fm/ghost/embed/${memberUuid}`;
    const params = new URLSearchParams();

    if (node.accentColor) {
        params.set('color', node.accentColor.replace(/^#/, ''));
    }
    if (node.backgroundColor) {
        params.set('background', node.backgroundColor.replace(/^#/, ''));
    }
