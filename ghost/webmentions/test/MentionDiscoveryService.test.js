...
describe('MentionDiscoveryService', function () {
    ...
    it('Returns null from a bad URL', async function () {
        let url = new URL('http://www.fake.com/');
        let scope = nock(url.href)
            .get('/')
            .reply(404);
    ...
    describe('Can parse headers', function () {
        ...
        it('Returns null for a valid non-html site', async function () {
            let url = new URL('http://www.veryrealsite.com');
            let mockResponse = nock(url.href)
                .get('/')
        ...
        it('Returns an endpoint from a site with a webmentions Link in the header', async function () {
            ...
            let url = new URL('http://testpage.com/');
            let mockResponse = nock(url.href)
                .get('/')
        ...
        it('Returns null with multiple Links in the header, one of which is for webmentions', async function () {
