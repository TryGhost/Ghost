...
const messages = {
    ...
    forceNoCustomer: 'Cannot find Stripe customer to force subscription',
    forceNoExistingSubscription: 'Cannot force subscription when customer does not have an existing subscription',
    forceTooManySubscriptions: 'Cannot force subscription when customer has multiple subscriptions',
    forceTooManySubscriptionItems: 'Cannot force subscription when existing subscription has multiple items',
    forceExistingSubscriptionNotRecurring: 'Cannot force subscription when existing subscription is not recurring'
};
...
module.exports = class MemberRepository {
    ...
    async update(data, options) {
        ...
        if (!initialMember) {
            throw new errors.NotFoundError({message: tpl(messages.memberNotFound, {id: options.id})});
        }
     ...
     * This will either:
     *
     * Create a new price on the Stripe product that is associated with the Ghost product, and update
     ...
     *
     * This method should be used in-conjunction with the `linkSubscription` method to ensure that the changes
     * made in Stripe are reflected in Ghost - This is not executed as part of this to allow for flexibility
     *
     * @param {Object} data
    ...
    async forceStripeSubscriptionToProduct(data, options) {
        ...
        if (!stripeCustomer) {
            throw new errors.BadRequestError({message: tpl(messages.forceNoCustomer)});
        }
        ...
        if (stripeCustomer.subscriptions.data.length === 0) {
            throw new errors.BadRequestError({message: tpl(messages.forceNoExistingSubscription)});
        }
        ...
        if (stripeCustomer.subscriptions.data.length > 1) {
            throw new errors.BadRequestError({message: tpl(messages.forceTooManySubscriptions)});
        }
        ...
        if (stripeSubscription.items.data.length > 1) {
            throw new errors.BadRequestError({message: tpl(messages.forceTooManySubscriptionItems)});
        }
        ...
        if (!stripeSubscriptionItemPriceInterval) {
            throw new errors.BadRequestError({message: tpl(messages.forceExistingSubscriptionNotRecurring)});
        }
        ...
        if (!ghostProduct) {
            throw new errors.NotFoundError({message: tpl(messages.productNotFound, {id: data.product_id})});
        }
