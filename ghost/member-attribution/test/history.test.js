...
describe('UrlHistory', function () {
    ...
    describe('getAttribution', function () {
        it('Returns empty if empty history', function () {
            const history = new UrlHistory([]);
            should(history.getAttribution()).eql({id: null, type: null, url: null});
        });

        it('Returns last url for now', function () {
            const history = new UrlHistory([{path: '/test', time: 123}]);
            should(history.getAttribution()).eql({type: 'url', id: null, url: '/test'});
        });
    });
});
