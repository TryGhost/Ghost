...
check_sum() {
    ...
    # Determine if the file has a 'hits' or 'pageviews' column
    if head -n1 "$file" | grep -q 'hits'; then
        column_name="hits"
    elif head -n1 "$file" | grep -q 'pageviews'; then
        column_name="pageviews"
    else
        echo "No 'hits' or 'pageviews' column found in $file"
