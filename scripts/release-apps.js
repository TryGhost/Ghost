...
async function ensureEnabledApp() {
  if (!app) {
    console.error(
      `${APP_NAME} is not a public app — add it to scripts/public-apps.json to release it here`,
    );
    process.exit(1);
  }
}
...
async function getNewVersion() {
  ...
  if (!['minor', 'major'].includes(bumpType)) {
    console.error(
      `Unknown bump type ${bumpTypeInput} - expected "minor" or "major" (patch releases are automated)`,
    );
    process.exit(1);
  }
...
async function main() {
  ...
  console.log(
    `Release commit created - please double check it and use "git commit --amend" to make any changes before opening a PR to merge into main`,
  );
}
