...
async function createRepository() {
    ...
    await writeFile(path.join(rootDirectory, '.agents', 'skills', 'example', 'SKILL.md'), '---\nname: example\n---\n');
...
test('accepts a correctly linked skill', async () => {
    ...
    await symlink('../../.agents/skills/example', path.join(rootDirectory, '.claude', 'skills', 'example'));
...
test('rejects a copied skill directory', async () => {
    ...
    assert.equal((await checkAgentSkillLinks(rootDirectory))[0], 'Claude skill entry is not a symlink: example');
});
