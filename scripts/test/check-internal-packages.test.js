...
async function createCompliantPackage(rootDirectory, directory = 'packages/example', manifest = compliantManifest(directory)) {
...
test('accepts documented migration and exempt states without applying the golden path', async () => {
    ...
    await writeJson(path.join(rootDirectory, 'packages', 'migrating', 'package.json'), {
        ...
        private: true,
        ghostPackage: {goldenPath: 'migration', reason: 'Imported unchanged before a separate modernization PR.'}
    });
