...
function main() {
  ...
  const lastPublishedSummary = lastPublishedMinor
    ? `, last published: ${lastPublishedMinor.tag}`
    : '';
  console.log(
    `Migration integrity checks passed (package version: ${safeVersion.raw}, safe: ${safeVersion.safe}${lastPublishedSummary}).`,
  );
}
