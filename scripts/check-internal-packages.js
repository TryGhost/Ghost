...
async function listWorkspacePackages(rootDirectory) {
  const { stdout } = await execFileAsync('pnpm', ['m', 'ls', '--depth', '-1', '--json'], {
    cwd: rootDirectory,
...
function usesStandardConfigFactory(config, moduleName, factoryName) {
  const importPattern = new RegExp(
    `^\\s*import\\s*\\{\\s*${factoryName}\\s*\\}\\s*from\\s*['"]${moduleName}['"];?\\s*$`,
    'm',
  );
...
async function validateConfigFiles(packageDirectory, manifestPath, errors) {
  ...
  if (await exists(tsconfigPath)) {
    try {
      ...
      addExactValueError(
        ...
        manifestPath,
        config.extends,
        '@internal/cfg-typescript/esm.json',
  ...
  if (await exists(testTsconfigPath)) {
    try {
      ...
      addExactValueError(
        ...
        config.compilerOptions?.rootDir,
        '..',
        'test/tsconfig.json compilerOptions.rootDir',
      );
      addExactValueError(
        errors,
        manifestPath,
        ...
        true,
        'test/tsconfig.json compilerOptions.noEmit',
      );
      if (!sameArray(config.include, ['../src/**/*', '**/*'])) {
  ...
  if (await exists(eslintPath)) {
    ...
    if (!usesStandardConfigFactory(config, '@internal/cfg-eslint', 'nodeLibConfig')) {
      errors.push(
        `${manifestPath}: eslint.config.mjs must use nodeLibConfig from @internal/cfg-eslint`,
      );
    }
  }
  ...
  if (await exists(vitestPath)) {
    ...
    if (!usesStandardConfigFactory(config, '@internal/cfg-vitest', 'createVitestConfig')) {
      errors.push(
        `${manifestPath}: vitest.config.ts must use createVitestConfig from @internal/cfg-vitest`,
      );
    }
  }
}

async function validateCompliantPackage({
  rootDirectory,
  packageDirectory,
  manifest,
  workspaceNames,
  packagePath: explicitPackagePath,
}) {
  const manifestPath = path.relative(rootDirectory, path.join(packageDirectory, 'package.json'));
  const packagePath =
    explicitPackagePath ?? path.relative(rootDirectory, packageDirectory).split(path.sep).join('/');
  const errors = [];

  if (
    typeof manifest.name !== 'string' ||
    !manifest.name.startsWith('@tryghost/') ||
    !isValidPackageName(manifest.name.slice('@tryghost/'.length))
  ) {
  ...
  if (!exports || typeof exports !== 'object' || Array.isArray(exports)) {
    ...
    for (const [entryPoint, conditions] of Object.entries(exports)) {
      ...
      if (!sameArray(Object.keys(conditions), ['source', 'types', 'default'])) {
        errors.push(
          `${manifestPath}: exports.${entryPoint} conditions must be source, types, default in that order`,
        );
        continue;
      }
      ...
      const sourceMatch =
        typeof conditions.source === 'string' && /^\.\/src\/(.+)\.ts$/.exec(conditions.source);
      ...
      addExactValueError(
        ...
        manifestPath,
        conditions.types,
        `./build/${sourceStem}.d.ts`,
        `exports.${entryPoint}.types`,
      );
      addExactValueError(
        errors,
        manifestPath,
        conditions.default,
        `./build/${sourceStem}.js`,
        `exports.${entryPoint}.default`,
      );
      if (!(await exists(path.join(packageDirectory, conditions.source)))) {
        errors.push(
          `${manifestPath}: exports.${entryPoint}.source does not exist (${conditions.source})`,
        );
      }
    }
  ...
  for (const javascriptFile of await findJavaScriptFiles(path.join(packageDirectory, 'src'))) {
    errors.push(
      `${manifestPath}: authored source must be TypeScript (${path.relative(packageDirectory, javascriptFile)})`,
    );
  }
  for (const javascriptFile of await findJavaScriptFiles(path.join(packageDirectory, 'test'))) {
    errors.push(
      `${manifestPath}: authored tests must be TypeScript (${path.relative(packageDirectory, javascriptFile)})`,
    );
  }
...
export async function checkInternalPackages(rootDirectory) {
  ...
  for (const pkg of packages) {
    const manifestPath = path.relative(
      rootDirectory,
      path.join(pkg.packageDirectory, 'package.json'),
    );
    ...
    if (pkg.manifest.private !== true) {
      if (status !== undefined) {
        errors.push(
          `${manifestPath}: ghostPackage.goldenPath is only valid for private internal packages`,
        );
      }
      continue;
    }
    ...
    if (!GOLDEN_PATH_STATUSES.has(status)) {
      errors.push(
        `${manifestPath}: ghostPackage.goldenPath must be one of compliant, migration, exempt`,
      );
      continue;
    }
    ...
    if (status === 'migration' || status === 'exempt') {
      if (
        typeof pkg.manifest.ghostPackage.reason !== 'string' ||
        pkg.manifest.ghostPackage.reason.trim().length === 0
      ) {
        errors.push(
          `${manifestPath}: ghostPackage.reason is required when goldenPath is ${status}`,
        );
      }
      continue;
    }
    ...
    errors.push(
      ...(await validateCompliantPackage({
        rootDirectory,
  ...
  try {
    ...
    templateManifest = JSON.parse(
      applyPackageTemplateTokens(templateManifestSource, {
        name: 'template',
