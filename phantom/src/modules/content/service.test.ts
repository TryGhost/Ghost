...
const createRepository = (): ContentRepository => {
    ...
    return {
        ...
        getPostByUuid: async (uuid) => posts.find((post) => post.uuid === uuid || post.id === uuid) ?? null,
        ...
        getTagById: async (id) => tags.find((tag) => tag.id === id) ?? null,
        updateTag: async (tag) => tag as TagRecord,
        deleteTag: async () => undefined,
        listTags: async () => tags,
        countPostsPerTag: async () => new Map(),
        linkTagToPost: async () => undefined,
        linkAuthorToPost: async () => undefined,
        getAuthorProfileById: async () => null,
