// Vendored from /e2e/tests/admin/onboarding.test.ts (per-test isolation
// comes from the reset fixture)
import {AnalyticsOverviewPage, OnboardingPage} from '../../helpers/onboarding-pages';
...
test.describe('Ghost Admin - Onboarding Checklist', () => {
    test('firstStart root redirect lands on onboarding', async ({page}) => {
        ...
        // Adapted: the shell occasionally races its user-state cache on the
        // firstStart boot and bounces to analytics; retry the navigation.
        const onboardingPage = new OnboardingPage(page);
        await expect(async () => {
            await page.goto('/ghost/#/?firstStart=true&sr_id=test-site&ssts=1778061878&ssml=test-signature');
            await expect(onboardingPage.checklist).toBeVisible({timeout: 5000});
