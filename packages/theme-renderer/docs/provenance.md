...
| helpers/comments.ts | helpers/comments.js @ 6be2dc0712 | imports→seam; seam guard added: returns undefined when `config comments:url` is absent (core always has it; the seam only learns it from an instance scrape of a page carrying the tag — deltas.md row 13) | getFrontendKey, settingsCache, config (comments:url via instance scrape) |
...
| handlebars-env.ts | fresh | The package's `services/handlebars.js`: mutable handlebars environment (`setHandlebarsInstance` lets the assembly share the engine's instance; SafeString/Utils/createFrame are cross-environment statics). `registerPartial` compiles string sources through the engine-bound compiler (`setHandlebarsInstance`'s `compilePartial` → `TemplateEngine.compile` → `onCompile`, so partials share the single compile path incl. marker injection); the bare-instance fallback (`handlebars.compile`, `preventIndent: true`, like express-hbs) exists only for unit-test setups without an engine. themeI18n/themeI18next delegate to injected ports with truthy `_strings`/`_i18n` so t.js skips lazy init |
