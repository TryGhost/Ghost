...
| # (slice 1) | Delta                                                                                 | Fix                                                                                                                                                                                                                                                                                                                                                                                                                           |
| ----------- | ------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1           | Asset URLs carried `?v=themerender` instead of the live per-boot hash                 | Inject via `createRenderer({config: {assetHash}})` — upstream-faithful (ghost/core's `getGlobalAssetHash` also lets a configured `assetHash` win). The preview scraper also captures current per-file `?v=` values as `config.assetHashes`, keyed by URL pathname, so content-based hashes remain distinct. Assets absent from the inspected page retain the global fallback. Without injection the constant default remains. |
