...
- ~~**Second compile path bypasses onCompile**~~ — done in slice 3: `setHandlebarsInstance(instance, compilePartial?)` lets the assembly bind the engine's compile, and `createRenderer` does (`bindSeamToEngine`), so core-helper partials compile through the same onCompile path as templates/layouts/theme partials (they carry no filename → no data-edit markers, by design — docs/markers.md). The bare-instance fallback in `hbs.registerPartial` remains only for unit-test setups without an engine, with identical compile options.
