...
export function createUrlUtils(options: CreateUrlUtilsOptions): UrlUtilsPort {
  ...
  function urlFor(context: any, data?: any, absolute?: boolean): string {
    ...
    if (
      _.isObject(context) &&
      !_.isArray(context) &&
      'relativeUrl' in context &&
      typeof (context as any).relativeUrl === 'string'
    ) {
      ...
      if (
        ...
        'nav' in data &&
        data.nav &&
        typeof data.nav === 'object' &&
        !_.isArray(data.nav) &&
        'url' in data.nav &&
        typeof data.nav.url === 'string'
      ) {
      ...
      if (
        ...
        ['admin', 'content'].includes(data.type)
      ) {
        apiPath += data.type;
      } else {
