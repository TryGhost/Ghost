...
import {openTagEnd, openTagNameAt, scanSourceTags, type OpenTagEnd} from '../engine/source-scanner.ts';
...
export function offsetAt(source: string, lineStarts: number[], position: SourcePosition): number | null {
 ...
 */
export function resolveAnchoredOffset(source: string, lineStarts: number[], position: SourcePosition, anchor: EditAnchor, offset: number | null): number {
 ...
 */
export function resolveMarkedOpenTag(source: string, position: SourcePosition, anchor?: EditAnchor): ResolvedOpenTag {
    ...
    if (!tagEnd.closed) {
        throw new errors.IncorrectUsageError({message: `unterminated open tag <${tagName}> at ${at}`});
    }
 ...
 */
export function editThemeFile<T extends ThemeFiles>(theme: T, marker: EditMarker, editFile: (source: string) => string): T {
