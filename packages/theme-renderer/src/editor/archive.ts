 ...
 * theme-editor-utils.ts, where it was developed and tested against real theme
 * archives. apps/admin now re-exports these from here — this file is the
 * single source of truth for the extract/pack behavior and its limits.
 *
 * Exported from the `@tryghost/theme-renderer/editor/archive` subpath: it is
 * an editor-side tool (Admin API theme zips), not part of the render contract.
 * Web-platform APIs only (ArrayBuffer, Uint8Array, TextEncoder, Blob) — safe
 * in browsers, workers, and Node ≥ 18.
 */
/* eslint-disable ghost/ghost-custom/ghost-error-usage --
   ThemeArchiveExtractionError is a plain browser Error subclass (thrown into
   UI code that switches on `.reason`), not a @tryghost/errors GhostError —
   kept as-is from the origin file so both consumers keep their catch sites. */
...
export const normaliseRelativePath = (input: string): string | null => {
...
export const detectCommonRoot = (paths: string[]): string => {
  if (paths.length === 0) {
    return '';
  }
...
export const cloneThemeFiles = (
  files: Record<string, ThemeEditorFile>,
): Record<string, ThemeEditorFile> => {
  ...
  for (const [zipPath, entry] of entries) {
    ...
    if (editable) {
      try {
        ...
        extractedBytes = trackExtractedBytes(
          extractedBytes,
...
export const packThemeArchive = async ({
  files,
  rootPrefix,
}: ThemeEditorSnapshot): Promise<Blob> => {
  const zip = new JSZip();
