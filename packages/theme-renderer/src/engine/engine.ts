...
import {
  done as resolverDone,
  hasResolvers,
  resolve as resolverResolve,
  TOKEN_PATTERN,
  unescapeToken,
  type ResolverCache,
} from './async-resolver.ts';
...
export class TemplateEngine {
  ...
  constructor(resolver: TemplateResolver, options: TemplateEngineOptions = {}) {
    ...
    this.handlebars.registerHelper(
      options.blockHelperName ?? 'block',
      function (this: unknown, name: string, helperOptions: Handlebars.HelperOptions) {
        let val: string | string[] | undefined = (helperOptions.data.root.blockCache as BlockCache)[
          name
        ];
    ...
    this.handlebars.registerHelper(
      options.contentHelperName ?? 'contentFor',
      function (this: unknown, name: string, helperOptions: Handlebars.HelperOptions) {
  ...
  registerAsyncHelper(
    name: string,
    fn: (this: unknown, context: unknown, cb: AsyncHelperCallback) => unknown,
  ): void;
  registerAsyncHelper(
    name: string,
    fn: (this: unknown, context: unknown, options: unknown, cb: AsyncHelperCallback) => unknown,
  ): void;
    ...
    this.handlebars.registerHelper(
      ...
      function (this: unknown, context?: unknown, options?: Handlebars.HelperOptions) {
        const getRootCache = (value: unknown): ResolverCache | undefined => {
          return (value as { data?: { root?: { resolverCache?: ResolverCache } } } | undefined)
            ?.data?.root?.resolverCache;
        };
        const resolverCache =
          (this as { resolverCache?: ResolverCache } | null)?.resolverCache ||
          getRootCache(context) ||
          getRootCache(options);
        if (!resolverCache) {
          throw new errors.IncorrectUsageError({
            message: `Could not find resolver cache in async helper ${name}.`,
          });
        }
        if (options && fn.length > 2) {
          const resolveFunc = (arr: unknown, cb: AsyncHelperCallback): unknown => {
            ...
            return (
              fn as (this: unknown, c: unknown, o: unknown, cb2: AsyncHelperCallback) => unknown
            ).call(this, ctx, opts, cb);
          };
          return resolverResolve(resolverCache, resolveFunc, [context, options]);
        }
        return resolverResolve(
          resolverCache,
          (fn as (this: unknown, c: unknown, cb2: AsyncHelperCallback) => unknown).bind(this),
          context,
        );
      },
    );
  ...
  compile(source: string, filename?: string): CompiledTemplate {
    ...
    if (typeof source !== 'string') {
      throw new errors.IncorrectUsageError({
        message: 'registerPartial must be a string for empty comment workaround',
      });
    }
  ...
  async render(
    ...
    const declaredLayoutTemplates = declaredLayout
      ? this.cacheLayout(declaredLayout, this.useCache)
      : null;
    ...
    if (typeof options.layout !== 'undefined' && !options.layout) {
      ...
      layoutTemplates = this.cacheLayout(
        this.layoutPath(templateName, options.layout),
        this.useCache,
      );
    } else {
  ...
  private renderWithLayouts(
    template: CompiledTemplate,
    locals: RenderContext,
    layoutTemplates: CompiledTemplate[] | null,
  ): string {
  ...
  updateLocalTemplateOptions(
    locals: RenderContext,
    localTemplateOptions: TemplateOptions | undefined,
  ): void {
  ...
  private async handleAsync(resolverCache: ResolverCache, html: string): Promise<string> {
    ...
    for (;;) {
      ...
      if (replaced === res) {
        ...
        throw new errors.IncorrectUsageError({
          message: 'Unable to resolve async helper placeholders in rendered output',
        });
      }
      res = replaced;
    }
