...
const THEME = {
  ...
  'default.hbs':
    '<html><head><title>{{meta_title}}</title>{{{ghost_head}}}</head><body class="{{body_class}}" data-scheme="{{@custom.color_scheme}}">{{{body}}}</body></html>',
  'index.hbs':
    '{{!< default}}<main>{{#foreach posts}}{{> "card"}}{{/foreach}}</main>{{pagination}}',
  'home.hbs': '{{!< default}}<main class="home">{{#foreach posts}}{{> "card"}}{{/foreach}}</main>',
  'post.hbs':
    '{{!< default}}{{#post}}<article class="{{post_class}}"><h1>{{title}}</h1>{{content}}</article>{{/post}}',
  'page.hbs':
    '{{!< default}}{{#post}}<section class="static-page"><h1>{{title}}</h1>{{content}}</section>{{/post}}',
...
function notFound(): Response {
  return json(
    { errors: [{ message: 'Resource not found error, cannot read post.', type: 'NotFoundError' }] },
    404,
  );
}
...
describe('createRenderer', function () {
  ...
  describe('page param', function () {
    ...
    it('404s /page/0/ locally without calling the posts API', async function () {
      ...
      const postRequests = requests.filter((u) =>
        u.pathname.startsWith('/ghost/api/content/posts'),
      );
    ...
    it('404s /tag/:slug/page/0/ locally without calling the API (taxonomy pagination)', async function () {
      ...
      const contentRequests = requests.filter(
        (u) =>
          u.pathname.startsWith('/ghost/api/content/') &&
          !u.pathname.startsWith('/ghost/api/content/settings/'),
      );
  ...
  it('falls through to the static page lookup when the post read 404s', async function () {
    ...
    assert.ok(
      paths.indexOf('/ghost/api/content/posts/slug/about/') <
        paths.indexOf('/ghost/api/content/pages/slug/about/'),
    );
  });
  ...
  describe('theme error templates', function () {
    it('renders the theme error-404 template for unknown urls', async function () {
      const { renderer } = await createTestRenderer({
        ...THEME,
        'error-404.hbs':
          '{{!< default}}<section class="error-404-tpl"><h1 class="error-code">{{statusCode}}</h1><p class="error-description">{{message}}</p></section>',
      });
      const response = await renderer.render(new Request(`${SITE_URL}missing/`));
