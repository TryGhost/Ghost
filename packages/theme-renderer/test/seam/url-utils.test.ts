...
describe('seam: url-utils', function () {
    ...
    it('urlJoin joins parts with single slashes', function () {
        assert.equal(urlUtils.urlJoin('http://localhost:2368/', '/assets/', '/css/screen.css'), 'http://localhost:2368/assets/css/screen.css');
    ...
    it('urlFor relativeUrl object', function () {
        ...
        assert.equal(urlUtils.urlFor({relativeUrl: '/about/'}, null, true), 'http://localhost:2368/about/');
    });

    it('urlFor passes through absolute/external urls', function () {
        assert.equal(urlUtils.urlFor({relativeUrl: 'https://example.com/x/'}), 'https://example.com/x/');
    });

    it('urlFor api', function () {
        assert.equal(urlUtils.urlFor('api', {type: 'content'}, true), 'http://localhost:2368/ghost/api/content/');
    });
    ...
    it('urlFor image', function () {
        assert.equal(urlUtils.urlFor('image', {image: '/content/images/2020/01/foo.jpg'}, true), 'http://localhost:2368/content/images/2020/01/foo.jpg');
        assert.equal(urlUtils.urlFor('image', {image: '/content/images/2020/01/foo.jpg'}), '/content/images/2020/01/foo.jpg');
    });
    ...
    it('absoluteToRelative strips the origin', function () {
        ...
        assert.equal(urlUtils.absoluteToRelative('https://other.com/welcome/'), 'https://other.com/welcome/');
    });
    ...
    it('honours subdirectory config', function () {
        ...
        assert.equal(subdirUtils.absoluteToRelative('http://localhost:2368/blog/welcome/'), '/blog/welcome/');
        assert.equal(subdirUtils.absoluteToRelative('http://localhost:2368/blog/welcome/', {withoutSubdirectory: true}), '/welcome/');
    });
});
