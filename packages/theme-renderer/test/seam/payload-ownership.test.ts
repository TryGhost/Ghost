...
describe('seam payload ownership assertion', function () {
    it('rejects the second fetch when the port returns the same payload object twice', async function () {
        ...
        configureTestDeps({depsOverrides: {api: {
            postsPublic: {
                browse(options: any) {
        ...
        const routerOptions = {resourceType: 'posts', query: {controller: 'postsPublic', type: 'browse', resource: 'posts'}};
        ...
        await assert.rejects(
            fetchData({}, routerOptions, {member: null}),
            (err: any) => {
