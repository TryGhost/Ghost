...
import {EDIT_MARKER_ATTRIBUTE, parseEditMarker, type EditMarker} from '../../src/engine/markers.ts';
...
describe('editor loop (click → edit → worker re-render → preview update)', function () {
    it('edits the site title and re-renders with untouched regions byte-identical', async function () {
        ...
        expect(editedEl.getAttribute(EDIT_MARKER_ATTRIBUTE)).toBe(`${marker.file}:${marker.line}:${marker.column}`);
        ...
        console.log(`[editor-loop] site-title: initial marked render ${first.durationMs.toFixed(0)}ms, re-render after edit ${second.durationMs.toFixed(0)}ms`);
    });
    ...
    it('editing one post-card marker location changes ALL cards (shared source position)', async function () {
        ...
        const cardTitles = (d: Document): (string | null)[] => [...d.querySelectorAll('.post-card-title')].map(e => e.textContent);
        expect(cardTitles(doc2)).toEqual(cardTitles(doc));

        // eslint-disable-next-line no-console
        console.log(`[editor-loop] post-card: initial marked render ${first.durationMs.toFixed(0)}ms, re-render after edit ${second.durationMs.toFixed(0)}ms`);
    });
});
