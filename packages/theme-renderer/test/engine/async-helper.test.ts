...
describe('TemplateEngine async helpers', function () {
  ...
  it('passes hash options through to three-argument helpers', async function () {
    ...
    engine.registerAsyncHelper(
      'meta',
      function (context: unknown, options: unknown, cb: (result: unknown) => void) {
  ...
  it(
    'settles (rejects) the render when an async helper returns a rejecting promise',
    { timeout: 2000 },
    async function () {
      const engine = new TemplateEngine(
        createResolver({
  ...
  it('resolves placeholders nested inside other async helper values', async function () {
    ...
    engine.registerAsyncHelper('outer', function (this: unknown, context, cb) {
      ...
      const rendered = sub({
        resolverCache: (context as { data: { root: { resolverCache: unknown } } }).data.root
          .resolverCache,
      });
      cb(rendered);
    });
  ...
  it('resolves placeholders registered during another helper´s async work (multi-pass)', async function () {
    ...
    engine.registerAsyncHelper('outer', function (context, cb) {
      const resolverCache = (context as { data: { root: { resolverCache: unknown } } }).data.root
        .resolverCache;
      setTimeout(() => {
  ...
  it('rejects instead of looping forever on an unresolvable placeholder lookalike', async function () {
    ...
    await assert.rejects(
      engine.render('index.hbs', {}),
      /Unable to resolve async helper placeholders/,
    );
  });
});
...
describe('token grammar', function () {
  ...
  it('TOKEN_PATTERN matches a freshly generated placeholder id, raw and HTML-escaped', function () {
    ...
    assert.equal(
      escaped.replace(TOKEN_PATTERN, ''),
      '',
      `escaped token not fully matched: ${escaped}`,
    );
  });
});
