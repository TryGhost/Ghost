...
describe('TemplateEngine template options', function () {
    ...
    it('keeps template options per engine instance, not module-global', async function () {
        ...
        const engineA = new TemplateEngine(createResolver(files), {templateOptions: {data: {site: {title: 'A'}}}});
        const engineB = new TemplateEngine(createResolver(files), {templateOptions: {data: {site: {title: 'B'}}}});
    ...
    it('reads local template options back with getLocalTemplateOptions', function () {
        ...
        assert.deepEqual(engine.getLocalTemplateOptions(locals), {data: {custom: {accent: 'red'}}});
    });
