...
import {injectEditMarkers, parseEditMarker, TemplateEngine, type TemplateResolver} from '../../src/index.ts';
...
describe('injectEditMarkers', function () {
    ...
    it('uses 1-based line and column of the tag’s "<" in the original source', function () {
        ...
        assert.equal(injectEditMarkers(source, 'index.hbs'), [
            '{{!< default}}',
    ...
    it('handles block helpers and mustache attributes inside the open tag', function () {
        assert.equal(
            injectEditMarkers('<div {{#if x}}class="a"{{/if}} data-x="{{concat "a" "b"}}">ok</div>', FILE),
    ...
    it('does not touch <script> content but marks the tag itself', function () {
        ...
        assert.equal(injectEditMarkers(source, FILE), [
    ...
    it('ignores a literal "<" in text content', function () {
        assert.equal(
            injectEditMarkers('<p>1 < 2</p>', FILE),
            '<p data-edit="t.hbs:1:1">1 < 2</p>'
        );
    });
