...
import {TemplateEngine, createEngineHelperRegistrar, type TemplateResolver} from '../../src/index.ts';
...
describe('createEngineHelperRegistrar', function () {
    ...
    it('dispatches block-style async helpers with (context, options)', async function () {
        configureTestDeps();
        const engine = new TemplateEngine(createResolver({'index.hbs': '{{#fetch "posts" limit="2"}}{{count}}{{/fetch}}'}));
        const registry = createHelperRegistry(createEngineHelperRegistrar(engine));
