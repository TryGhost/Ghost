...
describe('{{comments}}', function () {
  ...
  it('returns undefined when comments are off, or the post is inaccessible', async function () {
    ...
    assert.equal(
      await comments.call(
        { comment_id: 'c', id: 'p', access: false },
        { hash: {}, data: { site: {} } },
      ),
      undefined,
    );
  ...
  it('honors the mode/saturation/count/title hash params', async function () {
    ...
    const rendered = (await comments.call(
      { comment_id: 'post_test', id: 'post_id_123', access: true },
      {
        hash: { mode: 'dark', saturation: '80', count: false, title: 'Join the discussion' },
        data: { site: {} },
      },
    ))!.toString();
    ...
    assert.match(
      rendered,
      /data-accent-color=""/,
      'no site accent color → empty attribute, exactly like core',
    );
  });
