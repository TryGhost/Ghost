...
describe('{{#get}}', function () {
  ...
  it('uses read for slug queries', async function () {
    configureTestDeps({
      fetch: recordingFetch({ posts: [{ slug: 'welcome', title: 'W', html: '' }] }),
    });
    const { options } = blockOptions();
  ...
  it('renders the inverse for unknown resources', async function () {
    ...
    assert.equal(
      captured.inverseCalls[0].opts.data.error,
      'Invalid "products" resource given to get helper',
    );
  ...
  it('renders the inverse with the API error message on failure', async function () {
    configureTestDeps({
      fetch: (async () =>
        new Response(JSON.stringify({ errors: [{ message: 'Kaboom' }] }), {
          status: 500,
        })) as typeof globalThis.fetch,
    });
    const { options, captured } = blockOptions();
  ...
  it('exposes pagination as a block param', async function () {
    configureTestDeps({
      fetch: recordingFetch({
        posts: [],
        meta: { pagination: { page: 1, pages: 1, limit: 15, total: 0, next: null, prev: null } },
      }),
    });
    const { options, captured } = blockOptions();
