...
function headOptions({renderObject = {}, context = ['home'], settings = {}, hash = {}}: any = {}) {
    void settings;
    return {
        ...createHbsResponse({
...
describe('{{ghost_head}}', function () {
    beforeEach(function () {
        configureTestDeps({
            config: {
                ...
                sodoSearch: {url: 'https://cdn.example/sodo-search@{version}/umd/sodo-search.min.js', version: '1.7', styles: 'https://cdn.example/sodo-search@{version}/umd/main.css'}
            }
        });
    });
    ...
    it('renders the home head with meta, canonical, structured data and schema', async function () {
        ...
        assert.match(head, new RegExp(`<link rel="alternate" type="application/rss\\+xml" title="Ghost" href="${SITE_URL}rss/">`));
        ...
        assert.match(head, /<script defer src="https:\/\/cdn.example\/portal@2.51\/umd\/portal.min.js"/);
        ...
        assert.match(head, new RegExp(`<link href="${SITE_URL}webmentions/receive/" rel="webmention">`));
        ...
        assert.match(head, /<link rel="stylesheet" type="text\/css" href="\/public\/cards.min.css\?v=themerender">/);
    });
    ...
    it('renders post head with article og:type and canonical from the serializer url', async function () {
        ...
        assert.match(head, /<meta property="article:published_time" content="2024-01-10T00:00:00.000Z">/);
    ...
    it('respects the exclude hash', async function () {
        const result = await ghostHead(headOptions({hash: {exclude: 'metadata,card_assets,search'}}));
        const head = result!.toString();
    ...
    it('injects global and post codeinjection', async function () {
        ...
        const result = await ghostHead(headOptions({renderObject: {post, relativeUrl: '/welcome/'}, context: ['post']}));
        const head = result!.toString();
...
describe('{{ghost_foot}}', function () {
    it('outputs global codeinjection', function () {
        configureTestDeps({settingsOverrides: {codeinjection_foot: '<script>window.foot = true;</script>'}});
