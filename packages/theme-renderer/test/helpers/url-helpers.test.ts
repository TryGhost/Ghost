...
import {SITE_URL, configureTestDeps, createHbsResponse, teardownTestDeps} from '../utils/renderer-test-utils.ts';
...
describe('{{img_url}}', function () {
    ...
    it('returns absolute when requested', function () {
        const result = imgUrl.call({}, '/content/images/2024/01/pic.png', createHbsResponse({hash: {absolute: 'true'}}));
...
describe('{{authors}}', function () {
    it('autolinks authors', function () {
        const context = {
            authors: [
                {name: 'Cameron', slug: 'cameron', url: `${SITE_URL}author/cameron/`, visibility: 'public'}
            ]
        };
