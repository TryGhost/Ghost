...
describe('applyTextEdit', function () {
  it('replaces a static text child', function () {
    assert.equal(
      applyTextEdit(
        '<a class="gh-head-link" href="#">Sign in</a>',
        { line: 1, column: 1 },
        'Log in',
      ),
  ...
  it('edits the element at the marker position, not the first tag in the file', function () {
    const source = ['<div class="wrap">', '  <p>old text</p>', '  <p>keep me</p>', '</div>'].join(
      '\n',
    );
    assert.equal(
      applyTextEdit(source, { line: 2, column: 3 }, 'new text'),
      ['<div class="wrap">', '  <p>new text</p>', '  <p>keep me</p>', '</div>'].join('\n'),
    );
  });

  it('preserves leading/trailing whitespace around the text child', function () {
    const source = ['<h2 class="post-card-title">', '    {{title}}', '</h2>'].join('\n');
    assert.equal(
      applyTextEdit(source, { line: 1, column: 1 }, 'Edited'),
      ['<h2 class="post-card-title">', '    Edited', '</h2>'].join('\n'),
    );
  });
  ...
  it('handles mustache blocks and quoted ">" inside the open tag itself', function () {
    assert.equal(
      applyTextEdit(
        '<div {{#if x}}class="a"{{/if}} title="a>b">Text</div>',
        { line: 1, column: 1 },
        'New',
      ),
  ...
  describe('anchor verification (stale markers)', function () {
    ...
    it('applies when the anchored tag is at the marker position', function () {
      assert.equal(
        applyTextEdit(source, { line: 2, column: 3 }, 'Edited', { tagName: 'h2' }),
        [
          '<section class="feed">',
    ...
    it('anchor tag names are case-insensitive (DOM tagName is uppercase)', function () {
      assert.ok(
        applyTextEdit(source, { line: 2, column: 3 }, 'Edited', { tagName: 'H2' }).includes(
          '>Edited</h2>',
        ),
      );
    });
    ...
    it('re-locates a shifted marker when exactly one anchored tag is within ±3 lines', function () {
      ...
      assert.equal(
        applyTextEdit(shifted, { line: 2, column: 3 }, 'Edited', { tagName: 'h2' }),
        [
    ...
    it('fails loudly on a stale marker with no anchored tag nearby', function () {
      ...
      assert.throws(
        () => applyTextEdit(source, { line: 3, column: 3 }, 'x', { tagName: 'aside' }),
        /stale/,
      );
    });

    it('fails loudly when the re-locate window is ambiguous', function () {
      const twins = ['<ul>', '  <li class="a">one</li>', '  <li class="b">two</li>', '</ul>'].join(
        '\n',
      );
      // marker points at the <ul> line but claims <li> — two candidates
      // within the window, no safe pick
      assert.throws(
        () => applyTextEdit(twins, { line: 1, column: 1 }, 'x', { tagName: 'li' }),
        /stale/,
      );
    });
    ...
    it('does not re-locate to a longer tag name sharing the anchored prefix', function () {
      ...
      assert.throws(
        () => applyTextEdit(tricky, { line: 1, column: 1 }, 'x', { tagName: 'h' }),
        /stale/,
      );
    });
    ...
    it('throws stale-marker for a line past the end of the file (never edits a clamped line)', function () {
      ...
      assert.throws(
        () => applyTextEdit(source, { line: 900, column: 3 }, 'x', { tagName: 'h2' }),
        /stale/,
      );
      assert.throws(
        () => applyTextEdit(source, { line: 0, column: 1 }, 'x', { tagName: 'h2' }),
        /stale/,
      );
    });
    ...
    describe('re-locate candidates are scanner-consistent (markable tags only)', function () {
      it('does not re-locate into an HTML comment', function () {
        ...
        assert.throws(
          () => applyTextEdit(commented, { line: 3, column: 3 }, 'x', { tagName: 'h2' }),
          /stale/,
        );
      });
      ...
      it('does not re-locate into a handlebars comment', function () {
        ...
        assert.throws(
          () => applyTextEdit(hbsCommented, { line: 3, column: 1 }, 'x', { tagName: 'h2' }),
          /stale/,
        );
      });
      ...
      it('does not re-locate into a rawtext element body', function () {
        ...
        assert.throws(
          () => applyTextEdit(script, { line: 2, column: 1 }, 'x', { tagName: 'h2' }),
          /stale/,
        );
      });
      ...
      it('does not re-locate to a dynamic tag name', function () {
        const dynamic = '<div>\n<h{{level}}>x</h{{level}}>\n</div>';
        assert.throws(
          () => applyTextEdit(dynamic, { line: 1, column: 1 }, 'x', { tagName: 'h' }),
          /stale/,
        );
      });
    });
...
describe('applyThemeTextEdit', function () {
  ...
  it('returns a NEW files object with only the marker file edited', function () {
    const edited = applyThemeTextEdit(
      theme,
      { file: 'partials/card.hbs', line: 1, column: 1 },
      'Same Text',
    );
  ...
  it('passes the anchor through (stale marker fails at theme level too)', function () {
    assert.equal(
      applyThemeTextEdit(theme, { file: 'index.hbs', line: 1, column: 1 }, 'Hi', { tagName: 'h1' })[
        'index.hbs'
      ],
    ...
    assert.throws(
      () =>
        applyThemeTextEdit(theme, { file: 'index.hbs', line: 1, column: 1 }, 'Hi', {
          tagName: 'h6',
        }),
      /stale/,
    );
