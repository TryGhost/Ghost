...
import {
  applyAttributeEdit,
  applyAttributeEdits,
  applyThemeAttributeEdit,
  applyThemeAttributeEdits,
} from '../../src/editor/attribute-edit.ts';
...
describe('applyAttributeEdit', function () {
  describe('replacing an existing attribute', function () {
    it('replaces a double-quoted value in place', function () {
      assert.equal(
        applyAttributeEdit('<img class="hero" src="/old.jpg" alt="a">', AT, {
          name: 'src',
          value: '/content/images/new.jpg',
        }),
    ...
    it('replaces a value that is entirely a mustache expression (Casper img_url shape)', function () {
      assert.equal(
        applyAttributeEdit('<img src="{{img_url feature_image size="l"}}" alt="{{title}}">', AT, {
          name: 'src',
          value: '/content/images/new.jpg',
        }),
    ...
    it('leaves mustache-containing SIBLING attributes on the same tag untouched', function () {
      const source =
        '<img srcset="{{img_url feature_image size="s"}} 300w, {{img_url feature_image size="m"}} 600w" src="{{img_url feature_image}}" sizes="(max-width: 600px) 100vw">';
      assert.equal(
    ...
    it('a dynamic attribute NAME\'s value is never mistaken for attributes (data-{{x}}="src")', function () {
      ...
      assert.equal(
        applyAttributeEdit('<img data-{{x}}="src" src="/old.jpg">', AT, {
          name: 'src',
          value: '/new.jpg',
        }),
    ...
    it('a fully-dynamic attribute name\'s value is skipped the same way ({{x}}="src")', function () {
      assert.equal(
        applyAttributeEdit('<img {{dynamic}}="src" src="/old.jpg">', AT, {
          name: 'src',
          value: '/new.jpg',
        }),
    ...
    it('works on non-void elements too', function () {
      assert.equal(
        applyAttributeEdit('<a href="/old/" class="x">Link</a>', AT, {
          name: 'href',
          value: '/new/',
        }),
    ...
    it('edits the tag at the marker position, not the first tag in the file', function () {
      ...
      assert.equal(
        applyAttributeEdit(source, { line: 2, column: 3 }, { name: 'src', value: '/new.jpg' }),
        [
          '<figure class="wrap">',
    ...
    it('never edits past the open tag\'s ">"', function () {
      ...
      assert.equal(
        applyAttributeEdit('<div title="d">src="not an attribute"</div>', AT, {
          name: 'src',
          value: 'x',
        }),
  ...
  describe('inserting a missing attribute', function () {
    ...
    it('inserts before attribute lists that span mustache boundaries', function () {
      assert.equal(
        applyAttributeEdit('<img {{#if wide}}class="wide"{{/if}} alt="x">', AT, {
          name: 'src',
          value: '/new.jpg',
        }),
  ...
  describe('deleting an attribute (value: null)', function () {
    it('removes the attribute and its preceding whitespace', function () {
      assert.equal(
        applyAttributeEdit('<img src="/a.jpg" srcset="/a-s.jpg 300w" alt="x">', AT, {
          name: 'srcset',
          value: null,
        }),
        '<img src="/a.jpg" alt="x">',
    ...
    it('removes an attribute whose value contains quoted mustache arguments', function () {
      assert.equal(
        applyAttributeEdit(
          '<img src="/a.jpg" srcset="{{img_url feature_image size="s"}} 300w">',
          AT,
  ...
  describe('value contract (plain attribute text only)', function () {
    it('rejects handlebars syntax in the value (template injection)', function () {
      assert.throws(
        () => applyAttributeEdit('<img src="/a.jpg">', AT, { name: 'src', value: '{{@site.url}}' }),
        /handlebars|injection/i,
      );
      assert.throws(
        () =>
          applyAttributeEdit('<img src="/a.jpg">', AT, { name: 'src', value: 'stray }} braces' }),
        /handlebars|injection/i,
      );
    });
    ...
    it('rejects newlines in the value (edits must never shift line numbers)', function () {
      assert.throws(
        () => applyAttributeEdit('<img src="/a.jpg">', AT, { name: 'src', value: 'two\nlines' }),
    ...
    it('does not double-escape an already-escaped ampersand source when re-editing', function () {
      const once = applyAttributeEdit('<img src="/a.jpg">', AT, {
        name: 'src',
        value: '/new.jpg?a=1&b=2',
      });
  ...
  describe('attribute-name contract', function () {
    it('rejects names with quotes, spaces, equals, or angle brackets (attribute injection)', function () {
      for (const name of ['sr c', 'src=x', 'src"', `src'`, 'src>', 'src/', '', '  ', '{{src}}']) {
        assert.throws(
          () => applyAttributeEdit('<img src="/a.jpg">', AT, { name, value: 'x' }),
          /attribute name/i,
        );
      }
    });
    ...
    it('rejects the reserved data-edit marker attribute', function () {
      assert.throws(
        () => applyAttributeEdit('<img src="/a.jpg">', AT, { name: 'data-edit', value: 'x' }),
        /data-edit|reserved/i,
      );
    });
  });
  ...
  describe('positions the editor must refuse', function () {
    it('rejects a position that is not an element open tag', function () {
      assert.throws(
        () => applyAttributeEdit('plain text', AT, { name: 'src', value: 'x' }),
        /open tag/,
      );
      assert.throws(() => applyAttributeEdit('</p>', AT, { name: 'src', value: 'x' }), /open tag/);
      assert.throws(
        () => applyAttributeEdit('<!-- c -->', AT, { name: 'src', value: 'x' }),
        /open tag/,
      );
    });
    ...
    it('rejects an out-of-range position (unanchored)', function () {
      assert.throws(
        () => applyAttributeEdit('<img>', { line: 5, column: 1 }, { name: 'src', value: 'x' }),
        /outside/,
      );
    });
    ...
    it('rejects a dynamic tag name', function () {
      assert.throws(
        () => applyAttributeEdit('<h{{level}}>x</h{{level}}>', AT, { name: 'id', value: 'x' }),
        /open tag/,
      );
    });
    ...
    it('rejects an unterminated open tag', function () {
      assert.throws(
        () => applyAttributeEdit('<img src="/a.jpg', AT, { name: 'src', value: 'x' }),
        /unterminated/i,
      );
    });
    ...
    it('refuses to touch an attribute that lives inside a handlebars block on the tag', function () {
      ...
      assert.throws(
        () => applyAttributeEdit(conditional, AT, { name: 'srcset', value: 'x' }),
        /handlebars block/i,
      );
      assert.throws(
        () => applyAttributeEdit(conditional, AT, { name: 'srcset', value: null }),
        /handlebars block/i,
      );
    });
...
describe('applyAttributeEdits (batch)', function () {
  ...
  it('matches the sequential single-edit result byte for byte', function () {
    const input =
      '<img src="{{img_url feature_image size="l"}}" srcset="{{img_url feature_image size="s"}} 300w" sizes="(max-width: 600px) 100vw" alt="{{title}}">';
    const sequential = applyAttributeEdit(
      applyAttributeEdit(applyAttributeEdit(input, AT, { name: 'src', value: '/new.jpg' }), AT, {
        name: 'srcset',
        value: null,
      }),
      AT,
  ...
  it('an optional edit hitting the handlebars-block refusal is skipped, not fatal', function () {
    ...
    assert.equal(
      source,
      '<img {{#if lazy}}srcset="/a-s.jpg 300w"{{/if}} src="/new.jpg">',
      'the required src swap still lands',
    );
    assert.equal(
      skipped.length,
      1,
      'only the blocked srcset is skipped — absent sizes is a plain no-op',
    );
...
describe('applyThemeAttributeEdits', function () {
  ...
  it('returns a NEW files object plus the skipped optional edits', function () {
    const { theme: edited, skipped } = applyThemeAttributeEdits(
      theme,
    ...
    assert.equal(
      theme['index.hbs'],
      '<img class="hero" src="/old.jpg" srcset="/old-s.jpg 300w">',
      'input never mutated',
    );
  });
...
describe('applyThemeAttributeEdit', function () {
  ...
  it('returns a NEW files object with only the marker file edited', function () {
    const edited = applyThemeAttributeEdit(
      theme,
      { file: 'index.hbs', line: 1, column: 1 },
      { name: 'src', value: '/new.jpg' },
    );
  ...
  it('rejects a marker pointing at a file the theme does not contain', function () {
    assert.throws(
      () =>
        applyThemeAttributeEdit(
          theme,
          { file: 'partials/missing.hbs', line: 1, column: 1 },
          { name: 'src', value: 'x' },
        ),
