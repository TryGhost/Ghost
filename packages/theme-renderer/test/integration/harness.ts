...
export {
  ...
  type InstanceConfigScrape,
} from '../../src/editor/instance-config.ts';
...
export async function fetchFirstPost(
  contentApiKey: string,
): Promise<{ slug: string; url: string } | null> {
  const response = await fetch(
    `${GHOST_URL}/ghost/api/content/posts/?key=${contentApiKey}&limit=1&fields=slug,url`,
  );
