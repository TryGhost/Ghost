...
describe('source markers over the recorded Casper fixtures (hermetic)', function () {
  ...
  it('every marker seeks to the "<" of an open tag in the actual theme source', async function () {
    ...
    for (const value of markers) {
      ...
      assert.match(
        tagStart,
        /^<[a-zA-Z]/,
        `marker does not point at an open tag: ${value} → ${JSON.stringify(tagStart.slice(0, 20))}`,
      );
    }
  });
  ...
  it('site-header, post cards and the layout carry markers for the right Casper files and positions', async function () {
    ...
    assert.ok(
      marked.includes(
        `<div data-edit="${markerFor('index.hbs', '<div class="site-header-content')}"`,
      ),
    );
  ...
  it('interleaved marked and plain renders on one renderer both complete correctly', async function () {
    const themeWithNavOverride = {
      ...
      'partials/navigation.hbs':
        '<ul class="nav-override">{{#foreach navigation}}<li><a href="{{url absolute="true"}}">{{label}}</a></li>{{/foreach}}</ul>',
    };
    ...
    const request = (): Request =>
      new Request(new URL(instance.routes.home, instance.siteUrl).toString());
