...
describe.skipIf(Boolean(probe.unavailableReason))(
  `byte parity (${probe.unavailableReason || GHOST_URL})`,
  function () {
    it('scrapes the instance config the Content API cannot supply', function () {
      assert.deepEqual(
        scrape.missing,
        [],
        `live page scrapes missed: ${scrape.missing.join(', ')}`,
      );
    ...
    it('renders the tag route byte-identical to live modulo documented stubs', async function () {
      const tagsResponse = await fetch(
        `${GHOST_URL}/ghost/api/content/tags/?key=${probe.contentApiKey}&limit=1&fields=slug,url&filter=visibility:public`,
      );
