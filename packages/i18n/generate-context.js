...
(async () => {
  ...
  if (process.env.CI && newContent !== existingContent) {
    ...
    console.error(
      'context.json is out of date. Run `pnpm translate` in packages/i18n and commit the result.',
    );
    process.exit(1);
  }
  ...
  if (emptyKeys.length > 0) {
    if (process.env.CI) {
      ...
      // eslint-disable-next-line no-console
      console.error(
        'Translation keys are missing context descriptions in context.json:\n' + keyList,
      );
      // eslint-disable-next-line no-console
      console.error(
      ...
      // eslint-disable-next-line no-console
      console.warn(
        `Warning: ${emptyKeys.length} key(s) in context.json have empty descriptions. Please add context before committing.`,
      );
    }
  }
