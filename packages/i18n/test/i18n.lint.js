...
function parseIgnores(ignoreFile) {
  ...
  for (const [rule, ignored] of Object.entries(ignores)) {
    ...
    for (const [index, ignore] of ignored.entries()) {
      ...
      assert(
        typeof ignore.file === 'string',
        `Expected .overrides.${rule}[${index}].file to be a String`,
      );
      assert(
        typeof ignore.key === 'string',
        `Expected .overrides.${rule}[${index}].key to be a String`,
      );
      assert(
        typeof ignore.comment === 'string',
        `Expected .overrides.${rule}[${index}].comment to be a String`,
      );
...
class LinterContext {
  ...
  _reportError(message, ruleName, line, column, fix = undefined) {
    ...
    if (fix) {
      this._fixes.push({
        fix,
        resultIndex: this._currentResultIndex,
        messageIndex: this._currentMessageIndex,
      });
    }
  }
  ...
  _applyFixes() {
    ...
    for (const resultIndex of resultsToFilter) {
      this.summary.results[resultIndex].messages =
        this.summary.results[resultIndex].messages.filter(Boolean);
    }
...
async function exitWithSummary(summary) {
  ...
  if (summary.ignoreAllFlagProvidedWithoutFixFlag) {
    // eslint-disable-next-line no-console
    console.warn('--unsafe-ignore-all was provided without --fix; errors were not ignored');
  }
...
function ignoreTranslationError(ignores, rule, file, key) {
  ...
  ignores[rule].push({
    ...
    key,
    comment:
      'FIXME: This error was automatically ignored. Please update the translation or this comment.',
  });
}
  ...
  for (const [define, columnInKey] of defines.entries()) {
    ...
    if (!used.delete(define)) {
      ...
      context.reportTranslationError(
        ...
        column + columnInKey,
        context.ignoreAll
          ? (ignores) => ignoreTranslationError(ignores, rule, file, key)
          : undefined,
      );
    }
  }
