...
export function createBuilder<T extends object>(
  defaults: (overrides: Partial<T>) => T,
): Builder<T> {
  const build = (overrides: Partial<T> = {}): T => ({ ...defaults(overrides), ...overrides });
  build.many = (
    countOrList: number | Array<Partial<T>>,
...
export type RequiredBuilderInput<T, TRequiredKeys extends keyof T> = Required<
  Pick<T, TRequiredKeys>
> &
  Partial<Omit<T, TRequiredKeys>>;
...
export function createRequiredBuilder<T extends object, TRequiredKeys extends keyof T>(
  defaults: (input: RequiredBuilderInput<T, TRequiredKeys>) => Omit<T, TRequiredKeys>,
): RequiredBuilder<T, TRequiredKeys> {
  const build = (input: RequiredBuilderInput<T, TRequiredKeys>): T =>
    ({ ...defaults(input), ...input }) as T;
