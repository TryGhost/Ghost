...
import { SUPPORTED_INTERVALS, isCountablePeriodStart } from './date-utils.js';
...
export function resolve({
  ...
  for (const configuredName of Object.keys(limits)) {
    ...
    if (kind === 'max' || kind === 'maxPeriodic') {
      ...
      // Everything a period needs is checked here rather than where it is used. Otherwise
      // an interval or a date this cannot work with builds a limit that looks applied and
      // then throws the first time something asks it a question, in the middle of a
      // request that had nothing to do with configuring it.
      if (!SUPPORTED_INTERVALS.includes(subscription.interval)) {
        problems.push({
          limit: name,
          reason: `a periodic limit cannot be counted over "${subscription.interval}"`,
        });
        continue;
      }
