...
- `scheduleRecurring(envelope, {cron})` - register (or replace) the recurring schedule for the envelope's type.
...
`scheduleRecurring` is idempotent per type (re-scheduling replaces), and `cron`
is a 5-field expression or 6 fields with a leading seconds field. Delivery is
at-most-once in memory but at-least-once on a durable backend, so handlers must
tolerate redelivery.
