...
The adapter-base-route-settings package for Ghost.
...
```bash
pnpm --filter @tryghost/adapter-base-route-settings build   # compile to build/ with tsc (ESM)
pnpm --filter @tryghost/adapter-base-route-settings test    # type-check + unit tests
pnpm --filter @tryghost/adapter-base-route-settings dev     # rebuild on change
```
