...
describe('slugify()', function () {
  ...
  describe('<4.x markdown', function () {
    it('replaces all whitespace with empty string', function () {
      expect(slugify('test one\ttwo', { ghostVersion: '2.0', type: 'markdown' })).toEqual(
        'testonetwo',
      );
    });

    it('replaces all "non-word" chars with empty string', function () {
      expect(slugify('tést øne twö', { ghostVersion: '2.0', type: 'markdown' })).toEqual('tstnetw');
    });

    it('lower cases everything', function () {
      expect(slugify('TÉST ÓNE TWÖ', { ghostVersion: '2.0', type: 'markdown' })).toEqual('tstnetw');
    });
  });

  describe('<4.x mobiledoc', function () {
    it('replaces all white space with "-"', function () {
      expect(slugify('test one\ttwo', { ghostVersion: '3.0' })).toEqual('test-one-two');
    });

    it('replaces all "non-word" chars with "-"', function () {
      expect(slugify('tést øne twö', { ghostVersion: '3.0' })).toEqual('t-st-ne-tw-');
    });

    it('collapses multiple "-"', function () {
      expect(slugify('ñéïñ', { ghostVersion: '3.0' })).toBe('-');
    });
