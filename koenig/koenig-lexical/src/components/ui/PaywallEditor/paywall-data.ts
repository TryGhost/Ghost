// Shared data layer for the paywall editor modal (used by both the post card
// and the Settings > Paywalls page).
//
// Each "card" is a real Koenig Call to Action card, stored as its Lexical
// editor-state JSON (a string) per platform + audience. Callers own persistence
// (localStorage for Settings, the paywall node for a post).
