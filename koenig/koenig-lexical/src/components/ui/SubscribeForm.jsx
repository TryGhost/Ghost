...
export function SubscribeForm({dataTestId, placeholder, value, inputBorderStyle, buttonSize, buttonText, buttonStyle, onChange, onFocus, onBlur}) {
    ...
    return (
        <div className='relative flex'>
            <input
                ref={setRef}
                className={`relative w-full border border-black py-2 px-4 font-sans font-normal text-grey-900 focus-visible:outline-none dark:border-grey-900 dark:bg-grey-900 dark:text-white dark:placeholder:text-grey-800 ${(buttonSize === 'small' ? 'h-10 text-md leading-[4rem]' : (buttonSize === 'medium' ? 'h-11 text-[1.6rem] leading-[4.4rem]' : 'h-12 text-lg leading-[4.8rem]'))}`}
