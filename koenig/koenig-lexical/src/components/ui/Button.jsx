...
export function Button({color, size, width, value, placeholder}) {
    return (
        <button className={`inline-block rounded font-sans font-medium ${(width === 'regular') || 'w-full' } ${value ? 'opacity-100' : 'opacity-50' } ${(color === 'light') ? 'bg-white text-black' : 'bg-green text-white'} `} value={value} placeholder={placeholder}>
