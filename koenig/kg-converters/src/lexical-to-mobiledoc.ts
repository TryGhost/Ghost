    ...
    childWithFormats.children.forEach((child: LexicalNode, childIndex: number) => {
      if (TEXT_TYPES.includes(child.type)) {
        if (child.format !== 0) {
          ...
          if (
            !childWithFormats.children![childIndex + 1] ||
            !TEXT_TYPES.includes(childWithFormats.children![childIndex + 1].type)
          ) {
            ...
            const firstMissingFormatIndex = openMarkups.findIndex(
              (format: OpenMarkup) => !nextFormats.includes(format as string),
            );
          ...
          const markupIndexes = openedFormats.map((format) =>
            getOrSetMarkupIndex(format, mobiledoc),
          );
      ...
      if (child.type === 'link') {
        ...
        child.children?.forEach((linkChild: LexicalNode, linkChildIndex: number) => {
          if (linkChild.format !== 0) {
            ...
            openedMarkupIndexes.push(
              ...openedFormats.map((format) => getOrSetMarkupIndex(format, mobiledoc)),
            );
