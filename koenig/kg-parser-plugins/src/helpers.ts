import type {ParserPluginOptions} from './types.js';

export function addFigCaptionToPayload(node: Element, payload: Record<string, unknown>, {selector = 'figcaption', options}: {selector?: string; options: ParserPluginOptions}): void {
...
export function readImageAttributesFromNode(node: HTMLImageElement): Record<string, unknown> {
    ...
    if ((!node.width && !node.height) && node.getAttribute('data-image-dimensions')) {
        const match = (/^(\d*)x(\d*)$/gi).exec(node.getAttribute('data-image-dimensions')!);
        if (match) {
    ...
    if (node.parentNode && (node.parentNode as Element).tagName === 'A') {
        const href = (node.parentNode as HTMLAnchorElement).href;
