...
const getMaxResizedImgDimensions = function ({
  src,
  width,
  height,
  options = {},
}: ImgDimensionArgs = {}): { width?: number; height?: number } {
...
const generateImgSrcAttrs = function ({
  src,
  width,
  height,
  options = {},
}: GenerateImgAttrsArgs = {}): string | undefined {
  if (!src) {
    return;
  }
  ...
  if (options.target === 'email') {
    ...
    if (width && height) {
      ...
      if (
        ...
        const availableImageWidths = getAvailableImageWidths(
          { width },
          options.imageOptimization!.contentImageSizes!,
        );
        const srcWidth = availableImageWidths.find(
          (availableImageWidth) => availableImageWidth >= 1200,
        );
