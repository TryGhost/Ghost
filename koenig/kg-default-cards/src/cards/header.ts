...
const headerCard: Card = {
  ...
  render({ payload: _payload, env: { dom }, options: { ghostVersion } = {} }) {
    ...
    const templateData = {
      ...
      style: payload.style,
      buttonEnabled:
        payload.buttonEnabled && Boolean(payload.buttonUrl) && Boolean(payload.buttonText),
      ...
      backgroundImageStyle:
        payload.style === 'image' ? `background-image: url(${payload.backgroundImageSrc})` : '',
  ...
  absoluteToRelative(payload, options) {
    ...
    p.backgroundImageSrc =
      p.backgroundImageSrc && absoluteToRelative(p.backgroundImageSrc, options.siteUrl, options);
  ...
  relativeToAbsolute(payload, options) {
    ...
    p.buttonUrl =
      p.buttonUrl &&
      relativeToAbsolute(p.buttonUrl, options.siteUrl, options.itemUrl ?? '', options);
    p.backgroundImageSrc =
      p.backgroundImageSrc &&
      relativeToAbsolute(p.backgroundImageSrc, options.siteUrl, options.itemUrl ?? '', options);

    p.header =
      p.header && htmlRelativeToAbsolute(p.header, options.siteUrl, options.itemUrl ?? '', options);
    p.subheader =
      p.subheader &&
      htmlRelativeToAbsolute(p.subheader, options.siteUrl, options.itemUrl ?? '', options);
    return payload;
  },
  ...
  toTransformReady(payload, options) {
    const p = payload as HeaderPayload;
    p.buttonUrl =
      p.buttonUrl && toTransformReady(p.buttonUrl, options.siteUrl, options.itemUrl ?? '', options);
    p.backgroundImageSrc =
      p.backgroundImageSrc &&
      toTransformReady(p.backgroundImageSrc, options.siteUrl, options.itemUrl ?? '', options);

    p.header =
      p.header && htmlToTransformReady(p.header, options.siteUrl, options.itemUrl ?? '', options);
    p.subheader =
      p.subheader &&
      htmlToTransformReady(p.subheader, options.siteUrl, options.itemUrl ?? '', options);
    return payload;
  },
};
