...
const emailCtaCard: Card = {
  ...
  render({ payload, env: { dom }, options = {} }) {
    ...
    payload.html = payload.html
      ? (payload.html as string).replace(/\{(\w*?)(?:,? *"(.*?)")?\}/g, '%%$&%%')
      : '';
  ...
  absoluteToRelative(payload, options) {
    payload.html =
      payload.html && htmlAbsoluteToRelative(payload.html as string, options.siteUrl, options);
    payload.buttonUrl =
      payload.buttonUrl &&
      absoluteToRelative(payload.buttonUrl as string, options.siteUrl, options);
    return payload;
  },
  ...
  toTransformReady(payload, options) {
    payload.html =
      payload.html && htmlToTransformReady(payload.html as string, options.siteUrl, options);
    payload.buttonUrl =
      payload.buttonUrl && toTransformReady(payload.buttonUrl as string, options.siteUrl, options);
    return payload;
  },
};
