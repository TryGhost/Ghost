...
const embedCard: Card = {
  ...
  render({ payload, env: { dom }, options = {} }) {
    ...
    const metadata = payload.metadata as EmbedMetadata;
    const isVideoWithThumbnail = payload.type === 'video' && metadata && metadata.thumbnail_url;
    ...
    if (options.target === 'email' && isVideoWithThumbnail) {
      ...
      figure.appendChild(dom.createRawHTMLSection(payload.html as string));
    }
