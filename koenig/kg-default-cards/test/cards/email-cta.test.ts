...
describe('Email CTA card', function () {
  it('renders html with no replacements', function () {
    ...
    expect(serializer.serialize(card.render(opts))).toBe(
      '<div><p>Plain html with no replacements</p></div>',
    );
  });
  ...
  it('renders button with no payload.html', function () {
    ...
    expect(serializer.serialize(card.render(opts))).toContain('<a href="https://example.com"');

    expect(serializer.serialize(card.render(opts))).not.toContain('undefined');
  });
  ...
  it('does not render button with payload.showButton = false', function () {
    ...
    expect(serializer.serialize(card.render(opts))).not.toContain('<a href="https://example.com"');
  });
  ...
  it('wraps in div with data-gh-segment attribute', function () {
    ...
    expect(serializer.serialize(card.render(opts))).toContain('<div data-gh-segment="status:paid"');
  });
  ...
  it('wraps {foo, "test"} in %%', function () {
    ...
    expect(serializer.serialize(card.render(opts))).toContain(
      '<p>Testing %%{foo, "replacement fallbacks"}%% in %%{bar, "email card"}%%</p>',
    );
  });
  ...
  it('wraps {foo,  "test"} (extra spaces)', function () {
    ...
    expect(serializer.serialize(card.render(opts))).toContain('<p>Testing %%{foo,  "valid"}%%</p>');
  });
  ...
  it('does not wrap {foo invalid} (missing quotes)', function () {
    ...
    expect(serializer.serialize(card.render(opts))).toContain(
      '<p>Testing {foo invalid} in %%{bar}%%</p>',
    );
  });
  ...
  it('renders no dividers by default', function () {
    ...
    expect(serializer.serialize(card.render(opts))).not.toContain('<hr>');
  });
});
