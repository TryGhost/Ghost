...
describe('Embed card', function () {
  it('renders', function () {
    ...
    expect(serializer.serialize(card.render(opts))).toEqual(
      '<figure class="kg-card kg-embed-card"><h1>HEADING</h1><p>PARAGRAPH</p></figure>',
    );
  });
  ...
  it('Plain content renders', function () {
    ...
    expect(serializer.serialize(card.render(opts))).toEqual(
      '<figure class="kg-card kg-embed-card">CONTENT</figure>',
    );
  });
  ...
  it('Invalid HTML returns', function () {
    ...
    expect(serializer.serialize(card.render(opts))).toEqual(
      '<figure class="kg-card kg-embed-card"><h1>HEADING<</figure>',
    );
  });
  ...
  it('Renders caption when provided', function () {
    ...
    expect(serializer.serialize(card.render(opts))).toEqual(
      '<figure class="kg-card kg-embed-card kg-card-hascaption">Testing<figcaption><strong>Caption</strong></figcaption></figure>',
    );
  });
  ...
  it('renders nfts and escapes the JSON', function () {
    ...
    const parsedPayload = JSON.parse(
      decodeURIComponent(
        (dom.window.document.body.querySelector('.kg-nft-card > a') as HTMLAnchorElement).dataset
          .payload!,
      ),
    );
