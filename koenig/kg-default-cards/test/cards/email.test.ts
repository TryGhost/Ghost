...
describe('Email card', function () {
  it('renders html with no replacements', function () {
    ...
    expect(serializer.serialize(card.render(opts))).toBe('<p>Plain html with no replacements</p>');
  });
  ...
  it('wraps {foo} in %%', function () {
    ...
    expect(serializer.serialize(card.render(opts))).toBe('<p>Testing %%{foo}%% in %%{bar}%%</p>');
  });
  ...
  it('wraps {foo,  "test"} (extra spaces)', function () {
    ...
    expect(serializer.serialize(card.render(opts))).toBe('<p>Testing %%{foo,  "valid"}%%</p>');
  });
  ...
  it('does not wrap {invalid } (invalid whitespace)', function () {
    ...
    expect(serializer.serialize(card.render(opts))).toBe('<p>Testing {invalid } in %%{bar}%%</p>');
  });
