module.exports = function getAvailableImageWidths(image, contentImageSizes) {
    // get a sorted list of the available responsive widths
    const contentWidths = Object.values(contentImageSizes)
        .map(({width}) => width)
        .sort((a, b) => a - b);

    // select responsive widths that are usable based on the image width
    const availableImageWidths = contentWidths
    ...
    if (image.width > availableImageWidths[availableImageWidths.length - 1] && image.width < contentWidths[contentWidths.length - 1]) {
