...
describe('CardFactory', function () {
  describe('render', function () {
    it('adds comment wrapper when configured', function () {
      ...
      const opts = {
        env: { dom: new SimpleDomDocument() as unknown as DomProvider },
        payload: {},
        options: {},
      };
      ...
      expect(
        serializer.serialize(
          card.render(opts) as unknown as Parameters<typeof serializer.serialize>[0],
        ),
  ...
  describe('absoluteToRelative', function () {
    it('passes siteUrl in from factory options', function () {
      ...
      expect(card.absoluteToRelative({})).toHaveProperty(
        'receivedSiteUrl',
        'http://127.0.0.1:2368/',
      );
    });
  ...
  describe('relativeToAbsolute', function () {
    it('passes siteUrl in from factory options', function () {
      ...
      expect(card.relativeToAbsolute({})).toHaveProperty(
        'receivedSiteUrl',
        'http://127.0.0.1:2368/',
      );
    });
