import {
  createChartFallbackWorkflow,
  createChartPatchFromFile,
  createLabelColumnPatch,
} from '../src/editor-settings-model.ts';
...
describe('chart editor settings', () => {
  ...
  it('turns the chosen CSV into durable table properties and clears a stale fallback', () => {
    expect(
      createChartPatchFromFile(
        file('readers.csv', 'Quarter,Readers,Members\nQ1,1200,40\nQ2,1800,58'),
      ),
    ).toEqual({
      table: {
  ...
  it('selects the first numeric value series rather than an intervening text column', () => {
    expect(
      createChartPatchFromFile(
        file('readers.csv', 'Month,Region,Readers\nJan,EU,1200\nFeb,US,1800'),
      ),
    ).toMatchObject({
      labelColumn: 'Month',
      valueColumn: 'Readers',
  ...
  it('uses a later label when the first column is the numeric value series', () => {
    expect(
      createChartPatchFromFile(file('counts.csv', 'Count,Category\n10,A\n20,B')),
    ).toMatchObject({
  ...
  it('keeps the conventional first-column label when every column is numeric', () => {
    expect(
      createChartPatchFromFile(file('sales.csv', 'Year,Sales\n2025,100\n2026,120')),
    ).toMatchObject({
      labelColumn: 'Year',
      valueColumn: 'Sales',
    });
  ...
  it('rejects a table without a numeric value series', () => {
    expect(() =>
      createChartPatchFromFile(file('labels.csv', 'Month,Region\nJan,EU\nFeb,US')),
    ).toThrow('numeric value column');
  });
  ...
  it('allows only the latest fallback generation to attach its image', async () => {
    ...
    await vi.waitFor(() =>
      expect(proposePatch).toHaveBeenCalledWith({
        fallbackImageUrl: 'https://site.example/new.png',
      }),
    );
    uploads.get(1)!({ url: 'https://site.example/old.png' });
    await Promise.resolve();

    expect(proposePatch).not.toHaveBeenCalledWith({
      fallbackImageUrl: 'https://site.example/old.png',
    });
  });
  ...
  it('invalidates pending work when canonical props change outside the settings handler', async () => {
    ...
    const workflow = createChartFallbackWorkflow({
      renderImage: vi.fn(
        async (_table, config) => new Uint8Array([config.valueColumn === 'Readers' ? 1 : 2]),
      ),
      uploadImage: vi.fn(
        ({ bytes }: { bytes: Uint8Array }) =>
          new Promise<{ url: string }>((resolve) => {
