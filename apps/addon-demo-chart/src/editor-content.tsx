...
function renderChart(request: AddonEditorBlockRequest) {
    ...
    return {
        ...
        portableContent: fallbackImageUrl
            ? (
                <figure>
                    <img alt={`${title} chart`} src={fallbackImageUrl} />
                    <figcaption>{title} chart.</figcaption>
                </figure>
            )
            : null,
        css: `
            .chart-card { box-sizing:border-box;margin:0;padding:22px;border:1px solid #dedee3;border-radius:18px;background:#fff;color:#242426;font-family:ui-sans-serif,system-ui,sans-serif }
