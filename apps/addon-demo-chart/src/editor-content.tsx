import {
  defineEditorBlockRenderer,
  type AddonEditorBlockRequest,
} from '@tryghost/addon-kit/editor';
import {
  chartDescription,
  chartTitle,
  isChartTable,
...
function Chart({ request }: { request: AddonEditorBlockRequest }) {
  ...
  const description = chartDescription(config);
  const svg = useMemo(
    () => renderStaticChart(table, config),
    [
      table,
      config.type,
      config.labelColumn,
...
function renderChart(request: AddonEditorBlockRequest) {
  ...
  const fallbackImageUrl =
    typeof request.props.fallbackImageUrl === 'string' ? request.props.fallbackImageUrl : '';
  const table = isChartTable(request.props.table) ? request.props.table : sampleChartTable;
  const config = readChartConfig(request.props, table);
    ...
    portableContent: fallbackImageUrl ? (
      ...
      >
        <img
          ...
          style={{ borderRadius: '10px', display: 'block', height: 'auto', width: '100%' }}
        />
        <figcaption
          style={{ color: '#626269', fontSize: '13px', lineHeight: '1.4', marginTop: '10px' }}
        >
          {description}
        </figcaption>
      </figure>
