...
export function createChartFallbackWorkflow({
  renderImage,
  uploadImage,
  proposePatch,
  onError,
}: ChartFallbackWorkflowDependencies) {
...
export function createChartConfigurationPatch(
  patch: Record<string, unknown>,
): Record<string, unknown> {
  return { ...patch, fallbackImageUrl: null };
}

export function createLabelColumnPatch(
  table: ChartTable,
  labelColumn: string,
  valueColumn: string,
): Record<string, unknown> {
  const availableValueColumns = numericValueColumns(table, labelColumn);
  const nextValueColumn = availableValueColumns.includes(valueColumn)
    ? valueColumn
