...
export class AddonSandboxController {
  ...
  async start({ staticExecution = false }: { staticExecution?: boolean } = {}): Promise<void> {
    ...
    const loaded = new Promise<void>((resolvePromise, rejectPromise) => {
      ...
      iframe.addEventListener(
        'error',
        () => rejectPromise(new Error('Add-on sandbox failed to load')),
        { once: true },
      );
    });
    ...
    const staticStartup = staticExecution
      ? new Promise<void>((resolvePromise, rejectPromise) => {
          ...
          const listener = (event: MessageEvent) => {
            ...
            if (event.data?.type === 'ghost-addon-worker-error') {
              removeStaticStartupListener();
              rejectPromise(
                new Error(event.data.message || 'Static add-on Worker failed to start'),
              );
            }
          };
          removeStaticStartupListener = () => {
  ...
  async loadBundle(
    options: Omit<Parameters<SandboxExports['loadBundle']>[0], 'source'>,
  ): Promise<void> {
  ...
  renderBlock(
    options: Parameters<SandboxExports['renderBlock']>[0],
  ): Promise<Awaited<ReturnType<SandboxExports['renderBlock']>>> {
