    ...
    <AlertDialog>
      ...
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            Its card, page, and sidebar entry are removed immediately. Data the add-on keeps on its
            own backend is not touched.
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
...
function DetailActions({ detail }: { detail: DetailSource }) {
  ...
  if (!install) {
    return (
      <Button
        onClick={() => navigate(`/apps/install?manifest=${encodeURIComponent(detail.manifestUrl)}`)}
      >
        Install
      </Button>
    );
  ...
  return (
    <>
      {hasPage && (
        <Button variant="outline" onClick={() => navigate(`/apps/${detail.handle}`)}>
          Open
        </Button>
      )}
      {install.dev ? (
...
export function AddonDetailPage() {
  ...
  const detail = install
    ...
    : catalogItem?.manifest
      ? detailFromManifest(catalogItem.manifest, catalogItem.manifestUrl)
      : null;
    ...
    return (
      <AddonPageChrome title="Add-on not found">
        <Text as="p" tone="secondary">
          No installed or listed add-on matches this address.
        </Text>
        <div className="mt-4">
          <Button variant="outline" onClick={() => navigate('/apps/marketplace')}>
            Back to marketplace
          </Button>
        </div>
      </AddonPageChrome>
