 ...
 */
export function useAddonSurface({
  install,
  target,
  context,
  ...
  const shouldRenderEntry = install.targeting.find(
    (entry) => entry.target === SHOULD_RENDER_PAIRS[target],
  );
    ...
    const boot = async () => {
      ...
      if (shouldRenderEntry) {
        await controller.loadBundle({
          ...
          integrity: shouldRenderEntry.integrity,
        });
