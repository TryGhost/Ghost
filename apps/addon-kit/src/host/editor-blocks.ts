  ...
  return {
    ...
    async renderBlock({ addonHandle, blockName, props }) {
      const block = blocks.find(
        (candidate) => candidate.addonHandle === addonHandle && candidate.blockName === blockName,
      );
      const install = installs.find(
        (candidate) => candidate.enabled && candidate.handle === addonHandle,
      );
      const editor = install?.editor;
      if (
        !block ||
        !editor ||
        typeof editor.contentBundleUrl !== 'string' ||
        editor.contentBundleUrl.length === 0
      ) {
