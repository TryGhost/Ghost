...
describe('createAddonEditorBlocksConfig', function () {
  ...
  it('passes site presentation context to content and settings renderers', async function () {
    ...
    const surface = config.createSettingsSurface!({
      ...
      uploadImage: vi
        .fn()
        .mockResolvedValue({ url: 'https://site.example/content/images/chart.png' }),
    });
    await surface.ready;
  ...
  it('creates a long-lived remote settings surface for one block', async function () {
    ...
    const uploadGeneratedImage =
      controller.renderSettings.mock.calls[0][0].capabilities.uploadImage;
    const image = { name: 'chart.png', type: 'image/png', bytes: new Uint8Array([1, 2, 3]) };
    await expect(uploadGeneratedImage(image)).resolves.toEqual({
