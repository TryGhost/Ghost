...
function wrapBootstrapAsString(): Plugin {
  return {
    ...
    async closeBundle() {
      ...
      await writeFile(
        resolve(outDir, 'bootstrap.mjs'),
        `export default ${JSON.stringify(code)};\n`,
      );
      await writeFile(
        resolve(outDir, 'bootstrap.d.mts'),
...
export default defineConfig({
  build: {
    outDir,
    // The editor-host build writes a second entry into the same directory.
    // Dev runs both builds concurrently, so neither may clear the other's output.
    emptyOutDir: false,
    lib: {
