        ...
        <article className="event-card">
            ...
            {(event.location || event.url) && (
                <footer>
                    ...
                    {event.url && <a href={event.url}>Event details <span aria-hidden="true">→</span></a>}
                </footer>
            )}
        </article>
    );
...
function PortableEventCard({event}: {event: EventDetails}) {
    ...
    return (
        <article style={{borderBottom: '1px solid #b9b9b5', borderTop: '1px solid #b9b9b5', color: '#181816', padding: '22px 0 24px'}}>
            <p style={{color: '#6f6f6b', fontSize: '11px', fontWeight: '700', letterSpacing: '.12em', margin: '0 0 14px', textTransform: 'uppercase'}}>Event</p>
            <h2 style={{fontSize: '28px', fontWeight: '400', letterSpacing: '-.015em', lineHeight: '1.15', margin: '0 0 12px'}}>{event.title}</h2>
            <p style={{fontSize: '14px', fontWeight: '700', lineHeight: '1.5', margin: '0'}}>
                <time dateTime={event.startsAt}>{start.date}{start.time ? ` · ${start.time}` : ''}</time>
                <span style={{color: '#6f6f6b', fontWeight: '400'}}> · {start.timezone}</span>
            </p>
            {event.description && <p style={{color: '#3f3f3c', fontSize: '16px', lineHeight: '1.55', margin: '16px 0 0'}}>{event.description}</p>}
            {event.location && <p style={{color: '#6f6f6b', fontSize: '14px', lineHeight: '1.5', margin: '18px 0 0'}}>{event.location}</p>}
            {event.url && <a href={event.url} style={{color: '#181816', display: 'inline-block', fontSize: '14px', fontWeight: '700', marginTop: '12px', textDecoration: 'underline', textUnderlineOffset: '3px'}}>Event details →</a>}
        </article>
    );
}
