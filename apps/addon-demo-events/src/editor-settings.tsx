...
function Settings({ ghost }: { ghost: AddonEditorSettingsBridge }) {
  ...
  return (
    <>
      <GhEditorInput
        label="Title"
        value={stringProperty(props, 'title')}
      ...
      <GhEditorInput
        label="Location"
        value={stringProperty(props, 'location')}
        onChange={(event) => void propose({ location: event.detail })}
      />
      <GhEditorInput
        label="Description"
        value={stringProperty(props, 'description')}
        onChange={(event) => void propose({ description: event.detail })}
      />
      <GhEditorInput
        label="Event URL"
        value={stringProperty(props, 'url')}
        onChange={(event) => void propose({ url: event.detail })}
      />
    </>
  );
}
