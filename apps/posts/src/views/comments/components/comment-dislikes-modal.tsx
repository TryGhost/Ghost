...
import {Comment, useBrowseCommentDislikes} from '@tryghost/admin-x-framework/api/comments';
import {CommentAvatar} from './comment-avatar';
import {LucideIcon, formatTimestamp} from '@tryghost/shade/utils';
import {formatMemberName} from '@tryghost/shade/app';

interface CommentDislikesModalProps {
    comment: Comment;
    open: boolean;
    onOpenChange: (open: boolean) => void;
}

function CommentDislikesModal({comment, open, onOpenChange}: CommentDislikesModalProps) {
    const {data, isLoading} = useBrowseCommentDislikes(comment.id, {enabled: open});
    const dislikes = data?.comment_dislikes ?? [];
    const dislikeCount = comment.count?.dislikes ?? 0;
    const remainingCount = dislikeCount - dislikes.length;
...
export default CommentDislikesModal;
