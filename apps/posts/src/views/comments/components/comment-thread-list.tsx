...
function CommentRow({comment, isReply = false}: {comment: Comment; isReply?: boolean}) {
    ...
    return (
        <>
            ...
            >
                <div className={cn('flex items-start gap-3', isReply && 'pl-[44px]')}>
                    <div className={`relative flex size-6 min-w-6 items-center justify-center overflow-hidden rounded-full bg-accent md:size-8 md:min-w-8 ${comment.status === 'hidden' && 'opacity-50'}`}>
                        {comment.member?.id && comment.member.avatar_image && (
                            <div className='absolute inset-0'><img alt="Member avatar" src={comment.member.avatar_image} /></div>
                        )}
                        <div>
