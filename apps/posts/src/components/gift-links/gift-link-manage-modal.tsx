...
const GiftLinkManageModal: React.FC<GiftLinkManageModalProps> = ({open, onOpenChange, postId, postUrl}) => {
    ...
    // The host keeps this modal mounted between opens, so clear transient copy
    // state whenever the target post or open state changes to avoid it bleeding
    // across posts.
    useEffect(() => {
        setCopied(false);
    }, [postId, open]);
                        ...
                        <div className="flex min-w-0 flex-col gap-4">
                            ...
                            <div className="flex items-center justify-between gap-2">
                                <span className="text-sm text-muted-foreground" data-testid="gift-link-count">
                                    Opened {formatNumber(count)} {count === 1 ? 'time' : 'times'}
                                </span>
                                <Button
