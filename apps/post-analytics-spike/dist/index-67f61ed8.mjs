function yi(t) {
  if (t.__esModule)
    return t;
  var e = t.default;
  if (typeof e == "function") {
    var n = function r() {
      return this instanceof r ? Reflect.construct(e, arguments, this.constructor) : e.apply(this, arguments);
    };
    n.prototype = e.prototype;
  } else
    n = {};
  return Object.defineProperty(n, "__esModule", { value: !0 }), Object.keys(t).forEach(function(r) {
    var o = Object.getOwnPropertyDescriptor(t, r);
    Object.defineProperty(n, r, o.get ? o : {
      enumerable: !0,
      get: function() {
        return t[r];
      }
    });
  }), n;
}
var Er = { exports: {} }, be = {};
const Ct = React.Children, Wr = React.Component, Ur = React.Fragment, pi = React.Profiler, Di = React.PureComponent, hi = React.StrictMode, wi = React.Suspense, Si = React.__SECRET_INTERNALS_DO_NOT_USE_OR_YOU_WILL_BE_FIRED, vi = React.act, yn = React.cloneElement, ht = React.createContext, i = React.createElement, Li = React.createFactory, Ai = React.createRef, U = React, Y = React.forwardRef, Zt = React.isValidElement, fi = React.lazy, Pr = React.memo, xi = React.startTransition, Ci = React.unstable_act, _ = React.useCallback, Et = React.useContext, Ti = React.useDebugValue, zi = React.useDeferredValue, V = React.useEffect, ki = React.useId, Zi = React.useImperativeHandle, Ei = React.useInsertionEffect, Wi = React.useLayoutEffect, pn = React.useMemo, _r = React.useReducer, Ui = React.useRef, bt = React.useState, Pi = React.useSyncExternalStore, _i = React.useTransition, Or = React.version, Oi = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  Children: Ct,
  Component: Wr,
  Fragment: Ur,
  Profiler: pi,
  PureComponent: Di,
  StrictMode: hi,
  Suspense: wi,
  __SECRET_INTERNALS_DO_NOT_USE_OR_YOU_WILL_BE_FIRED: Si,
  act: vi,
  cloneElement: yn,
  createContext: ht,
  createElement: i,
  createFactory: Li,
  createRef: Ai,
  default: U,
  forwardRef: Y,
  isValidElement: Zt,
  lazy: fi,
  memo: Pr,
  startTransition: xi,
  unstable_act: Ci,
  useCallback: _,
  useContext: Et,
  useDebugValue: Ti,
  useDeferredValue: zi,
  useEffect: V,
  useId: ki,
  useImperativeHandle: Zi,
  useInsertionEffect: Ei,
  useLayoutEffect: Wi,
  useMemo: pn,
  useReducer: _r,
  useRef: Ui,
  useState: bt,
  useSyncExternalStore: Pi,
  useTransition: _i,
  version: Or
}, Symbol.toStringTag, { value: "Module" })), Ri = /* @__PURE__ */ yi(Oi);
/**
 * @license React
 * react-jsx-runtime.production.min.js
 *
 * Copyright (c) Facebook, Inc. and its affiliates.
 *
 * This source code is licensed under the MIT license found in the
 * LICENSE file in the root directory of this source tree.
 */
var Hi = Ri, Gi = Symbol.for("react.element"), Yi = Symbol.for("react.fragment"), Qi = Object.prototype.hasOwnProperty, Ji = Hi.__SECRET_INTERNALS_DO_NOT_USE_OR_YOU_WILL_BE_FIRED.ReactCurrentOwner, Bi = { key: !0, ref: !0, __self: !0, __source: !0 };
function Rr(t, e, n) {
  var r, o = {}, s = null, a = null;
  n !== void 0 && (s = "" + n), e.key !== void 0 && (s = "" + e.key), e.ref !== void 0 && (a = e.ref);
  for (r in e)
    Qi.call(e, r) && !Bi.hasOwnProperty(r) && (o[r] = e[r]);
  if (t && t.defaultProps)
    for (r in e = t.defaultProps, e)
      o[r] === void 0 && (o[r] = e[r]);
  return { $$typeof: Gi, type: t, key: s, ref: a, props: o, _owner: Ji.current };
}
be.Fragment = Yi;
be.jsx = Rr;
be.jsxs = Rr;
Er.exports = be;
var b = Er.exports;
function Hr(t) {
  var e, n, r = "";
  if (typeof t == "string" || typeof t == "number")
    r += t;
  else if (typeof t == "object")
    if (Array.isArray(t)) {
      var o = t.length;
      for (e = 0; e < o; e++)
        t[e] && (n = Hr(t[e])) && (r && (r += " "), r += n);
    } else
      for (n in t)
        t[n] && (r && (r += " "), r += n);
  return r;
}
function Dn() {
  for (var t, e, n = 0, r = "", o = arguments.length; n < o; n++)
    (t = arguments[n]) && (e = Hr(t)) && (r && (r += " "), r += e);
  return r;
}
const Vi = (t) => /* @__PURE__ */ i("svg", { xmlns: "http://www.w3.org/2000/svg", viewBox: "0 0 24 24", ...t }, /* @__PURE__ */ i("defs", null), /* @__PURE__ */ i("title", null, "add"), /* @__PURE__ */ i("line", { x1: 0.75, y1: 12, x2: 23.25, y2: 12, fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: "1.5px" }), /* @__PURE__ */ i("line", { x1: 12, y1: 0.75, x2: 12, y2: 23.25, fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: "1.5px" })), Xi = "data:image/svg+xml;base64,", Fi = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: Vi,
  default: Xi
}, Symbol.toStringTag, { value: "Module" })), qi = (t) => /* @__PURE__ */ i("svg", { xmlns: "http://www.w3.org/2000/svg", fill: "none", viewBox: "-0.75 -0.75 24 24", height: 24, width: 24, ...t }, /* @__PURE__ */ i("path", { stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", d: "M13.341093749999999 17.55496875c2.03146875 -0.408375 3.667125 -2.0639062499999996 4.07615625 -4.14796875 0.40903125 2.0840625 2.0442187499999998 3.73959375 4.07578125 4.14796875m0 0.00234375c-2.0315624999999997 0.408375 -3.667125 2.0639062499999996 -4.07615625 4.14796875 -0.40903125 -2.0840625 -2.0443125 -3.73959375 -4.07578125 -4.14796875", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", d: "m19.54621875 12.32025 0.56521875 -0.56521875c0.53071875 -0.53071875 0.8272499999999999 -1.25146875 0.8236875 -2.00203125l-0.0271875 -5.777896875000001c-0.00721875 -1.5429374999999999 -1.25625 -2.791940625 -2.7991875 -2.799225l-5.778 -0.027290625c-0.7505625 -0.003553125 -1.4713124999999998 0.293034375 -2.00203125 0.82374375L1.32765 10.97353125c-0.732223125 0.7321875 -0.7322203125000001 1.91934375 0.000009375 2.6516249999999997l7.13105625 7.131c0.732234375 0.73228125 1.9194093749999999 0.73228125 2.6516906249999996 0l0.94190625 -0.94190625", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeMiterlimit: 10, d: "M17.75428125 4.329000000000001c-0.1393125 -0.13935 -0.41803125 -0.139359375 -0.5574375 0 -0.1393125 0.13935 -0.1393125 0.418059375 0 0.557409375", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeMiterlimit: 10, d: "M17.7553125 4.328221875c0.13940625 0.13935 0.13940625 0.418059375 0 0.55741875 -0.1393125 0.13935 -0.41803125 0.13934062500000002 -0.55734375 -0.000009375", strokeWidth: 1.5 })), $i = "data:image/svg+xml;base64,", Ki = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: qi,
  default: $i
}, Symbol.toStringTag, { value: "Module" })), ts = (t) => /* @__PURE__ */ i("svg", { xmlns: "http://www.w3.org/2000/svg", fill: "none", viewBox: "0 0 10 10", ...t }, /* @__PURE__ */ i("path", { stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", d: "M9 1.5H1" }), /* @__PURE__ */ i("path", { stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", d: "M7.5 5h-5" }), /* @__PURE__ */ i("path", { stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", d: "M9 8.5H1" })), es = "data:image/svg+xml;base64,", ns = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: ts,
  default: es
}, Symbol.toStringTag, { value: "Module" })), rs = (t) => /* @__PURE__ */ i("svg", { xmlns: "http://www.w3.org/2000/svg", fill: "none", viewBox: "0 0 10 10", ...t }, /* @__PURE__ */ i("path", { stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", d: "M1 1.5h8" }), /* @__PURE__ */ i("path", { stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", d: "M1 5h5.5" }), /* @__PURE__ */ i("path", { stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", d: "M1 8.5h8" })), os = "data:image/svg+xml;base64,", is = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: rs,
  default: os
}, Symbol.toStringTag, { value: "Module" })), ss = (t) => /* @__PURE__ */ i("svg", { viewBox: "-0.75 -0.75 24 24", xmlns: "http://www.w3.org/2000/svg", height: 24, width: 24, ...t }, /* @__PURE__ */ i("path", { d: "m7.152187499999999 4.21875 -6.0375000000000005 6.0365625000000005a1.40625 1.40625 0 0 0 0 1.9884375l6.0375000000000005 6.0375000000000005", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { d: "m15.347812499999998 4.21875 6.0375000000000005 6.0365625000000005a1.40625 1.40625 0 0 1 0 1.9884375l-6.0375000000000005 6.0375000000000005", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 })), as = "data:image/svg+xml;base64,", cs = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: ss,
  default: as
}, Symbol.toStringTag, { value: "Module" })), ls = (t) => /* @__PURE__ */ i("svg", { id: "Regular", xmlns: "http://www.w3.org/2000/svg", viewBox: "0 0 24 24", strokeWidth: 1.5, ...t }, /* @__PURE__ */ i("defs", null), /* @__PURE__ */ i("title", null, "arrow-corner-left"), /* @__PURE__ */ i("path", { d: "M20.16 3.75 4.25 19.66", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round" }), /* @__PURE__ */ i("path", { d: "m4.25 4.66 0 15 15 0", fillRule: "evenodd", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round" })), us = "data:image/svg+xml;base64,", ds = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: ls,
  default: us
}, Symbol.toStringTag, { value: "Module" })), Ms = (t) => /* @__PURE__ */ i("svg", { id: "Regular", xmlns: "http://www.w3.org/2000/svg", viewBox: "0 0 24 24", strokeWidth: 1.5, ...t }, /* @__PURE__ */ i("defs", null), /* @__PURE__ */ i("title", null, "arrow-corner-right"), /* @__PURE__ */ i("path", { d: "m4 3.75 15.91 15.91", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round" }), /* @__PURE__ */ i("path", { d: "m4.91 19.66 15 0 0-15", fillRule: "evenodd", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round" })), Is = "data:image/svg+xml;base64,", gs = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: Ms,
  default: Is
}, Symbol.toStringTag, { value: "Module" })), js = (t) => /* @__PURE__ */ i("svg", { id: "Regular", xmlns: "http://www.w3.org/2000/svg", viewBox: "0 0 24 24", ...t }, /* @__PURE__ */ i("defs", null), /* @__PURE__ */ i("title", null, "arrow-down"), /* @__PURE__ */ i("line", { x1: 12, y1: 0.75, x2: 12, y2: 23.25, fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: "1.5px" }), /* @__PURE__ */ i("polyline", { points: "1.5 12.75 12 23.25 22.5 12.75", fillRule: "evenodd", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: "1.5px" })), Ns = "data:image/svg+xml;base64,", ms = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: js,
  default: Ns
}, Symbol.toStringTag, { value: "Module" })), bs = (t) => /* @__PURE__ */ i("svg", { id: "Regular", xmlns: "http://www.w3.org/2000/svg", viewBox: "0 0 24 24", ...t }, /* @__PURE__ */ i("defs", null), /* @__PURE__ */ i("title", null, "arrow-left"), /* @__PURE__ */ i("line", { x1: 23.25, y1: 12, x2: 0.75, y2: 12, fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: "1.5px" }), /* @__PURE__ */ i("polyline", { points: "11.25 1.5 0.75 12 11.25 22.5", fillRule: "evenodd", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: "1.5px" })), ys = "data:image/svg+xml;base64,", ps = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: bs,
  default: ys
}, Symbol.toStringTag, { value: "Module" })), Ds = (t) => /* @__PURE__ */ i("svg", { id: "Regular", xmlns: "http://www.w3.org/2000/svg", viewBox: "0 0 24 24", ...t }, /* @__PURE__ */ i("defs", null), /* @__PURE__ */ i("title", null, "arrow-right"), /* @__PURE__ */ i("line", { x1: 0.75, y1: 12, x2: 23.25, y2: 12, fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: "1.5px" }), /* @__PURE__ */ i("polyline", { points: "12.75 22.5 23.25 12 12.75 1.5", fillRule: "evenodd", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: "1.5px" })), hs = "data:image/svg+xml;base64,", ws = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: Ds,
  default: hs
}, Symbol.toStringTag, { value: "Module" })), Ss = (t) => /* @__PURE__ */ i("svg", { width: 24, height: 24, viewBox: "0 0 24 24", fill: "none", xmlns: "http://www.w3.org/2000/svg", ...t }, /* @__PURE__ */ i("path", { d: "M20.16 20.25L4.25 4.34", stroke: "currentColor", strokeWidth: 1.5, strokeLinecap: "round", strokeLinejoin: "round" }), /* @__PURE__ */ i("path", { d: "M4.25 19.3398V4.33984H19.25", stroke: "currentColor", strokeWidth: 1.5, strokeLinecap: "round", strokeLinejoin: "round" })), vs = "data:image/svg+xml;base64,", Ls = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: Ss,
  default: vs
}, Symbol.toStringTag, { value: "Module" })), As = (t) => /* @__PURE__ */ i("svg", { width: 24, height: 24, viewBox: "0 0 24 24", fill: "none", xmlns: "http://www.w3.org/2000/svg", ...t }, /* @__PURE__ */ i("path", { d: "M3.84 20.25L19.75 4.34", stroke: "currentColor", strokeWidth: 1.5, strokeLinecap: "round", strokeLinejoin: "round" }), /* @__PURE__ */ i("path", { d: "M19.75 19.3398V4.33984H4.75", stroke: "currentColor", strokeWidth: 1.5, strokeLinecap: "round", strokeLinejoin: "round" })), fs = "data:image/svg+xml;base64,", xs = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: As,
  default: fs
}, Symbol.toStringTag, { value: "Module" })), Cs = (t) => /* @__PURE__ */ i("svg", { id: "Regular", xmlns: "http://www.w3.org/2000/svg", viewBox: "0 0 24 24", ...t }, /* @__PURE__ */ i("defs", null), /* @__PURE__ */ i("title", null, "arrow-up"), /* @__PURE__ */ i("line", { x1: 12, y1: 23.25, x2: 12, y2: 0.75, fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: "1.5px" }), /* @__PURE__ */ i("polyline", { points: "22.5 11.25 12 0.75 1.5 11.25", fillRule: "evenodd", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: "1.5px" })), Ts = "data:image/svg+xml;base64,", zs = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: Cs,
  default: Ts
}, Symbol.toStringTag, { value: "Module" })), ks = (t) => /* @__PURE__ */ i("svg", { viewBox: "-0.75 -0.75 24 24", xmlns: "http://www.w3.org/2000/svg", height: 24, width: 24, ...t }, /* @__PURE__ */ i("path", { d: "M16.171875 11.25A4.921875 4.921875 0 1 1 11.25 6.328125 4.921875 4.921875 0 0 1 16.171875 11.25Z", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { d: "M16.171875 11.25v2.109375a2.8125 2.8125 0 0 0 5.625 0V11.25a10.5459375 10.5459375 0 1 0 -4.21875 8.4375", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 })), Zs = "data:image/svg+xml;base64,", Es = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: ks,
  default: Zs
}, Symbol.toStringTag, { value: "Module" })), Ws = (t) => /* @__PURE__ */ i("svg", { xmlns: "http://www.w3.org/2000/svg", fill: "none", viewBox: "-0.75 -0.75 24 24", height: 24, width: 24, ...t }, /* @__PURE__ */ i("path", { stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", d: "M0.9375 20.0625h1.8403125", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", d: "M19.723125 20.0625H21.5625", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", d: "M15.02625 20.0625h1.8403125", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", d: "M10.3303125 20.0625h1.839375", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", d: "M5.6343749999999995 20.0625h1.839375", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", d: "m0.9375 16.53 4.790625 -6.511875a3.1565625 3.1565625 0 0 1 3.1753125 -1.2225000000000001l4.685625 0.9590624999999999a3.1565625 3.1565625 0 0 0 3.17625 -1.2215624999999999l4.790625 -6.511875", strokeWidth: 1.5 })), Us = "data:image/svg+xml;base64,", Ps = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: Ws,
  default: Us
}, Symbol.toStringTag, { value: "Module" })), _s = (t) => /* @__PURE__ */ i("svg", { xmlns: "http://www.w3.org/2000/svg", viewBox: "0 0 24 24", id: "Alarm-Bell--Streamline-Streamline--3.0", height: 24, width: 24, ...t }, /* @__PURE__ */ i("desc", null, "Alarm Bell Streamline Icon: https://streamlinehq.com"), /* @__PURE__ */ i("defs", null), /* @__PURE__ */ i("title", null, "alarm-bell"), /* @__PURE__ */ i("path", { d: "M10 21.75a2.087 2.087 0 0 0 4.005 0", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { d: "m12 3 0 -2.25", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { d: "M12 3a7.5 7.5 0 0 1 7.5 7.5c0 7.046 1.5 8.25 1.5 8.25H3s1.5 -1.916 1.5 -8.25A7.5 7.5 0 0 1 12 3Z", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 })), Os = "data:image/svg+xml;base64,", Rs = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: _s,
  default: Os
}, Symbol.toStringTag, { value: "Module" })), Hs = (t) => /* @__PURE__ */ i("svg", { xmlns: "http://www.w3.org/2000/svg", fill: "none", viewBox: "-0.75 -0.75 24 24", height: 24, width: 24, ...t }, /* @__PURE__ */ i("path", { stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", d: "M17.578125 4.21875H2.109375A1.40625 1.40625 0 0 0 0.703125 5.625v8.4375a1.40625 1.40625 0 0 0 1.40625 1.40625h15.46875a1.40625 1.40625 0 0 0 1.40625 -1.40625V5.625a1.40625 1.40625 0 0 0 -1.40625 -1.40625Z", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { stroke: "currentColor", d: "M3.8671875 7.734375a0.3515625 0.3515625 0 1 1 0 -0.703125", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { stroke: "currentColor", d: "M3.8671875 7.734375a0.3515625 0.3515625 0 1 0 0 -0.703125", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { stroke: "currentColor", d: "M15.8203125 12.65625a0.3515625 0.3515625 0 0 1 0 -0.703125", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { stroke: "currentColor", d: "M15.8203125 12.65625a0.3515625 0.3515625 0 0 0 0 -0.703125", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", d: "M9.84375 12.65625a2.8125 2.8125 0 1 0 0 -5.625 2.8125 2.8125 0 0 0 0 5.625Z", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", d: "M21.796875 8.4375v8.4375a1.40625 1.40625 0 0 1 -1.40625 1.40625H4.921875", strokeWidth: 1.5 })), Gs = "data:image/svg+xml;base64,", Ys = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: Hs,
  default: Gs
}, Symbol.toStringTag, { value: "Module" })), Qs = (t) => /* @__PURE__ */ i("svg", { viewBox: "-0.75 -0.75 24 24", xmlns: "http://www.w3.org/2000/svg", height: 24, width: 24, ...t }, /* @__PURE__ */ i("path", { d: "M12.1875 21.474375a15.9271875 15.9271875 0 0 1 8.3025 -3.646875 1.5 1.5 0 0 0 1.3040625000000001 -1.4878125V2.2171875a1.5121875 1.5121875 0 0 0 -1.7203125 -1.5A16.009687500000002 16.009687500000002 0 0 0 12.1875 4.3125a1.53375 1.53375 0 0 1 -1.875 0A16.009687500000002 16.009687500000002 0 0 0 2.4234375 0.7190625 1.5121875 1.5121875 0 0 0 0.703125 2.2171875v14.1225a1.5 1.5 0 0 0 1.3040625000000001 1.4878125A15.9271875 15.9271875 0 0 1 10.3125 21.474375a1.5309375 1.5309375 0 0 0 1.875 0Z", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { d: "m11.25 4.629375 0 17.1665625", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 })), Js = "data:image/svg+xml;base64,", Bs = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: Qs,
  default: Js
}, Symbol.toStringTag, { value: "Module" })), Vs = (t) => /* @__PURE__ */ i("svg", { viewBox: "-0.75 -0.75 24 24", xmlns: "http://www.w3.org/2000/svg", height: 24, width: 24, ...t }, /* @__PURE__ */ i("path", { d: "m7.152187499999999 4.21875 -6.0375000000000005 6.0365625000000005a1.40625 1.40625 0 0 0 0 1.9884375l6.0375000000000005 6.0375000000000005", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { d: "m15.347812499999998 4.21875 6.0375000000000005 6.0365625000000005a1.40625 1.40625 0 0 1 0 1.9884375l-6.0375000000000005 6.0375000000000005", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 })), Xs = "data:image/svg+xml;base64,", Fs = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: Vs,
  default: Xs
}, Symbol.toStringTag, { value: "Module" })), qs = (t) => /* @__PURE__ */ i("svg", { xmlns: "http://www.w3.org/2000/svg", viewBox: "0 0 24 24", id: "Layout-Agenda--Streamline-Ultimate", height: 24, width: 24, ...t }, /* @__PURE__ */ i("desc", null, "Layout Agenda Streamline Icon: https://streamlinehq.com"), /* @__PURE__ */ i("defs", null), /* @__PURE__ */ i("title", null, "layout-agenda"), /* @__PURE__ */ i("path", { d: "M2.25 0.747h19.5s1.5 0 1.5 1.5v6s0 1.5 -1.5 1.5H2.25s-1.5 0 -1.5 -1.5v-6s0 -1.5 1.5 -1.5", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { d: "M2.25 14.247h19.5s1.5 0 1.5 1.5v6s0 1.5 -1.5 1.5H2.25s-1.5 0 -1.5 -1.5v-6s0 -1.5 1.5 -1.5", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 })), $s = "data:image/svg+xml;base64,", Ks = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: qs,
  default: $s
}, Symbol.toStringTag, { value: "Module" })), ta = (t) => /* @__PURE__ */ i("svg", { xmlns: "http://www.w3.org/2000/svg", viewBox: "-0.75 -0.75 24 24", height: 24, width: 24, ...t }, /* @__PURE__ */ i("defs", null), /* @__PURE__ */ i("title", null, "layout-module-1"), /* @__PURE__ */ i("path", { d: "M2.109375 0.7003125h5.625s1.40625 0 1.40625 1.40625v5.625s0 1.40625 -1.40625 1.40625h-5.625s-1.40625 0 -1.40625 -1.40625v-5.625s0 -1.40625 1.40625 -1.40625", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { d: "M2.109375 13.356562499999999h5.625s1.40625 0 1.40625 1.40625v5.625s0 1.40625 -1.40625 1.40625h-5.625s-1.40625 0 -1.40625 -1.40625v-5.625s0 -1.40625 1.40625 -1.40625", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { d: "M14.765625 0.7003125h5.625s1.40625 0 1.40625 1.40625v5.625s0 1.40625 -1.40625 1.40625h-5.625s-1.40625 0 -1.40625 -1.40625v-5.625s0 -1.40625 1.40625 -1.40625", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { d: "M14.765625 13.356562499999999h5.625s1.40625 0 1.40625 1.40625v5.625s0 1.40625 -1.40625 1.40625h-5.625s-1.40625 0 -1.40625 -1.40625v-5.625s0 -1.40625 1.40625 -1.40625", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 })), ea = "data:image/svg+xml;base64,", na = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: ta,
  default: ea
}, Symbol.toStringTag, { value: "Module" })), ra = (t) => /* @__PURE__ */ i("svg", { viewBox: "0 0 24 24", xmlns: "http://www.w3.org/2000/svg", strokeWidth: 1.5, ...t }, /* @__PURE__ */ i("path", { d: "M6,13.223,8.45,16.7a1.049,1.049,0,0,0,1.707.051L18,6.828", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round" }), /* @__PURE__ */ i("path", { d: "M0.750 11.999 A11.250 11.250 0 1 0 23.250 11.999 A11.250 11.250 0 1 0 0.750 11.999 Z", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round" })), oa = "data:image/svg+xml;base64,", ia = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: ra,
  default: oa
}, Symbol.toStringTag, { value: "Module" })), sa = (t) => /* @__PURE__ */ i("svg", { viewBox: "0 0 24 24", xmlns: "http://www.w3.org/2000/svg", strokeWidth: 1.5, ...t }, /* @__PURE__ */ i("path", { style: {
  fill: "none",
  stroke: "currentColor",
  strokeLinecap: "round",
  strokeLinejoin: "round"
}, d: "m1.6 14.512 7.065 7.065 13.676-19", fill: "none", strokeLinecap: "round", strokeLinejoin: "round" })), aa = "data:image/svg+xml;base64,", ca = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: sa,
  default: aa
}, Symbol.toStringTag, { value: "Module" })), la = (t) => /* @__PURE__ */ i("svg", { id: "Regular", xmlns: "http://www.w3.org/2000/svg", viewBox: "0 0 24 24", ...t }, /* @__PURE__ */ i("defs", null), /* @__PURE__ */ i("title", null, "arrow-down-1"), /* @__PURE__ */ i("path", { d: "M23.25,7.311,12.53,18.03a.749.749,0,0,1-1.06,0L.75,7.311", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: "1.5px", fillRule: "evenodd" })), ua = "data:image/svg+xml;base64,", da = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: la,
  default: ua
}, Symbol.toStringTag, { value: "Module" })), Ma = (t) => /* @__PURE__ */ i("svg", { id: "Regular", xmlns: "http://www.w3.org/2000/svg", viewBox: "0 0 24 24", ...t }, /* @__PURE__ */ i("defs", null), /* @__PURE__ */ i("title", null, "arrow-left-1"), /* @__PURE__ */ i("path", { d: "M16.25,23.25,5.53,12.53a.749.749,0,0,1,0-1.06L16.25.75", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: "1.5px", fillRule: "evenodd" })), Ia = "data:image/svg+xml;base64,", ga = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: Ma,
  default: Ia
}, Symbol.toStringTag, { value: "Module" })), ja = (t) => /* @__PURE__ */ i("svg", { id: "Regular", xmlns: "http://www.w3.org/2000/svg", viewBox: "0 0 24 24", ...t }, /* @__PURE__ */ i("defs", null), /* @__PURE__ */ i("title", null, "arrow-right-1"), /* @__PURE__ */ i("path", { d: "M5.5.75,16.22,11.47a.749.749,0,0,1,0,1.06L5.5,23.25", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: "1.5px", fillRule: "evenodd" })), Na = "data:image/svg+xml;base64,", ma = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: ja,
  default: Na
}, Symbol.toStringTag, { value: "Module" })), ba = (t) => /* @__PURE__ */ i("svg", { id: "Regular", xmlns: "http://www.w3.org/2000/svg", viewBox: "0 0 24 24", ...t }, /* @__PURE__ */ i("defs", null), /* @__PURE__ */ i("title", null, "arrow-up-1"), /* @__PURE__ */ i("path", { d: "M.75,17.189,11.47,6.47a.749.749,0,0,1,1.06,0L23.25,17.189", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: "1.5px", fillRule: "evenodd" })), ya = "data:image/svg+xml;base64,", pa = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: ba,
  default: ya
}, Symbol.toStringTag, { value: "Module" })), Da = (t) => /* @__PURE__ */ i("svg", { xmlns: "http://www.w3.org/2000/svg", viewBox: "0 0 24 24", ...t }, /* @__PURE__ */ i("defs", null), /* @__PURE__ */ i("title", null, "close"), /* @__PURE__ */ i("line", { x1: 0.75, y1: 23.249, x2: 23.25, y2: 0.749, fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: "1.5px" }), /* @__PURE__ */ i("line", { x1: 23.25, y1: 23.249, x2: 0.75, y2: 0.749, fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: "1.5px" })), ha = "data:image/svg+xml;base64,", wa = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: Da,
  default: ha
}, Symbol.toStringTag, { value: "Module" })), Sa = (t) => /* @__PURE__ */ i("svg", { xmlns: "http://www.w3.org/2000/svg", fill: "none", viewBox: "0 0 24 24", id: "Layout--Streamline-Ultimate", height: 24, width: 24, ...t }, /* @__PURE__ */ i("desc", null, "Layout Streamline Icon: https://streamlinehq.com"), /* @__PURE__ */ i("path", { stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", d: "m1.5 2.99707 0 18.00003c0 0.8284 0.67157 1.5 1.5 1.5l18 0c0.8284 0 1.5 -0.6716 1.5 -1.5l0 -18.00003c0 -0.82843 -0.6716 -1.5 -1.5 -1.5l-18 0c-0.82843 0 -1.5 0.67157 -1.5 1.5Z", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", d: "m12.0029 22.4971 0 -21.00003", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", d: "m12.0029 11.9971 10 0", strokeWidth: 1.5 })), va = "data:image/svg+xml;base64,", La = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: Sa,
  default: va
}, Symbol.toStringTag, { value: "Module" })), Aa = (t) => /* @__PURE__ */ i("svg", { xmlns: "http://www.w3.org/2000/svg", viewBox: "0 0 24 24", ...t }, /* @__PURE__ */ i("path", { d: "M12 1.34C5.66 1.34 0.5 5.59 0.5 10.81a8.58 8.58 0 0 0 3.18 6.54l-2.3 4.59a0.49 0.49 0 0 0 0.09 0.57 0.5 0.5 0 0 0 0.57 0.1l6.15 -2.86a13.44 13.44 0 0 0 3.81 0.54c6.34 0 11.5 -4.25 11.5 -9.48S18.34 1.34 12 1.34Z", fill: "currentColor", strokeWidth: "1.5px" })), fa = "data:image/svg+xml;base64,", xa = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: Aa,
  default: fa
}, Symbol.toStringTag, { value: "Module" })), Ca = (t) => /* @__PURE__ */ i("svg", { xmlns: "http://www.w3.org/2000/svg", fill: "none", viewBox: "0 0 24 24", ...t }, /* @__PURE__ */ i("path", { stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: "1.5px", d: "M12.658 2a9.307 9.307 0 0 0-8.15 4.788 9.326 9.326 0 0 0 .23 9.456L2 22l5.75-2.74a9.32 9.32 0 0 0 13.894-5.372 9.341 9.341 0 0 0-1.532-8.185A9.328 9.328 0 0 0 12.658 2Z" })), Ta = "data:image/svg+xml;base64,", za = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: Ca,
  default: Ta
}, Symbol.toStringTag, { value: "Module" })), ka = (t) => /* @__PURE__ */ i("svg", { xmlns: "http://www.w3.org/2000/svg", viewBox: "0 0 24 24", strokeWidth: "1.5px", ...t }, /* @__PURE__ */ i("g", null, /* @__PURE__ */ i("path", { d: "M21.92,17l1.32-10a.75.75,0,0,0-1.08-.78L17.88,9.56a.74.74,0,0,1-1.09-.16L12.56,3.22a.74.74,0,0,0-1.12,0L7.21,9.4a.74.74,0,0,1-1.09.16L1.84,6.3a.75.75,0,0,0-1.08.78L2.08,17Z", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round" }), /* @__PURE__ */ i("line", { x1: 2.25, y1: 21.03, x2: 21.75, y2: 21.03, fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round" }))), Za = "data:image/svg+xml;base64,", Ea = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: ka,
  default: Za
}, Symbol.toStringTag, { value: "Module" })), Wa = (t) => /* @__PURE__ */ i("svg", { viewBox: "-0.75 -0.75 24 24", xmlns: "http://www.w3.org/2000/svg", height: 24, width: 24, ...t }, /* @__PURE__ */ i("path", { d: "m2.109375 20.390625 18.28125 -18.28125Z", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { d: "M14.765625 17.578125a2.8125 2.8125 0 1 0 5.625 0 2.8125 2.8125 0 1 0 -5.625 0Z", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { d: "M2.109375 4.921875a2.8125 2.8125 0 1 0 5.625 0 2.8125 2.8125 0 1 0 -5.625 0Z", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 })), Ua = "data:image/svg+xml;base64,", Pa = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: Wa,
  default: Ua
}, Symbol.toStringTag, { value: "Module" })), _a = (t) => /* @__PURE__ */ i("svg", { xmlns: "http://www.w3.org/2000/svg", viewBox: "0 0 24 24", ...t }, /* @__PURE__ */ i("defs", null, /* @__PURE__ */ i("style", null, `
      circle{fill:currentColor}
    `)), /* @__PURE__ */ i("circle", { cx: 3.25, cy: 12, r: 2.6 }), /* @__PURE__ */ i("circle", { cx: 12, cy: 12, r: 2.6 }), /* @__PURE__ */ i("circle", { cx: 20.75, cy: 12, r: 2.6 })), Oa = "data:image/svg+xml;base64,", Ra = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: _a,
  default: Oa
}, Symbol.toStringTag, { value: "Module" })), Ha = (t) => /* @__PURE__ */ i("svg", { viewBox: "-0.75 -0.75 24 24", xmlns: "http://www.w3.org/2000/svg", height: 24, width: 24, ...t }, /* @__PURE__ */ i("path", { d: "m11.2509375 3.515625 0 11.25", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { d: "m7.0321875 10.546875 4.21875 4.21875 4.21875 -4.21875", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { d: "M21.797812500000003 14.765625v1.40625a2.8125 2.8125 0 0 1 -2.8125 2.8125h-15.46875a2.8125 2.8125 0 0 1 -2.8125 -2.8125v-1.40625", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 })), Ga = "data:image/svg+xml;base64,", Ya = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: Ha,
  default: Ga
}, Symbol.toStringTag, { value: "Module" })), Qa = (t) => /* @__PURE__ */ i("svg", { xmlns: "http://www.w3.org/2000/svg", fill: "none", viewBox: "-0.75 -0.75 24 24", height: 24, width: 24, ...t }, /* @__PURE__ */ i("path", { stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", d: "M15.703125 4.21875V1.640625a0.9375 0.9375 0 0 0 -0.9375 -0.9375h-13.125a0.9375 0.9375 0 0 0 -0.9375 0.9375v13.125a0.9375 0.9375 0 0 0 0.9375 0.9375H4.21875", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { stroke: "currentColor", strokeLinejoin: "round", d: "M6.796875 7.734375a0.9375 0.9375 0 0 1 0.9375 -0.9375h13.125a0.9375 0.9375 0 0 1 0.9375 0.9375v13.125a0.9375 0.9375 0 0 1 -0.9375 0.9375h-13.125a0.9375 0.9375 0 0 1 -0.9375 -0.9375v-13.125Z", strokeWidth: 1.5 })), Ja = "data:image/svg+xml;base64,", Ba = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: Qa,
  default: Ja
}, Symbol.toStringTag, { value: "Module" })), Va = (t) => /* @__PURE__ */ i("svg", { width: 24, height: 24, viewBox: "0 0 24 24", fill: "none", xmlns: "http://www.w3.org/2000/svg", ...t }, /* @__PURE__ */ i("circle", { cx: 6, cy: 12, r: 1.5, fill: "currentColor" }), /* @__PURE__ */ i("circle", { cx: 12, cy: 12, r: 1.5, fill: "currentColor" }), /* @__PURE__ */ i("path", { d: "M19.5 12C19.5 12.8284 18.8284 13.5 18 13.5C17.1716 13.5 16.5 12.8284 16.5 12C16.5 11.1716 17.1716 10.5 18 10.5C18.8284 10.5 19.5 11.1716 19.5 12Z", fill: "currentColor" })), Xa = "data:image/svg+xml;base64,", Fa = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: Va,
  default: Xa
}, Symbol.toStringTag, { value: "Module" })), qa = (t) => /* @__PURE__ */ i("svg", { viewBox: "-0.75 -0.75 24 24", xmlns: "http://www.w3.org/2000/svg", height: 24, width: 24, ...t }, /* @__PURE__ */ i("path", { d: "M10.546875 16.171875a5.625 5.625 0 1 0 11.25 0 5.625 5.625 0 1 0 -11.25 0Z", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { d: "m18.67875 14.536875 -2.7234374999999997 3.6309375000000004a0.705 0.705 0 0 1 -1.0603125 0.0759375l-1.40625 -1.40625", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { d: "M7.734375 14.765625h-5.625a1.40625 1.40625 0 0 1 -1.40625 -1.40625v-11.25a1.40625 1.40625 0 0 1 1.40625 -1.40625h16.875a1.40625 1.40625 0 0 1 1.40625 1.40625V8.4375", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { d: "m20.0728125 1.21875 -7.635 5.8725000000000005a3.10125 3.10125 0 0 1 -3.781875 0L1.0209375 1.21875", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 })), $a = "data:image/svg+xml;base64,", Ka = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: qa,
  default: $a
}, Symbol.toStringTag, { value: "Module" })), tc = (t) => /* @__PURE__ */ i("svg", { viewBox: "-0.75 -0.75 24 24", xmlns: "http://www.w3.org/2000/svg", height: 24, width: 24, ...t }, /* @__PURE__ */ i("path", { d: "m1.40625 4.453125 19.6875 0 0 14.0625 -19.6875 0Z", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { d: "m20.7759375 4.96875 -7.635 5.8725000000000005a3.10125 3.10125 0 0 1 -3.781875 0L1.7240625 4.96875", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 })), ec = "data:image/svg+xml;base64,", nc = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: tc,
  default: ec
}, Symbol.toStringTag, { value: "Module" })), rc = (t) => /* @__PURE__ */ i("svg", { xmlns: "http://www.w3.org/2000/svg", viewBox: "-0.75 -0.75 24 24", height: 24, width: 24, ...t }, /* @__PURE__ */ i("defs", null), /* @__PURE__ */ i("path", { d: "M21.796875 12.421875v5.859375a0.9375 0.9375 0 0 1 -0.9375 0.9375H1.640625a0.9375 0.9375 0 0 1 -0.9375 -0.9375V8.671875a0.9375 0.9375 0 0 1 0.9375 -0.9375H8.4375", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { d: "M18.125625 13.300312499999999A5.15625 5.15625 0 1 1 21.5625 8.4375", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { d: "M14.6878125 8.4375a1.7184375 1.7184375 0 1 0 3.436875 0 1.7184375 1.7184375 0 1 0 -3.436875 0", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { d: "M18.1246875 8.4375A1.719375 1.719375 0 0 0 21.5625 8.4375", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { d: "m4.3706249999999995 10.9378125 0 5.077500000000001", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 })), oc = "data:image/svg+xml;base64,", ic = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: rc,
  default: oc
}, Symbol.toStringTag, { value: "Module" })), sc = (t) => /* @__PURE__ */ i("svg", { xmlns: "http://www.w3.org/2000/svg", viewBox: "0 0 24 24", height: 24, width: 24, id: "Alert-Triangle--Streamline-Ultimate", ...t }, /* @__PURE__ */ i("desc", null, "Alert Triangle Streamline Icon: https://streamlinehq.com"), /* @__PURE__ */ i("path", { d: "m23.77 20.57 -10 -19A2 2 0 0 0 12 0.5a2 2 0 0 0 -1.77 1.07l-10 19a2 2 0 0 0 0.06 2A2 2 0 0 0 2 23.5h20a2 2 0 0 0 1.77 -2.93ZM11 8.5a1 1 0 0 1 2 0v6a1 1 0 0 1 -2 0ZM12.05 20a1.53 1.53 0 0 1 -1.52 -1.47A1.48 1.48 0 0 1 12 17a1.53 1.53 0 0 1 1.52 1.47A1.48 1.48 0 0 1 12.05 20Z", fill: "currentColor", strokeWidth: 1 })), ac = "data:image/svg+xml;base64,", cc = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: sc,
  default: ac
}, Symbol.toStringTag, { value: "Module" })), lc = (t) => /* @__PURE__ */ i("svg", { xmlns: "http://www.w3.org/2000/svg", fill: "none", viewBox: "-0.75 -0.75 24 24", height: 24, width: 24, ...t }, /* @__PURE__ */ i("path", { stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", d: "M18.09646875 20.3938125c0.674625 0 1.219125 -0.54459375 1.219125 -1.21921875V5.666521875c0 -0.325096875 -0.13003125 -0.6420750000000001 -0.36571875 -0.8696531249999999l-2.43825 -2.34075c-0.227625 -0.227578125 -0.5364375 -0.349490625 -0.85340625 -0.349490625H4.4042625c-0.674596875 0 -1.21914375 0.544546875 -1.21914375 1.21914375V19.17459375c0 0.674625 0.544546875 1.21921875 1.21914375 1.21921875H18.09646875Z", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", d: "m8.476865625 12.861375 2.774446875 2.77453125 2.77453125 -2.77453125", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", d: "m11.2490625 15.63534375 0 -8.770715625", strokeWidth: 1.5 })), uc = "data:image/svg+xml;base64,", dc = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: lc,
  default: uc
}, Symbol.toStringTag, { value: "Module" })), Mc = (t) => /* @__PURE__ */ i("svg", { xmlns: "http://www.w3.org/2000/svg", fill: "none", viewBox: "0 0 24 24", ...t }, /* @__PURE__ */ i("path", { stroke: "currentColor", strokeLinejoin: "round", strokeWidth: 2, d: "M17.041 12.025 6.91 22.156 1 23l.844-5.91L11.975 6.96m0-5.067 10.132 10.132" }), /* @__PURE__ */ i("path", { fill: "currentColor", d: "M17.885 1.05a3.581 3.581 0 1 1 5.066 5.065l-3.377 3.377-5.066-5.066 3.377-3.377Z" })), Ic = "data:image/svg+xml;base64,", gc = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: Mc,
  default: Ic
}, Symbol.toStringTag, { value: "Module" })), jc = (t) => /* @__PURE__ */ i("svg", { viewBox: "-0.75 -0.75 24 24", xmlns: "http://www.w3.org/2000/svg", height: 24, width: 24, ...t }, /* @__PURE__ */ i("path", { d: "M16.996875 7.265625h-3.99375V5.475a0.9375 0.9375 0 0 1 0.9375 -1.03125h2.8125v-3.75h-4.059375c-3.684375 0 -4.378125 2.8125 -4.378125 4.55625v2.015625h-2.8125v3.75h2.8125v10.78125h4.6875v-10.78125h3.609375Z", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 })), Nc = "data:image/svg+xml;base64,", mc = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: jc,
  default: Nc
}, Symbol.toStringTag, { value: "Module" })), bc = (t) => /* @__PURE__ */ i("svg", { xmlns: "http://www.w3.org/2000/svg", fill: "none", viewBox: "-0.75 -0.75 24 24", height: 24, width: 24, ...t }, /* @__PURE__ */ i("path", { stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeMiterlimit: 10, d: "M6.140625 10.828125c-1.78125 0 -3.28125 1.5 -3.28125 3.28125 0 1.5 0.375 3 1.21875 4.3125l0.65625 1.125c0.84375 1.40625 2.4375 2.25 4.03125 2.25h6.1875c2.625 0 4.6875 -2.0625 4.6875 -4.6875v-6.84375c0 -0.9375 -0.75 -1.6875 -1.6875 -1.6875s-1.6875 0.75 -1.6875 1.6875v-0.9375c0 -0.9375 -0.75 -1.6875 -1.6875 -1.6875s-1.6875 0.75 -1.6875 1.6875v0.28125l0 -0.75c0 -0.9375 -0.75 -1.6875 -1.6875 -1.6875s-1.6875 0.75 -1.6875 1.6875l0 0.215625m0 0.5343749999999999 0 -0.5343749999999999m-3.375 4.753125000000001V2.390625c0 -0.9375 0.75 -1.6875 1.6875 -1.6875s1.6875 0.75 1.6875 1.6875l0 6.684375", strokeWidth: 1.5 })), yc = "data:image/svg+xml;base64,", pc = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: bc,
  default: yc
}, Symbol.toStringTag, { value: "Module" })), Dc = (t) => /* @__PURE__ */ i("svg", { xmlns: "http://www.w3.org/2000/svg", viewBox: "0 0 24 24", ...t }, /* @__PURE__ */ i("defs", null), /* @__PURE__ */ i("title", null, "navigation-menu"), /* @__PURE__ */ i("line", { x1: 2.25, y1: 18.003, x2: 21.75, y2: 18.003, fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: "1.5px" }), /* @__PURE__ */ i("line", { x1: 2.25, y1: 12.003, x2: 21.75, y2: 12.003, fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: "1.5px" }), /* @__PURE__ */ i("line", { x1: 2.25, y1: 6.003, x2: 21.75, y2: 6.003, fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: "1.5px" })), hc = "data:image/svg+xml;base64,", wc = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: Dc,
  default: hc
}, Symbol.toStringTag, { value: "Module" })), Sc = (t) => /* @__PURE__ */ i("svg", { xmlns: "http://www.w3.org/2000/svg", fill: "none", viewBox: "0 0 24 24", id: "Hearts-Card--Streamline-Ultimate.svg", height: 24, width: 24, ...t }, /* @__PURE__ */ i("desc", null, "Hearts Card Streamline Icon: https://streamlinehq.com"), /* @__PURE__ */ i("path", { fill: "currentColor", fillRule: "evenodd", d: "M7.284 2.513a6.376 6.376 0 0 0 -4.146 11.22l8.014 7.42a1.25 1.25 0 0 0 1.698 0l8.024 -7.43A6.376 6.376 0 1 0 12 4.599a6.36 6.36 0 0 0 -4.716 -2.086Z", clipRule: "evenodd", strokeWidth: 1 })), vc = "data:image/svg+xml;base64,", Lc = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: Sc,
  default: vc
}, Symbol.toStringTag, { value: "Module" })), Ac = (t) => /* @__PURE__ */ i("svg", { width: 26, height: 24, viewBox: "0 0 26 24", xmlns: "http://www.w3.org/2000/svg", ...t }, /* @__PURE__ */ i("path", { d: "M23.651 5.357c-.878-1.717-2.269-2.728-4.173-3.034-1.904-.305-3.541.22-4.912 1.577L13 5.329 11.434 3.9c-1.371-1.356-3.009-1.881-4.913-1.575-1.904.306-3.295 1.317-4.172 3.035-1.222 2.42-.867 4.582 1.063 6.486L13 21.75l9.588-9.907c1.93-1.904 2.285-4.066 1.063-6.486z", strokeLinecap: "round", strokeLinejoin: "round", fill: "none", stroke: "currentColor", strokeWidth: 1.5 })), fc = "data:image/svg+xml;base64,", xc = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: Ac,
  default: fc
}, Symbol.toStringTag, { value: "Module" })), Cc = (t) => /* @__PURE__ */ i("svg", { xmlns: "http://www.w3.org/2000/svg", viewBox: "0 0 24 24", id: "House-Entrance--Streamline-Streamline--3.0", height: 24, width: 24, ...t }, /* @__PURE__ */ i("desc", null, "House Entrance Streamline Icon: https://streamlinehq.com"), /* @__PURE__ */ i("defs", null), /* @__PURE__ */ i("title", null, "house-entrance"), /* @__PURE__ */ i("path", { d: "M22.868 8.947 12 0.747l-10.878 8.2a1.177 1.177 0 0 0 -0.377 0.8v12.522a0.981 0.981 0 0 0 0.978 0.978h6.522V18a3.75 3.75 0 0 1 7.5 0v5.25h6.521a0.982 0.982 0 0 0 0.979 -0.978V9.747a1.181 1.181 0 0 0 -0.377 -0.8Z", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 })), Tc = "data:image/svg+xml;base64,", zc = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: Cc,
  default: Tc
}, Symbol.toStringTag, { value: "Module" })), kc = (t) => /* @__PURE__ */ i("svg", { viewBox: "0 0 20 20", xmlns: "http://www.w3.org/2000/svg", id: "Hyperlink-Circle--Streamline-Ultimate", height: 20, width: 20, ...t }, /* @__PURE__ */ i("desc", null, "Hyperlink Circle Streamline Icon: https://streamlinehq.com"), /* @__PURE__ */ i("path", { d: "M10.426416666666666 16.262500000000003C9.295 18.64975 6.448083333333334 19.675166666666666 4.054333333333333 18.55766666666667H4.054333333333333C1.6670833333333335 17.42625 0.6416666666666667 14.579250000000002 1.75925 12.185500000000001L3.2155 9.090583333333333C4.3465 6.7035 7.193166666666667 5.678 9.586583333333333 6.7955000000000005H9.586583333333333C10.948333333333334 7.437916666666666 11.928416666666667 8.6835 12.232583333333334 10.158083333333334", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { d: "M9.573916666666667 3.7375000000000003C10.705333333333334 1.3502500000000002 13.552333333333333 0.3248333333333333 15.946083333333334 1.442416666666667H15.946083333333334C18.33275 2.57375 19.358 5.4199166666666665 18.241166666666665 7.813416666666666L16.784833333333335 10.908333333333333C15.653416666666667 13.295583333333335 12.806500000000002 14.321 10.41275 13.203416666666666H10.41275C9.248583333333334 12.654916666666667 8.354916666666668 11.659916666666666 7.934333333333334 10.443666666666667", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 })), Zc = "data:image/svg+xml;base64,", Ec = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: kc,
  default: Zc
}, Symbol.toStringTag, { value: "Module" })), Wc = (t) => /* @__PURE__ */ i("svg", { xmlns: "http://www.w3.org/2000/svg", fill: "none", viewBox: "-0.75 -0.75 24 24", height: 24, width: 24, ...t }, /* @__PURE__ */ i("path", { stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", d: "M18.09553125 20.3938125c0.674625 0 1.21921875 -0.54459375 1.21921875 -1.21921875V5.666521875c0 -0.325096875 -0.13012500000000002 -0.6420750000000001 -0.3658125 -0.8696531249999999l-2.43825 -2.34075c-0.227625 -0.227578125 -0.5364375 -0.349490625 -0.85340625 -0.349490625H4.40334375c-0.6745875 0 -1.21914375 0.544546875 -1.21914375 1.21914375V19.17459375c0 0.674625 0.5445562500000001 1.21921875 1.21914375 1.21921875h13.692187500000001Z", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", d: "m8.47595625 9.638625 2.7744187499999997 -2.774559375L14.025 9.638625", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", d: "m11.248125 6.864684375 0 8.770659375000001", strokeWidth: 1.5 })), Uc = "data:image/svg+xml;base64,", Pc = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: Wc,
  default: Uc
}, Symbol.toStringTag, { value: "Module" })), _c = (t) => /* @__PURE__ */ i("svg", { xmlns: "http://www.w3.org/2000/svg", viewBox: "0 0 24 24", height: 24, width: 24, id: "Information-Circle--Streamline-Ultimate", ...t }, /* @__PURE__ */ i("desc", null, "Information Circle Streamline Icon: https://streamlinehq.com"), /* @__PURE__ */ i("path", { d: "M12 0a12 12 0 1 0 12 12A12 12 0 0 0 12 0Zm0.25 5a1.5 1.5 0 1 1 -1.5 1.5 1.5 1.5 0 0 1 1.5 -1.5Zm2.25 13.5h-4a1 1 0 0 1 0 -2h0.75a0.25 0.25 0 0 0 0.25 -0.25v-4.5a0.25 0.25 0 0 0 -0.25 -0.25h-0.75a1 1 0 0 1 0 -2h1a2 2 0 0 1 2 2v4.75a0.25 0.25 0 0 0 0.25 0.25h0.75a1 1 0 0 1 0 2Z", fill: "currentcolor", strokeWidth: 1 })), Oc = "data:image/svg+xml;base64,", Rc = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: _c,
  default: Oc
}, Symbol.toStringTag, { value: "Module" })), Hc = (t) => /* @__PURE__ */ i("svg", { viewBox: "0 0 46 43", ...t }, /* @__PURE__ */ i("title", null, "integration"), /* @__PURE__ */ i("g", { stroke: "currentColor", fill: "none", fillRule: "evenodd", strokeWidth: "1.5px" }, /* @__PURE__ */ i("path", { d: "M-1-3h48v48H-1z", stroke: "none" }), /* @__PURE__ */ i("g", { strokeLinecap: "round", strokeLinejoin: "round" }, /* @__PURE__ */ i("path", { d: "M32.932 6.574c.713.428 1.069 1.057 1.068 1.888v9.278l-11 7.076-11-7.076V8.462c0-.831.355-1.46 1.068-1.888l8.8-5.28c.755-.453 1.51-.453 2.264 0l8.8 5.28zM23 13.816v11" }), /* @__PURE__ */ i("path", { d: "M34 31.416l-11-6.6 11-7.076 10 6.426c.669.435 1.002 1.052 1 1.85v8.124c.002.798-.331 1.415-1 1.85l-8.8 5.66c-.793.51-1.587.51-2.38 0L23 35.34V24.816m11 6.6V42M23 24.816V35.34l-9.8 6.31c-.793.51-1.587.51-2.38 0l-8.8-5.66c-.678-.43-1.018-1.047-1.02-1.85v-8.124c-.002-.798.331-1.415 1-1.85l10-6.426 11 7.076-11 6.6m0 0L1.262 24.974M12 31.416V42m11-28.184L12.282 7.384m21.436 0L23 13.816m21.738 11.158L34 31.416" })))), Gc = "data:image/svg+xml;base64,", Yc = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: Hc,
  default: Gc
}, Symbol.toStringTag, { value: "Module" })), Qc = (t) => /* @__PURE__ */ i("svg", { xmlns: "http://www.w3.org/2000/svg", viewBox: "-0.75 -0.75 24 24", height: 24, width: 24, ...t }, /* @__PURE__ */ i("g", null, /* @__PURE__ */ i("path", { d: "M12.01875 13.603125 14.399999999999999 11.25l1.65 0.440625a1.4625000000000001 1.4625000000000001 0 0 0 1.415625 -0.440625 1.4812500000000002 1.4812500000000002 0 0 0 0.346875 -1.396875l-0.440625 -1.640625 0.7687499999999999 -0.7125 1.65 0.440625A1.4625000000000001 1.4625000000000001 0 0 0 21.20625 7.5 1.4812500000000002 1.4812500000000002 0 0 0 21.5625 6.1125l-0.440625 -1.640625a2.203125 2.203125 0 0 0 -3.121875 -3.121875l-9.103125 9.13125a5.896875 5.896875 0 1 0 3.121875 3.121875Z", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { d: "M3.99375 16.725a1.78125 1.78125 0 1 0 3.5625 0 1.78125 1.78125 0 1 0 -3.5625 0", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 }))), Jc = "data:image/svg+xml;base64,", Bc = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: Qc,
  default: Jc
}, Symbol.toStringTag, { value: "Module" })), Vc = (t) => /* @__PURE__ */ i("svg", { xmlns: "http://www.w3.org/2000/svg", fill: "none", viewBox: "-0.75 -0.75 24 24", height: 24, width: 24, ...t }, /* @__PURE__ */ i("path", { stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", d: "M6.305625 0.703125h9.84375", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", d: "M14.743125000000001 7.734375V0.703125h-7.03125v7.03125L1.3959375 17.451562499999998A2.8125 2.8125 0 0 0 3.75 21.796875h14.95125a2.8125 2.8125 0 0 0 2.3578125 -4.3453124999999995L14.743125000000001 7.734375Z", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", d: "M4.9696875 11.953125h12.515625", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", d: "M13.336875000000001 16.171875h2.8125", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", d: "M14.743125000000001 14.765625v2.8125", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", d: "M14.743125000000001 3.515625h-2.8125", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", d: "M14.743125000000001 6.328125h-2.8125", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { stroke: "currentColor", d: "M6.305625 18.6328125a0.3515625 0.3515625 0 0 1 0 -0.703125", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { stroke: "currentColor", d: "M6.305625 18.6328125a0.3515625 0.3515625 0 0 0 0 -0.703125", strokeWidth: 1.5 }), /* @__PURE__ */ i("g", null, /* @__PURE__ */ i("path", { stroke: "currentColor", d: "M9.118125000000001 15.8203125a0.3515625 0.3515625 0 0 1 0 -0.703125", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { stroke: "currentColor", d: "M9.118125000000001 15.8203125a0.3515625 0.3515625 0 0 0 0 -0.703125", strokeWidth: 1.5 }))), Xc = "data:image/svg+xml;base64,", Fc = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: Vc,
  default: Xc
}, Symbol.toStringTag, { value: "Module" })), qc = (t) => /* @__PURE__ */ i("svg", { xmlns: "http://www.w3.org/2000/svg", viewBox: "-0.75 -0.75 24 24", height: 24, width: 24, ...t }, /* @__PURE__ */ i("g", null, /* @__PURE__ */ i("path", { d: "M2.109375 0.703125h8.4375s1.40625 0 1.40625 1.40625v8.4375s0 1.40625 -1.40625 1.40625h-8.4375s-1.40625 0 -1.40625 -1.40625v-8.4375s0 -1.40625 1.40625 -1.40625", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { d: "M14.765625 10.546875h5.625a1.40625 1.40625 0 0 1 1.40625 1.40625v8.4375a1.40625 1.40625 0 0 1 -1.40625 1.40625h-8.4375a1.40625 1.40625 0 0 1 -1.40625 -1.40625v-5.625", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { d: "m14.53125 16.875 3.28125 0", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 }), /* @__PURE__ */ i("g", null, /* @__PURE__ */ i("path", { d: "m6.328125 3.515625 0 1.40625", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { d: "m3.515625 4.921875 5.625 0", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { d: "M7.734375 4.921875s-1.40625 4.21875 -4.21875 4.21875", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { d: "M6.328125 7.5a3.675 3.675 0 0 0 2.8125 1.621875", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 })), /* @__PURE__ */ i("path", { d: "M14.53125 18.984375v-3.75a1.640625 1.640625 0 0 1 3.28125 0v3.75", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 }))), $c = "data:image/svg+xml;base64,", Kc = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: qc,
  default: $c
}, Symbol.toStringTag, { value: "Module" })), t0 = (t) => /* @__PURE__ */ i("svg", { xmlns: "http://www.w3.org/2000/svg", viewBox: "0 0 24 24", ...t }, /* @__PURE__ */ i("defs", null), /* @__PURE__ */ i("title", null, "Desktop"), /* @__PURE__ */ i("path", { d: "M21,14.25V4.5A1.5,1.5,0,0,0,19.5,3H4.5A1.5,1.5,0,0,0,3,4.5v9.75Z", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: "1.5px" }), /* @__PURE__ */ i("path", { d: "M23.121,18.891A1.5,1.5,0,0,1,21.75,21H2.25A1.5,1.5,0,0,1,.879,18.891L3,14.25H21Z", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: "1.5px" }), /* @__PURE__ */ i("line", { x1: 10.5, y1: 18, x2: 13.5, y2: 18, fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: "1.5px" })), e0 = "data:image/svg+xml;base64,", n0 = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: t0,
  default: e0
}, Symbol.toStringTag, { value: "Module" })), r0 = (t) => /* @__PURE__ */ i("svg", { viewBox: "-0.75 -0.75 24 24", xmlns: "http://www.w3.org/2000/svg", height: 24, width: 24, ...t }, /* @__PURE__ */ i("path", { d: "M21.478125 6.5184375 11.90625 1.5675a1.4465625 1.4465625 0 0 0 -1.3275 0L1.00875 6.5184375a0.5765625 0.5765625 0 0 0 0 1.025625l9.5709375 4.950937499999999a1.4465625 1.4465625 0 0 0 1.3275 0L21.478125 7.544062500000001a0.5775 0.5775 0 0 0 0 -1.025625Z", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { d: "m0.7106250000000001 11.953125 9.8690625 4.760625a1.4465625 1.4465625 0 0 0 1.3275 0l9.897187500000001 -4.760625", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { d: "m0.7106250000000001 16.171875 9.8690625 4.760625a1.4465625 1.4465625 0 0 0 1.3275 0l9.897187500000001 -4.760625", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 })), o0 = "data:image/svg+xml;base64,", i0 = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: r0,
  default: o0
}, Symbol.toStringTag, { value: "Module" })), s0 = (t) => /* @__PURE__ */ i("svg", { xmlns: "http://www.w3.org/2000/svg", viewBox: "0 0 24 24", id: "Browser-Page-Layout--Streamline-Ultimate", height: 24, width: 24, ...t }, /* @__PURE__ */ i("desc", null, "Browser Page Layout Streamline Icon: https://streamlinehq.com"), /* @__PURE__ */ i("defs", null), /* @__PURE__ */ i("title", null, "browser-page-layout"), /* @__PURE__ */ i("path", { d: "M3 2.25h18s1.5 0 1.5 1.5v16.5s0 1.5 -1.5 1.5H3s-1.5 0 -1.5 -1.5V3.75s0 -1.5 1.5 -1.5", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { d: "m1.5 6.75 21 0", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { d: "m9 6.75 0 15", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { d: "m9 14.25 13.5 0", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 })), a0 = "data:image/svg+xml;base64,", c0 = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: s0,
  default: a0
}, Symbol.toStringTag, { value: "Module" })), l0 = (t) => /* @__PURE__ */ i("svg", { xmlns: "http://www.w3.org/2000/svg", viewBox: "-0.75 -0.75 24 24", height: 24, width: 24, ...t }, /* @__PURE__ */ i("defs", null), /* @__PURE__ */ i("title", null, "layout-headline"), /* @__PURE__ */ i("path", { d: "M2.109375 0.7003125h18.28125s1.40625 0 1.40625 1.40625v1.40625s0 1.40625 -1.40625 1.40625H2.109375s-1.40625 0 -1.40625 -1.40625v-1.40625s0 -1.40625 1.40625 -1.40625", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { d: "M2.109375 9.137812499999999h18.28125s1.40625 0 1.40625 1.40625v1.40625s0 1.40625 -1.40625 1.40625H2.109375s-1.40625 0 -1.40625 -1.40625v-1.40625s0 -1.40625 1.40625 -1.40625", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { d: "M2.109375 17.5753125h18.28125s1.40625 0 1.40625 1.40625v1.40625s0 1.40625 -1.40625 1.40625H2.109375s-1.40625 0 -1.40625 -1.40625v-1.40625s0 -1.40625 1.40625 -1.40625", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 })), u0 = "data:image/svg+xml;base64,", d0 = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: l0,
  default: u0
}, Symbol.toStringTag, { value: "Module" })), M0 = (t) => /* @__PURE__ */ i("svg", { xmlns: "http://www.w3.org/2000/svg", viewBox: "-0.75 -0.75 24 24", height: 24, width: 24, ...t }, /* @__PURE__ */ i("defs", null), /* @__PURE__ */ i("title", null, "layout-module-1"), /* @__PURE__ */ i("path", { d: "M2.109375 0.7003125h5.625s1.40625 0 1.40625 1.40625v5.625s0 1.40625 -1.40625 1.40625h-5.625s-1.40625 0 -1.40625 -1.40625v-5.625s0 -1.40625 1.40625 -1.40625", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { d: "M2.109375 13.356562499999999h5.625s1.40625 0 1.40625 1.40625v5.625s0 1.40625 -1.40625 1.40625h-5.625s-1.40625 0 -1.40625 -1.40625v-5.625s0 -1.40625 1.40625 -1.40625", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { d: "M14.765625 0.7003125h5.625s1.40625 0 1.40625 1.40625v5.625s0 1.40625 -1.40625 1.40625h-5.625s-1.40625 0 -1.40625 -1.40625v-5.625s0 -1.40625 1.40625 -1.40625", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { d: "M14.765625 13.356562499999999h5.625s1.40625 0 1.40625 1.40625v5.625s0 1.40625 -1.40625 1.40625h-5.625s-1.40625 0 -1.40625 -1.40625v-5.625s0 -1.40625 1.40625 -1.40625", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 })), I0 = "data:image/svg+xml;base64,", g0 = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: M0,
  default: I0
}, Symbol.toStringTag, { value: "Module" })), j0 = (t) => /* @__PURE__ */ i("svg", { xmlns: "http://www.w3.org/2000/svg", fill: "none", viewBox: "-0.75 -0.75 24 24", height: 24, width: 24, ...t }, /* @__PURE__ */ i("path", { stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", d: "M0.78375 9.6103125h1.3031249999999999c1.966875 0 3.855 -0.0684375 5.257499999999999 -1.4465625a7.5 7.5 0 0 0 2.2424999999999997 -5.2190625c0 -3.1734375 4.010624999999999 -1.6875 4.010624999999999 1.14375v3.646875a1.875 1.875 0 0 0 1.875 1.875h4.414687499999999c0.9806250000000001 0 1.8046875 0.7565625 1.8234375 1.7371874999999999 0.061875 3.1275 -0.459375 5.4028125 -1.7240625 7.824375 -0.729375 1.396875 -2.2434374999999998 2.175 -3.8184375000000004 2.1403125C5.2228125 21.065624999999997 6.6384375 19.21875 0.78375 19.21875", strokeWidth: 1.5 })), N0 = "data:image/svg+xml;base64,", m0 = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: j0,
  default: N0
}, Symbol.toStringTag, { value: "Module" })), b0 = (t) => /* @__PURE__ */ i("svg", { viewBox: "0 0 24 24", xmlns: "http://www.w3.org/2000/svg", strokeWidth: 1.5, ...t }, /* @__PURE__ */ i("path", { d: "M5.25 12.373h-3", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round" }), /* @__PURE__ */ i("path", { d: "m5.25 15.373-1.5 1.5", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round" }), /* @__PURE__ */ i("path", { d: "m5.25 9.373-1.5-1.5", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round" }), /* @__PURE__ */ i("path", { d: "M18.75 12.373h3", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round" }), /* @__PURE__ */ i("path", { d: "m18.75 15.373 1.5 1.5", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round" }), /* @__PURE__ */ i("path", { d: "m18.75 9.373 1.5-1.5", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round" }), /* @__PURE__ */ i("path", { d: "M8.25 9.373v-4.5A3.762 3.762 0 0 1 12 1.123h0a3.761 3.761 0 0 1 3.75 3.75v5.25a3.763 3.763 0 0 1-2.25 3.435 3.709 3.709 0 0 1-1.5.315", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round" }), /* @__PURE__ */ i("path", { d: "M15.75 14.623v4.5a3.76 3.76 0 0 1-3.75 3.75h0a3.761 3.761 0 0 1-3.75-3.75v-4.5a3.762 3.762 0 0 1 3.75-3.75", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round" })), y0 = "data:image/svg+xml;base64,", p0 = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: b0,
  default: y0
}, Symbol.toStringTag, { value: "Module" })), D0 = (t) => /* @__PURE__ */ i("svg", { viewBox: "-0.75 -0.75 24 24", xmlns: "http://www.w3.org/2000/svg", height: 24, width: 24, ...t }, /* @__PURE__ */ i("path", { d: "M5.315625 21.215625H0.759375V8.15625h4.55625Zm9.459375 -8.803125000000001a2.00625 2.00625 0 0 0 -2.00625 2.00625v6.796875H7.9781249999999995V8.15625h4.790625v1.490625a6.3374999999999995 6.3374999999999995 0 0 1 4.0125 -1.5c2.971875 0 5.034375 2.203125 5.034375 6.3843749999999995v6.684375H16.78125v-6.796875a2.00625 2.00625 0 0 0 -2.00625 -2.015625Zm-9.375 -8.774999999999999a2.34375 2.34375 0 1 1 -2.34375 -2.34375 2.34375 2.34375 0 0 1 2.325 2.34375Z", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 })), h0 = "data:image/svg+xml;base64,", w0 = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: D0,
  default: h0
}, Symbol.toStringTag, { value: "Module" })), S0 = (t) => /* @__PURE__ */ i("svg", { xmlns: "http://www.w3.org/2000/svg", viewBox: "-0.75 -0.75 24 24", height: 24, width: 24, ...t }, /* @__PURE__ */ i("defs", null), /* @__PURE__ */ i("title", null, "layout-headline"), /* @__PURE__ */ i("path", { d: "M2.109375 0.7003125h18.28125s1.40625 0 1.40625 1.40625v1.40625s0 1.40625 -1.40625 1.40625H2.109375s-1.40625 0 -1.40625 -1.40625v-1.40625s0 -1.40625 1.40625 -1.40625", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { d: "M2.109375 9.137812499999999h18.28125s1.40625 0 1.40625 1.40625v1.40625s0 1.40625 -1.40625 1.40625H2.109375s-1.40625 0 -1.40625 -1.40625v-1.40625s0 -1.40625 1.40625 -1.40625", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { d: "M2.109375 17.5753125h18.28125s1.40625 0 1.40625 1.40625v1.40625s0 1.40625 -1.40625 1.40625H2.109375s-1.40625 0 -1.40625 -1.40625v-1.40625s0 -1.40625 1.40625 -1.40625", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 })), v0 = "data:image/svg+xml;base64,", L0 = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: S0,
  default: v0
}, Symbol.toStringTag, { value: "Module" })), A0 = (t) => /* @__PURE__ */ i("svg", { xmlns: "http://www.w3.org/2000/svg", viewBox: "0 0 24 24", strokeWidth: "1.5px", ...t }, /* @__PURE__ */ i("defs", null), /* @__PURE__ */ i("title", null, "lock-1"), /* @__PURE__ */ i("rect", { x: 3.75, y: 9.75, width: 16.5, height: 13.5, rx: 1.5, ry: 1.5, fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round" }), /* @__PURE__ */ i("path", { d: "M6.75,9.75V6a5.25,5.25,0,0,1,10.5,0V9.75", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round" }), /* @__PURE__ */ i("line", { x1: 12, y1: 15, x2: 12, y2: 18, fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round" })), f0 = "data:image/svg+xml;base64,", x0 = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: A0,
  default: f0
}, Symbol.toStringTag, { value: "Module" })), C0 = (t) => /* @__PURE__ */ i("svg", { xmlns: "http://www.w3.org/2000/svg", viewBox: "0 0 24 24", strokeWidth: "1.5px", ...t }, /* @__PURE__ */ i("defs", null), /* @__PURE__ */ i("title", null, "lock-unlock"), /* @__PURE__ */ i("path", { d: "M.75,9.75V6a5.25,5.25,0,0,1,10.5,0V9.75", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round" }), /* @__PURE__ */ i("rect", { x: 6.75, y: 9.75, width: 16.5, height: 13.5, rx: 1.5, ry: 1.5, fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round" }), /* @__PURE__ */ i("line", { x1: 15, y1: 15, x2: 15, y2: 18, fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round" })), T0 = "data:image/svg+xml;base64,", z0 = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: C0,
  default: T0
}, Symbol.toStringTag, { value: "Module" })), k0 = (t) => /* @__PURE__ */ i("svg", { viewBox: "0 0 24 24", xmlns: "http://www.w3.org/2000/svg", strokeWidth: 1.5, ...t }, /* @__PURE__ */ i("path", { d: "M0.750 9.812 A9.063 9.063 0 1 0 18.876 9.812 A9.063 9.063 0 1 0 0.750 9.812 Z", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", transform: "translate(-3.056 4.62) rotate(-23.025)" }), /* @__PURE__ */ i("path", { d: "M16.221 16.22L23.25 23.25", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round" })), Z0 = "data:image/svg+xml;base64,", E0 = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: k0,
  default: Z0
}, Symbol.toStringTag, { value: "Module" })), W0 = (t) => /* @__PURE__ */ i("svg", { viewBox: "0 0 24 24", xmlns: "http://www.w3.org/2000/svg", strokeWidth: 1.5, ...t }, /* @__PURE__ */ i("path", { d: "M11.25 17.25a6 6 0 1 0 12 0 6 6 0 1 0-12 0Z", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round" }), /* @__PURE__ */ i("path", { d: "m13.008 21.491 8.484-8.483", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round" }), /* @__PURE__ */ i("path", { d: "M8.25 15.75h-6a1.5 1.5 0 0 1-1.5-1.5v-12a1.5 1.5 0 0 1 1.5-1.5h18a1.5 1.5 0 0 1 1.5 1.5V9", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round" }), /* @__PURE__ */ i("path", { d: "m21.411 1.3-8.144 6.264a3.308 3.308 0 0 1-4.034 0L1.089 1.3", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round" })), U0 = "data:image/svg+xml;base64,", P0 = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: W0,
  default: U0
}, Symbol.toStringTag, { value: "Module" })), _0 = (t) => /* @__PURE__ */ i("svg", { viewBox: "-0.75 -0.75 24 24", xmlns: "http://www.w3.org/2000/svg", height: 24, width: 24, ...t }, /* @__PURE__ */ i("path", { d: "M6.328125 14.296875H4.21875a3.515625 3.515625 0 0 1 0 -7.03125h2.109375Z", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { d: "M6.328125 14.296875a20.90625 20.90625 0 0 1 11.593125 3.5100000000000002l1.0631249999999999 0.70875V3.046875l-1.0631249999999999 0.70875A20.90625 20.90625 0 0 1 6.328125 7.265625Z", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { d: "m21.796875 9.375 0 2.8125", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { d: "M6.328125 14.296875A6.7865625 6.7865625 0 0 0 8.4375 19.21875", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 })), O0 = "data:image/svg+xml;base64,", R0 = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: _0,
  default: O0
}, Symbol.toStringTag, { value: "Module" })), H0 = (t) => /* @__PURE__ */ i("svg", { xmlns: "http://www.w3.org/2000/svg", viewBox: "0 0 24 24", ...t }, /* @__PURE__ */ i("title", null, "Mobile"), /* @__PURE__ */ i("g", null, /* @__PURE__ */ i("rect", { x: 5.25, y: 0.75, width: 13.5, height: 22.5, rx: 3, fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: "1.5px" }), /* @__PURE__ */ i("line", { x1: 5.25, y1: 17.75, x2: 18.75, y2: 17.75, fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: "1.5px" }))), G0 = "data:image/svg+xml;base64,", Y0 = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: H0,
  default: G0
}, Symbol.toStringTag, { value: "Module" })), Q0 = (t) => /* @__PURE__ */ i("svg", { xmlns: "http://www.w3.org/2000/svg", viewBox: "-0.75 -0.75 24 24", height: 24, width: 24, ...t }, /* @__PURE__ */ i("defs", null), /* @__PURE__ */ i("title", null, "module-three"), /* @__PURE__ */ i("path", { d: "M2.109375 12.65625H8.4375s1.40625 0 1.40625 1.40625v6.328125s0 1.40625 -1.40625 1.40625H2.109375s-1.40625 0 -1.40625 -1.40625V14.0625s0 -1.40625 1.40625 -1.40625", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { d: "M14.0625 12.65625h6.328125s1.40625 0 1.40625 1.40625v6.328125s0 1.40625 -1.40625 1.40625H14.0625s-1.40625 0 -1.40625 -1.40625V14.0625s0 -1.40625 1.40625 -1.40625", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { d: "M8.0859375 0.703125h6.328125s1.40625 0 1.40625 1.40625V8.4375s0 1.40625 -1.40625 1.40625h-6.328125s-1.40625 0 -1.40625 -1.40625V2.109375s0 -1.40625 1.40625 -1.40625", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 })), J0 = "data:image/svg+xml;base64,", B0 = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: Q0,
  default: J0
}, Symbol.toStringTag, { value: "Module" })), V0 = (t) => /* @__PURE__ */ i("svg", { xmlns: "http://www.w3.org/2000/svg", viewBox: "0 0 24 24", strokeWidth: 1.5, ...t }, /* @__PURE__ */ i("defs", null), /* @__PURE__ */ i("title", null, "money-bags"), /* @__PURE__ */ i("path", { d: "M14.045 7.988C16.091 9.4 18.75 12.8 18.75 15.863c0 3.107-3.361 5.625-6.75 5.625s-6.75-2.518-6.75-5.625c0-3.063 2.659-6.463 4.705-7.875L8.4 4.281a.9.9 0 0 1 .416-1.27 10.2 10.2 0 0 1 6.363 0 .9.9 0 0 1 .421 1.27Z", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round" }), /* @__PURE__ */ i("path", { fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", d: "M9.955 7.988h4.09" }), /* @__PURE__ */ i("path", { d: "M4.5 20.738c-3 0-3.75-3-3.75-5.114a7.512 7.512 0 0 1 3.58-6.136L3.066 7.665a.75.75 0 0 1 .616-1.177H6", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round" }), /* @__PURE__ */ i("path", { d: "M10.329 17.332a2.225 2.225 0 0 0 1.858.876c1.139 0 2.063-.693 2.063-1.548s-.924-1.546-2.063-1.546-2.062-.693-2.062-1.548.924-1.547 2.062-1.547a2.221 2.221 0 0 1 1.858.875", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round" }), /* @__PURE__ */ i("path", { fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", d: "M12.187 18.208v1.03" }), /* @__PURE__ */ i("path", { fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", d: "M12.187 10.988v1.031" }), /* @__PURE__ */ i("path", { d: "M19.5 20.738c3 0 3.75-3 3.75-5.114a7.512 7.512 0 0 0-3.58-6.136l1.264-1.823a.75.75 0 0 0-.616-1.177H18", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round" })), X0 = "data:image/svg+xml;base64,", F0 = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: V0,
  default: X0
}, Symbol.toStringTag, { value: "Module" })), q0 = (t) => /* @__PURE__ */ i("svg", { xmlns: "http://www.w3.org/2000/svg", viewBox: "-0.75 -0.75 24 24", height: 24, width: 24, ...t }, /* @__PURE__ */ i("defs", null), /* @__PURE__ */ i("title", null, "navigation-menu-4"), /* @__PURE__ */ i("path", { d: "M2.109375 0.7059375h18.28125s1.40625 0 1.40625 1.40625v18.28125s0 1.40625 -1.40625 1.40625H2.109375s-1.40625 0 -1.40625 -1.40625v-18.28125s0 -1.40625 1.40625 -1.40625", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { d: "m6.328125 7.0340625 9.84375 0", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { d: "m6.328125 11.252812500000001 9.84375 0", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { d: "m6.328125 15.471562500000001 9.84375 0", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 })), $0 = "data:image/svg+xml;base64,", K0 = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: q0,
  default: $0
}, Symbol.toStringTag, { value: "Module" })), tl = (t) => /* @__PURE__ */ i("svg", { xmlns: "http://www.w3.org/2000/svg", fill: "none", viewBox: "-0.75 -0.75 24 24", height: 24, width: 24, ...t }, /* @__PURE__ */ i("path", { stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", d: "M7.03125 0.703125H2.8125a1.40625 1.40625 0 0 0 -1.40625 1.40625v18.28125a1.40625 1.40625 0 0 0 1.40625 1.40625h4.21875a1.40625 1.40625 0 0 0 1.40625 -1.40625V2.109375A1.40625 1.40625 0 0 0 7.03125 0.703125Z", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", d: "m11.025 0.80625 3.9000000000000004 1.6125a1.415625 1.415625 0 0 1 0.7687499999999999 1.875L8.4375 20.390625", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", d: "m17.8875 5.428125 2.8125 3.121875a1.40625 1.40625 0 0 1 -0.09375 1.9875L8.26875 21.046875", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", d: "M1.40625 6.796875H8.4375", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", d: "M1.40625 12.890625H8.4375", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { stroke: "currentColor", d: "M4.86 18.9890625a0.3515625 0.3515625 0 0 1 0 -0.703125", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { stroke: "currentColor", d: "M4.86 18.9890625a0.3515625 0.3515625 0 0 0 0 -0.703125", strokeWidth: 1.5 })), el = "data:image/svg+xml;base64,", nl = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: tl,
  default: el
}, Symbol.toStringTag, { value: "Module" })), rl = (t) => /* @__PURE__ */ i("svg", { xmlns: "http://www.w3.org/2000/svg", viewBox: "0 0 24 24", strokeWidth: 1.5, ...t }, /* @__PURE__ */ i("defs", null), /* @__PURE__ */ i("title", null, "pencil"), /* @__PURE__ */ i("path", { d: "M22.19 1.81a3.639 3.639 0 0 0-5.17.035l-14.5 14.5L.75 23.25l6.905-1.771 14.5-14.5a3.637 3.637 0 0 0 .035-5.169Z", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round" }), /* @__PURE__ */ i("path", { d: "m16.606 2.26 5.134 5.134", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round" }), /* @__PURE__ */ i("path", { d: "m2.521 16.344 5.139 5.13", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round" })), ol = "data:image/svg+xml;base64,", il = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: rl,
  default: ol
}, Symbol.toStringTag, { value: "Module" })), sl = (t) => /* @__PURE__ */ i("svg", { xmlns: "http://www.w3.org/2000/svg", viewBox: "0 0 24 24", strokeWidth: 1.5, ...t }, /* @__PURE__ */ i("defs", null), /* @__PURE__ */ i("title", null, "picture-sun"), /* @__PURE__ */ i("path", { d: "M2.25.75h19.5s1.5 0 1.5 1.5v19.5s0 1.5-1.5 1.5H2.25s-1.5 0-1.5-1.5V2.25s0-1.5 1.5-1.5", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round" }), /* @__PURE__ */ i("path", { d: "M13.5 7.5a3 3 0 1 0 6 0 3 3 0 1 0-6 0", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round" }), /* @__PURE__ */ i("path", { d: "M3.961 14.959a8.194 8.194 0 0 1 11.694 4.149", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round" }), /* @__PURE__ */ i("path", { d: "M14.382 16.918a4.449 4.449 0 0 1 5.851-.125", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round" })), al = "data:image/svg+xml;base64,", cl = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: sl,
  default: al
}, Symbol.toStringTag, { value: "Module" })), ll = (t) => /* @__PURE__ */ i("svg", { xmlns: "http://www.w3.org/2000/svg", fill: "none", viewBox: "-0.75 -0.75 24 24", height: 24, width: 24, ...t }, /* @__PURE__ */ i("path", { stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", d: "M21.796875 8.4375a2.8125 2.8125 0 0 1 -2.8125 2.8125", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", d: "M9.375 7.03125h2.8125", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { stroke: "currentColor", d: "M5.9193750000000005 10.542187499999999a0.3515625 0.3515625 0 0 1 0 -0.703125", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { stroke: "currentColor", d: "M5.9193750000000005 10.542187499999999a0.3515625 0.3515625 0 0 0 0 -0.703125", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", d: "M7.40625 4.10625C6.309375 2.11875 3.515625 2.109375 3.515625 2.109375l0.590625 4.10625A7.415625 7.415625 0 0 0 2.4375 9.140625H0.703125v5.625h2.334375a7.903124999999999 7.903124999999999 0 0 0 1.875 2.2218750000000003V19.6875a0.7125 0.7125 0 0 0 0.703125 0.703125H7.03125a0.7125 0.7125 0 0 0 0.703125 -0.703125v-1.1625a8.924999999999999 8.924999999999999 0 0 0 5.625 0V19.6875a0.7125 0.7125 0 0 0 0.703125 0.703125h1.40625a0.7125 0.7125 0 0 0 0.703125 -0.703125v-2.68125a7.445625 7.445625 0 0 0 2.8125 -5.75625c0 -6.0843750000000005 -6.609375 -8.803125000000001 -11.578125 -7.14375Z", strokeWidth: 1.5 })), ul = "data:image/svg+xml;base64,", dl = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: ll,
  default: ul
}, Symbol.toStringTag, { value: "Module" })), Ml = (t) => /* @__PURE__ */ i("svg", { xmlns: "http://www.w3.org/2000/svg", viewBox: "0 0 24 24", ...t }, /* @__PURE__ */ i("path", { d: "M23.14 10.61 2.25 0.16A1.56 1.56 0 0 0 0 1.56v20.88a1.56 1.56 0 0 0 2.25 1.4l20.89 -10.45a1.55 1.55 0 0 0 0 -2.78Z", fill: "currentColor", strokeWidth: 1.5 })), Il = "data:image/svg+xml;base64,", gl = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: Ml,
  default: Il
}, Symbol.toStringTag, { value: "Module" })), jl = (t) => /* @__PURE__ */ i("svg", { width: 24, height: 24, viewBox: "0 0 24 24", fill: "none", xmlns: "http://www.w3.org/2000/svg", ...t }, /* @__PURE__ */ i("g", { clipPath: "url(#clip0_718_1014)" }, /* @__PURE__ */ i("path", { d: "M16.5261 11.0917C16.3752 10.3419 16.0406 9.6412 15.5523 9.05252C15.064 8.46385 14.4372 8.00556 13.7282 7.71874M10.1882 7.75382C9.17274 8.18744 8.34628 8.97062 7.85872 9.96133C7.37116 10.952 7.25477 12.0847 7.53068 13.1538M9.63714 15.9655C10.3514 16.3922 11.1682 16.6168 12.0002 16.6154C12.749 16.6162 13.4866 16.4344 14.1493 16.0859C14.812 15.7373 15.3797 15.2325 15.8033 14.6151M14.0042 19.5877C15.072 19.3054 16.0682 18.801 16.9277 18.1074C17.7872 17.4139 18.4907 16.5467 18.9922 15.5627C19.4937 14.5786 19.7819 13.4998 19.8379 12.3968C19.8939 11.2938 19.7166 10.1913 19.3174 9.16151M17.1796 6.10613C15.7488 4.84585 13.9069 4.15158 12.0002 4.15382C10.0945 4.15064 8.25339 4.84434 6.8236 6.10428M4.71898 9.07013C4.29776 10.1172 4.10731 11.2428 4.16062 12.3702C4.21393 13.4976 4.50975 14.6002 5.02791 15.6029C5.54606 16.6056 6.27437 17.4847 7.16315 18.1803C8.05193 18.876 9.08027 19.3717 10.1781 19.6338", stroke: "currentColor", strokeWidth: 1.5, strokeLinecap: "round" }), /* @__PURE__ */ i("path", { d: "M8.23731 22.4216C9.41239 22.8462 10.6789 23.0769 11.9998 23.0769C17.0952 23.0769 21.3875 19.6366 22.6798 14.9511M6.19547 2.5634C4.58338 3.55458 3.25226 4.94244 2.3292 6.59448C1.40614 8.24652 0.921948 10.1076 0.922853 12C0.922853 15.2723 2.34162 18.2132 4.59855 20.2412M22.9373 10.236C22.0918 4.95602 17.517 0.923096 11.9998 0.923096C11.3629 0.923096 10.7379 0.976634 10.1305 1.08002", stroke: "currentColor", strokeWidth: 1.5, strokeLinecap: "round" })), /* @__PURE__ */ i("defs", null, /* @__PURE__ */ i("clipPath", { id: "clip0_718_1014" }, /* @__PURE__ */ i("rect", { width: 24, height: 24 })))), Nl = "data:image/svg+xml;base64,", ml = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: jl,
  default: Nl
}, Symbol.toStringTag, { value: "Module" })), bl = (t) => /* @__PURE__ */ i("svg", { xmlns: "http://www.w3.org/2000/svg", fill: "none", viewBox: "-0.75 -0.75 24 24", height: 24, width: 24, ...t }, /* @__PURE__ */ i("path", { stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", d: "M8.4375 8.4375a2.8125 2.8125 0 1 1 3.75 2.6521875 1.40625 1.40625 0 0 0 -0.9375 1.3265625v0.943125", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { stroke: "currentColor", d: "M11.25 16.875a0.3515625 0.3515625 0 0 1 0 -0.703125", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { stroke: "currentColor", d: "M11.25 16.875a0.3515625 0.3515625 0 0 0 0 -0.703125", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { stroke: "currentColor", strokeMiterlimit: 10, d: "M11.25 21.796875c5.8246875000000005 0 10.546875 -4.7221874999999995 10.546875 -10.546875S17.0746875 0.703125 11.25 0.703125 0.703125 5.4253124999999995 0.703125 11.25 5.4253124999999995 21.796875 11.25 21.796875Z", strokeWidth: 1.5 })), yl = "data:image/svg+xml;base64,", pl = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: bl,
  default: yl
}, Symbol.toStringTag, { value: "Module" })), Dl = (t) => /* @__PURE__ */ i("svg", { viewBox: "-0.75 -0.75 24 24", xmlns: "http://www.w3.org/2000/svg", height: 24, width: 24, ...t }, /* @__PURE__ */ i("path", { d: "M21.796875 14.765625v5.625a1.40625 1.40625 0 0 1 -1.40625 1.40625h-8.4375a1.40625 1.40625 0 0 1 -1.40625 -1.40625v-5.625", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { d: "M21.796875 14.765625a1.40625 1.40625 0 0 0 -1.40625 -1.40625h-8.4375a1.40625 1.40625 0 0 0 -1.40625 1.40625L15.4265625 17.8125a1.40625 1.40625 0 0 0 1.490625 0Z", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { d: "M3.1640625 3.8671875a3.1640625 3.1640625 0 1 0 6.328125 0 3.1640625 3.1640625 0 1 0 -6.328125 0Z", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { d: "M13.0078125 3.1640625a2.4609375 2.4609375 0 1 0 4.921875 0 2.4609375 2.4609375 0 1 0 -4.921875 0Z", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { d: "M10.73625 10.542187499999999A5.6728125 5.6728125 0 0 0 0.703125 13.359375", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { d: "M19.6875 10.546875a4.20375 4.20375 0 0 0 -7.5346875 -2.578125", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 })), hl = "data:image/svg+xml;base64,", wl = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: Dl,
  default: hl
}, Symbol.toStringTag, { value: "Module" })), Sl = (t) => /* @__PURE__ */ i("svg", { viewBox: "0 0 24 24", xmlns: "http://www.w3.org/2000/svg", id: "Button-Refresh-Arrows--Streamline-Ultimate.svg", height: 24, width: 24, ...t }, /* @__PURE__ */ i("desc", null, "Button Refresh Arrows Streamline Icon: https://streamlinehq.com"), /* @__PURE__ */ i("path", { d: "m5.25 14.248 0 4.5 -4.5 0", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { d: "m18.75 9.748 0 -4.5 4.5 0", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { d: "M19.032 5.245A9.752 9.752 0 0 1 8.246 21", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { d: "M4.967 18.751A9.753 9.753 0 0 1 15.754 3", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 })), vl = "data:image/svg+xml;base64,", Ll = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: Sl,
  default: vl
}, Symbol.toStringTag, { value: "Module" })), Al = (t) => /* @__PURE__ */ i("svg", { xmlns: "http://www.w3.org/2000/svg", viewBox: "0 0 24 24", id: "Share-1--Streamline-Streamline--3.0.svg", height: 24, width: 24, ...t }, /* @__PURE__ */ i("desc", null, "Share 1 Streamline Icon: https://streamlinehq.com"), /* @__PURE__ */ i("defs", null), /* @__PURE__ */ i("title", null, "share-1"), /* @__PURE__ */ i("path", { d: "M17.25 8.25h1.5a1.5 1.5 0 0 1 1.5 1.5v12a1.5 1.5 0 0 1 -1.5 1.5H5.25a1.5 1.5 0 0 1 -1.5 -1.5v-12a1.5 1.5 0 0 1 1.5 -1.5h1.5", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { d: "m12 0.75 0 10.5", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { d: "M8.25 4.5 12 0.75l3.75 3.75", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 })), fl = "data:image/svg+xml;base64,", xl = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: Al,
  default: fl
}, Symbol.toStringTag, { value: "Module" })), Cl = (t) => /* @__PURE__ */ i("svg", { viewBox: "0 0 24 24", xmlns: "http://www.w3.org/2000/svg", strokeWidth: 1.5, ...t }, /* @__PURE__ */ i("path", { d: "M.75,17.251a6.753,6.753,0,0,1,9.4-6.208", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round" }), /* @__PURE__ */ i("path", { d: "M3.375 4.876 A4.125 4.125 0 1 0 11.625 4.876 A4.125 4.125 0 1 0 3.375 4.876 Z", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round" }), /* @__PURE__ */ i("path", { d: "M11.250 17.249 A6.000 6.000 0 1 0 23.250 17.249 A6.000 6.000 0 1 0 11.250 17.249 Z", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round" }), /* @__PURE__ */ i("path", { d: "M13.008 21.49L21.492 13.006", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round" })), Tl = "data:image/svg+xml;base64,", zl = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: Cl,
  default: Tl
}, Symbol.toStringTag, { value: "Module" })), kl = (t) => /* @__PURE__ */ i("svg", { xmlns: "http://www.w3.org/2000/svg", fill: "none", viewBox: "0 0 14 14", height: 16, width: 16, ...t }, /* @__PURE__ */ i("g", { id: "user-single-neutral--close-geometric-human-person-single-up-user" }, /* @__PURE__ */ i("path", { id: "Union", fill: "currentColor", fillRule: "evenodd", d: "M10.5 3.5C10.5 5.433 8.93295 7 6.99995 7C5.06695 7 3.49995 5.433 3.49995 3.5C3.49995 1.567 5.06695 0 6.99995 0C8.93295 0 10.5 1.567 10.5 3.5ZM0.320435 13.4C1.21244 10.56 3.86563 8.50003 6.99996 8.50003C10.1343 8.50003 12.7875 10.56 13.6795 13.4C13.7751 13.7044 13.537 14 13.2179 14H0.781996C0.462883 14 0.224811 13.7044 0.320435 13.4Z", clipRule: "evenodd" }))), Zl = "data:image/svg+xml;base64,", El = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: kl,
  default: Zl
}, Symbol.toStringTag, { value: "Module" })), Wl = (t) => /* @__PURE__ */ i("svg", { xmlns: "http://www.w3.org/2000/svg", viewBox: "0 0 24 24", height: 24, width: 24, id: "Check-Circle-1--Streamline-Ultimate", ...t }, /* @__PURE__ */ i("desc", null, "Check Circle 1 Streamline Icon: https://streamlinehq.com"), /* @__PURE__ */ i("path", { d: "M12 0a12 12 0 1 0 12 12A12 12 0 0 0 12 0Zm6.93 8.2 -6.85 9.29a1 1 0 0 1 -1.43 0.19l-4.89 -3.91a1 1 0 0 1 -0.15 -1.41A1 1 0 0 1 7 12.21l4.08 3.26L17.32 7a1 1 0 0 1 1.39 -0.21 1 1 0 0 1 0.22 1.41Z", fill: "currentcolor", strokeWidth: 1 })), Ul = "data:image/svg+xml;base64,", Pl = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: Wl,
  default: Ul
}, Symbol.toStringTag, { value: "Module" })), _l = (t) => /* @__PURE__ */ i("svg", { xmlns: "http://www.w3.org/2000/svg", fill: "none", viewBox: "-0.75 -0.75 24 24", height: 24, width: 24, ...t }, /* @__PURE__ */ i("path", { stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", d: "M8.437481250000001 17.98875c-0.26370937499999997 0.263625 -0.621328125 0.41175 -0.99421875 0.41175 -0.37288125 0 -0.730509375 -0.148125 -0.99421875 -0.41175l-5.042812499999999 -5.041875c-0.13067812499999998 -0.13059375 -0.23433749999999998 -0.28565625 -0.3050625 -0.45628125 -0.070734375 -0.17071875 -0.10713750000000001 -0.35362499999999997 -0.10713750000000001 -0.53840625 0 -0.1846875 0.036403125 -0.3676875 0.10713750000000001 -0.5383125000000001 0.070725 -0.17071875 0.174384375 -0.32578124999999997 0.3050625 -0.45637500000000003L11.25 1.11376875c0.13059375 -0.13055624999999998 0.28575 -0.2341059375 0.45637500000000003 -0.304723125 0.17071875 -0.07061625 0.35362499999999997 -0.10692 0.5383125000000001 -0.106835625h5.041875c0.3729375 0 0.73059375 0.1481578125 0.9943124999999999 0.4118775 0.26371875 0.263728125 0.4119375 0.6214125 0.4119375 0.9943687499999999v5.042812499999999c-0.00009375 0.372703125 -0.148125 0.730125 -0.4115625 0.99375", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { stroke: "currentColor", d: "M15.1771875 4.56939375c-0.19415625 0 -0.3515625 -0.15739687500000002 -0.3515625 -0.3515625 0 -0.19415625 0.15740625 -0.3515625 0.3515625 -0.3515625", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { stroke: "currentColor", d: "M15.1771875 4.56939375c0.19415625 0 0.3515625 -0.15739687500000002 0.3515625 -0.3515625 0 -0.19415625 -0.15740625 -0.3515625 -0.3515625 -0.3515625", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", d: "M15.8803125 21.795937499999997c3.10659375 0 5.625 -2.51840625 5.625 -5.625s-2.51840625 -5.625 -5.625 -5.625 -5.625 2.51840625 -5.625 5.625 2.51840625 5.625 5.625 5.625Z", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", d: "m19.858124999999998 12.193125 -7.95375 7.9546874999999995", strokeWidth: 1.5 })), Ol = "data:image/svg+xml;base64,", Rl = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: _l,
  default: Ol
}, Symbol.toStringTag, { value: "Module" })), Hl = (t) => /* @__PURE__ */ i("svg", { xmlns: "http://www.w3.org/2000/svg", fill: "none", viewBox: "-0.75 -0.75 24 24", height: 24, width: 24, ...t }, /* @__PURE__ */ i("path", { stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", d: "M8.437481250000001 17.98875c-0.26370937499999997 0.263625 -0.621328125 0.41175 -0.99421875 0.41175 -0.37288125 0 -0.730509375 -0.148125 -0.99421875 -0.41175l-5.042812499999999 -5.041875c-0.13067812499999998 -0.13059375 -0.23433749999999998 -0.28565625 -0.3050625 -0.45628125 -0.070734375 -0.17071875 -0.10713750000000001 -0.35362499999999997 -0.10713750000000001 -0.53840625 0 -0.1846875 0.036403125 -0.3676875 0.10713750000000001 -0.5383125000000001 0.070725 -0.17071875 0.174384375 -0.32578124999999997 0.3050625 -0.45637500000000003L11.25 1.11376875c0.13059375 -0.13055624999999998 0.28575 -0.2341059375 0.45637500000000003 -0.304723125 0.17071875 -0.07061625 0.35362499999999997 -0.10692 0.5383125000000001 -0.106835625h5.041875c0.3729375 0 0.73059375 0.1481578125 0.9943124999999999 0.4118775 0.26371875 0.263728125 0.4119375 0.6214125 0.4119375 0.9943687499999999v5.042812499999999c-0.00009375 0.372703125 -0.148125 0.730125 -0.4115625 0.99375", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { stroke: "currentColor", d: "M15.1771875 4.56939375c-0.19415625 0 -0.3515625 -0.15739687500000002 -0.3515625 -0.3515625 0 -0.19415625 0.15740625 -0.3515625 0.3515625 -0.3515625", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { stroke: "currentColor", d: "M15.1771875 4.56939375c0.19415625 0 0.3515625 -0.15739687500000002 0.3515625 -0.3515625 0 -0.19415625 -0.15740625 -0.3515625 -0.3515625 -0.3515625", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", d: "m18.38709375 14.53125 -2.7234374999999997 3.631875c-0.06046875 0.08053125 -0.13753125 0.14709375000000002 -0.22593749999999999 0.19528125000000002 -0.0885 0.0481875 -0.1861875 0.07678125 -0.28668750000000004 0.08390625 -0.10040625 0.007031249999999999 -0.20118750000000002 -0.0075 -0.29559375000000004 -0.04265625 -0.0943125 -0.035250000000000004 -0.18 -0.090375 -0.25115625 -0.16153125000000002l-1.40625 -1.40625", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", d: "M15.8803125 21.795937499999997c3.10659375 0 5.625 -2.51840625 5.625 -5.625s-2.51840625 -5.625 -5.625 -5.625 -5.625 2.51840625 -5.625 5.625 2.51840625 5.625 5.625 5.625Z", strokeWidth: 1.5 })), Gl = "data:image/svg+xml;base64,", Yl = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: Hl,
  default: Gl
}, Symbol.toStringTag, { value: "Module" })), Ql = (t) => /* @__PURE__ */ i("svg", { xmlns: "http://www.w3.org/2000/svg", viewBox: "-0.75 -0.75 24 24", height: 24, width: 24, ...t }, /* @__PURE__ */ i("defs", null), /* @__PURE__ */ i("title", null, "type-cursor"), /* @__PURE__ */ i("path", { d: "M2.109375 6.32625h18.28125s1.40625 0 1.40625 1.40625v7.03125s0 1.40625 -1.40625 1.40625H2.109375s-1.40625 0 -1.40625 -1.40625v-7.03125s0 -1.40625 1.40625 -1.40625", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { d: "m16.171875 17.57625 0 -12.65625", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { d: "M11.953125 21.795a4.21875 4.21875 0 0 0 4.21875 -4.21875 4.21875 4.21875 0 0 0 4.21875 4.21875", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { d: "M11.953125 0.70125a4.21875 4.21875 0 0 1 4.21875 4.21875 4.21875 4.21875 0 0 1 4.21875 -4.21875", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 })), Jl = "data:image/svg+xml;base64,", Bl = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: Ql,
  default: Jl
}, Symbol.toStringTag, { value: "Module" })), Vl = (t) => /* @__PURE__ */ i("svg", { xmlns: "http://www.w3.org/2000/svg", fill: "none", viewBox: "0 0 24 24", ...t }, /* @__PURE__ */ i("path", { stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5, d: "M22 14.429h-3.445a1.905 1.905 0 0 0-1.543.794l-4.617 7.095a1.622 1.622 0 0 1-2.783-.233 1.597 1.597 0 0 1-.103-1.1l.833-3.142a1.867 1.867 0 0 0-.993-2.164 1.911 1.911 0 0 0-.833-.193h-4.63A1.881 1.881 0 0 1 2.08 13.06v-.011l1.8-6.008v-.016c.733-2.36 1.992-3.97 4.47-3.97 5.933 0 5.594-.684 12.523 2.818.377.188.752.379 1.126.572V16.5" })), Xl = "data:image/svg+xml;base64,", Fl = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: Vl,
  default: Xl
}, Symbol.toStringTag, { value: "Module" })), ql = (t) => /* @__PURE__ */ i("svg", { xmlns: "http://www.w3.org/2000/svg", fill: "none", viewBox: "0 0 24 24", ...t }, /* @__PURE__ */ i("path", { stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5, d: "M2.001 10.571h3.443a1.907 1.907 0 0 0 1.543-.794l4.618-7.095a1.62 1.62 0 0 1 1.992-.537 1.598 1.598 0 0 1 .892 1.871l-.832 3.14a1.867 1.867 0 0 0 .993 2.165c.259.127.544.193.832.194h4.63a1.883 1.883 0 0 1 1.807 2.426v.011l-1.8 6.008v.015c-.733 2.36-1.993 3.97-4.47 3.97-5.933 0-5.593.684-12.524-2.818-.375-.188-.75-.38-1.125-.57v-9.89" })), $l = "data:image/svg+xml;base64,", Kl = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: ql,
  default: $l
}, Symbol.toStringTag, { value: "Module" })), tu = (t) => /* @__PURE__ */ i("svg", { xmlns: "http://www.w3.org/2000/svg", viewBox: "-0.75 -0.75 24 24", height: 24, width: 24, ...t }, /* @__PURE__ */ i("defs", null), /* @__PURE__ */ i("title", null, "time-reverse"), /* @__PURE__ */ i("path", { d: "m8.5903125 16.5028125 2.8115625 -2.8125 0.0009375 -4.6875", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { d: "m13.273125 6.4246875 -3.75 -3.046875 4.21875 -2.578125", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { d: "M3.4753125 17.4375a9.2221875 9.2221875 0 1 0 6.1068750000000005 -14.0296875", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { d: "M6.42375 4.6284375a9.346875 9.346875 0 0 0 -2.8528125 2.7525", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { d: "M2.19 10.78125a9.5728125 9.5728125 0 0 0 0.12187500000000001 3.9628125", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 })), eu = "data:image/svg+xml;base64,", nu = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: tu,
  default: eu
}, Symbol.toStringTag, { value: "Module" })), ru = (t) => /* @__PURE__ */ i("svg", { xmlns: "http://www.w3.org/2000/svg", fill: "none", viewBox: "0 0 24 24", strokeWidth: 1.5, ...t }, /* @__PURE__ */ i("path", { stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", d: "M18.0576 22.3846H5.94219C5.48317 22.3846 5.04294 22.2023 4.71836 21.8777C4.39377 21.5531 4.21143 21.1129 4.21143 20.6538V5.07692H19.7883V20.6538C19.7883 21.1129 19.606 21.5531 19.2814 21.8777C18.9568 22.2023 18.5166 22.3846 18.0576 22.3846Z" }), /* @__PURE__ */ i("path", { stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", d: "M9.40381 17.1923V10.2692" }), /* @__PURE__ */ i("path", { stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", d: "M14.5962 17.1923V10.2692" }), /* @__PURE__ */ i("path", { stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", d: "M0.75 5.07692H23.25" }), /* @__PURE__ */ i("path", { stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", d: "M14.5962 1.61539H9.40386C8.94484 1.61539 8.50461 1.79774 8.18003 2.12232C7.85544 2.4469 7.6731 2.88713 7.6731 3.34616V5.07693H16.3269V3.34616C16.3269 2.88713 16.1446 2.4469 15.82 2.12232C15.4954 1.79774 15.0552 1.61539 14.5962 1.61539Z" })), ou = "data:image/svg+xml;base64,", iu = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: ru,
  default: ou
}, Symbol.toStringTag, { value: "Module" })), su = (t) => /* @__PURE__ */ i("svg", { xmlns: "http://www.w3.org/2000/svg", fill: "none", viewBox: "-0.75 -0.75 24 24", height: 24, width: 24, ...t }, /* @__PURE__ */ i("path", { stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", d: "M16.5440625 21.724687499999998 0.703125 0.703125l5.2528125 0L21.796875 21.724687499999998h-5.2528125Z", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", d: "m21.0515625 0.703125 -8.3503125 8.954062500000001", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", d: "m1.4484374999999998 21.724687499999998 8.34375 -8.9475", strokeWidth: 1.5 })), au = "data:image/svg+xml;base64,", cu = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: su,
  default: au
}, Symbol.toStringTag, { value: "Module" })), lu = (t) => /* @__PURE__ */ i("svg", { xmlns: "http://www.w3.org/2000/svg", viewBox: "0 0 122.43 122.41", ...t }, /* @__PURE__ */ i("path", { d: "M83.86 54.15v34.13H38.57V54.15H0v68.26h122.43V54.15H83.86zM38.57 0h45.3v34.13h-45.3z" })), uu = "data:image/svg+xml;base64,PHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciIHZpZXdCb3g9IjAgMCAxMjIuNDMgMTIyLjQxIj4KICAgIDxwYXRoIGQ9Ik04My44NiA1NC4xNXYzNC4xM0gzOC41N1Y1NC4xNUgwdjY4LjI2aDEyMi40M1Y1NC4xNUg4My44NnpNMzguNTcgMGg0NS4zdjM0LjEzaC00NS4zeiIvPgo8L3N2Zz4=", du = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: lu,
  default: uu
}, Symbol.toStringTag, { value: "Module" })), Mu = (t) => /* @__PURE__ */ i("svg", { xmlns: "http://www.w3.org/2000/svg", viewBox: "0 0 24 24", strokeWidth: 1.5, ...t }, /* @__PURE__ */ i("defs", null), /* @__PURE__ */ i("title", null, "upload-bottom"), /* @__PURE__ */ i("path", { fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", d: "M12.001 15.75v-12" }), /* @__PURE__ */ i("path", { fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", d: "m16.501 8.25-4.5-4.5-4.5 4.5" }), /* @__PURE__ */ i("path", { d: "M23.251 15.75v1.5a3 3 0 0 1-3 3h-16.5a3 3 0 0 1-3-3v-1.5", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round" })), Iu = "data:image/svg+xml;base64,", gu = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: Mu,
  default: Iu
}, Symbol.toStringTag, { value: "Module" })), ju = (t) => /* @__PURE__ */ i("svg", { viewBox: "0 0 24 24", xmlns: "http://www.w3.org/2000/svg", strokeWidth: 1.5, ...t }, /* @__PURE__ */ i("path", { d: "M11.250 17.250 A6.000 6.000 0 1 0 23.250 17.250 A6.000 6.000 0 1 0 11.250 17.250 Z", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round" }), /* @__PURE__ */ i("path", { d: "M17.25 14.25L17.25 20.25", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round" }), /* @__PURE__ */ i("path", { d: "M14.25 17.25L20.25 17.25", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round" }), /* @__PURE__ */ i("path", { d: "M.75,17.25a6.753,6.753,0,0,1,9.4-6.208", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round" }), /* @__PURE__ */ i("path", { d: "M3.375 4.875 A4.125 4.125 0 1 0 11.625 4.875 A4.125 4.125 0 1 0 3.375 4.875 Z", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round" })), Nu = "data:image/svg+xml;base64,", mu = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: ju,
  default: Nu
}, Symbol.toStringTag, { value: "Module" })), bu = (t) => /* @__PURE__ */ i("svg", { xmlns: "http://www.w3.org/2000/svg", viewBox: "0 0 24 24", id: "Single-Neutral-Actions--Streamline-Ultimate.svg", height: 24, width: 24, ...t }, /* @__PURE__ */ i("desc", null, "Single Neutral Actions Streamline Icon: https://streamlinehq.com"), /* @__PURE__ */ i("g", null, /* @__PURE__ */ i("path", { d: "M5.9611 6.2789C5.9611 10.9277 10.9935 13.8332 15.0195 11.5088C16.8879 10.43 18.0389 8.4364 18.0389 6.2789C18.0389 1.6302 13.0065 -1.2753 8.9805 1.0491C7.1121 2.1278 5.9611 4.1214 5.9611 6.2789", fill: "currentColor", strokeWidth: 1 }), /* @__PURE__ */ i("path", { d: "M12 13.5892C6.7337 13.589 2.4649 17.8581 2.4649 23.1243C2.4649 23.4754 2.7495 23.76 3.1005 23.76H20.8995C21.2505 23.76 21.5351 23.4754 21.5351 23.1243C21.5351 17.8581 17.2663 13.589 12 13.5892Z", fill: "currentColor", strokeWidth: 1 }))), yu = "data:image/svg+xml;base64,", pu = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: bu,
  default: yu
}, Symbol.toStringTag, { value: "Module" })), Du = (t) => /* @__PURE__ */ i("svg", { viewBox: "-0.75 -0.75 24 24", xmlns: "http://www.w3.org/2000/svg", height: 24, width: 24, ...t }, /* @__PURE__ */ i("path", { d: "M0.703125 14.765625a7.03125 7.03125 0 1 0 14.0625 0 7.03125 7.03125 0 1 0 -14.0625 0Z", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { d: "M4.921875 13.359375a2.8125 2.8125 0 1 0 5.625 0 2.8125 2.8125 0 1 0 -5.625 0Z", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { d: "M12.3159375 20.0990625a5.1206249999999995 5.1206249999999995 0 0 0 -9.163124999999999 0", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { d: "M3.515625 4.921875v-2.8125a1.40625 1.40625 0 0 1 1.40625 -1.40625h9.9646875a1.40625 1.40625 0 0 1 0.99375 0.4115625l5.505 5.505a1.40625 1.40625 0 0 1 0.4115625 0.99375V20.390625a1.40625 1.40625 0 0 1 -1.40625 1.40625h-4.21875", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { d: "M21.796875 7.734375h-5.625a1.40625 1.40625 0 0 1 -1.40625 -1.40625v-5.625", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 })), hu = "data:image/svg+xml;base64,", wu = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: Du,
  default: hu
}, Symbol.toStringTag, { value: "Module" })), Su = (t) => /* @__PURE__ */ i("svg", { id: "Single-Neutral--Streamline-Streamline--3.0", xmlns: "http://www.w3.org/2000/svg", viewBox: "0 0 24 24", height: 24, width: 24, ...t }, /* @__PURE__ */ i("desc", null, "Single Neutral Streamline Icon: https://streamlinehq.com"), /* @__PURE__ */ i("defs", null), /* @__PURE__ */ i("title", null, "single-neutral"), /* @__PURE__ */ i("path", { d: "M6.75 6a5.25 5.25 0 1 0 10.5 0 5.25 5.25 0 1 0 -10.5 0", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { d: "M2.25 23.25a9.75 9.75 0 0 1 19.5 0", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 })), vu = "data:image/svg+xml;base64,", Lu = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: Su,
  default: vu
}, Symbol.toStringTag, { value: "Module" })), Au = (t) => /* @__PURE__ */ i("svg", { xmlns: "http://www.w3.org/2000/svg", fill: "none", viewBox: "0 0 24 24", ...t }, /* @__PURE__ */ i("path", { stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5, d: "M13.313 2.27521C13.1833 2.04051 12.9931 1.84486 12.7622 1.70861C12.5313 1.57235 12.2681 1.50049 12 1.50049C11.7318 1.50049 11.4686 1.57235 11.2377 1.70861C11.0068 1.84486 10.8166 2.04051 10.687 2.27521L0.936968 20.2752C0.810886 20.5036 0.746538 20.7609 0.750276 21.0217C0.754014 21.2825 0.825708 21.5379 0.958282 21.7625C1.09086 21.9872 1.27972 22.1734 1.50625 22.3028C1.73277 22.4321 1.98911 22.5002 2.24997 22.5002H21.75C22.0108 22.5002 22.2672 22.4321 22.4937 22.3028C22.7202 22.1734 22.9091 21.9872 23.0417 21.7625C23.1742 21.5379 23.2459 21.2825 23.2497 21.0217C23.2534 20.7609 23.189 20.5036 23.063 20.2752L13.313 2.27521Z" }), /* @__PURE__ */ i("path", { stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5, d: "M12 15V8.25" }), /* @__PURE__ */ i("path", { stroke: "currentColor", strokeWidth: 1.5, d: "M12 18.75C11.7929 18.75 11.625 18.5821 11.625 18.375C11.625 18.1679 11.7929 18 12 18" }), /* @__PURE__ */ i("path", { stroke: "currentColor", strokeWidth: 1.5, d: "M12 18.75C12.2071 18.75 12.375 18.5821 12.375 18.375C12.375 18.1679 12.2071 18 12 18" })), fu = "data:image/svg+xml;base64,", xu = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: Au,
  default: fu
}, Symbol.toStringTag, { value: "Module" })), Cu = (t) => /* @__PURE__ */ i("svg", { viewBox: "-0.75 -0.75 24 24", xmlns: "http://www.w3.org/2000/svg", height: 24, width: 24, ...t }, /* @__PURE__ */ i("path", { d: "M10.546875 16.171875a5.625 5.625 0 1 0 11.25 0 5.625 5.625 0 1 0 -11.25 0Z", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { d: "m18.658125000000002 16.171875 -2.48625 0 0 -2.4853125", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { d: "M9.838125 21.703125a10.5478125 10.5478125 0 1 1 11.866875 -11.85375", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { d: "M8.7084375 21.4884375C7.2825 19.3959375 6.328125 15.593437499999999 6.328125 11.25S7.2825 3.105 8.7084375 1.0115625", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { d: "m0.7265625 10.546875 8.9278125 0", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { d: "M2.8115625 4.921875 19.6875 4.921875", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { d: "m1.92 16.171875 5.814375 0", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 }), /* @__PURE__ */ i("path", { d: "M13.7915625 1.0115625a15.9215625 15.9215625 0 0 1 2.15625 6.69", fill: "none", stroke: "currentColor", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5 })), Tu = "data:image/svg+xml;base64,", zu = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  ReactComponent: Cu,
  default: Tu
}, Symbol.toStringTag, { value: "Module" })), ku = /* @__PURE__ */ Object.assign({ "../../assets/icons/add.svg": Fi, "../../assets/icons/ai-tagging-spark.svg": Ki, "../../assets/icons/align-center.svg": ns, "../../assets/icons/align-left.svg": is, "../../assets/icons/angle-brackets.svg": cs, "../../assets/icons/arrow-bottom-left.svg": ds, "../../assets/icons/arrow-bottom-right.svg": gs, "../../assets/icons/arrow-down.svg": ms, "../../assets/icons/arrow-left.svg": ps, "../../assets/icons/arrow-right.svg": ws, "../../assets/icons/arrow-top-left.svg": Ls, "../../assets/icons/arrow-top-right.svg": xs, "../../assets/icons/arrow-up.svg": zs, "../../assets/icons/at-sign.svg": Es, "../../assets/icons/baseline-chart.svg": Ps, "../../assets/icons/bell.svg": Rs, "../../assets/icons/bills.svg": Ys, "../../assets/icons/book-open.svg": Bs, "../../assets/icons/brackets.svg": Fs, "../../assets/icons/card-list.svg": Ks, "../../assets/icons/cardview.svg": na, "../../assets/icons/check-circle.svg": ia, "../../assets/icons/check.svg": ca, "../../assets/icons/chevron-down.svg": da, "../../assets/icons/chevron-left.svg": ga, "../../assets/icons/chevron-right.svg": ma, "../../assets/icons/chevron-up.svg": pa, "../../assets/icons/close.svg": wa, "../../assets/icons/column-layout.svg": La, "../../assets/icons/comment-fill.svg": xa, "../../assets/icons/comment.svg": za, "../../assets/icons/crown.svg": Ea, "../../assets/icons/discount.svg": Pa, "../../assets/icons/dotdotdot.svg": Ra, "../../assets/icons/download.svg": Ya, "../../assets/icons/duplicate.svg": Ba, "../../assets/icons/ellipsis.svg": Fa, "../../assets/icons/email-check.svg": Ka, "../../assets/icons/email.svg": nc, "../../assets/icons/emailfield.svg": ic, "../../assets/icons/error-fill.svg": cc, "../../assets/icons/export.svg": dc, "../../assets/icons/eyedropper.svg": gc, "../../assets/icons/facebook.svg": mc, "../../assets/icons/finger-up.svg": pc, "../../assets/icons/hamburger.svg": wc, "../../assets/icons/heart-fill.svg": Lc, "../../assets/icons/heart.svg": xc, "../../assets/icons/home.svg": zc, "../../assets/icons/hyperlink-circle.svg": Ec, "../../assets/icons/import.svg": Pc, "../../assets/icons/info-fill.svg": Rc, "../../assets/icons/integration.svg": Yc, "../../assets/icons/key.svg": Bc, "../../assets/icons/labs-flask.svg": Fc, "../../assets/icons/language.svg": Kc, "../../assets/icons/laptop.svg": n0, "../../assets/icons/layer.svg": i0, "../../assets/icons/layout-2-col.svg": c0, "../../assets/icons/layout-headline.svg": d0, "../../assets/icons/layout-module-1.svg": g0, "../../assets/icons/like.svg": m0, "../../assets/icons/link-broken.svg": p0, "../../assets/icons/linkedin.svg": w0, "../../assets/icons/listview.svg": L0, "../../assets/icons/lock-locked.svg": x0, "../../assets/icons/lock-unlocked.svg": z0, "../../assets/icons/magnifying-glass.svg": E0, "../../assets/icons/mail-block.svg": P0, "../../assets/icons/megaphone.svg": R0, "../../assets/icons/mobile.svg": Y0, "../../assets/icons/modules-3.svg": B0, "../../assets/icons/money-bags.svg": F0, "../../assets/icons/navigation.svg": K0, "../../assets/icons/palette.svg": nl, "../../assets/icons/pen.svg": il, "../../assets/icons/picture.svg": cl, "../../assets/icons/piggybank.svg": dl, "../../assets/icons/play-fill.svg": gl, "../../assets/icons/portal.svg": ml, "../../assets/icons/question-circle.svg": pl, "../../assets/icons/recepients.svg": wl, "../../assets/icons/reload.svg": Ll, "../../assets/icons/share.svg": xl, "../../assets/icons/single-user-block.svg": zl, "../../assets/icons/single-user-fill.svg": El, "../../assets/icons/success-fill.svg": Pl, "../../assets/icons/tags-block.svg": Rl, "../../assets/icons/tags-check.svg": Yl, "../../assets/icons/textfield.svg": Bl, "../../assets/icons/thumbs-down.svg": Fl, "../../assets/icons/thumbs-up.svg": Kl, "../../assets/icons/time-back.svg": nu, "../../assets/icons/trash.svg": iu, "../../assets/icons/twitter-x.svg": cu, "../../assets/icons/unsplash-logo.svg": du, "../../assets/icons/upload.svg": gu, "../../assets/icons/user-add.svg": mu, "../../assets/icons/user-fill.svg": pu, "../../assets/icons/user-page.svg": wu, "../../assets/icons/user.svg": Lu, "../../assets/icons/warning.svg": xu, "../../assets/icons/world-clock.svg": zu }), Re = ({ name: t, size: e = "md", colorClass: n = "", className: r = "" }) => {
  const { ReactComponent: o } = ku[`../../assets/icons/${t}.svg`];
  let s = "", a = {};
  if (typeof e == "number" && (a = {
    width: `${e}px`,
    height: `${e}px`
  }), !s)
    switch (e) {
      case "custom":
        break;
      case "2xs":
        s = "w-2 h-2";
        break;
      case "xs":
        s = "w-3 h-3";
        break;
      case "sm":
        s = "w-4 h-4";
        break;
      case "lg":
        s = "w-8 h-8";
        break;
      case "xl":
        s = "w-10 h-10";
        break;
      default:
        s = "w-5 h-5";
        break;
    }
  return s = Dn(
    s,
    n
  ), o ? /* @__PURE__ */ b.jsx(o, { className: `pointer-events-none ${s} ${r}`, style: a }) : null;
};
function Zu(t, e) {
  typeof t == "function" ? t(e) : t != null && (t.current = e);
}
function Eu(...t) {
  return (e) => t.forEach((n) => Zu(n, e));
}
var ye = Y((t, e) => {
  const { children: n, ...r } = t, o = Ct.toArray(n), s = o.find(Uu);
  if (s) {
    const a = s.props.children, c = o.map((l) => l === s ? Ct.count(a) > 1 ? Ct.only(null) : Zt(a) ? a.props.children : null : l);
    return /* @__PURE__ */ b.jsx(Xe, { ...r, ref: e, children: Zt(a) ? yn(a, void 0, c) : null });
  }
  return /* @__PURE__ */ b.jsx(Xe, { ...r, ref: e, children: n });
});
ye.displayName = "Slot";
var Xe = Y((t, e) => {
  const { children: n, ...r } = t;
  if (Zt(n)) {
    const o = _u(n);
    return yn(n, {
      ...Pu(r, n.props),
      // @ts-ignore
      ref: e ? Eu(e, o) : o
    });
  }
  return Ct.count(n) > 1 ? Ct.only(null) : null;
});
Xe.displayName = "SlotClone";
var Wu = ({ children: t }) => /* @__PURE__ */ b.jsx(b.Fragment, { children: t });
function Uu(t) {
  return Zt(t) && t.type === Wu;
}
function Pu(t, e) {
  const n = { ...e };
  for (const r in e) {
    const o = t[r], s = e[r];
    /^on[A-Z]/.test(r) ? o && s ? n[r] = (...c) => {
      s(...c), o(...c);
    } : o && (n[r] = o) : r === "style" ? n[r] = { ...o, ...s } : r === "className" && (n[r] = [o, s].filter(Boolean).join(" "));
  }
  return { ...t, ...n };
}
function _u(t) {
  var r, o;
  let e = (r = Object.getOwnPropertyDescriptor(t.props, "ref")) == null ? void 0 : r.get, n = e && "isReactWarning" in e && e.isReactWarning;
  return n ? t.ref : (e = (o = Object.getOwnPropertyDescriptor(t, "ref")) == null ? void 0 : o.get, n = e && "isReactWarning" in e && e.isReactWarning, n ? t.props.ref : t.props.ref || t.ref);
}
function Gr(t) {
  var e, n, r = "";
  if (typeof t == "string" || typeof t == "number")
    r += t;
  else if (typeof t == "object")
    if (Array.isArray(t))
      for (e = 0; e < t.length; e++)
        t[e] && (n = Gr(t[e])) && (r && (r += " "), r += n);
    else
      for (e in t)
        t[e] && (r && (r += " "), r += e);
  return r;
}
function Ou() {
  for (var t, e, n = 0, r = ""; n < arguments.length; )
    (t = arguments[n++]) && (e = Gr(t)) && (r && (r += " "), r += e);
  return r;
}
const Jn = (t) => typeof t == "boolean" ? "".concat(t) : t === 0 ? "0" : t, Bn = Ou, Yr = (t, e) => (n) => {
  var r;
  if ((e == null ? void 0 : e.variants) == null)
    return Bn(t, n == null ? void 0 : n.class, n == null ? void 0 : n.className);
  const { variants: o, defaultVariants: s } = e, a = Object.keys(o).map((u) => {
    const d = n == null ? void 0 : n[u], M = s == null ? void 0 : s[u];
    if (d === null)
      return null;
    const I = Jn(d) || Jn(M);
    return o[u][I];
  }), c = n && Object.entries(n).reduce((u, d) => {
    let [M, I] = d;
    return I === void 0 || (u[M] = I), u;
  }, {}), l = e == null || (r = e.compoundVariants) === null || r === void 0 ? void 0 : r.reduce((u, d) => {
    let { class: M, className: I, ...g } = d;
    return Object.entries(g).every((N) => {
      let [m, j] = N;
      return Array.isArray(j) ? j.includes({
        ...s,
        ...c
      }[m]) : {
        ...s,
        ...c
      }[m] === j;
    }) ? [
      ...u,
      M,
      I
    ] : u;
  }, []);
  return Bn(t, a, l, n == null ? void 0 : n.class, n == null ? void 0 : n.className);
}, hn = "-", Ru = (t) => {
  const e = Gu(t), {
    conflictingClassGroups: n,
    conflictingClassGroupModifiers: r
  } = t;
  return {
    getClassGroupId: (a) => {
      const c = a.split(hn);
      return c[0] === "" && c.length !== 1 && c.shift(), Qr(c, e) || Hu(a);
    },
    getConflictingClassGroupIds: (a, c) => {
      const l = n[a] || [];
      return c && r[a] ? [...l, ...r[a]] : l;
    }
  };
}, Qr = (t, e) => {
  var a;
  if (t.length === 0)
    return e.classGroupId;
  const n = t[0], r = e.nextPart.get(n), o = r ? Qr(t.slice(1), r) : void 0;
  if (o)
    return o;
  if (e.validators.length === 0)
    return;
  const s = t.join(hn);
  return (a = e.validators.find(({
    validator: c
  }) => c(s))) == null ? void 0 : a.classGroupId;
}, Vn = /^\[(.+)\]$/, Hu = (t) => {
  if (Vn.test(t)) {
    const e = Vn.exec(t)[1], n = e == null ? void 0 : e.substring(0, e.indexOf(":"));
    if (n)
      return "arbitrary.." + n;
  }
}, Gu = (t) => {
  const {
    theme: e,
    prefix: n
  } = t, r = {
    nextPart: /* @__PURE__ */ new Map(),
    validators: []
  };
  return Qu(Object.entries(t.classGroups), n).forEach(([s, a]) => {
    Fe(a, r, s, e);
  }), r;
}, Fe = (t, e, n, r) => {
  t.forEach((o) => {
    if (typeof o == "string") {
      const s = o === "" ? e : Xn(e, o);
      s.classGroupId = n;
      return;
    }
    if (typeof o == "function") {
      if (Yu(o)) {
        Fe(o(r), e, n, r);
        return;
      }
      e.validators.push({
        validator: o,
        classGroupId: n
      });
      return;
    }
    Object.entries(o).forEach(([s, a]) => {
      Fe(a, Xn(e, s), n, r);
    });
  });
}, Xn = (t, e) => {
  let n = t;
  return e.split(hn).forEach((r) => {
    n.nextPart.has(r) || n.nextPart.set(r, {
      nextPart: /* @__PURE__ */ new Map(),
      validators: []
    }), n = n.nextPart.get(r);
  }), n;
}, Yu = (t) => t.isThemeGetter, Qu = (t, e) => e ? t.map(([n, r]) => {
  const o = r.map((s) => typeof s == "string" ? e + s : typeof s == "object" ? Object.fromEntries(Object.entries(s).map(([a, c]) => [e + a, c])) : s);
  return [n, o];
}) : t, Ju = (t) => {
  if (t < 1)
    return {
      get: () => {
      },
      set: () => {
      }
    };
  let e = 0, n = /* @__PURE__ */ new Map(), r = /* @__PURE__ */ new Map();
  const o = (s, a) => {
    n.set(s, a), e++, e > t && (e = 0, r = n, n = /* @__PURE__ */ new Map());
  };
  return {
    get(s) {
      let a = n.get(s);
      if (a !== void 0)
        return a;
      if ((a = r.get(s)) !== void 0)
        return o(s, a), a;
    },
    set(s, a) {
      n.has(s) ? n.set(s, a) : o(s, a);
    }
  };
}, Jr = "!", Bu = (t) => {
  const {
    separator: e,
    experimentalParseClassName: n
  } = t, r = e.length === 1, o = e[0], s = e.length, a = (c) => {
    const l = [];
    let u = 0, d = 0, M;
    for (let j = 0; j < c.length; j++) {
      let p = c[j];
      if (u === 0) {
        if (p === o && (r || c.slice(j, j + s) === e)) {
          l.push(c.slice(d, j)), d = j + s;
          continue;
        }
        if (p === "/") {
          M = j;
          continue;
        }
      }
      p === "[" ? u++ : p === "]" && u--;
    }
    const I = l.length === 0 ? c : c.substring(d), g = I.startsWith(Jr), N = g ? I.substring(1) : I, m = M && M > d ? M - d : void 0;
    return {
      modifiers: l,
      hasImportantModifier: g,
      baseClassName: N,
      maybePostfixModifierPosition: m
    };
  };
  return n ? (c) => n({
    className: c,
    parseClassName: a
  }) : a;
}, Vu = (t) => {
  if (t.length <= 1)
    return t;
  const e = [];
  let n = [];
  return t.forEach((r) => {
    r[0] === "[" ? (e.push(...n.sort(), r), n = []) : n.push(r);
  }), e.push(...n.sort()), e;
}, Xu = (t) => ({
  cache: Ju(t.cacheSize),
  parseClassName: Bu(t),
  ...Ru(t)
}), Fu = /\s+/, qu = (t, e) => {
  const {
    parseClassName: n,
    getClassGroupId: r,
    getConflictingClassGroupIds: o
  } = e, s = [], a = t.trim().split(Fu);
  let c = "";
  for (let l = a.length - 1; l >= 0; l -= 1) {
    const u = a[l], {
      modifiers: d,
      hasImportantModifier: M,
      baseClassName: I,
      maybePostfixModifierPosition: g
    } = n(u);
    let N = !!g, m = r(N ? I.substring(0, g) : I);
    if (!m) {
      if (!N) {
        c = u + (c.length > 0 ? " " + c : c);
        continue;
      }
      if (m = r(I), !m) {
        c = u + (c.length > 0 ? " " + c : c);
        continue;
      }
      N = !1;
    }
    const j = Vu(d).join(":"), p = M ? j + Jr : j, A = p + m;
    if (s.includes(A))
      continue;
    s.push(A);
    const Z = o(m, N);
    for (let C = 0; C < Z.length; ++C) {
      const H = Z[C];
      s.push(p + H);
    }
    c = u + (c.length > 0 ? " " + c : c);
  }
  return c;
};
function $u() {
  let t = 0, e, n, r = "";
  for (; t < arguments.length; )
    (e = arguments[t++]) && (n = Br(e)) && (r && (r += " "), r += n);
  return r;
}
const Br = (t) => {
  if (typeof t == "string")
    return t;
  let e, n = "";
  for (let r = 0; r < t.length; r++)
    t[r] && (e = Br(t[r])) && (n && (n += " "), n += e);
  return n;
};
function Ku(t, ...e) {
  let n, r, o, s = a;
  function a(l) {
    const u = e.reduce((d, M) => M(d), t());
    return n = Xu(u), r = n.cache.get, o = n.cache.set, s = c, c(l);
  }
  function c(l) {
    const u = r(l);
    if (u)
      return u;
    const d = qu(l, n);
    return o(l, d), d;
  }
  return function() {
    return s($u.apply(null, arguments));
  };
}
const x = (t) => {
  const e = (n) => n[t] || [];
  return e.isThemeGetter = !0, e;
}, Vr = /^\[(?:([a-z-]+):)?(.+)\]$/i, td = /^\d+\/\d+$/, ed = /* @__PURE__ */ new Set(["px", "full", "screen"]), nd = /^(\d+(\.\d+)?)?(xs|sm|md|lg|xl)$/, rd = /\d+(%|px|r?em|[sdl]?v([hwib]|min|max)|pt|pc|in|cm|mm|cap|ch|ex|r?lh|cq(w|h|i|b|min|max))|\b(calc|min|max|clamp)\(.+\)|^0$/, od = /^(rgba?|hsla?|hwb|(ok)?(lab|lch))\(.+\)$/, id = /^(inset_)?-?((\d+)?\.?(\d+)[a-z]+|0)_-?((\d+)?\.?(\d+)[a-z]+|0)/, sd = /^(url|image|image-set|cross-fade|element|(repeating-)?(linear|radial|conic)-gradient)\(.+\)$/, K = (t) => Tt(t) || ed.has(t) || td.test(t), it = (t) => Ut(t, "length", gd), Tt = (t) => !!t && !Number.isNaN(Number(t)), He = (t) => Ut(t, "number", Tt), Ht = (t) => !!t && Number.isInteger(Number(t)), ad = (t) => t.endsWith("%") && Tt(t.slice(0, -1)), y = (t) => Vr.test(t), st = (t) => nd.test(t), cd = /* @__PURE__ */ new Set(["length", "size", "percentage"]), ld = (t) => Ut(t, cd, Xr), ud = (t) => Ut(t, "position", Xr), dd = /* @__PURE__ */ new Set(["image", "url"]), Md = (t) => Ut(t, dd, Nd), Id = (t) => Ut(t, "", jd), Gt = () => !0, Ut = (t, e, n) => {
  const r = Vr.exec(t);
  return r ? r[1] ? typeof e == "string" ? r[1] === e : e.has(r[1]) : n(r[2]) : !1;
}, gd = (t) => (
  // `colorFunctionRegex` check is necessary because color functions can have percentages in them which which would be incorrectly classified as lengths.
  // For example, `hsl(0 0% 0%)` would be classified as a length without this check.
  // I could also use lookbehind assertion in `lengthUnitRegex` but that isn't supported widely enough.
  rd.test(t) && !od.test(t)
), Xr = () => !1, jd = (t) => id.test(t), Nd = (t) => sd.test(t), md = () => {
  const t = x("colors"), e = x("spacing"), n = x("blur"), r = x("brightness"), o = x("borderColor"), s = x("borderRadius"), a = x("borderSpacing"), c = x("borderWidth"), l = x("contrast"), u = x("grayscale"), d = x("hueRotate"), M = x("invert"), I = x("gap"), g = x("gradientColorStops"), N = x("gradientColorStopPositions"), m = x("inset"), j = x("margin"), p = x("opacity"), A = x("padding"), Z = x("saturate"), C = x("scale"), H = x("sepia"), S = x("skew"), D = x("space"), L = x("translate"), P = () => ["auto", "contain", "none"], gt = () => ["auto", "hidden", "clip", "visible", "scroll"], jt = () => ["auto", y, e], f = () => [y, e], Xt = () => ["", K, it], Ft = () => ["auto", Tt, y], Gn = () => ["bottom", "center", "left", "left-bottom", "left-top", "right", "right-bottom", "right-top", "top"], qt = () => ["solid", "dashed", "dotted", "double", "none"], Yn = () => ["normal", "multiply", "screen", "overlay", "darken", "lighten", "color-dodge", "color-burn", "hard-light", "soft-light", "difference", "exclusion", "hue", "saturation", "color", "luminosity"], Oe = () => ["start", "end", "center", "between", "around", "evenly", "stretch"], Rt = () => ["", "0", y], Qn = () => ["auto", "avoid", "all", "avoid-page", "page", "left", "right", "column"], $ = () => [Tt, y];
  return {
    cacheSize: 500,
    separator: ":",
    theme: {
      colors: [Gt],
      spacing: [K, it],
      blur: ["none", "", st, y],
      brightness: $(),
      borderColor: [t],
      borderRadius: ["none", "", "full", st, y],
      borderSpacing: f(),
      borderWidth: Xt(),
      contrast: $(),
      grayscale: Rt(),
      hueRotate: $(),
      invert: Rt(),
      gap: f(),
      gradientColorStops: [t],
      gradientColorStopPositions: [ad, it],
      inset: jt(),
      margin: jt(),
      opacity: $(),
      padding: f(),
      saturate: $(),
      scale: $(),
      sepia: Rt(),
      skew: $(),
      space: f(),
      translate: f()
    },
    classGroups: {
      // Layout
      /**
       * Aspect Ratio
       * @see https://tailwindcss.com/docs/aspect-ratio
       */
      aspect: [{
        aspect: ["auto", "square", "video", y]
      }],
      /**
       * Container
       * @see https://tailwindcss.com/docs/container
       */
      container: ["container"],
      /**
       * Columns
       * @see https://tailwindcss.com/docs/columns
       */
      columns: [{
        columns: [st]
      }],
      /**
       * Break After
       * @see https://tailwindcss.com/docs/break-after
       */
      "break-after": [{
        "break-after": Qn()
      }],
      /**
       * Break Before
       * @see https://tailwindcss.com/docs/break-before
       */
      "break-before": [{
        "break-before": Qn()
      }],
      /**
       * Break Inside
       * @see https://tailwindcss.com/docs/break-inside
       */
      "break-inside": [{
        "break-inside": ["auto", "avoid", "avoid-page", "avoid-column"]
      }],
      /**
       * Box Decoration Break
       * @see https://tailwindcss.com/docs/box-decoration-break
       */
      "box-decoration": [{
        "box-decoration": ["slice", "clone"]
      }],
      /**
       * Box Sizing
       * @see https://tailwindcss.com/docs/box-sizing
       */
      box: [{
        box: ["border", "content"]
      }],
      /**
       * Display
       * @see https://tailwindcss.com/docs/display
       */
      display: ["block", "inline-block", "inline", "flex", "inline-flex", "table", "inline-table", "table-caption", "table-cell", "table-column", "table-column-group", "table-footer-group", "table-header-group", "table-row-group", "table-row", "flow-root", "grid", "inline-grid", "contents", "list-item", "hidden"],
      /**
       * Floats
       * @see https://tailwindcss.com/docs/float
       */
      float: [{
        float: ["right", "left", "none", "start", "end"]
      }],
      /**
       * Clear
       * @see https://tailwindcss.com/docs/clear
       */
      clear: [{
        clear: ["left", "right", "both", "none", "start", "end"]
      }],
      /**
       * Isolation
       * @see https://tailwindcss.com/docs/isolation
       */
      isolation: ["isolate", "isolation-auto"],
      /**
       * Object Fit
       * @see https://tailwindcss.com/docs/object-fit
       */
      "object-fit": [{
        object: ["contain", "cover", "fill", "none", "scale-down"]
      }],
      /**
       * Object Position
       * @see https://tailwindcss.com/docs/object-position
       */
      "object-position": [{
        object: [...Gn(), y]
      }],
      /**
       * Overflow
       * @see https://tailwindcss.com/docs/overflow
       */
      overflow: [{
        overflow: gt()
      }],
      /**
       * Overflow X
       * @see https://tailwindcss.com/docs/overflow
       */
      "overflow-x": [{
        "overflow-x": gt()
      }],
      /**
       * Overflow Y
       * @see https://tailwindcss.com/docs/overflow
       */
      "overflow-y": [{
        "overflow-y": gt()
      }],
      /**
       * Overscroll Behavior
       * @see https://tailwindcss.com/docs/overscroll-behavior
       */
      overscroll: [{
        overscroll: P()
      }],
      /**
       * Overscroll Behavior X
       * @see https://tailwindcss.com/docs/overscroll-behavior
       */
      "overscroll-x": [{
        "overscroll-x": P()
      }],
      /**
       * Overscroll Behavior Y
       * @see https://tailwindcss.com/docs/overscroll-behavior
       */
      "overscroll-y": [{
        "overscroll-y": P()
      }],
      /**
       * Position
       * @see https://tailwindcss.com/docs/position
       */
      position: ["static", "fixed", "absolute", "relative", "sticky"],
      /**
       * Top / Right / Bottom / Left
       * @see https://tailwindcss.com/docs/top-right-bottom-left
       */
      inset: [{
        inset: [m]
      }],
      /**
       * Right / Left
       * @see https://tailwindcss.com/docs/top-right-bottom-left
       */
      "inset-x": [{
        "inset-x": [m]
      }],
      /**
       * Top / Bottom
       * @see https://tailwindcss.com/docs/top-right-bottom-left
       */
      "inset-y": [{
        "inset-y": [m]
      }],
      /**
       * Start
       * @see https://tailwindcss.com/docs/top-right-bottom-left
       */
      start: [{
        start: [m]
      }],
      /**
       * End
       * @see https://tailwindcss.com/docs/top-right-bottom-left
       */
      end: [{
        end: [m]
      }],
      /**
       * Top
       * @see https://tailwindcss.com/docs/top-right-bottom-left
       */
      top: [{
        top: [m]
      }],
      /**
       * Right
       * @see https://tailwindcss.com/docs/top-right-bottom-left
       */
      right: [{
        right: [m]
      }],
      /**
       * Bottom
       * @see https://tailwindcss.com/docs/top-right-bottom-left
       */
      bottom: [{
        bottom: [m]
      }],
      /**
       * Left
       * @see https://tailwindcss.com/docs/top-right-bottom-left
       */
      left: [{
        left: [m]
      }],
      /**
       * Visibility
       * @see https://tailwindcss.com/docs/visibility
       */
      visibility: ["visible", "invisible", "collapse"],
      /**
       * Z-Index
       * @see https://tailwindcss.com/docs/z-index
       */
      z: [{
        z: ["auto", Ht, y]
      }],
      // Flexbox and Grid
      /**
       * Flex Basis
       * @see https://tailwindcss.com/docs/flex-basis
       */
      basis: [{
        basis: jt()
      }],
      /**
       * Flex Direction
       * @see https://tailwindcss.com/docs/flex-direction
       */
      "flex-direction": [{
        flex: ["row", "row-reverse", "col", "col-reverse"]
      }],
      /**
       * Flex Wrap
       * @see https://tailwindcss.com/docs/flex-wrap
       */
      "flex-wrap": [{
        flex: ["wrap", "wrap-reverse", "nowrap"]
      }],
      /**
       * Flex
       * @see https://tailwindcss.com/docs/flex
       */
      flex: [{
        flex: ["1", "auto", "initial", "none", y]
      }],
      /**
       * Flex Grow
       * @see https://tailwindcss.com/docs/flex-grow
       */
      grow: [{
        grow: Rt()
      }],
      /**
       * Flex Shrink
       * @see https://tailwindcss.com/docs/flex-shrink
       */
      shrink: [{
        shrink: Rt()
      }],
      /**
       * Order
       * @see https://tailwindcss.com/docs/order
       */
      order: [{
        order: ["first", "last", "none", Ht, y]
      }],
      /**
       * Grid Template Columns
       * @see https://tailwindcss.com/docs/grid-template-columns
       */
      "grid-cols": [{
        "grid-cols": [Gt]
      }],
      /**
       * Grid Column Start / End
       * @see https://tailwindcss.com/docs/grid-column
       */
      "col-start-end": [{
        col: ["auto", {
          span: ["full", Ht, y]
        }, y]
      }],
      /**
       * Grid Column Start
       * @see https://tailwindcss.com/docs/grid-column
       */
      "col-start": [{
        "col-start": Ft()
      }],
      /**
       * Grid Column End
       * @see https://tailwindcss.com/docs/grid-column
       */
      "col-end": [{
        "col-end": Ft()
      }],
      /**
       * Grid Template Rows
       * @see https://tailwindcss.com/docs/grid-template-rows
       */
      "grid-rows": [{
        "grid-rows": [Gt]
      }],
      /**
       * Grid Row Start / End
       * @see https://tailwindcss.com/docs/grid-row
       */
      "row-start-end": [{
        row: ["auto", {
          span: [Ht, y]
        }, y]
      }],
      /**
       * Grid Row Start
       * @see https://tailwindcss.com/docs/grid-row
       */
      "row-start": [{
        "row-start": Ft()
      }],
      /**
       * Grid Row End
       * @see https://tailwindcss.com/docs/grid-row
       */
      "row-end": [{
        "row-end": Ft()
      }],
      /**
       * Grid Auto Flow
       * @see https://tailwindcss.com/docs/grid-auto-flow
       */
      "grid-flow": [{
        "grid-flow": ["row", "col", "dense", "row-dense", "col-dense"]
      }],
      /**
       * Grid Auto Columns
       * @see https://tailwindcss.com/docs/grid-auto-columns
       */
      "auto-cols": [{
        "auto-cols": ["auto", "min", "max", "fr", y]
      }],
      /**
       * Grid Auto Rows
       * @see https://tailwindcss.com/docs/grid-auto-rows
       */
      "auto-rows": [{
        "auto-rows": ["auto", "min", "max", "fr", y]
      }],
      /**
       * Gap
       * @see https://tailwindcss.com/docs/gap
       */
      gap: [{
        gap: [I]
      }],
      /**
       * Gap X
       * @see https://tailwindcss.com/docs/gap
       */
      "gap-x": [{
        "gap-x": [I]
      }],
      /**
       * Gap Y
       * @see https://tailwindcss.com/docs/gap
       */
      "gap-y": [{
        "gap-y": [I]
      }],
      /**
       * Justify Content
       * @see https://tailwindcss.com/docs/justify-content
       */
      "justify-content": [{
        justify: ["normal", ...Oe()]
      }],
      /**
       * Justify Items
       * @see https://tailwindcss.com/docs/justify-items
       */
      "justify-items": [{
        "justify-items": ["start", "end", "center", "stretch"]
      }],
      /**
       * Justify Self
       * @see https://tailwindcss.com/docs/justify-self
       */
      "justify-self": [{
        "justify-self": ["auto", "start", "end", "center", "stretch"]
      }],
      /**
       * Align Content
       * @see https://tailwindcss.com/docs/align-content
       */
      "align-content": [{
        content: ["normal", ...Oe(), "baseline"]
      }],
      /**
       * Align Items
       * @see https://tailwindcss.com/docs/align-items
       */
      "align-items": [{
        items: ["start", "end", "center", "baseline", "stretch"]
      }],
      /**
       * Align Self
       * @see https://tailwindcss.com/docs/align-self
       */
      "align-self": [{
        self: ["auto", "start", "end", "center", "stretch", "baseline"]
      }],
      /**
       * Place Content
       * @see https://tailwindcss.com/docs/place-content
       */
      "place-content": [{
        "place-content": [...Oe(), "baseline"]
      }],
      /**
       * Place Items
       * @see https://tailwindcss.com/docs/place-items
       */
      "place-items": [{
        "place-items": ["start", "end", "center", "baseline", "stretch"]
      }],
      /**
       * Place Self
       * @see https://tailwindcss.com/docs/place-self
       */
      "place-self": [{
        "place-self": ["auto", "start", "end", "center", "stretch"]
      }],
      // Spacing
      /**
       * Padding
       * @see https://tailwindcss.com/docs/padding
       */
      p: [{
        p: [A]
      }],
      /**
       * Padding X
       * @see https://tailwindcss.com/docs/padding
       */
      px: [{
        px: [A]
      }],
      /**
       * Padding Y
       * @see https://tailwindcss.com/docs/padding
       */
      py: [{
        py: [A]
      }],
      /**
       * Padding Start
       * @see https://tailwindcss.com/docs/padding
       */
      ps: [{
        ps: [A]
      }],
      /**
       * Padding End
       * @see https://tailwindcss.com/docs/padding
       */
      pe: [{
        pe: [A]
      }],
      /**
       * Padding Top
       * @see https://tailwindcss.com/docs/padding
       */
      pt: [{
        pt: [A]
      }],
      /**
       * Padding Right
       * @see https://tailwindcss.com/docs/padding
       */
      pr: [{
        pr: [A]
      }],
      /**
       * Padding Bottom
       * @see https://tailwindcss.com/docs/padding
       */
      pb: [{
        pb: [A]
      }],
      /**
       * Padding Left
       * @see https://tailwindcss.com/docs/padding
       */
      pl: [{
        pl: [A]
      }],
      /**
       * Margin
       * @see https://tailwindcss.com/docs/margin
       */
      m: [{
        m: [j]
      }],
      /**
       * Margin X
       * @see https://tailwindcss.com/docs/margin
       */
      mx: [{
        mx: [j]
      }],
      /**
       * Margin Y
       * @see https://tailwindcss.com/docs/margin
       */
      my: [{
        my: [j]
      }],
      /**
       * Margin Start
       * @see https://tailwindcss.com/docs/margin
       */
      ms: [{
        ms: [j]
      }],
      /**
       * Margin End
       * @see https://tailwindcss.com/docs/margin
       */
      me: [{
        me: [j]
      }],
      /**
       * Margin Top
       * @see https://tailwindcss.com/docs/margin
       */
      mt: [{
        mt: [j]
      }],
      /**
       * Margin Right
       * @see https://tailwindcss.com/docs/margin
       */
      mr: [{
        mr: [j]
      }],
      /**
       * Margin Bottom
       * @see https://tailwindcss.com/docs/margin
       */
      mb: [{
        mb: [j]
      }],
      /**
       * Margin Left
       * @see https://tailwindcss.com/docs/margin
       */
      ml: [{
        ml: [j]
      }],
      /**
       * Space Between X
       * @see https://tailwindcss.com/docs/space
       */
      "space-x": [{
        "space-x": [D]
      }],
      /**
       * Space Between X Reverse
       * @see https://tailwindcss.com/docs/space
       */
      "space-x-reverse": ["space-x-reverse"],
      /**
       * Space Between Y
       * @see https://tailwindcss.com/docs/space
       */
      "space-y": [{
        "space-y": [D]
      }],
      /**
       * Space Between Y Reverse
       * @see https://tailwindcss.com/docs/space
       */
      "space-y-reverse": ["space-y-reverse"],
      // Sizing
      /**
       * Width
       * @see https://tailwindcss.com/docs/width
       */
      w: [{
        w: ["auto", "min", "max", "fit", "svw", "lvw", "dvw", y, e]
      }],
      /**
       * Min-Width
       * @see https://tailwindcss.com/docs/min-width
       */
      "min-w": [{
        "min-w": [y, e, "min", "max", "fit"]
      }],
      /**
       * Max-Width
       * @see https://tailwindcss.com/docs/max-width
       */
      "max-w": [{
        "max-w": [y, e, "none", "full", "min", "max", "fit", "prose", {
          screen: [st]
        }, st]
      }],
      /**
       * Height
       * @see https://tailwindcss.com/docs/height
       */
      h: [{
        h: [y, e, "auto", "min", "max", "fit", "svh", "lvh", "dvh"]
      }],
      /**
       * Min-Height
       * @see https://tailwindcss.com/docs/min-height
       */
      "min-h": [{
        "min-h": [y, e, "min", "max", "fit", "svh", "lvh", "dvh"]
      }],
      /**
       * Max-Height
       * @see https://tailwindcss.com/docs/max-height
       */
      "max-h": [{
        "max-h": [y, e, "min", "max", "fit", "svh", "lvh", "dvh"]
      }],
      /**
       * Size
       * @see https://tailwindcss.com/docs/size
       */
      size: [{
        size: [y, e, "auto", "min", "max", "fit"]
      }],
      // Typography
      /**
       * Font Size
       * @see https://tailwindcss.com/docs/font-size
       */
      "font-size": [{
        text: ["base", st, it]
      }],
      /**
       * Font Smoothing
       * @see https://tailwindcss.com/docs/font-smoothing
       */
      "font-smoothing": ["antialiased", "subpixel-antialiased"],
      /**
       * Font Style
       * @see https://tailwindcss.com/docs/font-style
       */
      "font-style": ["italic", "not-italic"],
      /**
       * Font Weight
       * @see https://tailwindcss.com/docs/font-weight
       */
      "font-weight": [{
        font: ["thin", "extralight", "light", "normal", "medium", "semibold", "bold", "extrabold", "black", He]
      }],
      /**
       * Font Family
       * @see https://tailwindcss.com/docs/font-family
       */
      "font-family": [{
        font: [Gt]
      }],
      /**
       * Font Variant Numeric
       * @see https://tailwindcss.com/docs/font-variant-numeric
       */
      "fvn-normal": ["normal-nums"],
      /**
       * Font Variant Numeric
       * @see https://tailwindcss.com/docs/font-variant-numeric
       */
      "fvn-ordinal": ["ordinal"],
      /**
       * Font Variant Numeric
       * @see https://tailwindcss.com/docs/font-variant-numeric
       */
      "fvn-slashed-zero": ["slashed-zero"],
      /**
       * Font Variant Numeric
       * @see https://tailwindcss.com/docs/font-variant-numeric
       */
      "fvn-figure": ["lining-nums", "oldstyle-nums"],
      /**
       * Font Variant Numeric
       * @see https://tailwindcss.com/docs/font-variant-numeric
       */
      "fvn-spacing": ["proportional-nums", "tabular-nums"],
      /**
       * Font Variant Numeric
       * @see https://tailwindcss.com/docs/font-variant-numeric
       */
      "fvn-fraction": ["diagonal-fractions", "stacked-fractons"],
      /**
       * Letter Spacing
       * @see https://tailwindcss.com/docs/letter-spacing
       */
      tracking: [{
        tracking: ["tighter", "tight", "normal", "wide", "wider", "widest", y]
      }],
      /**
       * Line Clamp
       * @see https://tailwindcss.com/docs/line-clamp
       */
      "line-clamp": [{
        "line-clamp": ["none", Tt, He]
      }],
      /**
       * Line Height
       * @see https://tailwindcss.com/docs/line-height
       */
      leading: [{
        leading: ["none", "tight", "snug", "normal", "relaxed", "loose", K, y]
      }],
      /**
       * List Style Image
       * @see https://tailwindcss.com/docs/list-style-image
       */
      "list-image": [{
        "list-image": ["none", y]
      }],
      /**
       * List Style Type
       * @see https://tailwindcss.com/docs/list-style-type
       */
      "list-style-type": [{
        list: ["none", "disc", "decimal", y]
      }],
      /**
       * List Style Position
       * @see https://tailwindcss.com/docs/list-style-position
       */
      "list-style-position": [{
        list: ["inside", "outside"]
      }],
      /**
       * Placeholder Color
       * @deprecated since Tailwind CSS v3.0.0
       * @see https://tailwindcss.com/docs/placeholder-color
       */
      "placeholder-color": [{
        placeholder: [t]
      }],
      /**
       * Placeholder Opacity
       * @see https://tailwindcss.com/docs/placeholder-opacity
       */
      "placeholder-opacity": [{
        "placeholder-opacity": [p]
      }],
      /**
       * Text Alignment
       * @see https://tailwindcss.com/docs/text-align
       */
      "text-alignment": [{
        text: ["left", "center", "right", "justify", "start", "end"]
      }],
      /**
       * Text Color
       * @see https://tailwindcss.com/docs/text-color
       */
      "text-color": [{
        text: [t]
      }],
      /**
       * Text Opacity
       * @see https://tailwindcss.com/docs/text-opacity
       */
      "text-opacity": [{
        "text-opacity": [p]
      }],
      /**
       * Text Decoration
       * @see https://tailwindcss.com/docs/text-decoration
       */
      "text-decoration": ["underline", "overline", "line-through", "no-underline"],
      /**
       * Text Decoration Style
       * @see https://tailwindcss.com/docs/text-decoration-style
       */
      "text-decoration-style": [{
        decoration: [...qt(), "wavy"]
      }],
      /**
       * Text Decoration Thickness
       * @see https://tailwindcss.com/docs/text-decoration-thickness
       */
      "text-decoration-thickness": [{
        decoration: ["auto", "from-font", K, it]
      }],
      /**
       * Text Underline Offset
       * @see https://tailwindcss.com/docs/text-underline-offset
       */
      "underline-offset": [{
        "underline-offset": ["auto", K, y]
      }],
      /**
       * Text Decoration Color
       * @see https://tailwindcss.com/docs/text-decoration-color
       */
      "text-decoration-color": [{
        decoration: [t]
      }],
      /**
       * Text Transform
       * @see https://tailwindcss.com/docs/text-transform
       */
      "text-transform": ["uppercase", "lowercase", "capitalize", "normal-case"],
      /**
       * Text Overflow
       * @see https://tailwindcss.com/docs/text-overflow
       */
      "text-overflow": ["truncate", "text-ellipsis", "text-clip"],
      /**
       * Text Wrap
       * @see https://tailwindcss.com/docs/text-wrap
       */
      "text-wrap": [{
        text: ["wrap", "nowrap", "balance", "pretty"]
      }],
      /**
       * Text Indent
       * @see https://tailwindcss.com/docs/text-indent
       */
      indent: [{
        indent: f()
      }],
      /**
       * Vertical Alignment
       * @see https://tailwindcss.com/docs/vertical-align
       */
      "vertical-align": [{
        align: ["baseline", "top", "middle", "bottom", "text-top", "text-bottom", "sub", "super", y]
      }],
      /**
       * Whitespace
       * @see https://tailwindcss.com/docs/whitespace
       */
      whitespace: [{
        whitespace: ["normal", "nowrap", "pre", "pre-line", "pre-wrap", "break-spaces"]
      }],
      /**
       * Word Break
       * @see https://tailwindcss.com/docs/word-break
       */
      break: [{
        break: ["normal", "words", "all", "keep"]
      }],
      /**
       * Hyphens
       * @see https://tailwindcss.com/docs/hyphens
       */
      hyphens: [{
        hyphens: ["none", "manual", "auto"]
      }],
      /**
       * Content
       * @see https://tailwindcss.com/docs/content
       */
      content: [{
        content: ["none", y]
      }],
      // Backgrounds
      /**
       * Background Attachment
       * @see https://tailwindcss.com/docs/background-attachment
       */
      "bg-attachment": [{
        bg: ["fixed", "local", "scroll"]
      }],
      /**
       * Background Clip
       * @see https://tailwindcss.com/docs/background-clip
       */
      "bg-clip": [{
        "bg-clip": ["border", "padding", "content", "text"]
      }],
      /**
       * Background Opacity
       * @deprecated since Tailwind CSS v3.0.0
       * @see https://tailwindcss.com/docs/background-opacity
       */
      "bg-opacity": [{
        "bg-opacity": [p]
      }],
      /**
       * Background Origin
       * @see https://tailwindcss.com/docs/background-origin
       */
      "bg-origin": [{
        "bg-origin": ["border", "padding", "content"]
      }],
      /**
       * Background Position
       * @see https://tailwindcss.com/docs/background-position
       */
      "bg-position": [{
        bg: [...Gn(), ud]
      }],
      /**
       * Background Repeat
       * @see https://tailwindcss.com/docs/background-repeat
       */
      "bg-repeat": [{
        bg: ["no-repeat", {
          repeat: ["", "x", "y", "round", "space"]
        }]
      }],
      /**
       * Background Size
       * @see https://tailwindcss.com/docs/background-size
       */
      "bg-size": [{
        bg: ["auto", "cover", "contain", ld]
      }],
      /**
       * Background Image
       * @see https://tailwindcss.com/docs/background-image
       */
      "bg-image": [{
        bg: ["none", {
          "gradient-to": ["t", "tr", "r", "br", "b", "bl", "l", "tl"]
        }, Md]
      }],
      /**
       * Background Color
       * @see https://tailwindcss.com/docs/background-color
       */
      "bg-color": [{
        bg: [t]
      }],
      /**
       * Gradient Color Stops From Position
       * @see https://tailwindcss.com/docs/gradient-color-stops
       */
      "gradient-from-pos": [{
        from: [N]
      }],
      /**
       * Gradient Color Stops Via Position
       * @see https://tailwindcss.com/docs/gradient-color-stops
       */
      "gradient-via-pos": [{
        via: [N]
      }],
      /**
       * Gradient Color Stops To Position
       * @see https://tailwindcss.com/docs/gradient-color-stops
       */
      "gradient-to-pos": [{
        to: [N]
      }],
      /**
       * Gradient Color Stops From
       * @see https://tailwindcss.com/docs/gradient-color-stops
       */
      "gradient-from": [{
        from: [g]
      }],
      /**
       * Gradient Color Stops Via
       * @see https://tailwindcss.com/docs/gradient-color-stops
       */
      "gradient-via": [{
        via: [g]
      }],
      /**
       * Gradient Color Stops To
       * @see https://tailwindcss.com/docs/gradient-color-stops
       */
      "gradient-to": [{
        to: [g]
      }],
      // Borders
      /**
       * Border Radius
       * @see https://tailwindcss.com/docs/border-radius
       */
      rounded: [{
        rounded: [s]
      }],
      /**
       * Border Radius Start
       * @see https://tailwindcss.com/docs/border-radius
       */
      "rounded-s": [{
        "rounded-s": [s]
      }],
      /**
       * Border Radius End
       * @see https://tailwindcss.com/docs/border-radius
       */
      "rounded-e": [{
        "rounded-e": [s]
      }],
      /**
       * Border Radius Top
       * @see https://tailwindcss.com/docs/border-radius
       */
      "rounded-t": [{
        "rounded-t": [s]
      }],
      /**
       * Border Radius Right
       * @see https://tailwindcss.com/docs/border-radius
       */
      "rounded-r": [{
        "rounded-r": [s]
      }],
      /**
       * Border Radius Bottom
       * @see https://tailwindcss.com/docs/border-radius
       */
      "rounded-b": [{
        "rounded-b": [s]
      }],
      /**
       * Border Radius Left
       * @see https://tailwindcss.com/docs/border-radius
       */
      "rounded-l": [{
        "rounded-l": [s]
      }],
      /**
       * Border Radius Start Start
       * @see https://tailwindcss.com/docs/border-radius
       */
      "rounded-ss": [{
        "rounded-ss": [s]
      }],
      /**
       * Border Radius Start End
       * @see https://tailwindcss.com/docs/border-radius
       */
      "rounded-se": [{
        "rounded-se": [s]
      }],
      /**
       * Border Radius End End
       * @see https://tailwindcss.com/docs/border-radius
       */
      "rounded-ee": [{
        "rounded-ee": [s]
      }],
      /**
       * Border Radius End Start
       * @see https://tailwindcss.com/docs/border-radius
       */
      "rounded-es": [{
        "rounded-es": [s]
      }],
      /**
       * Border Radius Top Left
       * @see https://tailwindcss.com/docs/border-radius
       */
      "rounded-tl": [{
        "rounded-tl": [s]
      }],
      /**
       * Border Radius Top Right
       * @see https://tailwindcss.com/docs/border-radius
       */
      "rounded-tr": [{
        "rounded-tr": [s]
      }],
      /**
       * Border Radius Bottom Right
       * @see https://tailwindcss.com/docs/border-radius
       */
      "rounded-br": [{
        "rounded-br": [s]
      }],
      /**
       * Border Radius Bottom Left
       * @see https://tailwindcss.com/docs/border-radius
       */
      "rounded-bl": [{
        "rounded-bl": [s]
      }],
      /**
       * Border Width
       * @see https://tailwindcss.com/docs/border-width
       */
      "border-w": [{
        border: [c]
      }],
      /**
       * Border Width X
       * @see https://tailwindcss.com/docs/border-width
       */
      "border-w-x": [{
        "border-x": [c]
      }],
      /**
       * Border Width Y
       * @see https://tailwindcss.com/docs/border-width
       */
      "border-w-y": [{
        "border-y": [c]
      }],
      /**
       * Border Width Start
       * @see https://tailwindcss.com/docs/border-width
       */
      "border-w-s": [{
        "border-s": [c]
      }],
      /**
       * Border Width End
       * @see https://tailwindcss.com/docs/border-width
       */
      "border-w-e": [{
        "border-e": [c]
      }],
      /**
       * Border Width Top
       * @see https://tailwindcss.com/docs/border-width
       */
      "border-w-t": [{
        "border-t": [c]
      }],
      /**
       * Border Width Right
       * @see https://tailwindcss.com/docs/border-width
       */
      "border-w-r": [{
        "border-r": [c]
      }],
      /**
       * Border Width Bottom
       * @see https://tailwindcss.com/docs/border-width
       */
      "border-w-b": [{
        "border-b": [c]
      }],
      /**
       * Border Width Left
       * @see https://tailwindcss.com/docs/border-width
       */
      "border-w-l": [{
        "border-l": [c]
      }],
      /**
       * Border Opacity
       * @see https://tailwindcss.com/docs/border-opacity
       */
      "border-opacity": [{
        "border-opacity": [p]
      }],
      /**
       * Border Style
       * @see https://tailwindcss.com/docs/border-style
       */
      "border-style": [{
        border: [...qt(), "hidden"]
      }],
      /**
       * Divide Width X
       * @see https://tailwindcss.com/docs/divide-width
       */
      "divide-x": [{
        "divide-x": [c]
      }],
      /**
       * Divide Width X Reverse
       * @see https://tailwindcss.com/docs/divide-width
       */
      "divide-x-reverse": ["divide-x-reverse"],
      /**
       * Divide Width Y
       * @see https://tailwindcss.com/docs/divide-width
       */
      "divide-y": [{
        "divide-y": [c]
      }],
      /**
       * Divide Width Y Reverse
       * @see https://tailwindcss.com/docs/divide-width
       */
      "divide-y-reverse": ["divide-y-reverse"],
      /**
       * Divide Opacity
       * @see https://tailwindcss.com/docs/divide-opacity
       */
      "divide-opacity": [{
        "divide-opacity": [p]
      }],
      /**
       * Divide Style
       * @see https://tailwindcss.com/docs/divide-style
       */
      "divide-style": [{
        divide: qt()
      }],
      /**
       * Border Color
       * @see https://tailwindcss.com/docs/border-color
       */
      "border-color": [{
        border: [o]
      }],
      /**
       * Border Color X
       * @see https://tailwindcss.com/docs/border-color
       */
      "border-color-x": [{
        "border-x": [o]
      }],
      /**
       * Border Color Y
       * @see https://tailwindcss.com/docs/border-color
       */
      "border-color-y": [{
        "border-y": [o]
      }],
      /**
       * Border Color S
       * @see https://tailwindcss.com/docs/border-color
       */
      "border-color-s": [{
        "border-s": [o]
      }],
      /**
       * Border Color E
       * @see https://tailwindcss.com/docs/border-color
       */
      "border-color-e": [{
        "border-e": [o]
      }],
      /**
       * Border Color Top
       * @see https://tailwindcss.com/docs/border-color
       */
      "border-color-t": [{
        "border-t": [o]
      }],
      /**
       * Border Color Right
       * @see https://tailwindcss.com/docs/border-color
       */
      "border-color-r": [{
        "border-r": [o]
      }],
      /**
       * Border Color Bottom
       * @see https://tailwindcss.com/docs/border-color
       */
      "border-color-b": [{
        "border-b": [o]
      }],
      /**
       * Border Color Left
       * @see https://tailwindcss.com/docs/border-color
       */
      "border-color-l": [{
        "border-l": [o]
      }],
      /**
       * Divide Color
       * @see https://tailwindcss.com/docs/divide-color
       */
      "divide-color": [{
        divide: [o]
      }],
      /**
       * Outline Style
       * @see https://tailwindcss.com/docs/outline-style
       */
      "outline-style": [{
        outline: ["", ...qt()]
      }],
      /**
       * Outline Offset
       * @see https://tailwindcss.com/docs/outline-offset
       */
      "outline-offset": [{
        "outline-offset": [K, y]
      }],
      /**
       * Outline Width
       * @see https://tailwindcss.com/docs/outline-width
       */
      "outline-w": [{
        outline: [K, it]
      }],
      /**
       * Outline Color
       * @see https://tailwindcss.com/docs/outline-color
       */
      "outline-color": [{
        outline: [t]
      }],
      /**
       * Ring Width
       * @see https://tailwindcss.com/docs/ring-width
       */
      "ring-w": [{
        ring: Xt()
      }],
      /**
       * Ring Width Inset
       * @see https://tailwindcss.com/docs/ring-width
       */
      "ring-w-inset": ["ring-inset"],
      /**
       * Ring Color
       * @see https://tailwindcss.com/docs/ring-color
       */
      "ring-color": [{
        ring: [t]
      }],
      /**
       * Ring Opacity
       * @see https://tailwindcss.com/docs/ring-opacity
       */
      "ring-opacity": [{
        "ring-opacity": [p]
      }],
      /**
       * Ring Offset Width
       * @see https://tailwindcss.com/docs/ring-offset-width
       */
      "ring-offset-w": [{
        "ring-offset": [K, it]
      }],
      /**
       * Ring Offset Color
       * @see https://tailwindcss.com/docs/ring-offset-color
       */
      "ring-offset-color": [{
        "ring-offset": [t]
      }],
      // Effects
      /**
       * Box Shadow
       * @see https://tailwindcss.com/docs/box-shadow
       */
      shadow: [{
        shadow: ["", "inner", "none", st, Id]
      }],
      /**
       * Box Shadow Color
       * @see https://tailwindcss.com/docs/box-shadow-color
       */
      "shadow-color": [{
        shadow: [Gt]
      }],
      /**
       * Opacity
       * @see https://tailwindcss.com/docs/opacity
       */
      opacity: [{
        opacity: [p]
      }],
      /**
       * Mix Blend Mode
       * @see https://tailwindcss.com/docs/mix-blend-mode
       */
      "mix-blend": [{
        "mix-blend": [...Yn(), "plus-lighter", "plus-darker"]
      }],
      /**
       * Background Blend Mode
       * @see https://tailwindcss.com/docs/background-blend-mode
       */
      "bg-blend": [{
        "bg-blend": Yn()
      }],
      // Filters
      /**
       * Filter
       * @deprecated since Tailwind CSS v3.0.0
       * @see https://tailwindcss.com/docs/filter
       */
      filter: [{
        filter: ["", "none"]
      }],
      /**
       * Blur
       * @see https://tailwindcss.com/docs/blur
       */
      blur: [{
        blur: [n]
      }],
      /**
       * Brightness
       * @see https://tailwindcss.com/docs/brightness
       */
      brightness: [{
        brightness: [r]
      }],
      /**
       * Contrast
       * @see https://tailwindcss.com/docs/contrast
       */
      contrast: [{
        contrast: [l]
      }],
      /**
       * Drop Shadow
       * @see https://tailwindcss.com/docs/drop-shadow
       */
      "drop-shadow": [{
        "drop-shadow": ["", "none", st, y]
      }],
      /**
       * Grayscale
       * @see https://tailwindcss.com/docs/grayscale
       */
      grayscale: [{
        grayscale: [u]
      }],
      /**
       * Hue Rotate
       * @see https://tailwindcss.com/docs/hue-rotate
       */
      "hue-rotate": [{
        "hue-rotate": [d]
      }],
      /**
       * Invert
       * @see https://tailwindcss.com/docs/invert
       */
      invert: [{
        invert: [M]
      }],
      /**
       * Saturate
       * @see https://tailwindcss.com/docs/saturate
       */
      saturate: [{
        saturate: [Z]
      }],
      /**
       * Sepia
       * @see https://tailwindcss.com/docs/sepia
       */
      sepia: [{
        sepia: [H]
      }],
      /**
       * Backdrop Filter
       * @deprecated since Tailwind CSS v3.0.0
       * @see https://tailwindcss.com/docs/backdrop-filter
       */
      "backdrop-filter": [{
        "backdrop-filter": ["", "none"]
      }],
      /**
       * Backdrop Blur
       * @see https://tailwindcss.com/docs/backdrop-blur
       */
      "backdrop-blur": [{
        "backdrop-blur": [n]
      }],
      /**
       * Backdrop Brightness
       * @see https://tailwindcss.com/docs/backdrop-brightness
       */
      "backdrop-brightness": [{
        "backdrop-brightness": [r]
      }],
      /**
       * Backdrop Contrast
       * @see https://tailwindcss.com/docs/backdrop-contrast
       */
      "backdrop-contrast": [{
        "backdrop-contrast": [l]
      }],
      /**
       * Backdrop Grayscale
       * @see https://tailwindcss.com/docs/backdrop-grayscale
       */
      "backdrop-grayscale": [{
        "backdrop-grayscale": [u]
      }],
      /**
       * Backdrop Hue Rotate
       * @see https://tailwindcss.com/docs/backdrop-hue-rotate
       */
      "backdrop-hue-rotate": [{
        "backdrop-hue-rotate": [d]
      }],
      /**
       * Backdrop Invert
       * @see https://tailwindcss.com/docs/backdrop-invert
       */
      "backdrop-invert": [{
        "backdrop-invert": [M]
      }],
      /**
       * Backdrop Opacity
       * @see https://tailwindcss.com/docs/backdrop-opacity
       */
      "backdrop-opacity": [{
        "backdrop-opacity": [p]
      }],
      /**
       * Backdrop Saturate
       * @see https://tailwindcss.com/docs/backdrop-saturate
       */
      "backdrop-saturate": [{
        "backdrop-saturate": [Z]
      }],
      /**
       * Backdrop Sepia
       * @see https://tailwindcss.com/docs/backdrop-sepia
       */
      "backdrop-sepia": [{
        "backdrop-sepia": [H]
      }],
      // Tables
      /**
       * Border Collapse
       * @see https://tailwindcss.com/docs/border-collapse
       */
      "border-collapse": [{
        border: ["collapse", "separate"]
      }],
      /**
       * Border Spacing
       * @see https://tailwindcss.com/docs/border-spacing
       */
      "border-spacing": [{
        "border-spacing": [a]
      }],
      /**
       * Border Spacing X
       * @see https://tailwindcss.com/docs/border-spacing
       */
      "border-spacing-x": [{
        "border-spacing-x": [a]
      }],
      /**
       * Border Spacing Y
       * @see https://tailwindcss.com/docs/border-spacing
       */
      "border-spacing-y": [{
        "border-spacing-y": [a]
      }],
      /**
       * Table Layout
       * @see https://tailwindcss.com/docs/table-layout
       */
      "table-layout": [{
        table: ["auto", "fixed"]
      }],
      /**
       * Caption Side
       * @see https://tailwindcss.com/docs/caption-side
       */
      caption: [{
        caption: ["top", "bottom"]
      }],
      // Transitions and Animation
      /**
       * Tranisition Property
       * @see https://tailwindcss.com/docs/transition-property
       */
      transition: [{
        transition: ["none", "all", "", "colors", "opacity", "shadow", "transform", y]
      }],
      /**
       * Transition Duration
       * @see https://tailwindcss.com/docs/transition-duration
       */
      duration: [{
        duration: $()
      }],
      /**
       * Transition Timing Function
       * @see https://tailwindcss.com/docs/transition-timing-function
       */
      ease: [{
        ease: ["linear", "in", "out", "in-out", y]
      }],
      /**
       * Transition Delay
       * @see https://tailwindcss.com/docs/transition-delay
       */
      delay: [{
        delay: $()
      }],
      /**
       * Animation
       * @see https://tailwindcss.com/docs/animation
       */
      animate: [{
        animate: ["none", "spin", "ping", "pulse", "bounce", y]
      }],
      // Transforms
      /**
       * Transform
       * @see https://tailwindcss.com/docs/transform
       */
      transform: [{
        transform: ["", "gpu", "none"]
      }],
      /**
       * Scale
       * @see https://tailwindcss.com/docs/scale
       */
      scale: [{
        scale: [C]
      }],
      /**
       * Scale X
       * @see https://tailwindcss.com/docs/scale
       */
      "scale-x": [{
        "scale-x": [C]
      }],
      /**
       * Scale Y
       * @see https://tailwindcss.com/docs/scale
       */
      "scale-y": [{
        "scale-y": [C]
      }],
      /**
       * Rotate
       * @see https://tailwindcss.com/docs/rotate
       */
      rotate: [{
        rotate: [Ht, y]
      }],
      /**
       * Translate X
       * @see https://tailwindcss.com/docs/translate
       */
      "translate-x": [{
        "translate-x": [L]
      }],
      /**
       * Translate Y
       * @see https://tailwindcss.com/docs/translate
       */
      "translate-y": [{
        "translate-y": [L]
      }],
      /**
       * Skew X
       * @see https://tailwindcss.com/docs/skew
       */
      "skew-x": [{
        "skew-x": [S]
      }],
      /**
       * Skew Y
       * @see https://tailwindcss.com/docs/skew
       */
      "skew-y": [{
        "skew-y": [S]
      }],
      /**
       * Transform Origin
       * @see https://tailwindcss.com/docs/transform-origin
       */
      "transform-origin": [{
        origin: ["center", "top", "top-right", "right", "bottom-right", "bottom", "bottom-left", "left", "top-left", y]
      }],
      // Interactivity
      /**
       * Accent Color
       * @see https://tailwindcss.com/docs/accent-color
       */
      accent: [{
        accent: ["auto", t]
      }],
      /**
       * Appearance
       * @see https://tailwindcss.com/docs/appearance
       */
      appearance: [{
        appearance: ["none", "auto"]
      }],
      /**
       * Cursor
       * @see https://tailwindcss.com/docs/cursor
       */
      cursor: [{
        cursor: ["auto", "default", "pointer", "wait", "text", "move", "help", "not-allowed", "none", "context-menu", "progress", "cell", "crosshair", "vertical-text", "alias", "copy", "no-drop", "grab", "grabbing", "all-scroll", "col-resize", "row-resize", "n-resize", "e-resize", "s-resize", "w-resize", "ne-resize", "nw-resize", "se-resize", "sw-resize", "ew-resize", "ns-resize", "nesw-resize", "nwse-resize", "zoom-in", "zoom-out", y]
      }],
      /**
       * Caret Color
       * @see https://tailwindcss.com/docs/just-in-time-mode#caret-color-utilities
       */
      "caret-color": [{
        caret: [t]
      }],
      /**
       * Pointer Events
       * @see https://tailwindcss.com/docs/pointer-events
       */
      "pointer-events": [{
        "pointer-events": ["none", "auto"]
      }],
      /**
       * Resize
       * @see https://tailwindcss.com/docs/resize
       */
      resize: [{
        resize: ["none", "y", "x", ""]
      }],
      /**
       * Scroll Behavior
       * @see https://tailwindcss.com/docs/scroll-behavior
       */
      "scroll-behavior": [{
        scroll: ["auto", "smooth"]
      }],
      /**
       * Scroll Margin
       * @see https://tailwindcss.com/docs/scroll-margin
       */
      "scroll-m": [{
        "scroll-m": f()
      }],
      /**
       * Scroll Margin X
       * @see https://tailwindcss.com/docs/scroll-margin
       */
      "scroll-mx": [{
        "scroll-mx": f()
      }],
      /**
       * Scroll Margin Y
       * @see https://tailwindcss.com/docs/scroll-margin
       */
      "scroll-my": [{
        "scroll-my": f()
      }],
      /**
       * Scroll Margin Start
       * @see https://tailwindcss.com/docs/scroll-margin
       */
      "scroll-ms": [{
        "scroll-ms": f()
      }],
      /**
       * Scroll Margin End
       * @see https://tailwindcss.com/docs/scroll-margin
       */
      "scroll-me": [{
        "scroll-me": f()
      }],
      /**
       * Scroll Margin Top
       * @see https://tailwindcss.com/docs/scroll-margin
       */
      "scroll-mt": [{
        "scroll-mt": f()
      }],
      /**
       * Scroll Margin Right
       * @see https://tailwindcss.com/docs/scroll-margin
       */
      "scroll-mr": [{
        "scroll-mr": f()
      }],
      /**
       * Scroll Margin Bottom
       * @see https://tailwindcss.com/docs/scroll-margin
       */
      "scroll-mb": [{
        "scroll-mb": f()
      }],
      /**
       * Scroll Margin Left
       * @see https://tailwindcss.com/docs/scroll-margin
       */
      "scroll-ml": [{
        "scroll-ml": f()
      }],
      /**
       * Scroll Padding
       * @see https://tailwindcss.com/docs/scroll-padding
       */
      "scroll-p": [{
        "scroll-p": f()
      }],
      /**
       * Scroll Padding X
       * @see https://tailwindcss.com/docs/scroll-padding
       */
      "scroll-px": [{
        "scroll-px": f()
      }],
      /**
       * Scroll Padding Y
       * @see https://tailwindcss.com/docs/scroll-padding
       */
      "scroll-py": [{
        "scroll-py": f()
      }],
      /**
       * Scroll Padding Start
       * @see https://tailwindcss.com/docs/scroll-padding
       */
      "scroll-ps": [{
        "scroll-ps": f()
      }],
      /**
       * Scroll Padding End
       * @see https://tailwindcss.com/docs/scroll-padding
       */
      "scroll-pe": [{
        "scroll-pe": f()
      }],
      /**
       * Scroll Padding Top
       * @see https://tailwindcss.com/docs/scroll-padding
       */
      "scroll-pt": [{
        "scroll-pt": f()
      }],
      /**
       * Scroll Padding Right
       * @see https://tailwindcss.com/docs/scroll-padding
       */
      "scroll-pr": [{
        "scroll-pr": f()
      }],
      /**
       * Scroll Padding Bottom
       * @see https://tailwindcss.com/docs/scroll-padding
       */
      "scroll-pb": [{
        "scroll-pb": f()
      }],
      /**
       * Scroll Padding Left
       * @see https://tailwindcss.com/docs/scroll-padding
       */
      "scroll-pl": [{
        "scroll-pl": f()
      }],
      /**
       * Scroll Snap Align
       * @see https://tailwindcss.com/docs/scroll-snap-align
       */
      "snap-align": [{
        snap: ["start", "end", "center", "align-none"]
      }],
      /**
       * Scroll Snap Stop
       * @see https://tailwindcss.com/docs/scroll-snap-stop
       */
      "snap-stop": [{
        snap: ["normal", "always"]
      }],
      /**
       * Scroll Snap Type
       * @see https://tailwindcss.com/docs/scroll-snap-type
       */
      "snap-type": [{
        snap: ["none", "x", "y", "both"]
      }],
      /**
       * Scroll Snap Type Strictness
       * @see https://tailwindcss.com/docs/scroll-snap-type
       */
      "snap-strictness": [{
        snap: ["mandatory", "proximity"]
      }],
      /**
       * Touch Action
       * @see https://tailwindcss.com/docs/touch-action
       */
      touch: [{
        touch: ["auto", "none", "manipulation"]
      }],
      /**
       * Touch Action X
       * @see https://tailwindcss.com/docs/touch-action
       */
      "touch-x": [{
        "touch-pan": ["x", "left", "right"]
      }],
      /**
       * Touch Action Y
       * @see https://tailwindcss.com/docs/touch-action
       */
      "touch-y": [{
        "touch-pan": ["y", "up", "down"]
      }],
      /**
       * Touch Action Pinch Zoom
       * @see https://tailwindcss.com/docs/touch-action
       */
      "touch-pz": ["touch-pinch-zoom"],
      /**
       * User Select
       * @see https://tailwindcss.com/docs/user-select
       */
      select: [{
        select: ["none", "text", "all", "auto"]
      }],
      /**
       * Will Change
       * @see https://tailwindcss.com/docs/will-change
       */
      "will-change": [{
        "will-change": ["auto", "scroll", "contents", "transform", y]
      }],
      // SVG
      /**
       * Fill
       * @see https://tailwindcss.com/docs/fill
       */
      fill: [{
        fill: [t, "none"]
      }],
      /**
       * Stroke Width
       * @see https://tailwindcss.com/docs/stroke-width
       */
      "stroke-w": [{
        stroke: [K, it, He]
      }],
      /**
       * Stroke
       * @see https://tailwindcss.com/docs/stroke
       */
      stroke: [{
        stroke: [t, "none"]
      }],
      // Accessibility
      /**
       * Screen Readers
       * @see https://tailwindcss.com/docs/screen-readers
       */
      sr: ["sr-only", "not-sr-only"],
      /**
       * Forced Color Adjust
       * @see https://tailwindcss.com/docs/forced-color-adjust
       */
      "forced-color-adjust": [{
        "forced-color-adjust": ["auto", "none"]
      }]
    },
    conflictingClassGroups: {
      overflow: ["overflow-x", "overflow-y"],
      overscroll: ["overscroll-x", "overscroll-y"],
      inset: ["inset-x", "inset-y", "start", "end", "top", "right", "bottom", "left"],
      "inset-x": ["right", "left"],
      "inset-y": ["top", "bottom"],
      flex: ["basis", "grow", "shrink"],
      gap: ["gap-x", "gap-y"],
      p: ["px", "py", "ps", "pe", "pt", "pr", "pb", "pl"],
      px: ["pr", "pl"],
      py: ["pt", "pb"],
      m: ["mx", "my", "ms", "me", "mt", "mr", "mb", "ml"],
      mx: ["mr", "ml"],
      my: ["mt", "mb"],
      size: ["w", "h"],
      "font-size": ["leading"],
      "fvn-normal": ["fvn-ordinal", "fvn-slashed-zero", "fvn-figure", "fvn-spacing", "fvn-fraction"],
      "fvn-ordinal": ["fvn-normal"],
      "fvn-slashed-zero": ["fvn-normal"],
      "fvn-figure": ["fvn-normal"],
      "fvn-spacing": ["fvn-normal"],
      "fvn-fraction": ["fvn-normal"],
      "line-clamp": ["display", "overflow"],
      rounded: ["rounded-s", "rounded-e", "rounded-t", "rounded-r", "rounded-b", "rounded-l", "rounded-ss", "rounded-se", "rounded-ee", "rounded-es", "rounded-tl", "rounded-tr", "rounded-br", "rounded-bl"],
      "rounded-s": ["rounded-ss", "rounded-es"],
      "rounded-e": ["rounded-se", "rounded-ee"],
      "rounded-t": ["rounded-tl", "rounded-tr"],
      "rounded-r": ["rounded-tr", "rounded-br"],
      "rounded-b": ["rounded-br", "rounded-bl"],
      "rounded-l": ["rounded-tl", "rounded-bl"],
      "border-spacing": ["border-spacing-x", "border-spacing-y"],
      "border-w": ["border-w-s", "border-w-e", "border-w-t", "border-w-r", "border-w-b", "border-w-l"],
      "border-w-x": ["border-w-r", "border-w-l"],
      "border-w-y": ["border-w-t", "border-w-b"],
      "border-color": ["border-color-s", "border-color-e", "border-color-t", "border-color-r", "border-color-b", "border-color-l"],
      "border-color-x": ["border-color-r", "border-color-l"],
      "border-color-y": ["border-color-t", "border-color-b"],
      "scroll-m": ["scroll-mx", "scroll-my", "scroll-ms", "scroll-me", "scroll-mt", "scroll-mr", "scroll-mb", "scroll-ml"],
      "scroll-mx": ["scroll-mr", "scroll-ml"],
      "scroll-my": ["scroll-mt", "scroll-mb"],
      "scroll-p": ["scroll-px", "scroll-py", "scroll-ps", "scroll-pe", "scroll-pt", "scroll-pr", "scroll-pb", "scroll-pl"],
      "scroll-px": ["scroll-pr", "scroll-pl"],
      "scroll-py": ["scroll-pt", "scroll-pb"],
      touch: ["touch-x", "touch-y", "touch-pz"],
      "touch-x": ["touch"],
      "touch-y": ["touch"],
      "touch-pz": ["touch"]
    },
    conflictingClassGroupModifiers: {
      "font-size": ["leading"]
    }
  };
}, bd = /* @__PURE__ */ Ku(md);
function wt(...t) {
  return bd(Dn(t));
}
const yd = Yr(
  "",
  {
    variants: {
      size: {
        pagetitle: "scroll-m-20 text-2xl font-bold tracking-tighter lg:text-3xl",
        1: "scroll-m-20 text-3xl font-bold tracking-tight lg:text-4xl",
        2: "scroll-m-20 border-b pb-2 text-3xl font-semibold tracking-tight first:mt-0",
        3: "text-2xl font-semibold tracking-tight",
        4: "text-xl font-semibold tracking-tight",
        5: "text-lg font-semibold tracking-tight",
        6: "text-base font-semibold tracking-tight"
      }
    },
    defaultVariants: {
      size: 1
    }
  }
), Fr = Y(
  ({ className: t, size: e = "pagetitle", asChild: n = !1, ...r }, o) => {
    const s = n ? ye : e === "pagetitle" ? "h1" : `h${e}`;
    return /* @__PURE__ */ b.jsx(
      s,
      {
        ref: o,
        className: wt(yd({ size: e, className: t })),
        ...r
      }
    );
  }
);
Fr.displayName = "Heading";
/**
 * @license lucide-react v0.460.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */
const pd = (t) => t.replace(/([a-z0-9])([A-Z])/g, "$1-$2").toLowerCase(), qr = (...t) => t.filter((e, n, r) => !!e && e.trim() !== "" && r.indexOf(e) === n).join(" ").trim();
/**
 * @license lucide-react v0.460.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */
var Dd = {
  xmlns: "http://www.w3.org/2000/svg",
  width: 24,
  height: 24,
  viewBox: "0 0 24 24",
  fill: "none",
  stroke: "currentColor",
  strokeWidth: 2,
  strokeLinecap: "round",
  strokeLinejoin: "round"
};
/**
 * @license lucide-react v0.460.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */
const hd = Y(
  ({
    color: t = "currentColor",
    size: e = 24,
    strokeWidth: n = 2,
    absoluteStrokeWidth: r,
    className: o = "",
    children: s,
    iconNode: a,
    ...c
  }, l) => i(
    "svg",
    {
      ref: l,
      ...Dd,
      width: e,
      height: e,
      stroke: t,
      strokeWidth: r ? Number(n) * 24 / Number(e) : n,
      className: qr("lucide", o),
      ...c
    },
    [
      ...a.map(([u, d]) => i(u, d)),
      ...Array.isArray(s) ? s : [s]
    ]
  )
);
/**
 * @license lucide-react v0.460.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */
const wd = (t, e) => {
  const n = Y(
    ({ className: r, ...o }, s) => i(hd, {
      ref: s,
      iconNode: e,
      className: qr(`lucide-${pd(t)}`, r),
      ...o
    })
  );
  return n.displayName = `${t}`, n;
};
/**
 * @license lucide-react v0.460.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */
const Sd = wd("ChevronRight", [
  ["path", { d: "m9 18 6-6-6-6", key: "mthhwq" }]
]), $r = Y(({ ...t }, e) => /* @__PURE__ */ b.jsx("nav", { ref: e, "aria-label": "breadcrumb", ...t }));
$r.displayName = "Breadcrumb";
const Kr = Y(({ className: t, ...e }, n) => /* @__PURE__ */ b.jsx(
  "ol",
  {
    ref: n,
    className: wt(
      "flex flex-wrap items-center gap-1.5 break-words text-sm text-neutral-500 sm:gap-2.5 dark:text-neutral-400",
      t
    ),
    ...e
  }
));
Kr.displayName = "BreadcrumbList";
const to = Y(({ className: t, ...e }, n) => /* @__PURE__ */ b.jsx(
  "li",
  {
    ref: n,
    className: wt("inline-flex items-center gap-1.5", t),
    ...e
  }
));
to.displayName = "BreadcrumbItem";
const eo = Y(({ asChild: t, className: e, ...n }, r) => {
  const o = t ? ye : "a";
  return /* @__PURE__ */ b.jsx(
    o,
    {
      ref: r,
      className: wt("transition-colors hover:text-neutral-950 dark:hover:text-neutral-50", e),
      ...n
    }
  );
});
eo.displayName = "BreadcrumbLink";
const no = Y(({ className: t, ...e }, n) => /* @__PURE__ */ b.jsx(
  "span",
  {
    ref: n,
    "aria-current": "page",
    "aria-disabled": "true",
    className: wt("font-normal text-neutral-950 dark:text-neutral-50", t),
    role: "link",
    ...e
  }
));
no.displayName = "BreadcrumbPage";
const ro = ({
  children: t,
  className: e,
  ...n
}) => /* @__PURE__ */ b.jsx(
  "li",
  {
    "aria-hidden": "true",
    className: wt("[&>svg]:w-3.5 [&>svg]:h-3.5", e),
    role: "presentation",
    ...n,
    children: t ?? /* @__PURE__ */ b.jsx(Sd, {})
  }
);
ro.displayName = "BreadcrumbSeparator";
const vd = Yr(
  "inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium ring-offset-white transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-neutral-950 focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 dark:ring-offset-neutral-950 dark:focus-visible:ring-neutral-300 [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0",
  {
    variants: {
      variant: {
        default: "bg-neutral-900 text-neutral-50 hover:bg-neutral-900/90 dark:bg-neutral-50 dark:text-neutral-900 dark:hover:bg-neutral-50/90",
        destructive: "dark:bg-red-900 dark:hover:bg-red-900/90 bg-red-500 text-neutral-50 hover:bg-red-500/90 dark:text-neutral-50",
        outline: "border border-neutral-200 bg-white hover:bg-neutral-100 hover:text-neutral-900 dark:border-neutral-800 dark:bg-neutral-950 dark:hover:bg-neutral-800 dark:hover:text-neutral-50",
        secondary: "bg-neutral-100 text-neutral-900 hover:bg-neutral-100/80 dark:bg-neutral-800 dark:text-neutral-50 dark:hover:bg-neutral-800/80",
        ghost: "hover:bg-neutral-100 hover:text-neutral-900 dark:hover:bg-neutral-800 dark:hover:text-neutral-50",
        link: "text-neutral-900 underline-offset-4 hover:underline dark:text-neutral-50"
      },
      size: {
        default: "h-9 px-3 py-2",
        sm: "h-8 rounded-md px-3",
        lg: "h-11 rounded-md px-8",
        icon: "h-9 w-9"
      }
    },
    defaultVariants: {
      variant: "default",
      size: "default"
    }
  }
), se = Y(
  ({ className: t, variant: e, size: n, asChild: r = !1, ...o }, s) => {
    const a = r ? ye : "button";
    return /* @__PURE__ */ b.jsx(
      a,
      {
        ref: s,
        className: wt(vd({ variant: e, size: n, className: t })),
        ...o
      }
    );
  }
);
se.displayName = "Button";
const Ld = U.createContext({ isDirty: !1, setGlobalDirtyState: () => {
} }), Ad = ({ children: t }) => {
  const [e, n] = bt([]), r = _((o, s) => {
    n((a) => s && !a.includes(o) ? [...a, o] : !s && a.includes(o) ? a.filter((c) => c !== o) : a);
  }, []);
  return /* @__PURE__ */ b.jsx(Ld.Provider, { value: { isDirty: e.length > 0, setGlobalDirtyState: r }, children: t });
};
ReactDOM.__SECRET_INTERNALS_DO_NOT_USE_OR_YOU_WILL_BE_FIRED;
ReactDOM.createPortal;
ReactDOM.createRoot;
ReactDOM;
ReactDOM.findDOMNode;
ReactDOM.flushSync;
ReactDOM.hydrate;
ReactDOM.hydrateRoot;
ReactDOM.render;
ReactDOM.unmountComponentAtNode;
ReactDOM.unstable_batchedUpdates;
ReactDOM.unstable_renderSubtreeIntoContainer;
ReactDOM.version;
function fd(t) {
  const e = Object.prototype.toString.call(t);
  return e === "[object Window]" || // In Electron context the Window object serializes to [object global]
  e === "[object global]";
}
function xd(t) {
  return "nodeType" in t;
}
function Cd(t) {
  var e, n;
  return t ? fd(t) ? t : xd(t) && (e = (n = t.ownerDocument) == null ? void 0 : n.defaultView) != null ? e : window : window;
}
var Fn;
(function(t) {
  t.DragStart = "dragStart", t.DragMove = "dragMove", t.DragEnd = "dragEnd", t.DragCancel = "dragCancel", t.DragOver = "dragOver", t.RegisterDroppable = "registerDroppable", t.SetDroppableDisabled = "setDroppableDisabled", t.UnregisterDroppable = "unregisterDroppable";
})(Fn || (Fn = {}));
const Td = /* @__PURE__ */ Object.freeze({
  x: 0,
  y: 0
});
function zd(t) {
  if (t.startsWith("matrix3d(")) {
    const e = t.slice(9, -1).split(/, /);
    return {
      x: +e[12],
      y: +e[13],
      scaleX: +e[0],
      scaleY: +e[5]
    };
  } else if (t.startsWith("matrix(")) {
    const e = t.slice(7, -1).split(/, /);
    return {
      x: +e[4],
      y: +e[5],
      scaleX: +e[0],
      scaleY: +e[3]
    };
  }
  return null;
}
function kd(t, e, n) {
  const r = zd(e);
  if (!r)
    return t;
  const {
    scaleX: o,
    scaleY: s,
    x: a,
    y: c
  } = r, l = t.left - a - (1 - o) * parseFloat(n), u = t.top - c - (1 - s) * parseFloat(n.slice(n.indexOf(" ") + 1)), d = o ? t.width / o : t.width, M = s ? t.height / s : t.height;
  return {
    width: d,
    height: M,
    top: u,
    right: l + d,
    bottom: u + M,
    left: l
  };
}
const Zd = {
  ignoreTransform: !1
};
function oo(t, e) {
  e === void 0 && (e = Zd);
  let n = t.getBoundingClientRect();
  if (e.ignoreTransform) {
    const {
      transform: u,
      transformOrigin: d
    } = Cd(t).getComputedStyle(t);
    u && (n = kd(n, u, d));
  }
  const {
    top: r,
    left: o,
    width: s,
    height: a,
    bottom: c,
    right: l
  } = n;
  return {
    top: r,
    left: o,
    width: s,
    height: a,
    bottom: c,
    right: l
  };
}
function qn(t) {
  return oo(t, {
    ignoreTransform: !0
  });
}
var xt;
(function(t) {
  t[t.Forward = 1] = "Forward", t[t.Backward = -1] = "Backward";
})(xt || (xt = {}));
var $n;
(function(t) {
  t.Click = "click", t.DragStart = "dragstart", t.Keydown = "keydown", t.ContextMenu = "contextmenu", t.Resize = "resize", t.SelectionChange = "selectionchange", t.VisibilityChange = "visibilitychange";
})($n || ($n = {}));
var J;
(function(t) {
  t.Space = "Space", t.Down = "ArrowDown", t.Right = "ArrowRight", t.Left = "ArrowLeft", t.Up = "ArrowUp", t.Esc = "Escape", t.Enter = "Enter";
})(J || (J = {}));
J.Space, J.Enter, J.Esc, J.Space, J.Enter;
var Kn;
(function(t) {
  t[t.RightClick = 2] = "RightClick";
})(Kn || (Kn = {}));
var tr;
(function(t) {
  t[t.Pointer = 0] = "Pointer", t[t.DraggableRect = 1] = "DraggableRect";
})(tr || (tr = {}));
var er;
(function(t) {
  t[t.TreeOrder = 0] = "TreeOrder", t[t.ReversedTreeOrder = 1] = "ReversedTreeOrder";
})(er || (er = {}));
xt.Backward + "", xt.Forward + "", xt.Backward + "", xt.Forward + "";
var qe;
(function(t) {
  t[t.Always = 0] = "Always", t[t.BeforeDragging = 1] = "BeforeDragging", t[t.WhileDragging = 2] = "WhileDragging";
})(qe || (qe = {}));
var $e;
(function(t) {
  t.Optimized = "optimized";
})($e || ($e = {}));
qe.WhileDragging, $e.Optimized;
({
  ...Td
});
var nr;
(function(t) {
  t[t.Uninitialized = 0] = "Uninitialized", t[t.Initializing = 1] = "Initializing", t[t.Initialized = 2] = "Initialized";
})(nr || (nr = {}));
J.Down, J.Right, J.Up, J.Left;
var T = globalThis && globalThis.__assign || function() {
  return T = Object.assign || function(t) {
    for (var e, n = 1, r = arguments.length; n < r; n++) {
      e = arguments[n];
      for (var o in e)
        Object.prototype.hasOwnProperty.call(e, o) && (t[o] = e[o]);
    }
    return t;
  }, T.apply(this, arguments);
}, io = globalThis && globalThis.__rest || function(t, e) {
  var n = {};
  for (var r in t)
    Object.prototype.hasOwnProperty.call(t, r) && e.indexOf(r) < 0 && (n[r] = t[r]);
  if (t != null && typeof Object.getOwnPropertySymbols == "function")
    for (var o = 0, r = Object.getOwnPropertySymbols(t); o < r.length; o++)
      e.indexOf(r[o]) < 0 && Object.prototype.propertyIsEnumerable.call(t, r[o]) && (n[r[o]] = t[r[o]]);
  return n;
}, Ge = Symbol("NiceModalId"), wn = {}, Pt = U.createContext(wn), so = U.createContext(null), X = {}, Jt = {}, Ed = 0, _t = function() {
  throw new Error("No dispatch method detected, did you embed your app with NiceModal.Provider?");
}, ao = function() {
  return "_nice_modal_" + Ed++;
}, co = function(t, e) {
  var n, r, o;
  switch (t === void 0 && (t = wn), e.type) {
    case "nice-modal/show": {
      var s = e.payload, a = s.modalId, c = s.args;
      return T(T({}, t), (n = {}, n[a] = T(T({}, t[a]), {
        id: a,
        args: c,
        // If modal is not mounted, mount it first then make it visible.
        // There is logic inside HOC wrapper to make it visible after its first mount.
        // This mechanism ensures the entering transition.
        visible: !!Jt[a],
        delayVisible: !Jt[a]
      }), n));
    }
    case "nice-modal/hide": {
      var a = e.payload.modalId;
      return t[a] ? T(T({}, t), (r = {}, r[a] = T(T({}, t[a]), { visible: !1 }), r)) : t;
    }
    case "nice-modal/remove": {
      var a = e.payload.modalId, l = T({}, t);
      return delete l[a], l;
    }
    case "nice-modal/set-flags": {
      var u = e.payload, a = u.modalId, d = u.flags;
      return T(T({}, t), (o = {}, o[a] = T(T({}, t[a]), d), o));
    }
    default:
      return t;
  }
};
function Wd(t) {
  var e;
  return (e = X[t]) === null || e === void 0 ? void 0 : e.comp;
}
function Ud(t, e) {
  return {
    type: "nice-modal/show",
    payload: {
      modalId: t,
      args: e
    }
  };
}
function Pd(t, e) {
  return {
    type: "nice-modal/set-flags",
    payload: {
      modalId: t,
      flags: e
    }
  };
}
function _d(t) {
  return {
    type: "nice-modal/hide",
    payload: {
      modalId: t
    }
  };
}
function Od(t) {
  return {
    type: "nice-modal/remove",
    payload: {
      modalId: t
    }
  };
}
var nt = {}, zt = {}, pe = function(t) {
  return typeof t == "string" ? t : (t[Ge] || (t[Ge] = ao()), t[Ge]);
};
function Sn(t, e) {
  var n = pe(t);
  if (typeof t != "string" && !X[n] && De(n, t), _t(Ud(n, e)), !nt[n]) {
    var r, o, s = new Promise(function(a, c) {
      r = a, o = c;
    });
    nt[n] = {
      resolve: r,
      reject: o,
      promise: s
    };
  }
  return nt[n].promise;
}
function vn(t) {
  var e = pe(t);
  if (_t(_d(e)), delete nt[e], !zt[e]) {
    var n, r, o = new Promise(function(s, a) {
      n = s, r = a;
    });
    zt[e] = {
      resolve: n,
      reject: r,
      promise: o
    };
  }
  return zt[e].promise;
}
var lo = function(t) {
  var e = pe(t);
  _t(Od(e)), delete nt[e], delete zt[e];
}, Rd = function(t, e) {
  _t(Pd(t, e));
};
function uo(t, e) {
  var n = Et(Pt), r = Et(so), o = null, s = t && typeof t != "string";
  if (t ? o = pe(t) : o = r, !o)
    throw new Error("No modal id found in NiceModal.useModal.");
  var a = o;
  V(function() {
    s && !X[a] && De(a, t, e);
  }, [s, a, t, e]);
  var c = n[a], l = _(function(N) {
    return Sn(a, N);
  }, [a]), u = _(function() {
    return vn(a);
  }, [a]), d = _(function() {
    return lo(a);
  }, [a]), M = _(function(N) {
    var m;
    (m = nt[a]) === null || m === void 0 || m.resolve(N), delete nt[a];
  }, [a]), I = _(function(N) {
    var m;
    (m = nt[a]) === null || m === void 0 || m.reject(N), delete nt[a];
  }, [a]), g = _(function(N) {
    var m;
    (m = zt[a]) === null || m === void 0 || m.resolve(N), delete zt[a];
  }, [a]);
  return pn(function() {
    return {
      id: a,
      args: c == null ? void 0 : c.args,
      visible: !!(c != null && c.visible),
      keepMounted: !!(c != null && c.keepMounted),
      show: l,
      hide: u,
      remove: d,
      resolve: M,
      reject: I,
      resolveHide: g
    };
  }, [
    a,
    c == null ? void 0 : c.args,
    c == null ? void 0 : c.visible,
    c == null ? void 0 : c.keepMounted,
    l,
    u,
    d,
    M,
    I,
    g
  ]);
}
var Hd = function(t) {
  return function(e) {
    var n, r = e.defaultVisible, o = e.keepMounted, s = e.id, a = io(e, ["defaultVisible", "keepMounted", "id"]), c = uo(s), l = c.args, u = c.show, d = Et(Pt), M = !!d[s];
    V(function() {
      return r && u(), Jt[s] = !0, function() {
        delete Jt[s];
      };
    }, [s, u, r]), V(function() {
      o && Rd(s, { keepMounted: !0 });
    }, [s, o]);
    var I = (n = d[s]) === null || n === void 0 ? void 0 : n.delayVisible;
    return V(function() {
      I && u(l);
    }, [I, l, u]), M ? U.createElement(
      so.Provider,
      { value: s },
      U.createElement(t, T({}, a, l))
    ) : null;
  };
}, De = function(t, e, n) {
  X[t] ? X[t].props = n : X[t] = { comp: e, props: n };
}, Gd = function(t) {
  delete X[t];
}, Mo = function() {
  var t = Et(Pt), e = Object.keys(t).filter(function(r) {
    return !!t[r];
  });
  e.forEach(function(r) {
    if (!X[r] && !Jt[r]) {
      console.warn("No modal found for id: " + r + ". Please check the id or if it is registered or declared via JSX.");
      return;
    }
  });
  var n = e.filter(function(r) {
    return X[r];
  }).map(function(r) {
    return T({ id: r }, X[r]);
  });
  return U.createElement(U.Fragment, null, n.map(function(r) {
    return U.createElement(r.comp, T({ key: r.id, id: r.id }, r.props));
  }));
}, Yd = function(t) {
  var e = t.children, n = _r(co, wn), r = n[0];
  return _t = n[1], U.createElement(
    Pt.Provider,
    { value: r },
    e,
    U.createElement(Mo, null)
  );
}, Qd = function(t) {
  var e = t.children, n = t.dispatch, r = t.modals;
  return !n || !r ? U.createElement(Yd, null, e) : (_t = n, U.createElement(
    Pt.Provider,
    { value: r },
    e,
    U.createElement(Mo, null)
  ));
}, Jd = function(t) {
  var e = t.id, n = t.component;
  return V(function() {
    return De(e, n), function() {
      Gd(e);
    };
  }, [e, n]), null;
}, Bd = function(t) {
  var e, n = t.modal, r = t.handler, o = r === void 0 ? {} : r, s = io(t, ["modal", "handler"]), a = pn(function() {
    return ao();
  }, []), c = typeof n == "string" ? (e = X[n]) === null || e === void 0 ? void 0 : e.comp : n;
  if (!o)
    throw new Error("No handler found in NiceModal.ModalHolder.");
  if (!c)
    throw new Error("No modal found for id: " + n + " in NiceModal.ModalHolder.");
  return o.show = _(function(l) {
    return Sn(a, l);
  }, [a]), o.hide = _(function() {
    return vn(a);
  }, [a]), U.createElement(c, T({ id: a }, s));
}, Vd = function(t) {
  return {
    visible: t.visible,
    onOk: function() {
      return t.hide();
    },
    onCancel: function() {
      return t.hide();
    },
    afterClose: function() {
      t.resolveHide(), t.keepMounted || t.remove();
    }
  };
}, Xd = function(t) {
  return {
    visible: t.visible,
    onClose: function() {
      return t.hide();
    },
    afterVisibleChange: function(e) {
      e || t.resolveHide(), !e && !t.keepMounted && t.remove();
    }
  };
}, Fd = function(t) {
  return {
    open: t.visible,
    onClose: function() {
      return t.hide();
    },
    onExited: function() {
      t.resolveHide(), !t.keepMounted && t.remove();
    }
  };
}, qd = function(t) {
  return {
    show: t.visible,
    onHide: function() {
      return t.hide();
    },
    onExited: function() {
      t.resolveHide(), !t.keepMounted && t.remove();
    }
  };
}, Io = {
  Provider: Qd,
  ModalDef: Jd,
  ModalHolder: Bd,
  NiceModalContext: Pt,
  create: Hd,
  register: De,
  getModal: Wd,
  show: Sn,
  hide: vn,
  remove: lo,
  useModal: uo,
  reducer: co,
  antdModal: Vd,
  antdDrawer: Xd,
  muiDialog: Fd,
  bootstrapDialog: qd
};
let $d = { data: "" }, Kd = (t) => typeof window == "object" ? ((t ? t.querySelector("#_goober") : window._goober) || Object.assign((t || document.head).appendChild(document.createElement("style")), { innerHTML: " ", id: "_goober" })).firstChild : t || $d, t1 = /(?:([\u0080-\uFFFF\w-%@]+) *:? *([^{;]+?);|([^;}{]*?) *{)|(}\s*)/g, e1 = /\/\*[^]*?\*\/|  +/g, rr = /\n+/g, lt = (t, e) => {
  let n = "", r = "", o = "";
  for (let s in t) {
    let a = t[s];
    s[0] == "@" ? s[1] == "i" ? n = s + " " + a + ";" : r += s[1] == "f" ? lt(a, s) : s + "{" + lt(a, s[1] == "k" ? "" : e) + "}" : typeof a == "object" ? r += lt(a, e ? e.replace(/([^,])+/g, (c) => s.replace(/(^:.*)|([^,])+/g, (l) => /&/.test(l) ? l.replace(/&/g, c) : c ? c + " " + l : l)) : s) : a != null && (s = /^--/.test(s) ? s : s.replace(/[A-Z]/g, "-$&").toLowerCase(), o += lt.p ? lt.p(s, a) : s + ":" + a + ";");
  }
  return n + (e && o ? e + "{" + o + "}" : o) + r;
}, tt = {}, go = (t) => {
  if (typeof t == "object") {
    let e = "";
    for (let n in t)
      e += n + go(t[n]);
    return e;
  }
  return t;
}, n1 = (t, e, n, r, o) => {
  let s = go(t), a = tt[s] || (tt[s] = ((l) => {
    let u = 0, d = 11;
    for (; u < l.length; )
      d = 101 * d + l.charCodeAt(u++) >>> 0;
    return "go" + d;
  })(s));
  if (!tt[a]) {
    let l = s !== t ? t : ((u) => {
      let d, M, I = [{}];
      for (; d = t1.exec(u.replace(e1, "")); )
        d[4] ? I.shift() : d[3] ? (M = d[3].replace(rr, " ").trim(), I.unshift(I[0][M] = I[0][M] || {})) : I[0][d[1]] = d[2].replace(rr, " ").trim();
      return I[0];
    })(t);
    tt[a] = lt(o ? { ["@keyframes " + a]: l } : l, n ? "" : "." + a);
  }
  let c = n && tt.g ? tt.g : null;
  return n && (tt.g = tt[a]), ((l, u, d, M) => {
    M ? u.data = u.data.replace(M, l) : u.data.indexOf(l) === -1 && (u.data = d ? l + u.data : u.data + l);
  })(tt[a], e, r, c), a;
}, r1 = (t, e, n) => t.reduce((r, o, s) => {
  let a = e[s];
  if (a && a.call) {
    let c = a(n), l = c && c.props && c.props.className || /^go/.test(c) && c;
    a = l ? "." + l : c && typeof c == "object" ? c.props ? "" : lt(c, "") : c === !1 ? "" : c;
  }
  return r + o + (a ?? "");
}, "");
function he(t) {
  let e = this || {}, n = t.call ? t(e.p) : t;
  return n1(n.unshift ? n.raw ? r1(n, [].slice.call(arguments, 1), e.p) : n.reduce((r, o) => Object.assign(r, o && o.call ? o(e.p) : o), {}) : n, Kd(e.target), e.g, e.o, e.k);
}
let jo, Ke, tn;
he.bind({ g: 1 });
let ot = he.bind({ k: 1 });
function o1(t, e, n, r) {
  lt.p = e, jo = t, Ke = n, tn = r;
}
function Mt(t, e) {
  let n = this || {};
  return function() {
    let r = arguments;
    function o(s, a) {
      let c = Object.assign({}, s), l = c.className || o.className;
      n.p = Object.assign({ theme: Ke && Ke() }, c), n.o = / *go\d+/.test(l), c.className = he.apply(n, r) + (l ? " " + l : ""), e && (c.ref = a);
      let u = t;
      return t[0] && (u = c.as || t, delete c.as), tn && u[0] && tn(c), jo(u, c);
    }
    return e ? e(o) : o;
  };
}
var i1 = (t) => typeof t == "function", de = (t, e) => i1(t) ? t(e) : t, s1 = (() => {
  let t = 0;
  return () => (++t).toString();
})(), No = (() => {
  let t;
  return () => {
    if (t === void 0 && typeof window < "u") {
      let e = matchMedia("(prefers-reduced-motion: reduce)");
      t = !e || e.matches;
    }
    return t;
  };
})(), a1 = 20, ae = /* @__PURE__ */ new Map(), c1 = 1e3, or = (t) => {
  if (ae.has(t))
    return;
  let e = setTimeout(() => {
    ae.delete(t), St({ type: 4, toastId: t });
  }, c1);
  ae.set(t, e);
}, l1 = (t) => {
  let e = ae.get(t);
  e && clearTimeout(e);
}, en = (t, e) => {
  switch (e.type) {
    case 0:
      return { ...t, toasts: [e.toast, ...t.toasts].slice(0, a1) };
    case 1:
      return e.toast.id && l1(e.toast.id), { ...t, toasts: t.toasts.map((s) => s.id === e.toast.id ? { ...s, ...e.toast } : s) };
    case 2:
      let { toast: n } = e;
      return t.toasts.find((s) => s.id === n.id) ? en(t, { type: 1, toast: n }) : en(t, { type: 0, toast: n });
    case 3:
      let { toastId: r } = e;
      return r ? or(r) : t.toasts.forEach((s) => {
        or(s.id);
      }), { ...t, toasts: t.toasts.map((s) => s.id === r || r === void 0 ? { ...s, visible: !1 } : s) };
    case 4:
      return e.toastId === void 0 ? { ...t, toasts: [] } : { ...t, toasts: t.toasts.filter((s) => s.id !== e.toastId) };
    case 5:
      return { ...t, pausedAt: e.time };
    case 6:
      let o = e.time - (t.pausedAt || 0);
      return { ...t, pausedAt: void 0, toasts: t.toasts.map((s) => ({ ...s, pauseDuration: s.pauseDuration + o })) };
  }
}, ce = [], le = { toasts: [], pausedAt: void 0 }, St = (t) => {
  le = en(le, t), ce.forEach((e) => {
    e(le);
  });
}, u1 = { blank: 4e3, error: 4e3, success: 2e3, loading: 1 / 0, custom: 4e3 }, d1 = (t = {}) => {
  let [e, n] = bt(le);
  V(() => (ce.push(n), () => {
    let o = ce.indexOf(n);
    o > -1 && ce.splice(o, 1);
  }), [e]);
  let r = e.toasts.map((o) => {
    var s, a;
    return { ...t, ...t[o.type], ...o, duration: o.duration || ((s = t[o.type]) == null ? void 0 : s.duration) || (t == null ? void 0 : t.duration) || u1[o.type], style: { ...t.style, ...(a = t[o.type]) == null ? void 0 : a.style, ...o.style } };
  });
  return { ...e, toasts: r };
}, M1 = (t, e = "blank", n) => ({ createdAt: Date.now(), visible: !0, type: e, ariaProps: { role: "status", "aria-live": "polite" }, message: t, pauseDuration: 0, ...n, id: (n == null ? void 0 : n.id) || s1() }), Bt = (t) => (e, n) => {
  let r = M1(e, t, n);
  return St({ type: 2, toast: r }), r.id;
}, G = (t, e) => Bt("blank")(t, e);
G.error = Bt("error");
G.success = Bt("success");
G.loading = Bt("loading");
G.custom = Bt("custom");
G.dismiss = (t) => {
  St({ type: 3, toastId: t });
};
G.remove = (t) => St({ type: 4, toastId: t });
G.promise = (t, e, n) => {
  let r = G.loading(e.loading, { ...n, ...n == null ? void 0 : n.loading });
  return t.then((o) => (G.success(de(e.success, o), { id: r, ...n, ...n == null ? void 0 : n.success }), o)).catch((o) => {
    G.error(de(e.error, o), { id: r, ...n, ...n == null ? void 0 : n.error });
  }), t;
};
var I1 = (t, e) => {
  St({ type: 1, toast: { id: t, height: e } });
}, g1 = () => {
  St({ type: 5, time: Date.now() });
}, j1 = (t) => {
  let { toasts: e, pausedAt: n } = d1(t);
  V(() => {
    if (n)
      return;
    let s = Date.now(), a = e.map((c) => {
      if (c.duration === 1 / 0)
        return;
      let l = (c.duration || 0) + c.pauseDuration - (s - c.createdAt);
      if (l < 0) {
        c.visible && G.dismiss(c.id);
        return;
      }
      return setTimeout(() => G.dismiss(c.id), l);
    });
    return () => {
      a.forEach((c) => c && clearTimeout(c));
    };
  }, [e, n]);
  let r = _(() => {
    n && St({ type: 6, time: Date.now() });
  }, [n]), o = _((s, a) => {
    let { reverseOrder: c = !1, gutter: l = 8, defaultPosition: u } = a || {}, d = e.filter((g) => (g.position || u) === (s.position || u) && g.height), M = d.findIndex((g) => g.id === s.id), I = d.filter((g, N) => N < M && g.visible).length;
    return d.filter((g) => g.visible).slice(...c ? [I + 1] : [0, I]).reduce((g, N) => g + (N.height || 0) + l, 0);
  }, [e]);
  return { toasts: e, handlers: { updateHeight: I1, startPause: g1, endPause: r, calculateOffset: o } };
}, N1 = ot`
from {
  transform: scale(0) rotate(45deg);
	opacity: 0;
}
to {
 transform: scale(1) rotate(45deg);
  opacity: 1;
}`, m1 = ot`
from {
  transform: scale(0);
  opacity: 0;
}
to {
  transform: scale(1);
  opacity: 1;
}`, b1 = ot`
from {
  transform: scale(0) rotate(90deg);
	opacity: 0;
}
to {
  transform: scale(1) rotate(90deg);
	opacity: 1;
}`, y1 = Mt("div")`
  width: 20px;
  opacity: 0;
  height: 20px;
  border-radius: 10px;
  background: ${(t) => t.primary || "#ff4b4b"};
  position: relative;
  transform: rotate(45deg);

  animation: ${N1} 0.3s cubic-bezier(0.175, 0.885, 0.32, 1.275)
    forwards;
  animation-delay: 100ms;

  &:after,
  &:before {
    content: '';
    animation: ${m1} 0.15s ease-out forwards;
    animation-delay: 150ms;
    position: absolute;
    border-radius: 3px;
    opacity: 0;
    background: ${(t) => t.secondary || "#fff"};
    bottom: 9px;
    left: 4px;
    height: 2px;
    width: 12px;
  }

  &:before {
    animation: ${b1} 0.15s ease-out forwards;
    animation-delay: 180ms;
    transform: rotate(90deg);
  }
`, p1 = ot`
  from {
    transform: rotate(0deg);
  }
  to {
    transform: rotate(360deg);
  }
`, D1 = Mt("div")`
  width: 12px;
  height: 12px;
  box-sizing: border-box;
  border: 2px solid;
  border-radius: 100%;
  border-color: ${(t) => t.secondary || "#e0e0e0"};
  border-right-color: ${(t) => t.primary || "#616161"};
  animation: ${p1} 1s linear infinite;
`, h1 = ot`
from {
  transform: scale(0) rotate(45deg);
	opacity: 0;
}
to {
  transform: scale(1) rotate(45deg);
	opacity: 1;
}`, w1 = ot`
0% {
	height: 0;
	width: 0;
	opacity: 0;
}
40% {
  height: 0;
	width: 6px;
	opacity: 1;
}
100% {
  opacity: 1;
  height: 10px;
}`, S1 = Mt("div")`
  width: 20px;
  opacity: 0;
  height: 20px;
  border-radius: 10px;
  background: ${(t) => t.primary || "#61d345"};
  position: relative;
  transform: rotate(45deg);

  animation: ${h1} 0.3s cubic-bezier(0.175, 0.885, 0.32, 1.275)
    forwards;
  animation-delay: 100ms;
  &:after {
    content: '';
    box-sizing: border-box;
    animation: ${w1} 0.2s ease-out forwards;
    opacity: 0;
    animation-delay: 200ms;
    position: absolute;
    border-right: 2px solid;
    border-bottom: 2px solid;
    border-color: ${(t) => t.secondary || "#fff"};
    bottom: 6px;
    left: 6px;
    height: 10px;
    width: 6px;
  }
`, v1 = Mt("div")`
  position: absolute;
`, L1 = Mt("div")`
  position: relative;
  display: flex;
  justify-content: center;
  align-items: center;
  min-width: 20px;
  min-height: 20px;
`, A1 = ot`
from {
  transform: scale(0.6);
  opacity: 0.4;
}
to {
  transform: scale(1);
  opacity: 1;
}`, f1 = Mt("div")`
  position: relative;
  transform: scale(0.6);
  opacity: 0.4;
  min-width: 20px;
  animation: ${A1} 0.3s 0.12s cubic-bezier(0.175, 0.885, 0.32, 1.275)
    forwards;
`, x1 = ({ toast: t }) => {
  let { icon: e, type: n, iconTheme: r } = t;
  return e !== void 0 ? typeof e == "string" ? i(f1, null, e) : e : n === "blank" ? null : i(L1, null, i(D1, { ...r }), n !== "loading" && i(v1, null, n === "error" ? i(y1, { ...r }) : i(S1, { ...r })));
}, C1 = (t) => `
0% {transform: translate3d(0,${t * -200}%,0) scale(.6); opacity:.5;}
100% {transform: translate3d(0,0,0) scale(1); opacity:1;}
`, T1 = (t) => `
0% {transform: translate3d(0,0,-1px) scale(1); opacity:1;}
100% {transform: translate3d(0,${t * -150}%,-1px) scale(.6); opacity:0;}
`, z1 = "0%{opacity:0;} 100%{opacity:1;}", k1 = "0%{opacity:1;} 100%{opacity:0;}", Z1 = Mt("div")`
  display: flex;
  align-items: center;
  background: #fff;
  color: #363636;
  line-height: 1.3;
  will-change: transform;
  box-shadow: 0 3px 10px rgba(0, 0, 0, 0.1), 0 3px 3px rgba(0, 0, 0, 0.05);
  max-width: 350px;
  pointer-events: auto;
  padding: 8px 10px;
  border-radius: 8px;
`, E1 = Mt("div")`
  display: flex;
  justify-content: center;
  margin: 4px 10px;
  color: inherit;
  flex: 1 1 auto;
  white-space: pre-line;
`, W1 = (t, e) => {
  let n = t.includes("top") ? 1 : -1, [r, o] = No() ? [z1, k1] : [C1(n), T1(n)];
  return { animation: e ? `${ot(r)} 0.35s cubic-bezier(.21,1.02,.73,1) forwards` : `${ot(o)} 0.4s forwards cubic-bezier(.06,.71,.55,1)` };
}, U1 = Pr(({ toast: t, position: e, style: n, children: r }) => {
  let o = t.height ? W1(t.position || e || "top-center", t.visible) : { opacity: 0 }, s = i(x1, { toast: t }), a = i(E1, { ...t.ariaProps }, de(t.message, t));
  return i(Z1, { className: t.className, style: { ...o, ...n, ...t.style } }, typeof r == "function" ? r({ icon: s, message: a }) : i(Ur, null, s, a));
});
o1(i);
var P1 = ({ id: t, className: e, style: n, onHeightUpdate: r, children: o }) => {
  let s = _((a) => {
    if (a) {
      let c = () => {
        let l = a.getBoundingClientRect().height;
        r(t, l);
      };
      c(), new MutationObserver(c).observe(a, { subtree: !0, childList: !0, characterData: !0 });
    }
  }, [t, r]);
  return i("div", { ref: s, className: e, style: n }, o);
}, _1 = (t, e) => {
  let n = t.includes("top"), r = n ? { top: 0 } : { bottom: 0 }, o = t.includes("center") ? { justifyContent: "center" } : t.includes("right") ? { justifyContent: "flex-end" } : {};
  return { left: 0, right: 0, display: "flex", position: "absolute", transition: No() ? void 0 : "all 230ms cubic-bezier(.21,1.02,.73,1)", transform: `translateY(${e * (n ? 1 : -1)}px)`, ...r, ...o };
}, O1 = he`
  z-index: 9999;
  > * {
    pointer-events: auto;
  }
`, $t = 16, R1 = ({ reverseOrder: t, position: e = "top-center", toastOptions: n, gutter: r, children: o, containerStyle: s, containerClassName: a }) => {
  let { toasts: c, handlers: l } = j1(n);
  return i("div", { style: { position: "fixed", zIndex: 9999, top: $t, left: $t, right: $t, bottom: $t, pointerEvents: "none", ...s }, className: a, onMouseEnter: l.startPause, onMouseLeave: l.endPause }, c.map((u) => {
    let d = u.position || e, M = l.calculateOffset(u, { reverseOrder: t, gutter: r, defaultPosition: e }), I = _1(d, M);
    return i(P1, { id: u.id, key: u.id, onHeightUpdate: l.updateHeight, className: u.visible ? O1 : "", style: I }, u.type === "custom" ? de(u.message, u) : o ? o(u) : i(U1, { toast: u, position: d }));
  }));
};
const H1 = ht({
  isAnyTextFieldFocused: !1,
  setFocusState: () => {
  },
  fetchKoenigLexical: async () => {
  },
  darkMode: !1
}), G1 = ({ fetchKoenigLexical: t, darkMode: e, children: n }) => {
  const [r, o] = bt(!1), s = (a) => {
    o(a);
  };
  return /* @__PURE__ */ b.jsx(H1.Provider, { value: { isAnyTextFieldFocused: r, setFocusState: s, fetchKoenigLexical: t, darkMode: e }, children: /* @__PURE__ */ b.jsxs(Ad, { children: [
    /* @__PURE__ */ b.jsx(R1, {}),
    /* @__PURE__ */ b.jsx(Io.Provider, { children: n })
  ] }) });
}, Y1 = ({ darkMode: t, fetchKoenigLexical: e, className: n, children: r, ...o }) => {
  const s = Dn(
    "admin-x-base",
    t && "dark",
    n
  );
  return /* @__PURE__ */ b.jsx("div", { className: s, ...o, children: /* @__PURE__ */ b.jsx(G1, { darkMode: t, fetchKoenigLexical: e, children: r }) });
}, mo = Object.prototype.toString;
function Ln(t) {
  switch (mo.call(t)) {
    case "[object Error]":
    case "[object Exception]":
    case "[object DOMException]":
      return !0;
    default:
      return pt(t, Error);
  }
}
function Ot(t, e) {
  return mo.call(t) === `[object ${e}]`;
}
function An(t) {
  return Ot(t, "ErrorEvent");
}
function ir(t) {
  return Ot(t, "DOMError");
}
function Q1(t) {
  return Ot(t, "DOMException");
}
function rt(t) {
  return Ot(t, "String");
}
function bo(t) {
  return typeof t == "object" && t !== null && "__sentry_template_string__" in t && "__sentry_template_values__" in t;
}
function yo(t) {
  return t === null || bo(t) || typeof t != "object" && typeof t != "function";
}
function we(t) {
  return Ot(t, "Object");
}
function Se(t) {
  return typeof Event < "u" && pt(t, Event);
}
function J1(t) {
  return typeof Element < "u" && pt(t, Element);
}
function B1(t) {
  return Ot(t, "RegExp");
}
function fn(t) {
  return !!(t && t.then && typeof t.then == "function");
}
function V1(t) {
  return we(t) && "nativeEvent" in t && "preventDefault" in t && "stopPropagation" in t;
}
function X1(t) {
  return typeof t == "number" && t !== t;
}
function pt(t, e) {
  try {
    return t instanceof e;
  } catch {
    return !1;
  }
}
function po(t) {
  return !!(typeof t == "object" && t !== null && (t.__isVue || t._isVue));
}
function nn(t, e = 0) {
  return typeof t != "string" || e === 0 || t.length <= e ? t : `${t.slice(0, e)}...`;
}
function sr(t, e) {
  if (!Array.isArray(t))
    return "";
  const n = [];
  for (let r = 0; r < t.length; r++) {
    const o = t[r];
    try {
      po(o) ? n.push("[VueViewModel]") : n.push(String(o));
    } catch {
      n.push("[value cannot be serialized]");
    }
  }
  return n.join(e);
}
function F1(t, e, n = !1) {
  return rt(t) ? B1(e) ? e.test(t) : rt(e) ? n ? t === e : t.includes(e) : !1 : !1;
}
function ve(t, e = [], n = !1) {
  return e.some((r) => F1(t, r, n));
}
function q1(t, e, n = 250, r, o, s, a) {
  if (!s.exception || !s.exception.values || !a || !pt(a.originalException, Error))
    return;
  const c = s.exception.values.length > 0 ? s.exception.values[s.exception.values.length - 1] : void 0;
  c && (s.exception.values = $1(
    rn(
      t,
      e,
      o,
      a.originalException,
      r,
      s.exception.values,
      c,
      0
    ),
    n
  ));
}
function rn(t, e, n, r, o, s, a, c) {
  if (s.length >= n + 1)
    return s;
  let l = [...s];
  if (pt(r[o], Error)) {
    ar(a, c);
    const u = t(e, r[o]), d = l.length;
    cr(u, o, d, c), l = rn(
      t,
      e,
      n,
      r[o],
      o,
      [u, ...l],
      u,
      d
    );
  }
  return Array.isArray(r.errors) && r.errors.forEach((u, d) => {
    if (pt(u, Error)) {
      ar(a, c);
      const M = t(e, u), I = l.length;
      cr(M, `errors[${d}]`, I, c), l = rn(
        t,
        e,
        n,
        u,
        o,
        [M, ...l],
        M,
        I
      );
    }
  }), l;
}
function ar(t, e) {
  t.mechanism = t.mechanism || { type: "generic", handled: !0 }, t.mechanism = {
    ...t.mechanism,
    ...t.type === "AggregateError" && { is_exception_group: !0 },
    exception_id: e
  };
}
function cr(t, e, n, r) {
  t.mechanism = t.mechanism || { type: "generic", handled: !0 }, t.mechanism = {
    ...t.mechanism,
    type: "chained",
    source: e,
    exception_id: n,
    parent_id: r
  };
}
function $1(t, e) {
  return t.map((n) => (n.value && (n.value = nn(n.value, e)), n));
}
function Kt(t) {
  return t && t.Math == Math ? t : void 0;
}
const v = typeof globalThis == "object" && Kt(globalThis) || // eslint-disable-next-line no-restricted-globals
typeof window == "object" && Kt(window) || typeof self == "object" && Kt(self) || typeof global == "object" && Kt(global) || function() {
  return this;
}() || {};
function xn() {
  return v;
}
function Do(t, e, n) {
  const r = n || v, o = r.__SENTRY__ = r.__SENTRY__ || {};
  return o[t] || (o[t] = e());
}
const Cn = xn(), K1 = 80;
function ho(t, e = {}) {
  if (!t)
    return "<unknown>";
  try {
    let n = t;
    const r = 5, o = [];
    let s = 0, a = 0;
    const c = " > ", l = c.length;
    let u;
    const d = Array.isArray(e) ? e : e.keyAttrs, M = !Array.isArray(e) && e.maxStringLength || K1;
    for (; n && s++ < r && (u = tM(n, d), !(u === "html" || s > 1 && a + o.length * l + u.length >= M)); )
      o.push(u), a += u.length, n = n.parentNode;
    return o.reverse().join(c);
  } catch {
    return "<unknown>";
  }
}
function tM(t, e) {
  const n = t, r = [];
  let o, s, a, c, l;
  if (!n || !n.tagName)
    return "";
  if (Cn.HTMLElement && n instanceof HTMLElement && n.dataset && n.dataset.sentryComponent)
    return n.dataset.sentryComponent;
  r.push(n.tagName.toLowerCase());
  const u = e && e.length ? e.filter((M) => n.getAttribute(M)).map((M) => [M, n.getAttribute(M)]) : null;
  if (u && u.length)
    u.forEach((M) => {
      r.push(`[${M[0]}="${M[1]}"]`);
    });
  else if (n.id && r.push(`#${n.id}`), o = n.className, o && rt(o))
    for (s = o.split(/\s+/), l = 0; l < s.length; l++)
      r.push(`.${s[l]}`);
  const d = ["aria-label", "type", "name", "title", "alt"];
  for (l = 0; l < d.length; l++)
    a = d[l], c = n.getAttribute(a), c && r.push(`[${a}="${c}"]`);
  return r.join("");
}
function eM() {
  try {
    return Cn.document.location.href;
  } catch {
    return "";
  }
}
function nM(t) {
  if (!Cn.HTMLElement)
    return null;
  let e = t;
  const n = 5;
  for (let r = 0; r < n; r++) {
    if (!e)
      return null;
    if (e instanceof HTMLElement && e.dataset.sentryComponent)
      return e.dataset.sentryComponent;
    e = e.parentNode;
  }
  return null;
}
const Vt = typeof __SENTRY_DEBUG__ > "u" || __SENTRY_DEBUG__, rM = "Sentry Logger ", on = [
  "debug",
  "info",
  "warn",
  "error",
  "log",
  "assert",
  "trace"
], Me = {};
function Tn(t) {
  if (!("console" in v))
    return t();
  const e = v.console, n = {}, r = Object.keys(Me);
  r.forEach((o) => {
    const s = Me[o];
    n[o] = e[o], e[o] = s;
  });
  try {
    return t();
  } finally {
    r.forEach((o) => {
      e[o] = n[o];
    });
  }
}
function oM() {
  let t = !1;
  const e = {
    enable: () => {
      t = !0;
    },
    disable: () => {
      t = !1;
    },
    isEnabled: () => t
  };
  return Vt ? on.forEach((n) => {
    e[n] = (...r) => {
      t && Tn(() => {
        v.console[n](`${rM}[${n}]:`, ...r);
      });
    };
  }) : on.forEach((n) => {
    e[n] = () => {
    };
  }), e;
}
const h = oM(), iM = /^(?:(\w+):)\/\/(?:(\w+)(?::(\w+)?)?@)([\w.-]+)(?::(\d+))?\/(.+)/;
function sM(t) {
  return t === "http" || t === "https";
}
function aM(t, e = !1) {
  const { host: n, path: r, pass: o, port: s, projectId: a, protocol: c, publicKey: l } = t;
  return `${c}://${l}${e && o ? `:${o}` : ""}@${n}${s ? `:${s}` : ""}/${r && `${r}/`}${a}`;
}
function cM(t) {
  const e = iM.exec(t);
  if (!e) {
    Tn(() => {
      console.error(`Invalid Sentry Dsn: ${t}`);
    });
    return;
  }
  const [n, r, o = "", s, a = "", c] = e.slice(1);
  let l = "", u = c;
  const d = u.split("/");
  if (d.length > 1 && (l = d.slice(0, -1).join("/"), u = d.pop()), u) {
    const M = u.match(/^\d+/);
    M && (u = M[0]);
  }
  return wo({ host: s, pass: o, path: l, projectId: u, port: a, protocol: n, publicKey: r });
}
function wo(t) {
  return {
    protocol: t.protocol,
    publicKey: t.publicKey || "",
    pass: t.pass || "",
    host: t.host,
    port: t.port || "",
    path: t.path || "",
    projectId: t.projectId
  };
}
function lM(t) {
  if (!Vt)
    return !0;
  const { port: e, projectId: n, protocol: r } = t;
  return ["protocol", "publicKey", "host", "projectId"].find((a) => t[a] ? !1 : (h.error(`Invalid Sentry Dsn: ${a} missing`), !0)) ? !1 : n.match(/^\d+$/) ? sM(r) ? e && isNaN(parseInt(e, 10)) ? (h.error(`Invalid Sentry Dsn: Invalid port ${e}`), !1) : !0 : (h.error(`Invalid Sentry Dsn: Invalid protocol ${r}`), !1) : (h.error(`Invalid Sentry Dsn: Invalid projectId ${n}`), !1);
}
function uM(t) {
  const e = typeof t == "string" ? cM(t) : wo(t);
  if (!(!e || !lM(e)))
    return e;
}
function E(t, e, n) {
  if (!(e in t))
    return;
  const r = t[e], o = n(r);
  typeof o == "function" && So(o, r), t[e] = o;
}
function Ie(t, e, n) {
  try {
    Object.defineProperty(t, e, {
      // enumerable: false, // the default, so we can save on bundle size by not explicitly setting it
      value: n,
      writable: !0,
      configurable: !0
    });
  } catch {
    Vt && h.log(`Failed to add non-enumerable property "${e}" to object`, t);
  }
}
function So(t, e) {
  try {
    const n = e.prototype || {};
    t.prototype = e.prototype = n, Ie(t, "__sentry_original__", e);
  } catch {
  }
}
function zn(t) {
  return t.__sentry_original__;
}
function vo(t) {
  if (Ln(t))
    return {
      message: t.message,
      name: t.name,
      stack: t.stack,
      ...ur(t)
    };
  if (Se(t)) {
    const e = {
      type: t.type,
      target: lr(t.target),
      currentTarget: lr(t.currentTarget),
      ...ur(t)
    };
    return typeof CustomEvent < "u" && pt(t, CustomEvent) && (e.detail = t.detail), e;
  } else
    return t;
}
function lr(t) {
  try {
    return J1(t) ? ho(t) : Object.prototype.toString.call(t);
  } catch {
    return "<unknown>";
  }
}
function ur(t) {
  if (typeof t == "object" && t !== null) {
    const e = {};
    for (const n in t)
      Object.prototype.hasOwnProperty.call(t, n) && (e[n] = t[n]);
    return e;
  } else
    return {};
}
function dM(t, e = 40) {
  const n = Object.keys(vo(t));
  if (n.sort(), !n.length)
    return "[object has no keys]";
  if (n[0].length >= e)
    return nn(n[0], e);
  for (let r = n.length; r > 0; r--) {
    const o = n.slice(0, r).join(", ");
    if (!(o.length > e))
      return r === n.length ? o : nn(o, e);
  }
  return "";
}
function Nt(t) {
  return sn(t, /* @__PURE__ */ new Map());
}
function sn(t, e) {
  if (MM(t)) {
    const n = e.get(t);
    if (n !== void 0)
      return n;
    const r = {};
    e.set(t, r);
    for (const o of Object.keys(t))
      typeof t[o] < "u" && (r[o] = sn(t[o], e));
    return r;
  }
  if (Array.isArray(t)) {
    const n = e.get(t);
    if (n !== void 0)
      return n;
    const r = [];
    return e.set(t, r), t.forEach((o) => {
      r.push(sn(o, e));
    }), r;
  }
  return t;
}
function MM(t) {
  if (!we(t))
    return !1;
  try {
    const e = Object.getPrototypeOf(t).constructor.name;
    return !e || e === "Object";
  } catch {
    return !0;
  }
}
const Ye = "<anonymous>";
function dt(t) {
  try {
    return !t || typeof t != "function" ? Ye : t.name || Ye;
  } catch {
    return Ye;
  }
}
const ue = {}, dr = {};
function vt(t, e) {
  ue[t] = ue[t] || [], ue[t].push(e);
}
function Lt(t, e) {
  dr[t] || (e(), dr[t] = !0);
}
function F(t, e) {
  const n = t && ue[t];
  if (n)
    for (const r of n)
      try {
        r(e);
      } catch (o) {
        Vt && h.error(
          `Error while triggering instrumentation handler.
Type: ${t}
Name: ${dt(r)}
Error:`,
          o
        );
      }
}
function IM(t) {
  const e = "console";
  vt(e, t), Lt(e, gM);
}
function gM() {
  "console" in v && on.forEach(function(t) {
    t in v.console && E(v.console, t, function(e) {
      return Me[t] = e, function(...n) {
        F("console", { args: n, level: t });
        const o = Me[t];
        o && o.apply(v.console, n);
      };
    });
  });
}
function q() {
  const t = v, e = t.crypto || t.msCrypto;
  let n = () => Math.random() * 16;
  try {
    if (e && e.randomUUID)
      return e.randomUUID().replace(/-/g, "");
    e && e.getRandomValues && (n = () => {
      const r = new Uint8Array(1);
      return e.getRandomValues(r), r[0];
    });
  } catch {
  }
  return ([1e7] + 1e3 + 4e3 + 8e3 + 1e11).replace(
    /[018]/g,
    (r) => (
      // eslint-disable-next-line no-bitwise
      (r ^ (n() & 15) >> r / 4).toString(16)
    )
  );
}
function Lo(t) {
  return t.exception && t.exception.values ? t.exception.values[0] : void 0;
}
function ut(t) {
  const { message: e, event_id: n } = t;
  if (e)
    return e;
  const r = Lo(t);
  return r ? r.type && r.value ? `${r.type}: ${r.value}` : r.type || r.value || n || "<unknown>" : n || "<unknown>";
}
function an(t, e, n) {
  const r = t.exception = t.exception || {}, o = r.values = r.values || [], s = o[0] = o[0] || {};
  s.value || (s.value = e || ""), s.type || (s.type = n || "Error");
}
function cn(t, e) {
  const n = Lo(t);
  if (!n)
    return;
  const r = { type: "generic", handled: !0 }, o = n.mechanism;
  if (n.mechanism = { ...r, ...o, ...e }, e && "data" in e) {
    const s = { ...o && o.data, ...e.data };
    n.mechanism.data = s;
  }
}
function jM(t) {
  return Array.isArray(t) ? t : [t];
}
const ft = v, NM = 1e3;
let Mr, ln, un;
function mM(t) {
  const e = "dom";
  vt(e, t), Lt(e, bM);
}
function bM() {
  if (!ft.document)
    return;
  const t = F.bind(null, "dom"), e = Ir(t, !0);
  ft.document.addEventListener("click", e, !1), ft.document.addEventListener("keypress", e, !1), ["EventTarget", "Node"].forEach((n) => {
    const r = ft[n] && ft[n].prototype;
    !r || !r.hasOwnProperty || !r.hasOwnProperty("addEventListener") || (E(r, "addEventListener", function(o) {
      return function(s, a, c) {
        if (s === "click" || s == "keypress")
          try {
            const l = this, u = l.__sentry_instrumentation_handlers__ = l.__sentry_instrumentation_handlers__ || {}, d = u[s] = u[s] || { refCount: 0 };
            if (!d.handler) {
              const M = Ir(t);
              d.handler = M, o.call(this, s, M, c);
            }
            d.refCount++;
          } catch {
          }
        return o.call(this, s, a, c);
      };
    }), E(
      r,
      "removeEventListener",
      function(o) {
        return function(s, a, c) {
          if (s === "click" || s == "keypress")
            try {
              const l = this, u = l.__sentry_instrumentation_handlers__ || {}, d = u[s];
              d && (d.refCount--, d.refCount <= 0 && (o.call(this, s, d.handler, c), d.handler = void 0, delete u[s]), Object.keys(u).length === 0 && delete l.__sentry_instrumentation_handlers__);
            } catch {
            }
          return o.call(this, s, a, c);
        };
      }
    ));
  });
}
function yM(t) {
  if (t.type !== ln)
    return !1;
  try {
    if (!t.target || t.target._sentryId !== un)
      return !1;
  } catch {
  }
  return !0;
}
function pM(t, e) {
  return t !== "keypress" ? !1 : !e || !e.tagName ? !0 : !(e.tagName === "INPUT" || e.tagName === "TEXTAREA" || e.isContentEditable);
}
function Ir(t, e = !1) {
  return (n) => {
    if (!n || n._sentryCaptured)
      return;
    const r = DM(n);
    if (pM(n.type, r))
      return;
    Ie(n, "_sentryCaptured", !0), r && !r._sentryId && Ie(r, "_sentryId", q());
    const o = n.type === "keypress" ? "input" : n.type;
    yM(n) || (t({ event: n, name: o, global: e }), ln = n.type, un = r ? r._sentryId : void 0), clearTimeout(Mr), Mr = ft.setTimeout(() => {
      un = void 0, ln = void 0;
    }, NM);
  };
}
function DM(t) {
  try {
    return t.target;
  } catch {
    return null;
  }
}
const dn = xn();
function hM() {
  if (!("fetch" in dn))
    return !1;
  try {
    return new Headers(), new Request("http://www.example.com"), new Response(), !0;
  } catch {
    return !1;
  }
}
function gr(t) {
  return t && /^function fetch\(\)\s+\{\s+\[native code\]\s+\}$/.test(t.toString());
}
function wM() {
  if (typeof EdgeRuntime == "string")
    return !0;
  if (!hM())
    return !1;
  if (gr(dn.fetch))
    return !0;
  let t = !1;
  const e = dn.document;
  if (e && typeof e.createElement == "function")
    try {
      const n = e.createElement("iframe");
      n.hidden = !0, e.head.appendChild(n), n.contentWindow && n.contentWindow.fetch && (t = gr(n.contentWindow.fetch)), e.head.removeChild(n);
    } catch (n) {
      Vt && h.warn("Could not create sandbox iframe for pure fetch check, bailing to window.fetch: ", n);
    }
  return t;
}
function SM(t) {
  const e = "fetch";
  vt(e, t), Lt(e, vM);
}
function vM() {
  wM() && E(v, "fetch", function(t) {
    return function(...e) {
      const { method: n, url: r } = LM(e), o = {
        args: e,
        fetchData: {
          method: n,
          url: r
        },
        startTimestamp: Date.now()
      };
      return F("fetch", {
        ...o
      }), t.apply(v, e).then(
        (s) => {
          const a = {
            ...o,
            endTimestamp: Date.now(),
            response: s
          };
          return F("fetch", a), s;
        },
        (s) => {
          const a = {
            ...o,
            endTimestamp: Date.now(),
            error: s
          };
          throw F("fetch", a), s;
        }
      );
    };
  });
}
function Mn(t, e) {
  return !!t && typeof t == "object" && !!t[e];
}
function jr(t) {
  return typeof t == "string" ? t : t ? Mn(t, "url") ? t.url : t.toString ? t.toString() : "" : "";
}
function LM(t) {
  if (t.length === 0)
    return { method: "GET", url: "" };
  if (t.length === 2) {
    const [n, r] = t;
    return {
      url: jr(n),
      method: Mn(r, "method") ? String(r.method).toUpperCase() : "GET"
    };
  }
  const e = t[0];
  return {
    url: jr(e),
    method: Mn(e, "method") ? String(e.method).toUpperCase() : "GET"
  };
}
let te = null;
function AM(t) {
  const e = "error";
  vt(e, t), Lt(e, fM);
}
function fM() {
  te = v.onerror, v.onerror = function(t, e, n, r, o) {
    return F("error", {
      column: r,
      error: o,
      line: n,
      msg: t,
      url: e
    }), te && !te.__SENTRY_LOADER__ ? te.apply(this, arguments) : !1;
  }, v.onerror.__SENTRY_INSTRUMENTED__ = !0;
}
let ee = null;
function xM(t) {
  const e = "unhandledrejection";
  vt(e, t), Lt(e, CM);
}
function CM() {
  ee = v.onunhandledrejection, v.onunhandledrejection = function(t) {
    return F("unhandledrejection", t), ee && !ee.__SENTRY_LOADER__ ? ee.apply(this, arguments) : !0;
  }, v.onunhandledrejection.__SENTRY_INSTRUMENTED__ = !0;
}
const ne = xn();
function TM() {
  const t = ne.chrome, e = t && t.app && t.app.runtime, n = "history" in ne && !!ne.history.pushState && !!ne.history.replaceState;
  return !e && n;
}
const Yt = v;
let re;
function zM(t) {
  const e = "history";
  vt(e, t), Lt(e, kM);
}
function kM() {
  if (!TM())
    return;
  const t = Yt.onpopstate;
  Yt.onpopstate = function(...n) {
    const r = Yt.location.href, o = re;
    if (re = r, F("history", { from: o, to: r }), t)
      try {
        return t.apply(this, n);
      } catch {
      }
  };
  function e(n) {
    return function(...r) {
      const o = r.length > 2 ? r[2] : void 0;
      if (o) {
        const s = re, a = String(o);
        re = a, F("history", { from: s, to: a });
      }
      return n.apply(this, r);
    };
  }
  E(Yt.history, "pushState", e), E(Yt.history, "replaceState", e);
}
const ZM = v, Qt = "__sentry_xhr_v3__";
function EM(t) {
  const e = "xhr";
  vt(e, t), Lt(e, WM);
}
function WM() {
  if (!ZM.XMLHttpRequest)
    return;
  const t = XMLHttpRequest.prototype;
  E(t, "open", function(e) {
    return function(...n) {
      const r = Date.now(), o = rt(n[0]) ? n[0].toUpperCase() : void 0, s = UM(n[1]);
      if (!o || !s)
        return e.apply(this, n);
      this[Qt] = {
        method: o,
        url: s,
        request_headers: {}
      }, o === "POST" && s.match(/sentry_key/) && (this.__sentry_own_request__ = !0);
      const a = () => {
        const c = this[Qt];
        if (c && this.readyState === 4) {
          try {
            c.status_code = this.status;
          } catch {
          }
          const l = {
            args: [o, s],
            endTimestamp: Date.now(),
            startTimestamp: r,
            xhr: this
          };
          F("xhr", l);
        }
      };
      return "onreadystatechange" in this && typeof this.onreadystatechange == "function" ? E(this, "onreadystatechange", function(c) {
        return function(...l) {
          return a(), c.apply(this, l);
        };
      }) : this.addEventListener("readystatechange", a), E(this, "setRequestHeader", function(c) {
        return function(...l) {
          const [u, d] = l, M = this[Qt];
          return M && rt(u) && rt(d) && (M.request_headers[u.toLowerCase()] = d), c.apply(this, l);
        };
      }), e.apply(this, n);
    };
  }), E(t, "send", function(e) {
    return function(...n) {
      const r = this[Qt];
      if (!r)
        return e.apply(this, n);
      n[0] !== void 0 && (r.body = n[0]);
      const o = {
        args: [r.method, r.url],
        startTimestamp: Date.now(),
        xhr: this
      };
      return F("xhr", o), e.apply(this, n);
    };
  });
}
function UM(t) {
  if (rt(t))
    return t;
  try {
    return t.toString();
  } catch {
  }
}
function PM() {
  const t = typeof WeakSet == "function", e = t ? /* @__PURE__ */ new WeakSet() : [];
  function n(o) {
    if (t)
      return e.has(o) ? !0 : (e.add(o), !1);
    for (let s = 0; s < e.length; s++)
      if (e[s] === o)
        return !0;
    return e.push(o), !1;
  }
  function r(o) {
    if (t)
      e.delete(o);
    else
      for (let s = 0; s < e.length; s++)
        if (e[s] === o) {
          e.splice(s, 1);
          break;
        }
  }
  return [n, r];
}
function _M(t, e = 100, n = 1 / 0) {
  try {
    return In("", t, e, n);
  } catch (r) {
    return { ERROR: `**non-serializable** (${r})` };
  }
}
function Ao(t, e = 3, n = 100 * 1024) {
  const r = _M(t, e);
  return GM(r) > n ? Ao(t, e - 1, n) : r;
}
function In(t, e, n = 1 / 0, r = 1 / 0, o = PM()) {
  const [s, a] = o;
  if (e == null || // this matches null and undefined -> eqeq not eqeqeq
  ["number", "boolean", "string"].includes(typeof e) && !X1(e))
    return e;
  const c = OM(t, e);
  if (!c.startsWith("[object "))
    return c;
  if (e.__sentry_skip_normalization__)
    return e;
  const l = typeof e.__sentry_override_normalization_depth__ == "number" ? e.__sentry_override_normalization_depth__ : n;
  if (l === 0)
    return c.replace("object ", "");
  if (s(e))
    return "[Circular ~]";
  const u = e;
  if (u && typeof u.toJSON == "function")
    try {
      const g = u.toJSON();
      return In("", g, l - 1, r, o);
    } catch {
    }
  const d = Array.isArray(e) ? [] : {};
  let M = 0;
  const I = vo(e);
  for (const g in I) {
    if (!Object.prototype.hasOwnProperty.call(I, g))
      continue;
    if (M >= r) {
      d[g] = "[MaxProperties ~]";
      break;
    }
    const N = I[g];
    d[g] = In(g, N, l - 1, r, o), M++;
  }
  return a(e), d;
}
function OM(t, e) {
  try {
    if (t === "domain" && e && typeof e == "object" && e._events)
      return "[Domain]";
    if (t === "domainEmitter")
      return "[DomainEmitter]";
    if (typeof global < "u" && e === global)
      return "[Global]";
    if (typeof window < "u" && e === window)
      return "[Window]";
    if (typeof document < "u" && e === document)
      return "[Document]";
    if (po(e))
      return "[VueViewModel]";
    if (V1(e))
      return "[SyntheticEvent]";
    if (typeof e == "number" && e !== e)
      return "[NaN]";
    if (typeof e == "function")
      return `[Function: ${dt(e)}]`;
    if (typeof e == "symbol")
      return `[${String(e)}]`;
    if (typeof e == "bigint")
      return `[BigInt: ${String(e)}]`;
    const n = RM(e);
    return /^HTML(\w*)Element$/.test(n) ? `[HTMLElement: ${n}]` : `[object ${n}]`;
  } catch (n) {
    return `**non-serializable** (${n})`;
  }
}
function RM(t) {
  const e = Object.getPrototypeOf(t);
  return e ? e.constructor.name : "null prototype";
}
function HM(t) {
  return ~-encodeURI(t).split(/%..|./).length;
}
function GM(t) {
  return HM(JSON.stringify(t));
}
var et;
(function(t) {
  t[t.PENDING = 0] = "PENDING";
  const n = 1;
  t[t.RESOLVED = n] = "RESOLVED";
  const r = 2;
  t[t.REJECTED = r] = "REJECTED";
})(et || (et = {}));
class at {
  constructor(e) {
    at.prototype.__init.call(this), at.prototype.__init2.call(this), at.prototype.__init3.call(this), at.prototype.__init4.call(this), this._state = et.PENDING, this._handlers = [];
    try {
      e(this._resolve, this._reject);
    } catch (n) {
      this._reject(n);
    }
  }
  /** JSDoc */
  then(e, n) {
    return new at((r, o) => {
      this._handlers.push([
        !1,
        (s) => {
          if (!e)
            r(s);
          else
            try {
              r(e(s));
            } catch (a) {
              o(a);
            }
        },
        (s) => {
          if (!n)
            o(s);
          else
            try {
              r(n(s));
            } catch (a) {
              o(a);
            }
        }
      ]), this._executeHandlers();
    });
  }
  /** JSDoc */
  catch(e) {
    return this.then((n) => n, e);
  }
  /** JSDoc */
  finally(e) {
    return new at((n, r) => {
      let o, s;
      return this.then(
        (a) => {
          s = !1, o = a, e && e();
        },
        (a) => {
          s = !0, o = a, e && e();
        }
      ).then(() => {
        if (s) {
          r(o);
          return;
        }
        n(o);
      });
    });
  }
  /** JSDoc */
  __init() {
    this._resolve = (e) => {
      this._setResult(et.RESOLVED, e);
    };
  }
  /** JSDoc */
  __init2() {
    this._reject = (e) => {
      this._setResult(et.REJECTED, e);
    };
  }
  /** JSDoc */
  __init3() {
    this._setResult = (e, n) => {
      if (this._state === et.PENDING) {
        if (fn(n)) {
          n.then(this._resolve, this._reject);
          return;
        }
        this._state = e, this._value = n, this._executeHandlers();
      }
    };
  }
  /** JSDoc */
  __init4() {
    this._executeHandlers = () => {
      if (this._state === et.PENDING)
        return;
      const e = this._handlers.slice();
      this._handlers = [], e.forEach((n) => {
        n[0] || (this._state === et.RESOLVED && n[1](this._value), this._state === et.REJECTED && n[2](this._value), n[0] = !0);
      });
    };
  }
}
function Qe(t) {
  if (!t)
    return {};
  const e = t.match(/^(([^:/?#]+):)?(\/\/([^/?#]*))?([^?#]*)(\?([^#]*))?(#(.*))?$/);
  if (!e)
    return {};
  const n = e[6] || "", r = e[8] || "";
  return {
    host: e[4],
    path: e[5],
    protocol: e[2],
    search: n,
    hash: r,
    relative: e[5] + n + r
    // everything minus origin
  };
}
const YM = ["fatal", "error", "warning", "log", "info", "debug"];
function QM(t) {
  return t === "warn" ? "warning" : YM.includes(t) ? t : "log";
}
const fo = 1e3;
function kn() {
  return Date.now() / fo;
}
function JM() {
  const { performance: t } = v;
  if (!t || !t.now)
    return kn;
  const e = Date.now() - t.now(), n = t.timeOrigin == null ? e : t.timeOrigin;
  return () => (n + t.now()) / fo;
}
const xo = JM();
(() => {
  const { performance: t } = v;
  if (!t || !t.now)
    return;
  const e = 3600 * 1e3, n = t.now(), r = Date.now(), o = t.timeOrigin ? Math.abs(t.timeOrigin + n - r) : e, s = o < e, a = t.timing && t.timing.navigationStart, l = typeof a == "number" ? Math.abs(a + n - r) : e, u = l < e;
  return s || u ? o <= l ? t.timeOrigin : a : r;
})();
const B = typeof __SENTRY_DEBUG__ > "u" || __SENTRY_DEBUG__, Co = "production";
function BM() {
  return Do("globalEventProcessors", () => []);
}
function gn(t, e, n, r = 0) {
  return new at((o, s) => {
    const a = t[r];
    if (e === null || typeof a != "function")
      o(e);
    else {
      const c = a({ ...e }, n);
      B && a.id && c === null && h.log(`Event processor "${a.id}" dropped event`), fn(c) ? c.then((l) => gn(t, l, n, r + 1).then(o)).then(null, s) : gn(t, c, n, r + 1).then(o).then(null, s);
    }
  });
}
function VM(t) {
  const e = xo(), n = {
    sid: q(),
    init: !0,
    timestamp: e,
    started: e,
    duration: 0,
    status: "ok",
    errors: 0,
    ignoreDuration: !1,
    toJSON: () => FM(n)
  };
  return t && Le(n, t), n;
}
function Le(t, e = {}) {
  if (e.user && (!t.ipAddress && e.user.ip_address && (t.ipAddress = e.user.ip_address), !t.did && !e.did && (t.did = e.user.id || e.user.email || e.user.username)), t.timestamp = e.timestamp || xo(), e.abnormal_mechanism && (t.abnormal_mechanism = e.abnormal_mechanism), e.ignoreDuration && (t.ignoreDuration = e.ignoreDuration), e.sid && (t.sid = e.sid.length === 32 ? e.sid : q()), e.init !== void 0 && (t.init = e.init), !t.did && e.did && (t.did = `${e.did}`), typeof e.started == "number" && (t.started = e.started), t.ignoreDuration)
    t.duration = void 0;
  else if (typeof e.duration == "number")
    t.duration = e.duration;
  else {
    const n = t.timestamp - t.started;
    t.duration = n >= 0 ? n : 0;
  }
  e.release && (t.release = e.release), e.environment && (t.environment = e.environment), !t.ipAddress && e.ipAddress && (t.ipAddress = e.ipAddress), !t.userAgent && e.userAgent && (t.userAgent = e.userAgent), typeof e.errors == "number" && (t.errors = e.errors), e.status && (t.status = e.status);
}
function XM(t, e) {
  let n = {};
  e ? n = { status: e } : t.status === "ok" && (n = { status: "exited" }), Le(t, n);
}
function FM(t) {
  return Nt({
    sid: `${t.sid}`,
    init: t.init,
    // Make sure that sec is converted to ms for date constructor
    started: new Date(t.started * 1e3).toISOString(),
    timestamp: new Date(t.timestamp * 1e3).toISOString(),
    status: t.status,
    errors: t.errors,
    did: typeof t.did == "number" || typeof t.did == "string" ? `${t.did}` : void 0,
    duration: t.duration,
    abnormal_mechanism: t.abnormal_mechanism,
    attrs: {
      release: t.release,
      environment: t.environment,
      ip_address: t.ipAddress,
      user_agent: t.userAgent
    }
  });
}
const qM = 1;
function $M(t) {
  const { spanId: e, traceId: n } = t.spanContext(), { data: r, op: o, parent_span_id: s, status: a, tags: c, origin: l } = ge(t);
  return Nt({
    data: r,
    op: o,
    parent_span_id: s,
    span_id: e,
    status: a,
    tags: c,
    trace_id: n,
    origin: l
  });
}
function ge(t) {
  return KM(t) ? t.getSpanJSON() : typeof t.toJSON == "function" ? t.toJSON() : {};
}
function KM(t) {
  return typeof t.getSpanJSON == "function";
}
function tI(t) {
  const { traceFlags: e } = t.spanContext();
  return !!(e & qM);
}
function eI(t) {
  if (t)
    return nI(t) ? { captureContext: t } : oI(t) ? {
      captureContext: t
    } : t;
}
function nI(t) {
  return t instanceof yt || typeof t == "function";
}
const rI = [
  "user",
  "level",
  "extra",
  "contexts",
  "tags",
  "fingerprint",
  "requestSession",
  "propagationContext"
];
function oI(t) {
  return Object.keys(t).some((e) => rI.includes(e));
}
function To(t, e) {
  return At().captureException(t, eI(e));
}
function zo(t, e) {
  return At().captureEvent(t, e);
}
function Dt(t, e) {
  At().addBreadcrumb(t, e);
}
function ko(...t) {
  const e = At();
  if (t.length === 2) {
    const [n, r] = t;
    return n ? e.withScope(() => (e.getStackTop().scope = n, r(n))) : e.withScope(r);
  }
  return e.withScope(t[0]);
}
function O() {
  return At().getClient();
}
function iI() {
  return At().getScope();
}
function Zo(t) {
  return t.transaction;
}
function sI(t, e, n) {
  const r = e.getOptions(), { publicKey: o } = e.getDsn() || {}, { segment: s } = n && n.getUser() || {}, a = Nt({
    environment: r.environment || Co,
    release: r.release,
    user_segment: s,
    public_key: o,
    trace_id: t
  });
  return e.emit && e.emit("createDsc", a), a;
}
function aI(t) {
  const e = O();
  if (!e)
    return {};
  const n = sI(ge(t).trace_id || "", e, iI()), r = Zo(t);
  if (!r)
    return n;
  const o = r && r._frozenDynamicSamplingContext;
  if (o)
    return o;
  const { sampleRate: s, source: a } = r.metadata;
  s != null && (n.sample_rate = `${s}`);
  const c = ge(r);
  return a && a !== "url" && (n.transaction = c.description), n.sampled = String(tI(r)), e.emit && e.emit("createDsc", n), n;
}
function cI(t, e) {
  const { fingerprint: n, span: r, breadcrumbs: o, sdkProcessingMetadata: s } = e;
  lI(t, e), r && MI(t, r), II(t, n), uI(t, o), dI(t, s);
}
function lI(t, e) {
  const {
    extra: n,
    tags: r,
    user: o,
    contexts: s,
    level: a,
    // eslint-disable-next-line deprecation/deprecation
    transactionName: c
  } = e, l = Nt(n);
  l && Object.keys(l).length && (t.extra = { ...l, ...t.extra });
  const u = Nt(r);
  u && Object.keys(u).length && (t.tags = { ...u, ...t.tags });
  const d = Nt(o);
  d && Object.keys(d).length && (t.user = { ...d, ...t.user });
  const M = Nt(s);
  M && Object.keys(M).length && (t.contexts = { ...M, ...t.contexts }), a && (t.level = a), c && (t.transaction = c);
}
function uI(t, e) {
  const n = [...t.breadcrumbs || [], ...e];
  t.breadcrumbs = n.length ? n : void 0;
}
function dI(t, e) {
  t.sdkProcessingMetadata = {
    ...t.sdkProcessingMetadata,
    ...e
  };
}
function MI(t, e) {
  t.contexts = { trace: $M(e), ...t.contexts };
  const n = Zo(e);
  if (n) {
    t.sdkProcessingMetadata = {
      dynamicSamplingContext: aI(e),
      ...t.sdkProcessingMetadata
    };
    const r = ge(n).description;
    r && (t.tags = { transaction: r, ...t.tags });
  }
}
function II(t, e) {
  t.fingerprint = t.fingerprint ? jM(t.fingerprint) : [], e && (t.fingerprint = t.fingerprint.concat(e)), t.fingerprint && !t.fingerprint.length && delete t.fingerprint;
}
const gI = 100;
class yt {
  /** Flag if notifying is happening. */
  /** Callback for client to receive scope changes. */
  /** Callback list that will be called after {@link applyToEvent}. */
  /** Array of breadcrumbs. */
  /** User */
  /** Tags */
  /** Extra */
  /** Contexts */
  /** Attachments */
  /** Propagation Context for distributed tracing */
  /**
   * A place to stash data which is needed at some point in the SDK's event processing pipeline but which shouldn't get
   * sent to Sentry
   */
  /** Fingerprint */
  /** Severity */
  // eslint-disable-next-line deprecation/deprecation
  /**
   * Transaction Name
   */
  /** Span */
  /** Session */
  /** Request Mode Session Status */
  /** The client on this scope */
  // NOTE: Any field which gets added here should get added not only to the constructor but also to the `clone` method.
  constructor() {
    this._notifyingListeners = !1, this._scopeListeners = [], this._eventProcessors = [], this._breadcrumbs = [], this._attachments = [], this._user = {}, this._tags = {}, this._extra = {}, this._contexts = {}, this._sdkProcessingMetadata = {}, this._propagationContext = Nr();
  }
  /**
   * Inherit values from the parent scope.
   * @deprecated Use `scope.clone()` and `new Scope()` instead.
   */
  static clone(e) {
    return e ? e.clone() : new yt();
  }
  /**
   * Clone this scope instance.
   */
  clone() {
    const e = new yt();
    return e._breadcrumbs = [...this._breadcrumbs], e._tags = { ...this._tags }, e._extra = { ...this._extra }, e._contexts = { ...this._contexts }, e._user = this._user, e._level = this._level, e._span = this._span, e._session = this._session, e._transactionName = this._transactionName, e._fingerprint = this._fingerprint, e._eventProcessors = [...this._eventProcessors], e._requestSession = this._requestSession, e._attachments = [...this._attachments], e._sdkProcessingMetadata = { ...this._sdkProcessingMetadata }, e._propagationContext = { ...this._propagationContext }, e._client = this._client, e;
  }
  /** Update the client on the scope. */
  setClient(e) {
    this._client = e;
  }
  /**
   * Get the client assigned to this scope.
   *
   * It is generally recommended to use the global function `Sentry.getClient()` instead, unless you know what you are doing.
   */
  getClient() {
    return this._client;
  }
  /**
   * Add internal on change listener. Used for sub SDKs that need to store the scope.
   * @hidden
   */
  addScopeListener(e) {
    this._scopeListeners.push(e);
  }
  /**
   * @inheritDoc
   */
  addEventProcessor(e) {
    return this._eventProcessors.push(e), this;
  }
  /**
   * @inheritDoc
   */
  setUser(e) {
    return this._user = e || {
      email: void 0,
      id: void 0,
      ip_address: void 0,
      segment: void 0,
      username: void 0
    }, this._session && Le(this._session, { user: e }), this._notifyScopeListeners(), this;
  }
  /**
   * @inheritDoc
   */
  getUser() {
    return this._user;
  }
  /**
   * @inheritDoc
   */
  getRequestSession() {
    return this._requestSession;
  }
  /**
   * @inheritDoc
   */
  setRequestSession(e) {
    return this._requestSession = e, this;
  }
  /**
   * @inheritDoc
   */
  setTags(e) {
    return this._tags = {
      ...this._tags,
      ...e
    }, this._notifyScopeListeners(), this;
  }
  /**
   * @inheritDoc
   */
  setTag(e, n) {
    return this._tags = { ...this._tags, [e]: n }, this._notifyScopeListeners(), this;
  }
  /**
   * @inheritDoc
   */
  setExtras(e) {
    return this._extra = {
      ...this._extra,
      ...e
    }, this._notifyScopeListeners(), this;
  }
  /**
   * @inheritDoc
   */
  setExtra(e, n) {
    return this._extra = { ...this._extra, [e]: n }, this._notifyScopeListeners(), this;
  }
  /**
   * @inheritDoc
   */
  setFingerprint(e) {
    return this._fingerprint = e, this._notifyScopeListeners(), this;
  }
  /**
   * @inheritDoc
   */
  setLevel(e) {
    return this._level = e, this._notifyScopeListeners(), this;
  }
  /**
   * Sets the transaction name on the scope for future events.
   */
  setTransactionName(e) {
    return this._transactionName = e, this._notifyScopeListeners(), this;
  }
  /**
   * @inheritDoc
   */
  setContext(e, n) {
    return n === null ? delete this._contexts[e] : this._contexts[e] = n, this._notifyScopeListeners(), this;
  }
  /**
   * Sets the Span on the scope.
   * @param span Span
   * @deprecated Instead of setting a span on a scope, use `startSpan()`/`startSpanManual()` instead.
   */
  setSpan(e) {
    return this._span = e, this._notifyScopeListeners(), this;
  }
  /**
   * Returns the `Span` if there is one.
   * @deprecated Use `getActiveSpan()` instead.
   */
  getSpan() {
    return this._span;
  }
  /**
   * Returns the `Transaction` attached to the scope (if there is one).
   * @deprecated You should not rely on the transaction, but just use `startSpan()` APIs instead.
   */
  getTransaction() {
    const e = this._span;
    return e && e.transaction;
  }
  /**
   * @inheritDoc
   */
  setSession(e) {
    return e ? this._session = e : delete this._session, this._notifyScopeListeners(), this;
  }
  /**
   * @inheritDoc
   */
  getSession() {
    return this._session;
  }
  /**
   * @inheritDoc
   */
  update(e) {
    if (!e)
      return this;
    const n = typeof e == "function" ? e(this) : e;
    if (n instanceof yt) {
      const r = n.getScopeData();
      this._tags = { ...this._tags, ...r.tags }, this._extra = { ...this._extra, ...r.extra }, this._contexts = { ...this._contexts, ...r.contexts }, r.user && Object.keys(r.user).length && (this._user = r.user), r.level && (this._level = r.level), r.fingerprint.length && (this._fingerprint = r.fingerprint), n.getRequestSession() && (this._requestSession = n.getRequestSession()), r.propagationContext && (this._propagationContext = r.propagationContext);
    } else if (we(n)) {
      const r = e;
      this._tags = { ...this._tags, ...r.tags }, this._extra = { ...this._extra, ...r.extra }, this._contexts = { ...this._contexts, ...r.contexts }, r.user && (this._user = r.user), r.level && (this._level = r.level), r.fingerprint && (this._fingerprint = r.fingerprint), r.requestSession && (this._requestSession = r.requestSession), r.propagationContext && (this._propagationContext = r.propagationContext);
    }
    return this;
  }
  /**
   * @inheritDoc
   */
  clear() {
    return this._breadcrumbs = [], this._tags = {}, this._extra = {}, this._user = {}, this._contexts = {}, this._level = void 0, this._transactionName = void 0, this._fingerprint = void 0, this._requestSession = void 0, this._span = void 0, this._session = void 0, this._notifyScopeListeners(), this._attachments = [], this._propagationContext = Nr(), this;
  }
  /**
   * @inheritDoc
   */
  addBreadcrumb(e, n) {
    const r = typeof n == "number" ? n : gI;
    if (r <= 0)
      return this;
    const o = {
      timestamp: kn(),
      ...e
    }, s = this._breadcrumbs;
    return s.push(o), this._breadcrumbs = s.length > r ? s.slice(-r) : s, this._notifyScopeListeners(), this;
  }
  /**
   * @inheritDoc
   */
  getLastBreadcrumb() {
    return this._breadcrumbs[this._breadcrumbs.length - 1];
  }
  /**
   * @inheritDoc
   */
  clearBreadcrumbs() {
    return this._breadcrumbs = [], this._notifyScopeListeners(), this;
  }
  /**
   * @inheritDoc
   */
  addAttachment(e) {
    return this._attachments.push(e), this;
  }
  /**
   * @inheritDoc
   * @deprecated Use `getScopeData()` instead.
   */
  getAttachments() {
    return this.getScopeData().attachments;
  }
  /**
   * @inheritDoc
   */
  clearAttachments() {
    return this._attachments = [], this;
  }
  /** @inheritDoc */
  getScopeData() {
    const {
      _breadcrumbs: e,
      _attachments: n,
      _contexts: r,
      _tags: o,
      _extra: s,
      _user: a,
      _level: c,
      _fingerprint: l,
      _eventProcessors: u,
      _propagationContext: d,
      _sdkProcessingMetadata: M,
      _transactionName: I,
      _span: g
    } = this;
    return {
      breadcrumbs: e,
      attachments: n,
      contexts: r,
      tags: o,
      extra: s,
      user: a,
      level: c,
      fingerprint: l || [],
      eventProcessors: u,
      propagationContext: d,
      sdkProcessingMetadata: M,
      transactionName: I,
      span: g
    };
  }
  /**
   * Applies data from the scope to the event and runs all event processors on it.
   *
   * @param event Event
   * @param hint Object containing additional information about the original exception, for use by the event processors.
   * @hidden
   * @deprecated Use `applyScopeDataToEvent()` directly
   */
  applyToEvent(e, n = {}, r = []) {
    cI(e, this.getScopeData());
    const o = [
      ...r,
      // eslint-disable-next-line deprecation/deprecation
      ...BM(),
      ...this._eventProcessors
    ];
    return gn(o, e, n);
  }
  /**
   * Add data which will be accessible during event processing but won't get sent to Sentry
   */
  setSDKProcessingMetadata(e) {
    return this._sdkProcessingMetadata = { ...this._sdkProcessingMetadata, ...e }, this;
  }
  /**
   * @inheritDoc
   */
  setPropagationContext(e) {
    return this._propagationContext = e, this;
  }
  /**
   * @inheritDoc
   */
  getPropagationContext() {
    return this._propagationContext;
  }
  /**
   * Capture an exception for this scope.
   *
   * @param exception The exception to capture.
   * @param hint Optinal additional data to attach to the Sentry event.
   * @returns the id of the captured Sentry event.
   */
  captureException(e, n) {
    const r = n && n.event_id ? n.event_id : q();
    if (!this._client)
      return h.warn("No client configured on scope - will not capture exception!"), r;
    const o = new Error("Sentry syntheticException");
    return this._client.captureException(
      e,
      {
        originalException: e,
        syntheticException: o,
        ...n,
        event_id: r
      },
      this
    ), r;
  }
  /**
   * Capture a message for this scope.
   *
   * @param message The message to capture.
   * @param level An optional severity level to report the message with.
   * @param hint Optional additional data to attach to the Sentry event.
   * @returns the id of the captured message.
   */
  captureMessage(e, n, r) {
    const o = r && r.event_id ? r.event_id : q();
    if (!this._client)
      return h.warn("No client configured on scope - will not capture message!"), o;
    const s = new Error(e);
    return this._client.captureMessage(
      e,
      n,
      {
        originalException: e,
        syntheticException: s,
        ...r,
        event_id: o
      },
      this
    ), o;
  }
  /**
   * Captures a manually created event for this scope and sends it to Sentry.
   *
   * @param exception The event to capture.
   * @param hint Optional additional data to attach to the Sentry event.
   * @returns the id of the captured event.
   */
  captureEvent(e, n) {
    const r = n && n.event_id ? n.event_id : q();
    return this._client ? (this._client.captureEvent(e, { ...n, event_id: r }, this), r) : (h.warn("No client configured on scope - will not capture event!"), r);
  }
  /**
   * This will be called on every set call.
   */
  _notifyScopeListeners() {
    this._notifyingListeners || (this._notifyingListeners = !0, this._scopeListeners.forEach((e) => {
      e(this);
    }), this._notifyingListeners = !1);
  }
}
function Nr() {
  return {
    traceId: q(),
    spanId: q().substring(16)
  };
}
const jI = "7.119.2", Eo = parseFloat(jI), NI = 100;
class Wo {
  /** Is a {@link Layer}[] containing the client and scope */
  /** Contains the last event id of a captured event.  */
  /**
   * Creates a new instance of the hub, will push one {@link Layer} into the
   * internal stack on creation.
   *
   * @param client bound to the hub.
   * @param scope bound to the hub.
   * @param version number, higher number means higher priority.
   *
   * @deprecated Instantiation of Hub objects is deprecated and the constructor will be removed in version 8 of the SDK.
   *
   * If you are currently using the Hub for multi-client use like so:
   *
   * ```
   * // OLD
   * const hub = new Hub();
   * hub.bindClient(client);
   * makeMain(hub)
   * ```
   *
   * instead initialize the client as follows:
   *
   * ```
   * // NEW
   * Sentry.withIsolationScope(() => {
   *    Sentry.setCurrentClient(client);
   *    client.init();
   * });
   * ```
   *
   * If you are using the Hub to capture events like so:
   *
   * ```
   * // OLD
   * const client = new Client();
   * const hub = new Hub(client);
   * hub.captureException()
   * ```
   *
   * instead capture isolated events as follows:
   *
   * ```
   * // NEW
   * const client = new Client();
   * const scope = new Scope();
   * scope.setClient(client);
   * scope.captureException();
   * ```
   */
  constructor(e, n, r, o = Eo) {
    this._version = o;
    let s;
    n ? s = n : (s = new yt(), s.setClient(e));
    let a;
    r ? a = r : (a = new yt(), a.setClient(e)), this._stack = [{ scope: s }], e && this.bindClient(e), this._isolationScope = a;
  }
  /**
   * Checks if this hub's version is older than the given version.
   *
   * @param version A version number to compare to.
   * @return True if the given version is newer; otherwise false.
   *
   * @deprecated This will be removed in v8.
   */
  isOlderThan(e) {
    return this._version < e;
  }
  /**
   * This binds the given client to the current scope.
   * @param client An SDK client (client) instance.
   *
   * @deprecated Use `initAndBind()` directly, or `setCurrentClient()` and/or `client.init()` instead.
   */
  bindClient(e) {
    const n = this.getStackTop();
    n.client = e, n.scope.setClient(e), e && e.setupIntegrations && e.setupIntegrations();
  }
  /**
   * @inheritDoc
   *
   * @deprecated Use `withScope` instead.
   */
  pushScope() {
    const e = this.getScope().clone();
    return this.getStack().push({
      // eslint-disable-next-line deprecation/deprecation
      client: this.getClient(),
      scope: e
    }), e;
  }
  /**
   * @inheritDoc
   *
   * @deprecated Use `withScope` instead.
   */
  popScope() {
    return this.getStack().length <= 1 ? !1 : !!this.getStack().pop();
  }
  /**
   * @inheritDoc
   *
   * @deprecated Use `Sentry.withScope()` instead.
   */
  withScope(e) {
    const n = this.pushScope();
    let r;
    try {
      r = e(n);
    } catch (o) {
      throw this.popScope(), o;
    }
    return fn(r) ? r.then(
      (o) => (this.popScope(), o),
      (o) => {
        throw this.popScope(), o;
      }
    ) : (this.popScope(), r);
  }
  /**
   * @inheritDoc
   *
   * @deprecated Use `Sentry.getClient()` instead.
   */
  getClient() {
    return this.getStackTop().client;
  }
  /**
   * Returns the scope of the top stack.
   *
   * @deprecated Use `Sentry.getCurrentScope()` instead.
   */
  getScope() {
    return this.getStackTop().scope;
  }
  /**
   * @deprecated Use `Sentry.getIsolationScope()` instead.
   */
  getIsolationScope() {
    return this._isolationScope;
  }
  /**
   * Returns the scope stack for domains or the process.
   * @deprecated This will be removed in v8.
   */
  getStack() {
    return this._stack;
  }
  /**
   * Returns the topmost scope layer in the order domain > local > process.
   * @deprecated This will be removed in v8.
   */
  getStackTop() {
    return this._stack[this._stack.length - 1];
  }
  /**
   * @inheritDoc
   *
   * @deprecated Use `Sentry.captureException()` instead.
   */
  captureException(e, n) {
    const r = this._lastEventId = n && n.event_id ? n.event_id : q(), o = new Error("Sentry syntheticException");
    return this.getScope().captureException(e, {
      originalException: e,
      syntheticException: o,
      ...n,
      event_id: r
    }), r;
  }
  /**
   * @inheritDoc
   *
   * @deprecated Use  `Sentry.captureMessage()` instead.
   */
  captureMessage(e, n, r) {
    const o = this._lastEventId = r && r.event_id ? r.event_id : q(), s = new Error(e);
    return this.getScope().captureMessage(e, n, {
      originalException: e,
      syntheticException: s,
      ...r,
      event_id: o
    }), o;
  }
  /**
   * @inheritDoc
   *
   * @deprecated Use `Sentry.captureEvent()` instead.
   */
  captureEvent(e, n) {
    const r = n && n.event_id ? n.event_id : q();
    return e.type || (this._lastEventId = r), this.getScope().captureEvent(e, { ...n, event_id: r }), r;
  }
  /**
   * @inheritDoc
   *
   * @deprecated This will be removed in v8.
   */
  lastEventId() {
    return this._lastEventId;
  }
  /**
   * @inheritDoc
   *
   * @deprecated Use `Sentry.addBreadcrumb()` instead.
   */
  addBreadcrumb(e, n) {
    const { scope: r, client: o } = this.getStackTop();
    if (!o)
      return;
    const { beforeBreadcrumb: s = null, maxBreadcrumbs: a = NI } = o.getOptions && o.getOptions() || {};
    if (a <= 0)
      return;
    const l = { timestamp: kn(), ...e }, u = s ? Tn(() => s(l, n)) : l;
    u !== null && (o.emit && o.emit("beforeAddBreadcrumb", u, n), r.addBreadcrumb(u, a));
  }
  /**
   * @inheritDoc
   * @deprecated Use `Sentry.setUser()` instead.
   */
  setUser(e) {
    this.getScope().setUser(e), this.getIsolationScope().setUser(e);
  }
  /**
   * @inheritDoc
   * @deprecated Use `Sentry.setTags()` instead.
   */
  setTags(e) {
    this.getScope().setTags(e), this.getIsolationScope().setTags(e);
  }
  /**
   * @inheritDoc
   * @deprecated Use `Sentry.setExtras()` instead.
   */
  setExtras(e) {
    this.getScope().setExtras(e), this.getIsolationScope().setExtras(e);
  }
  /**
   * @inheritDoc
   * @deprecated Use `Sentry.setTag()` instead.
   */
  setTag(e, n) {
    this.getScope().setTag(e, n), this.getIsolationScope().setTag(e, n);
  }
  /**
   * @inheritDoc
   * @deprecated Use `Sentry.setExtra()` instead.
   */
  setExtra(e, n) {
    this.getScope().setExtra(e, n), this.getIsolationScope().setExtra(e, n);
  }
  /**
   * @inheritDoc
   * @deprecated Use `Sentry.setContext()` instead.
   */
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  setContext(e, n) {
    this.getScope().setContext(e, n), this.getIsolationScope().setContext(e, n);
  }
  /**
   * @inheritDoc
   *
   * @deprecated Use `getScope()` directly.
   */
  configureScope(e) {
    const { scope: n, client: r } = this.getStackTop();
    r && e(n);
  }
  /**
   * @inheritDoc
   */
  // eslint-disable-next-line deprecation/deprecation
  run(e) {
    const n = mr(this);
    try {
      e(this);
    } finally {
      mr(n);
    }
  }
  /**
   * @inheritDoc
   * @deprecated Use `Sentry.getClient().getIntegrationByName()` instead.
   */
  getIntegration(e) {
    const n = this.getClient();
    if (!n)
      return null;
    try {
      return n.getIntegration(e);
    } catch {
      return B && h.warn(`Cannot retrieve integration ${e.id} from the current Hub`), null;
    }
  }
  /**
   * Starts a new `Transaction` and returns it. This is the entry point to manual tracing instrumentation.
   *
   * A tree structure can be built by adding child spans to the transaction, and child spans to other spans. To start a
   * new child span within the transaction or any span, call the respective `.startChild()` method.
   *
   * Every child span must be finished before the transaction is finished, otherwise the unfinished spans are discarded.
   *
   * The transaction must be finished with a call to its `.end()` method, at which point the transaction with all its
   * finished child spans will be sent to Sentry.
   *
   * @param context Properties of the new `Transaction`.
   * @param customSamplingContext Information given to the transaction sampling function (along with context-dependent
   * default values). See {@link Options.tracesSampler}.
   *
   * @returns The transaction which was just started
   *
   * @deprecated Use `startSpan()`, `startSpanManual()` or `startInactiveSpan()` instead.
   */
  startTransaction(e, n) {
    const r = this._callExtensionMethod("startTransaction", e, n);
    return B && !r && (this.getClient() ? h.warn(`Tracing extension 'startTransaction' has not been added. Call 'addTracingExtensions' before calling 'init':
Sentry.addTracingExtensions();
Sentry.init({...});
`) : h.warn(
      "Tracing extension 'startTransaction' is missing. You should 'init' the SDK before calling 'startTransaction'"
    )), r;
  }
  /**
   * @inheritDoc
   * @deprecated Use `spanToTraceHeader()` instead.
   */
  traceHeaders() {
    return this._callExtensionMethod("traceHeaders");
  }
  /**
   * @inheritDoc
   *
   * @deprecated Use top level `captureSession` instead.
   */
  captureSession(e = !1) {
    if (e)
      return this.endSession();
    this._sendSessionUpdate();
  }
  /**
   * @inheritDoc
   * @deprecated Use top level `endSession` instead.
   */
  endSession() {
    const n = this.getStackTop().scope, r = n.getSession();
    r && XM(r), this._sendSessionUpdate(), n.setSession();
  }
  /**
   * @inheritDoc
   * @deprecated Use top level `startSession` instead.
   */
  startSession(e) {
    const { scope: n, client: r } = this.getStackTop(), { release: o, environment: s = Co } = r && r.getOptions() || {}, { userAgent: a } = v.navigator || {}, c = VM({
      release: o,
      environment: s,
      user: n.getUser(),
      ...a && { userAgent: a },
      ...e
    }), l = n.getSession && n.getSession();
    return l && l.status === "ok" && Le(l, { status: "exited" }), this.endSession(), n.setSession(c), c;
  }
  /**
   * Returns if default PII should be sent to Sentry and propagated in ourgoing requests
   * when Tracing is used.
   *
   * @deprecated Use top-level `getClient().getOptions().sendDefaultPii` instead. This function
   * only unnecessarily increased API surface but only wrapped accessing the option.
   */
  shouldSendDefaultPii() {
    const e = this.getClient(), n = e && e.getOptions();
    return !!(n && n.sendDefaultPii);
  }
  /**
   * Sends the current Session on the scope
   */
  _sendSessionUpdate() {
    const { scope: e, client: n } = this.getStackTop(), r = e.getSession();
    r && n && n.captureSession && n.captureSession(r);
  }
  /**
   * Calls global extension method and binding current instance to the function call
   */
  // @ts-expect-error Function lacks ending return statement and return type does not include 'undefined'. ts(2366)
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  _callExtensionMethod(e, ...n) {
    const o = Ae().__SENTRY__;
    if (o && o.extensions && typeof o.extensions[e] == "function")
      return o.extensions[e].apply(this, n);
    B && h.warn(`Extension method ${e} couldn't be found, doing nothing.`);
  }
}
function Ae() {
  return v.__SENTRY__ = v.__SENTRY__ || {
    extensions: {},
    hub: void 0
  }, v;
}
function mr(t) {
  const e = Ae(), n = jn(e);
  return Uo(e, t), n;
}
function At() {
  const t = Ae();
  if (t.__SENTRY__ && t.__SENTRY__.acs) {
    const e = t.__SENTRY__.acs.getCurrentHub();
    if (e)
      return e;
  }
  return mI(t);
}
function mI(t = Ae()) {
  return (!bI(t) || // eslint-disable-next-line deprecation/deprecation
  jn(t).isOlderThan(Eo)) && Uo(t, new Wo()), jn(t);
}
function bI(t) {
  return !!(t && t.__SENTRY__ && t.__SENTRY__.hub);
}
function jn(t) {
  return Do("hub", () => new Wo(), t);
}
function Uo(t, e) {
  if (!t)
    return !1;
  const n = t.__SENTRY__ = t.__SENTRY__ || {};
  return n.hub = e, !0;
}
function yI(t) {
  const e = t.protocol ? `${t.protocol}:` : "", n = t.port ? `:${t.port}` : "";
  return `${e}//${t.host}${n}${t.path ? `/${t.path}` : ""}/api/`;
}
function pI(t, e) {
  const n = uM(t);
  if (!n)
    return "";
  const r = `${yI(n)}embed/error-page/`;
  let o = `dsn=${aM(n)}`;
  for (const s in e)
    if (s !== "dsn" && s !== "onClose")
      if (s === "user") {
        const a = e.user;
        if (!a)
          continue;
        a.name && (o += `&name=${encodeURIComponent(a.name)}`), a.email && (o += `&email=${encodeURIComponent(a.email)}`);
      } else
        o += `&${encodeURIComponent(s)}=${encodeURIComponent(e[s])}`;
  return `${r}?${o}`;
}
function It(t, e) {
  return Object.assign(
    function(...r) {
      return e(...r);
    },
    { id: t }
  );
}
const DI = [
  /^Script error\.?$/,
  /^Javascript error: Script error\.? on line 0$/,
  /^ResizeObserver loop completed with undelivered notifications.$/,
  /^Cannot redefine property: googletag$/
], hI = [
  /^.*\/healthcheck$/,
  /^.*\/healthy$/,
  /^.*\/live$/,
  /^.*\/ready$/,
  /^.*\/heartbeat$/,
  /^.*\/health$/,
  /^.*\/healthz$/
], Po = "InboundFilters", wI = (t = {}) => ({
  name: Po,
  // TODO v8: Remove this
  setupOnce() {
  },
  // eslint-disable-line @typescript-eslint/no-empty-function
  processEvent(e, n, r) {
    const o = r.getOptions(), s = SI(t, o);
    return vI(e, s) ? null : e;
  }
}), _o = wI;
It(
  Po,
  _o
);
function SI(t = {}, e = {}) {
  return {
    allowUrls: [...t.allowUrls || [], ...e.allowUrls || []],
    denyUrls: [...t.denyUrls || [], ...e.denyUrls || []],
    ignoreErrors: [
      ...t.ignoreErrors || [],
      ...e.ignoreErrors || [],
      ...t.disableErrorDefaults ? [] : DI
    ],
    ignoreTransactions: [
      ...t.ignoreTransactions || [],
      ...e.ignoreTransactions || [],
      ...t.disableTransactionDefaults ? [] : hI
    ],
    ignoreInternal: t.ignoreInternal !== void 0 ? t.ignoreInternal : !0
  };
}
function vI(t, e) {
  return e.ignoreInternal && TI(t) ? (B && h.warn(`Event dropped due to being internal Sentry Error.
Event: ${ut(t)}`), !0) : LI(t, e.ignoreErrors) ? (B && h.warn(
    `Event dropped due to being matched by \`ignoreErrors\` option.
Event: ${ut(t)}`
  ), !0) : AI(t, e.ignoreTransactions) ? (B && h.warn(
    `Event dropped due to being matched by \`ignoreTransactions\` option.
Event: ${ut(t)}`
  ), !0) : fI(t, e.denyUrls) ? (B && h.warn(
    `Event dropped due to being matched by \`denyUrls\` option.
Event: ${ut(
      t
    )}.
Url: ${je(t)}`
  ), !0) : xI(t, e.allowUrls) ? !1 : (B && h.warn(
    `Event dropped due to not being matched by \`allowUrls\` option.
Event: ${ut(
      t
    )}.
Url: ${je(t)}`
  ), !0);
}
function LI(t, e) {
  return t.type || !e || !e.length ? !1 : CI(t).some((n) => ve(n, e));
}
function AI(t, e) {
  if (t.type !== "transaction" || !e || !e.length)
    return !1;
  const n = t.transaction;
  return n ? ve(n, e) : !1;
}
function fI(t, e) {
  if (!e || !e.length)
    return !1;
  const n = je(t);
  return n ? ve(n, e) : !1;
}
function xI(t, e) {
  if (!e || !e.length)
    return !0;
  const n = je(t);
  return n ? ve(n, e) : !0;
}
function CI(t) {
  const e = [];
  t.message && e.push(t.message);
  let n;
  try {
    n = t.exception.values[t.exception.values.length - 1];
  } catch {
  }
  return n && n.value && (e.push(n.value), n.type && e.push(`${n.type}: ${n.value}`)), B && e.length === 0 && h.error(`Could not extract message for event ${ut(t)}`), e;
}
function TI(t) {
  try {
    return t.exception.values[0].type === "SentryError";
  } catch {
  }
  return !1;
}
function zI(t = []) {
  for (let e = t.length - 1; e >= 0; e--) {
    const n = t[e];
    if (n && n.filename !== "<anonymous>" && n.filename !== "[native code]")
      return n.filename || null;
  }
  return null;
}
function je(t) {
  try {
    let e;
    try {
      e = t.exception.values[0].stacktrace.frames;
    } catch {
    }
    return e ? zI(e) : null;
  } catch {
    return B && h.error(`Cannot extract url for event ${ut(t)}`), null;
  }
}
let br;
const Oo = "FunctionToString", yr = /* @__PURE__ */ new WeakMap(), kI = () => ({
  name: Oo,
  setupOnce() {
    br = Function.prototype.toString;
    try {
      Function.prototype.toString = function(...t) {
        const e = zn(this), n = yr.has(O()) && e !== void 0 ? e : this;
        return br.apply(n, t);
      };
    } catch {
    }
  },
  setup(t) {
    yr.set(t, !0);
  }
}), Ro = kI;
It(
  Oo,
  Ro
);
const z = v;
let Nn = 0;
function Ho() {
  return Nn > 0;
}
function ZI() {
  Nn++, setTimeout(() => {
    Nn--;
  });
}
function Wt(t, e = {}, n) {
  if (typeof t != "function")
    return t;
  try {
    const o = t.__sentry_wrapped__;
    if (o)
      return typeof o == "function" ? o : t;
    if (zn(t))
      return t;
  } catch {
    return t;
  }
  const r = function() {
    const o = Array.prototype.slice.call(arguments);
    try {
      n && typeof n == "function" && n.apply(this, arguments);
      const s = o.map((a) => Wt(a, e));
      return t.apply(this, s);
    } catch (s) {
      throw ZI(), ko((a) => {
        a.addEventProcessor((c) => (e.mechanism && (an(c, void 0, void 0), cn(c, e.mechanism)), c.extra = {
          ...c.extra,
          arguments: o
        }, c)), To(s);
      }), s;
    }
  };
  try {
    for (const o in t)
      Object.prototype.hasOwnProperty.call(t, o) && (r[o] = t[o]);
  } catch {
  }
  So(r, t), Ie(t, "__sentry_wrapped__", r);
  try {
    Object.getOwnPropertyDescriptor(r, "name").configurable && Object.defineProperty(r, "name", {
      get() {
        return t.name;
      }
    });
  } catch {
  }
  return r;
}
const kt = typeof __SENTRY_DEBUG__ > "u" || __SENTRY_DEBUG__;
function Go(t, e) {
  const n = Zn(t, e), r = {
    type: e && e.name,
    value: PI(e)
  };
  return n.length && (r.stacktrace = { frames: n }), r.type === void 0 && r.value === "" && (r.value = "Unrecoverable error caught"), r;
}
function EI(t, e, n, r) {
  const o = O(), s = o && o.getOptions().normalizeDepth, a = {
    exception: {
      values: [
        {
          type: Se(e) ? e.constructor.name : r ? "UnhandledRejection" : "Error",
          value: _I(e, { isUnhandledRejection: r })
        }
      ]
    },
    extra: {
      __serialized__: Ao(e, s)
    }
  };
  if (n) {
    const c = Zn(t, n);
    c.length && (a.exception.values[0].stacktrace = { frames: c });
  }
  return a;
}
function Je(t, e) {
  return {
    exception: {
      values: [Go(t, e)]
    }
  };
}
function Zn(t, e) {
  const n = e.stacktrace || e.stack || "", r = UI(e);
  try {
    return t(n, r);
  } catch {
  }
  return [];
}
const WI = /Minified React error #\d+;/i;
function UI(t) {
  if (t) {
    if (typeof t.framesToPop == "number")
      return t.framesToPop;
    if (WI.test(t.message))
      return 1;
  }
  return 0;
}
function PI(t) {
  const e = t && t.message;
  return e ? e.error && typeof e.error.message == "string" ? e.error.message : e : "No error message";
}
function Yo(t, e, n, r, o) {
  let s;
  if (An(e) && e.error)
    return Je(t, e.error);
  if (ir(e) || Q1(e)) {
    const a = e;
    if ("stack" in e)
      s = Je(t, e);
    else {
      const c = a.name || (ir(a) ? "DOMError" : "DOMException"), l = a.message ? `${c}: ${a.message}` : c;
      s = pr(t, l, n, r), an(s, l);
    }
    return "code" in a && (s.tags = { ...s.tags, "DOMException.code": `${a.code}` }), s;
  }
  return Ln(e) ? Je(t, e) : we(e) || Se(e) ? (s = EI(t, e, n, o), cn(s, {
    synthetic: !0
  }), s) : (s = pr(t, e, n, r), an(s, `${e}`, void 0), cn(s, {
    synthetic: !0
  }), s);
}
function pr(t, e, n, r) {
  const o = {};
  if (r && n) {
    const s = Zn(t, n);
    s.length && (o.exception = {
      values: [{ value: e, stacktrace: { frames: s } }]
    });
  }
  if (bo(e)) {
    const { __sentry_template_string__: s, __sentry_template_values__: a } = e;
    return o.logentry = {
      message: s,
      params: a
    }, o;
  }
  return o.message = e, o;
}
function _I(t, { isUnhandledRejection: e }) {
  const n = dM(t), r = e ? "promise rejection" : "exception";
  return An(t) ? `Event \`ErrorEvent\` captured as ${r} with message \`${t.message}\`` : Se(t) ? `Event \`${OI(t)}\` (type=${t.type}) captured as ${r}` : `Object captured as ${r} with keys: ${n}`;
}
function OI(t) {
  try {
    const e = Object.getPrototypeOf(t);
    return e ? e.constructor.name : void 0;
  } catch {
  }
}
const oe = 1024, Qo = "Breadcrumbs", RI = (t = {}) => {
  const e = {
    console: !0,
    dom: !0,
    fetch: !0,
    history: !0,
    sentry: !0,
    xhr: !0,
    ...t
  };
  return {
    name: Qo,
    // TODO v8: Remove this
    setupOnce() {
    },
    // eslint-disable-line @typescript-eslint/no-empty-function
    setup(n) {
      e.console && IM(YI(n)), e.dom && mM(GI(n, e.dom)), e.xhr && EM(QI(n)), e.fetch && SM(JI(n)), e.history && zM(BI(n)), e.sentry && n.on && n.on("beforeSendEvent", HI(n));
    }
  };
}, Jo = RI;
It(Qo, Jo);
function HI(t) {
  return function(n) {
    O() === t && Dt(
      {
        category: `sentry.${n.type === "transaction" ? "transaction" : "event"}`,
        event_id: n.event_id,
        level: n.level,
        message: ut(n)
      },
      {
        event: n
      }
    );
  };
}
function GI(t, e) {
  return function(r) {
    if (O() !== t)
      return;
    let o, s, a = typeof e == "object" ? e.serializeAttribute : void 0, c = typeof e == "object" && typeof e.maxStringLength == "number" ? e.maxStringLength : void 0;
    c && c > oe && (kt && h.warn(
      `\`dom.maxStringLength\` cannot exceed ${oe}, but a value of ${c} was configured. Sentry will use ${oe} instead.`
    ), c = oe), typeof a == "string" && (a = [a]);
    try {
      const u = r.event, d = VI(u) ? u.target : u;
      o = ho(d, { keyAttrs: a, maxStringLength: c }), s = nM(d);
    } catch {
      o = "<unknown>";
    }
    if (o.length === 0)
      return;
    const l = {
      category: `ui.${r.name}`,
      message: o
    };
    s && (l.data = { "ui.component_name": s }), Dt(l, {
      event: r.event,
      name: r.name,
      global: r.global
    });
  };
}
function YI(t) {
  return function(n) {
    if (O() !== t)
      return;
    const r = {
      category: "console",
      data: {
        arguments: n.args,
        logger: "console"
      },
      level: QM(n.level),
      message: sr(n.args, " ")
    };
    if (n.level === "assert")
      if (n.args[0] === !1)
        r.message = `Assertion failed: ${sr(n.args.slice(1), " ") || "console.assert"}`, r.data.arguments = n.args.slice(1);
      else
        return;
    Dt(r, {
      input: n.args,
      level: n.level
    });
  };
}
function QI(t) {
  return function(n) {
    if (O() !== t)
      return;
    const { startTimestamp: r, endTimestamp: o } = n, s = n.xhr[Qt];
    if (!r || !o || !s)
      return;
    const { method: a, url: c, status_code: l, body: u } = s, d = {
      method: a,
      url: c,
      status_code: l
    }, M = {
      xhr: n.xhr,
      input: u,
      startTimestamp: r,
      endTimestamp: o
    };
    Dt(
      {
        category: "xhr",
        data: d,
        type: "http"
      },
      M
    );
  };
}
function JI(t) {
  return function(n) {
    if (O() !== t)
      return;
    const { startTimestamp: r, endTimestamp: o } = n;
    if (o && !(n.fetchData.url.match(/sentry_key/) && n.fetchData.method === "POST"))
      if (n.error) {
        const s = n.fetchData, a = {
          data: n.error,
          input: n.args,
          startTimestamp: r,
          endTimestamp: o
        };
        Dt(
          {
            category: "fetch",
            data: s,
            level: "error",
            type: "http"
          },
          a
        );
      } else {
        const s = n.response, a = {
          ...n.fetchData,
          status_code: s && s.status
        }, c = {
          input: n.args,
          response: s,
          startTimestamp: r,
          endTimestamp: o
        };
        Dt(
          {
            category: "fetch",
            data: a,
            type: "http"
          },
          c
        );
      }
  };
}
function BI(t) {
  return function(n) {
    if (O() !== t)
      return;
    let r = n.from, o = n.to;
    const s = Qe(z.location.href);
    let a = r ? Qe(r) : void 0;
    const c = Qe(o);
    (!a || !a.path) && (a = s), s.protocol === c.protocol && s.host === c.host && (o = c.relative), s.protocol === a.protocol && s.host === a.host && (r = a.relative), Dt({
      category: "navigation",
      data: {
        from: r,
        to: o
      }
    });
  };
}
function VI(t) {
  return !!t && !!t.target;
}
const Bo = "Dedupe", XI = () => {
  let t;
  return {
    name: Bo,
    // TODO v8: Remove this
    setupOnce() {
    },
    // eslint-disable-line @typescript-eslint/no-empty-function
    processEvent(e) {
      if (e.type)
        return e;
      try {
        if (FI(e, t))
          return kt && h.warn("Event dropped due to being a duplicate of previously captured event."), null;
      } catch {
      }
      return t = e;
    }
  };
}, Vo = XI;
It(Bo, Vo);
function FI(t, e) {
  return e ? !!(qI(t, e) || $I(t, e)) : !1;
}
function qI(t, e) {
  const n = t.message, r = e.message;
  return !(!n && !r || n && !r || !n && r || n !== r || !Fo(t, e) || !Xo(t, e));
}
function $I(t, e) {
  const n = Dr(e), r = Dr(t);
  return !(!n || !r || n.type !== r.type || n.value !== r.value || !Fo(t, e) || !Xo(t, e));
}
function Xo(t, e) {
  let n = hr(t), r = hr(e);
  if (!n && !r)
    return !0;
  if (n && !r || !n && r || (n = n, r = r, r.length !== n.length))
    return !1;
  for (let o = 0; o < r.length; o++) {
    const s = r[o], a = n[o];
    if (s.filename !== a.filename || s.lineno !== a.lineno || s.colno !== a.colno || s.function !== a.function)
      return !1;
  }
  return !0;
}
function Fo(t, e) {
  let n = t.fingerprint, r = e.fingerprint;
  if (!n && !r)
    return !0;
  if (n && !r || !n && r)
    return !1;
  n = n, r = r;
  try {
    return n.join("") === r.join("");
  } catch {
    return !1;
  }
}
function Dr(t) {
  return t.exception && t.exception.values && t.exception.values[0];
}
function hr(t) {
  const e = t.exception;
  if (e)
    try {
      return e.values[0].stacktrace.frames;
    } catch {
      return;
    }
}
const qo = "GlobalHandlers", KI = (t = {}) => {
  const e = {
    onerror: !0,
    onunhandledrejection: !0,
    ...t
  };
  return {
    name: qo,
    setupOnce() {
      Error.stackTraceLimit = 50;
    },
    setup(n) {
      e.onerror && (t2(n), wr("onerror")), e.onunhandledrejection && (e2(n), wr("onunhandledrejection"));
    }
  };
}, $o = KI;
It(
  qo,
  $o
);
function t2(t) {
  AM((e) => {
    const { stackParser: n, attachStacktrace: r } = ti();
    if (O() !== t || Ho())
      return;
    const { msg: o, url: s, line: a, column: c, error: l } = e, u = l === void 0 && rt(o) ? o2(o, s, a, c) : Ko(
      Yo(n, l || o, void 0, r, !1),
      s,
      a,
      c
    );
    u.level = "error", zo(u, {
      originalException: l,
      mechanism: {
        handled: !1,
        type: "onerror"
      }
    });
  });
}
function e2(t) {
  xM((e) => {
    const { stackParser: n, attachStacktrace: r } = ti();
    if (O() !== t || Ho())
      return;
    const o = n2(e), s = yo(o) ? r2(o) : Yo(n, o, void 0, r, !0);
    s.level = "error", zo(s, {
      originalException: o,
      mechanism: {
        handled: !1,
        type: "onunhandledrejection"
      }
    });
  });
}
function n2(t) {
  if (yo(t))
    return t;
  const e = t;
  try {
    if ("reason" in e)
      return e.reason;
    if ("detail" in e && "reason" in e.detail)
      return e.detail.reason;
  } catch {
  }
  return t;
}
function r2(t) {
  return {
    exception: {
      values: [
        {
          type: "UnhandledRejection",
          // String() is needed because the Primitive type includes symbols (which can't be automatically stringified)
          value: `Non-Error promise rejection captured with value: ${String(t)}`
        }
      ]
    }
  };
}
function o2(t, e, n, r) {
  const o = /^(?:[Uu]ncaught (?:exception: )?)?(?:((?:Eval|Internal|Range|Reference|Syntax|Type|URI|)Error): )?(.*)$/i;
  let s = An(t) ? t.message : t, a = "Error";
  const c = s.match(o);
  return c && (a = c[1], s = c[2]), Ko({
    exception: {
      values: [
        {
          type: a,
          value: s
        }
      ]
    }
  }, e, n, r);
}
function Ko(t, e, n, r) {
  const o = t.exception = t.exception || {}, s = o.values = o.values || [], a = s[0] = s[0] || {}, c = a.stacktrace = a.stacktrace || {}, l = c.frames = c.frames || [], u = isNaN(parseInt(r, 10)) ? void 0 : r, d = isNaN(parseInt(n, 10)) ? void 0 : n, M = rt(e) && e.length > 0 ? e : eM();
  return l.length === 0 && l.push({
    colno: u,
    filename: M,
    function: "?",
    in_app: !0,
    lineno: d
  }), t;
}
function wr(t) {
  kt && h.log(`Global Handler attached: ${t}`);
}
function ti() {
  const t = O();
  return t && t.getOptions() || {
    stackParser: () => [],
    attachStacktrace: !1
  };
}
const ei = "HttpContext", i2 = () => ({
  name: ei,
  // TODO v8: Remove this
  setupOnce() {
  },
  // eslint-disable-line @typescript-eslint/no-empty-function
  preprocessEvent(t) {
    if (!z.navigator && !z.location && !z.document)
      return;
    const e = t.request && t.request.url || z.location && z.location.href, { referrer: n } = z.document || {}, { userAgent: r } = z.navigator || {}, o = {
      ...t.request && t.request.headers,
      ...n && { Referer: n },
      ...r && { "User-Agent": r }
    }, s = { ...t.request, ...e && { url: e }, headers: o };
    t.request = s;
  }
}), ni = i2;
It(ei, ni);
const s2 = "cause", a2 = 5, ri = "LinkedErrors", c2 = (t = {}) => {
  const e = t.limit || a2, n = t.key || s2;
  return {
    name: ri,
    // TODO v8: Remove this
    setupOnce() {
    },
    // eslint-disable-line @typescript-eslint/no-empty-function
    preprocessEvent(r, o, s) {
      const a = s.getOptions();
      q1(
        // This differs from the LinkedErrors integration in core by using a different exceptionFromError function
        Go,
        a.stackParser,
        a.maxValueLength,
        n,
        e,
        r,
        o
      );
    }
  };
}, oi = c2;
It(ri, oi);
const l2 = [
  "EventTarget",
  "Window",
  "Node",
  "ApplicationCache",
  "AudioTrackList",
  "BroadcastChannel",
  "ChannelMergerNode",
  "CryptoOperation",
  "EventSource",
  "FileReader",
  "HTMLUnknownElement",
  "IDBDatabase",
  "IDBRequest",
  "IDBTransaction",
  "KeyOperation",
  "MediaController",
  "MessagePort",
  "ModalWindow",
  "Notification",
  "SVGElementInstance",
  "Screen",
  "SharedWorker",
  "TextTrack",
  "TextTrackCue",
  "TextTrackList",
  "WebSocket",
  "WebSocketWorker",
  "Worker",
  "XMLHttpRequest",
  "XMLHttpRequestEventTarget",
  "XMLHttpRequestUpload"
], ii = "TryCatch", u2 = (t = {}) => {
  const e = {
    XMLHttpRequest: !0,
    eventTarget: !0,
    requestAnimationFrame: !0,
    setInterval: !0,
    setTimeout: !0,
    ...t
  };
  return {
    name: ii,
    // TODO: This currently only works for the first client this is setup
    // We may want to adjust this to check for client etc.
    setupOnce() {
      e.setTimeout && E(z, "setTimeout", Sr), e.setInterval && E(z, "setInterval", Sr), e.requestAnimationFrame && E(z, "requestAnimationFrame", d2), e.XMLHttpRequest && "XMLHttpRequest" in z && E(XMLHttpRequest.prototype, "send", M2);
      const n = e.eventTarget;
      n && (Array.isArray(n) ? n : l2).forEach(I2);
    }
  };
}, si = u2;
It(
  ii,
  si
);
function Sr(t) {
  return function(...e) {
    const n = e[0];
    return e[0] = Wt(n, {
      mechanism: {
        data: { function: dt(t) },
        handled: !1,
        type: "instrument"
      }
    }), t.apply(this, e);
  };
}
function d2(t) {
  return function(e) {
    return t.apply(this, [
      Wt(e, {
        mechanism: {
          data: {
            function: "requestAnimationFrame",
            handler: dt(t)
          },
          handled: !1,
          type: "instrument"
        }
      })
    ]);
  };
}
function M2(t) {
  return function(...e) {
    const n = this;
    return ["onload", "onerror", "onprogress", "onreadystatechange"].forEach((o) => {
      o in n && typeof n[o] == "function" && E(n, o, function(s) {
        const a = {
          mechanism: {
            data: {
              function: o,
              handler: dt(s)
            },
            handled: !1,
            type: "instrument"
          }
        }, c = zn(s);
        return c && (a.mechanism.data.handler = dt(c)), Wt(s, a);
      });
    }), t.apply(this, e);
  };
}
function I2(t) {
  const e = z, n = e[t] && e[t].prototype;
  !n || !n.hasOwnProperty || !n.hasOwnProperty("addEventListener") || (E(n, "addEventListener", function(r) {
    return function(o, s, a) {
      try {
        typeof s.handleEvent == "function" && (s.handleEvent = Wt(s.handleEvent, {
          mechanism: {
            data: {
              function: "handleEvent",
              handler: dt(s),
              target: t
            },
            handled: !1,
            type: "instrument"
          }
        }));
      } catch {
      }
      return r.apply(this, [
        o,
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        Wt(s, {
          mechanism: {
            data: {
              function: "addEventListener",
              handler: dt(s),
              target: t
            },
            handled: !1,
            type: "instrument"
          }
        }),
        a
      ]);
    };
  }), E(
    n,
    "removeEventListener",
    function(r) {
      return function(o, s, a) {
        const c = s;
        try {
          const l = c && c.__sentry_wrapped__;
          l && r.call(this, o, l, a);
        } catch {
        }
        return r.call(this, o, c, a);
      };
    }
  ));
}
_o(), Ro(), si(), Jo(), $o(), oi(), Vo(), ni();
const vr = (t = {}, e = At()) => {
  if (!z.document) {
    kt && h.error("Global document not defined in showReportDialog call");
    return;
  }
  const { client: n, scope: r } = e.getStackTop(), o = t.dsn || n && n.getDsn();
  if (!o) {
    kt && h.error("DSN not configured for showReportDialog call");
    return;
  }
  r && (t.user = {
    ...r.getUser(),
    ...t.user
  }), t.eventId || (t.eventId = e.lastEventId());
  const s = z.document.createElement("script");
  s.async = !0, s.crossOrigin = "anonymous", s.src = pI(o, t), t.onLoad && (s.onload = t.onLoad);
  const { onClose: a } = t;
  if (a) {
    const l = (u) => {
      if (u.data === "__sentry_reportdialog_closed__")
        try {
          a();
        } finally {
          z.removeEventListener("message", l);
        }
    };
    z.addEventListener("message", l);
  }
  const c = z.document.head || z.document.body;
  c ? c.appendChild(s) : kt && h.error("Not injecting report dialog. No injection point found in HTML");
};
var ai = { exports: {} }, w = {};
/** @license React v16.13.1
 * react-is.production.min.js
 *
 * Copyright (c) Facebook, Inc. and its affiliates.
 *
 * This source code is licensed under the MIT license found in the
 * LICENSE file in the root directory of this source tree.
 */
var k = typeof Symbol == "function" && Symbol.for, En = k ? Symbol.for("react.element") : 60103, Wn = k ? Symbol.for("react.portal") : 60106, fe = k ? Symbol.for("react.fragment") : 60107, xe = k ? Symbol.for("react.strict_mode") : 60108, Ce = k ? Symbol.for("react.profiler") : 60114, Te = k ? Symbol.for("react.provider") : 60109, ze = k ? Symbol.for("react.context") : 60110, Un = k ? Symbol.for("react.async_mode") : 60111, ke = k ? Symbol.for("react.concurrent_mode") : 60111, Ze = k ? Symbol.for("react.forward_ref") : 60112, Ee = k ? Symbol.for("react.suspense") : 60113, g2 = k ? Symbol.for("react.suspense_list") : 60120, We = k ? Symbol.for("react.memo") : 60115, Ue = k ? Symbol.for("react.lazy") : 60116, j2 = k ? Symbol.for("react.block") : 60121, N2 = k ? Symbol.for("react.fundamental") : 60117, m2 = k ? Symbol.for("react.responder") : 60118, b2 = k ? Symbol.for("react.scope") : 60119;
function R(t) {
  if (typeof t == "object" && t !== null) {
    var e = t.$$typeof;
    switch (e) {
      case En:
        switch (t = t.type, t) {
          case Un:
          case ke:
          case fe:
          case Ce:
          case xe:
          case Ee:
            return t;
          default:
            switch (t = t && t.$$typeof, t) {
              case ze:
              case Ze:
              case Ue:
              case We:
              case Te:
                return t;
              default:
                return e;
            }
        }
      case Wn:
        return e;
    }
  }
}
function ci(t) {
  return R(t) === ke;
}
w.AsyncMode = Un;
w.ConcurrentMode = ke;
w.ContextConsumer = ze;
w.ContextProvider = Te;
w.Element = En;
w.ForwardRef = Ze;
w.Fragment = fe;
w.Lazy = Ue;
w.Memo = We;
w.Portal = Wn;
w.Profiler = Ce;
w.StrictMode = xe;
w.Suspense = Ee;
w.isAsyncMode = function(t) {
  return ci(t) || R(t) === Un;
};
w.isConcurrentMode = ci;
w.isContextConsumer = function(t) {
  return R(t) === ze;
};
w.isContextProvider = function(t) {
  return R(t) === Te;
};
w.isElement = function(t) {
  return typeof t == "object" && t !== null && t.$$typeof === En;
};
w.isForwardRef = function(t) {
  return R(t) === Ze;
};
w.isFragment = function(t) {
  return R(t) === fe;
};
w.isLazy = function(t) {
  return R(t) === Ue;
};
w.isMemo = function(t) {
  return R(t) === We;
};
w.isPortal = function(t) {
  return R(t) === Wn;
};
w.isProfiler = function(t) {
  return R(t) === Ce;
};
w.isStrictMode = function(t) {
  return R(t) === xe;
};
w.isSuspense = function(t) {
  return R(t) === Ee;
};
w.isValidElementType = function(t) {
  return typeof t == "string" || typeof t == "function" || t === fe || t === ke || t === Ce || t === xe || t === Ee || t === g2 || typeof t == "object" && t !== null && (t.$$typeof === Ue || t.$$typeof === We || t.$$typeof === Te || t.$$typeof === ze || t.$$typeof === Ze || t.$$typeof === N2 || t.$$typeof === m2 || t.$$typeof === b2 || t.$$typeof === j2);
};
w.typeOf = R;
ai.exports = w;
var y2 = ai.exports, li = y2, p2 = {
  $$typeof: !0,
  render: !0,
  defaultProps: !0,
  displayName: !0,
  propTypes: !0
}, D2 = {
  $$typeof: !0,
  compare: !0,
  defaultProps: !0,
  displayName: !0,
  propTypes: !0,
  type: !0
}, ui = {};
ui[li.ForwardRef] = p2;
ui[li.Memo] = D2;
const h2 = typeof __SENTRY_DEBUG__ > "u" || __SENTRY_DEBUG__;
function w2(t) {
  const e = t.match(/^([^.]+)/);
  return e !== null && parseInt(e[0]) >= 17;
}
const Lr = {
  componentStack: null,
  error: null,
  eventId: null
};
function S2(t, e) {
  const n = /* @__PURE__ */ new WeakMap();
  function r(o, s) {
    if (!n.has(o)) {
      if (o.cause)
        return n.set(o, !0), r(o.cause, s);
      o.cause = s;
    }
  }
  r(t, e);
}
class Pn extends Wr {
  constructor(e) {
    super(e), Pn.prototype.__init.call(this), this.state = Lr, this._openFallbackReportDialog = !0;
    const n = O();
    n && n.on && e.showDialog && (this._openFallbackReportDialog = !1, n.on("afterSendEvent", (r) => {
      !r.type && r.event_id === this._lastEventId && vr({ ...e.dialogOptions, eventId: this._lastEventId });
    }));
  }
  componentDidCatch(e, { componentStack: n }) {
    const { beforeCapture: r, onError: o, showDialog: s, dialogOptions: a } = this.props;
    ko((c) => {
      if (w2(Or) && Ln(e)) {
        const u = new Error(e.message);
        u.name = `React ErrorBoundary ${e.name}`, u.stack = n, S2(e, u);
      }
      r && r(c, e, n);
      const l = To(e, {
        captureContext: {
          contexts: { react: { componentStack: n } }
        },
        // If users provide a fallback component we can assume they are handling the error.
        // Therefore, we set the mechanism depending on the presence of the fallback prop.
        mechanism: { handled: !!this.props.fallback }
      });
      o && o(e, n, l), s && (this._lastEventId = l, this._openFallbackReportDialog && vr({ ...a, eventId: l })), this.setState({ error: e, componentStack: n, eventId: l });
    });
  }
  componentDidMount() {
    const { onMount: e } = this.props;
    e && e();
  }
  componentWillUnmount() {
    const { error: e, componentStack: n, eventId: r } = this.state, { onUnmount: o } = this.props;
    o && o(e, n, r);
  }
  __init() {
    this.resetErrorBoundary = () => {
      const { onReset: e } = this.props, { error: n, componentStack: r, eventId: o } = this.state;
      e && e(n, r, o), this.setState(Lr);
    };
  }
  render() {
    const { fallback: e, children: n } = this.props, r = this.state;
    if (r.error) {
      let o;
      return typeof e == "function" ? o = e({
        error: r.error,
        componentStack: r.componentStack,
        resetError: this.resetErrorBoundary,
        eventId: r.eventId
      }) : o = e, Zt(o) ? o : (e && h2 && h.warn("fallback did not produce a valid ReactElement"), null);
    }
    return typeof n == "function" ? n() : n;
  }
}
class Pe {
  constructor() {
    this.listeners = /* @__PURE__ */ new Set(), this.subscribe = this.subscribe.bind(this);
  }
  subscribe(e) {
    const n = {
      listener: e
    };
    return this.listeners.add(n), this.onSubscribe(), () => {
      this.listeners.delete(n), this.onUnsubscribe();
    };
  }
  hasListeners() {
    return this.listeners.size > 0;
  }
  onSubscribe() {
  }
  onUnsubscribe() {
  }
}
const _n = typeof window > "u" || "Deno" in window;
function Q() {
}
function v2(t, e) {
  return typeof t == "function" ? t(e) : t;
}
function L2(t) {
  return typeof t == "number" && t >= 0 && t !== 1 / 0;
}
function A2(t, e) {
  return Math.max(t + (e || 0) - Date.now(), 0);
}
function ie(t, e, n) {
  return _e(t) ? typeof e == "function" ? {
    ...n,
    queryKey: t,
    queryFn: e
  } : {
    ...e,
    queryKey: t
  } : t;
}
function ct(t, e, n) {
  return _e(t) ? [{
    ...e,
    queryKey: t
  }, n] : [t || {}, e];
}
function Ar(t, e) {
  const {
    type: n = "all",
    exact: r,
    fetchStatus: o,
    predicate: s,
    queryKey: a,
    stale: c
  } = t;
  if (_e(a)) {
    if (r) {
      if (e.queryHash !== On(a, e.options))
        return !1;
    } else if (!Ne(e.queryKey, a))
      return !1;
  }
  if (n !== "all") {
    const l = e.isActive();
    if (n === "active" && !l || n === "inactive" && l)
      return !1;
  }
  return !(typeof c == "boolean" && e.isStale() !== c || typeof o < "u" && o !== e.state.fetchStatus || s && !s(e));
}
function fr(t, e) {
  const {
    exact: n,
    fetching: r,
    predicate: o,
    mutationKey: s
  } = t;
  if (_e(s)) {
    if (!e.options.mutationKey)
      return !1;
    if (n) {
      if (mt(e.options.mutationKey) !== mt(s))
        return !1;
    } else if (!Ne(e.options.mutationKey, s))
      return !1;
  }
  return !(typeof r == "boolean" && e.state.status === "loading" !== r || o && !o(e));
}
function On(t, e) {
  return ((e == null ? void 0 : e.queryKeyHashFn) || mt)(t);
}
function mt(t) {
  return JSON.stringify(t, (e, n) => mn(n) ? Object.keys(n).sort().reduce((r, o) => (r[o] = n[o], r), {}) : n);
}
function Ne(t, e) {
  return di(t, e);
}
function di(t, e) {
  return t === e ? !0 : typeof t != typeof e ? !1 : t && e && typeof t == "object" && typeof e == "object" ? !Object.keys(e).some((n) => !di(t[n], e[n])) : !1;
}
function Mi(t, e) {
  if (t === e)
    return t;
  const n = xr(t) && xr(e);
  if (n || mn(t) && mn(e)) {
    const r = n ? t.length : Object.keys(t).length, o = n ? e : Object.keys(e), s = o.length, a = n ? [] : {};
    let c = 0;
    for (let l = 0; l < s; l++) {
      const u = n ? l : o[l];
      a[u] = Mi(t[u], e[u]), a[u] === t[u] && c++;
    }
    return r === s && c === r ? t : a;
  }
  return e;
}
function xr(t) {
  return Array.isArray(t) && t.length === Object.keys(t).length;
}
function mn(t) {
  if (!Cr(t))
    return !1;
  const e = t.constructor;
  if (typeof e > "u")
    return !0;
  const n = e.prototype;
  return !(!Cr(n) || !n.hasOwnProperty("isPrototypeOf"));
}
function Cr(t) {
  return Object.prototype.toString.call(t) === "[object Object]";
}
function _e(t) {
  return Array.isArray(t);
}
function Ii(t) {
  return new Promise((e) => {
    setTimeout(e, t);
  });
}
function Tr(t) {
  Ii(0).then(t);
}
function f2() {
  if (typeof AbortController == "function")
    return new AbortController();
}
function x2(t, e, n) {
  return n.isDataEqual != null && n.isDataEqual(t, e) ? t : typeof n.structuralSharing == "function" ? n.structuralSharing(t, e) : n.structuralSharing !== !1 ? Mi(t, e) : e;
}
class C2 extends Pe {
  constructor() {
    super(), this.setup = (e) => {
      if (!_n && window.addEventListener) {
        const n = () => e();
        return window.addEventListener("visibilitychange", n, !1), window.addEventListener("focus", n, !1), () => {
          window.removeEventListener("visibilitychange", n), window.removeEventListener("focus", n);
        };
      }
    };
  }
  onSubscribe() {
    this.cleanup || this.setEventListener(this.setup);
  }
  onUnsubscribe() {
    if (!this.hasListeners()) {
      var e;
      (e = this.cleanup) == null || e.call(this), this.cleanup = void 0;
    }
  }
  setEventListener(e) {
    var n;
    this.setup = e, (n = this.cleanup) == null || n.call(this), this.cleanup = e((r) => {
      typeof r == "boolean" ? this.setFocused(r) : this.onFocus();
    });
  }
  setFocused(e) {
    this.focused !== e && (this.focused = e, this.onFocus());
  }
  onFocus() {
    this.listeners.forEach(({
      listener: e
    }) => {
      e();
    });
  }
  isFocused() {
    return typeof this.focused == "boolean" ? this.focused : typeof document > "u" ? !0 : [void 0, "visible", "prerender"].includes(document.visibilityState);
  }
}
const bn = new C2(), zr = ["online", "offline"];
class T2 extends Pe {
  constructor() {
    super(), this.setup = (e) => {
      if (!_n && window.addEventListener) {
        const n = () => e();
        return zr.forEach((r) => {
          window.addEventListener(r, n, !1);
        }), () => {
          zr.forEach((r) => {
            window.removeEventListener(r, n);
          });
        };
      }
    };
  }
  onSubscribe() {
    this.cleanup || this.setEventListener(this.setup);
  }
  onUnsubscribe() {
    if (!this.hasListeners()) {
      var e;
      (e = this.cleanup) == null || e.call(this), this.cleanup = void 0;
    }
  }
  setEventListener(e) {
    var n;
    this.setup = e, (n = this.cleanup) == null || n.call(this), this.cleanup = e((r) => {
      typeof r == "boolean" ? this.setOnline(r) : this.onOnline();
    });
  }
  setOnline(e) {
    this.online !== e && (this.online = e, this.onOnline());
  }
  onOnline() {
    this.listeners.forEach(({
      listener: e
    }) => {
      e();
    });
  }
  isOnline() {
    return typeof this.online == "boolean" ? this.online : typeof navigator > "u" || typeof navigator.onLine > "u" ? !0 : navigator.onLine;
  }
}
const me = new T2();
function z2(t) {
  return Math.min(1e3 * 2 ** t, 3e4);
}
function Rn(t) {
  return (t ?? "online") === "online" ? me.isOnline() : !0;
}
class gi {
  constructor(e) {
    this.revert = e == null ? void 0 : e.revert, this.silent = e == null ? void 0 : e.silent;
  }
}
function Be(t) {
  return t instanceof gi;
}
function ji(t) {
  let e = !1, n = 0, r = !1, o, s, a;
  const c = new Promise((j, p) => {
    s = j, a = p;
  }), l = (j) => {
    r || (g(new gi(j)), t.abort == null || t.abort());
  }, u = () => {
    e = !0;
  }, d = () => {
    e = !1;
  }, M = () => !bn.isFocused() || t.networkMode !== "always" && !me.isOnline(), I = (j) => {
    r || (r = !0, t.onSuccess == null || t.onSuccess(j), o == null || o(), s(j));
  }, g = (j) => {
    r || (r = !0, t.onError == null || t.onError(j), o == null || o(), a(j));
  }, N = () => new Promise((j) => {
    o = (p) => {
      const A = r || !M();
      return A && j(p), A;
    }, t.onPause == null || t.onPause();
  }).then(() => {
    o = void 0, r || t.onContinue == null || t.onContinue();
  }), m = () => {
    if (r)
      return;
    let j;
    try {
      j = t.fn();
    } catch (p) {
      j = Promise.reject(p);
    }
    Promise.resolve(j).then(I).catch((p) => {
      var A, Z;
      if (r)
        return;
      const C = (A = t.retry) != null ? A : 3, H = (Z = t.retryDelay) != null ? Z : z2, S = typeof H == "function" ? H(n, p) : H, D = C === !0 || typeof C == "number" && n < C || typeof C == "function" && C(n, p);
      if (e || !D) {
        g(p);
        return;
      }
      n++, t.onFail == null || t.onFail(n, p), Ii(S).then(() => {
        if (M())
          return N();
      }).then(() => {
        e ? g(p) : m();
      });
    });
  };
  return Rn(t.networkMode) ? m() : N().then(m), {
    promise: c,
    cancel: l,
    continue: () => (o == null ? void 0 : o()) ? c : Promise.resolve(),
    cancelRetry: u,
    continueRetry: d
  };
}
const Hn = console;
function k2() {
  let t = [], e = 0, n = (d) => {
    d();
  }, r = (d) => {
    d();
  };
  const o = (d) => {
    let M;
    e++;
    try {
      M = d();
    } finally {
      e--, e || c();
    }
    return M;
  }, s = (d) => {
    e ? t.push(d) : Tr(() => {
      n(d);
    });
  }, a = (d) => (...M) => {
    s(() => {
      d(...M);
    });
  }, c = () => {
    const d = t;
    t = [], d.length && Tr(() => {
      r(() => {
        d.forEach((M) => {
          n(M);
        });
      });
    });
  };
  return {
    batch: o,
    batchCalls: a,
    schedule: s,
    setNotifyFunction: (d) => {
      n = d;
    },
    setBatchNotifyFunction: (d) => {
      r = d;
    }
  };
}
const W = k2();
class Ni {
  destroy() {
    this.clearGcTimeout();
  }
  scheduleGc() {
    this.clearGcTimeout(), L2(this.cacheTime) && (this.gcTimeout = setTimeout(() => {
      this.optionalRemove();
    }, this.cacheTime));
  }
  updateCacheTime(e) {
    this.cacheTime = Math.max(this.cacheTime || 0, e ?? (_n ? 1 / 0 : 5 * 60 * 1e3));
  }
  clearGcTimeout() {
    this.gcTimeout && (clearTimeout(this.gcTimeout), this.gcTimeout = void 0);
  }
}
class Z2 extends Ni {
  constructor(e) {
    super(), this.abortSignalConsumed = !1, this.defaultOptions = e.defaultOptions, this.setOptions(e.options), this.observers = [], this.cache = e.cache, this.logger = e.logger || Hn, this.queryKey = e.queryKey, this.queryHash = e.queryHash, this.initialState = e.state || E2(this.options), this.state = this.initialState, this.scheduleGc();
  }
  get meta() {
    return this.options.meta;
  }
  setOptions(e) {
    this.options = {
      ...this.defaultOptions,
      ...e
    }, this.updateCacheTime(this.options.cacheTime);
  }
  optionalRemove() {
    !this.observers.length && this.state.fetchStatus === "idle" && this.cache.remove(this);
  }
  setData(e, n) {
    const r = x2(this.state.data, e, this.options);
    return this.dispatch({
      data: r,
      type: "success",
      dataUpdatedAt: n == null ? void 0 : n.updatedAt,
      manual: n == null ? void 0 : n.manual
    }), r;
  }
  setState(e, n) {
    this.dispatch({
      type: "setState",
      state: e,
      setStateOptions: n
    });
  }
  cancel(e) {
    var n;
    const r = this.promise;
    return (n = this.retryer) == null || n.cancel(e), r ? r.then(Q).catch(Q) : Promise.resolve();
  }
  destroy() {
    super.destroy(), this.cancel({
      silent: !0
    });
  }
  reset() {
    this.destroy(), this.setState(this.initialState);
  }
  isActive() {
    return this.observers.some((e) => e.options.enabled !== !1);
  }
  isDisabled() {
    return this.getObserversCount() > 0 && !this.isActive();
  }
  isStale() {
    return this.state.isInvalidated || !this.state.dataUpdatedAt || this.observers.some((e) => e.getCurrentResult().isStale);
  }
  isStaleByTime(e = 0) {
    return this.state.isInvalidated || !this.state.dataUpdatedAt || !A2(this.state.dataUpdatedAt, e);
  }
  onFocus() {
    var e;
    const n = this.observers.find((r) => r.shouldFetchOnWindowFocus());
    n && n.refetch({
      cancelRefetch: !1
    }), (e = this.retryer) == null || e.continue();
  }
  onOnline() {
    var e;
    const n = this.observers.find((r) => r.shouldFetchOnReconnect());
    n && n.refetch({
      cancelRefetch: !1
    }), (e = this.retryer) == null || e.continue();
  }
  addObserver(e) {
    this.observers.includes(e) || (this.observers.push(e), this.clearGcTimeout(), this.cache.notify({
      type: "observerAdded",
      query: this,
      observer: e
    }));
  }
  removeObserver(e) {
    this.observers.includes(e) && (this.observers = this.observers.filter((n) => n !== e), this.observers.length || (this.retryer && (this.abortSignalConsumed ? this.retryer.cancel({
      revert: !0
    }) : this.retryer.cancelRetry()), this.scheduleGc()), this.cache.notify({
      type: "observerRemoved",
      query: this,
      observer: e
    }));
  }
  getObserversCount() {
    return this.observers.length;
  }
  invalidate() {
    this.state.isInvalidated || this.dispatch({
      type: "invalidate"
    });
  }
  fetch(e, n) {
    var r, o;
    if (this.state.fetchStatus !== "idle") {
      if (this.state.dataUpdatedAt && n != null && n.cancelRefetch)
        this.cancel({
          silent: !0
        });
      else if (this.promise) {
        var s;
        return (s = this.retryer) == null || s.continueRetry(), this.promise;
      }
    }
    if (e && this.setOptions(e), !this.options.queryFn) {
      const g = this.observers.find((N) => N.options.queryFn);
      g && this.setOptions(g.options);
    }
    const a = f2(), c = {
      queryKey: this.queryKey,
      pageParam: void 0,
      meta: this.meta
    }, l = (g) => {
      Object.defineProperty(g, "signal", {
        enumerable: !0,
        get: () => {
          if (a)
            return this.abortSignalConsumed = !0, a.signal;
        }
      });
    };
    l(c);
    const u = () => this.options.queryFn ? (this.abortSignalConsumed = !1, this.options.queryFn(c)) : Promise.reject("Missing queryFn for queryKey '" + this.options.queryHash + "'"), d = {
      fetchOptions: n,
      options: this.options,
      queryKey: this.queryKey,
      state: this.state,
      fetchFn: u
    };
    if (l(d), (r = this.options.behavior) == null || r.onFetch(d), this.revertState = this.state, this.state.fetchStatus === "idle" || this.state.fetchMeta !== ((o = d.fetchOptions) == null ? void 0 : o.meta)) {
      var M;
      this.dispatch({
        type: "fetch",
        meta: (M = d.fetchOptions) == null ? void 0 : M.meta
      });
    }
    const I = (g) => {
      if (Be(g) && g.silent || this.dispatch({
        type: "error",
        error: g
      }), !Be(g)) {
        var N, m, j, p;
        (N = (m = this.cache.config).onError) == null || N.call(m, g, this), (j = (p = this.cache.config).onSettled) == null || j.call(p, this.state.data, g, this);
      }
      this.isFetchingOptimistic || this.scheduleGc(), this.isFetchingOptimistic = !1;
    };
    return this.retryer = ji({
      fn: d.fetchFn,
      abort: a == null ? void 0 : a.abort.bind(a),
      onSuccess: (g) => {
        var N, m, j, p;
        if (typeof g > "u") {
          I(new Error(this.queryHash + " data is undefined"));
          return;
        }
        this.setData(g), (N = (m = this.cache.config).onSuccess) == null || N.call(m, g, this), (j = (p = this.cache.config).onSettled) == null || j.call(p, g, this.state.error, this), this.isFetchingOptimistic || this.scheduleGc(), this.isFetchingOptimistic = !1;
      },
      onError: I,
      onFail: (g, N) => {
        this.dispatch({
          type: "failed",
          failureCount: g,
          error: N
        });
      },
      onPause: () => {
        this.dispatch({
          type: "pause"
        });
      },
      onContinue: () => {
        this.dispatch({
          type: "continue"
        });
      },
      retry: d.options.retry,
      retryDelay: d.options.retryDelay,
      networkMode: d.options.networkMode
    }), this.promise = this.retryer.promise, this.promise;
  }
  dispatch(e) {
    const n = (r) => {
      var o, s;
      switch (e.type) {
        case "failed":
          return {
            ...r,
            fetchFailureCount: e.failureCount,
            fetchFailureReason: e.error
          };
        case "pause":
          return {
            ...r,
            fetchStatus: "paused"
          };
        case "continue":
          return {
            ...r,
            fetchStatus: "fetching"
          };
        case "fetch":
          return {
            ...r,
            fetchFailureCount: 0,
            fetchFailureReason: null,
            fetchMeta: (o = e.meta) != null ? o : null,
            fetchStatus: Rn(this.options.networkMode) ? "fetching" : "paused",
            ...!r.dataUpdatedAt && {
              error: null,
              status: "loading"
            }
          };
        case "success":
          return {
            ...r,
            data: e.data,
            dataUpdateCount: r.dataUpdateCount + 1,
            dataUpdatedAt: (s = e.dataUpdatedAt) != null ? s : Date.now(),
            error: null,
            isInvalidated: !1,
            status: "success",
            ...!e.manual && {
              fetchStatus: "idle",
              fetchFailureCount: 0,
              fetchFailureReason: null
            }
          };
        case "error":
          const a = e.error;
          return Be(a) && a.revert && this.revertState ? {
            ...this.revertState,
            fetchStatus: "idle"
          } : {
            ...r,
            error: a,
            errorUpdateCount: r.errorUpdateCount + 1,
            errorUpdatedAt: Date.now(),
            fetchFailureCount: r.fetchFailureCount + 1,
            fetchFailureReason: a,
            fetchStatus: "idle",
            status: "error"
          };
        case "invalidate":
          return {
            ...r,
            isInvalidated: !0
          };
        case "setState":
          return {
            ...r,
            ...e.state
          };
      }
    };
    this.state = n(this.state), W.batch(() => {
      this.observers.forEach((r) => {
        r.onQueryUpdate(e);
      }), this.cache.notify({
        query: this,
        type: "updated",
        action: e
      });
    });
  }
}
function E2(t) {
  const e = typeof t.initialData == "function" ? t.initialData() : t.initialData, n = typeof e < "u", r = n ? typeof t.initialDataUpdatedAt == "function" ? t.initialDataUpdatedAt() : t.initialDataUpdatedAt : 0;
  return {
    data: e,
    dataUpdateCount: 0,
    dataUpdatedAt: n ? r ?? Date.now() : 0,
    error: null,
    errorUpdateCount: 0,
    errorUpdatedAt: 0,
    fetchFailureCount: 0,
    fetchFailureReason: null,
    fetchMeta: null,
    isInvalidated: !1,
    status: n ? "success" : "loading",
    fetchStatus: "idle"
  };
}
class W2 extends Pe {
  constructor(e) {
    super(), this.config = e || {}, this.queries = [], this.queriesMap = {};
  }
  build(e, n, r) {
    var o;
    const s = n.queryKey, a = (o = n.queryHash) != null ? o : On(s, n);
    let c = this.get(a);
    return c || (c = new Z2({
      cache: this,
      logger: e.getLogger(),
      queryKey: s,
      queryHash: a,
      options: e.defaultQueryOptions(n),
      state: r,
      defaultOptions: e.getQueryDefaults(s)
    }), this.add(c)), c;
  }
  add(e) {
    this.queriesMap[e.queryHash] || (this.queriesMap[e.queryHash] = e, this.queries.push(e), this.notify({
      type: "added",
      query: e
    }));
  }
  remove(e) {
    const n = this.queriesMap[e.queryHash];
    n && (e.destroy(), this.queries = this.queries.filter((r) => r !== e), n === e && delete this.queriesMap[e.queryHash], this.notify({
      type: "removed",
      query: e
    }));
  }
  clear() {
    W.batch(() => {
      this.queries.forEach((e) => {
        this.remove(e);
      });
    });
  }
  get(e) {
    return this.queriesMap[e];
  }
  getAll() {
    return this.queries;
  }
  find(e, n) {
    const [r] = ct(e, n);
    return typeof r.exact > "u" && (r.exact = !0), this.queries.find((o) => Ar(r, o));
  }
  findAll(e, n) {
    const [r] = ct(e, n);
    return Object.keys(r).length > 0 ? this.queries.filter((o) => Ar(r, o)) : this.queries;
  }
  notify(e) {
    W.batch(() => {
      this.listeners.forEach(({
        listener: n
      }) => {
        n(e);
      });
    });
  }
  onFocus() {
    W.batch(() => {
      this.queries.forEach((e) => {
        e.onFocus();
      });
    });
  }
  onOnline() {
    W.batch(() => {
      this.queries.forEach((e) => {
        e.onOnline();
      });
    });
  }
}
class U2 extends Ni {
  constructor(e) {
    super(), this.defaultOptions = e.defaultOptions, this.mutationId = e.mutationId, this.mutationCache = e.mutationCache, this.logger = e.logger || Hn, this.observers = [], this.state = e.state || P2(), this.setOptions(e.options), this.scheduleGc();
  }
  setOptions(e) {
    this.options = {
      ...this.defaultOptions,
      ...e
    }, this.updateCacheTime(this.options.cacheTime);
  }
  get meta() {
    return this.options.meta;
  }
  setState(e) {
    this.dispatch({
      type: "setState",
      state: e
    });
  }
  addObserver(e) {
    this.observers.includes(e) || (this.observers.push(e), this.clearGcTimeout(), this.mutationCache.notify({
      type: "observerAdded",
      mutation: this,
      observer: e
    }));
  }
  removeObserver(e) {
    this.observers = this.observers.filter((n) => n !== e), this.scheduleGc(), this.mutationCache.notify({
      type: "observerRemoved",
      mutation: this,
      observer: e
    });
  }
  optionalRemove() {
    this.observers.length || (this.state.status === "loading" ? this.scheduleGc() : this.mutationCache.remove(this));
  }
  continue() {
    var e, n;
    return (e = (n = this.retryer) == null ? void 0 : n.continue()) != null ? e : this.execute();
  }
  async execute() {
    const e = () => {
      var D;
      return this.retryer = ji({
        fn: () => this.options.mutationFn ? this.options.mutationFn(this.state.variables) : Promise.reject("No mutationFn found"),
        onFail: (L, P) => {
          this.dispatch({
            type: "failed",
            failureCount: L,
            error: P
          });
        },
        onPause: () => {
          this.dispatch({
            type: "pause"
          });
        },
        onContinue: () => {
          this.dispatch({
            type: "continue"
          });
        },
        retry: (D = this.options.retry) != null ? D : 0,
        retryDelay: this.options.retryDelay,
        networkMode: this.options.networkMode
      }), this.retryer.promise;
    }, n = this.state.status === "loading";
    try {
      var r, o, s, a, c, l, u, d;
      if (!n) {
        var M, I, g, N;
        this.dispatch({
          type: "loading",
          variables: this.options.variables
        }), await ((M = (I = this.mutationCache.config).onMutate) == null ? void 0 : M.call(I, this.state.variables, this));
        const L = await ((g = (N = this.options).onMutate) == null ? void 0 : g.call(N, this.state.variables));
        L !== this.state.context && this.dispatch({
          type: "loading",
          context: L,
          variables: this.state.variables
        });
      }
      const D = await e();
      return await ((r = (o = this.mutationCache.config).onSuccess) == null ? void 0 : r.call(o, D, this.state.variables, this.state.context, this)), await ((s = (a = this.options).onSuccess) == null ? void 0 : s.call(a, D, this.state.variables, this.state.context)), await ((c = (l = this.mutationCache.config).onSettled) == null ? void 0 : c.call(l, D, null, this.state.variables, this.state.context, this)), await ((u = (d = this.options).onSettled) == null ? void 0 : u.call(d, D, null, this.state.variables, this.state.context)), this.dispatch({
        type: "success",
        data: D
      }), D;
    } catch (D) {
      try {
        var m, j, p, A, Z, C, H, S;
        throw await ((m = (j = this.mutationCache.config).onError) == null ? void 0 : m.call(j, D, this.state.variables, this.state.context, this)), await ((p = (A = this.options).onError) == null ? void 0 : p.call(A, D, this.state.variables, this.state.context)), await ((Z = (C = this.mutationCache.config).onSettled) == null ? void 0 : Z.call(C, void 0, D, this.state.variables, this.state.context, this)), await ((H = (S = this.options).onSettled) == null ? void 0 : H.call(S, void 0, D, this.state.variables, this.state.context)), D;
      } finally {
        this.dispatch({
          type: "error",
          error: D
        });
      }
    }
  }
  dispatch(e) {
    const n = (r) => {
      switch (e.type) {
        case "failed":
          return {
            ...r,
            failureCount: e.failureCount,
            failureReason: e.error
          };
        case "pause":
          return {
            ...r,
            isPaused: !0
          };
        case "continue":
          return {
            ...r,
            isPaused: !1
          };
        case "loading":
          return {
            ...r,
            context: e.context,
            data: void 0,
            failureCount: 0,
            failureReason: null,
            error: null,
            isPaused: !Rn(this.options.networkMode),
            status: "loading",
            variables: e.variables
          };
        case "success":
          return {
            ...r,
            data: e.data,
            failureCount: 0,
            failureReason: null,
            error: null,
            status: "success",
            isPaused: !1
          };
        case "error":
          return {
            ...r,
            data: void 0,
            error: e.error,
            failureCount: r.failureCount + 1,
            failureReason: e.error,
            isPaused: !1,
            status: "error"
          };
        case "setState":
          return {
            ...r,
            ...e.state
          };
      }
    };
    this.state = n(this.state), W.batch(() => {
      this.observers.forEach((r) => {
        r.onMutationUpdate(e);
      }), this.mutationCache.notify({
        mutation: this,
        type: "updated",
        action: e
      });
    });
  }
}
function P2() {
  return {
    context: void 0,
    data: void 0,
    error: null,
    failureCount: 0,
    failureReason: null,
    isPaused: !1,
    status: "idle",
    variables: void 0
  };
}
class _2 extends Pe {
  constructor(e) {
    super(), this.config = e || {}, this.mutations = [], this.mutationId = 0;
  }
  build(e, n, r) {
    const o = new U2({
      mutationCache: this,
      logger: e.getLogger(),
      mutationId: ++this.mutationId,
      options: e.defaultMutationOptions(n),
      state: r,
      defaultOptions: n.mutationKey ? e.getMutationDefaults(n.mutationKey) : void 0
    });
    return this.add(o), o;
  }
  add(e) {
    this.mutations.push(e), this.notify({
      type: "added",
      mutation: e
    });
  }
  remove(e) {
    this.mutations = this.mutations.filter((n) => n !== e), this.notify({
      type: "removed",
      mutation: e
    });
  }
  clear() {
    W.batch(() => {
      this.mutations.forEach((e) => {
        this.remove(e);
      });
    });
  }
  getAll() {
    return this.mutations;
  }
  find(e) {
    return typeof e.exact > "u" && (e.exact = !0), this.mutations.find((n) => fr(e, n));
  }
  findAll(e) {
    return this.mutations.filter((n) => fr(e, n));
  }
  notify(e) {
    W.batch(() => {
      this.listeners.forEach(({
        listener: n
      }) => {
        n(e);
      });
    });
  }
  resumePausedMutations() {
    var e;
    return this.resuming = ((e = this.resuming) != null ? e : Promise.resolve()).then(() => {
      const n = this.mutations.filter((r) => r.state.isPaused);
      return W.batch(() => n.reduce((r, o) => r.then(() => o.continue().catch(Q)), Promise.resolve()));
    }).then(() => {
      this.resuming = void 0;
    }), this.resuming;
  }
}
function O2() {
  return {
    onFetch: (t) => {
      t.fetchFn = () => {
        var e, n, r, o, s, a;
        const c = (e = t.fetchOptions) == null || (n = e.meta) == null ? void 0 : n.refetchPage, l = (r = t.fetchOptions) == null || (o = r.meta) == null ? void 0 : o.fetchMore, u = l == null ? void 0 : l.pageParam, d = (l == null ? void 0 : l.direction) === "forward", M = (l == null ? void 0 : l.direction) === "backward", I = ((s = t.state.data) == null ? void 0 : s.pages) || [], g = ((a = t.state.data) == null ? void 0 : a.pageParams) || [];
        let N = g, m = !1;
        const j = (S) => {
          Object.defineProperty(S, "signal", {
            enumerable: !0,
            get: () => {
              var D;
              if ((D = t.signal) != null && D.aborted)
                m = !0;
              else {
                var L;
                (L = t.signal) == null || L.addEventListener("abort", () => {
                  m = !0;
                });
              }
              return t.signal;
            }
          });
        }, p = t.options.queryFn || (() => Promise.reject("Missing queryFn for queryKey '" + t.options.queryHash + "'")), A = (S, D, L, P) => (N = P ? [D, ...N] : [...N, D], P ? [L, ...S] : [...S, L]), Z = (S, D, L, P) => {
          if (m)
            return Promise.reject("Cancelled");
          if (typeof L > "u" && !D && S.length)
            return Promise.resolve(S);
          const gt = {
            queryKey: t.queryKey,
            pageParam: L,
            meta: t.options.meta
          };
          j(gt);
          const jt = p(gt);
          return Promise.resolve(jt).then((Xt) => A(S, L, Xt, P));
        };
        let C;
        if (!I.length)
          C = Z([]);
        else if (d) {
          const S = typeof u < "u", D = S ? u : kr(t.options, I);
          C = Z(I, S, D);
        } else if (M) {
          const S = typeof u < "u", D = S ? u : R2(t.options, I);
          C = Z(I, S, D, !0);
        } else {
          N = [];
          const S = typeof t.options.getNextPageParam > "u";
          C = (c && I[0] ? c(I[0], 0, I) : !0) ? Z([], S, g[0]) : Promise.resolve(A([], g[0], I[0]));
          for (let L = 1; L < I.length; L++)
            C = C.then((P) => {
              if (c && I[L] ? c(I[L], L, I) : !0) {
                const jt = S ? g[L] : kr(t.options, P);
                return Z(P, S, jt);
              }
              return Promise.resolve(A(P, g[L], I[L]));
            });
        }
        return C.then((S) => ({
          pages: S,
          pageParams: N
        }));
      };
    }
  };
}
function kr(t, e) {
  return t.getNextPageParam == null ? void 0 : t.getNextPageParam(e[e.length - 1], e);
}
function R2(t, e) {
  return t.getPreviousPageParam == null ? void 0 : t.getPreviousPageParam(e[0], e);
}
class H2 {
  constructor(e = {}) {
    this.queryCache = e.queryCache || new W2(), this.mutationCache = e.mutationCache || new _2(), this.logger = e.logger || Hn, this.defaultOptions = e.defaultOptions || {}, this.queryDefaults = [], this.mutationDefaults = [], this.mountCount = 0;
  }
  mount() {
    this.mountCount++, this.mountCount === 1 && (this.unsubscribeFocus = bn.subscribe(() => {
      bn.isFocused() && (this.resumePausedMutations(), this.queryCache.onFocus());
    }), this.unsubscribeOnline = me.subscribe(() => {
      me.isOnline() && (this.resumePausedMutations(), this.queryCache.onOnline());
    }));
  }
  unmount() {
    var e, n;
    this.mountCount--, this.mountCount === 0 && ((e = this.unsubscribeFocus) == null || e.call(this), this.unsubscribeFocus = void 0, (n = this.unsubscribeOnline) == null || n.call(this), this.unsubscribeOnline = void 0);
  }
  isFetching(e, n) {
    const [r] = ct(e, n);
    return r.fetchStatus = "fetching", this.queryCache.findAll(r).length;
  }
  isMutating(e) {
    return this.mutationCache.findAll({
      ...e,
      fetching: !0
    }).length;
  }
  getQueryData(e, n) {
    var r;
    return (r = this.queryCache.find(e, n)) == null ? void 0 : r.state.data;
  }
  ensureQueryData(e, n, r) {
    const o = ie(e, n, r), s = this.getQueryData(o.queryKey);
    return s ? Promise.resolve(s) : this.fetchQuery(o);
  }
  getQueriesData(e) {
    return this.getQueryCache().findAll(e).map(({
      queryKey: n,
      state: r
    }) => {
      const o = r.data;
      return [n, o];
    });
  }
  setQueryData(e, n, r) {
    const o = this.queryCache.find(e), s = o == null ? void 0 : o.state.data, a = v2(n, s);
    if (typeof a > "u")
      return;
    const c = ie(e), l = this.defaultQueryOptions(c);
    return this.queryCache.build(this, l).setData(a, {
      ...r,
      manual: !0
    });
  }
  setQueriesData(e, n, r) {
    return W.batch(() => this.getQueryCache().findAll(e).map(({
      queryKey: o
    }) => [o, this.setQueryData(o, n, r)]));
  }
  getQueryState(e, n) {
    var r;
    return (r = this.queryCache.find(e, n)) == null ? void 0 : r.state;
  }
  removeQueries(e, n) {
    const [r] = ct(e, n), o = this.queryCache;
    W.batch(() => {
      o.findAll(r).forEach((s) => {
        o.remove(s);
      });
    });
  }
  resetQueries(e, n, r) {
    const [o, s] = ct(e, n, r), a = this.queryCache, c = {
      type: "active",
      ...o
    };
    return W.batch(() => (a.findAll(o).forEach((l) => {
      l.reset();
    }), this.refetchQueries(c, s)));
  }
  cancelQueries(e, n, r) {
    const [o, s = {}] = ct(e, n, r);
    typeof s.revert > "u" && (s.revert = !0);
    const a = W.batch(() => this.queryCache.findAll(o).map((c) => c.cancel(s)));
    return Promise.all(a).then(Q).catch(Q);
  }
  invalidateQueries(e, n, r) {
    const [o, s] = ct(e, n, r);
    return W.batch(() => {
      var a, c;
      if (this.queryCache.findAll(o).forEach((u) => {
        u.invalidate();
      }), o.refetchType === "none")
        return Promise.resolve();
      const l = {
        ...o,
        type: (a = (c = o.refetchType) != null ? c : o.type) != null ? a : "active"
      };
      return this.refetchQueries(l, s);
    });
  }
  refetchQueries(e, n, r) {
    const [o, s] = ct(e, n, r), a = W.batch(() => this.queryCache.findAll(o).filter((l) => !l.isDisabled()).map((l) => {
      var u;
      return l.fetch(void 0, {
        ...s,
        cancelRefetch: (u = s == null ? void 0 : s.cancelRefetch) != null ? u : !0,
        meta: {
          refetchPage: o.refetchPage
        }
      });
    }));
    let c = Promise.all(a).then(Q);
    return s != null && s.throwOnError || (c = c.catch(Q)), c;
  }
  fetchQuery(e, n, r) {
    const o = ie(e, n, r), s = this.defaultQueryOptions(o);
    typeof s.retry > "u" && (s.retry = !1);
    const a = this.queryCache.build(this, s);
    return a.isStaleByTime(s.staleTime) ? a.fetch(s) : Promise.resolve(a.state.data);
  }
  prefetchQuery(e, n, r) {
    return this.fetchQuery(e, n, r).then(Q).catch(Q);
  }
  fetchInfiniteQuery(e, n, r) {
    const o = ie(e, n, r);
    return o.behavior = O2(), this.fetchQuery(o);
  }
  prefetchInfiniteQuery(e, n, r) {
    return this.fetchInfiniteQuery(e, n, r).then(Q).catch(Q);
  }
  resumePausedMutations() {
    return this.mutationCache.resumePausedMutations();
  }
  getQueryCache() {
    return this.queryCache;
  }
  getMutationCache() {
    return this.mutationCache;
  }
  getLogger() {
    return this.logger;
  }
  getDefaultOptions() {
    return this.defaultOptions;
  }
  setDefaultOptions(e) {
    this.defaultOptions = e;
  }
  setQueryDefaults(e, n) {
    const r = this.queryDefaults.find((o) => mt(e) === mt(o.queryKey));
    r ? r.defaultOptions = n : this.queryDefaults.push({
      queryKey: e,
      defaultOptions: n
    });
  }
  getQueryDefaults(e) {
    if (!e)
      return;
    const n = this.queryDefaults.find((r) => Ne(e, r.queryKey));
    return n == null ? void 0 : n.defaultOptions;
  }
  setMutationDefaults(e, n) {
    const r = this.mutationDefaults.find((o) => mt(e) === mt(o.mutationKey));
    r ? r.defaultOptions = n : this.mutationDefaults.push({
      mutationKey: e,
      defaultOptions: n
    });
  }
  getMutationDefaults(e) {
    if (!e)
      return;
    const n = this.mutationDefaults.find((r) => Ne(e, r.mutationKey));
    return n == null ? void 0 : n.defaultOptions;
  }
  defaultQueryOptions(e) {
    if (e != null && e._defaulted)
      return e;
    const n = {
      ...this.defaultOptions.queries,
      ...this.getQueryDefaults(e == null ? void 0 : e.queryKey),
      ...e,
      _defaulted: !0
    };
    return !n.queryHash && n.queryKey && (n.queryHash = On(n.queryKey, n)), typeof n.refetchOnReconnect > "u" && (n.refetchOnReconnect = n.networkMode !== "always"), typeof n.useErrorBoundary > "u" && (n.useErrorBoundary = !!n.suspense), n;
  }
  defaultMutationOptions(e) {
    return e != null && e._defaulted ? e : {
      ...this.defaultOptions.mutations,
      ...this.getMutationDefaults(e == null ? void 0 : e.mutationKey),
      ...e,
      _defaulted: !0
    };
  }
  clear() {
    this.queryCache.clear(), this.mutationCache.clear();
  }
}
const Zr = /* @__PURE__ */ ht(void 0), G2 = /* @__PURE__ */ ht(!1);
function Y2(t, e) {
  return t || (e && typeof window < "u" ? (window.ReactQueryClientContext || (window.ReactQueryClientContext = Zr), window.ReactQueryClientContext) : Zr);
}
const Q2 = ({
  client: t,
  children: e,
  context: n,
  contextSharing: r = !1
}) => {
  V(() => (t.mount(), () => {
    t.unmount();
  }), [t]);
  const o = Y2(n, r);
  return /* @__PURE__ */ i(G2.Provider, {
    value: !n && r
  }, /* @__PURE__ */ i(o.Provider, {
    value: t
  }, e));
}, mi = window.adminXQueryClient || new H2({
  defaultOptions: {
    queries: {
      refetchOnWindowFocus: !1,
      staleTime: 5 * (60 * 1e3),
      // 5 mins
      cacheTime: 10 * (60 * 1e3),
      // 10 mins
      // We have custom retry logic for specific errors in fetchApi()
      retry: !1,
      networkMode: "always"
    }
  }
});
window.adminXQueryClient || (window.adminXQueryClient = mi);
const bi = ht({
  ghostVersion: "",
  externalNavigate: () => {
  },
  unsplashConfig: {
    Authorization: "",
    "Accept-Version": "",
    "Content-Type": "",
    "App-Pragma": "",
    "X-Unsplash-Cache": !0
  },
  sentryDSN: null,
  onUpdate: () => {
  },
  onInvalidate: () => {
  },
  onDelete: () => {
  }
});
function J2({ children: t, ...e }) {
  return /* @__PURE__ */ b.jsx(Pn, { children: /* @__PURE__ */ b.jsx(Q2, { client: mi, children: /* @__PURE__ */ b.jsx(bi.Provider, { value: e, children: t }) }) });
}
const B2 = () => Et(bi), V2 = ht({
  route: "",
  updateRoute: () => {
  },
  loadingModal: !1,
  eventTarget: new EventTarget()
});
function X2(t, e) {
  if (!e)
    return null;
  const n = new RegExp(`/${t}/(.*)`), r = e == null ? void 0 : e.match(n);
  return r ? r[1] : null;
}
const F2 = (t, e, n, r) => {
  let o = window.location.hash;
  o = o.substring(1);
  const s = `${window.location.protocol}//${window.location.hostname}`, a = new URL(o, s), c = X2(t, a.pathname);
  if (!r || !n)
    return { pathName: c || "" };
  const l = a.searchParams;
  if (c && r && n) {
    const [, u] = Object.entries(r).find(([I]) => Ve(e || "", I)) || [], [d, M] = Object.entries(r).find(([I]) => Ve(c, I)) || [];
    return {
      pathName: c,
      changingModal: M && M !== u,
      modal: d && M ? (
        // we should consider adding '&& modalName !== currentModalName' here, but this breaks tests
        n().then(({ default: I }) => {
          Io.show(I[M], { pathName: c, params: Ve(c, d), searchParams: l });
        })
      ) : void 0
    };
  }
  return { pathName: "" };
}, Ve = (t, e) => {
  const n = new RegExp("^" + e.replace(/:(\w+)/g, "(?<$1>[^/]+)") + "/?$"), r = t.match(n);
  if (r)
    return r.groups || {};
}, q2 = ({ basePath: t, modals: e, children: n }) => {
  const { externalNavigate: r } = B2(), [o, s] = bt(void 0), [a, c] = bt(!1), [l] = bt(new EventTarget()), u = _((d) => {
    const M = typeof d == "string" ? { route: d } : d;
    if (M.isExternal) {
      r(M);
      return;
    }
    const I = M.route.replace(/^\//, "");
    I === o || (I ? window.location.hash = `/${t}/${I}` : window.location.hash = `/${t}`), l.dispatchEvent(new CustomEvent("routeChange", { detail: { newPath: I, oldPath: o } }));
  }, [t, l, r, o]);
  return V(() => {
    setTimeout(() => {
      e == null || e.load();
    }, 1e3);
  }, []), V(() => {
    const d = () => {
      s((M) => {
        const { pathName: I, modal: g, changingModal: N } = F2(t, M, e == null ? void 0 : e.load, e == null ? void 0 : e.paths);
        return g && N && (c(!0), g.then(() => c(!1))), I;
      });
    };
    return d(), window.addEventListener("hashchange", d), () => {
      window.removeEventListener("hashchange", d);
    };
  }, []), o === void 0 ? null : /* @__PURE__ */ b.jsx(
    V2.Provider,
    {
      value: {
        route: o,
        updateRoute: u,
        loadingModal: a,
        eventTarget: l
      },
      children: n
    }
  );
}, $2 = () => (
  // The div below should be converted into an app container component in the design system
  /* @__PURE__ */ b.jsxs("div", { className: "p-8 pt-9", children: [
    /* @__PURE__ */ b.jsxs("header", { children: [
      /* @__PURE__ */ b.jsx($r, { children: /* @__PURE__ */ b.jsxs(Kr, { children: [
        /* @__PURE__ */ b.jsx(to, { children: /* @__PURE__ */ b.jsx(eo, { href: "/posts/", children: "Posts" }) }),
        /* @__PURE__ */ b.jsx(ro, {}),
        /* @__PURE__ */ b.jsx(no, { children: "Analytics" })
      ] }) }),
      /* @__PURE__ */ b.jsxs("div", { className: "flex items-start justify-between mt-2", children: [
        /* @__PURE__ */ b.jsx(Fr, { size: "pagetitle", children: "Post analytics" }),
        /* @__PURE__ */ b.jsxs("div", { className: "flex items-center mt-1 gap-2", children: [
          /* @__PURE__ */ b.jsxs(se, { variant: "outline", children: [
            /* @__PURE__ */ b.jsx(Re, { name: "reload" }),
            " Refresh"
          ] }),
          /* @__PURE__ */ b.jsxs(se, { variant: "outline", children: [
            /* @__PURE__ */ b.jsx(Re, { name: "share" }),
            " Share"
          ] }),
          /* @__PURE__ */ b.jsx(se, { variant: "outline", children: /* @__PURE__ */ b.jsx(Re, { name: "ellipsis" }) })
        ] })
      ] })
    ] }),
    /* @__PURE__ */ b.jsx("div", { className: "mt-8 border rounded-lg border-grey-300 min-h-[500px] flex items-center justify-center text-grey-500", children: "TK" })
  ] })
), K2 = {
  paths: {
    "demo-modal": "DemoModal"
  },
  load: async () => import("./modals-ee4bac33.mjs")
}, t5 = ({ framework: t, designSystem: e }) => /* @__PURE__ */ b.jsx(J2, { ...t, children: /* @__PURE__ */ b.jsx(q2, { basePath: "post-analytics-spike", modals: K2, children: /* @__PURE__ */ b.jsx(Y1, { className: "post-analytics-spike", ...e, children: /* @__PURE__ */ b.jsx($2, {}) }) }) });
export {
  t5 as A,
  Io as N,
  b as j
};
//# sourceMappingURL=index-67f61ed8.mjs.map
