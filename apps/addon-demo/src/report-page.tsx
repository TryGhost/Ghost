...
function Overview({ report }: { report: SeoReport }) {
  ...
  return (
    <GhStack gap="lg">
      ...
      <GhText tone="muted">
        Last crawled {timeAgo(report.lastCrawledAt)} · {report.postsScanned} posts scanned
      </GhText>
    </GhStack>
  );
}
      ...
      {[...report.crawls].reverse().map((crawl) => (
        <GhStack key={crawl.at} gap="sm">
          <GhInline gap="sm" justify="between">
            ...
            <GhText tone="muted">
              score {crawl.score}/100 · {crawl.postsScanned} posts
            </GhText>
          </GhInline>
          <GhSeparator />
        </GhStack>
  ...
  if (!report) {
    return (
      <GhStack gap="md">
        ...
        <GhText tone="muted">
          Crawl your site to check meta descriptions, feature images, titles and slugs across your
          posts.
        </GhText>
        <GhButton disabled={busy} onPress={crawl}>
          Run first crawl
        </GhButton>
      </GhStack>
    );
  ...
  return (
    <GhStack gap="lg">
      <GhTabs
        value={section}
        onChange={(event) =>
          void ghost.navigate(`/apps/seo-assistant-demo${event.detail === '/' ? '' : event.detail}`)
        }
      >
        {Object.entries(SECTIONS).map(([sectionPath, label]) => (
          <GhTab key={sectionPath} value={sectionPath}>
            {label}
          </GhTab>
        ))}
      </GhTabs>
      ...
      <GhInline gap="sm">
        ...
        <GhButton disabled={busy} variant="destructive" onPress={clear}>
          Clear report
        </GhButton>
      </GhInline>
    </GhStack>
