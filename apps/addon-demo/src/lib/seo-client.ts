...
function check(
  id: string,
  label: string,
  failures: SeoCheckDetail[],
  options?: { errorAt?: number },
): SeoCheck {
  const errorAt = options?.errorAt ?? Infinity;
  const severity: SeoSeverity =
    failures.length === 0 ? 'pass' : failures.length >= errorAt ? 'error' : 'warning';
  return {
    id,
    label,
    severity,
...
function deriveChecks(posts: AdminPost[]): SeoCheck[] {
  ...
  const longTitles = posts
    ...
    .map((post) => ({
      title: titled(post),
      note: `Title is ${post.title!.length} characters — search engines truncate after 60`,
    }));
...
export async function runCrawl(ghost: GhostBridge): Promise<SeoReport> {
  const response = await ghost.fetch(
    '/ghost/api/admin/posts/?limit=50&fields=title,slug,custom_excerpt,feature_image,status',
  );
  if (!response.ok) {
...
export function openIssueCount(report: SeoReport): number {
  return report.checks.reduce(
    (sum, item) => sum + (item.severity === 'pass' ? 0 : item.details.length),
    0,
  );
}
