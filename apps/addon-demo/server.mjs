...
const server = createServer(async (req, res) => {
  ...
  if (url.pathname.startsWith('/api/')) {
    ...
    if (!identity) {
      sendJson(res, 401, {
        error: 'Missing Ghost identity — this endpoint is only reachable through ghost.fetch',
      });
      return;
    }
    ...
    if (url.pathname === '/api/crawl' && req.method === 'POST') {
      ...
      report = {
        ...
        crawls: [
          ...(report?.crawls ?? []),
          { at: crawledAt, score, postsScanned: Number(body.postsScanned ?? 0) },
        ].slice(-10),
      };
      sendJson(res, 200, { report });
      return;
...
server.listen(PORT, () => {
  console.log(
    `Add-on demo provider serving on http://localhost:${PORT} (manifest at /manifest.json)`,
