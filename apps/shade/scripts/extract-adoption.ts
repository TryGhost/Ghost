...
const ADMIN_REACT_APPS = ['admin', 'activitypub'] as const;
...
function main(): void {
  const apps = ADMIN_REACT_APPS.map((name) => scanApp(name)).filter(
    (a): a is AppReport => a !== null,
  );
  ...
  const shadeAdoptionPct =
    adminReactFilesTotal === 0
      ? 0
  ...
  for (const app of apps) {
    for (const c of app.shadeComponents) {
      ...
      aggregateShadeComponentCounts.set(
        c.name,
        (aggregateShadeComponentCounts.get(c.name) ?? 0) + c.count,
      );
    }
