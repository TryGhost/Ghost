...
export const ColorPickerOutput = ({...props}: ColorPickerOutputProps) => {
    ...
    return (
        <Select value={mode} onValueChange={setMode}>
            <SelectTrigger className="h-8 w-22 shrink-0 text-xs" {...props}>
...
export const ColorPickerFormat = ({
    ...
    const handleInputChange = (value: string, index?: number) => {
        try {
            ...
            } else if (mode === 'rgb' && index !== undefined) {
                const rgb = color
                    .rgb()
                    .array()
                    .map(v => Math.round(v));
                rgb[index] = parseInt(value, 10) || 0;
                const newColor = Color.rgb(rgb[0], rgb[1], rgb[2]);
                setHue(newColor.hue());
                setSaturation(newColor.saturationl());
                setLightness(newColor.lightness());
            } else if (mode === 'hsl' && index !== undefined) {
                const hsl = color
                    .hsl()
                    .array()
                    .map(v => Math.round(v));
                hsl[index] = parseInt(value, 10) || 0;
                const newColor = Color.hsl(hsl[0], hsl[1], hsl[2]);
                setHue(newColor.hue());
                setSaturation(newColor.saturationl());
                setLightness(newColor.lightness());
            }
    ...
    if (mode === 'hex' || mode === 'css') {
        ...
        const formattedValue = mode === 'hex' ? hex : cssValue;
    ...
    if (mode === 'rgb' || mode === 'hsl') {
        ...
        return (
            <div
                className={cn(
                    '-space-x-px flex flex-1 justify-stretch items-stretch rounded-md',
                    className
                )}
                {...props}
            >
