...
**Accessibility:**
- Use \`bannerRole="status"\` for non-critical updates (default)
- Use \`bannerRole="alert"\` for important messages requiring attention
- Use \`bannerRole="region"\` with \`ariaLabel\` for landmark sections
- Default \`aria-live="polite"\` won't interrupt screen readers
- Use \`aria-live="assertive"\` for urgent messages`
            }
        }
    },
    argTypes: {
...
export const AccessibilityAlert: Story = {
    args: {
        ...
        bannerRole: 'alert',
        ariaLive: 'assertive',
        ariaLabel: 'Critical error notification',
        children: (
            <div className="flex items-start gap-3">
                <XCircle className="mt-0.5 size-5 text-red-600" />
                <div className="text-sm">
                    <strong className="mb-1 block">Critical Error</strong>
                    <p>This uses role=&quot;alert&quot; and aria-live=&quot;assertive&quot; for screen readers.</p>
                </div>
            </div>
        )
    }
