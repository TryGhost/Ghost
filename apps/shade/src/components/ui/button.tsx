    ...
    'inline-flex items-center justify-center gap-1.5 rounded-md text-control whitespace-nowrap transition-colors focus-visible:ring-1 focus-visible:ring-focus-ring focus-visible:outline-hidden disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0 [&_svg]:stroke-[1.5px]',
    {
        variants: {
            variant: {
                ...
                outline: 'border border-control-border bg-background font-medium hover:bg-button-hover hover:text-accent-foreground',
                ...
                dropdown: 'border border-control-border bg-background hover:bg-button-hover hover:text-accent-foreground'
            },
            size: {
...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
    ({className, variant, size, asChild = false, children, ...props}, ref) => {
        ...
        const content = variant === 'dropdown' ? (
            <>
                {children}
                <ChevronDown className="-mr-0.5! -ml-1! size-4 stroke-[2px]! opacity-50" strokeWidth={2} />
            </>
        ) : children;
