...
const CopyField = React.forwardRef<HTMLDivElement, CopyFieldProps>(
  ({ children, className, disabled = false, value, ...props }, ref) => {
    ...
    const contextValue = React.useMemo(
      () => ({ copied, copy, disabled, labelId, value }),
      [copied, copy, disabled, labelId, value],
    );
...
const CopyFieldLabel = React.forwardRef<HTMLElement, React.ComponentProps<typeof Text>>(
  ({ className, id, ...props }, ref) => {
    ...
    return (
      <Text
        ...
        as="div"
        className={cn(
          'text-control! font-medium group-data-[disabled=true]/copy-field:opacity-50',
          className,
        )}
...
const CopyFieldContent = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div
      ref={ref}
      className={cn(
...
const CopyFieldValue = React.forwardRef<HTMLElement, React.ComponentProps<typeof Text>>(
...
const CopyFieldCopyButton = React.forwardRef<HTMLButtonElement, CopyFieldCopyButtonProps>(
  ({ children = 'Copy', className, copiedLabel = 'Copied', disabled, onClick, ...props }, ref) => {
    ...
    return (
      <Button
        ref={ref}
        className={cn(
          'bg-surface-elevated hover:border-border-strong/40 hover:bg-surface-elevated',
          className,
        )}
