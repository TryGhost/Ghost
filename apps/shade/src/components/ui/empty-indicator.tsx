...
const EmptyBadge = React.forwardRef<HTMLDivElement, EmptyBadgeProps>(
  ({ children, className, ...props }, ref) => {
    return (
      <div
        ref={ref}
        className={cn(
          'flex size-12 max-h-12 max-w-12 items-center justify-center rounded-full bg-muted [&_svg]:size-4 [&_svg]:shrink-0 [&_svg]:text-muted-foreground',
          className,
        )}
        {...props}
      >
...
const EmptyIndicator = React.forwardRef<HTMLDivElement, EmptyIndicatorProps>(
  ({ children, className, title, description, actions, ...props }, ref) => {
    return (
      <div
        ref={ref}
        ...
        {...props}
      >
        <EmptyBadge>{children}</EmptyBadge>
        <div className="flex max-w-[320px] flex-col gap-1.5">
          ...
          {description && (
            <p className="text-sm leading-tight text-pretty text-muted-foreground">{description}</p>
          )}
        </div>
