...
function filterOptionsBySearch<T = unknown>(
  options: FilterOption<T>[],
  search: string,
): FilterOption<T>[] {
...
export function MultiSelectCombobox<T = unknown>({
  ...
  const i18n = useMemo(() => ({ ...DEFAULT_MULTI_SELECT_I18N, ...i18nOverrides }), [i18nOverrides]);
    ...
    if (optionsFromResolved.length > 0) {
      setCachedSelectedOptions((prev) => {
        ...
        for (const value of values) {
          const option =
            optionsFromResolved.find((opt) => opt.value === value) ??
  ...
  const defaultRenderItem = ({ option, isSelected, onSelect }: RenderItemProps<T>) => (
    ...
    >
      ...
      <div className="flex flex-col overflow-hidden">
        ...
        {option.detail && (
          <span className="truncate text-sm text-muted-foreground" title={option.detail}>
            {option.detail}
          </span>
        )}
      </div>
    ...
    <div className={cn('w-full', className)}>
      <Command shouldFilter={source.shouldClientFilter}>
        ...
        <CommandList className="outline-hidden">
          ...
          {unselectedOptions.length > 0 && (
            <>
              ...
              {unselectedGroups.map((group) => (
                <CommandGroup
                  key={group.key === undefined ? 'ungrouped' : `group:${group.key}`}
                  heading={group.label}
                >
          ...
          {source.hasMore && (
            <>
              {(visibleSelectedOptions.length > 0 || unselectedOptions.length > 0) && (
                <CommandSeparator />
              )}
              <div className="p-1.5">
                <button
