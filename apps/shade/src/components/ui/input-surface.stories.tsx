...
export const Self: Story = {
  parameters: {
    docs: {
      description: {
        story:
          'Recipe applied directly to a focusable element. Used by `Input`, `Textarea`, and the `Select` trigger.',
      },
    },
  },
  render: () => (
    <input className={cn(inputSurface('self'), inputBase)} placeholder="Tab to focus me" />
  ),
};
...
export const Within: Story = {
  parameters: {
    docs: {
      description: {
        story:
          'Recipe applied to a wrapper that contains a focusable child. The wrapper itself has no focus, but the focus ring lights up when any descendant becomes `:focus-visible`.',
      },
    },
  },
  render: () => (
...
export const States: Story = {
  parameters: {
    docs: {
      description: {
        story:
          'Default, invalid (`aria-invalid="true"`), and disabled field states for `\'self\'` mode. Text-entry controls compose `disabledFieldSelf` to keep values readable.',
      },
    },
  },
  render: () => (
    <div className="flex flex-col gap-3">
      <input className={cn(inputSurface('self'), inputBase)} placeholder="Default" />
      <input className={cn(inputSurface('self'), inputBase)} placeholder="Invalid" aria-invalid />
      <input
...
export const WithinStates: Story = {
  parameters: {
    docs: {
      description: {
        story:
          "Default and invalid states for `'within'` mode. Disabled state is left to the wrapper consumer (the recipe doesn't handle disabled in `within` mode since wrappers aren't themselves disableable).",
      },
    },
  },
  render: () => (
...
export const CustomFocusScope: Story = {
  parameters: {
    docs: {
      description: {
        story:
          'For wrappers that should only react to a specific descendant (e.g. `InputGroup`, where focusing a button inside the group must NOT trigger the surface ring), compose `inputSurfaceClasses` atoms with a literal `has-[…]:` selector. The literal class string is required so Tailwind can detect it at build time.',
      },
    },
  },
  render: () => (
...
export const WhatTheRecipeOwns: Story = {
  parameters: {
    docs: {
      description: {
        story:
          'Where the line is between the recipe and the consumer. Keep size, padding, and typography concerns out of the recipe.',
      },
    },
  },
  render: () => (
