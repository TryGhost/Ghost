...
const KpiCardHeader: React.FC<React.HTMLAttributes<HTMLDivElement>> = ({
  children,
  className,
  ...props
}) => {
...
const KpiCardHeaderLabel: React.FC<React.HTMLAttributes<HTMLDivElement>> = ({
  children,
  className,
  color,
  ...props
  ...
  return (
    ...
    >
      {color && (
        <div
          className="ml-1 size-2 rounded-full opacity-50"
...
const KpiCardHeaderValue: React.FC<KpiCardValueProps> = ({
  value,
  diffDirection,
  diffValue,
  diffTooltip,
  ...
  return <MetricValue trailing={trailing} value={value} valueTestId="kpi-card-header-value" />;
};
