...
const meta = {
  ...
  parameters: {
    docs: {
      description: {
        component:
          'Card header layout for displaying a KPI metric with optional trend badge. Composes `MetricValue` and `TrendBadge` from the Components layer. Use inside a `Card` (from Components / Card).',
      },
    },
  },
    ...
    <Card className="w-[350px]">
      <KpiCardHeader className="relative flex grow flex-row items-start justify-between gap-5 border-none pb-4">
        <div className="flex grow flex-col gap-1.5 border-none pb-0">
          ...
          <KpiCardHeaderValue diffDirection="up" diffValue="+12.5%" value="2,547" />
        </div>
      </KpiCardHeader>
    ...
    <Card className="w-[350px]">
      <KpiCardHeader className="relative flex grow flex-row items-start justify-between gap-5 border-none pb-4">
        <div className="flex grow flex-col gap-1.5 border-none pb-0">
          ...
          <KpiCardHeaderValue diffDirection="down" diffValue="-3.2%" value="1,234" />
        </div>
      </KpiCardHeader>
      <CardContent>Chart placeholder content</CardContent>
    </Card>
    ...
    <Card className="w-[350px]">
      <KpiCardHeader className="relative flex grow flex-row items-start justify-between gap-5 border-none pb-4">
        <div className="flex grow flex-col gap-1.5 border-none pb-0">
          ...
          <KpiCardHeaderValue diffDirection="up" diffValue="+8.7%" value="$4,567" />
        </div>
      </KpiCardHeader>
      <CardContent>Chart placeholder content</CardContent>
    </Card>
    ...
    <Card className="w-[350px]">
      <KpiCardHeader className="relative flex grow flex-row items-start justify-between gap-5 border-none pb-4">
        <div className="flex grow flex-col gap-1.5 border-none pb-0">
          ...
          <KpiCardHeaderValue diffDirection="empty" value="15,789" />
        </div>
      </KpiCardHeader>
      <CardContent>Chart placeholder content</CardContent>
    </Card>
      ...
      <KpiCardHeader className="relative flex grow flex-row items-start justify-between gap-5 border-none pb-4">
        <div className="flex grow flex-col gap-1.5 border-none pb-0">
          <KpiCardHeaderLabel className="transition-all group-hover:text-foreground">
            <span
              className="inline-block size-2 rounded-full opacity-50"
              style={{ backgroundColor: 'var(--chart-teal)' }}
            ></span>
            <User size={16} strokeWidth={1.5} />
            Members
          </KpiCardHeaderLabel>
          <KpiCardHeaderValue diffDirection="same" diffValue="0%" value="2,456" />
        </div>
        <Button
    ...
    <Card className="w-[350px]">
      <KpiCardHeader className="relative flex grow flex-row items-start justify-between gap-5 border-none pb-4">
        <div className="flex grow flex-col gap-1.5 border-none pb-0">
          ...
          <KpiCardHeaderValue diffDirection="hidden" value="$125,890" />
        </div>
      </KpiCardHeader>
      <CardContent>Chart placeholder content</CardContent>
    </Card>
