  ...
  ],
  {
    variants: {
      variant: {
        solid: 'bg-secondary',
        outline:
          'border border-border bg-background hover:bg-secondary has-[[data-slot=switch]]:hover:bg-transparent has-[>[data-slot=filters-input-wrapper]]:hover:bg-background',
      },
      size: {
        lg: 'h-10 gap-1.5 px-2 text-sm [&_svg:not([class*=size-])]:size-4',
        md: 'h-(--control-height) gap-1.5 px-2.5 [&_svg:not([class*=size-])]:size-4',
...
export function FilterDatePicker<T = unknown>({
  ...
  const handleInputBlur = (e: React.FocusEvent<HTMLInputElement>) => {
    ...
    const dateValue =
      inputValue && !parsedInputValue ? formatFilterDateValue(new Date()) : inputValue;
      ...
      <DropdownMenuContent align="start" className="w-fit min-w-fit">
        {options.map((option) => (
          ...
          >
            ...
            <Check
              className={`ms-auto text-primary ${option.value === value ? 'opacity-100' : 'opacity-0'}`}
            />
          </DropdownMenuItem>
        ))}
      </DropdownMenuContent>
...
export function FilterSegmentInput({
  value,
  onChange,
  placeholder,
  ariaLabel,
...
function filterOptionsBySearchInput<T = unknown>(
  options: FilterOption<T>[],
  searchInput: string,
): FilterOption<T>[] {
    ...
    if (optionsFromField.length > 0) {
      setCachedSelectedOptions((prev) => {
        ...
        for (const value of effectiveValues) {
          ...
          const option =
            optionsFromField.find((opt) => opt.value === value) ??
          ...
          {field.customValueRenderer ? (
            ...
            <>
              {selectedOptions.length > 0 && selectedOptions.some((option) => option.icon) && (
                <div
                  className={cn(
                    'flex shrink-0 items-center -space-x-0.5',
              ...
              {selectedOptions.length === 1 ? (
                <span
                  className="min-w-0 truncate text-accent-foreground"
                  title={
                    selectedOptions[0].detail
                      ? `${selectedOptions[0].label} - ${selectedOptions[0].detail}`
...
function FilterValueSelector<T = unknown>({
  ...
  return (
    ...
    >
      ...
      <PopoverContent
        className={cn(
          'data-[state=closed]:animation-none! w-36 p-0 data-[state=closed]:duration-0!',
...
export function Filters<T = unknown>({
  ...
  const selectedFieldForOptions = selectedFieldKeyForOptions
    ? fieldsMap[selectedFieldKeyForOptions]
    : null;
            ...
            >
              {selectedFieldForOptions ? (
                ...
                <SelectOptionsPopover<T>
                  ...
                  onChange={(values) =>
                    addFilterWithOption(selectedFieldForOptions, values as unknown[])
                  }
