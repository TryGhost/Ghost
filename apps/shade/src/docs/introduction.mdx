...
The line between those four layers is the most important thing to understand about Shade. We cover it in detail on the [Architecture](?path=/docs/overview-architecture--docs) page, including a glossary of terms.
...
If you're new here, read the [Architecture](?path=/docs/overview-architecture--docs) page next. It explains the layers (tokens, primitives, components, recipes, patterns) with a glossary so the vocabulary makes sense, and a short decision rule for choosing where new code belongs.
...
- **Patterns** — product compositions (page headers, list pages, KPI cards, filters, charts).
- **Recipes** — shared visual rules like `inputSurface` that shape every form control's chrome.
- **Posts–Stats** — interim shared components between the Posts and Stats apps until they merge.
- **Deprecated** — legacy layout shells (`ListHeader`, `ViewHeader`, etc.) being phased out.
