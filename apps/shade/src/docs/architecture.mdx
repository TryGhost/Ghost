...
Page-shaped patterns (those that represent a recurring kind of Admin page — List page, Analytics page, Detail page) are documented in [`docs/page-types.md`](../../docs/page-types.md).
...
- **You're reaching for Shade because you can't decide where the thing belongs in the product.** That's a feature-placement question — solve it with the [page types](../../docs/page-types.md) first. Promoting to Shade to dodge a structural decision will fossilize the wrong placement.
...
A page-shaped pattern (`ListPage`, future `DetailPage`) must map to a named entry in [`page-types.md`](../../docs/page-types.md). If a candidate doesn't fit any named page type, either the candidate is wrong or the taxonomy needs a new entry — resolve that before shipping the pattern.
