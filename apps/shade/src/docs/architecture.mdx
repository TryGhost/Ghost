...
Pattern files live in `components/patterns/` and are exposed via `@tryghost/shade/patterns`. A few component-level features (currently `kpi/` and `charts/`) still live under `components/features/` and are exposed via `@tryghost/shade/components` — they're slated to move to `components/ui/` in a follow-up.
...
│   ├── patterns/        Product compositions (PageHeader, ListPage, Filters, ShareModal, …)
│   ├── features/        Component-level features pending move to ui/ (KPI, charts)
...
**Stories live next to their component** as `<name>.stories.tsx`. The Storybook title puts each story in the right sidebar group: `Foundations / X`, `Primitives / X`, `Components / X`, `Layout / X`, `Patterns / X`, `Features / X`, `Posts–Stats / X`.
