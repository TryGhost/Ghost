...
import {
  ...
  KpiCardHeaderLabel,
  KpiCardHeaderValue,
} from '@/components/patterns/kpi-card';
import {
  Table,
  TableBody,
  TableCell,
...
const Dashboard = () => {
  ...
  const adminXDsFilesRemaining = apps.reduce(
    (sum: number, app: AppRow) => sum + app.adminXDsFiles,
    0,
  );
  ...
  return (
    <div className="shade" style={{ padding: '32px', maxWidth: '1240px', margin: '0 auto' }}>
        ...
        <Card>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <KpiCardHeader>
              ...
              />
              <span className="text-xs text-muted-foreground">
                unique names imported from <code>@tryghost/shade*</code> across the four adopter
                apps
              </span>
            </KpiCardHeader>
            <KpiCardHeader>
      ...
      <section className="mb-8">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Files in each Admin React app, split by which design system they import.
              &quot;Neither&quot; covers files that don&apos;t import a design system (utilities,
      ...
      <section className="mb-8 grid grid-cols-1 gap-6 lg:grid-cols-2">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Descriptive only: the names imported most often from <code>@tryghost/shade*</code>{' '}
              across admin, posts, stats, and activitypub. High counts don&apos;t mean &quot;more is
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Every admin-x-design-system export still imported across the Admin React apps, ranked
              by usage. Replacing the most-used ones with Shade equivalents yields the biggest wins.
            </CardDescription>
          </CardHeader>
          <CardContent>
      ...
      <section className="mb-8">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              UMD bundles served to site visitors via the CDN. They have their own per-app styling
              and are <strong>out of scope</strong> for the Shade migration.
            </CardDescription>
          </CardHeader>
      ...
      <footer className="mt-12 border-t border-border pt-6 text-xs text-muted-foreground">
        <p className="mb-2">
          <strong>How this is measured.</strong> The extraction script scans every{' '}
          <code>.ts/.tsx</code> file under each app&apos;s <code>src/</code> (excluding stories and
          tests) and records which files import from <code>@tryghost/shade*</code> or{' '}
          <code>@tryghost/admin-x-design-system</code>. Component counts are the unique named
          imports parsed from those statements.
        </p>
        <p>
          <strong>What this is not.</strong> &quot;Shade adoption %&quot; is intentionally absent.
          Many files (utilities, hooks, types, contexts) legitimately don&apos;t need a design
          system, so counting them as &quot;not adopted&quot; would be misleading. The number that
          matters for the migration is the count of files still importing the legacy DS, plus the
          punch list of specific exports still in use.
        </p>
      </footer>
    </div>
  );
