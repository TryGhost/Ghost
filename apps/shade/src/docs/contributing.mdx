...
`useIsMobile` is available from `@tryghost/shade/utils` for consumers that need
the same responsive boundary as `SidebarProvider`: mobile below 801px, desktop
from 801px. Reuse this hook instead of duplicating the breakpoint in app code.
