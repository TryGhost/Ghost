...
The rules below apply to the foundational controls in `components/ui/` — the generic, reusable controls listed in the sidebar under **Components**. Recipes and smaller helpers in the same folder follow the same spirit but don't always carry formal rule documents.

Some surfaces deliberately sit outside the simple rules — large compounds that mix layout, behavior, and styling in ways the standard contract doesn't fit cleanly. Those are documented at the component level rather than against the shared rules.
...
**What's intentionally out of scope?** Workflow-specific behavior, product-specific styling, things that belong in a Pattern wrapper instead.
...
- If behavior is workflow-specific, route it to `patterns/`, not into the base component.
...
## Components vs. patterns: the dividing line

The honest answer is: pick `components` when the abstraction is genuinely generic, and `patterns` when it encodes product workflow.

The smell test: if the component name reads like English you'd use in any web app, it's a component. If it reads like Ghost-specific jargon, it's a pattern.

When a shared control starts collecting workflow-specific props, that's the signal to extract a Pattern wrapper instead of growing the base API.
