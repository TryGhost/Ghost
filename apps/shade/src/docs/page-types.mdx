...
**Analytics surfaces** — Post Analytics (in `apps/posts`) and Site Stats (in `apps/stats`) are visually rich pages with KPI strips, charts, and ranked content, and `PageHeader` has slots (`ContextStrip`, `Hero`, `Nav`) designed to support them. But they live in two apps that are scheduled to merge, and their page-level composition is intentionally kept inside those apps rather than promoted to a generic Ghost-wide page type. When the apps merge and the analytics surface stabilises, this taxonomy may grow back into an "Analytics page" entry — today it's scoped work, not a recurring shape new features need to identify with.
