  ...
  return (
    <tr className="border-b border-border-default last:border-b-0">
      <td className="py-2 pr-6">
        <code className="text-sm font-medium text-text-primary">{breakpoint.name}</code>
      </td>
      <td className="py-2 pr-6">
        <code className="text-2xs text-text-secondary">{breakpoint.cssVar}</code>
      </td>
      <td className="py-2">
        <code className="text-2xs text-text-tertiary">{value || '—'}</code>
      </td>
    </tr>
  );
}
...
export function BreakpointTable({ breakpoints }: { breakpoints: Breakpoint[] }) {
  return (
    <table className="w-full text-left">
      ...
      <tbody>
        {breakpoints.map((b) => (
          <BreakpointRow key={b.cssVar} breakpoint={b} />
        ))}
      </tbody>
    </table>
  );
