...
function SpacingRow({ step }: { step: Step }) {
  ...
  const widthValue = baseValue
    ? `calc(${baseValue} * ${step.multiplier})`
    : `calc(0.4rem * ${step.multiplier})`;
  return (
    <div className="flex items-center gap-4 border-b border-border-default py-3 last:border-b-0">
      ...
      <div className="h-3 rounded-sm bg-primary" style={{ width: widthValue }} aria-hidden />
  ...
  return (
    <div className="flex items-baseline gap-3 rounded-md border border-border-default bg-surface-elevated p-4">
      ...
      <span className="ml-auto text-2xs text-text-tertiary">
        Every Tailwind spacing utility multiplies this base. `p-4` = base × 4.
      </span>
    </div>
  );
}
