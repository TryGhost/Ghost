...
export function TypeScale({ steps }: { steps: Step[] }) {
  return (
    <section className="flex flex-col">
      {steps.map((step) => (
        <ScaleRow key={step.cssVar} step={step} />
      ))}
    </section>
  );
}
  ...
  return (
    <section className="flex flex-col gap-4">
      {families.map((f) => (
        <div
          key={f.cssVar}
          className="flex flex-col gap-2 border-b border-border-default pb-4 last:border-b-0"
        >
  ...
  return (
    <section className="grid gap-6 sm:grid-cols-2">
      {values.map((v) => (
        <div
          key={v.cssVar}
          className="flex flex-col gap-2 rounded-md border border-border-default p-4"
        >
          ...
          >
            Each token controls the vertical rhythm of body and heading text. The same paragraph
            rendered at different line-heights shows how the rhythm shifts.
          </p>
        </div>
      ))}
    </section>
