...
describe('search index', function () {
        ...
        posts: [
          {
            ...
            excerpt:
              'We are sitting by the pool and smashing out search features. Barcelona life at 59 is great!',
  ...
  test('searching handles CJK characters correctly', async () => {
        ...
        posts: [
          {
            ...
            excerpt:
              '要是系統發送電子報時遇到永久失敗的情形，English 該帳號將停止接收電子報 Regisztráljon fizetős fiókot  يتطابق  a  المثابرة كل يوم hozzásćzólások írásához あなたのリクエストはこのサイトの管理者に送信されます。Пријавете го овој коментар Dołączdo płatnej społeczności {{publication}}, by zaąćcąć komećantować. vietnamese: Yêu cầu nhà cung cấp dịch vụ email hỗ trợ bengali: নিউরো সার্জন',
    ...
    expect(searchResults.posts[0].url).toEqual(
      'http://localhost/ghost/visting-china-as-a-polyglot/',
    );
  ...
  test('unit tests for cjkEncoderPresetCodepoint', () => {
    ...
    expect(tokenizeCjkByCodePoint('将通过所有测试。')).toEqual([
      '将',
      '通',
      '过',
      '所',
