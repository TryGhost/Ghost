...
describe('App', function () {
  ...
  test('shows gift redemption success notification when popup is open on load', async () => {
    window.location = new URL(
      'http://example.com/?action=subscribe&success=true#/portal/account?giftRedemption=true',
    );
    ...
    expect(
      within(notificationFrame.contentDocument).getByText("Gift redeemed! You're all set."),
    ).toBeInTheDocument();
  });

  test('shows gift redemption error notification when popup is open on load', async () => {
    window.location = new URL(
      'http://example.com/?action=subscribe&success=false#/portal/account?giftRedemption=true',
    );
    ...
    expect(notificationFrame).toBeInTheDocument();
    expect(
      within(notificationFrame.contentDocument).getByText('Gift could not be redeemed'),
    ).toBeInTheDocument();
    expect(
      within(notificationFrame.contentDocument).getByText(
  ...
  test('prefers locale prop over site locale for i18n language', async () => {
    ...
    const utils = appRender(<App siteUrl="http://example.com" api={ghostApi} locale="en" />);
  ...
  test('parses retention offer preview query data into account cancellation flow', () => {
    const app = new App({ siteUrl: 'http://example.com' });
    const previewData = app.fetchOfferQueryStrData(
      'redemption_type=retention&display_title=Before%2520you%2520go&display_description=Please%2520stay&type=percent&amount=100&duration=repeating&duration_in_months=2&cadence=month&tier_id=product_123&enabled=false',
    );
  ...
  test('uses the selected tier price for retention offer preview members', () => {
    ...
    const app = new App({ siteUrl: 'http://example.com' });
    const previewData = app.fetchOfferQueryStrData(
      `redemption_type=retention&display_title=Stay&display_description=Please%2520stay&type=percent&amount=25&duration=forever&duration_in_months=0&cadence=year&tier_id=${paidProduct.id}`,
    );
