...
const setup = (overrides) => {
  const { mockDoActionFn, context, ...utils } = render(<AccountEmailPage />, {
    overrideContext: {
      ...overrides,
    },
  });
...
const getEmailPreferenceToggles = ({ queryAllByTestId }) => {
  return [...queryAllByTestId('newsletter-toggle'), ...queryAllByTestId('comment-toggle')];
};
...
describe('Account Email Page', () => {
  ...
  test('can unsubscribe from all emails', async () => {
    ...
    const { mockDoActionFn, unsubscribeAllBtn, container } = setup({
      site: siteData,
      member: getMemberData({ newsletters: newsletterData }),
    });
    ...
    expect(mockDoActionFn).toHaveBeenCalledWith('showPopupNotification', {
      action: 'updated:success',
      message: 'Unsubscribed from all emails.',
    });
    expect(mockDoActionFn).toHaveBeenLastCalledWith('updateNewsletterPreference', {
      newsletters: [],
  ...
  test('can update newsletter preferences', async () => {
    ...
    const { mockDoActionFn, getAllByTestId, container } = setup({
      site: siteData,
      member: getMemberData({ newsletters: newsletterData }),
    });
    let checkboxes = container.querySelectorAll('input[type="checkbox"]');
    ...
    fireEvent.click(subscriptionToggles[0]);
    expect(mockDoActionFn).toHaveBeenCalledWith('updateNewsletterPreference', {
      newsletters: [{ id: newsletterData[1].id }, { id: newsletterData[0].id }],
    });
  });
  ...
  test('can update comment notifications', async () => {
    ...
    const {
      mockDoActionFn: mockA,
      getAllByTestId: getA,
      unmount,
    } = setup({ site: siteData, member: memberOff });
    fireEvent.click(getA('switch-input')[0]);
    expect(mockA).toHaveBeenCalledWith('updateNewsletterPreference', {
      enableCommentNotifications: true,
    });
    unmount();

    const memberOn = { ...getMemberData(), enable_comment_notifications: true };
    const { mockDoActionFn: mockB, getAllByTestId: getB } = setup({
      site: siteData,
      member: memberOn,
    });
    fireEvent.click(getB('switch-input')[0]);
    expect(mockB).toHaveBeenCalledWith('updateNewsletterPreference', {
      enableCommentNotifications: false,
    });
  });
  ...
  test('displays help for members with email suppressions', async () => {
    ...
    const { getByText } = setup({
      ...
      member: getMemberData({
        newsletters: newsletterData,
        email_suppressions: { suppressed: false },
      }),
    });
  ...
  test('newsletters are not visible when newsletters are disabled on the site but has comments enabled', async () => {
    ...
    expect(unsubscribeBtns[0].textContent).toContain(
  ...
  test('newsletter row is keyboard-activatable and toggles the subscription', async () => {
    ...
    const { mockDoActionFn, getAllByTestId } = setup({
      site: siteData,
      member: getMemberData({ newsletters: newsletterData }),
    });
  ...
  test('clicking the inner switch does not double-fire the section toggle', async () => {
    ...
    const updateCalls = mockDoActionFn.mock.calls.filter(
      (call) => call[0] === 'updateNewsletterPreference',
    );
