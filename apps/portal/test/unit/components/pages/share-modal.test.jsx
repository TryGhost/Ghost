...
describe('ShareModal', () => {
  ...
  test('renders share actions and uses DOM metadata for social links', () => {
    ...
    expect(getByTestId('share-preview-image')).toHaveAttribute(
      'src',
      'https://example.com/post.jpg',
    );
    expect(getByTestId('share-preview-favicon')).toHaveAttribute(
      'src',
    ...
    expect(container.querySelector('.gh-portal-share-preview-meta')).not.toHaveTextContent(
      'Example site| Jane Doe',
    );
    ...
    expect(threadsUrl.searchParams.get('text')).toBe(
      'Example post title https://example.com/post?ref=test',
    );

    expect(blueskyUrl.origin + blueskyUrl.pathname).toBe('https://bsky.app/intent/compose');
    expect(blueskyUrl.searchParams.get('text')).toBe(
      'Example post title https://example.com/post?ref=test',
    );
  });
  ...
  test('falls back to twitter image when og image is missing', () => {
    ...
    expect(getByTestId('share-preview-image')).toHaveAttribute(
      'src',
      'https://canonical.example/twitter-image.jpg',
    );
  });
