...
const setupTest = (options = {}) => {
  const { otcRef = null, action = 'init:success', ...contextOverrides } = options;

  const { mockDoActionFn, ...utils } = render(<MagicLinkPage />, {
    overrideContext: {
      otcRef,
      action,
      ...contextOverrides,
...
describe('MagicLinkPage', () => {
  ...
  describe('OTC flow edge cases', () => {
    test('does not render form without otcRef', () => {
      ...
      expect(
        utils.queryByText(/You can also use the one-time code to sign in here/i),
