...
describe('error messages are set correctly', () => {
  ...
  test('handles 400 error without defaultMessage', async () => {
    ...
    expect(chooseBestErrorMessage(humanReadableError, null)).toEqual(
      'translated This is a 400 error',
    );
  });
  ...
  test('handles an error with defaultMessage not a special message', async () => {
    ...
    expect(chooseBestErrorMessage(humanReadableError, 'translated default message')).toEqual(
  ...
  test('handles an error with defaultMessage that is a special message', async () => {
    const error = new Response(
      '{"errors":[{"message":"Too many attempts try again in {number} minutes."}]}',
      { status: 400 },
    );
    const humanReadableError = await HumanReadableError.fromApiResponse(error);
    expect(chooseBestErrorMessage(humanReadableError, 'this is the default message')).toEqual(
      'translated Too many attempts try again in {number} minutes.',
    );
  });

  test('handles an error when the message has a number', async () => {
    const error = new Response(
      '{"errors":[{"message":"Too many attempts try again in 10 minutes."}]}',
      { status: 400 },
    );
    const humanReadableError = await HumanReadableError.fromApiResponse(error);
    expect(chooseBestErrorMessage(humanReadableError, 'this is the default message')).toEqual(
      'translated Too many attempts try again in 10 minutes.',
    );
  });
  ...
  test('handles a 500 error', async () => {
    ...
    const humanReadableError = await HumanReadableError.fromApiResponse(error);
    expect(chooseBestErrorMessage(humanReadableError, null)).toEqual(
      'translated A server error occurred',
    );
  });
  ...
  test('gets the magic link error message correctly', async () => {
    const error = new Response('{"errors":[{"message":"Failed to send magic link email"}]}', {
      status: 400,
    });
    const humanReadableError = await HumanReadableError.fromApiResponse(error);
    expect(chooseBestErrorMessage(humanReadableError, null)).toEqual(
      'translated Failed to send magic link email',
    );
  });
});
