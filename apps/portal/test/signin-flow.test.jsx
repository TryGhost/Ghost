...
describe('Signin', () => {
  describe('on single tier site', () => {
    ...
    test('with default settings', async () => {
      ...
      const description = await within(popupIframeDocument).findByText(
        /An email has been sent to jamie@example.com/i,
      );
    ...
    test('with inbox link', async () => {
      const { ghostApi, emailInput, popupIframeDocument, submitButton } = await setup({
        site: FixtureSite.singleTier.basic,
      });
...
describe('Signin', () => {
  ...
  describe('redirect parameter handling', () => {
    ...
    test('redirect parameter is not passed to sendMagicLink when pageData.redirect is not set', async () => {
      ...
      const lastCall =
        ghostApi.member.sendMagicLink.mock.calls[
          ghostApi.member.sendMagicLink.mock.calls.length - 1
        ][0];
...
describe('OTC Integration Flow', () => {
  ...
  test('OTC verification with invalid code shows error', async () => {
    ...
    const errorNotification =
      await within(popupIframeDocument).findByText(/Invalid verification code/i);
  ...
  test('OTC verification without redirectUrl shows default error', async () => {
    ...
    const errorNotification =
      await within(popupIframeDocument).findByText(/Failed to verify code/i);
    expect(errorNotification).toBeInTheDocument();
  });
  ...
  test('OTC verification with API error shows error message', async () => {
    ...
    const errorNotification = await within(popupIframeDocument).findByText(
      /Failed to verify code, please try again/i,
    );
    expect(errorNotification).toBeInTheDocument();
  });
