  ...
  describe('Valid feedback URL', () => {
    describe('Logged in', () => {
      test('Autosubmits feedback', async () => {
        Object.defineProperty(window, 'location', {
          value: new URL(
            `${siteData.url}/${postSlug}/#/feedback/${postId}/1/?uuid=${memberData.uuid}&key=key`,
          ),
          writable: true,
        });
      ...
      test('Requires confirmation w/o uuid or key params', async () => {
        ...
        expect(popupFrame).toBeInTheDocument();
        expect(
          within(popupIframeDocument).getByText('Give feedback on this post'),
        ).toBeInTheDocument();
    ...
    describe('Logged out', () => {
      ...
      test('Requires login without key', async () => {
        Object.defineProperty(window, 'location', {
          value: new URL(
            `${siteData.url}/${postSlug}/#/feedback/${postId}/1/?uuid=${memberData.uuid}`,
          ),
          writable: true,
        });
        const { ghostApi, popupFrame, popupIframeDocument } = await setup(siteData, null, true);
    ...
    test('Error on fail to submit', async () => {
      ...
      expect(
        within(popupIframeDocument).getByText(/There was a problem submitting your feedback/),
      ).toBeInTheDocument();
    });
  });
