...
describe('Signup', () => {
  describe('as free member on single tier site', () => {
    test('with default settings', async () => {
      ...
      const continueButton = within(popupIframeDocument).queryAllByRole('button', {
        name: 'Continue',
      });
    ...
    test('with inbox link', async () => {
      const { emailInput, nameInput, popupIframeDocument, chooseBtns } = await setup({
        site: FixtureSite.singleTier.basic,
      });
      ...
      const inboxLinkButton = await within(popupIframeDocument).findByRole('link', {
        name: /open proton mail/i,
      });
    ...
    test('hides inbox links on iOS', async () => {
      ...
      try {
        ...
        const inboxLinkButton = within(popupIframeDocument).queryByRole('link', {
          name: /open proton mail/i,
        });
    ...
    test('with only free plan', async () => {
      ...
      const freeProduct = FixtureSite.singleTier.onlyFreePlan.products.find(
        (p) => p.type === 'free',
      );
  ...
  describe('as paid member on single tier site', () => {
    test('with default settings on monthly plan', async () => {
      ...
      await waitFor(() => {
        expect(within(popupIframeDocument).queryAllByText(benefitText).length).toBeGreaterThan(0);
      });
      expect(emailInput).toHaveValue('jamie@example.com');
      expect(nameInput).toHaveValue('Jamie Larsen');
      fireEvent.click(submitButton);
    ...
    test('with only paid plans available', async () => {
      ...
      const submitButton = within(popupIframeDocument).queryAllByRole('button', {
        name: 'Continue',
      });
    ...
    test('via direct signup link shows an error instead of the magic link page', async () => {
      ...
      try {
        const checkoutError = Object.assign(new Error('A subscription exists for this Member.'), {
        ...
        await waitFor(() => {
          expect(
            within(popupFrame.contentDocument).queryByText(
              /You already have an active subscription\./i,
            ),
          ).toBeInTheDocument();
        });
        expect(
          within(popupFrame.contentDocument).queryByText(/Now check your email!/i),
  ...
  describe('as free member on multi tier site', () => {
    ...
    test('with only free plan available', async () => {
      ...
      const freeProduct = FixtureSite.multipleTiers.onlyFreePlan.products.find(
        (p) => p.type === 'free',
      );
      const benefitText = freeProduct.benefits[0].name;
