...
const setup = async ({ site, member = null, newsletters }, loggedOut = false) => {
  ...
  ghostApi.member.updateNewsletters = vi.fn(
    ({ uuid: memberUuid, newsletters: newNewsletters, enableCommentNotifications }) => {
      return Promise.resolve({
        uuid: memberUuid,
        newsletters: newNewsletters,
  ...
  const triggerButton = within(triggerButtonFrame.contentDocument).getByTestId(
...
describe('Newsletter Subscriptions', () => {
  ...
  test('toggle subscribing to a newsletter', async () => {
    ...
    const expectedSubscriptions = Newsletters.filter((n) => n.id !== Newsletters[0].id).map(
      (n) => ({ id: n.id }),
    );
    expect(ghostApi.member.update).toHaveBeenLastCalledWith({ newsletters: expectedSubscriptions });
  ...
  test('unsubscribe from all newsletters when logged in', async () => {
    ...
    expect(manageSubscriptionsButton).toBeInTheDocument();
    await userEvent.click(manageSubscriptionsButton);
    const unsubscribeAllButton = within(popupIframeDocument).queryByRole('button', {
  ...
  describe('from the unsubscribe link > UnsubscribePage', () => {
    test('unsubscribe via email link while not logged in', async () => {
      ...
      Object.defineProperty(window, 'location', {
        value: new URL(
          `https://portal.localhost/?action=unsubscribe&uuid=${FixtureMember.subbedToNewsletter.uuid}&newsletter=${Newsletters[0].uuid}&key=hashedMemberUuid`,
        ),
        writable: true,
      });
    ...
    test('unsubscribe via email link while logged in', async () => {
      ...
      const { ghostApi, popupFrame, popupIframeDocument, triggerButton, queryByTitle } =
        await setup({
          site: FixtureSite.singleTier.onlyFreePlanWithoutStripe,
          member: FixtureMember.subbedToNewsletter,
          newsletters: Newsletters,
    ...
    test('unsubscribe link without a key param', async () => {
      ...
      Object.defineProperty(window, 'location', {
        value: new URL(
          `https://portal.localhost/?action=unsubscribe&uuid=${FixtureMember.subbedToNewsletter.uuid}&newsletter=${Newsletters[0].uuid}`,
        ),
        writable: true,
      });
