...
describe('Logged-in free member', () => {
  describe('can upgrade on single tier site', () => {
    ...
    test('to an offer via link with portal disabled', async () => {
      window.location.hash = `#/portal/offers/${FixtureOffer.id}`;
      const { ghostApi, popupFrame, triggerButtonFrame, submitButton, offerDescription } =
        await offerSetup({
  ...
  describe('can upgrade on multi tier site', () => {
    ...
    test('to an offer via link', async () => {
      window.location.hash = `#/portal/offers/${FixtureOffer.id}`;
      const { ghostApi, popupFrame, emailInput, nameInput, submitButton, offerDescription } =
        await offerSetup({
          site: FixtureSite.multipleTiers.basic,
          member: FixtureMember.altFree,
          offer: FixtureOffer,
...
describe('Logged-in complimentary member', () => {
  describe('can upgrade on single tier site', () => {
    test('with default settings on monthly plan', async () => {
      const { ghostApi, popupFrame, triggerButtonFrame, popupIframeDocument, accountHomeTitle } =
        await setup({
          site: FixtureSite.singleTier.basic,
      ...
      const changePlanButton = within(popupIframeDocument).queryByRole('button', {
        name: 'Change',
      });
