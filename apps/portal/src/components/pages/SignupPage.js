...
function SignupPage() {
    const {member, site, action, brandColor, pageQuery, doAction} = useContext(AppContext);
    ...
    // Handle the default plan selection
    useEffect(() => {
        const prices = getSitePrices({site, pageQuery});
        const selectedPriceId = getSelectedPriceId(prices, plan);
        if (selectedPriceId !== plan) {
            setPlan(selectedPriceId);
        }
    }, [site, pageQuery, plan]);
    ...
    const getFormErrors = () => {
        ...
        const checkboxError = checkboxRequired && !termsCheckboxChecked;

        return {
            ...ValidateInputForm({fields: getInputFields()}),
    ...
    const doSignup = () => {
        ...
        const hasOnlyCheckboxError = validationErrors?.checkbox && Object.values(otherErrors).every(error => !error);
        ...
        if (!hasFormErrors) {
            if (hasMultipleNewsletters({site})) {
                ...
                setPageData({name, email, plan, phonenumber, token});
                setErrors({});
            } else {
                setErrors({});
    ...
    const handleChooseSignup = (e, planToSelect) => {
        e.preventDefault();
        setPlan(planToSelect);

        // Perform validation and signup immediately with the planToSelect parameter
        const validationErrors = getFormErrors();
        setErrors(validationErrors);
        ...
        if (!hasFormErrors) {
            if (hasMultipleNewsletters({site})) {
                setShowNewsletterSelection(true);
                setPageData({name, email, plan: planToSelect, phonenumber, token});
                setErrors({});
            } else {
                setErrors({});
                doAction('signup', {name, email, phonenumber, plan: planToSelect, token});
            }
        }
    };
    ...
    const handleSelectPlan = (e, priceId) => {
        e && e.preventDefault();
    ...
    const renderProducts = () => {
        ...
        return (
            <>
                <ProductsSection
                    handleChooseSignup={(...args) => handleChooseSignup(...args)}
                    products={products}
    ...
    const renderForm = () => {
        ...
        if (!isSignupAllowed({site}) || !hasAvailablePrices({site, pageQuery})) {
            if (!isSigninAllowed({site})) {
                return renderMembersDisabledMessage();
            }
    ...
    const renderPaidMembersOnlyMessage = () => {
        return (
            <section>
                <div className='gh-portal-section'>
                    <p
    ...
    const renderMembersDisabledMessage = () => {
        return (
            <section>
                <div className='gh-portal-section'>
                    <p
    ...
    const getClassNames = () => {
        ...
        let {sectionClass} = getClassNames();
        return (
            <>
