...
export function removePortalLinkFromUrl() {
  ...
  if (
    path &&
    (portalLinkRegex.test(path) || giftRedemptionLinkRegex.test(path) || shareLinkRegex.test(path))
  ) {
...
export function isSignupAllowed({ site }) {
  const hasSignupAccess =
    site?.members_signup_access === 'all' || site?.members_signup_access === 'paid';
...
export const getSupportAddress = ({ site }) => {
  const { members_support_address: oldSupportAddress, support_email_address: supportAddress } =
    site || {};
