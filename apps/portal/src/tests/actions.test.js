...
describe('verifyOTC action', () => {
    ...
    describe('with membersSigninOTCAlpha flag enabled', () => {
        ...
        test('returns actionErrorMessage when verification fails without redirectUrl', async () => {
            ...
            expect(typeof result.actionErrorMessage).toBe('string');
        ...
        test('passes redirect parameter to verifyOTC API call', async () => {
            const mockApi = {
                member: {
                    getIntegrityToken: vi.fn(() => Promise.resolve('token-123')),
                    verifyOTC: vi.fn(() => Promise.resolve({
        ...
        test('includes integrity token in all API calls', async () => {
            const mockApi = {
                member: {
                    getIntegrityToken: vi.fn(() => Promise.resolve('integrity-token-xyz')),
                    verifyOTC: vi.fn(() => Promise.resolve({
                        redirectUrl: 'https://example.com/success'
                    }))
                }
            ...
            expect(mockApi.member.getIntegrityToken).toHaveBeenCalled();
            expect(mockApi.member.verifyOTC).toHaveBeenCalledWith(
                expect.objectContaining({
                    integrityToken: 'integrity-token-xyz'
                })
            );
        });
    });
    ...
    describe('edge cases', () => {
        ...
        test('handles missing labs configuration', async () => {
            const mockApi = {
                member: {
                    getIntegrityToken: vi.fn(() => Promise.resolve('token-123')),
                    verifyOTC: vi.fn(() => Promise.resolve({
