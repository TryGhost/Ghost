...
function getSiteData() {
    ...
    if (scriptTag) {
        ...
        return {siteUrl, apiKey, apiUrl, locale};
    }
    return {};
}
...
function init() {
    ...
    const {siteUrl: customSiteUrl, apiKey, apiUrl, locale} = getSiteData();
    const siteUrl = customSiteUrl || window.location.origin;
    setup({siteUrl});
    ReactDOM.render(
        <React.StrictMode>
            <App siteUrl={siteUrl} customSiteUrl={customSiteUrl} apiKey={apiKey} apiUrl={apiUrl} locale={locale}/>
        </React.StrictMode>,
