...
describe('Unit: Route: tag', function () {
    ...
    // own instantiation.
    function setupRoute(owner, {routeName = 'route:tag'} = {}) {
    ...
    it('aborts the Ember transition because React owns the tag detail route', function () {
        const {route, router, navigate, requireAuthentication} = setupRoute(this.owner);
    ...
    it('writes the React URL for a named transition', function () {
        const {route, router, navigate} = setupRoute(this.owner);
        const transition = {
            abort: sinon.spy(),
            intent: {},
            to: {name: 'tag', params: {tag_slug: 'my-tag'}}
    ...
    it('writes the create URL for a named transition into tag.new', function () {
        const {route, router, navigate} = setupRoute(this.owner, {routeName: 'route:tag.new'});
    ...
    it('restores the URL together with the history state react-router keeps', function () {
        const {route, replaceState} = setupRoute(this.owner);
