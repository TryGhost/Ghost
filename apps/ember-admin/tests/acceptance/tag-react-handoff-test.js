...
// React owns /tags/:slug. Ember's side of that handshake is the tag route's
// beforeModel: it aborts so the Ember tag editor stays unrendered, and drives
// window.location.hash so navigations Ember itself starts (the Cmd-K search
// modal) still land somewhere — an aborted transition never reaches updateURL,
// and the two apps share the hash.

// `visit()` rejects with TransitionAborted whenever the route aborts, which is
// the whole point of the handoff. Swallow only that rejection so the
...
describe('Acceptance: tag React handoff', function () {
