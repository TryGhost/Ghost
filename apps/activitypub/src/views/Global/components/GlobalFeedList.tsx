...
import Reader from '@views/Inbox/components/Reader';
...
export type GlobalFeedListProps = {
    isLoading: boolean,
    activities: Activity[],
    fetchNextPage: () => void,
    hasNextPage: boolean,
...
const GlobalFeedList:React.FC<GlobalFeedListProps> = ({
    isLoading,
    activities,
    fetchNextPage,
    hasNextPage,
                                            ...
                                            {activities.map((activity, index) => (
                                                ...
                                                >
                                                    <FeedItem
                                                        ...
                                                        type={activity.type}
                                                        onClick={() => {
                                                            navigate(`/global/${encodeURIComponent(activity.id)}`);
                                                        }}
                                                    />
                        ...
                        <div className='flex w-full flex-col items-center gap-10'>
                            <div className='flex w-full max-w-[620px] flex-col items-center'>
                                <EmptyViewIndicator>
                                    ...
                                    <div>The Global Feed shows <span className='text-black dark:text-white'>long-form posts from all Ghost publishers</span>. It&apos;s empty for now, but posts will appear as Ghost publishers share content.</div>
            ...
            >
                <DialogContent className='inset-y-3 h-[calc(100vh-24px)] w-screen max-w-[calc(100vw-24px)] !animate-none p-0 focus:outline-none dark:bg-gray-950'>
                    <DialogHeader className='hidden'>
                        <DialogTitle>Global Feed</DialogTitle>
                        <DialogDescription>Global feed containing long form articles from all Ghost publishers</DialogDescription>
                    </DialogHeader>
