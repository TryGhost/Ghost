...
const NotificationIcon: React.FC<NotificationIconProps> = ({
  ...
  switch (notificationType) {
    case 'follow':
      icon = (
        <LucideIcon.UserRoundCheck
          className={`-mt-0.5 -mr-0.5 ${size === 'sm' && 'size-[11px]'}`}
          color={iconColor}
          size={iconSize}
    ...
    case 'like':
      icon = (
        <LucideIcon.Heart
          className={`${size === 'sm' ? 'size-[11px]' : 'mt-px size-5'}`}
          color={iconColor}
          strokeWidth={strokeWidth}
        />
      );
    ...
    case 'mention':
      icon = (
        <LucideIcon.AtSign
          className={`${size === 'sm' ? 'size-[12px]' : 'size-5'}`}
          color={iconColor}
          size={iconSize}
          strokeWidth={strokeWidth}
        />
  ...
  return (
    <div
      className={`flex ${size === 'sm' ? 'size-5' : 'size-9'} items-center justify-center rounded-full ${badgeColor} ${className && className}`}
    >
      {icon}
    </div>
  );
