...
export const ExploreProfile: React.FC<
  ExploreProfileProps & {
            ...
            <div className="flex gap-3">
              ...
              <div className="break-anywhere -mt-0.5 flex grow flex-col">
                <span className="line-clamp-1 font-semibold text-black dark:text-white">
                  {!isLoading ? profile.name : <Skeleton className="w-full max-w-48" />}
                </span>
                <span className="line-clamp-1 text-md text-gray-700 dark:text-gray-600">
                  {!isLoading ? profile.handle : <Skeleton className="w-32" />}
                </span>
              </div>
            </div>
            ...
            profile.bio && (
              <div
                dangerouslySetInnerHTML={{
                  __html: sanitizeHtml(openLinksInNewTab(stripHtml(profile.bio, ['a', 'br']))),
                }}
...
const Explore: React.FC = () => {
  ...
  const { exploreProfilesQuery, updateExploreProfile } = useExploreProfilesForUserByTopic(
    'index',
    topic,
  );
  const {
      ...
      {!isExplainerClosed && (
        <div className="relative mb-6 flex items-start gap-1 rounded-md bg-gradient-to-r from-[#CFB0FF66] to-[#B6E8FF66] p-4 pr-10 dark:from-[#CFB0FF20] dark:to-[#B6E8FF20]">
          ...
          <Button
            ...
            variant="link"
            onClick={() => setExplainerClosed(true)}
          >
