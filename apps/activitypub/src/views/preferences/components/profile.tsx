...
const ProfileCard: React.FC<ProfileCardProps> = memo(
  ({
    ...
    const bannerImageSrc =
      isScreenshot && bannerDataUrl ? bannerDataUrl : account?.bannerImageUrl || coverImage;
    const avatarImageSrc =
      isScreenshot && avatarDataUrl ? avatarDataUrl : account?.avatarUrl || publicationIcon;

    return (
      <div
        className={`relative z-20 flex flex-col ${margin} ${cardWidth} ${cardHeight} rounded-[32px] ${borderClass} ${format === 'square' ? 'flex flex-col' : ''}`}
      ...
      >
        <div className="relative h-48 p-2">
          {bannerImageSrc ? (
            ...
            <div
              ...
              style={{
                background: `linear-gradient(to bottom, ${hexToRgba(backgroundColor === 'accent' ? '#ffffff' : accentColor || '#15171a', 1)}, ${hexToRgba(backgroundColor === 'accent' ? '#ffffff' : accentColor || '#15171a', 0.5)})`,
              }}
            >
              <DotsPattern
                className="absolute"
          ...
          {avatarImageSrc && (
            <div
              className="absolute bottom-0 left-1/2 -mb-8 -translate-x-1/2 rounded-full border-8 [&_img]:size-16! [&>div]:size-16!"
        ...
        <div
          className={`flex grow flex-col items-center p-6 ${account?.avatarUrl || publicationIcon ? 'pt-9' : 'pt-3'} text-center ${format === 'square' ? 'flex-1 justify-center' : ''}`}
        >
          <H2 className={`${isScreenshot && 'tracking-normal'}`} style={{ color: textColor }}>
            {!isLoading ? account?.name : <Skeleton className="w-32" />}
          </H2>
          <span
            className={`mt-1.5 leading-tight ${isScreenshot && 'tracking-normal'}`}
          ...
          <div
            ...
            style={{
              ...
              background: accentColor
                ? `linear-gradient(to top right, ${hexToRgba(backgroundColor === 'accent' ? '#ffffff' : accentColor, backgroundColor === 'dark' ? 0.12 : 0.04)}, ${hexToRgba(backgroundColor === 'accent' ? '#ffffff' : accentColor, backgroundColor === 'dark' ? 0.48 : 0.16)})`
                : undefined,
            }}
          >
      ...
      <div className="flex flex-col gap-5">
        ...
        <div className="relative flex flex-col items-center overflow-hidden rounded-2xl bg-gray-50">
            ...
            <div className="flex items-center gap-2">
              ...
              >
                <svg fill="none" viewBox="0 0 40 40">
                  ...
                  <path
                    className="social-facebook_svg__fb"
          ...
          {(account?.bannerImageUrl || coverImage) && (
            <DotsPattern
              className={`absolute top-1/2 left-1/2 h-[600px] w-[598px] -translate-1/2 ${backgroundColor === 'dark' && 'z-10'}`}
        ...
        >
          ...
          {(account?.bannerImageUrl || coverImage) && (
            <DotsPattern
              className={`absolute top-[-44px] left-[-62.5px] h-[600px] w-[598px] ${backgroundColor === 'dark' && 'z-10'}`}
              style={{ color: getDotsPatternColor() }}
            />
          )}
          <div
          ...
          />
          <img
            className="absolute top-12 left-1/2 mt-0.5 max-w-none -translate-x-1/2"
