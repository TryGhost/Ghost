...
import {
  useAccountDomainForUser,
  useAccountForUser,
  useUpdateAccountDomainMutationForUser,
  useUpdateAccountMutationForUser,
  useValidateAccountDomainMutationForUser,
...
const Domain: React.FC = () => {
  ...
  const showInstructions = Boolean(
    isAddingDomain && normalizedDomain && (!activeDomain || validatedDomain === normalizedDomain),
  );
  const canAdd = Boolean(isEditingDomain && normalizedDomain && !isAddingDomain);
  const canValidate = Boolean(
    showInstructions && normalizedDomain && normalizedDomain !== validatedDomain,
  );
  const canSave = Boolean(validatedDomain && normalizedDomain === validatedDomain);
  const redirectSource = normalizedDomain
    ? `https://${normalizedDomain}/.well-known/webfinger`
    : '';
  ...
  const previewHandleParts = {
    ...
    domain:
      (isEditingDomain || isAddingDomain) && domainInput
        ? (normalizedDomain ?? domainInput.trim())
        : handleParts.domain,
  };
  const previewHandle = `@${previewHandleParts.username}@${previewHandleParts.domain}`;
  const profileImageUrl = getImageUrl(
    account?.avatarUrl || siteData?.site?.icon,
    siteData?.site?.url,
  );
  ...
  const handleAdd = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const formDomain = normalizeDomainInput(
      new FormData(event.currentTarget).get('domain')?.toString() ?? '',
    );
            ...
            <Field>
              {isLoadingDomain ? (
                ...
                <p className="text-sm text-muted-foreground">
                  Could not load your Social Web handle.
                </p>
              ) : (
              ...
              <Inline className="flex-col items-stretch sm:flex-row sm:items-center" gap="md">
                ...
                {isEditingUsername ? (
                  <Inline gap="md" justify="end">
                    ...
                    <Button
                      className="h-9 text-sm sm:w-auto"
                      disabled={isUsernameDisabled || !canSaveUsername}
                      type="submit"
                    >
                      {isUpdatingAccount ? (
                        <>
        ...
        <div className="mt-10">
          {activeDomain && !validatedDomain ? (
            <Field data-invalid={fieldError ? true : undefined}>
              ...
              {fieldError && <FieldError id="social-web-domain-error">{fieldError}</FieldError>}
            </Field>
          ) : (
            <form onSubmit={handleAdd}>
                  ...
                  {!showInstructions &&
                    (isEditingDomain ? (
                      <Inline gap="md" justify="end">
                        ...
                        <Button
                          className="h-9 text-sm sm:w-auto"
                          disabled={isDisabled || !canAdd || hasDomainLoadError}
                          type="submit"
                        >
                          Activate
                        </Button>
                ...
                <div className="mt-6 rounded-md border border-border-default bg-surface-elevated p-4">
                  ...
                  <div className="mt-4 flex flex-col gap-3 text-sm">
                    <div>
                      ...
                      <code className="mt-1 block overflow-x-auto rounded-sm bg-background px-3 py-2 text-foreground">
                        {redirectSource}
                      </code>
                    </div>
                    <div>
              ...
              {showInstructions && (
                ...
                >
                  ...
                  <Inline gap="md" justify="end">
                    <Button disabled={isDisabled} variant="outline" onClick={handleCancel}>
                      Cancel
                    </Button>
                    {canSave ? (
                      ...
                      <Button
                        disabled={isDisabled || !canValidate || hasDomainLoadError}
