...
const PostsTab: React.FC<{ handle: string }> = ({ handle }) => {
  ...
  const posts =
    data?.pages.flatMap((page: { posts: Activity[] }) => page.posts) ??
  ...
  return (
    <Posts
      ...
      isLoading={isLoading}
      noResultsMessage={
        handle ? `${handle} hasn't posted anything yet` : `You haven't posted anything yet.`
      }
      posts={posts}
    />
  );
...
const FollowingTab: React.FC<{ handle: string }> = ({ handle }) => {
  ...
  const { data, fetchNextPage, hasNextPage, isFetchingNextPage, isLoading } = accountQuery;
...
const Profile: React.FC<ProfileProps> = () => {
  ...
  const {
    ...
    error: accountError,
    refetch,
  } = useAccountForUser('index', params.handle || 'me');
