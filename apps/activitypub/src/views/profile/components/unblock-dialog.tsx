...
const UnblockDialog: React.FC<UnblockDialogProps> = ({
  ...
  const renderDualView = () => (
    <>
      <AlertDialogHeader>
        <AlertDialogTitle className="mb-1 flex flex-col gap-1">Unblock</AlertDialogTitle>
          ...
          <div className="flex flex-col rounded-md border">
            <div className="flex justify-between gap-6 p-5">
              <div className="flex flex-col gap-1">
                <H4>Unblock user</H4>
                <p>
                  <span className="font-semibold text-black">{handle}</span> will be able to follow
                  you and engage with your public posts.
                </p>
              </div>
              <Button
                className={`gap-1 ${dialogState.userUnblocked ? 'pointer-events-none border-green bg-green text-white hover:bg-green hover:text-white' : 'text-red hover:text-red-400'}`}
                variant="outline"
            ...
            <div className="flex justify-between gap-6 p-5">
              <div className="flex flex-col gap-1">
                ...
                <p>
                  Users from <span className="font-semibold text-black">{domain}</span> will be able
                  to follow you and engage with your public posts.
                </p>
              </div>
              <Button
                className={`gap-1 ${dialogState.domainUnblocked ? 'pointer-events-none border-green bg-green text-white hover:bg-green hover:text-white' : 'text-red hover:text-red-400'}`}
                variant="outline"
    ...
    return (
      <>
        ...
        <AlertDialogFooter>
          ...
          <Button onClick={isUserBlock ? handleUnblock : handleDomainUnblock}>Unblock</Button>
        </AlertDialogFooter>
      </>
    );
  };
