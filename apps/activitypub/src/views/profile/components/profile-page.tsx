...
import {
  useAccountForUser,
  useBlockDomainMutationForUser,
  useBlockMutationForUser,
  useUnblockDomainMutationForUser,
  useUnblockMutationForUser,
...
const ProfilePage: React.FC<ProfilePageProps> = ({
  ...
  const tabSlug = params.handle ? params.tab || '' : location.pathname.split('/').pop() || '';

  const allowedTabs = useMemo<ProfileTab[]>(
    () => (likesEnabled ? ['likes', 'following', 'followers'] : ['following', 'followers']),
    [likesEnabled],
  );
  const activeTab: ProfileTab = allowedTabs.includes(tabSlug as ProfileTab)
    ? (tabSlug as ProfileTab)
    : 'posts';
      ...
      <div className="z-0 mx-[max(-4vw,-24px)] -mt-9 flex flex-col items-center pb-16">
        <div className="mx-auto w-full">
          <>
            ...
            <div
              className={`mx-auto max-w-[620px] px-6 ${!account?.bannerImageUrl && !canGoBack ? '-mt-8' : '-mt-12'}`}
            >
              <div className="flex items-end justify-between">
                ...
                {!isCurrentUser && !isLoadingAccount && (
                  <div className="flex gap-2">
                    ...
                    >
                      <Button aria-label="Open profile menu" variant="outline">
                        <LucideIcon.Ellipsis />
                      </Button>
                    </ProfileMenu>
                  </div>
              ...
              <H2 className="break-anywhere mt-4 truncate">
                {!isLoadingAccount ? account?.name : <Skeleton className="w-32" />}
              </H2>
              <div className="mb-4 flex items-center gap-2">
                ...
                >
                  <span className="truncate">
                    {!isLoadingAccount ? account?.handle : <Skeleton className="w-full max-w-56" />}
                  </span>
                </a>
                {!isLoadingAccount && (
                  <Button
                ...
                {account?.followsMe && !isLoadingAccount && (
                  <Badge className="mt-px whitespace-nowrap" variant="secondary">
                    Follows you
                  </Badge>
                )}
              </div>
              {(account?.bio || customFields?.length > 0) && (
                <div
                  ref={contentRef}
                  className={`ap-profile-content break-anywhere relative text-[1.5rem] [&>p]:mb-3 ${isExpanded ? 'max-h-none pb-7' : 'max-h-[160px] overflow-hidden'} relative`}
                >
                  {!isLoadingAccount ? (
                    <div
                      dangerouslySetInnerHTML={{
                        __html: sanitizeHtml(
                          openLinksInNewTab(stripHtml(account?.bio ?? '', ['a', 'br'])),
                        ),
                      }}
                    />
                  ) : (
                  ...
                  {customFields?.map((attachment: { name: string; value: string }) => (
                    ...
                    >
                      ...
                      <span
                        dangerouslySetInnerHTML={{ __html: sanitizeHtml(attachment.value) }}
              ...
              >
                <TabsList>
                  ...
                  {likesEnabled && (
                    <TabsTrigger value="likes">
                      Likes
                      <TabsTriggerCount>
                        {abbreviateNumber(account?.likedCount || 0)}
                      </TabsTriggerCount>
                    </TabsTrigger>
                  )}
                  <TabsTrigger value="following">
                    Following
                    <TabsTriggerCount>
                      {abbreviateNumber(account?.followingCount || 0)}
                    </TabsTriggerCount>
                  </TabsTrigger>
                  <TabsTrigger value="followers">
                    Followers
                    <TabsTriggerCount>
                      {abbreviateNumber(account?.followerCount || 0)}
                    </TabsTriggerCount>
                  </TabsTrigger>
                </TabsList>
                ...
                {likesEnabled && <TabsContent value="likes">{likesTab}</TabsContent>}
                <TabsContent value="following">{followingTab}</TabsContent>
