            ...
            <div className={`mx-auto px-8 pt-5 pb-10 max-lg:px-0`}>
              {!threadParents.length && (
                <ProfilePreviewHoverCard
                  ...
                  isCurrentUser={currentPost.object.authored}
                >
                  <div
                    className={`col-[2/3] mx-auto flex w-full cursor-pointer items-center gap-3 ${canGoBack ? 'pt-10 max-md:pt-5' : 'pt-5'}`}
                  >
                    <div className="relative z-10">
                      <APAvatar
                        author={currentPost.actor}
                        showFollowButton={
                          !currentPost.object.authored && !currentPost.actor.followedByMe
                        }
                      />
                    </div>
                    <div
                    ...
                    >
                      <div className="flex w-full">
                        <span className="min-w-0 truncate font-semibold whitespace-nowrap hover:underline">
                          {currentPost.actor.name}
                        </span>
                      </div>
                      <div className="flex w-full">
                        <span className='truncate text-gray-700 after:mx-1 after:font-normal after:text-gray-700 after:content-["·"]'>
                          {getHandle(currentPost.actor)}
                        </span>
              ...
              {threadParents.map((item) => {
                return item.object.type === 'Tombstone' ? (
                  ...
                  <FeedItem
                    ...
                    onClick={() => {
                      navigate(
                        `/${item.object.type === 'Article' ? 'reader' : 'notes'}/${encodeURIComponent(item.object.id)}`,
                      );
                    }}
                  />
                );
              ...
              <div
                ref={postRef}
                className={`${canGoBack ? 'scroll-mt-[12px]' : 'scroll-mt-[124px]'}`}
              >
