...
import FeedItem, {
  ...
  getAttachment,
} from '@src/components/feed/feed-item';
...
} from '@src/utils/content-formatters';
...
const MEDIA_ELEMENT_SELECTOR =
  'audio, canvas, embed, iframe, img, object, picture, source, svg, video';
...
const ArticleBody: React.FC<{
  ...
  const darkMode =
    (document.documentElement.classList.contains('dark') && backgroundColor === 'SYSTEM') ||
    backgroundColor === 'DARK';

  const cssContent = articleBodyStyles();
  const shouldEnforceVideoCardInlinePlayback =
    typeof window !== 'undefined' &&
    typeof window.matchMedia === 'function' &&
    window.matchMedia('(hover: none) and (pointer: coarse)').matches;

  const articleHtml = useMemo(() => {
    const transformedHtml = shouldEnforceVideoCardInlinePlayback
      ? enforceVideoCardInlinePlayback(html)
      : html;
  ...
  useEffect(() => {
    ...
    const handleLoad = () => {
      ...
      const highestLevel = Math.min(...headingElements.map((el) => parseInt(el.tagName[1])));
...
const FeedItemDivider: React.FC = () => <div className="h-px bg-black/[8%] dark:bg-gray-950"></div>;
...
export const Reader: React.FC<ReaderProps> = ({ postId = null, onClose }) => {
  const {
    backgroundColor,
  ...
  const {
    ...
    hasMedia:
      ...
      (object ? getAttachment(object) !== null : false) ||
      articleHtmlHasMedia,
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      const totalHeight =
        (article as HTMLElement).offsetHeight - (container as HTMLElement).offsetHeight;
  ...
  if (isLoadingContent) {
    return (
      <div
        className={`max-h-full overflow-auto rounded-md ${backgroundColor === 'DARK' && 'dark'} ${(backgroundColor === 'LIGHT' || backgroundColor === 'SEPIA') && 'light'} ${COLOR_OPTIONS[backgroundColor].background}`}
      >
        <div className="flex h-full flex-col">
          <div className="relative flex-1">
            <div
              className={`sticky top-0 z-50 flex h-[102px] items-center justify-center rounded-t-md border-b max-md:h-[68px] ${COLOR_OPTIONS[backgroundColor].background} ${COLOR_OPTIONS[backgroundColor].border}`}
            >
              <div
                    ...
                    <div className="flex cursor-pointer items-center gap-3">
                      ...
                      >
                        <div className="flex w-full">
                          <span className="min-w-0 truncate font-semibold whitespace-nowrap text-black hover:underline dark:text-white">
                            {isLoadingContent ? <Skeleton className="w-20" /> : actor.name}
                          </span>
                        </div>
                        <div className="flex w-full">
                          {!isLoadingContent && (
                            <span className='truncate text-gray-700 after:mx-1 after:font-normal after:text-gray-700 after:content-["·"]'>
                              {getHandle(actor)}
                            </span>
                          )}
              ...
              {!isLoadingContent && (
                <div className="grow overflow-y-auto">
                  <div className={`mx-auto px-6 pt-5 pb-10`} style={{ maxWidth: currentMaxWidth }}>
                    ...
                    {object.type === 'Tombstone' && <DeletedFeedItem last={true} />}

                    <div
                      className="mx-auto w-full border-t border-black/[8%] dark:border-gray-950"
                      style={{ maxWidth: currentGridWidth }}
                    >
                        ...
                        return (
                          <React.Fragment key={replyGroup.mainReply.id}>
                            ...
                            {hasChain &&
                              ...
                              replyGroup.chain
                                .slice(1)
                                .map((chainItem: Activity, chainIndex: number) => {
                                  const isLastChainItem =
                                    chainIndex === replyGroup.chain.slice(1).length - 1;
                                  const hasMoreReplies =
                                    hasMoreChildReplies && hasMoreChildReplies(groupIndex);
                            ...
                            {hasChain &&
                              ...
                              isFullyExpanded &&
                              hasMoreChildReplies &&
                              hasMoreChildReplies(groupIndex) && (
                                <ShowRepliesButton
