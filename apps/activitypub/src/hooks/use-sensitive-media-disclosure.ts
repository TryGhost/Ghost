...
export function useSensitiveMediaDisclosure({
  ...
  const [contentWarningMinHeight, setContentWarningMinHeight] = useState<number | undefined>(
    undefined,
  );
  ...
  const isSensitiveMediaGated =
    hasSensitiveMedia && !hasContentWarning && !showSensitiveMediaByDefault;
  const shouldHideSensitiveMedia =
    isSensitiveMediaGated && (isSensitiveMediaManuallyHidden || !isSensitiveMediaRevealed);
