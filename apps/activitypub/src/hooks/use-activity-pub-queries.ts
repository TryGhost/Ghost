...
} from '../utils/pending-activity';
import { mapPostToActivity } from '../utils/posts';
import { toast } from 'sonner';
import { useCallback, useEffect, useState } from 'react';

export type ActivityPubCollectionQueryResult<TData> = UseInfiniteQueryResult<
  ActivityPubCollectionResponse<TData>
>;
              ...
              if (item.object.id === id) {
                return {
                  ...
                  object: {
                    ...
                    liked: liked,
                    likeCount: Math.max(
                      liked ? (item.object.likeCount || 0) + 1 : (item.object.likeCount || 0) - 1,
                      0,
                    ),
                  },
                };
...
function updateAccountFollowsCaches(
  queryClient: QueryClient,
  targetHandle: string,
  isFollowing: boolean,
) {
...
function updateNotificationsLikedCache(
  queryClient: QueryClient,
  handle: string,
  id: string,
  liked: boolean,
...
function updateNotificationsRepostCache(
  queryClient: QueryClient,
  handle: string,
  id: string,
  reposted: boolean,
                  ...
                  return {
                    ...
                    post: {
                      ...
                      repostCount: Math.max(
                        reposted
                          ? notification.post.repostCount + 1
                          : notification.post.repostCount - 1,
                        0,
                      ),
                    },
...
function updateNotificationsReplyCountCache(
  queryClient: QueryClient,
  handle: string,
  id: string,
  delta: number,
...
function updateReplyCacheOnce(queryClient: QueryClient, id: string, delta: number) {
  ...
  const replyChainQueryKey = QUERY_KEYS.replyChain(null);
  queryClient.setQueriesData(
    { queryKey: replyChainQueryKey, exact: false },
...
export function useBlockDomainMutationForUser(handle: string) {
  ...
  return useMutation({
    ...
    onMutate: (data: { url: string; handle?: string }) => {
      ...
      queryClient.setQueryData(QUERY_KEYS.account(data.handle), (currentAccount?: Account) => {
        if (!currentAccount) {
          return currentAccount;
        }
        return {
...
export function useBlockMutationForUser(handle: string) {
  ...
  return useMutation({
    ...
    onMutate: (account: Account) => {
      queryClient.setQueryData(QUERY_KEYS.account(account.handle), (currentAccount?: Account) => {
        if (!currentAccount) {
          return currentAccount;
        }
        return {
              ...
              if (item.object.id === id) {
                return {
                  ...
                  object: {
                    ...
                    reposted: reposted,
                    repostCount: Math.max(
                      reposted ? item.object.repostCount + 1 : item.object.repostCount - 1,
                      0,
                    ),
                  },
                };
    ...
    const updatePost = (post: Post) => {
      if (post.id === id) {
        return {
          ...
          repostedByMe: reposted,
          repostCount: Math.max(
            reposted ? (post.repostCount || 0) + 1 : (post.repostCount || 0) - 1,
            0,
          ),
        };
      }
...
export function useUnfollowMutationForUser(
  handle: string,
  onSuccess: () => void,
  onError: () => void,
) {
  ...
  return useMutation({
    ...
    onSuccess(_, fullHandle) {
      ...
      queryClient.setQueryData(
        profileQueryKey,
        (currentProfile?: { followedByMe: boolean; followerCount: number }) => {
          if (!currentProfile) {
          ...
          return {
            ...
            followedByMe: false,
            followerCount:
              currentProfile.followerCount - 1 < 0 ? 0 : currentProfile.followerCount - 1,
          };
        },
      );
      ...
      queryClient.setQueryData(
        QUERY_KEYS.exploreProfiles(handle),
        (
          current:
            | {
                pages: Array<{
                  results: Record<string, { categoryName: string; sites: Account[] }>;
                }>;
              }
            | undefined,
        ) => {
          ...
          const updatedPages = current.pages.map((page) => {
            const updatedResults = Object.entries(page.results).reduce(
      ...
      queryClient.setQueriesData(
        { queryKey: ['suggested_profiles'], exact: false },
...
export function useFollowMutationForUser(
  handle: string,
  onSuccess: () => void,
  onError: () => void,
) {
      ...
      if (existingData) {
        queryClient.setQueryData(
          ...
          (oldData?: {
            ...
            const followedAccount = queryClient.getQueryData<Account>(
              QUERY_KEYS.account(fullHandle === 'me' ? 'index' : fullHandle),
            );
            if (!followedAccount) {
              return oldData;
            }
...
export function useNoteMutationForUser(handle: string, actorProps?: ActorProperties) {
  ...
  return useMutation({
    ...
    onError(error: { message: string; statusCode: number }, _variables, context) {
      ...
      removeActivityFromPaginatedCollection(
        queryClient,
        queryKeyPostsByAccount,
        'posts',
        context?.id ?? '',
...
export function useAccountForUser(
  handle: string,
...
export function useFeedForUser(options: { enabled: boolean }) {
  ...
  const updateFeedActivity = (id: string, updated: Partial<Activity>) => {
    updateActivityInPaginatedCollection(queryClient, queryKey, 'posts', id, (activity) => ({
      ...activity,
      ...updated,
    }));
  };
...
export function usePostsByAccount(profileHandle: string, options: { enabled: boolean }) {
  ...
  const postsByAccountQuery = useInfiniteQuery({
    ...
    async queryFn({ pageParam }: { pageParam?: string }) {
      ...
      const api = createActivityPubAPI('index', siteUrl);
      return api
        .getPostsByAccount(profileHandle, pageParam)
        .then((response) => {
          return {
            posts: response.posts.map(mapPostToActivity),
            next: response.next,
...
export function useDeleteMutationForUser(handle: string) {
  ...
  return useMutation({
    ...
    onMutate: ({ id, parentId }) => {
      ...
      const previousFeed = queryClient.getQueryData<{ pages: { posts: Activity[] }[] }>(
        QUERY_KEYS.feed,
      );
      ...
      // - Decrement the reply count of the parent post if applicable
      const previousInbox = queryClient.getQueryData<{ pages: { posts: Activity[] }[] }>(
        QUERY_KEYS.inbox,
      );
      ...
      const previousOutbox = queryClient.getQueryData<{ pages: { data: Activity[] }[] }>(
        outboxQueryKey,
      );
      ...
      const previousLiked = queryClient.getQueryData<{ pages: { data: Activity[] }[] }>(
        likedQueryKey,
      );
      ...
      if (wasLiked) {
        queryClient.setQueryData(
          QUERY_KEYS.account(handle === 'me' ? 'index' : handle),
      ...
      const previousProfilePosts = queryClient.getQueriesData<{ pages: { posts: Activity[] }[] }>({
        queryKey: profilePostsQueryKey,
      });

      queryClient.setQueriesData(
        { queryKey: profilePostsQueryKey },
        (current?: { pages: { posts: Activity[] }[] }) => {
          if (!current) {
            return current;
          }
      ...
      const previousPostsByAccount = queryClient.getQueryData<{ pages: { posts: Activity[] }[] }>(
        postsByAccountKey,
      );
      ...
      const previousPostsLikedByAccount = queryClient.getQueryData<{
        pages: { posts: Activity[] }[];
      }>(postsLikedByAccountKey);
    ...
    onError: (_err, _variables, context) => {
      ...
      if (context.previousPostsLikedByAccount) {
        queryClient.setQueryData(
          QUERY_KEYS.postsLikedByAccount,
          context.previousPostsLikedByAccount,
        );
      }
    },
...
export function useReplyChainForUser(handle: string, postApId: string | null) {
  ...
  const loadMoreChildReplies = useCallback(
    async (childIndex: number) => {
      ...
      try {
        ...
        const replyForPagination =
          child.chain.length > 1 ? child.chain[child.chain.length - 2] : child.post;
...
export function useExploreProfilesForUserByTopic(handle: string, topic: string) {
  ...
  const updateExploreProfile = (id: string, updated: Partial<Account>) => {
    queryClient.setQueryData(
      queryKey,
      (current: { pages: Array<{ accounts: Account[] }> } | undefined) => {
        if (!current) {
          return current;
        }
