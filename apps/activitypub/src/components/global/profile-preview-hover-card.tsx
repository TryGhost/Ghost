...
const ProfilePreviewHoverCard: React.FC<ProfilePreviewHoverCardProps> = ({
  ...
  }, [accountQuery, isLoading, hasLoadingError, hasCompleteAccountData, shouldFetch, targetHandle]);
  ...
  const avatarUrl =
    displayData?.avatarUrl ?? (actor && isActorProperties(actor) ? actor.icon?.url : null) ?? null;
  const followsYou = Boolean(displayData?.followsMe);
  const followingCount =
    typeof displayData?.followingCount === 'number'
      ? displayData.followingCount
      : Number(displayData?.followingCount) || 0;
  const followerCount =
    typeof displayData?.followerCount === 'number'
      ? displayData.followerCount
      : Number(displayData?.followerCount) || 0;
                ...
                <>
                  <span className="cursor-pointer hover:underline" onClick={handleFollowingClick}>
                    <span className="font-bold text-black dark:text-white">
                      {abbreviateNumber(followingCount)}
                    </span>{' '}
                    Following
                  </span>
