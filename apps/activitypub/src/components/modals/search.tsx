...
const Search: React.FC<SearchProps> = ({ onOpenChange, query, setQuery }) => {
  ...
  const { searchQuery, updateAccountSearchResult: updateResult } = useSearchForUser(
    'index',
  ...
  const hasSuggestedProfiles =
    isLoadingSuggestedProfiles || (suggestedProfilesData && suggestedProfilesData.length > 0);
  ...
  const matchingTopics = useMemo(() => {
    ...
    return topics.filter((topic) => {
      ...
      return (
        topic.name.toLowerCase().startsWith(normalizedQuery) ||
  ...
  const showSuggested =
    query.length < 2 || (!lastResultState && shouldSearch && matchingTopics.length === 0);
  const showNoResults = !showSuggested && lastResultState === 'none' && matchingTopics.length === 0;
  const showSearchResults =
    !showSuggested && (displayResults.length > 0 || matchingTopics.length > 0);
  ...
  useEffect(() => {
    ...
    const container =
      element.closest('[data-radix-scroll-area-viewport]') || element.closest('.overflow-y-auto');
    if (!container) {
    ...
    if (elementRect.top < containerRect.top + STICKY_HEADER_HEIGHT) {
      ...
      container.scrollTo({
        top:
          container.scrollTop + (elementRect.bottom - containerRect.bottom + STICKY_HEADER_HEIGHT),
        behavior: 'smooth',
      });
    }
  }, [selectedIndex]);
      ...
      <div className="h-full">
        ...
        {showSearchResults && (
          <div className="mt-[-14px] pb-2">
            {searchResults.map((item, index) => {
              ...
              if (item.type === 'topic') {
                return (
                  <div key={item.data.slug} ref={(el) => (itemRefs.current[index] = el)}>
                    <ActivityItem
        ...
        {showSuggested && hasSuggestedProfiles && (
          <>
            ...
            <SuggestedProfiles onOpenChange={onOpenChange} />
          </>
        )}
      </div>
    </>
