...
const Layout: React.FC<React.HTMLAttributes<HTMLDivElement>> = ({ children, ...props }) => {
  ...
  return (
    <div
      ref={containerRef}
      className={`h-screen w-full ${isOnboarded && 'overflow-y-auto'}`}
    ...
    >
          ...
          <>
            <div className="block grid-cols-[auto_320px] items-start lg:grid">
              <div className="z-0 min-w-0">
                <Header
                  showBorder={
                    !(currentPage === 'reader' && hasTopics) && !(currentPage === 'explore')
                  }
            ...
            {isMobileSidebarOpen && (
              <div className="fixed inset-0 z-40 lg:hidden" onClick={closeMobileSidebar} />
            )}
          </>
        ) : (
          <Onboarding />
      ...
      <NewNoteModal open={isNewNoteModalOpen} onOpenChange={setIsNewNoteModalOpen} />
    </div>
  );
};
