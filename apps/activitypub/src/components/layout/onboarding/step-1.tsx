      ...
      <div className="isolate flex h-full flex-col items-stretch justify-between">
        <div className="relative z-20 flex justify-between px-14">
          <div className="flex flex-col gap-4 pb-20 text-xl font-medium">
            <h1 className="max-w-sm text-[3.6rem] tracking-[-.025em]">
              Increase your reach, with the social web.
            </h1>
            <div className="flex max-w-[600px] flex-col gap-4">
              ...
              <p>
                {account ? (
                  <>
                    <strong>{account.name}</strong> is now part of the world’s largest open network.
                  </>
                ) : (
        ...
        <div className="relative z-10 h-full">
          <img
            className="absolute top-[calc(-280px)] left-1/2 w-full max-w-[1300px] min-w-[1240px] -translate-x-1/2 dark:hidden"
            src={apNodes}
          />
          <img
            className="absolute top-[calc(-280px)] left-1/2 hidden w-full max-w-[1300px] min-w-[1240px] -translate-x-1/2 dark:visible! dark:block!"
            src={apNodesDark}
          />
          <div className="relative mx-auto mt-0 flex w-96 flex-col gap-3 rounded-lg bg-white p-6 shadow-xl before:absolute before:inset-[80px] before:-z-10 before:rounded-full before:bg-[radial-gradient(circle,#4b5563,#6b7280,#9ca3af)] before:blur-[1000px] xxl:mt-[calc(-280px+20vw)] min-[1800px]:mt-14 dark:border dark:border-gray-950 dark:bg-[#101114] dark:shadow-[#1e1b4b]/5 dark:before:bg-[radial-gradient(circle,#6366f1,#a855f7,#ec4899)]">
            ...
            <div className="flex flex-col items-start gap-1">
              ...
              >
                <div className="relative flex h-full items-center gap-1">
                  ...
                  <span
                    className={`absolute right-full flex items-center gap-3 font-mono text-sm font-medium text-nowrap text-purple uppercase after:mr-3 after:h-[1.5px] after:w-24 after:scale-0 after:bg-purple after:will-change-transform after:content-[""] ${!usersLoading && 'after:animate-onboarding-handle-line'}`}
                  >
                    <span
                      className={`opacity-0 ${!usersLoading && 'animate-onboarding-handle-label'}`}
                    >
                      Your social web handle
                    </span>
                  </span>
                </div>
              </div>
            ...
            <div className="mt-1 flex gap-2 text-sm text-gray-800 dark:text-gray-600">
              ...
              Authored by {firstThreeUsers[0]?.name}
              {(meta?.pagination.total ?? 0) > 1 &&
                ` and ${(meta?.pagination.total ?? 1) - 1} others`}
            </div>
          </div>
        </div>
      </div>
