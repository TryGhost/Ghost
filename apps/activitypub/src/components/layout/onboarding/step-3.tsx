...
const MenuItem: React.FC<{
  ...
  return (
    <div
      className={`flex h-[30px] items-center gap-1.5 rounded-sm px-2 text-sm font-medium  ${selected ? 'bg-gray-100 text-black' : 'text-gray-700 dark:text-gray-600'}`}
    >
      {children}
    </div>
  );
      ...
      <div className="grid h-full grid-cols-[auto_248px] gap-4">
        <div>
          {inboxList.map((item) => (
            <div
              key={item.publisher}
              className="flex items-center justify-between gap-8 border-b border-gray-200 py-4 dark:border-gray-900"
            >
              <div className="flex flex-col gap-1">
                <div className="flex items-center gap-1.5 text-sm font-semibold">
                  <div
                    className="size-4 rounded-full bg-cover bg-center bg-no-repeat"
                    style={{
                ...
                <div className="text-sm text-gray-700 dark:text-gray-600">{item.excerpt}</div>
              </div>
              <div>
                <div
                  className="h-20 w-[120px] min-w-[120px] rounded-sm bg-cover bg-center bg-no-repeat"
                  style={{
  ...
  const feedList = [
    {
      ...
      post: (
        <div>
          <span className="text-purple">@newyorktimes&apos;s</span> veteran tech reporter{' '}
          <span className="text-purple">@Markoff</span> interviewed some of{' '}
          <span className="text-purple">#TeamFediverse</span> including our CEO{' '}
          <span className="text-purple">@mike</span> for a feature on the rise of decentralized
          social media. &quot;It goes back to the original principles...{' '}
    ...
    },
    {
      ...
      post: (
        <div>
          This sucks. I met my wife on the basic HTML version of Gmail.{' '}
          <span className="text-purple">techmeme.com/23092...</span>
        </div>
      ),
      likes: 118,
      replies: 32,
    ...
    },
    {
      ...
      post: (
        <div>
          At #FediverseHouse, after <span className="text-purple">@pluralistic</span>&apos;s talk, I
          came up and thanked him for speaking. We&apos;ve never met before in person, so I asked if
          ...
          {feedList.map((item) => (
            <>
              <div className="flex w-full items-start gap-3 px-3">
                <div
                  className="size-9 max-h-9 min-h-9 max-w-9 min-w-9 rounded-full bg-cover bg-center bg-no-repeat"
                  style={{
                    backgroundImage: `url(${item.avatar})`,
                  }}
                ></div>
                <div className="flex flex-col gap-3 text-sm">
                  <div className="mt-0.5 flex flex-col gap-0.5">
                    ...
                    <div className="leading-tighter text-gray-700 dark:text-gray-600">
                      {item.handle}
                    </div>
                  </div>
                  <div>{item.post}</div>
    ...
    <>
      ...
      <div className="mt-8 flex w-full flex-col items-center">
        <div className="flex w-full max-w-[610px] flex-col gap-5">
          <H1 className="mx-auto max-w-[520px]">
          ...
          ></div>
          <div className="mx-auto max-w-[520px] font-serif text-lg">
            The TikTok ban and Donald Trump&apos;s rise to power show how fragile our social media
            accounts are. We must normalize and invest in decentralized social media.
          </div>
          <div className="mx-auto max-w-[520px] font-serif">
            If it wasn’t already obvious, the last 72 hours have made it crystal clear that it is
            urgent to build and mainstream alternative, decentralized social media platforms that
            are resistant to government censorship and control, are not owned by oligarchs and
            dominated by their algorithms, and in which users own their follower list and can port
            it elsewhere easily and without restriction.
          </div>
        </div>
      </div>
    </>
