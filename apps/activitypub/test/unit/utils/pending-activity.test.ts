...
describe('Pending Activity Utils', function () {
  describe('generatePendingActivityId', function () {
    it('should generate a pending activity id', function () {
      expect(generatePendingActivityId()).toMatch(
        new RegExp(
          `^${PENDING_ACTIVITY_PREFIX}[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$`,
        ),
      );
    });
  });
  ...
  describe('isPendingActivity', function () {
    it('should return true if the activity is pending', function () {
      ...
      expect(
        isPendingActivity(`${PENDING_ACTIVITY_PREFIX}7c81cd06-b4bb-4b30-a745-8332df794398`),
      ).toBe(true);
    });
  });
  ...
  describe('generatePendingActivity', function () {
    ...
    it('should attach an uploaded image url to the pending activity', function () {
      ...
      const id = generatePendingActivityId();
      const pendingActivity = generatePendingActivity(
        actor,
        id,
        'Pending note',
  ...
  describe('formatPendingActivityContent', function () {
    it('should format the content', function () {
      ...
      expect(formatPendingActivityContent(content)).toBe(
