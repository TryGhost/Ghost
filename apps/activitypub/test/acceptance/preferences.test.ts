...
test.describe('Preferences', async () => {
  ...
  test('I still see moderation controls when the preferences endpoint is unavailable', async ({
    page,
  }) => {
    await mockApi({
      page,
  ...
  test('I can open the Social Web domain screen from preferences', async ({ page }) => {
    ...
    await expect(
      page.getByRole('heading', { exact: true, name: 'Your social web handle' }),
    ).toBeVisible();
    ...
    await expect(page.getByLabel('Social web domain', { exact: true })).toHaveValue(
      'blog.site.com',
    );
  ...
  test('I can see a configured Social Web domain', async ({ page }) => {
    ...
    await expect
      .poll(() => page.evaluate(() => window.localStorage.getItem('copiedHandle')))
      .toBe('@index@site.com');
  });
  ...
  test('I see Social Web domain validation errors without losing the instructions', async ({
    page,
  }) => {
    const errorCases = [
      {
