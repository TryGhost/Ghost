...
test.describe('Feed', async () => {
  ...
  test('I can publish a note', async ({ page }) => {
    const { lastApiRequests } = await mockApi({
      page,
      requests: {
        getFeed: {
          method: 'GET',
    ...
    const postButton = noteDialog.getByTestId('post-button');
  ...
  test('I can view a list of notes in the Feed', async ({ page }) => {
    ...
    expect(firstFeedItemText).toContain(
      firstPost.content.replace(/<[^>]*>?/g, '').substring(0, 100),
    );
    expect(firstFeedItemText).toContain(
      new Date(firstPost.publishedAt).toLocaleString('en-GB', { month: 'short', day: 'numeric' }),
    );
  });

  test('sensitive media follows the saved preference and can be revealed per post', async ({
    page,
  }) => {
    const imageFixture =
    ...
    await expect(
      sensitivePost.getByTestId('sensitive-media-overlay').getByText('Sensitive media'),
    ).toBeVisible();
  ...
  test('sensitive media shown by preference does not show a local hide button', async ({
    page,
  }) => {
    const imageFixture =
      '<svg xmlns="http://www.w3.org/2000/svg" width="1" height="1"><rect width="1" height="1" fill="black"/></svg>';
    ...
    await expect(
      shownSensitivePost.getByRole('button', { name: 'Hide sensitive media' }),
    ).toHaveCount(0);
  ...
  test('content warnings hide warned content until revealed', async ({ page }) => {
    ...
    const warnedPost = page.locator(
      '[data-object-id="https://test.instance/users/user6/statuses/content-warning"]',
    );
