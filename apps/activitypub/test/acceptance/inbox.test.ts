...
test.describe('Inbox', async () => {
  ...
  test('I can click on a post to view it', async ({ page }) => {
    ...
    await expect(
      iframeContent.getByText(postFixture.content.replace(/<[^>]*>?/g, '').trim()),
    ).toBeVisible(); // Content (inside the iframe)
  });

  test('sensitive inbox article media is hidden and content warnings hide the reader body', async ({
    page,
  }) => {
    const sensitivePost = {
    ...
    await expect(
      firstInboxItem.getByText('This excerpt should stay visible in the inbox row.'),
    ).toBeVisible();
    await expect(firstInboxItem.getByTestId('sensitive-media-overlay')).toBeVisible();
    await expect(
      firstInboxItem.locator(
        'img[src="https://techblog.example/content/images/sensitive-reader.jpg"]',
      ),
    ).toHaveCount(0);
    ...
    await expect(
      modal.getByText('This sensitive reader body should stay hidden until revealed.'),
    ).toHaveCount(0);
    ...
    const iframeContent = iframe.contentFrame();
    await expect(
      iframeContent.getByText('This sensitive reader body should stay hidden until revealed.'),
    ).toBeVisible();
  });
  ...
  test('sensitive reader article media can be revealed locally', async ({ page }) => {
    const sensitivePost = {
      ...
      content:
        '<p>This sensitive reader text should stay visible.</p><iframe src="https://www.youtube.com/embed/test"></iframe><img src="https://techblog.example/content/images/inline-sensitive.jpg" alt="Inline sensitive image">',
    ...
    const iframeContent = iframe.contentFrame();
    await expect(
      iframeContent.getByText('This sensitive reader text should stay visible.'),
    ).toBeVisible();
    ...
    await expect(
      iframeContent.locator(
        'img[src="https://techblog.example/content/images/sensitive-reader-media.jpg"]',
      ),
    ).toBeHidden();
    await expect(
      iframeContent.locator(
        'img[src="https://techblog.example/content/images/inline-sensitive.jpg"]',
      ),
    ).toBeHidden();
    await expect(
      iframeContent.locator('iframe[src="https://www.youtube.com/embed/test"]'),
    ).toBeHidden();
    ...
    await expect(iframeContent.locator('body')).toHaveAttribute(
      'data-sensitive-media-marker',
      'stable',
    );
    await expect(
      iframeContent.locator(
  ...
  test('changing reading options does not reload the article iframe', async ({ page }) => {
    ...
    await page
      ...
      .getByText('Reading options article')
      .click();

    const modal = page.getByRole('dialog');
    const iframeContent = modal.locator('iframe').contentFrame();
    await expect(
      iframeContent.getByText('This article body should survive a font change.'),
    ).toBeVisible();
  ...
  test('sensitive reader articles without media do not show a media warning', async ({ page }) => {
    ...
    const iframeContent = iframe.contentFrame();
    await expect(
      iframeContent.getByText('This sensitive reader text has no media to hide.'),
    ).toBeVisible();
  });
