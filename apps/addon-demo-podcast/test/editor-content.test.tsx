...
describe('podcast editor block', function () {
  it('keeps a durable episode presentation while opting into player hydration', async function () {
    ...
    expect(output.portableHtml).toContain(
      'https://podcasts.example/episodes/independent-publishing',
    );
    expect(output.portableHtml).toContain('0:00 / 42 min');
    expect(output.portableHtml).toContain('height="36"');
    expect(output.portableHtml).toContain('width="36"');
    expect(output.portableHtml).not.toContain('display:table-cell');
