...
const contentTypes: Record<string, string> = {'.js': 'text/javascript; charset=utf-8', '.json': 'application/json; charset=utf-8'};
...
createServer(async (request, response) => {
    ...
    if (url.pathname === '/api/resolve' && request.method === 'POST') {
        ...
        try {
            ...
            sendJson(response, 400, {error: error instanceof Error ? error.message : 'Episode resolution failed'});
        }
        return;
    }
