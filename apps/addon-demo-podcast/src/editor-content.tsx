...
function EpisodeDetails({ episode, interactive }: { episode: Episode; interactive: boolean }) {
  ...
  if (!episode.title) {
    return (
      <article className="podcast-card podcast-card--empty">
        ...
        <p>
          Paste an episode URL in the card settings to resolve its artwork, description, and audio.
        </p>
      </article>
    );
  }
  ...
  return (
    <article className="podcast-card">
      {episode.canonicalUrl && artwork ? (
        <a className="podcast-card__artwork" href={episode.canonicalUrl}>
          {artwork}
        </a>
      ) : (
        artwork
      ...
      <div className="podcast-card__body">
        {episode.showName && (
          <p className="podcast-card__show">
            {episode.canonicalUrl ? (
              <a href={episode.canonicalUrl}>{episode.showName}</a>
            ) : (
              episode.showName
            )}
          </p>
        ...
        <h2>
          {episode.canonicalUrl ? (
            <a href={episode.canonicalUrl}>{episode.title}</a>
          ) : (
            episode.title
          )}
        </h2>
          ...
          <div className="podcast-card__player">
            ...
            <div className="podcast-card__transport">
              <input
                ...
                step="0.1"
                style={{
                  background: `linear-gradient(to right, #181816 ${progress}%, #d6d6d1 ${progress}%)`,
                }}
                type="range"
                value={progress}
              ...
              <div className="podcast-card__controls">
                <div className="podcast-card__actions">
                  <button
                    ...
                    className="podcast-card__control"
                    type="button"
                    onClick={() => seekBy(-10)}
                  >
                    <svg aria-hidden="true" viewBox="0 0 24 24">
                      <path d="M5 8V4L2 7l3 3V8a8 8 0 1 1-1 4" />
                      <text x="12" y="15">
                        10
                      </text>
                    </svg>
                  ...
                  <button
                    aria-label={`Playback speed ${playbackRate.spokenLabel} times`}
                    className="podcast-card__speed"
                    type="button"
                  ...
                  >
                    <svg aria-hidden="true" viewBox="0 0 24 24">
                      <path d="M19 8V4l3 3-3 3V8a8 8 0 1 0 1 4" />
                      <text x="12" y="15">
                        30
                      </text>
                    </svg>
                ...
                <span className="podcast-card__time">
                  <time>{formatTimecode(currentTime)}</time>
...
function PortableEpisodeDetails({ episode }: { episode: Episode }) {
  if (!episode.title) {
    return (
      ...
      >
        ...
        <h2 style={{ fontSize: '26px', fontWeight: '400', lineHeight: '1.15', margin: '0 0 8px' }}>
          Add a podcast episode
        </h2>
        <p style={{ color: '#4a4a47', lineHeight: '1.5', margin: '0' }}>
          Paste an episode URL in the card settings to resolve its details.
        </p>
      </article>
    );
  }
  ...
  const player = (
    ...
    >
      <tbody>
        <tr>
          <td style={{ padding: '0', verticalAlign: 'middle' }} width="36">
            <table
              role="presentation"
              style={{ borderCollapse: 'separate', borderSpacing: '0', width: '36px' }}
              width="36"
            >
              <tbody>
                <tr>
          ...
          <td style={{ padding: '0 12px', verticalAlign: 'middle' }}>
            <span
              style={{ background: '#d6d6d1', display: 'block', height: '3px', width: '100%' }}
            ></span>
          </td>
          <td
            style={{
          ...
          <tr>
            {episode.artworkUrl && (
              <td style={{ padding: '0 18px 0 0', verticalAlign: 'top', width: '88px' }}>
                <img
                  alt={`${episode.showName || episode.title} artwork`}
                  src={episode.artworkUrl}
                  style={{ display: 'block', height: '88px', objectFit: 'cover', width: '88px' }}
                />
              </td>
            )}
