...
async function readBoundedText(body: ResponseBody, maxBytes: number, label: string): Promise<string> {
    if (!body) {
        return '';
    }
...
function decodeHtml(value: string): string {
    return value
        ...
        .replace(/&#x([\da-f]+);/gi, (_match, codePoint: string) => String.fromCodePoint(Number.parseInt(codePoint, 16)))
        .replaceAll('&amp;', '&')
        .replaceAll('&quot;', '"')
        .replaceAll('&#39;', '\'')
        .replaceAll('&apos;', '\'')
...
function feedHref(html: string): string {
    for (const match of html.matchAll(/<link\b[^>]*>/gi)) {
        ...
        if (relations.includes('alternate') && ['application/rss+xml', 'application/atom+xml', 'application/xml', 'text/xml'].includes(type)) {
...
function elementContent(markup: string, name: string): string {
    const escaped = name.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    const match = new RegExp(`<${escaped}(?:\\s[^>]*)?>([\\s\\S]*?)<\\/${escaped}\\s*>`, 'i').exec(markup);
    return match?.[1]?.trim() || '';
}
...
function parseFeedEpisode(feed: string, feedUrl: string, submittedUrl: string, matchUrl?: string): ResolvedEpisode {
    ...
    const channelImage = attributeValue(openingTag(channel, 'itunes:image'), 'href') || textContent(elementContent(channel, 'image'), 'url');
    const items = Array.from(feed.matchAll(/<item\b[^>]*>([\s\S]*?)<\/item\s*>/gi), match => match[1]);
    ...
    const item = items.find((candidate) => {
        ...
        return [textContent(candidate, 'link'), textContent(candidate, 'guid')].some(value => comparableUrl(value) === expectedUrl);
    });
    if (!item) {
        throw new Error(matchUrl ? 'The podcast feed does not contain this episode' : 'The podcast feed does not contain an audio episode');
    }
    ...
    const itemUrl = optionalHttpsUrl(textContent(item, 'link'), feedUrl) || optionalHttpsUrl(textContent(item, 'guid'), feedUrl);
    return {
        submittedUrl,
        ...
        artworkUrl: optionalHttpsUrl(attributeValue(openingTag(item, 'itunes:image'), 'href') || channelImage, feedUrl),
        audioUrl,
...
function isFeedResponse(url: string, contentType: string): boolean {
    return /(?:application|text)\/(?:rss\+xml|atom\+xml|xml)/i.test(contentType)
        || /(?:\.rss|\.xml)(?:$|[?#])/i.test(url)
        || /\/feed(?:\/|$|[?#])/i.test(url);
}
...
async function loadDocument(value: string, fetchPage: FetchPage, expectedFeed = false) {
    ...
    for (let redirects = 0; redirects <= 5; redirects += 1) {
        response = await fetchPage(currentUrl, {
            headers: {accept: expectedFeed ? 'application/rss+xml,application/xml,text/xml' : 'text/html,application/xhtml+xml,application/rss+xml,application/xml,text/xml'},
            redirect: 'manual',
        ...
        if (!location || redirects === 5) {
            ...
            throw new Error(`${expectedFeed ? 'Podcast feed' : 'Episode page'} redirected too many times`);
        }
        await cancelBody(response.body);
    ...
    const isFeed = expectedFeed || isFeedResponse(finalUrl, response.headers.get('content-type') || '');
