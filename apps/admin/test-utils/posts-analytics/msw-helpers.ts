...
export const mockData = {
  ...
  newsletterBasicStat: (
    overrides: Partial<NewsletterBasicStatItem> = {},
  ): NewsletterBasicStatItem => ({
  ...
  newsletterClickStat: (
    overrides: Partial<NewsletterClickStatItem> = {},
  ): NewsletterClickStatItem => ({
    post_id: 'test-post-id',
    total_clicks: 50,
    email_count: 1000,
    click_rate: 0.05,
  ...
  postReferrer: (
    overrides: Partial<import('@tryghost/admin-x-framework/api/stats').PostReferrerStatItem> = {},
  ) => ({
    source: 'Google',
  ...
  postGrowthStat: (
    overrides: Partial<import('@tryghost/admin-x-framework/api/stats').PostGrowthStatItem> = {},
  ) => ({
  ...
  mrrHistory: (
    overrides: Partial<import('@tryghost/admin-x-framework/api/stats').MrrHistoryItem> = {},
  ) => ({
    date: '2024-01-01',
    mrr: 50000,
    currency: 'usd',
  ...
  feedbackList: (items: Partial<FeedbackItem>[] = []) => ({
    feedback: items.map((item) => mockData.feedback(item)),
  }),
  linksList: (items: Partial<LinkItem>[] = []) => ({
    links: items.map((item) => mockData.link(item)),
  }),
  newsletterBasicStatsList: (stats: Partial<NewsletterBasicStatItem>[] = []) => ({
    stats: stats.map((item) => mockData.newsletterBasicStat(item)),
  }),
  newsletterClickStatsList: (stats: Partial<NewsletterClickStatItem>[] = []) => ({
    stats: stats.map((item) => mockData.newsletterClickStat(item)),
  }),
  postReferrersList: (
    stats: Partial<import('@tryghost/admin-x-framework/api/stats').PostReferrerStatItem>[] = [],
  ) => ({
    stats: stats.map((item) => mockData.postReferrer(item)),
    meta: {
      pagination: { page: 1, limit: 10, pages: 1, total: stats.length, next: null, prev: null },
    },
  }),
  postGrowthStatsList: (
    stats: Partial<import('@tryghost/admin-x-framework/api/stats').PostGrowthStatItem>[] = [],
  ) => ({
    stats: stats.map((item) => mockData.postGrowthStat(item)),
    meta: {
      pagination: { page: 1, limit: 15, pages: 1, total: stats.length, next: null, prev: null },
    },
  }),
  mrrHistoryList: (
...
export const mockServer = {
  setup(config: MockServerConfig = {}) {
    ...
    if (config.posts !== undefined) {
      handlers.push(
        createGhostHandler('get', '/ghost/api/admin/posts/*', mockData.posts(config.posts)),
      );
    }
    ...
    if (config.feedback !== undefined) {
      handlers.push(
        createGhostHandler(
          ...
          mockData.feedbackList(config.feedback),
        ),
      );
    }
    ...
    if (config.links !== undefined) {
      handlers.push(
        createGhostHandler('get', '/ghost/api/admin/links/', mockData.linksList(config.links)),
      );
    }
    ...
    if (config.newsletterBasicStats !== undefined) {
      handlers.push(
        createGhostHandler(
          ...
          mockData.newsletterBasicStatsList(config.newsletterBasicStats),
        ),
      );
    }
    ...
    if (config.newsletterClickStats !== undefined) {
      handlers.push(
        createGhostHandler(
          ...
          mockData.newsletterClickStatsList(config.newsletterClickStats),
        ),
      );
    }
    ...
    if (config.postReferrers !== undefined) {
      handlers.push(
        createGhostHandler(
          'get',
          '/ghost/api/admin/stats/posts/*/top-referrers',
          mockData.postReferrersList(config.postReferrers),
        ),
      );
    }
    ...
    if (config.postGrowthStats !== undefined) {
      handlers.push(
        createGhostHandler(
          'get',
          '/ghost/api/admin/stats/posts/*/growth',
          mockData.postGrowthStatsList(config.postGrowthStats),
        ),
      );
    }
    ...
    if (config.mrrHistory !== undefined) {
      const mrrData =
        config.mrrHistory.totals !== undefined
    ...
    handlers.push(
      createGhostHandler('get', '/ghost/api/admin/site/', { site: config.site || DEFAULT_SITE }),
    );
    handlers.push(
      createGhostHandler('get', '/ghost/api/admin/config/', {
        config: config.config || DEFAULT_CONFIG,
      }),
    );
    handlers.push(
      createGhostHandler('get', '/ghost/api/admin/settings/', {
        settings: config.settings || DEFAULT_SETTINGS,
      }),
    );
    handlers.push(
      createGhostHandler('get', '/ghost/api/admin/tinybird/token/', {
        tinybird: { token: config.tinybirdToken || DEFAULT_TINYBIRD_TOKEN },
      }),
    );
