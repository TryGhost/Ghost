...
function editedSettingsEqual(
  actual: EditedSettings | undefined,
  expected: EditedSettings,
): boolean {
  return (
    actual?.length === expected.length &&
    expected.every((expectedSetting) =>
      actual.some(
        (setting) => setting.key === expectedSetting.key && setting.value === expectedSetting.value,
      ),
    )
  );
}
...
function matchesExpected(expected: string | RegExp, actual: string | undefined): boolean {
  ...
  const stateless =
    expected.global || expected.sticky
      ? new RegExp(expected.source, expected.flags.replace(/[gy]/g, ''))
      : expected;
...
async function pollEditedSettings(
  ...
  const seen =
    actual === undefined
      ? 'no settings edit captured yet'
      : `the last edit was ${JSON.stringify(actual)}`;
...
async function pollSitePreview(
  ...
  return {
    pass,
    message: () =>
      `expected the capture ${isNot ? 'not ' : ''}to have requested preview parameters ${JSON.stringify(expected)}, but ${seen}`,
  };
}
