...
type InputOf<TBuilder> = TBuilder extends (input: infer TInput) => unknown
  ? NonNullable<TInput>
  : never;
...
export const fakeAdminStats = {
  memberCount(options: MemberCountOptions = {}): EndpointCapture {
    ...
    const response = {
      stats: memberStatusStat.many(stats),
      meta: { totals },
    } satisfies MemberCountHistoryResponseType;
  ...
  mrr(options: MrrOptions = {}): EndpointCapture {
    ...
    const response = {
      stats: mrrHistoryStat.many(stats),
      meta: { totals },
    } satisfies MrrHistoryResponseType;
  ...
  subscriptions(options: SubscriptionOptions = {}): EndpointCapture {
    ...
    } satisfies SubscriptionStatsResponseType;
  ...
  topContent(stats: Array<InputOf<typeof topContentStat>> = []): EndpointCapture {
    const response = {
      stats: topContentStat.many(stats),
      meta: {},
    } satisfies TopContentResponseType;
  ...
  topPosts(stats: Array<InputOf<typeof topPostStat>> = []): EndpointCapture {
    const response = {
      stats: topPostStat.many(stats),
      meta: {},
    } satisfies TopPostsStatsResponseType;
  ...
  post(postId: string, overrides: PostStatsOverrides = {}): EndpointCapture {
    const response = {
      stats: [postStats({ id: postId, ...overrides })],
    } satisfies PostStatsResponseType;
    return fakeAdminEndpoint('GET', endpoint(`/stats/posts/${postId}/stats/`), response);
  },

  postReferrers(
    postId: string,
    stats: Array<InputOf<typeof postReferrerStat>> = [],
  ): EndpointCapture {
    const response = {
      stats: postReferrerStat.many(stats),
      meta: {},
    } satisfies PostReferrersResponseType;
  ...
  postGrowth(postId: string, overrides: PostGrowthOverrides = {}): EndpointCapture {
    const response = {
      stats: [postGrowthStat({ post_id: postId, ...overrides })],
      meta: {},
  ...
  newsletterSubscribers(
    { values = [], ...overrides }: NewsletterSubscriberInput = { total: 0 },
  ): EndpointCapture {
    const response = {
      stats: [
        newsletterSubscriberStat({
  ...
  newsletterBasic(stats: Array<InputOf<typeof newsletterBasicStat>> = []): EndpointCapture {
    const response = {
      stats: newsletterBasicStat.many(stats),
      meta: {},
    } satisfies NewsletterBasicStatsResponseType;
  ...
  newsletterClicks(stats: Array<InputOf<typeof newsletterClickStat>> = []): EndpointCapture {
    const response = {
      stats: newsletterClickStat.many(stats),
      meta: {},
