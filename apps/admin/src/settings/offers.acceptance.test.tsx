...
describe('Offers', () => {
  ...
  it('sorts signup and retention offers by date, name and redemptions', async () => {
    ...
    fakeOffers([
      ...
      retentionOffer({
        name: 'Monthly retention offer',
        created_at: '2026-01-03T12:00:00.000Z',
        redemption_count: 7,
      }),
      retentionOffer({
        name: 'Yearly retention offer',
        cadence: 'year',
  ...
  describe('Retention offers', () => {
    ...
    it('lists monthly and yearly retention offers', async () => {
      ...
      await expect
        ...
        .toHaveAttribute(
          'href',
          '/ghost/#/members?filter=offer_redemptions%3A%5Bretention-month-active%2Cretention-month-archived%5D',
        );
    ...
    it('renders existing retention offers in edit mode', async () => {
      ...
      await expect
        ...
        .toHaveAttribute(
          ...
          expect.stringContaining(
            'offer_redemptions%3A%5Bretention-month-active%2Cretention-month-archived%5D',
          ),
        );
      await expect.element(monthlyModal.getByLabelText('Enable monthly retention')).toBeChecked();
      await expect
        .element(monthlyModal.getByLabelText('Display title'))
        .toHaveValue('Stay monthly');
      await expect
        .element(monthlyModal.getByLabelText('Display description'))
        .toHaveValue('Monthly description');
      ...
      await expect
        ...
        .toHaveAttribute(
          'href',
          expect.stringContaining(
            'offer_redemptions%3A%5Bretention-year-active%2Cretention-year-archived%5D',
          ),
        );
      ...
      await expect
        .element(yearlyModal.getByLabelText('Display description'))
        .toHaveValue('Yearly description');
    ...
    it('shows validation errors for invalid retention values on save', async () => {
      ...
      await expect
        .element(retentionModal.getByText('Enter an amount between 1 and 100%.'))
        .toBeVisible();
      await expect.element(saveButton).toBeEnabled();
      ...
      await saveButton.click();
      await expect
        .element(retentionModal.getByText('Enter a whole number of months between 1 and 99.'))
        .toBeVisible();
    ...
    it('renders the portal preview for retention offers', async () => {
      ...
      await retentionModal
        .getByLabelText('Display description')
        .fill('Please stay <script>alert(1)</script>');
      await retentionModal.getByRole('radio', { name: /Free month\(s\)/ }).click();
      ...
      await expect
        .poll(() => decodeURIComponent(previewParams().get('display_title') ?? ''))
        .toBe('Before you go');
      let params = previewParams();
      expect(decodeURIComponent(params.get('display_description') ?? '')).toBe(
        'Please stay <script>alert(1)</script>',
      );
    ...
    it('creates a new retention offer when terms change', async () => {
      ...
      const addApi = fakeAdminEndpoint('POST', '/offers/', {
        offers: [
          { id: 'retention-offer-monthly', name: 'Monthly retention', code: 'monthly-retention' },
        ],
      });
      await renderAdminApp('/settings/offers/edit/retention/monthly', withStripe());
      ...
      const createdOffer = (
        addApi.lastRequest?.body as { offers: Array<{ name: string; code: string }> }
      ).offers[0];
    ...
    it('edits the existing retention offer when only display fields change', async () => {
      ...
      const editApi = fakeAdminEndpoint('PUT', '/offers/retention-month-active/', {
        offers: [
          { id: 'retention-month-active', name: 'Monthly retention', code: 'monthly-retention' },
        ],
      });
      await renderAdminApp('/settings/offers/edit/retention/monthly', withStripe());
