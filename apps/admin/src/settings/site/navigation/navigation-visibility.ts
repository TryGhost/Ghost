...
export const getNavigationVisibility = (
  ...
  navigationVisibilityOptions.some((option) => option.value === visibility)
    ? (visibility as NavigationItemVisibility)
    : 'public';

export const getVisibilityLabel = (visibility: NavigationItemVisibility) =>
  navigationVisibilityLabels[visibility];

export const getAudienceVisibility = (
  visibility: NavigationItemVisibility,
...
export const getVisibilityFromAudiences = (
  audiences: Record<VisibilityAudience, boolean>,
): NavigationItemVisibility => {
  const key = [audiences['public-visitors'], audiences['free-members'], audiences['paid-members']]
    .map((value) => (value ? '1' : '0'))
    .join('') as AudienceKey;
