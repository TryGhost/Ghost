...
const NavigationVisibilityDropdown: React.FC<NavigationVisibilityDropdownProps> = ({
  idPrefix,
  item,
  clearError,
  updateItem,
  ...
  return (
    <>
      <label className="sr-only" id={visibilityLabelId}>
        Visibility
      </label>
      <DropdownMenu>
        ...
        >
          <div className="flex flex-col">
            {visibilityAudienceOptions
              .filter((option) => option.key !== 'paid-members' || showPaidVisibility)
              .map((option) => {
                  ...
                  >
                    ...
                    <Switch
                      ...
                      onCheckedChange={(isChecked) =>
                        updateAudienceVisibility(option.key, isChecked)
                      }
      ...
      {item.errors.visibility && (
        <div className="mt-1 text-xs text-red" id={visibilityErrorId} role="alert">
          {item.errors.visibility}
        </div>
      )}
    </>
