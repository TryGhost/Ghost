...
const FontOption: React.FC<{ option: FontSelectOption; selected?: boolean }> = ({
  option,
  selected,
}) => (
  <span
    className={`flex w-full items-center gap-4 ${option.className ?? ''}`}
    data-testid={selected ? 'select-current-option' : 'select-option'}
    data-value={option.value}
  >
    <span className="flex size-12 shrink-0 items-center justify-center rounded-md border border-border-default bg-surface-elevated text-2xl font-bold shadow-xs">
      Aa
    </span>
    <span className="flex min-w-0 flex-col">
      <span className="text-md">{option.label}</span>
      <span className="truncate font-sans text-sm font-normal text-muted-foreground">
        {option.hint}
      </span>
    </span>
  </span>
...
const GlobalSettings: React.FC<{
  ...
  const [bodyFont, setBodyFont] = useState(
    CUSTOM_FONTS.heading.find((f) => f.name === values.bodyFont) || {
      name: DEFAULT_FONT,
      creator: themeNameVersion,
    },
  );
  ...
  customHeadingFonts.unshift({
    label: DEFAULT_FONT,
    value: DEFAULT_FONT,
    hint: themeNameVersion,
  ...
  const selectedHeadingFont =
    customHeadingFonts.find((option) => option.value === headingFont.name) || customHeadingFonts[0];
  const selectedBodyFont =
    customBodyFonts.find((option) => option.value === bodyFont.name) || customBodyFonts[0];
              ...
              {values.icon ? (
                <ImageUploadPreview background="checkerboard">
                  ...
                  <ImageUploadActions className="top-1 right-1">
                    <ImageUploadAction
                      aria-label="Remove publication icon"
                      data-testid="image-delete-button"
                      onClick={() => updateSetting('icon', null)}
                    >
                      <Trash2 />
                    </ImageUploadAction>
                  </ImageUploadActions>
                ...
                <ImageUploadDropzone
                  inputId="logo"
                  onDropAccepted={([file]) => void uploadSettingImage('icon', file)}
                >
                  Upload icon
                </ImageUploadDropzone>
              )}
            ...
            <ImageUpload className="h-15 w-40">
              {values.logo ? (
                <ImageUploadPreview background="checkerboard">
                  ...
                  <ImageUploadActions className="top-1 right-1">
                    <ImageUploadAction
                      aria-label="Remove publication logo"
                      data-testid="image-delete-button"
                ...
                <ImageUploadDropzone
                  inputId="site-logo"
                  onDropAccepted={([file]) => void uploadSettingImage('logo', file)}
                >
                  Upload logo
                </ImageUploadDropzone>
              )}
          ...
          <ImageUpload className="h-23.75 w-40">
            {values.coverImage ? (
              <ImageUploadPreview>
                ...
                <ImageUploadActions className="top-1 right-1">
                  ...
                  <ImageUploadAction
                    ...
                    data-testid="image-delete-button"
                    onClick={() => updateSetting('cover_image', null)}
                  >
                    <Trash2 />
                  </ImageUploadAction>
                </ImageUploadActions>
              ...
              <>
                <ImageUploadDropzone
                  inputId="cover"
                  onDropAccepted={([file]) => void uploadSettingImage('cover_image', file)}
                >
                  Upload cover
                </ImageUploadDropzone>
                {unsplashEnabled && (
