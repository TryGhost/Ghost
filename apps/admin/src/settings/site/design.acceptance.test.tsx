...
function fakeDesignWorld(
  themeSettings: CustomThemeSetting[] = [],
  { withPost = false } = {},
): {
...
describe('Design settings', () => {
  ...
  it('hides the design builder when the backend does not expose it', async () => {
    ...
    await expect(
      settingsScreen.design().getByRole('button', { name: 'Build with AI' }),
    ).toHaveCount(0);
    await expect
      .element(settingsScreen.design().getByRole('button', { name: 'Customize' }))
      .toBeVisible();
  });
  ...
  it('confirms before discarding unsaved theme-tab changes', async () => {
    const emailSignupText: CustomThemeSetting = {
  ...
  it('picks a cover image from the Unsplash photo grid', async () => {
    ...
    const downloadApi = fakeEndpoint('GET', unsplashPhoto.links.download_location, {
      url: testImage,
    });
    await renderAdminApp('/settings/design/edit');
    ...
    await expect
      .element(modal.getByTestId('publication-cover').getByTestId('image-upload-container'))
      .toBeVisible();
  ...
  it('previews and saves custom theme settings', async () => {
    ...
    await expect(homepagePreview).toHaveRequestedPreview({
      custom: JSON.stringify({ navigation_layout: 'Logo in the middle' }),
    });
    await modal.getByRole('button', { name: 'Save' }).click();
    await expect
      .poll(() => editThemeApi.lastRequest?.body)
      .toMatchObject({
  ...
  it('previews custom fonts and can restore theme defaults', async () => {
    ...
    await renderAdminApp('/settings/design/edit', {
      boot: {
        browseSettings: {
          response: settingsResponse({ settings: { heading_font: 'Cardo', body_font: 'Inter' } }),
        },
      },
    });
  ...
  it.each([{ supportsCustomFonts: true }, { supportsCustomFonts: false }])(
    'shows legacy theme font settings only when custom fonts are unsupported',
    async ({ supportsCustomFonts }) => {
      const titleFont: CustomThemeSetting = {
        id: 'title-font',
        key: 'title_font',
        type: 'select',
      ...
      const bodyFont: CustomThemeSetting = {
        id: 'body-font',
        key: 'body_font',
        type: 'select',
        options: ['Modern sans-serif', 'Elegant serif'],
