...
import { fakeEditSettings, fakeOffers, fakeSearchIndex, fakeSettingsScreens, offer, renderAdminApp, settingsResponse } from "@test-utils/acceptance";
...
describe("Navigation settings", () => {
    ...
    it("suggests membership links, offers and content in the URL dropdown", async () => {
        ...
        await expect.element(suggestions().getByRole("option", { name: /Gift subscriptions/ })).toBeInTheDocument();
        await expect.element(suggestions().getByRole("option", { name: /Tips and donations/ })).toBeInTheDocument();
        await expect.element(suggestions().getByRole("option", { name: /Offer — Black Friday/ })).toBeInTheDocument();
        await expect.element(suggestions().getByRole("option", { name: /^About/ })).toBeInTheDocument();
        await expect.element(suggestions().getByRole("option", { name: /^Welcome to Ghost/ })).toBeInTheDocument();
        ...
        await suggestions().getByRole("option", { name: /Gift subscriptions/ }).click();
    ...
    it("stores a picked page as a relative URL", async () => {
        ...
        await suggestions().getByRole("option", { name: /^About/ }).click();
        ...
        await expect(settingsApi).toHaveEditedSettings([
            { key: "navigation", value: '[{"url":"/","label":"Home"},{"url":"/about/","label":"About"},{"url":"/about/","label":"About us"}]' },
        ]);
    });
    ...
    it("commits an in-progress URL edit when saving with Cmd+S", async () => {
        ...
        await expect(settingsScreen.navigationModal()).toHaveCount(0);
        await expect(settingsApi).toHaveEditedSettings([
            { key: "navigation", value: '[{"url":"/contact/","label":"Home"},{"url":"/about/","label":"About"}]' },
        ]);
    });
