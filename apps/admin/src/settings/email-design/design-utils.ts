...
function resolveSectionTitleColor(
  value: string | null,
  accentColor: string,
  bgColor: string,
): string {
...
export function resolveAllColors(
  ...
  const headerTextColor =
    headerBgColor === 'transparent' ? textColor : textColorForBackgroundColor(headerBgColor).hex();
  const secondaryHeaderTextColor =
    headerBgColor === 'transparent'
      ? secondaryTextColor
      : textColorForBackgroundColor(headerBgColor).alpha(0.5).toString();
