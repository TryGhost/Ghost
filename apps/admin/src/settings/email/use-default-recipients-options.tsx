...
const useDefaultRecipientsOptions = (
  selectedOption: string,
  defaultEmailRecipientsFilter?: string | null,
) => {
  const tiers = useFilterableApi<Tier, 'tiers', 'name'>({
    path: '/tiers/',
    filterKey: 'name',
    responseKey: 'tiers',
  });
  const labels = useFilterableApi<Label, 'labels', 'name'>({
    path: '/labels/',
    filterKey: 'name',
    responseKey: 'labels',
  });
  const offers = useFilterableApi<Offer, 'offers', 'name'>({
    path: '/offers/',
    filterKey: 'name',
    responseKey: 'offers',
  });
  ...
  const offerOption = (offer: Offer): SegmentOption => ({
    value: `offer_redemptions:${offer.id}`,
    label: offer.name,
  });

  const loadOptions = async (input: string, callback: (options: SegmentOptions) => void) => {
    const [tiersData, labelsData, offersData] = await Promise.all([
      tiers.loadData(input),
      labels.loadData(input),
      offers.loadData(input),
    ]);

    const segmentOptionGroups: SegmentOptionGroup[] = [
      {
        label: 'Member status',
        options: SIMPLE_SEGMENT_OPTIONS.filter(({ label }) =>
      ...
      },
      {
        label: 'Active Tiers',
        options:
          tiersData.filter(({ active, type }) => active && type !== 'free').map(tierOption) || [],
      },
      {
  ...
  useEffect(() => {
    ...
    if (
      ...
      (hydrationState === 'idle' || filterChanged)
    ) {
