...
describe('Newsletter settings', () => {
  it('creates a newsletter and inserts it into the visible list', async () => {
    ...
    fakeAdminEndpoint(
      'GET',
      /^\/members\/\?.*filter=newsletters/,
      browseResponse('members', [], { limit: 1 }),
    );
    ...
    await expect.element(settingsScreen.newsletterModal()).toBeVisible();
    await expect
      .element(settingsScreen.newsletterModal().getByPlaceholder('Weekly Roundup'))
      .toHaveValue(created.name);
    ...
    await expect
      .element(settingsScreen.newsletters().getByText(created.name, { exact: true }))
      .toBeVisible();
  });
  ...
  it('validates and updates a newsletter in the visible list', async () => {
    const updated = {
      ...
      body_font_category: 'sans_serif' as const,
    };
    fakeNewsletterWorld();
  ...
  describe('email addresses', () => {
    ...
    it('restricts a managed sender address to the configured sending domain', async () => {
      ...
      await modal.getByRole('button', { name: 'Save' }).click();
      await expect
        .element(modal.getByText('Email address must end with @customdomain.com'))
        .toBeVisible();
  ...
  it('moves newsletters between archived and active lists after status changes', async () => {
    ...
    expect(reactivateApi.lastRequest?.body).toMatchObject({
      newsletters: [{ id: archivedNewsletter.id, status: 'active' }],
    });
    ...
    expect(archiveApi.lastRequest?.body).toMatchObject({
      newsletters: [{ id: activeNewsletter.id, status: 'archived' }],
    });
  });
