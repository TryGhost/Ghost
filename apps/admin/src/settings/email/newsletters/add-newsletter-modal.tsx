...
const AddNewsletterModal: React.FC = () => {
  ...
  const { data: members } = useBrowseMembers({
    searchParams: {
      filter: 'newsletters.status:active+email_disabled:0',
      limit: '1',
      page: '1',
  ...
  useEffect(() => {
    ...
    limiter
      .errorIfWouldGoOverLimit('newsletters')
      .catch((error) => {
    ...
    >
      <FieldGroup className="mt-10 gap-8">
        ...
        <Field orientation="horizontal">
          <FieldContent>
            <FieldLabel htmlFor="opt-in-existing-subscribers">
              Opt-in existing subscribers
            </FieldLabel>
            <FieldDescription>
          ...
          <Switch
            ...
            id="opt-in-existing-subscribers"
