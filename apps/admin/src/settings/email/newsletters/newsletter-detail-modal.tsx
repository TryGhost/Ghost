...
const ReplyToEmailField: React.FC<{
  ...
  return (
    <Field data-invalid={Boolean(errors.sender_reply_to) || undefined}>
      ...
      <Input
        aria-invalid={Boolean(errors.sender_reply_to) || undefined}
    ...
    if (newsletter.status === 'active') {
      confirm({
        ...
        prompt: (
          <>
            <div className="mb-6">
              Your newsletter <strong>{newsletter.name}</strong> will no longer be visible to
              members or available as an option when publishing new posts.
            </div>
        ...
        <>
          ...
          <div className="my-5 flex w-full items-start">
            ...
            <FieldGroup className="gap-8">
              <Field orientation="horizontal">
                <FieldContent>
                  <FieldLabel htmlFor="newsletter-show-badge">
                    Promote independent publishing
                  </FieldLabel>
                  <FieldDescription>
                    Show you&apos;re a part of the indie publishing movement with a small badge in
                    the footer
                  </FieldDescription>
                </FieldContent>
                <Switch
                  checked={Boolean(newsletter.show_badge)}
                  id="newsletter-show-badge"
                  onCheckedChange={(checked) => updateNewsletter({ show_badge: checked })}
                />
              </Field>
            </FieldGroup>
          </div>
...
const NewsletterDetailModalContent: React.FC<{ newsletter: Newsletter; onlyOne: boolean }> = ({
  ...
  const {
    ...
    savingDelay: 500,
    onSave: async () => {
      const { meta: { sent_email_verification: [emailToVerify] = [] } = {} } =
