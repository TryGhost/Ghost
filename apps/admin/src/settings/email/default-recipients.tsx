...
import useDefaultRecipientsOptions, {
  type SegmentOption,
  type SegmentOptions,
} from './use-default-recipients-options';
...
const flattenSegmentOptions = (options: SegmentOptions): SegmentOption[] =>
...
const DefaultRecipients: React.FC<{ keywords: string[] }> = ({ keywords }) => {
  ...
  const [selectedOption, setSelectedOption] = useState(
    getDefaultRecipientValue({
      defaultEmailRecipients,
      defaultEmailRecipientsFilter,
    }),
  );
  const selectedRecipientLabel = RECIPIENT_FILTER_OPTIONS.find(
  ...
  const {
    ...
    retrySelectedSegments,
    selectedSegments,
    setSelectedSegments,
  } = useDefaultRecipientsOptions(selectedOption, defaultEmailRecipientsFilter);
  ...
  useEffect(() => {
    if (selectedSegments) {
      setSegmentOptions((current) => {
        ...
        const missingSelected = selectedSegments.filter(
  ...
  const updateSelectedSegments = (values: string[]) => {
    const availableOptions = [
      ...(selectedSegments || []),
      ...flattenSegmentOptions(segmentOptions),
    ];
  ...
  const flattenedSegmentOptions = segmentOptions.flatMap((option) =>
    'options' in option
      ...
      <SettingGroupContent columns={1}>
        <Field>
          ...
          <Select value={selectedOption} onValueChange={setDefaultRecipientValue}>
            <SelectTrigger
              aria-label="Default Newsletter recipients"
              data-testid="default-recipients-select"
            >
              <SelectValue>{selectedRecipientLabel}</SelectValue>
            </SelectTrigger>
            <SelectContent>
        ...
        {selectedOption === 'segment' && (
          <Field>
                    ...
                    {selectedSegments.length
                      ? selectedSegments.map((option, index) => (
                          <React.Fragment key={option.value}>
                            <span>{option.label}</span>
                            {index < selectedSegments.length - 1 ? <span>,&nbsp;</span> : null}
                          </React.Fragment>
                        ))
                      : 'Select...'}
                  </ComboboxValue>
            ...
            {hydrationState === 'error' && (
              <FieldError>Could not load the saved filter. Try again.</FieldError>
            )}
          </Field>
        )}
      </SettingGroupContent>
