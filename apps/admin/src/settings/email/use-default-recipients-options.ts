...
/**
 * Segment options for the default-recipients picker, ported from the legacy
 * email/use-default-recipients-options.tsx: tier/label/offer lookups plus the
 * saved-filter hydration state machine (idle → loading → ready/error with
 * retry).
 */
...
// The legacy utils/helpers.ts isObjectId — anything else in a filter is a
// label/offer prefix or a status segment.
const isObjectId = (value: string) => /^[a-z0-9]{24}$/.test(value);

export function useDefaultRecipientsOptions(selectedOption: string, defaultEmailRecipientsFilter?: string | null) {
    ...
    const hydratingFilter = useRef<string | null | undefined>(undefined);
