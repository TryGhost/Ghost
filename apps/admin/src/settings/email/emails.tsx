...
const EmailsGroup: React.FC<{ keywords: string[]; newslettersEnabled: boolean }> = ({
  keywords,
  newslettersEnabled,
}) => {
  ...
  const [selectedTab, setSelectedTab] = useState<'newsletters' | 'transactional'>(
    newslettersEnabled ? 'newsletters' : 'transactional',
  );
  ...
  useEffect(() => {
    ...
    const verify = async () => {
      try {
        const { meta: { email_verified: emailVerified } = {} } = await verifySenderUpdate({
    ...
    >
      ...
      >
        <div className="flex items-center justify-between border-b border-grey-200 dark:border-grey-900">
          ...
          {newslettersEnabled && selectedTab === 'newsletters' && (
            <Select
              value={newslettersFilter}
              onValueChange={(value) => setNewslettersFilter(value as NewslettersFilter)}
            >
              <SelectTrigger
                className="h-7 w-auto gap-1 border-0 bg-transparent px-2 text-sm shadow-none hover:bg-muted focus:ring-0 focus:ring-offset-0"
                data-testid="newsletters-filter"
              >
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
