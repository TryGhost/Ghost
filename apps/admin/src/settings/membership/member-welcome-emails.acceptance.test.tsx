...
type AutomatedEmailFixture = Omit<
  typeof freeWelcomeEmail,
...
const previewResponse = (
  ...
  html = '<!doctype html><html><body><p>Preview content</p></body></html>',
) => ({
...
function fakeRecentPosts() {
  fakeAdminEndpoint('GET', /^\/posts\/\?filter=status%3Apublished&fields=/, {
    posts: [],
    meta: { pagination: { page: 1, limit: 5, pages: 0, total: 0, next: null, prev: null } },
  });
}
...
async function renderWelcomeEmails(
  emails: AutomatedEmailFixture[] = [freeWelcomeEmail],
...
async function openWelcomeEmailModal(
  emails: AutomatedEmailFixture[] = [freeWelcomeEmail],
  options?: RenderAdminAppOptions,
) {
...
describe('Member welcome emails', () => {
  ...
  it('keeps a raw subject template editable and saves it without rendered replacements', async () => {
    ...
    fakeAdminEndpoint(
      'POST',
      `/automated_emails/${templatedEmail.id}/preview/`,
      previewResponse('Welcome Jamie'),
    );
    const editApi = fakeAdminEndpoint('PUT', `/automated_emails/${templatedEmail.id}/`, {
      automated_emails: [templatedEmail],
    });
    await renderAdminApp('/settings/memberemails');
  ...
  it('creates a missing welcome-email row as active when enabled', async () => {
    ...
    fakeAutomatedEmails([]);
    const addApi = fakeAdminEndpoint('POST', '/automated_emails/', {
      automated_emails: [freeWelcomeEmail],
    });
    await renderAdminApp('/settings/memberemails');
  ...
  it('updates an inactive row to active', async () => {
    ...
    const editApi = fakeAdminEndpoint('PUT', `/automated_emails/${inactive.id}/`, {
      automated_emails: [freeWelcomeEmail],
    });
    await renderAdminApp('/settings/memberemails');
  ...
  it('shows and enables the paid welcome email row when Stripe is connected', async () => {
    ...
    const addApi = fakeAdminEndpoint('POST', '/automated_emails/', {
      automated_emails: [{ ...paidWelcomeEmail, status: 'active' }],
    });
    const stripe = settingsResponse({
      settings: {
    ...
    await renderAdminApp('/settings/memberemails', {
      boot: { browseSettings: { response: stripe } },
    });
  ...
  it("keeps the newest draft's preview when a stale preview response arrives late", async () => {
    const namedPreview = (text: string) =>
      previewResponse('Preview Subject', `<!doctype html><html><body><p>${text}</p></body></html>`);
    const modal = await openWelcomeEmailModal();
    ...
    const srcdoc =
      (
        settingsScreen.welcomeEmailPreviewIframe().query() as HTMLIFrameElement | null
      )?.getAttribute('srcdoc') ?? '';
  ...
  describe('customization', () => {
    ...
    it('shows and saves the publication icon choice only when an icon exists', async () => {
      ...
      const editDesignApi = fakeAdminEndpoint('PUT', '/automated_emails/design/', {
        automated_email_design: [automatedEmailDesign],
      });
      const addApi = fakeAdminEndpoint('POST', '/automated_emails/', {
        automated_emails: [paidWelcomeEmail],
      });
      const senderApi = fakeAdminEndpoint('PUT', '/automated_emails/senders/', {
        automated_emails: [freeWelcomeEmail, paidWelcomeEmail],
      });
      ...
      const iconSwitch = iconLabel
        .element()
        .parentElement?.querySelector<HTMLElement>('[role="switch"]');
    ...
    it('uses newsletter-derived placeholders and hides unavailable managed sender email', async () => {
      ...
      const settings = settingsResponse({
        settings: { default_email_address: 'test@example.com' },
      });
      await renderAdminApp('/settings/memberemails', {
      ...
      await expect
        ...
        .toHaveAttribute('placeholder', 'Sender');
      await expect(modal.getByLabelText('Sender email')).toHaveCount(0);
      await expect
        .element(modal.getByLabelText('Reply-to email'))
        .toHaveAttribute('placeholder', 'test@example.com');
    });
    ...
    it('uses an explicit newsletter reply-to address as the reply-to placeholder', async () => {
      ...
      await expect
        .element(modal.getByLabelText('Reply-to email'))
        .toHaveAttribute('placeholder', 'custom-reply@example.com');
