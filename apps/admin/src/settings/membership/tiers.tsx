...
const StripeConnectedButton: React.FC<{ className?: string; onClick: () => void }> = ({
  ...
  className = clsx('h-[34px] shrink-0 gap-2 px-3 font-semibold', className);
  return (
    <Button
      className={className}
...
const Tiers: React.FC<{ keywords: string[] }> = ({ keywords }) => {
  ...
  const defaultPaidTierCurrency =
    activeTiers.find((tier) => tier.type === 'paid' && tier.currency)?.currency || 'USD';
  ...
  const backendSupportsMachinePayments = settings?.some(
    (s) => s.key === 'machine_payments_enabled',
  );
  const canEnableMachinePayments = llmsEnabled && stripeEnabled;
  const machinePaymentsCurrency = (machinePaymentsCurrencyValue ||
    defaultPaidTierCurrency) as string;
  ...
  const amountInput = useCurrencyInput(machinePaymentsAmount, (cents) => {
    const validationError = validateCurrencyAmount(cents, machinePaymentsCurrency, {
      allowZero: false,
    });
  ...
  if (stripeEnabled) {
    content = (
      <Tabs value={selectedTab} variant="underline" onValueChange={setSelectedTab}>
        ...
        <TabsContent value="active-tiers">
          <TiersList tab="active-tiers" tiers={sortTiers(activeTiers)} />
        </TabsContent>
        <TabsContent value="archived-tiers">
          <TiersList tab="archive-tiers" tiers={sortTiers(archivedTiers)} />
        </TabsContent>
      </Tabs>
    );
  } else {
    content = (
      <TiersList tab="free-tier" tiers={activeTiers.filter((tier) => tier.type === 'free')} />
    );
  }
        ...
        <SettingGroupContent className="border-t border-border pt-6" columns={1}>
          <div className="flex items-start justify-between gap-4">
            ...
            <Switch
              ...
              onCheckedChange={(checked) => {
                saveMachinePaymentSettings([{ key: 'machine_payments_enabled', value: checked }]);
              }}
            />
          </div>
          ...
          {machinePaymentsEnabled && canEnableMachinePayments && (
            <Field
              ...
              data-invalid={Boolean(machinePaymentsAmountError) || undefined}
            >
              <FieldLabel htmlFor="machine-payments-amount">Price per post</FieldLabel>
              <FieldDescription>
                SPT/card charges use this currency. Tempo crypto charges the same minor units as
                USDC.
              </FieldDescription>
                      ...
                      <MultiSelectCombobox
                        ...
                        onChange={(values) => {
                          ...
                          setMachinePaymentsAmountError(
                            validateCurrencyAmount(machinePaymentsAmount, next, {
                              allowZero: false,
                            }),
                          );
