...
const Analytics: React.FC<{ keywords: string[] }> = ({ keywords }) => {
  ...
  const inputs = (
    <SettingGroupContent className="analytics-settings gap-y-0!" columns={1}>
      <Field
        className="py-4"
        data-disabled={!isWebAnalyticsAvailable || undefined}
        ...
        <FieldContent>
          ...
          <FieldDescription>
            {!isWebAnalyticsConfigured ? (
              ...
              <>
                Cookie-free, first party traffic analytics for your site, powered by{' '}
                <a
                  className="text-green"
                  href="https://ghost.org/integrations/tinybird"
        ...
        <div className="mb-5 rounded-md border border-grey-200 bg-grey-50 px-4 py-2.5 dark:border-grey-900 dark:bg-grey-900">
          <span className="flex items-start gap-2">
            <span>
              ...
              and requires configuration to start collecting data.{' '}
              <a
                className="font-medium text-green"
                href="https://docs.ghost.org/install/docker#tinybird-integration"
                rel="noopener noreferrer"
                target="_blank"
              >
                Get started &rarr;
      ...
      <Field className="py-4" orientation="horizontal">
        ...
        <Switch
          checked={Boolean(trackEmailOpens)}
          id="email-opens"
          onCheckedChange={(checked) => handleToggleChange('email_track_opens', checked)}
        />
      </Field>
      <Separator />
      <Field
        className="py-4"
        data-disabled={isEmailTrackClicksReadOnly || undefined}
        orientation="horizontal"
      >
        <FieldContent>
        ...
        <Switch
          checked={Boolean(trackEmailClicks)}
          disabled={isEmailTrackClicksReadOnly}
          id="email-clicks"
          onCheckedChange={(checked) => handleToggleChange('email_track_clicks', checked)}
        />
      </Field>
      <Separator />
      <Field className="py-4" orientation="horizontal">
        ...
        <Switch
          checked={Boolean(trackMemberSources)}
          id="member-sources"
          onCheckedChange={(checked) => handleToggleChange('members_track_sources', checked)}
        />
      </Field>
      <Separator />
      <Field className="py-4" orientation="horizontal">
        ...
        <Switch
          checked={Boolean(outboundLinkTagging)}
          id="outbound-link-tagging"
          onCheckedChange={(checked) => handleToggleChange('outbound_link_tagging', checked)}
        />
      </Field>
    </SettingGroupContent>
  );
  ...
  return (
    ...
    >
      ...
      <div className="items-center-mt-1 flex justify-between">
        <a
          className="font-medium text-green"
          href="https://ghost.org/help/post-analytics/"
          rel="noopener noreferrer"
          target="_blank"
        >
