...
import { type EmailDraft, getEmailValidationErrors } from "@/automations/components/email-modal/validation";

/**
 * Preview state for the welcome email modal, ported from the legacy
 * member-emails/use-welcome-email-preview.ts: validates the draft before
 * requesting, and only the latest request may update the preview (stale
...
export const useWelcomeEmailPreview = ({ automatedEmailId, previewWelcomeEmail, setErrors }: {
    ...
    previewWelcomeEmail: (payload: EmailDraft & { id: string }) => Promise<WelcomeEmailPreviewResponse>;
