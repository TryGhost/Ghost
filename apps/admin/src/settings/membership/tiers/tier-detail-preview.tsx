...
export const TrialDaysLabel: React.FC<{ size?: 'sm' | 'md'; trialDays: number }> = ({
  size = 'md',
  trialDays,
}) => {
  if (!trialDays) {
...
const DiscountLabel: React.FC<{ discount: number }> = ({ discount }) => {
  ...
  return (
    <span className="mt-1 leading-none font-semibold text-ghostaccent">
      {formatNumber(discount)}% discount
    </span>
  );
};
...
const TierDetailPreview: React.FC<TierDetailPreviewProps> = ({ tier, isFreeTier }) => {
  ...
  return (
    <div className="-mt-2" data-testid="tier-preview">
      <div className="flex items-start justify-between">
        ...
        {!isFreeTier && (
          <div className="flex gap-1">
            <Button
              className={
                showingYearly ? 'h-auto p-0 text-muted-foreground' : 'h-auto p-0 text-foreground'
              }
              type="button"
              variant="link"
              onClick={() => setShowingYearly(false)}
            >
              Monthly
            </Button>
            <Button
              className={
                showingYearly
                  ? 'ml-2 h-auto p-0 text-foreground'
                  : 'ml-2 h-auto p-0 text-muted-foreground'
              }
              type="button"
              variant="link"
              onClick={() => setShowingYearly(true)}
            >
              Yearly
            </Button>
          </div>
        ...
        <div className="flex-column relative flex min-h-[200px] w-full max-w-[420px] scale-90 items-start justify-stretch rounded bg-white p-4">
          <div className="min-h-[56px] w-full">
            ...
            <div className="mt-4 flex w-full flex-row flex-wrap items-end justify-between gap-x-1 gap-y-[10px]">
              <div
                className={`flex flex-wrap text-black ${((showingYearly && tier?.yearly_price === undefined) || (!showingYearly && tier?.monthly_price === undefined)) && !isFreeTier ? 'opacity-30' : ''}`}
              >
                <span className="self-start text-[2.7rem] leading-[1.115] font-bold uppercase">
                  {currencySymbol}
                </span>
                <span className="text-[3.4rem] leading-none font-bold tracking-tight break-all">
                  {formatNumber(showingYearly ? yearlyPrice : monthlyPrice, {
