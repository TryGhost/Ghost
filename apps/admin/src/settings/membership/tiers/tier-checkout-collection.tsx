...
/**
 * The countries a Stripe checkout can ship to. Mirrors the server's authoritative list
 * (stripe/services/checkout/allowed-countries.ts), which is measured against the live API
 * at Ghost's pinned version — not the SDK's `AllowedCountry` union, which wrongly omits
 * `SD`. Anything wider would let a publisher save a country Stripe refuses at session
 * create, which breaks checkout; anything narrower silently strips countries from an
 * "all countries" save.
 */
