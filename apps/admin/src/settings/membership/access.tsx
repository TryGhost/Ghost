...
const DEFAULT_PRELAUNCH_MESSAGE =
...
const Access: React.FC<{ keywords: string[] }> = ({ keywords }) => {
  ...
  const tierOptions = tierOptionGroups.flatMap((group) =>
    group.options.map((option) => ({ ...option, metadata: { group: group.label } })),
  );
  const selectedTierLabels = tierOptions
    .filter((option) => contentVisibilityTiers.includes(option.value))
    ...
    <SettingGroupContent className="gap-y-4" columns={1}>
      {isTrialMode && (
        <div className="-m-5 overflow-hidden p-5">
          <Banner
            className="mb-2 flex w-full cursor-default flex-col gap-4 border-0 p-6 pt-5 transition-none hover:translate-y-0 hover:scale-100 hover:shadow-[-7px_-6px_42px_8px_rgb(75_225_226_/_28%),7px_6px_42px_8px_rgb(202_103_255_/_32%)] md:flex-row md:items-center md:justify-between dark:hover:shadow-[-7px_-6px_42px_8px_rgb(75_225_226_/_36%),7px_6px_42px_8px_rgb(202_103_255_/_38%)]"
            size="lg"
            variant="gradient"
          >
            <div>
              <div className="text-base font-semibold">{preLaunchTitle}</div>
              <div className="mt-2 text-gray-700">{preLaunchMessage}</div>
            </div>
            <ShadeButton className="shrink-0 self-start md:self-center" asChild>
              <a href={preLaunchUpgradeUrl}>Upgrade now</a>
            </ShadeButton>
          </Banner>
        </div>
      )}
      ...
      <div className="flex flex-col content-center items-center gap-4 md:flex-row">
        ...
        <div className="w-full md:flex-1">
          <Field>
            ...
            >
              <SelectTrigger
                aria-label="Who should be able to subscribe to your site?"
                data-testid="subscription-access-select"
              >
                <SelectValue>
                  {getAccessOptionLabel(MEMBERS_SIGNUP_ACCESS_OPTIONS, membersSignupAccess)}
                </SelectValue>
              </SelectTrigger>
      ...
      <div className="flex flex-col content-center items-center gap-4 md:flex-row">
        ...
        <div className="w-full md:flex-1">
          <Field>
            ...
            <Select
              value={defaultContentVisibility}
              onValueChange={(value) => {
            ...
            >
              <SelectTrigger
                aria-label="Who should have access to new posts?"
                data-testid="default-post-access-select"
              >
                <SelectValue>
                  {getAccessOptionLabel(
                    DEFAULT_CONTENT_VISIBILITY_OPTIONS,
                    defaultContentVisibility,
                  )}
                </SelectValue>
              </SelectTrigger>
              <SelectContent>
                {renderAccessOptions(DEFAULT_CONTENT_VISIBILITY_OPTIONS)}
              </SelectContent>
            </Select>
          </Field>
        </div>
            ...
            <Field>
              ...
              <Combobox open={tiersOpen} onOpenChange={setTiersOpen}>
                <ComboboxTrigger aria-label="Select specific tiers" data-testid="tiers-select">
                  <ComboboxValue placeholder={!selectedTierLabels}>
                    {selectedTierLabels || 'Select...'}
                  </ComboboxValue>
                </ComboboxTrigger>
                <ComboboxContent>
                  <MultiSelectCombobox
      ...
      <div className="flex flex-col content-center items-center gap-4 md:flex-row">
        ...
        <div className="w-full md:flex-1">
          <Field>
            ...
            >
              <SelectTrigger aria-label="Who can comment on posts?" data-testid="commenting-select">
                <SelectValue>
                  {getAccessOptionLabel(COMMENTS_ENABLED_OPTIONS, commentsEnabled)}
                </SelectValue>
              </SelectTrigger>
