...
describe('Access settings', () => {
  ...
  it('regenerates a locked private-site access code server-side', async () => {
    ...
    const regenerateApi = fakeAdminEndpoint(
      'POST',
  ...
  it('shows the default pre-launch banner copy when public site access is disabled', async () => {
    ...
    await expect.element(section.getByText('Pre-launch mode', { exact: true })).toBeVisible();
    await expect
      .element(section.getByText(/During your free trial, a private access code is required/))
      .toBeVisible();
    await expect
      .element(section.getByRole('link', { name: 'Upgrade now' }))
      .toHaveAttribute('href', '#/pro/billing/plans');
  });
  ...
  it('uses configurable title, message, and link for the pre-launch banner', async () => {
    ...
    await expect
      .element(section.getByText('Your site is private while you evaluate the platform.'))
      .toBeVisible();
    await expect
      .element(section.getByRole('link', { name: 'Upgrade now' }))
      .toHaveAttribute('href', 'https://billing.example.com/upgrade');
  ...
  it('disables dependent settings when signup is disabled', async () => {
    ...
    await expect
      .element(settingsScreen.portal().getByRole('button', { name: 'Customize' }))
      .toBeDisabled();
    await expect
