...
/**
 * The Stripe-branded buttons, rebuilt locally from the legacy
 * admin-x-design-system StripeButton/StripeConnectedButton pair: the branded
 * purple button (the wordmark carries a "Connect with Stripe" sr-only label)
 * and the "Connected to Stripe" status button.
 */

const STRIPE_WORDMARK = (
...
export function StripeButton({ label, className, href, target, onClick }: StripeButtonProps) {
    const content = label ?? (
        <>
    ...
    if (href) {
        return (
            <a className={stripeButtonClasses(Boolean(label), className)} href={href} rel="noopener noreferrer" target={target}>
                {content}
            </a>
        );
    }
    ...
    return (
        <button className={stripeButtonClasses(Boolean(label), className)} type="button" onClick={onClick}>
            {content}
        </button>
    );
}

export function StripeConnectedButton({ className, onClick }: { className?: string; onClick: () => void }) {
    return (
        <button
            className={cn(
                "group flex shrink-0 items-center justify-center rounded border border-border px-3 py-1.5 font-semibold whitespace-nowrap text-foreground transition-all hover:border-border-strong",
                className,
            )}
            data-testid="stripe-connected"
            type="button"
            onClick={onClick}
        >
            <span className="inline-flex size-2 rounded-full bg-state-success transition-all group-hover:bg-[#625BF6]"></span>
