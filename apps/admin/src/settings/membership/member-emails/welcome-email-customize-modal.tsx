...
} from './default-welcome-email-values';
import {
  type GeneralSettings,
  type WelcomeEmailCustomizeFormState,
  buildAutomatedEmailDesignPayload,
  mapApiToDesignSettings,
    ...
    <section>
      ...
      <div className="flex flex-col gap-4">
        <div className="flex flex-col gap-1.5">
          <label className="font-medium" htmlFor="welcome-email-sender-name">
            Sender name
          </label>
          <Input
        ...
        {showSenderEmailInput ? (
          <div className="flex flex-col gap-1.5">
            <label className="font-medium" htmlFor="welcome-email-sender-email">
              Sender email
            </label>
            <Input
        ...
        <div className="flex flex-col gap-1.5">
          <label className="font-medium" htmlFor="welcome-email-reply-to-email">
            Reply-to email
          </label>
          <Input
    ...
    <section>
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="mt-2 flex flex-col gap-1.5">
          <label className="font-medium" htmlFor="welcome-email-footer">
            Email footer
          </label>
          <Textarea
...
const WelcomeEmailCustomizeModal: React.FC<{ onClose: () => void }> = ({ onClose }) => {
  ...
  const ensureWelcomeEmailRows = useCallback(async () => {
    const existingBySlug = new Map(
      (automatedEmailsData?.automated_emails || []).map((email) => [email.slug, email]),
    );
  ...
  const { formState, saveState, updateForm, setFormState, handleSave, okProps, errors } =
    useForm<WelcomeEmailCustomizeFormState>({
      initialState: {
      ...
      onSave: async (state) => {
        ...
        const { meta: { sent_email_verification: sentEmailVerification = [] } = {} } =
          await editAutomatedEmailSenders(senderPayload);
      ...
      onValidate: (state) => {
        ...
        if (showSenderEmailInput && senderEmail) {
          if (!validator.isEmail(senderEmail)) {
            ...
            senderEmailDomain &&
            senderEmail.split('@')[1]?.toLowerCase() !== senderEmailDomain.toLowerCase()
          ) {
  ...
  useEffect(() => {
    ...
    replyToEmailInput,
    senderEmailInput,
    senderInputsHydrated,
    senderNameInput,
    setFormState,
  ]);

  const handleDesignChange = useCallback(
    (updates: Partial<EmailDesignSettings>) => {
      setHasSaveError(false);
      toast.dismiss(SAVE_ERROR_TOAST_ID);
      updateForm((state) => ({
        ...state,
        ...
        preview={
          isError ? (
            ...
            <EmailPreview
              ...
              senderEmail={
                generalSettings.senderEmail || senderEmailPlaceholder || defaultEmailAddress || ''
              }
              senderName={
                generalSettings.senderName || senderNamePlaceholder || siteTitle || 'Your site'
              }
              settings={designSettings}
