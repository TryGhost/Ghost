...
const EmailPreviewModalContent = React.forwardRef<HTMLDivElement, EmailPreviewModalContentProps>(
  (
    { title, centeredHeaderContent, headerActions, children, className, isEditMode = false },
    ref,
  ) => (
    <div
      ref={ref}
    ...
    >
      <div className="sticky top-0 grid shrink-0 grid-cols-[1fr_auto_1fr] items-center border-b border-gray-200 bg-white px-5 py-3 dark:border-gray-900 dark:bg-gray-950">
        <h3 className="justify-self-start text-xl font-semibold">{title}</h3>
...
const EmailPreviewEmailHeader: React.FC<EmailPreviewEmailHeaderProps> = ({
  children,
  className,
}) => (
  <div
    className={cn(
      'relative isolate z-20 mx-auto w-full max-w-[780px] rounded-t-lg border border-b-0 border-gray-200 bg-white px-6 py-4 transition-[max-width,padding] duration-300 ease-out motion-reduce:transition-none dark:border-gray-900 dark:bg-gray-950',
      className,
    )}
  >
    {children}
...
const WelcomeEmailModal: React.FC<WelcomeEmailModalProps & { onClose: () => void }> = ({
  emailType = 'free',
  automatedEmail,
  onClose,
}) => {
  ...
  const { resolvedSenderName, resolvedSenderEmail, resolvedReplyToEmail, hasDistinctReplyTo } =
    ...
    >
        ...
        centeredHeaderContent={
          ...
          >
            <TabsList className="grid w-[240px] grid-cols-2 bg-gray-100 shadow-[inset_0_0_0_1px_rgba(0,0,0,0.04)]">
              <TabsTrigger
                className="w-full justify-center data-[state=active]:bg-white dark:data-[state=active]:bg-white dark:data-[state=active]:text-black"
      ...
      >
        <div className="flex grow flex-col items-center p-6">
          ...
          <EmailPreviewBody
            className={cn(
              mode === 'preview' && 'bg-white shadow-sm dark:bg-gray-950',
              mode === 'edit' && 'px-6',
          ...
          >
            ...
            {mode === 'preview' && <WelcomeEmailPreviewFrame previewState={previewFrameState} />}
          </EmailPreviewBody>
          {mode === 'edit' && errors.lexical && (
            <FieldError className="mt-2 max-w-[740px]">{errors.lexical}</FieldError>
          )}
        </div>
      </EmailPreviewModalContent>
