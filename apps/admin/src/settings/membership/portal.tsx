...
const SignupOptionImage: React.FC<{ color: string; title: string; price: string }> = ({
  title,
  color,
  price,
}) => {
  ...
  return (
    <TopLevelGroup
      customButtons={
        <Button
          className="mt-[-5px]"
          disabled={membersSignupAccess === 'none'}
          size="sm"
          type="button"
          variant="ghost"
        ...
        <div className="grid w-full max-w-[440px] grid-cols-3 gap-5 rounded-t-xl bg-white px-9 py-6 shadow-xl">
          <div className="col-span-3 mb-1 flex flex-col items-center justify-center">
            {icon ? (
              ...
              <div
                className="flex aspect-square items-center justify-center overflow-hidden rounded-full p-1 text-white"
                style={{
                  backgroundColor: color,
                }}
              >
