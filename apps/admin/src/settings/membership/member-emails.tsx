...
import WelcomeEmailModal, {
  type WelcomeEmailModalProps,
...
const EmailPreviewRow: React.FC<{
  ...
  return (
    <ActionListItem data-testid={`${emailType}-welcome-email-row`}>
        ...
        >
          ...
          <span className="min-w-0 grow">
            <span
              ...
              data-testid={`${emailType}-welcome-email-title`}
            >
              {title}
            </span>
      ...
      <ActionListItemActions>
        <div
          className={`flex items-center gap-7 ${isBusy && !isInitialLoading ? 'pointer-events-none' : ''}`}
        >
          {isInitialLoading ? (
...
const MemberEmails: React.FC<{ keywords: string[] }> = ({ keywords }) => {
  ...
  const { mutateAsync: addAutomatedEmail, isPending: isAddingAutomatedEmail } =
    useAddAutomatedEmail();
  const { mutateAsync: editAutomatedEmail, isPending: isEditingAutomatedEmail } =
    useEditAutomatedEmail();
  ...
  return (
    ...
    >
      ...
      {editingEmail && (
        <DialogPortal>
          <WelcomeEmailModal
            key={editingEmail.automatedEmail.id}
            {...editingEmail}
