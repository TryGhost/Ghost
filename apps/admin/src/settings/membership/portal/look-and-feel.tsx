      ...
      <FieldGroup className="mb-10 gap-8">
        <Field orientation="horizontal">
          ...
          <Switch
            ...
            id="show-portal-button"
            onCheckedChange={(checked) => updateSetting('portal_button', checked)}
          />
        </Field>
        ...
        {portalButton && (
          <>
            <Field>
              ...
              <Select
                value={portalButtonStyle as string}
                onValueChange={(value) => updateSetting('portal_button_style', value)}
              >
                <SelectTrigger aria-label="Button style">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  {portalButtonOptions.map((option) => (
            ...
            {portalButtonStyle?.toString()?.includes('icon') && (
              <Stack gap="sm">
                ...
                <Grid
                  className="w-full gap-px rounded-(--input-group-radius) bg-muted p-0.5"
                  columns={6}
                  gap="none"
                >
                  <ToggleGroup
                  ...
                  >
                    {defaultButtonIcons.map((iconConfig) => (
                      <ToggleGroupItem
                        ...
                        className="group h-[46px] w-full min-w-0 rounded-[calc(var(--radius-md)+1px)] p-3"
                        value={iconConfig.value}
                      >
                        <BrandIcon
                          className="size-[18px] opacity-70 transition-opacity group-hover:opacity-100 group-data-[state=on]:opacity-100"
                  ...
                  <ImageUpload
                    className={`h-[46px] w-full min-w-0 overflow-visible rounded-[calc(var(--radius-md)+1px)] ${currentIcon === uploadedIcon ? 'bg-background shadow-sm' : ''}`}
                  >
                    {uploadedIcon ? (
                      ...
                      <ImageUploadDropzone
                        ...
                        className="size-full rounded-[calc(var(--radius-md)+1px)] border-0 bg-transparent p-0 hover:bg-surface-elevated"
                        inputAriaLabel="Upload Portal icon"
