...
import {Field, FieldError, FieldGroup, FieldLabel, Input, Switch} from '@tryghost/shade/components';
...
const AccountPage: React.FC<{
    ...
    const [portalAccountGiftPromotion] = getSettingValues(localSettings, ['portal_account_gift_promotion']);
    const showAccountGiftPromotionSetting = config.labs.giftSubCustomization === true
    ...
    return <div className='mt-7'><FieldGroup className='mb-10 gap-8'>
        {showAccountGiftPromotionSetting && <Field orientation='horizontal'>
            <FieldLabel htmlFor='portal-account-gift-promotion'>Display option to purchase gift</FieldLabel>
            <Switch checked={Boolean(portalAccountGiftPromotion)} id='portal-account-gift-promotion' onCheckedChange={checked => updateSetting('portal_account_gift_promotion', checked)} />
        </Field>}
