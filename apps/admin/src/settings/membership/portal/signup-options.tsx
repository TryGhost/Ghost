...
const SignupOptions: React.FC<{
    ...
    const [membersSignupAccess, portalName, portalSignupGiftPromotion, portalSignupTermsHtml, portalSignupCheckboxRequired, portalPlansJson, portalDefaultPlan] = getSettingValues(
        localSettings, ['members_signup_access', 'portal_name', 'portal_signup_gift_promotion', 'portal_signup_terms_html', 'portal_signup_checkbox_required', 'portal_plans', 'portal_default_plan']
    );
    ...
    const showSignupGiftPromotionSetting = config.labs.giftSubCustomization === true
    ...
    return <div className='mt-7'><FieldGroup className='mb-10 gap-8'>
        ...
        {showSignupGiftPromotionSetting && <Field orientation='horizontal'>
            <FieldLabel htmlFor='portal-signup-gift-promotion'>Display option to purchase gift</FieldLabel>
            <Switch checked={Boolean(portalSignupGiftPromotion)} id='portal-signup-gift-promotion' onCheckedChange={checked => updateSetting('portal_signup_gift_promotion', checked)} />
        </Field>}
        ...
        <HtmlField
            ...
            hint={errors.portal_signup_terms_html || <>Recommended: <strong>{formatNumber(signupTermsMaxLength)}</strong> characters. You&apos;ve used <strong className="text-green">{formatNumber(signupTermsLength)}</strong></>}
            nodes='MINIMAL_NODES'
