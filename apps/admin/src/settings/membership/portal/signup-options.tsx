...
const SignupOptions: React.FC<{
  ...
  const arePaidTiersVisible =
    isStripeEnabled &&
    paidActiveTiers.length > 0 &&
    paidActiveTiers.some((tier) => tier.visibility === 'public');
        ...
        <FieldSet>
          ...
          <FieldGroup data-slot="checkbox-group">
            {tiersCheckboxes.map((checkbox) => (
              <Field
                key={checkbox.value}
                data-disabled={checkbox.disabled || undefined}
          ...
          <>
            <FieldSet>
              ...
              <FieldGroup data-slot="checkbox-group">
                {['monthly', 'yearly'].map((plan) => (
                  <Field
                    key={plan}
                    data-disabled={!isSignupAllowed || undefined}
                    orientation="horizontal"
                  >
                    <Checkbox
                    ...
                    />
                    <FieldLabel className="capitalize" htmlFor={`portal-plan-${plan}`}>
                      {plan}
                    </FieldLabel>
                  </Field>
                ))}
            ...
            {portalPlans.includes('yearly') && portalPlans.includes('monthly') && (
              <Field>
                ...
                <Select
                  value={typeof portalDefaultPlan === 'string' ? portalDefaultPlan : ''}
                  onValueChange={(value) => updateSetting('portal_default_plan', value)}
                >
                  <SelectTrigger aria-label="Default price at signup">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    {defaultPlanOptions.map((option) => (
        ...
        {portalSignupTermsHtml?.toString() && (
          <Field data-disabled={!isSignupAllowed || undefined} orientation="horizontal">
            ...
            <Switch
              ...
              disabled={!isSignupAllowed}
              id="portal-require-agreement"
              onCheckedChange={(checked) =>
                updateSetting('portal_signup_checkbox_required', checked)
              }
            />
          </Field>
        )}
