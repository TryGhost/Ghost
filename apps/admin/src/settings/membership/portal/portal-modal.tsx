...
const portalSidebarTabClassName =
  'border-b-2 border-transparent after:hidden data-[state=active]:border-foreground';
...
const Sidebar: React.FC<{
  ...
  return (
    <div className="pt-4">
      ...
      >
        <TabsList>
          <TabsTrigger className={portalSidebarTabClassName} value="signupOptions">
            Signup options
          </TabsTrigger>
          <TabsTrigger className={portalSidebarTabClassName} value="lookAndFeel">
            Look & feel
          </TabsTrigger>
          <TabsTrigger className={portalSidebarTabClassName} value="accountPage">
            Account page
          </TabsTrigger>
        </TabsList>
...
const PortalModal: React.FC = () => {
  ...
  useEffect(() => {
    const checkToken = async ({ token }: { token: string }) => {
      try {
        ...
        const [supportEmail] = getSettingValues<string>(verifiedSettings, [
        ...
        confirm({
          ...
          prompt: (
            <>
              Your support email address has been changed to <strong>{supportEmail}</strong>.
            </>
          ),
          okLabel: 'Close',
          cancelLabel: '',
  ...
  const { formState, setFormState, saveState, handleSave, updateForm, okProps } = useForm<{
    settings: Dirtyable<Setting>[];
    tiers: Dirtyable<Tier>[];
  }>({
    initialState: {
      settings,
      tiers: allTiers || [],
    ...
    onSave: async () => {
      ...
      setFormState((state) => ({
        ...state,
        tiers: formState.tiers.map((tier) => ({ ...tier, dirty: false })),
      }));
      ...
      setFormState((state) => ({
        ...state,
        settings: formState.settings.map((setting) => ({ ...setting, dirty: false })),
      }));

      if (meta?.sent_email_verification) {
        const newEmail = formState.settings.find(
          (setting) => setting.key === 'members_support_address',
        )?.value;
        const currentEmail =
          currentSettings.find((setting) => setting.key === 'support_email_address')?.value ||
          fullEmailAddress(
            currentSettings
              .find((setting) => setting.key === 'members_support_address')
              ?.value?.toString() || 'noreply',
            siteData,
            config,
          );
        ...
        confirm({
          ...
          prompt: (
            <>
              We&apos;ve sent a confirmation email to <strong>{newEmail}</strong>. Until verified,
              your support email address will remain {currentEmail}.
            </>
          ),
          okLabel: 'Close',
          cancelLabel: '',
  ...
  const previewTabs = (
    <Tabs
      ...
      variant="button"
      onValueChange={(value) => onSelectURL(value as PreviewTab)}
    >
      <TabsList>
