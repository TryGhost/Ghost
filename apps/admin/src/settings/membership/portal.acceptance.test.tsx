...
describe("Portal settings", () => {
    ...
    it("saves the independent signup and account gift promotion settings", async () => {
        ...
        await expect.poll(() => getPreviewParam(preview.element() as HTMLIFrameElement, "signupGiftPromotion")).toBe("true");
        await signupGiftPromotion.click();
        await expect.poll(() => getPreviewParam(preview.element() as HTMLIFrameElement, "signupGiftPromotion")).toBe("false");
        ...
        await expect.poll(() => getPreviewParam(preview.element() as HTMLIFrameElement, "accountGiftPromotion")).toBe("true");
        await accountGiftPromotion.click();
        await expect.poll(() => getPreviewParam(preview.element() as HTMLIFrameElement, "accountGiftPromotion")).toBe("false");
        await modal.getByRole("button", {name: "Save"}).click();
    ...
    it("hides gift promotion settings when the backend does not provide them", async () => {
        ...
        settings.settings = settings.settings.filter(({key}) => ![
