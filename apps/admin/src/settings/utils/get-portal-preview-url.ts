...
export const getPortalPreviewUrl = ({
  ...
  selectedTab,
  previewTheme,
}: portalPreviewUrlTypes): string | null => {
  if (!siteData?.url) {
    return null;
  }
  ...
  const portalPlans = JSON.parse(
    getSettingValue<string>(settings, 'portal_plans') || '',
  ) as string[];
  ...
  settingsParam.append(
    'isMonthly',
    checkStripeEnabled(settings, config) && portalPlans.includes('monthly') ? 'true' : 'false',
  );
  settingsParam.append(
    'isYearly',
    checkStripeEnabled(settings, config) && portalPlans.includes('yearly') ? 'true' : 'false',
  );
  settingsParam.append('page', selectedTab === 'account' ? 'accountHome' : 'signup');
  settingsParam.append('previewTheme', previewTheme);
  settingsParam.append(
    'buttonIcon',
    encodeURIComponent(getSettingValue(settings, 'portal_button_icon') || 'icon-1'),
  );
  settingsParam.append(
    'signupButtonText',
    encodeURIComponent(getSettingValue(settings, 'portal_button_signup_text') || ''),
  );
  settingsParam.append(
    'membersSignupAccess',
    getSettingValue(settings, 'members_signup_access') || 'all',
  );
  settingsParam.append(
    'allowSelfSignup',
    getSettingValue<string>(settings, 'allow_self_signup') || 'false',
  );
  settingsParam.append(
    'signupTermsHtml',
    getSettingValue(settings, 'portal_signup_terms_html') || '',
  );
  settingsParam.append(
    'signupCheckboxRequired',
    getSettingValue(settings, 'portal_signup_checkbox_required') ? 'true' : 'false',
  );
  ...
  if (getSettingValue<boolean>(settings, 'transistor')) {
    settingsParam.append(
      ...
      JSON.stringify({
        ...
        description:
          getSettingValue<string>(settings, 'transistor_portal_description') ||
          'Access your RSS feeds',
        button_text: getSettingValue<string>(settings, 'transistor_portal_button_text') || 'Manage',
        url_template:
          getSettingValue<string>(settings, 'transistor_portal_url_template') ||
