...
export const getPortalPreviewUrl = ({settings, config, tiers, siteData, selectedTab, previewTheme} : portalPreviewUrlTypes): string | null => {
    ...
    settingsParam.append('signupGiftPromotion', getSettingValue(settings, 'portal_signup_gift_promotion') ? 'true' : 'false');
    settingsParam.append('accountGiftPromotion', getSettingValue(settings, 'portal_account_gift_promotion') ? 'true' : 'false');
