...
const useSearchService = () => {
    ...
    const highlightKeywords = (text: ReactNode): ReactNode => {
        ...
        if (typeof text === 'string') {
            ...
            return (text as React.ReactNode[]).map(part => highlightKeywords(part));
        } else if (text && typeof text === 'object' && text) {
            return React.Children.map(text, (child) => {
                if (child && typeof child === 'object' && 'props' in child) {
                    return highlightKeywords((child as React.ReactElement<{children?: React.ReactNode}>).props.children);
                }
                return child;
            });
        } else {
