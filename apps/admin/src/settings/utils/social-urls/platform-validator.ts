...
export const createPlatformValidator = (
  ...
  const { domains, www, regionalSubdomain, pathTypes, fullPath, transformUsername, errors } =
    definition;

  const compiledPathTypes: CompiledSocialPathType[] = pathTypes.map((pathType) => ({
    ...pathType,
    check: compileUsernameRule(pathType.rule),
  }));

  // longest prefix first so 'channel/' wins over '@' and '' matches last
  const urlPathTypes = [...compiledPathTypes].sort(
    (a, b) => b.urlPrefix.length - a.urlPrefix.length,
  );
  const handlePrefixes = compiledPathTypes
    .flatMap((pathType) =>
      [pathType.storagePrefix, ...(pathType.handleAliases ?? [])]
  ...
  const parseHandle = (input: string) => {
    ...
    if (match) {
      return {
        ...
        rawUsername: cleaned.slice(match.prefix.length),
        atPrefixConsumed: !match.pathType.tolerateLeadingAt,
      };
    }
  ...
  const extractParts = (
    ...
    region?: string;
    pathType: CompiledSocialPathType;
    rawUsername: string;
    atPrefixConsumed: boolean;
  } => {
    if (!isUrlInput(input)) {
    ...
    // pub/name/12/34/567)
    const rawUsername =
      !fullPath && !pathType.rule.nestedSegments ? fullRawUsername.split('/')[0] : fullRawUsername;
    ...
    return {
      ...
      pathType,
      rawUsername,
      atPrefixConsumed: pathType.urlPrefix !== '' && !pathType.tolerateLeadingAt,
    };
  };
