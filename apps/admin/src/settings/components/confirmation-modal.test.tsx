...
describe('ConfirmationModal', () => {
  ...
  it('renders the supplied content and confirms without closing implicitly', async () => {
    ...
    expect(screen.getByRole('heading', { name: 'Delete newsletter?' })).toHaveClass(
      'font-semibold',
    );
    expect(screen.getByRole('heading', { name: 'Delete newsletter?' })).not.toHaveClass(
      'font-bold',
    );
    expect(screen.getByRole('button', { name: 'Keep it' })).toHaveClass(
      'border-control-border',
      'bg-transparent',
    );
    expect(screen.getByRole('button', { name: 'Delete' }).parentElement).not.toHaveClass(
      '[&>button]:min-w-20',
    );
  ...
  it('disables both actions and shows the running label while confirming', async () => {
    ...
    expect(runningButton.firstElementChild).toHaveClass(
      'animate-spin',
      'border-current/20',
      'before:bg-current',
    );
