...
import {type EditorResource, type KoenigInstance, type KoenigLexicalModule, loadKoenig} from './koenig-loader';
...
interface KoenigWrapperProps extends KoenigEditorBaseProps {
    ...
    children: (koenig: KoenigLexicalModule) => React.ReactNode
...
export const KoenigWrapper: React.FC<KoenigWrapperProps> = ({
    ...
    const koenig = useMemo(() => new Proxy({} as KoenigLexicalModule, {
        get: (_target, prop) => {
            return editor.read()[prop as keyof KoenigLexicalModule];
        }
    }), [editor]);
