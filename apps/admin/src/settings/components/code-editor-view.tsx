...
const CodeEditorView = forwardRef<ReactCodeMirrorRef, CodeEditorProps>(function CodeEditorView(
  ...
  useEffect(() => {
    ...
    Promise.all(extensions.map((extension) => Promise.resolve(extension))).then(
      (nextExtensions) => {
        if (!cancelled) {
  ...
  return (
    <>
      ...
      <div className={height === 'full' ? 'h-full' : ''} style={{ width }}>
        ...
        />
        {title && (
          <FieldLabel className="order-1" htmlFor={id}>
            {title}
          </FieldLabel>
        )}
        {hint && (
          <FieldDescription className={clsx('order-3 mt-1', error && 'text-destructive')}>
            {hint}
          </FieldDescription>
        )}
      </div>
