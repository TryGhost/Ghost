...
describe('Staff roles', () => {
  it('groups staff under role tabs with accurate counts', async () => {
    ...
    await expect
      .element(section.getByTestId('owner-user').getByText('Owner User', { exact: false }))
      .toBeVisible();
    await expect
  ...
  it('lets Editors edit only Contributors without exposing role management', async () => {
    ...
    const editApi = fakeAdminEndpoint(
      'PUT',
      `/users/${contributor.id}/?include=roles`,
      ({ body }) => body,
    );
    await renderAdminApp('/settings/staff', { boot });
    ...
    for (const [tab, email] of [
      ['Administrators', administrator.email],
      ['Editors', otherEditor.email],
