...
describe('SEO meta settings', () => {
  ...
  it('re-enables persisted-disabled LLM structured data', async () => {
    ...
    await renderAdminApp('/settings', {
      boot: {
        browseSettings: { response: settingsResponse({ settings: { llms_enabled: false } }) },
      },
    });
  ...
  it('edits the X card', async () => {
    ...
    await expect
      .poll(() => document.getElementById('twitter-image')?.getAttribute('src'))
      .toBe('http://example.com/image.png');
  ...
  it('navigates between card tabs', async () => {
    ...
    await expect
      .element(tabs.getByRole('tab', { name: 'Search' }))
      .toHaveAttribute('aria-selected', 'true');
    await expect.element(section.getByLabelText('Meta title')).toBeVisible();

    await section.getByRole('tab', { name: 'Facebook card' }).click();
    await expect
      .element(tabs.getByRole('tab', { name: 'Facebook card' }))
      .toHaveAttribute('aria-selected', 'true');
    ...
    await section.getByRole('tab', { name: 'X card' }).click();
    await expect
      .element(tabs.getByRole('tab', { name: 'X card' }))
      .toHaveAttribute('aria-selected', 'true');
  ...
  it('deletes an existing Facebook card image', async () => {
    ...
    await renderAdminApp('/settings', {
      boot: {
        browseSettings: {
          response: settingsResponse({ settings: { og_image: 'http://example.com/original.png' } }),
        },
      },
    });
    ...
    await expect
      .poll(() => document.getElementById('facebook-image')?.getAttribute('src'))
      .toBe('http://example.com/original.png');
