...
} from '@/settings/utils/social-urls';
...
const getSocialUrls = (localSettings: Setting[] | null) => {
  ...
  return Object.fromEntries(
    SOCIAL_PLATFORM_CONFIGS.map((config, index) => {
...
const SocialAccounts: React.FC<{ keywords: string[] }> = ({ keywords }) => {
  ...
  const backendSupportsNewPlatforms = NEW_PLATFORM_KEYS.some((key) =>
    localSettings?.some((s) => s.key === key),
  );
  ...
  const handleSaveClick = () => {
    const formErrors = visiblePlatforms.reduce<Partial<Record<SocialPlatformKey, string>>>(
      (current, config) => {
