...
describe('Staff security settings', () => {
  it.each(['Owner', 'Administrator'] as const)(
    'shows email 2FA controls to %ss',
    async (roleName) => {
      ...
      await expect
        .element(section.getByText('Require email 2FA codes to be used on all staff logins'))
        .toBeVisible();
  ...
  it('does not show email 2FA controls to Editors', async () => {
    ...
    await expect(
      settingsScreen.users().getByText('Security settings', { exact: true }),
    ).toHaveCount(0);
  });
