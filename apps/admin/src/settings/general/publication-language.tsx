...
const PublicationLanguage: React.FC<{ keywords: string[] }> = ({ keywords }) => {
  ...
  const isCustomValue = Boolean(
    publicationLanguage && !localeOptions.some((option) => option.value === publicationLanguage),
  );
  ...
  const selectedLocale = localeOptionsWithOther.find(
    (option) => option.value === publicationLanguage,
  );

  React.useEffect(() => {
    if (
      publicationLanguage &&
      localeOptions.some((option) => option.value === publicationLanguage)
    ) {
    ...
    isOtherSelected || isCustomValue ? (
      <>
        <Field data-invalid={Boolean(errors.publicationLanguage || validationError) || undefined}>
          ...
          />
          {validationError || errors.publicationLanguage ? (
            <FieldError>{validationError || errors.publicationLanguage}</FieldError>
          ) : (
            <FieldDescription>Enter a custom locale code.</FieldDescription>
          )}
        </Field>
        <Button
      ...
      <Field data-invalid={Boolean(errors.publicationLanguage) || undefined}>
        ...
        {errors.publicationLanguage ? (
          <FieldError id={languageErrorId}>{errors.publicationLanguage}</FieldError>
        ) : (
  ...
  return (
    <TopLevelGroup
      ...
      onSave={handleSave}
    >
      <SettingGroupContent columns={1}>{languageField}</SettingGroupContent>
    </TopLevelGroup>
  );
};
