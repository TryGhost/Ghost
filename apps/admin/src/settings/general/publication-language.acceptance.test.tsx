...
describe('Publication language settings', () => {
  ...
  it('shows a validation error for an invalid custom locale', async () => {
    ...
    await settingsScreen
      .publicationLanguage()
      .getByLabelText('Site language')
      .fill('invalid--locale');
    await settingsScreen.publicationLanguage().getByRole('button', { name: 'Save' }).click();

    await expect
      .element(settingsScreen.publicationLanguage().getByText('Invalid locale format'))
      .toBeVisible();
  ...
  it('displays a stored custom locale', async () => {
    ...
    await expect
      .element(settingsScreen.publicationLanguage().getByLabelText('Site language'))
      .toHaveValue('cy');
  });
