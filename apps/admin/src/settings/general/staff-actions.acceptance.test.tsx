...
describe('Staff actions', () => {
  it('suspends an active user', async () => {
    ...
    await expect
      .element(
        settingsScreen.userDetailModal().getByText('Author User (Suspended)', { exact: true }),
      )
      .toBeVisible();
    expect(editApi.lastRequest?.body).toMatchObject({
      users: [{ email: author.email, status: 'inactive' }],
    });
  });
  ...
  it('deletes a user and returns to the staff list', async () => {
    ...
    await expect
      .element(settingsScreen.confirmationModal().getByText(`#${author.slug}`, { exact: false }))
      .toBeVisible();
