...
const TimeZone: React.FC<{ keywords: string[] }> = ({ keywords }) => {
  ...
  const timezoneOptions: Array<{ value: string; label: string }> = timezoneDataWithGMTOffset().map(
    (tzOption: TimezoneDataDropdownOption) => {
      return {
        value: tzOption.name,
      ...
      <SettingGroupContent columns={1}>
        <Field>
          ...
          <Combobox open={timezoneOpen} onOpenChange={setTimezoneOpen}>
            <ComboboxTrigger aria-label="Site timezone" data-testid="timezone-select">
              <ComboboxValue>{selectedTimezone?.label}</ComboboxValue>
            </ComboboxTrigger>
            <ComboboxContent>
              <MultiSelectCombobox
