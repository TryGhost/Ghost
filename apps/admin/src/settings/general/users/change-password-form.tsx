...
const ChangePasswordForm: React.FC<{ user: User }> = ({ user }) => {
  ...
  const savePassword = async () => {
    ...
    try {
      ...
      setSaveState('');
      toast.error(
        e instanceof ValidationError ? e.message : `Couldn't update password. Please try again.`,
      );
  ...
  const form = (
    <>
      {isCurrentUser && (
        <Field data-invalid={Boolean(errors.oldPassword) || undefined}>
          <FieldLabel htmlFor="old-password">Old password</FieldLabel>
          <Input
            ref={oldPasswordRef}
            aria-invalid={Boolean(errors.oldPassword) || undefined}
      ...
      <Field data-invalid={Boolean(errors.confirmNewPassword) || undefined}>
        ...
        <Input
          ...
          aria-invalid={Boolean(errors.confirmNewPassword) || undefined}
  ...
  return <>{editPassword ? form : initialView}</>;
};
