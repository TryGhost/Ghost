...
const UserDetailModalContent: React.FC<{
  user: User;
  ...
  const {
    ...
    onValidate: (values) => {
      return Object.entries(validators).reduce<ErrorMessages>((newErrors, [key, validate]) => {
        ...
        const isUnchangedSocialField =
          (SOCIAL_PLATFORM_KEYS as readonly string[]).includes(key) &&
      ...
      prompt: (
        <>
          <p className="mb-3">
            ...
            deleted and all their posts will be automatically assigned to{' '}
  ...
  const canMakeOwner =
    isOwnerUser(currentUser) && isAdminUser(formState) && formState.status !== 'inactive';
      ...
      <div>
        <div className={`relative ${isStandalonePage ? '-mx-10 -mt-10' : '-mx-8 -mt-8 rounded-t'}`}>
          <div
            className={clsx(
              ...
              isStandalonePage && formState.cover_image && 'min-h-[30vmin]',
            )}
            style={{
          ...
          >
            <div
              className={clsx(
                ...
                isStandalonePage && standaloneContentClasses,
              )}
            >
              <div className="flex flex-nowrap items-start justify-between gap-3">
                <div>
                  <ImageUpload className="-ml-1 size-20 overflow-visible rounded-full">
                    {formState.profile_image ? (
                      <ImageUploadPreview className="rounded-full">
                        ...
                        <ImageUploadActions className="top-1 right-1">
                          ...
                          <ImageUploadAction
                            ...
                            className="rounded-full"
                            onClick={() => handleImageDelete('profile_image')}
                          >
                            <Trash2 />
                          </ImageUploadAction>
                        </ImageUploadActions>
                      ...
                      <ImageUploadDropzone
                        className="rounded-full bg-surface-inverse text-surface-inverse-foreground opacity-80 hover:opacity-100"
                        inputId="avatar"
                        inputTestId="profile-image-upload"
                        onDropAccepted={(files) =>
                          void handleImageUpload('profile_image', files[0])
                        }
                      >
                ...
                <div className="flex flex-nowrap items-start gap-3">
                  {formState.cover_image ? (
                    <div className="flex flex-nowrap items-end justify-end gap-4">
                      <img
                        alt=""
                        className="hidden"
                      ...
                      <Button
                        className={coverButtonClasses}
                        type="button"
                        onClick={() => handleImageDelete('cover_image')}
                      >
                        Delete cover image
                      </Button>
                    </div>
                    ...
                    <Dropzone
                      ...
                      variant="button"
                      onDropAccepted={(files) => void handleImageUpload('cover_image', files[0])}
                    >
                      Upload cover image
                    </Dropzone>
                  )}
              ...
              <div>
                <Text
                  as="h3"
                  className={clsx(
                    'break-words md:text-2xl md:break-normal',
                    formState.cover_image ? 'text-white' : 'text-foreground',
                  )}
                  leading="heading"
                  size="xl"
                  weight="bold"
        ...
        <div className={clsx('mt-6 flex flex-col', isStandalonePage && standaloneContentClasses)}>
          <Tabs value={selectedTab} variant="underline" onValueChange={handleTabChange}>
            <TabsList>
              <TabsTrigger title="Profile" value="profile">
                Profile
              </TabsTrigger>
            ...
            <TabsContent className="pt-4" value="social-links">
              <SocialLinksTab
                clearError={clearError}
                errors={errors}
                setUserData={setUserData}
...
const UserDetailModal: React.FC = () => {
  ...
  const { data: fetchedUserData, error } = useGetUserBySlug(slug || '', {
    enabled: !isCurrentUser && !!slug,
  ...
  const hasResolvedMissingUser =
    !isCurrentUser && !!slug && !user && (isNotFoundError || fetchedUserData !== undefined);
  ...
  return displayUser ? (
    <UserDetailModalContent user={displayUser} onDeletingUserChange={setIsDeletingUser} />
  ) : null;
};
