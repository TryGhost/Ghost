...
describe('Staff profiles', () => {
  it('validates required and bounded profile fields without saving', async () => {
    ...
    const editApi = fakeAdminEndpoint(
      'PUT',
      `/users/${administrator.id}/?include=roles`,
      ({ body }) => body,
    );
  ...
  it('normalizes Facebook and X handles and rejects unrelated URLs', async () => {
    ...
    await expect
      .element(
        modal.getByText('The URL must be in a format like https://www.facebook.com/yourPage'),
      )
      .toBeVisible();
    ...
    await modal.getByTitle('Social Links').click();
    await expect
      .element(modal.getByText('Your Username is not a valid Twitter Username'))
      .toBeVisible();
    await x.fill('ghost');
    await modal.getByTitle('Social Links').click();
  ...
  it('saves core profile fields and the server response', async () => {
    ...
    const editApi = fakeAdminEndpoint('PUT', `/users/${administrator.id}/?include=roles`, {
      users: [saved],
    });
    await renderAdminApp(`/settings/staff/${administrator.slug}`, { boot });
  ...
  it('uploads profile picture and cover image and saves their URLs', async () => {
    ...
    const uploadApi = fakeAdminEndpoint('POST', '/images/upload/', () => {
      const url =
        uploadCount === 0 ? 'http://example.com/profile.png' : 'http://example.com/cover.png';
      uploadCount += 1;
    ...
    const modal = settingsScreen.userDetailModal();
    await modal
      .getByTestId('profile-image-upload')
      .upload(new File(['fake-profile-image'], 'profile.png', { type: 'image/png' }));
    await expect
      .element(modal.getByTestId('profile-image-preview'))
      .toHaveAttribute('src', 'http://example.com/profile.png');

    await modal
      .getByTestId('cover-image-upload')
      .upload(new File(['fake-cover-image'], 'cover.png', { type: 'image/png' }));
    await expect
  ...
  it('shows and regenerates a staff token only on your own profile', async () => {
    ...
    const regenerateApi = fakeAdminEndpoint('PUT', '/users/me/token/', {
      apiKey: { ...apiKey, secret: 'new-secret' },
    });
  ...
  it("does not expose the staff token on another user's profile", async () => {
    ...
    await expect
      .element(settingsScreen.userDetailModal().getByLabelText('Email', { exact: true }))
      .toBeVisible();
  ...
  it('reports a missing user and returns to the staff list', async () => {
    ...
    fakeAdminEndpoint(
      'GET',
      '/users/slug/unknown-user/?include=roles',
      {
...
const socialCases = [
  {
    ...
    invalid: 'https://www.notthreads.com',
    error: 'The URL must be in a format like https://www.threads.net/@yourUsername',
    valid: 'https://www.threads.net/@username',
    field: 'threads',
    stored: '@username',
  },
  {
    ...
    invalid: 'https://notbluesky.com',
    error: 'The URL must be in a format like https://bsky.app/profile/yourUsername',
    valid: 'https://bsky.app/profile/username',
    field: 'bluesky',
    stored: 'username',
  },
  {
    ...
    invalid: 'https://notlinkedin.com',
    error: 'The URL must be in a format like https://www.linkedin.com/in/yourUsername',
    valid: 'https://www.linkedin.com/in/yourUsername',
    field: 'linkedin',
    stored: 'yourUsername',
  },
  {
    ...
    invalid: 'https://twitter.com/johnsmith',
    error: 'The URL must be in a format like https://www.instagram.com/yourUsername',
    valid: 'https://www.instagram.com/yourUsername',
    field: 'instagram',
    stored: 'yourUsername',
  },
  {
    ...
    valid: 'https://www.youtube.com/@yourUsername',
    field: 'youtube',
    stored: '@yourUsername',
  },
  {
    label: 'TikTok',
    testId: 'tiktok-input',
    invalid: 'https://www.tik.com/nottiktok',
    error: 'The URL must be in a format like https://www.tiktok.com/@yourUsername',
    valid: 'https://www.tiktok.com/@yourUsername',
    field: 'tiktok',
    stored: '@yourUsername',
  },
  {
    ...
    invalid: 'https://mastodon.social/johnsmith',
    error:
    ...
    valid: '@johnsmith@mastodon.social',
    field: 'mastodon',
    stored: 'mastodon.social/@johnsmith',
  },
] as const;

describe('Staff profile social links', () => {
  it.each(socialCases)(
    'validates and normalizes $label URLs',
    async ({ testId, invalid, error, valid, field, stored }) => {
      const owner = user('Owner');
      const administrator = user('Administrator');
      const { boot } = fakeStaffWorld({ currentUser: owner, users: [owner, administrator] });
      const editApi = fakeAdminEndpoint(
