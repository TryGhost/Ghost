...
import { useConfig, useSettingsCurrentUser, useSettings } from '@/settings/hooks/use-settings-data';
...
const Owner: React.FC<OwnerProps> = ({ user }) => {
  ...
  return (
    <div
      className={clsx('group flex gap-3', hasAdminAccess(currentUser) && 'cursor-pointer')}
      ...
      <div className="flex flex-col">
        <span>
          ...
          {hasAdminAccess(currentUser) && (
            <Button
              className="ml-2 h-auto p-0 text-green group-hover:visible hover:text-green md:invisible"
              type="button"
              variant="link"
              onClick={(event) => {
...
const Users: React.FC<{ keywords: string[]; highlight?: boolean }> = ({
  keywords,
  highlight = true,
}) => {
  const {
  ...
  return (
    ...
    >
        ...
        >
          <TabsList>
            <TabsTrigger value="administrators">
              Administrators
              {adminUsers.length > 0 && (
                <TabsTriggerCount>{formatNumber(adminUsers.length)}</TabsTriggerCount>
              )}
            </TabsTrigger>
            <TabsTrigger value="editors">
              Editors
              {editorUsers.length > 0 && (
                <TabsTriggerCount>{formatNumber(editorUsers.length)}</TabsTriggerCount>
              )}
            </TabsTrigger>
            <TabsTrigger value="authors">
              Authors
              {authorUsers.length > 0 && (
                <TabsTriggerCount>{formatNumber(authorUsers.length)}</TabsTriggerCount>
              )}
            </TabsTrigger>
            <TabsTrigger value="contributors">
              Contributors
              {contributorUsers.length > 0 && (
                <TabsTriggerCount>{formatNumber(contributorUsers.length)}</TabsTriggerCount>
              )}
            </TabsTrigger>
            <TabsTrigger value="invited">
              Invited
              {totalInvites > 0 && (
                <TabsTriggerCount>{formatNumber(totalInvites)}</TabsTriggerCount>
              )}
            </TabsTrigger>
          </TabsList>
          <TabsContent value="administrators">
            <UsersList groupname="administrators" users={adminUsers} />
          </TabsContent>
          <TabsContent value="editors">
            <UsersList groupname="editors" users={editorUsers} />
          </TabsContent>
          <TabsContent value="authors">
            <UsersList groupname="authors" users={authorUsers} />
          </TabsContent>
          <TabsContent value="contributors">
            <UsersList groupname="contributors" users={contributorUsers} />
          </TabsContent>
          <TabsContent value="invited">
            <InvitesUserList users={invites} />
          </TabsContent>
        </Tabs>
      )}
      ...
      {config?.security?.staffDeviceVerification && hasAdminAccess(currentUser) && (
        <div
          className={`flex flex-col gap-6 ${users.length > 1 || invites.length > 0 ? '-mt-6' : ''}`}
        >
          <Separator />
