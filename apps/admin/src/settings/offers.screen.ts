...
export const offersScreen = {
  ...
  manageOffersButton: () =>
    page.getByTestId(testIds.section).getByRole('button', { name: names.manageOffersButton }),
  ...
  retentionRedemptionsLink: (cadence: 'monthly' | 'yearly') =>
    page.getByTestId(testIds.retentionRedemptionsLink(cadence)),
  newOfferButton: () =>
    page.getByTestId(testIds.listModal).getByRole('button', { name: names.newOfferButton }),

  addModal: () => page.getByTestId(testIds.addModal),
  addSidebar: () => page.getByTestId(testIds.addSidebar),
  publishButton: () =>
    page.getByTestId(testIds.addModal).getByRole('button', { name: names.publishButton }),
