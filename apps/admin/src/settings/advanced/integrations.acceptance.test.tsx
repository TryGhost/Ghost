...
const customIntegrationsLimit = {
  customIntegrations: { disabled: true, error: 'Your plan does not support custom integrations' },
};
...
function customIntegration() {
  return integration({
    ...
    name: 'My integration',
    api_keys: [
      apiKey('admin-api-secret'),
      { ...apiKey('content-api-secret'), id: 'content-key-id', type: 'content' },
    ],
  });
}
...
describe('Advanced integrations', () => {
  it('creates, edits, and deletes a custom integration with dirty-state protection', async () => {
    ...
    const createApi = fakeAdminEndpoint('POST', /^\/integrations\/\?include=/, {
      integrations: [custom],
    });
    const editApi = fakeAdminEndpoint('PUT', /^\/integrations\/custom-id\/\?include=/, {
      integrations: [{ ...custom, description: 'Test description' }],
    });
  ...
  it('regenerates a custom integration Admin API key', async () => {
    ...
    const refreshApi = fakeAdminEndpoint('POST', /^\/integrations\/.+\/api_key\/.+\/refresh\/$/, {
      integrations: [{ ...custom, api_keys: [apiKey('new-api-key')] }],
    });
    await renderAdminApp('/settings/integrations');
  ...
  it('creates, edits, and deletes custom integration webhooks', async () => {
    ...
    const editWebhookApi = fakeAdminEndpoint('PUT', '/webhooks/webhook-id/', {
      webhooks: [{ ...webhook, name: 'Updated webhook' }],
    });
  ...
  it('blocks custom integration creation when the host limit is active', async () => {
    ...
    await settingsScreen
      ...
      .getByRole('button', { name: 'Add custom integration' })
      .click();
    await expect
  ...
  it('shows only Unsplash as active in the initial integration list', async () => {
    ...
    await expect
      .element(section.getByTestId('unsplash-integration').getByText('Active', { exact: true }))
      .toBeVisible();
  });
  ...
  it("sends the upgrade CTA to a host's own billing app when one is configured", async () => {
    fakeSettingsScreens();
    await renderAdminApp('/settings/integrations', {
      boot: { browseConfig: { response: limitedConfig('#/pro/billing/plans') } },
    });
  ...
  it('uploads both Pintura assets and stores their returned URLs', async () => {
    ...
    const uploadApi = fakeAdminEndpoint('POST', '/files/upload/', () => {
      const url =
        uploadCount === 0 ? 'http://example.com/pintura-umd.js' : 'http://example.com/pintura.css';
      uploadCount += 1;
    ...
    await page
      .elementLocator(inputs[0])
      .upload(new File(['script'], 'pintura-umd.js', { type: 'text/javascript' }));
    await expect(settingsApi).toHaveEditedSettings([
    ...
    await page
      .elementLocator(inputs[1])
      .upload(new File(['styles'], 'pintura.css', { type: 'text/css' }));
    await expect(settingsApi).toHaveEditedSettings([
  ...
  it('shows and regenerates the Zapier Admin API key', async () => {
    ...
    const refreshApi = fakeAdminEndpoint('POST', /^\/integrations\/.+\/api_key\/.+\/refresh\/$/, {
      integrations: [{ ...zapier, api_keys: [apiKey('new-api-key')] }],
    });
    await renderAdminApp('/settings/integrations');
