...
const SpamFilters: React.FC<{ keywords: string[] }> = ({ keywords }) => {
  ...
  const [initialBlockedEmailDomainsJSON] = getSettingValues(localSettings, [
    'blocked_email_domains',
  ]) as string[];
  const initialBlockedEmailDomains = JSON.parse(initialBlockedEmailDomainsJSON || '[]') as string[];
  const [blockedEmailDomains, setBlockedEmailDomains] = React.useState(
    initialBlockedEmailDomains.join('\n'),
  );
  ...
  const hint = (
    <>
      Prevent unwanted signups by blocking email domains. Add one domain per line, e.g.,{' '}
      <code>spam.xyz</code> to block signups from email addresses like <code>hello@spam.xyz</code>.
    </>
  );
    ...
    >
      <SettingGroupContent columns={1}>
        <Field data-invalid={Boolean(errors.blockedEmailDomains) || undefined}>
          ...
          />
          {errors.blockedEmailDomains ? (
            <FieldError>{errors.blockedEmailDomains}</FieldError>
          ) : (
