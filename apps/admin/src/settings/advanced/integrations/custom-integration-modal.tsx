...
import {
  ...
  useEditIntegration,
} from '@tryghost/admin-x-framework/api/integrations';
          ...
          <ImageUpload className="size-25">
            {formState.icon_image ? (
              <ImageUploadPreview>
                ...
                <ImageUploadActions>
                  <ImageUploadAction
                    ...
                    onClick={() => updateForm((state) => ({ ...state, icon_image: null }))}
                  >
                    <Trash2 />
                  </ImageUploadAction>
                </ImageUploadActions>
              ...
              <ImageUploadDropzone
                ...
                inputId="custom-integration-icon"
                onDropAccepted={([file]) => void handleIconUpload(file)}
              >
                Upload icon
              </ImageUploadDropzone>
            )}
            ...
            keys={[
              {
                ...
                hint: contentKeyRegenerated ? (
                  <div className="text-green">Content API Key was successfully regenerated</div>
                ) : undefined,
                onRegenerate: () =>
                  contentApiKey && handleRegenerate(contentApiKey, setContentKeyRegenerated),
              },
              {
                id: 'admin-api-key',
                label: 'Admin API key',
                text: adminApiKey?.secret,
                hint: adminKeyRegenerated ? (
                  <div className="text-green">Admin API Key was successfully regenerated</div>
                ) : undefined,
                onRegenerate: () =>
                  adminApiKey && handleRegenerate(adminApiKey, setAdminKeyRegenerated),
              },
              {
                id: 'api-url',
                label: 'API URL',
