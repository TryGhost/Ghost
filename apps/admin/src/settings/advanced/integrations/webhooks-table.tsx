    ...
    <Stack gap="none">
      <Table>
        <TableHeader>
          <TableRow>
            <TableHead>
              {formatNumber(integration.webhooks?.length || 0)}{' '}
              {integration.webhooks?.length === 1 ? 'webhook' : 'webhooks'}
            </TableHead>
            ...
            >
              ...
              <TableCell className="w-1/4 py-3 pr-6 text-sm">
                {webhook.last_triggered_at &&
                  new Date(webhook.last_triggered_at).toLocaleString('default', {
                    weekday: 'short',
                    month: 'short',
                    day: 'numeric',
                    year: 'numeric',
      ...
      <Inline className="py-4" justify="center">
        <Button size="sm" type="button" variant="outline" onClick={showAddWebhookModal}>
          <LucideIcon.Plus /> Add webhook
        </Button>
      </Inline>
      <Separator />
