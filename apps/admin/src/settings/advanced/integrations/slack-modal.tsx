...
function SlackModal() {
  ...
  const { localSettings, updateSetting, handleSave, validate, errors, clearError, okProps } =
    useSettingGroup({
      onValidate: () => {
  ...
  const [slackUrl, slackUsername] = getSettingValues<string>(localSettings, [
    'slack_url',
    'slack_username',
  ]);
    ...
    >
      ...
      <div className="mt-7">
        <FieldSet className="gap-0">
          <FieldLegend className="mb-3 text-md! leading-supertight font-bold md:text-lg!">
            Slack configuration
          </FieldLegend>
          <FieldGroup className="gap-8 rounded-sm border border-border-default p-4 md:p-7">
            <Field data-invalid={Boolean(errors.slackUrl) || undefined}>
              ...
              <Input
                aria-invalid={Boolean(errors.slackUrl) || undefined}
              ...
              />
              {errors.slackUrl ? (
                <FieldError>{errors.slackUrl}</FieldError>
              ) : (
                <FieldDescription>
                  <>
                    Automatically send newly published posts to a channel in Slack or any
                    Slack-compatible service like Discord or Mattermost. Set up a new incoming
                    webhook{' '}
                    <a href="https://my.slack.com/apps/new/A0F7XDUAZ-incoming-webhooks">here</a>,
                    and grab the URL.
                  </>
                </FieldDescription>
              )}
            ...
            <div className="flex w-full flex-col gap-2 md:flex-row md:items-center">
              ...
              <Button type="button" variant="outline" onClick={() => void handleTestClick()}>
