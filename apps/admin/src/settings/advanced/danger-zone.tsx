  ...
  const handleResetAuth = () => {
    confirm({
      ...
      prompt: (
        <>
          <p className="mb-4">
            This rotates every API key on your site. Any integration using one will stop working
      ...
      onOk: async (modal) => {
        try {
          ...
          toast.success(
            `Rotated ${formatNumber(keys)} API ${keys === 1 ? 'key' : 'keys'} and locked ${formatNumber(users)} ${users === 1 ? 'user' : 'users'}. You will be signed out shortly.`,
          );
          modal?.remove();
      ...
      <ActionList>
        ...
        <ActionListItem data-testid="reset-all-authentication" hover={false}>
          <ActionListItemContent className="py-3 pr-6">
            ...
            <div className="text-sm text-muted-foreground">
              Rotate every API key, sign out every staff user, and require a password reset. Use
              after a suspected credential compromise.
            </div>
          </ActionListItemContent>
          <ActionListItemActions>
            <Button
              aria-label="Reset all authentication"
              size="sm"
              type="button"
              variant="destructive"
