  ...
  for (let x = 4; x < window.innerWidth; x += 24) {
    for (let y = 4; y < window.innerHeight; y += 24) {
      ...
      if (element && !element.closest('#modal-backdrop')) {
        hits.push(
          `${x},${y}: <${element.tagName.toLowerCase()} id="${element.id}" data-testid="${element.getAttribute('data-testid') ?? ''}">`,
        );
      }
    }
  }
...
describe('Advanced settings', () => {
  ...
  it('deletes all content after confirmation', async () => {
    ...
    await settingsScreen
      .section('dangerzone')
      .getByRole('button', { name: 'Delete all content' })
      .click();
    await settingsScreen
      .confirmationModal()
  ...
  it('resets authentication after confirmation', async () => {
    ...
    await settingsScreen
      .section('dangerzone')
      .getByRole('button', { name: 'Reset all authentication' })
      .click();
    await settingsScreen
      .confirmationModal()
      .getByRole('button', { name: 'Reset all authentication' })
  ...
  it('resets all gift links and reports the affected count', async () => {
    ...
    await settingsScreen
      .section('dangerzone')
      .getByTestId('reset-all-gift-links')
  ...
  it.each([
    ...
    await expect(settingsApi).toHaveEditedSettings([
      { key: 'blocked_email_domains', value: saved },
    ]);
  });
  ...
  it.each([
    ...
    await renderAdminApp('/settings/spam-filters', {
      boot: {
        browseSettings: {
          response: advancedSettings({ blocked_email_domains: '["initial.xyz","junk.com"]' }),
        },
      },
    });
    ...
    const section = settingsScreen.section('spam-filters');
    await expect
      .element(section.getByLabelText('Blocked email domains'))
      .toHaveValue('initial.xyz\njunk.com');
    await section.getByLabelText('Blocked email domains').fill(input);
    await section.getByRole('button', { name: 'Save' }).click();
  ...
  it('imports content from the universal importer and confirms the import is queued', async () => {
    ...
    await settingsScreen
      .section('migrationtools')
      .getByRole('button', { name: 'Universal import' })
      .click();
    ...
    await page
      ...
      .upload(new File(['{}'], 'content.json', { type: 'application/json' }));
  ...
  it('enables the automations beta only after confirmation', async () => {
    ...
    await expect
      .poll(
        () => settingsApi.lastRequest?.settings.find((setting) => setting.key === 'labs')?.value,
      )
  ...
  it.each([
    {
      kind: 'redirects',
      uploadPath: '/redirects/upload/',
      downloadPath: '/redirects/download/',
      filename: 'redirects.yml',
    },
    {
      kind: 'routes',
      uploadPath: '/settings/routes/yaml/',
      downloadPath: '/settings/routes/yaml/',
      filename: 'routes.yml',
    },
  ])(
    'uploads and downloads $kind from Labs',
    async ({ kind, uploadPath, downloadPath, filename }) => {
      fakeSettingsScreens();
  ...
  it.each([
    {
      kind: 'redirects',
      modalTestId: 'modal-redirects-editor',
      heading: 'Redirects',
      uploadPath: '/redirects/upload/',
      downloadPath: '/redirects/download/',
      yaml: '301:\n  /old/: /new/\n',
    ...
    {
      kind: 'routes',
      modalTestId: 'modal-routes-editor',
      heading: 'Routes',
      uploadPath: '/settings/routes/yaml/',
      downloadPath: '/settings/routes/yaml/',
      yaml: 'routes:\n  /about/: about\n',
    ...
    'edits $kind inline from Labs',
    async ({ kind, modalTestId, heading, uploadPath, downloadPath, yaml, successText }) => {
      fakeSettingsScreens();
      const uploadApi = fakeAdminEndpoint('POST', uploadPath, {});
      fakeAdminEndpoint('GET', downloadPath, new TextEncoder().encode(yaml).buffer, {
  ...
  it('shows a validation error and keeps the editor open when saving invalid redirects', async () => {
    ...
    fakeAdminEndpoint(
      'GET',
      '/redirects/download/',
      new TextEncoder().encode('301:\n  /old/: /new/\n').buffer,
      { contentType: 'text/yaml' },
    );
    fakeAdminEndpoint(
      'POST',
      '/redirects/upload/',
      { errors: [{ type: 'BadRequestError', message: errorMessage }] },
      { status: 400 },
    );
    await renderAdminApp('/settings/labs');
  ...
  it('browses and filters history using the expected NQL', async () => {
    ...
    const actions = [
      ...
      ...['setting-1', 'setting-2'].map((id, index) => ({
        id,
        resource_id: 'setting',
        ...
        created_at: `2023-08-11T12:3${index}:02.000Z`,
        actor,
        resource: { id: 'setting', slug: 'navigation' },
      })),
      ...['post-1', 'post-2'].map((id, index) => ({
        ...
        created_at: `2023-08-11T12:2${index}:02.000Z`,
        actor,
        resource: { id: 'post', slug: 'clunkers', title: 'The Clunkers Hall of Shame' },
      })),
    ];
    const actionsApi = fakeActions(({ filter }) =>
      filter?.includes('post')
        ? actions.filter((action) => action.resource_type !== 'post')
        : actions,
    );
    ...
    await expect
      .poll(() => new URL(actionsApi.requests.at(-1)!.url).searchParams.get('filter'))
      .toContain('actor_id:');
    ...
    const indicatorGap =
      dropdownIndicator!.getBoundingClientRect().left -
