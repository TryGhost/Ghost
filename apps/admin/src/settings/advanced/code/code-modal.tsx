...
const CodeModal: React.FC<CodeModalProps> = ({ onClose }) => {
  const { localSettings, handleSave, updateSetting } = useSettingGroup();

  const [headerContent, footerContent] = getSettingValues<string>(localSettings, [
    'codeinjection_head',
    'codeinjection_foot',
  ]);
      ...
      <div className="flex h-full flex-col">
        <div className="mb-4 flex items-center justify-between">
          ...
          <Inline gap="md">
            ...
            <Button disabled={isSaving} type="button" onClick={() => void onSaveClick()}>
              {savingTitle}
            </Button>
          </Inline>
        </div>
