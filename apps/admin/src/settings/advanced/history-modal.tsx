...
import {
  ...
  isBulkAction,
  useBrowseActions,
} from '@tryghost/admin-x-framework/api/actions';
import {
  ActionList,
  ActionListItem,
...
const HistoryAvatar: React.FC<{ action: Action }> = ({ action }) => {
  const actorName = action.actor?.name || action.actor?.slug;
...
const HistoryFilter: React.FC<{
  ...
  useEffect(() => {
    ...
    void usersApiRef.current
      .loadInitialValues([userId], 'id')
      .then(([user]) => {
        if (!cancelled && user) {
      ...
      <Popover>
        ...
        <PopoverContent align="end" className="z-[9999] w-[220px]" data-testid="history-filters">
          <Stack gap="2xl">
            <Stack gap="md">
              <HistoryFilterToggle
                excludedItems={excludedEvents}
                item="added"
                label="Added"
                toggleItem={toggleEventType}
              />
              <HistoryFilterToggle
                excludedItems={excludedEvents}
                item="edited"
        ...
        >
          ...
          >
            <PopoverTrigger asChild>
              <button
                aria-label="Staff"
                className="flex min-w-0 flex-1 items-center justify-between px-3 text-control"
                data-testid="history-staff-filter"
                role="combobox"
                type="button"
              >
                <span
                  className={
                    searchedStaff ? 'truncate pr-8' : 'truncate pr-8 text-muted-foreground'
                  }
                >
                  {searchedStaff?.label ?? 'Search staff'}
                </span>
            ...
            <PopoverContent align="start" className="z-[9999] w-72 p-0">
              <MultiSelectCombobox
                ...
                options={
                  searchedStaff &&
                  !staffOptions.some((option) => option.value === searchedStaff.value)
                    ? [searchedStaff, ...staffOptions]
                    : staffOptions
                }
                shouldFilter={false}
          ...
          {searchedStaff && (
            <button
              ...
              className="absolute top-1/2 right-10 z-10 flex size-8 -translate-y-1/2 items-center justify-center text-muted-foreground hover:text-foreground"
              type="button"
              onClick={() => {
                resetStaff();
...
const HistoryActionDescription: React.FC<{ action: Action }> = ({ action }) => {
  ...
  if (
    action.resource_type === 'security_action' &&
    action.context?.action_name === 'reset_authentication'
  ) {
    const apiKeysRotated =
      typeof action.context.api_keys_rotated === 'number' ? action.context.api_keys_rotated : null;
    const usersLocked =
      typeof action.context.users_locked === 'number' ? action.context.users_locked : null;
    const details = [
      apiKeysRotated !== null
        ? `${formatNumber(apiKeysRotated)} API ${apiKeysRotated === 1 ? 'key' : 'keys'} rotated`
        : null,
      usersLocked !== null
        ? `${formatNumber(usersLocked)} ${usersLocked === 1 ? 'user' : 'users'} locked`
        : null,
    ].filter(Boolean);
...
const formatDateForFilter = (date: Date) => {
  ...
  const parts = partsList.reduce<Record<string, string>>(
...
function HistoryModal() {
  ...
  const { data, fetchNextPage, isFetchingNextPage } = useBrowseActions({
    ...
    getNextPageParams: (lastPage, otherParams) => ({
      ...
      filter: [
        otherParams.filter,
        lastPage.actions.length &&
          `created_at:<'${formatDateForFilter(new Date(lastPage.actions[lastPage.actions.length - 1].created_at))}'`,
      ].join('+'),
    }),
  ...
  const toggleValue = (
    setter: (fn: (values: string[]) => string[]) => void,
    value: string,
    included: boolean,
  ) => {
    setter((values) =>
      included ? values.concat(value) : values.filter((current) => current !== value),
    );
  };
    ...
    <SettingsModal
      ...
      topRightContent={
        <HistoryFilter
          ...
          toggleResourceType={(resource, included) =>
            toggleValue(setExcludedResources, resource, !included)
          }
          userId={userId}
        />
      }
                        ...
                        <ActionListItemContent className="flex items-center gap-3 py-3">
                          ...
                          <div className="min-w-0 grow">
                            <div>
                              ...
                              {isBulkAction(action) ? '' : ': '}
                              {!isBulkAction(action) && (
                                <HistoryActionDescription action={action} />
                              )}
                            ...
                            <div className="text-sm text-muted-foreground">
                              {[
                                new Date(action.created_at).toLocaleDateString('default', {
                                  year: 'numeric',
                                  month: 'short',
                                  day: '2-digit',
                                }),
                                new Date(action.created_at).toLocaleTimeString('default', {
                                  hour: '2-digit',
                                  minute: '2-digit',
                                  second: '2-digit',
                                }),
