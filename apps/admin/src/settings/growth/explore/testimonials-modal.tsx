...
const TestimonialsModal = () => {
  ...
  const { formState, updateForm, saveState, handleSave, errors, clearError } = useForm<FormState>({
    ...
    onSave: async (): Promise<void> => {
      const prevPlatform = ['none', 'other', ''].includes(formState.prev_platform)
        ? undefined
        : formState.prev_platform;
      const payload = {
        ghost_uuid: siteUuid,
            ...
            <div>
              ...
              <div className="mt-8">
                <Field data-invalid={Boolean(errors.content) || undefined}>
                  <FieldLabel className="sr-only" htmlFor="testimonial-content">
                    Quote
                  </FieldLabel>
                  <Textarea
              ...
              <div className="mt-4 ml-0.5">
                <div className="flex items-center gap-2">
                  <Avatar
                    className="size-10"
                    colorSeed={staffUserName || staffUserEmail || undefined}
                    initials={
                      staffUserName || staffUserEmail
                        ? getMemberInitials({ name: staffUserName, email: staffUserEmail })
                        : undefined
                    }
              ...
              <div className="mt-2 flex items-center gap-4">
                <div className="grow">
                  <Field data-invalid={Boolean(errors.prev_platform) || undefined}>
                    ...
                    >
                      <SelectTrigger
                        aria-describedby={errors.prev_platform ? platformErrorId : undefined}
                        aria-invalid={Boolean(errors.prev_platform) || undefined}
                        aria-label="Previous platform"
                        data-testid="migrated-from"
                      >
                        <SelectValue placeholder="Previous platform" />
                      </SelectTrigger>
                      <SelectContent>
                        {migratedFromOptions.map((option) => (
                    ...
                    {errors.prev_platform && (
                      <FieldError id={platformErrorId}>{errors.prev_platform}</FieldError>
                    )}
                  </Field>
                </div>
                <Button
