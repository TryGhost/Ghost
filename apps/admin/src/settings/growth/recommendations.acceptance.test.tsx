...
function fakeRecommendations(recommendations = [firstRecommendation, secondRecommendation]) {
  return fakeAdminEndpoint(
    'GET',
    /^\/recommendations\/\?/,
    browseResponse('recommendations', recommendations, { limit: 5 }),
  );
}
...
describe('Recommendations settings', () => {
  ...
  it('rejects a URL that is already recommended', async () => {
    fakeSettingsScreens();
    fakeRecommendations();
    fakeAdminEndpoint('POST', '/recommendations/check/', {
  ...
  it('validates and edits a recommendation', async () => {
    ...
    const editApi = fakeAdminEndpoint('PUT', `/recommendations/${firstRecommendation.id}/`, {
  ...
  it('deletes a recommendation only after confirmation', async () => {
    ...
    const deleteApi = fakeAdminEndpoint(
      'DELETE',
      `/recommendations/${firstRecommendation.id}/`,
      {},
    );

    await section.getByTestId('recommendation-list-item').first().click();
    await settingsScreen
      .section('edit-recommendation-modal')
  ...
  it('shows incoming recommendations and whether they can be recommended back', async () => {
    ...
    fakeAdminEndpoint(
      ...
      browseResponse('recommendations', incomingRecommendations, { limit: 5 }),
    );
    ...
    await expect
      .poll(currentRoute)
      .toBe(
        `/settings/recommendations/add?url=${encodeURIComponent(incomingRecommendations[0].url)}`,
      );
  });
  ...
  it.each([
    ...
    fakeAdminEndpoint(
      'GET',
      /^\/incoming_recommendations\/\?/,
      browseResponse('recommendations', [unsafeRecommendation], { limit: 5 }),
    );
    await renderAdminApp('/settings/recommendations');
