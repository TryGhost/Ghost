...
const Network: React.FC<{ keywords: string[] }> = ({ keywords }) => {
  ...
  const isHostedOnLocalhost =
    isProduction && ['localhost', '127.0.0.1', '::1'].includes(window.location.hostname);
  const isHostedOnIP = isProduction && validator.isIP(window.location.hostname);

  const isDisabledByHosting =
    isDisabledByConfig || isHostedOnSubdirectory || isHostedOnLocalhost || isHostedOnIP;
  ...
  const toggleSocialWebSetting = async (checked: boolean) => {
    const updatedSetting: Setting[] = [{ key: 'social_web', value: checked }];
            ...
            <div className="flex w-full gap-1.5 rounded-md border border-border-default bg-muted p-3">
              ...
              <div className="-mt-0.5">
                {isDisabledByPrivateMode ? (
                  <>
                    Network is automatically disabled while your site is in{' '}
                    <span
                      className="cursor-pointer text-green"
                      onClick={() => updateRoute('members')}
                    >
                      private mode
                    </span>
                  </>
                  ...
                  <>
                    You need to configure a supported custom domain to use this feature.{' '}
                    <a
                      className="text-green"
                      href="https://ghost.org/help/social-web/#custom-domain-required"
