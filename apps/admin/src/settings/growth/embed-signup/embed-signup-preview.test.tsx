...
describe('EmbedSignupPreview', () => {
    it('generates the preview when its asynchronously loaded content arrives', async () => {
        ...
        rerender(
            <EmbedSignupPreview
                ...
                html={'<div data-preview-layout="all-in-one"><script data-background-color="#08090c" src="/signup-form.js"></script></div>'}
                style="all-in-one"
            />
        );
    ...
    it('waits for matching markup before rebuilding a changed layout', async () => {
        const brandedHtml = '<div data-preview-layout="all-in-one" style="height:100vh"><script data-background-color="#08090c" src="/signup-form.js"></script></div>';
        const minimalHtml = '<div data-preview-layout="minimal" style="min-height:58px;width:calc(100% - 48px);position:absolute;left:50%;top:50%;transform:translate(-50%, -50%)"><script src="/signup-form.js"></script></div>';
        const {container, rerender} = render(
        ...
        await waitFor(() => {
            const frames = Array.from(container.querySelectorAll('iframe'));
            expect(frames.some(frame => frame.dataset.previewLayout === 'minimal' && frame.contentDocument?.querySelector('[data-preview-layout="minimal"]'))).toBe(true);
        });
    });
