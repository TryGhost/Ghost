...
import {
  type EmbedSignupLayout,
  getEmbedPreviewLayoutMarker,
} from '@/settings/utils/generate-embed-code';
...
const EmbedSignupPreview: React.FC<EmbedSignupPreviewProps> = ({
  backgroundColor,
  html,
  style,
}) => {
  ...
  const generateContentForEmbed = useCallback(
  ...
  const updateContentForEmbed = useCallback(
    (iframe: HTMLIFrameElement) => {
      ...
      Array.from(targetScript.attributes)
        .filter(
          (attribute) => attribute.name.startsWith('data-') && !nextAttributes.has(attribute.name),
        )
