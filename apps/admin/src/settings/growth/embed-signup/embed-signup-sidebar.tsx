...
const EmbedSignupSidebar: React.FC<SidebarProps> = ({
  selectedLayout,
  accentColor,
  handleColorToggle,
  selectedColor,
  ...
  const { loadData } = useFilterableApi<Label>({
    path: '/labels/',
    filterKey: 'name',
    responseKey: 'labels',
  });
      ...
      if (request === requestSequence.current) {
        setLabelOptions([
          ...
          ...loadedOptions.filter(
            (option) => !currentSelectedLabels.some((selected) => selected.value === option.value),
          ),
        ]);
      }
    } catch {
  ...
  useEffect(() => {
    setLabelOptions((current) => [
      ...(selectedLabels || []),
      ...current.filter(
        (option) => !selectedLabels?.some((selected) => selected.value === option.value),
      ),
    ]);
  }, [selectedLabels]);
  ...
  return (
    <Stack
      className="h-auto border-border p-6 max-lg:border-t lg:h-[calc(100vh-16vmin)] lg:max-h-[645px] lg:overflow-y-scroll lg:border-l"
      gap="none"
    >
      <div>
        <Text as="h4" className="mb-8 md:text-xl" leading="heading" size="lg" weight="bold">
          Embed signup form
        </Text>
        <FieldGroup className="gap-6">
          ...
          <Field>
              ...
              <ComboboxTrigger aria-label="Labels at signup">
                <ComboboxValue placeholder={!selectedLabels?.length}>
                  {selectedLabels?.length
                    ...
                    : 'Pick one or more labels (optional)'}
                </ComboboxValue>
              </ComboboxTrigger>
              <ComboboxContent>
                <MultiSelectCombobox
                  footer={({ searchInput, clearSearch }) => {
                    ...
                    return (
                      <div className="border-t p-1">
                        <button
                          className="flex h-8 w-full items-center justify-start gap-2 rounded-xs px-2 text-sm hover:bg-interactive-hover"
                          type="button"
                          onClick={() => {
            ...
            <FieldDescription>
              Will be applied to all members signing up via this form
            </FieldDescription>
          </Field>
          <Field>
            ...
            <Textarea
              ...
              id="embed-signup-code"
              value={`${embedScript}`}
              readOnly
            />
            <FieldDescription>
              Paste this code onto any website where you&apos;d like your signup to appear.
            </FieldDescription>
