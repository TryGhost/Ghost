...
const donationsCurrencySchema = z.string().refine((currency) => supportedCurrencies.has(currency));

const donationsSuggestedAmountNumberSchema = z
  .number()
  .finite()
  .int()
  .nonnegative()
...
const donationsSuggestedAmountSchema = z.union([
  ...
  z.string().regex(/^\d+$/).transform(Number).pipe(donationsSuggestedAmountNumberSchema),
]);
...
export const tipsAndDonationsSettingsSchema = z.object({
  ...
  donations_suggested_amount: donationsSuggestedAmountSchema.default(
    DEFAULT_DONATIONS_SUGGESTED_AMOUNT,
  ),
});

export type TipsAndDonationsSettings = z.infer<typeof tipsAndDonationsSettingsSchema>;

export const parseTipsAndDonationsSettings = (settings: unknown): TipsAndDonationsSettings =>
  tipsAndDonationsSettingsSchema.parse(settings);
