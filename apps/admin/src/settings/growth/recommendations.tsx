...
const Recommendations: React.FC<{ keywords: string[] }> = ({ keywords }) => {
  const { saveState, handleSave } = useSettingGroup();

  // Fetch "Your recommendations"
  const {
    data: { meta: recommendationsMeta, recommendations } = {},
  ...
  const {
    data: { recommendations: incomingRecommendations, meta: incomingRecommendationsMeta } = {},
    isLoading: areIncomingRecommendationsLoading,
    hasNextPage: hasIncomingRecommendationsNextPage,
    fetchNextPage: fetchIncomingRecommendationsNextPage,
  } = useBrowseIncomingRecommendations({
    searchParams: {
      limit: '5',
      order: 'created_at desc',
  ...
  const groupDescription = (
    <>
      Recommend any publication that your audience will find valuable, and find out when others are
      recommending you.
    </>
  );
  ...
  const buttons = (
    <Button
      className="mt-[-5px] hidden md:!visible md:!block"
      size="sm"
      type="button"
      variant="ghost"
      onClick={() => {
      ...
      <Tabs value={selectedTab} variant="underline" onValueChange={setSelectedTab}>
        <TabsList>
          <TabsTrigger value="your-recommendations">
            ...
            {typeof recommendationsMeta?.pagination?.total === 'number' && (
              <TabsTriggerCount>
                {formatNumber(recommendationsMeta.pagination.total)}
              </TabsTriggerCount>
            )}
          </TabsTrigger>
          <TabsTrigger value="recommending-you">
            Recommending you
            {typeof incomingRecommendationsMeta?.pagination?.total === 'number' && (
              <TabsTriggerCount>
                {formatNumber(incomingRecommendationsMeta.pagination.total)}
              </TabsTriggerCount>
            )}
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="recommending-you">
          <IncomingRecommendationList
            ...
            isLoading={areIncomingRecommendationsLoading || areStatsLoading}
            showMore={showMoreMentions}
            stats={stats ?? []}
          />
        </TabsContent>
      </Tabs>
