...
describe('Ghost Explore settings', () => {
      ...
      boot: {
        browseSettings: {
          response: settingsResponse({
            settings: { explore_ping: false, explore_ping_growth: false },
          }),
        },
      },
    });
  ...
  it('can share growth data with Ghost Explore', async () => {
    ...
    await renderAdminApp('/settings', {
      boot: {
        browseSettings: {
          response: settingsResponse({
            settings: { explore_ping: true, explore_ping_growth: false },
          }),
        },
      },
    });
  ...
  it('renders a preview with the member count', async () => {
    ...
    await renderAdminApp('/settings', {
      boot: {
        browseSettings: {
          response: settingsResponse({
            settings: { explore_ping: true, explore_ping_growth: true },
          }),
        },
        browseMembersCount: {
          response: {
            members: [],
            meta: {
              pagination: { page: 1, limit: 1, pages: 1000, total: 1000, next: 2, prev: null },
            },
          },
        },
      },
  ...
  it('can send a testimonial', async () => {
    ...
    await expect
      .poll(() => testimonialApi.lastRequest?.body)
      .toEqual({
