...
import {
  createOfferRedemptionFilterUrl,
    ...
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end" className="z-[9999] min-w-[200px] normal-case">
        ...
        <DropdownMenuItem onSelect={onDirectionChange}>
          {sortDirection === 'asc' ? (
            <LucideIcon.ArrowUp className="size-3" />
          ) : (
            <LucideIcon.ArrowDown className="size-3" />
          )}
...
const RetentionOfferRow: React.FC<{
  ...
  const redemptionFilterUrl =
    offer.redemptions > 0 && offer.redemptionOfferIds.length > 0
  ...
  return (
    <TableRow data-testid="retention-offer-item">
      ...
      <TableCell className="p-0 whitespace-nowrap">
        ...
        >
          {offer.status === 'active' ? (
            <Badge className="rounded-full uppercase" variant="success">
              Active
            </Badge>
          ) : (
    ...
    <TableRow className={archived ? 'opacity-60' : undefined} data-testid="offer-item">
      ...
      <TableCell className="p-0 whitespace-nowrap">
        ...
        >
          ...
          {offer.type !== 'trial' ? (
            <span className='relative text-sm text-muted-foreground before:absolute before:-inset-x-0.5 before:top-1/2 before:rotate-[-20deg] before:border-t before:content-[""]'>
      ...
      <TableCell className="p-0 whitespace-nowrap">
        {offer.redemption_count > 0 && offer.id ? (
          <a
            className="block cursor-pointer p-5 hover:underline"
            href={createOfferRedemptionFilterUrl(offer.id)}
          >
...
const getOfferListItemRedemptions = (item: OfferListItem): number =>
  item.kind === 'retention' ? item.offer.redemptions : item.offer.redemption_count;
const getOfferListItemCreatedAt = (item: OfferListItem): string | null =>
  item.kind === 'retention' ? item.offer.createdAt : item.offer.created_at || null;

const sortOfferListItems = (
  items: OfferListItem[],
  sortOption: string,
  sortDirection: string,
): OfferListItem[] => {
...
export const OffersIndexModal: React.FC = () => {
  ...
  const { offersShowArchived: showArchived, setOffersShowArchived: setShowArchived } =
    useOffersShowArchived();
  ...
  const listLayoutOutput = (
    <div className="overflow-x-auto">
      <Table className="m-0 min-w-[900px]">
        ...
        <TableHeader>
          <TableRow>
            <TableHead className="sticky left-0 z-10 h-auto bg-background p-0 pb-2.5 pl-5 uppercase">
              Name
            </TableHead>
  ...
  return (
    ...
    >
      <Stack className="h-full pt-8">{listLayoutOutput}</Stack>
    </SettingsModal>
  );
};
