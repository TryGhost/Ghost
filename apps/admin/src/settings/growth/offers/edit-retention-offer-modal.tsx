...
import {
  ...
  useEditOffer,
  useInvalidateOffers,
} from '@tryghost/admin-x-framework/api/offers';
import { PreviewModalContent } from '@/settings/components/preview-modal';
import {
  createOfferRedemptionsFilterUrl,
  formatOfferTimestamp,
  generateRetentionOfferName,
} from './offer-helpers';
import { formatNumber } from '@tryghost/shade/utils';
import {
  getOfferPortalPreviewUrl,
  type offerPortalPreviewUrlTypes,
} from '@/settings/utils/get-offers-portal-preview-url';
...
const typeOptions = [
  ...
  {
    title: 'Free month(s)',
    description: 'Give free access for a limited time',
    value: 'free_months',
  },
];
...
const getRetentionOfferFormState = (
  offer: Offer | null,
  cadence: 'monthly' | 'yearly' = 'monthly',
): RetentionOfferFormState => {
  ...
  const repeatingDurationInMonths =
    offer.duration === 'repeating' && offer.duration_in_months
      ? offer.duration_in_months
      : defaultState.durationInMonths;
  ...
  return {
    ...
    freeMonths: isFreeMonths
      ? offer.duration_in_months || defaultState.freeMonths
      : defaultState.freeMonths,
  };
};
...
const getFormOfferTerms = ({
  ...
  const durationInMonths =
    duration === 'repeating' ? normalizeDurationInMonths(formState.durationInMonths) : 0;
...
const hasFormChangesFromDefault = (
  formState: RetentionOfferFormState,
  defaultState: RetentionOfferFormState,
): boolean => {
  return (
    formState.displayTitle !== defaultState.displayTitle ||
      ...
      <FieldGroup className="mb-10 grow gap-8">
              ...
              <div className="flex items-end justify-between">
                <div className="flex flex-col gap-5">
                  <div className="flex flex-col gap-1.5">
                    ...
                    <span>
                      {formatNumber(redemptions)} {redemptions === 1 ? 'redemption' : 'redemptions'}
                    </span>
                  </div>
                ...
                {redemptions > 0 && membersFilterUrl ? (
                  <a className="font-semibold text-green" href={membersFilterUrl}>
                    See members →
                  </a>
                ) : null}
              </div>
        ...
        <section className="mt-2">
          <Field orientation="horizontal">
            <FieldContent>
              <FieldLabel htmlFor={`${cadence}-retention-enabled`}>
                Enable {cadence} retention
              </FieldLabel>
              <FieldDescription>
                {cadence === 'monthly' ? 'Applied to monthly plans' : 'Applied to annual plans'}
              </FieldDescription>
            </FieldContent>
            <Switch
          ...
          <>
            <section className="mt-2">
              ...
              <div className="flex flex-col gap-6">
                ...
                <Field>
                  <FieldLabel htmlFor="retention-display-description">
                    Display description
                  </FieldLabel>
                  <Textarea
                ...
                {cadence === 'monthly' && (
                  <RadioGroup
                    ...
                    onValueChange={(value) => {
                      ...
                      updateForm((state) => ({
                        ...state,
                        type: value as RetentionOfferFormState['type'],
                      }));
                    }}
                  >
                    ...
                    <Field data-invalid={Boolean(errors.amount) || undefined}>
                      ...
                      <InputGroup data-invalid={Boolean(errors.amount) || undefined}>
                        <InputGroupInput
                          ...
                          type="number"
                          value={
                            formState.percentAmount === 0 ? '' : String(formState.percentAmount)
                          }
                          onChange={(e) => {
                      ...
                      <div className="-mt-4">
                        <Field data-invalid={Boolean(errors.durationInMonths) || undefined}>
                          <InputGroup data-invalid={Boolean(errors.durationInMonths) || undefined}>
                            <InputGroupInput
                              aria-invalid={Boolean(errors.durationInMonths) || undefined}
                              data-testid="duration-months-input"
                              type="number"
                              value={
                                formState.durationInMonths === 0
                                  ? ''
                                  : String(formState.durationInMonths)
                              }
                              onChange={(e) => {
                                const nextValue = Number(e.target.value);
                                updateForm((state) => ({
                            ...
                            <InputGroupAddon align="inline-end">
                              <InputGroupText>
                                {formState.durationInMonths === 1 ? 'month' : 'months'}
                              </InputGroupText>
                            </InputGroupAddon>
                          </InputGroup>
                          {errors.durationInMonths && (
                            <FieldError>{errors.durationInMonths}</FieldError>
                          )}
                        </Field>
                      </div>
                    )}
                    ...
                    <InputGroup data-invalid={Boolean(errors.amount) || undefined}>
                      <InputGroupInput
                        ...
                        onChange={(e) => {
                          ...
                          updateForm((state) => ({
                            ...state,
                            freeMonths: Number.isNaN(nextValue) ? 0 : nextValue,
                          }));
                        }}
                        onKeyDown={() => clearError('amount')}
                      />
                      <InputGroupAddon align="inline-end">
                        <InputGroupText>
                          {formState.freeMonths === 1 ? 'month' : 'months'}
                        </InputGroupText>
                      </InputGroupAddon>
                    </InputGroup>
                    {errors.amount && <FieldError>{errors.amount}</FieldError>}
...
const EditRetentionOfferModal: React.FC<{ cadence: 'monthly' | 'yearly' }> = ({ cadence }) => {
  ...
  const {
    ...
    onSave: async () => {
      ...
      const defaultState = getDefaultState(cadence);
      const shouldCreateInactiveDraft =
        !formState.enabled &&
        !editableRetentionOffer &&
        hasFormChangesFromDefault(formState, defaultState);
      ...
      const createRetentionOffer = async (status: 'active' | 'archived') => {
        ...
        await addOffer({
          ...
          duration_in_months:
            formTerms.duration === 'repeating' ? formTerms.durationInMonths : null,
          currency: null,
          status,
    ...
    onValidate: () => {
      ...
      if (formState.type === 'percent') {
        ...
        if (
          formState.duration === 'repeating' &&
          !isValidMonthDuration(formState.durationInMonths)
        ) {
  ...
  useEffect(() => {
    if (
      ...
      isFetchingOffers ||
      saveState === 'unsaved' ||
      initializedOfferKey === currentOfferKey
    ) {
      return;
    }
  ...
  const previewData: offerPortalPreviewUrlTypes = useMemo(() => {
    ...
    cadence,
    formState,
    lastPreviewFreeMonths,
    lastPreviewPercentAmount,
    offerCadence,
  ]);
  ...
  const preview = <PortalFrame href={href || ''} portalParent="offers" />;
  ...
  return (
    <PreviewModalContent
      ...
      previewToolbarBreadcrumbs={
        <SettingsBreadcrumbs current={breadcrumbTitle} label="Offers" onBack={goBack} />
      }
      sidebar={sidebar}
      size="lg"
