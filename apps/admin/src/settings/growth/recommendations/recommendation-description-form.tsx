...
function RecommendationDescriptionForm<T extends EditOrAddRecommendation>({
  showURL,
  formState,
  updateForm,
  errors,
  ...
  return (
    <FieldGroup className="mt-10 gap-8">
      <div>
        <Text
          as="h6"
          className="mb-2 block tracking-wider uppercase"
          leading="normal"
          size="xs"
          tone="secondary"
          weight="semibold"
            ...
            <div className="">
              ...
              >
                ...
                {formState.one_click_subscribe && (
                  <span className="flex pl-6 text-md font-semibold whitespace-nowrap text-green">
                    Subscribe
                  </span>
                )}
              </a>
      ...
      <Field data-invalid={Boolean(errors.description) || undefined}>
        ...
        />
        <FieldDescription>
          Max: <strong>{formatNumber(200)}</strong> characters. You&#8217;ve used{' '}
          <strong className={descriptionLengthColor}>{formatNumber(descriptionLength)}</strong>
        </FieldDescription>
      </Field>
    </FieldGroup>
  );
