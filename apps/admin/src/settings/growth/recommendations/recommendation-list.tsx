...
const RecommendationItem: React.FC<{ recommendation: Recommendation; onEdit: () => void }> = ({
  recommendation,
  onEdit,
}) => {
  ...
  const count =
    (showSubscribers ? recommendation.count?.subscribers : recommendation.count?.clicks) || 0;
  ...
  if (recommendations.length) {
    return (
      <>
        ...
        <ActionList>
          {recommendations.map((recommendation) => (
            <RecommendationItem
              ...
              onEdit={() => setEditingRecommendation(recommendation)}
            />
          ))}
        </ActionList>
        <div className="border-t border-border pt-2">
          {showMore?.hasMore && (
            <Button
              className="mb-2 h-auto p-0 text-green hover:text-green"
              type="button"
              variant="link"
            ...
            <TooltipProvider delayDuration={0}>
              <Tooltip>
                <TooltipTrigger asChild>
                  ...
                  >
                    {copied ? (
                      <LucideIcon.CircleCheck className="size-3.5! text-green" />
                    ) : (
                      <LucideIcon.Copy className="size-3.5! text-muted-foreground" />
                    )}
                  </Button>
                </TooltipTrigger>
    ...
    return (
      <NoValueLabel>
        ...
        <span className="mt-2 max-w-[40ch] text-center text-sm">
          Need inspiration?{' '}
          <a
            className="text-green hover:text-green-400"
            href="https://ghost.org/explore"
            rel="noopener noreferrer"
            target="_blank"
          >
