...
function getMemberIncludes(filters: FilterPredicate[]): string {
  ...
  for (const filter of filters) {
    const columnInclude = resolveField(memberFields, filter.field, 'UTC')?.definition.metadata
      ?.columnInclude;
...
export function buildMemberListSearchParams({
  filters,
  nql,
  search,
}: BuildMemberListSearchParamsOptions): Record<string, string> | undefined {
  if (!nql && !search) {
    return undefined;
  }
...
export function buildMemberOperationParams({ nql, search }: BuildMemberOperationParamsOptions): {
  all?: true;
  filter?: string;
  search?: string;
} {
...
const fixedColumnValues: Record<string, ColumnValueReader | undefined> = {
  labels: (member) =>
    member.labels?.length ? { text: member.labels.map((l) => l.name).join(', ') } : null,

  tiers: (member) =>
    member.tiers?.length ? { text: member.tiers.map((t) => t.name).join(', ') } : null,
  ...
  'subscriptions.start_date': (member, timezone) =>
    formatDateColumn(member.current_subscription?.start_date, timezone),

  'subscriptions.current_period_end': (member, timezone) =>
    formatDateColumn(member.current_subscription?.current_period_end, timezone),

  offer_redemptions: (member) => {
    const offers = member.subscriptions?.map((s) => s.offer?.name).filter(Boolean);
    return offers?.length ? { text: offers.join(', ') } : null;
  },
};
