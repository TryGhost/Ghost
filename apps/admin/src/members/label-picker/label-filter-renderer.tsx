...
const LabelFilterRenderer: React.FC<CustomRendererProps<string>> = ({
  field,
  values,
  onChange,
}) => {
  ...
  const renderItem = useCallback(
    ({ option, isSelected, onSelect }: RenderItemProps<string>) => {
      const metadataId = option.metadata?.id;
      const labelId =
        typeof metadataId === 'string' || typeof metadataId === 'number' ? String(metadataId) : '';
  ...
  const renderFooter = useCallback(
    ({ searchInput, clearSearch }: FooterRenderProps) => {
