...
describe('LabelFilterRenderer', () => {
  ...
  it('keeps the typed name when creation fails', async () => {
    ...
    await waitFor(() =>
      expect(picker.createLabel).toHaveBeenCalledWith('Duplicate beyond first page'),
    );
    expect(picker.toggleLabel).not.toHaveBeenCalled();
  ...
  it('hides the create action when an exact match is already loaded', () => {
    makePicker({
      labels: [
        { id: '1', name: 'Existing-Label', slug: 'existing-label', created_at: '', updated_at: '' },
      ],
    });
    ...
    expect(
      screen.queryByRole('button', { name: 'Create "existing-label"' }),
