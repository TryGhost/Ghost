import {
  ...
  resolveSlugsToLabels,
  toggleMemberNewsletter,
...
describe('buildCustomFieldSavePayload', () => {
  ...
  it('sends an address emptied of everything as a cleared field', () => {
    ...
    expect(buildCustomFieldSavePayload('mem_1', 'home_address', { line1: '', city: '  ' })).toEqual(
      { id: 'mem_1', metafields: { custom: { home_address: null } } },
    );
  });
});
...
describe('isDraftInSyncWithServer', () => {
  ...
  it('ignores whitespace-only differences (normalized comparison)', () => {
    expect(isDraftInSyncWithServer({ ...server, name: 'Ada ', email: ' ada@x.co' }, server)).toBe(
      true,
    );
  });
...
describe('getCustomFieldValidationErrors', () => {
  ...
  it('returns no errors for valid values', () => {
    expect(
      getCustomFieldValidationErrors({ job_title: 'Editor', home_address: validAddress }, fields),
    ).toEqual({});
  });
  ...
  it('accepts a partial address — no sub-field is required', () => {
    ...
    // than incomplete.
    expect(
      getCustomFieldValidationErrors({ home_address: { line1: '1 Main St' } }, fields),
    ).toEqual({});
  ...
  it('raises nothing for an address of nothing but whitespace, which reads as cleared', () => {
    ...
    expect(buildCustomFieldSavePayload('m1', 'home_address', whitespaceOnly).metafields).toEqual({
  ...
  it('lets a sub-field with a format be emptied, not just one with a bound', () => {
    ...
    expect(buildCustomFieldSavePayload('m1', 'home_address', clearedCountry).metafields).toEqual({
  ...
  it('checks the value the save sends, not the one the screen shows', () => {
    ...
    expect(buildCustomFieldSavePayload('m1', 'home_address', emptiedParts).metafields).toEqual({
...
describe('parseCustomFieldServerErrors', () => {
  ...
  it('returns undefined for failures that are not custom-fields shaped', () => {
    expect(
      parseCustomFieldServerErrors(
        serverError([{ property: 'email', context: 'Invalid', message: 'Invalid' }]),
      ),
    ).toBeUndefined();
...
describe('resolveSlugsToLabels', () => {
  ...
  it('falls back to using the slug as the name for an unknown slug', () => {
    expect(resolveSlugsToLabels(['mystery'], known)).toEqual([
