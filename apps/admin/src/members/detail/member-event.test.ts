...
describe('parseMemberEvent — subject', () => {
  it('uses member.name when trimmed non-empty', () => {
    const parsed = parseMemberEvent(
      ev('signup_event', { member: { name: '  Ada  ', email: 'ada@x.co' } }),
      defaultCtx,
    );
  ...
  it('falls back to member.email when name is empty/whitespace', () => {
    const parsed = parseMemberEvent(
      ev('signup_event', { member: { name: '   ', email: 'ada@x.co' } }),
      defaultCtx,
    );
  ...
  it('falls back to top-level name/email when member is absent (guest event)', () => {
    ...
    expect(parseMemberEvent(ev('signup_event', { email: 'x@x.co' }), defaultCtx).subject).toBe(
      'x@x.co',
    );
...
describe('parseMemberEvent — action text', () => {
  ...
  it('subscription_event created with signup flag → "signed up"', () => {
    ...
    expect(
      parseMemberEvent(ev('subscription_event', { type: 'created', signup: true }), defaultCtx)
        .action,
    ).toBe('signed up');
  });

  it('subscription_event created without signup flag → "started paid subscription"', () => {
    expect(parseMemberEvent(ev('subscription_event', { type: 'created' }), defaultCtx).action).toBe(
  ...
  it('subscription_event canceled/reactivated/expired/updated', () => {
    expect(
      parseMemberEvent(ev('subscription_event', { type: 'canceled' }), defaultCtx).action,
    ).toBe('canceled paid subscription');
    expect(
      parseMemberEvent(ev('subscription_event', { type: 'reactivated' }), defaultCtx).action,
    ).toBe('reactivated paid subscription');
    expect(parseMemberEvent(ev('subscription_event', { type: 'expired' }), defaultCtx).action).toBe(
      'ended paid subscription',
    );
    expect(parseMemberEvent(ev('subscription_event', { type: 'updated' }), defaultCtx).action).toBe(
  ...
  it('newsletter_event without hasMultipleNewsletters → generic "newsletter"', () => {
    const parsed = parseMemberEvent(
      ev('newsletter_event', { subscribed: true, newsletter: { name: 'Weekly' } }),
      defaultCtx,
    );
  ...
  it('newsletter_event with hasMultipleNewsletters → uses newsletter.name', () => {
    const parsed = parseMemberEvent(
      ev('newsletter_event', { subscribed: false, newsletter: { name: 'Weekly' } }),
      { ...defaultCtx, hasMultipleNewsletters: true },
    );
    expect(parsed.action).toBe('unsubscribed from Weekly');
  });

  it('comment_event with parent → "replied to comment"', () => {
    expect(parseMemberEvent(ev('comment_event', { parent: { id: 'c1' } }), defaultCtx).action).toBe(
      'replied to comment',
    );
  });
  ...
  it('feedback_event with score 1/0', () => {
    expect(parseMemberEvent(ev('feedback_event', { score: 1 }), defaultCtx).action).toBe(
      'more like this',
    );
    expect(parseMemberEvent(ev('feedback_event', { score: 0 }), defaultCtx).action).toBe(
      'less like this',
    );
  });
  ...
  it('email_change_event with from/to fields', () => {
    expect(
      parseMemberEvent(
        ev('email_change_event', { from_email: 'a@x.co', to_email: 'b@x.co' }),
        defaultCtx,
      ).action,
    ).toBe('Email address changed from a@x.co to b@x.co');
  });
  ...
  it('automated_email_sent_event with free/paid slug → welcome (Free/Paid)', () => {
    expect(
      parseMemberEvent(
        ev('automated_email_sent_event', { automatedEmail: { slug: 'welcome-free' } }),
        defaultCtx,
      ).action,
    ).toBe('received welcome email (Free)');
    expect(
      parseMemberEvent(
        ev('automated_email_sent_event', { automatedEmail: { slug: 'welcome-paid' } }),
        defaultCtx,
      ).action,
  ...
  it('automated_email_sent_event from automation → uses subject', () => {
    const parsed = parseMemberEvent(
      ev('automated_email_sent_event', {
        automatedEmail: { source: 'automation_action_revision', subject: '  Hi there  ' },
      }),
      defaultCtx,
    );
  ...
  it('automated_email_sent_event with missing/empty subject → subject-less copy (no "null" leak)', () => {
    ...
    const missing = parseMemberEvent(
      ev('automated_email_sent_event', {
        automatedEmail: { source: 'automation_action_revision' },
      }),
      defaultCtx,
    );
    expect(missing.action).toBe('received automated email');
    expect(missing.actionTitle).toBe('received automated email');
    const blank = parseMemberEvent(
      ev('automated_email_sent_event', {
        automatedEmail: { source: 'automation_action_revision', subject: '   ' },
      }),
      defaultCtx,
    );
...
describe('parseMemberEvent — object / url / route', () => {
  ...
  it('comment_event with post → object=title, url=post.url, no route', () => {
    const parsed = parseMemberEvent(
      ev('comment_event', { post: { title: 'Post', url: '/post/', id: 'p_x' } }),
      defaultCtx,
    );
  ...
  it('click_event with post → object + route to post analytics', () => {
    const parsed = parseMemberEvent(
      ev('click_event', { post: { title: 'Post', url: '/post/', id: 'p_c' } }),
      defaultCtx,
    );
    expect(parsed.object).toBe('Post');
...
describe('parseMemberEvent — info', () => {
  it('signup_event on paid-members-enabled site → "Free" for free signups', () => {
    const parsed = parseMemberEvent(ev('signup_event', { created_with_status: 'free' }), {
      ...defaultCtx,
  ...
  it('signup_event on paid-members-enabled site with paid signup → null (info suppressed)', () => {
    const parsed = parseMemberEvent(ev('signup_event', { created_with_status: 'paid' }), {
      ...defaultCtx,
      paidMembersEnabled: true,
    });
  ...
  it('gift_redemption_event → tier name', () => {
    expect(
      parseMemberEvent(ev('gift_redemption_event', { tier_name: 'Gold' }), defaultCtx).info,
    ).toBe('Gold');
  });
});
...
describe('parseMemberEvent — gift purchase amount formatting', () => {
  ...
  it('falls back gracefully when purchase data is missing', () => {
    expect(parseMemberEvent(ev('gift_purchase_event'), defaultCtx).action).toBe(
      'Purchased gift subscription',
    );
  });
});
...
describe('parseMemberEvent — meta', () => {
  ...
  it('returns undefined timestamp when data.created_at is missing (defensive — render layer decides how to show it)', () => {
    const raw = { type: 'signup_event', data: {} } as {
      type: string;
  ...
  it('normalizes memberId + trims member.name', () => {
    const parsed = parseMemberEvent(
      ev('signup_event', { member_id: 'm_1', member: { name: '  Ada  ', email: 'ada@x.co' } }),
      defaultCtx,
    );
  ...
  it('falls back to member.id when member_id is missing', () => {
    const parsed = parseMemberEvent(
      ev('signup_event', { member: { id: 'm_2', name: 'X', email: 'x@x.co' } }),
      defaultCtx,
    );
  ...
  it('description for click_event returns the link.to URL (raw fallback)', () => {
    expect(
      parseMemberEvent(
        ev('click_event', { link: { to: 'https://example.com/a?ref=x' } }),
        defaultCtx,
      ).description,
    ).toBe('https://example.com/a?ref=x');
  });
