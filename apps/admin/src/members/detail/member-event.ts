...
function getIcon(event: RawMemberEvent): string {
  ...
  if (
    ...
    event.type === 'gift_redemption_event' ||
    event.type === 'gift_ended_event'
  ) {
    icon = 'gift';
  }
...
function getAction(event: RawMemberEvent, hasMultipleNewsletters: boolean): string | undefined {
  ...
  if (event.type === 'automated_email_sent_event') {
    const auto = event.data.automatedEmail as
      | { source?: string; slug?: string; subject?: string }
      | undefined;
...
function getSource(event: RawMemberEvent): { name: string; url: string | null } | null {
  const attribution = event.data.attribution as
 ...
 */
export function parseMemberEvent(
  event: RawMemberEvent,
  ...
  const member = event.data.member
    ? { ...event.data.member, name: trimmedName ?? undefined }
    : event.data.member;
