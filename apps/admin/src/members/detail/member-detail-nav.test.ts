...
describe('deriveMemberDetailBackPath', () => {
  ...
  it('ignores a back param that points outside the members area', () => {
    expect(deriveMemberDetailBackPath(`?back=${encodeURIComponent('/settings/staff')}`)).toBe(
      '/members',
    );
  });
  ...
  it('ignores an absolute URL in the back param to avoid open redirects', () => {
    expect(
      deriveMemberDetailBackPath(`?back=${encodeURIComponent('https://evil.example.com/members')}`),
    ).toBe('/members');
  });
  ...
  it('ignores a protocol-relative URL in the back param', () => {
    ...
    expect(deriveMemberDetailBackPath(`?back=${encodeURIComponent('//evil.example.com')}`)).toBe(
      '/members',
    );
  });
  ...
  it('treats any /members-prefixed path as internal (intentional Ember parity)', () => {
    ...
    expect(deriveMemberDetailBackPath(`?back=${encodeURIComponent('/members.evil.com')}`)).toBe(
      '/members.evil.com',
    );
  });
