...
import {
  ...
  getMemberReferrerSource,
  parseMemberGeolocation,
} from './member-detail-format';
...
describe('parseMemberGeolocation', () => {
  ...
  it('parses a geolocation JSON string into an object', () => {
    expect(
      parseMemberGeolocation('{"country":"Turkey","country_code":"TR","region":"Izmir"}'),
...
describe('formatMemberLocation', () => {
  ...
  it('shows "Region, US" for US members that have a region', () => {
    expect(
      formatMemberLocation('{"country":"United States","country_code":"US","region":"California"}'),
    ).toBe('California, US');
  });

  it('shows the country for non-US members', () => {
    expect(
      formatMemberLocation('{"country":"Germany","country_code":"DE","region":"Berlin"}'),
    ).toBe('Germany');
  });
  ...
  it('shows the country for a US member that has no region', () => {
    ...
    expect(formatMemberLocation('{"country":"United States","country_code":"US"}')).toBe(
      'United States',
    );
  });
