...
const MemberDetailPage: React.FC<MemberDetailPageProps> = ({paidMembersEnabled, newslettersUiEnabled, engagementEnabled}) => {
    ...
    // Ember's `unsaved-changes` service can't guard this screen: with
    // `memberDetailsReact` on, the Ember member route aborts and never
    // registers into it.
    const guard = useNavigationGuard(() => !bypassGuardRef.current && hasUnsavedChanges);
    ...
    return (
        <Box className='size-full'><Container className='relative flex h-full flex-col' size='page'>
            <DetailPage data-testid='member-detail'>
                ...
                <AlertDialog open={guard.isBlocked} onOpenChange={(open) => {
                    if (!open) {
                        guard.reset();
                    }
                }}>
                    <AlertDialogContent>
                        ...
                        <AlertDialogFooter>
                            ...
                            <Button variant='destructive' onClick={() => guard.proceed()}>Leave</Button>
                        </AlertDialogFooter>
                    </AlertDialogContent>
                </AlertDialog>
            </DetailPage>
