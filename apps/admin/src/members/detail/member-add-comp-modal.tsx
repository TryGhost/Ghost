...
const MemberAddCompModal: React.FC<MemberAddCompModalProps> = ({ open, onOpenChange, member }) => {
  ...
  const activePaidSubscriptions = (member.subscriptions ?? []).filter(
    (sub: MemberSubscription) =>
      classifyMemberSubscription(sub) === 'paid' && CANCELABLE_STATUSES.has(sub.status),
  );

  // Native <input type="date"> minimum: don't allow picking a past day.
  const todayLocal = moment().format('YYYY-MM-DD');

  const canSubmit = !!selectedTierId && !isSubmitting && (duration !== 'custom' || !!customDate);
    ...
    >
      <DialogContent data-testid="add-comp-modal">
        <DialogHeader>
          ...
          <DialogDescription>
            Grants this member access to a paid tier for the chosen duration. Any active paid
        ...
        <div className="flex flex-col gap-4">
          <div className="flex flex-col gap-1.5">
            ...
            <Select value={selectedTierId} onValueChange={setSelectedTierId}>
              <SelectTrigger
                data-testid="comp-tier-select"
                disabled={isLoadingTiers || isSubmitting}
                id="comp-tier"
              >
              ...
              <SelectContent>
                {availablePaidTiers.map((tier) => (
                  <SelectItem key={tier.id} data-tier-id={tier.id} value={tier.id}>
                    {tier.name}
                  </SelectItem>
                ))}
              </SelectContent>
          ...
          <div className="flex flex-col gap-1.5">
            ...
            >
              <SelectTrigger
                data-testid="comp-duration-select"
