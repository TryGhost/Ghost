...
describe('waiting protects precision, not the clauses', () => {
    ...
    it('still waits when the source is named as a clause key, wherever it sits', () => {
        expect(canReadMemberFilter("custom_fields.key:'company'", {customFields: undefined})).toBe(false);
        expect(canReadMemberFilter("(status:paid+custom_fields.key:'company')", {customFields: undefined})).toBe(false);
        expect(canReadMemberFilter("status:paid,custom_fields.key:'company'", {customFields: undefined})).toBe(false);
        expect(canReadMemberFilter("custom_fields.key:-'company'", {customFields: undefined})).toBe(false);
    });
