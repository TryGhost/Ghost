...
import {
  ...
  useDeleteMemberView,
  useSaveMemberView,
} from '@/members/hooks/use-member-views';
...
const ManageViewPopoverContent: React.FC<ManageViewPopoverContentProps> = ({
  filter,
  existingViews,
  activeView,
  onDeleted,
...
const ManageViewPopover: React.FC<ManageViewPopoverProps> = ({
  filter,
  existingViews,
  activeView,
  onDeleted,
  ...
  const contentKey = activeView
    ? `edit:${activeView.name}:${activeView.filter.filter}`
    : `save:${filter}`;
