...
const MembersActions: React.FC<MembersActionsProps> = ({
  ...
  const membersBackPath =
    location.pathname === '/members' ? `${location.pathname}${currentSearch}` : '/members';
  ...
  const handleExport = useCallback(async () => {
    try {
      ...
      toast.error('Export failed', {
        description:
          'There was a problem downloading your member data. Please check your connection and try again.',
      });
      throw e;
    }
  }, [nql, search]);
  ...
  const handleAddLabel = useCallback(
    async (labelIds: string[]) => {
      try {
        ...
        toast.success(
          labelIds.length > 1 ? 'Labels added successfully' : 'Label added successfully',
        );
      } catch {
  ...
  const handleRemoveLabel = useCallback(
    async (labelIds: string[]) => {
      try {
        ...
        toast.success(
          labelIds.length > 1 ? 'Labels removed successfully' : 'Label removed successfully',
        );
      } catch {
  ...
  const handleUnsubscribe = useCallback(
    async (newsletterIds: string[] | null) => {
  ...
  const handleExportBackup = useCallback(async () => {
    try {
      ...
      toast.error('Export failed', {
        description:
          'There was a problem downloading your backup. Please check your connection and try again.',
      });
      throw e;
    }
  }, [nql, search]);
        ...
        <PageHeader.ActionGroup.Primary>
          <PageHeader.Action fallbackVariant="default" label="New member" asChild primary>
            <a
              ...
              href={newMemberHref}
            >
              <LucideIcon.Plus className={isAdmin7 ? 'stroke-2!' : 'sm:hidden'} />
