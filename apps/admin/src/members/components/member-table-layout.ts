...
export function getMemberTableLayout({
  ...
  const totalVisibleBaseWidth = visibleBaseColumns.reduce(
  ...
  const baseColumnWidthPercentages = visibleBaseColumns.reduce<
    Record<BaseMemberTableColumnKey, number>
  >(
    (acc, column) => {
  ...
  const minTableWidth =
    visibleBaseColumns.reduce(
      (total, column) => total + BASE_MEMBER_TABLE_COLUMN_MIN_WIDTHS[column.key],
      0,
    ) +
...
export function getMemberTableLayoutStyles(layout: MemberTableLayout): MemberTableLayoutStyles {
  ...
  return {
    ...
    columnStyles: {
      member: createColumnStyle(
        layout.baseColumnWidthPercentages.member,
        layout.baseColumnMinWidths.member,
      ),
      status: createColumnStyle(
        layout.baseColumnWidthPercentages.status,
        layout.baseColumnMinWidths.status,
      ),
      openRate: createColumnStyle(
        layout.baseColumnWidthPercentages.openRate,
        layout.baseColumnMinWidths.openRate,
      ),
      location: createColumnStyle(
        layout.baseColumnWidthPercentages.location,
        layout.baseColumnMinWidths.location,
      ),
      created: createColumnStyle(
        layout.baseColumnWidthPercentages.created,
        layout.baseColumnMinWidths.created,
      ),
