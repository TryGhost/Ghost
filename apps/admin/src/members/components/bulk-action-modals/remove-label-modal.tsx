...
export function RemoveLabelModal({
  ...
  const availableOptionSource = {
    ...picker.optionSource,
    options: picker.optionSource.options.filter((option) =>
      memberLabelSlugs.has(String(option.value)),
    ),
  };
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="gap-5" onOpenAutoFocus={(e) => e.preventDefault()}>
        ...
        <LabelPicker
          ...
          resolvedSelectedLabels={picker.resolvedSelectedLabels.filter((label) =>
            memberLabelSlugs.has(label.slug),
          )}
          ...
          <Button disabled={selectedSlugs.length === 0 || isLoading} onClick={handleConfirm}>
            {isLoading
              ...
              : selectedSlugs.length > 1
                ? `Remove ${formatNumber(selectedSlugs.length)} labels`
                : 'Remove label'}
          </Button>
        </DialogFooter>
      </DialogContent>
