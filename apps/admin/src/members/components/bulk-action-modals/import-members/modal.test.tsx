...
function renderModal(props: Partial<Parameters<typeof ImportMembersModal>[0]> = {}) {
  return render(<ImportMembersModal open onOpenChange={() => {}} {...props} />);
}
...
describe('ImportMembersModal', () => {
  beforeEach(() => {
    ...
    vi.stubGlobal(
      'fetch',
      vi.fn(() => new Response(null, { status: 202 })),
    );
  ...
  it('uploads members through the members API mutation so member queries are invalidated', async () => {
    ...
    expect(mockImportMembers.mock.calls[0][0]).toEqual(
  ...
  it('shows the file size error when the upload is too large', async () => {
    mockImportMembers.mockRejectedValueOnce(
      new RequestEntityTooLargeError(new Response(null, { status: 413 }), 'too large'),
    );
  ...
  it('shows the email verification host limit error without retrying', async () => {
    const onComplete = vi.fn();
    const errorData = createApiError(
      'HostLimitError',
      'Please verify your email before importing this many members.',
      'EMAIL_VERIFICATION_NEEDED',
    );
    mockImportMembers.mockRejectedValueOnce(
      new HostLimitError(new Response(JSON.stringify(errorData), { status: 403 }), errorData),
    );
    ...
    expect(
      screen.getByText('Please verify your email before importing this many members.'),
    ).toBeInTheDocument();
  ...
  it('shows validation errors returned by the upload API', async () => {
    ...
    mockImportMembers.mockRejectedValueOnce(
      new ValidationError(new Response(JSON.stringify(errorData), { status: 422 }), errorData),
    );
    ...
    expect(
      await screen.findByText('Please map Email to one of the fields in the CSV.'),
    ).toBeInTheDocument();
  });
  ...
  it('shows data import errors returned by the upload API', async () => {
    ...
    mockImportMembers.mockRejectedValueOnce(
      new JSONError(new Response(JSON.stringify(errorData), { status: 422 }), errorData),
    );
