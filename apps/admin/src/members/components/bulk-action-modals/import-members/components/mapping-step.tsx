...
import { MembersFieldMapping } from '@/members/components/bulk-action-modals/import-members/mapping';
...
export function MappingStep({
  ...
  const currentlyDisplayedData: MappingPreviewRow[] =
    fileData && fileData.length > 0 && mapping
  ...
  return (
    <>
                      ...
                      currentlyDisplayedData.map((row) => (
                        <TableRow key={row.key} className={cn(!row.mapTo && 'bg-muted')}>
                          ...
                          <TableCell>
                            ...
                            >
                              <SelectTrigger
                                className={cn('h-8 text-sm', !row.mapTo && 'text-muted-foreground')}
                              >
                                <SelectValue />
                              </SelectTrigger>
                              <SelectContent>
      ...
      <DialogFooter className="mt-5">
        ...
        <Button disabled={status === 'UPLOADING' || membersCount === 0} onClick={onUpload}>
          {status === 'UPLOADING' ? (
