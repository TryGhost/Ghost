...
import {
  ...
  importReducer,
} from '@/members/components/bulk-action-modals/import-members/reducer';
import {
...
export function ImportMembersModal({
  ...
  const handleUpdateMapping = useCallback(
    (from: string, to: string | null) => {
      ...
      const nextError =
        state.fileData && state.fileData.length === 0
  ...
  const handleFieldCreated = useCallback(
    (columnKey: string, column: string | null) => {
      if (column) {
  ...
  const handleUpload = useCallback(
    async (importMapping: Record<string, string | null>) => {
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      ...
      >
        <DialogHeader>
          ...
          <DialogDescription className="sr-only">Import members from a CSV file.</DialogDescription>
        </DialogHeader>
        ...
        {state.status === 'PROCESSING' && (
          <ProcessingStep onClose={() => handleOpenChange(false)} onUploadAnotherFile={reset} />
        )}
      ...
      <AlertDialog open={dialogProps.open} onOpenChange={dialogProps.onOpenChange}>
        ...
        <AlertDialogContent
          className="z-[1100]"
          data-testid="import-members-leave-guard"
        ...
        >
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              The columns you have mapped will be lost.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Keep mapping</AlertDialogCancel>
            <Button variant="destructive" onClick={dialogProps.onConfirm}>
              Leave
            </Button>
          </AlertDialogFooter>
        </AlertDialogContent>
