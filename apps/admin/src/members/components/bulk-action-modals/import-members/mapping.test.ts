...
describe('mapping helpers', () => {
  ...
  it('detects import tier mapping when enabled', () => {
    const mapping = detectFieldTypes([{ email: 'member@example.com', import_tier: 'Gold' }], {
  ...
  it('includes Gift ID in field mappings', () => {
    ...
    expect(mappings.find((m) => m.value === 'gift_id')).toEqual({
      label: 'Gift ID',
      value: 'gift_id',
    });
  });
  ...
  it('auto-detects gift_id', () => {
    const mapping = detectFieldTypes([{ email: 'user@example.com', gift_id: 'some-gift-uuid' }]);
