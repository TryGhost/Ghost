...
export function MappingStep({
  ...
  const createFieldForRow = async (
    ...
    if (!field) {
      ...
      setSaveError(
        'The field was created, but this column could not be mapped to it. Choose it from the list.',
      );
      setCanRetry(false);
  ...
  const isDimmed = (row: MappingPreviewRow) =>
    Boolean(createFieldForColumn) && createFieldForColumn !== row.key;
  ...
  const isImported = (row: MappingPreviewRow) =>
    !excludedColumns.has(row.key) && (Boolean(row.mapTo) || pendingColumns.has(row.key));
  ...
  const importRefusal = (): IncompleteImport | null => {
    ...
    if (!Object.values(importMapping).includes('email')) {
      return {
        message:
          'Please map "Email" to one of the fields in the CSV, and make sure it is selected.',
        columns: new Set(),
      };
    }
  ...
  return (
    <>
          ...
          <>
                        ...
                        <div className="flex items-center justify-between">
                          <span>
                            ...
                            <span className="text-muted-foreground">
                              (#{formatNumber(dataPreviewIndex + 1)})
                            </span>
                          </span>
                          ...
                          >
                            ...
                            <TableCell
                              className={cn(
                                ...
                                !isImported(row) && 'text-muted-foreground',
                              )}
                            >
                              {row.key}
                            </TableCell>
                            <TableCell
                              className={cn(
                                'text-sm break-all',
                                (!row.value || !isImported(row)) && 'text-muted-foreground',
                              )}
                            >
                              {row.value || '\u00A0'}
                            </TableCell>
                            <TableCell>
                              ...
                              <FieldPicker
                                ...
                                onOpenChange={(next) =>
                                  next ? showPicker(row.key, '') : setOpenPicker(null)
                                }
                                onSearchChange={(search) =>
                                  setOpenPicker({ columnKey: row.key, search })
                                }
                                onSelect={(target) => {
                                  setIncomplete(null);
                              ...
                              <TableCell className="p-2" colSpan={columnCount}>
                                ...
                                <div className="rounded-lg border bg-surface-elevated-2 p-3 shadow-lg">
                                  <CreateFieldForm
                                    ...
                                    onSubmit={(name, type) =>
                                      void createFieldForRow(row.key, name, type)
                                    }
                                  />
                                </div>
                              </TableCell>
            ...
            {membersCount > 0 && (
              <p className="text-sm text-muted-foreground">
                If an email address in your CSV matches an existing member, they will be updated
      ...
      <DialogFooter className="mt-5">
        <Button disabled={status === 'UPLOADING'} variant="outline" onClick={onStartOver}>
          Start over
        </Button>
        <Button disabled={status === 'UPLOADING' || membersCount === 0} onClick={handleImport}>
          {status === 'UPLOADING' ? (
