...
describe('buildMemberDetailPath', () => {
  it('preserves the current members list URL in the member detail path', () => {
    expect(
      buildMemberDetailPath('member-id', '/members?filter=label%3A%5BVIP%5D&search=alice'),
    ).toBe(
      '/members/member-id?back=%2Fmembers%3Ffilter%3Dlabel%253A%255BVIP%255D%26search%3Dalice',
    );
  });
