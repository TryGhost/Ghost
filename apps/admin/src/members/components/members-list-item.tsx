    ...
    <div className="flex min-w-0 items-center gap-3">
      ...
      <div className="min-w-0">
        ...
        {item.name && item.email && (
          <div className="truncate text-muted-foreground" data-testid="member-email">
            {item.email}
          </div>
        )}
      </div>
  ...
  return (
    <div className="flex min-w-0 justify-start">
      <div className="min-w-0">
        ...
        {tierNames && <div className="truncate text-sm text-muted-foreground">{tierNames}</div>}
      </div>
    </div>
  );
}
...
function MembersListItemOpenRate({ emailOpenRate }: { emailOpenRate: number | null | undefined }) {
  ...
  return (
    <div className={cn('text-base', isKnown ? 'text-foreground' : 'text-muted-foreground')}>
...
function MembersListItemCreated({ createdAt }: { createdAt: string }) {
  return (
    <div>
      <div className="text-base">{moment.utc(createdAt).format('D MMM YYYY')}</div>
      <div className="text-base text-muted-foreground">{moment.utc(createdAt).fromNow()}</div>
    </div>
  );
}
...
function MembersListItemDynamicColumn({
  ...
  return (
    <div className="min-w-0">
      ...
      <div className="truncate text-base" title={value.text}>
        {value.text}
      </div>
      {value.subtext && (
...
const MembersListItem = forwardRef<
  HTMLTableRowElement,
  MembersListItemProps & Omit<React.HTMLAttributes<HTMLTableRowElement>, 'onClick'>
  ...
  return (
    ...
    >
          ...
          <>
            <div
              ...
              style={{
                ...
                background:
                  'linear-gradient(to right, var(--background) 0px, color-mix(in hsl, var(--background) 78%, transparent) 6px, color-mix(in hsl, var(--background) 28%, transparent) 16px, transparent 24px)',
              }}
            />
            <div
              aria-hidden="true"
      ...
      {activeColumns.map((col) => (
        ...
        >
          <MembersListItemDynamicColumn column={col} member={item} timezone={timezone} />
        </TableCell>
      ))}
    </TableRow>
  );
