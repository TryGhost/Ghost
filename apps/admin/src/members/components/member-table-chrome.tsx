...
const PINNED_EDGE_FADE_STYLE = {
  left: '100%',
  background:
    'linear-gradient(to right, var(--members-sticky-fade-base) 0px, color-mix(in hsl, var(--members-sticky-fade-base) 78%, transparent) 6px, color-mix(in hsl, var(--members-sticky-fade-base) 28%, transparent) 16px, transparent 24px)',
} as CSSProperties;
...
export const MembersTableColGroup = ({
  ...
  return (
    <colgroup>
      <col
        className="max-sm:!w-full max-sm:!min-w-0 sm:max-lg:!w-auto sm:max-lg:!min-w-0"
        style={columnStyles.member}
      />
      <col className="hidden sm:table-column" style={columnStyles.status} />
      {showEmailOpenRate && (
        <col className="hidden lg:table-column" style={columnStyles.openRate} />
      )}
...
export const MembersTableHeader = ({
  ...
  return (
    <TableHeader
      ref={headerRef}
      className={cn(
        'hidden bg-transparent lg:table-header-group [&_th]:whitespace-nowrap',
        className,
      )}
    >
      <TableRow>
    ...
    >
      <Table className="w-full table-fixed border-collapse">
        ...
        <TableHeader className="bg-transparent lg:table-header-group [&_th]:whitespace-nowrap">
          <TableRow>
            <TableHead className="bg-transparent px-4">Member</TableHead>
          </TableRow>
        </TableHeader>
      </Table>
      {showPinnedEdge && (
