import ManageViewPopover from '@/members/components/manage-view-popover';
...
vi.mock('@/members/hooks/use-member-views', () => ({
  useSaveMemberView: () => saveMemberView,
  useDeleteMemberView: () => deleteMemberView,
}));

describe('ManageViewPopover', () => {
  it('resets the name when switching from an active saved view to save mode', () => {
    ...
    rerender(
      <ManageViewPopover activeView={null} existingViews={[activeView]} filter={unsavedFilter} />,
    );
