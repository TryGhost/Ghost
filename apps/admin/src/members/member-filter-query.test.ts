...
describe('member-filter-query', () => {
  ...
  it('reports relative-date filters as timezone-sensitive', () => {
    ...
    expect(hasTimezoneSensitiveMemberFilter('subscriptions.current_period_end:<=now+14d')).toBe(
      true,
    );
...
describe('isPredicateEnabled', () => {
  ...
  it('returns true for predicates whose operator is declared by the field map', () => {
    expect(
      isPredicateEnabled({ field: 'created_at', operator: 'in-the-last', values: [7] }, fields),
    ).toBe(true);

    expect(isPredicateEnabled({ field: 'status', operator: 'is', values: ['paid'] }, fields)).toBe(
      true,
    );
  });
