...
import { describeField } from '@/shared/filters';
...
describe('memberFields', () => {
  ...
  it('keeps the expected operators for key member fields', () => {
    ...
    expect(memberFields.email_count.operators).toEqual(['is', 'is-greater', 'is-less']);
...
describe('multipleActiveSubscriptionsCodec', () => {
  ...
  it('serializes the Yes/No predicate to count comparisons', () => {
    ...
    expect(field.codec.serialize(predicate, context)).toEqual(['count.active_stripe_customers:>1']);
  ...
  it('rejects unknown values and other operators', () => {
    ...
    expect(
      field.codec.serialize({ ...predicate, operator: 'is-greater', values: ['true'] }, context),
    ).toBeNull();
  });
