...
describe('Members label filters', () => {
    ...
    const membersApi = fakeMembers(
      ({ filter }) =>
        filter
          ...
          : [member({ name: 'VIP Member', labels: [vip] }), member({ name: 'Regular Member' })],
      { labels: [vip] },
    );
  ...
  it('selects multiple labels', async () => {
    ...
    const membersApi = fakeMembers(
      ({ filter }) => (filter?.includes('premium') ? member.many(3) : member.many(4)),
      { labels: [vip, premium] },
    );
    await renderAdminApp('/members');
  ...
  it('searches label options', async () => {
    ...
    const membersApi = fakeMembers(
      ({ filter }) => (filter ? [member({ name: 'Targeted Member' })] : member.many(3)),
      {
  ...
  it('deselects a label', async () => {
    ...
    const membersApi = fakeMembers(
      ({ filter }) =>
        filter === 'label:[premium]' ? [member({ name: 'Premium Member' })] : member.many(2),
      { labels: [vip, premium] },
    );
    await renderAdminApp('/members');
  ...
  it('renames a label inline', async () => {
    ...
    await expect
      ...
      .toEqual({ labels: [{ id: oldLabel.id, name: 'New-Name' }] });
  });
