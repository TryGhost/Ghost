...
export function isSharedViewFilterEqual(
  filterA: Record<string, string | null>,
  filterB: Record<string, string | null>,
): boolean {
...
export function findMatchingSharedViewIndexes(
  views: SharedView[],
  target: SharedViewIdentity,
): number[] {
  return views.flatMap((view, index) => (isSharedViewEqual(view, target) ? [index] : []));
}

export function hasSharedViewNameConflict(
  views: SharedView[],
  target: SharedViewName,
...
export function parseAllSharedViewsJSON(json: string): AllSharedViewsParseResult {
  try {
    ...
    if (!Array.isArray(parsed)) {
      ...
      console.error(
        'Failed to parse shared_views setting:',
        new Error('shared_views is not an array'),
      );
