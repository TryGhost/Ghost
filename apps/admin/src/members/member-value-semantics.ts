  ...
  return {
    ...
    serialize({ operator, values }): SerializedValue | null {
      ...
      return affirmative
        ? {
            join: 'and',
            fragments: [{ expression: String(optedIn) }, { key: EMAIL_DISABLED, expression: '0' }],
          }
        : {
            join: 'or',
            fragments: [{ expression: String(!optedIn) }, { key: EMAIL_DISABLED, expression: '1' }],
          };
    },
    parse() {
      return null;
