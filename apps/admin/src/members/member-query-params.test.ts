...
describe('a subscription column reads the resolved current_subscription', () => {
  const activeMonthly = sub({
    id: 'a',
    status: 'active',
    plan: { id: 'p', nickname: 'M', interval: 'month', currency: 'usd', amount: 1000 },
  });
  const cancelledYearly = sub({
    id: 'c',
    status: 'canceled',
    plan: { id: 'p', nickname: 'Y', interval: 'year', currency: 'usd', amount: 10000 },
  });

  it('reads status and plan from current_subscription', () => {
    const m = member({
      current_subscription: activeMonthly,
      subscriptions: [activeMonthly, cancelledYearly],
    });
...
describe('custom field columns', () => {
  ...
  it('names one column per field filtered on, from the field itself', () => {
    expect(
      getMemberActiveColumns(filterOn('job_title'), { customFields }).map(({ key, label }) => ({
        key,
        label,
      })),
