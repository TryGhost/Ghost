...
describe('normalizeTagDraft', () => {
  it('trims ordinary fields while preserving code injection whitespace', () => {
    const normalized = normalizeTagDraft(
      draft({
        name: '  News  ',
...
describe('validateTagField', () => {
  it('requires a name', () => {
    expect(validateTagField('name', draft({ name: '' }))).toBe(
  ...
  it('rejects names starting with a comma', () => {
    expect(validateTagField('name', draft({ name: ',News' }))).toBe(
      "Tag names can't start with commas.",
    );
  });

  it('caps name, slug, description and meta fields at their Ember limits', () => {
    expect(validateTagField('name', draft({ name: 'a'.repeat(192) }))).toBe(
      'Tag names cannot be longer than 191 characters.',
    );
    expect(validateTagField('name', draft({ name: 'a'.repeat(191) }))).toBeNull();
    expect(validateTagField('slug', draft({ slug: 'a'.repeat(192) }))).toBe(
      'URL cannot be longer than 191 characters.',
    );
    expect(validateTagField('description', draft({ description: 'a'.repeat(501) }))).toBe(
      'Description cannot be longer than 500 characters.',
    );
    expect(validateTagField('metaTitle', draft({ metaTitle: 'a'.repeat(301) }))).toBe(
      'Meta Title cannot be longer than 300 characters.',
    );
    expect(validateTagField('metaDescription', draft({ metaDescription: 'a'.repeat(501) }))).toBe(
  ...
  it('accepts empty canonical URLs and rejects unparseable ones', () => {
    ...
    expect(
      validateTagField('canonicalUrl', draft({ canonicalUrl: 'https://example.com/canonical' })),
    ).toBeNull();
    expect(validateTagField('canonicalUrl', draft({ canonicalUrl: 'not a url' }))).toBe(
...
describe('URL helpers', () => {
  ...
  it('prefers the canonical URL for the tag URL', () => {
    expect(getTagUrl(draft({ canonicalUrl: 'https://elsewhere.com/news' }), blogUrl)).toBe(
      'https://elsewhere.com/news/',
    );
    expect(getTagUrl(draft({ canonicalUrl: 'https://elsewhere.com/news/' }), blogUrl)).toBe(
      'https://elsewhere.com/news/',
    );
