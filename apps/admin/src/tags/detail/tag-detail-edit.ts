 ...
 */
export function normalizeTagDraft(
  draft: TagEditableFields,
  touchedFields?: ReadonlySet<TagFieldName>,
): TagEditableFields {
  ...
  for (const key of Object.keys(draft) as TagFieldName[]) {
    const shouldTrim =
      ...
      (!touchedFields || touchedFields.has(key));
...
export function validateTagField(field: TagFieldName, draft: TagEditableFields): string | null {
  ...
  switch (field) {
    case 'name':
      ...
      return null;
    case 'slug':
      return charLength(value) > SLUG_MAX_LENGTH
        ? 'URL cannot be longer than 191 characters.'
        : null;
    case 'description':
      return charLength(value) > DESCRIPTION_MAX_LENGTH
        ? 'Description cannot be longer than 500 characters.'
        : null;
    case 'metaTitle':
      return charLength(value) > META_TITLE_MAX_LENGTH
        ? 'Meta Title cannot be longer than 300 characters.'
        : null;
    case 'metaDescription':
      return charLength(value) > META_DESCRIPTION_MAX_LENGTH
...
const VALIDATED_FIELDS: TagFieldName[] = [
  'name',
  'slug',
  'description',
  'metaTitle',
 ...
 */
export function buildTagSavePayload(
  draft: TagEditableFields,
...
export function getTagUrl(
  draft: Pick<TagEditableFields, 'canonicalUrl' | 'slug'>,
  blogUrl: string,
): string {
...
export function getSeoTitle(
  draft: Pick<TagEditableFields, 'name' | 'metaTitle'>,
  siteTitle: string,
): string {
...
export function getSeoDescription(
  draft: Pick<TagEditableFields, 'description' | 'metaDescription'>,
): string {
