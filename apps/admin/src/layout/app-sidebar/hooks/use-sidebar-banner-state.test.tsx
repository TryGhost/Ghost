...
>(() => ({ showUpgradeBanner: false, trialDaysRemaining: 0 }));
const mockUseWhatsNewStatus = vi.fn<() => { showWhatsNewBanner: boolean }>(() => ({
...
vi.mock('../upgrade-banner', () => ({
  default: ({ trialDaysRemaining }: { trialDaysRemaining: number }) => (
    <div>Upgrade Banner ({trialDaysRemaining})</div>
  ),
}));
