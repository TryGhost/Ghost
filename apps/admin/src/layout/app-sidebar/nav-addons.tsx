...
function AppsNavItemContent({ isActive }: { isActive: boolean }) {
  return (
    <NavMenuItem.Link isActive={isActive} to="apps">
...
export function NavAddons({ ...props }: React.ComponentProps<typeof SidebarGroup>) {
  ...
  const entries = installs.filter(
    (install) =>
      install.enabled &&
      install.sidebar &&
      install.targeting.some((entry) => entry.target === PAGE_TARGET),
  );
  ...
  const isEntryActive = (handle: string) =>
    location.pathname === `/apps/${handle}` || location.pathname.startsWith(`/apps/${handle}/`);
