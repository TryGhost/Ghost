...
describe('buildPostViewUrl', () => {
  ...
  it('drops nulls rather than emitting empty params', () => {
    expect(buildPostViewUrl('posts', { type: 'draft', tag: null, author: null })).toBe(
      'posts?type=draft',
    );
  });
...
describe('isPostViewActive', () => {
  ...
  it('does not match a different route', () => {
    expect(
      isPostViewActive({ pathname: '/pages', search: '?type=draft' }, 'posts', { type: 'draft' }),
    ).toBe(false);
  });
  ...
  it('does not match when the URL is missing one of the view params', () => {
    expect(isPostViewActive(at('?type=draft'), 'posts', { type: 'draft', tag: 'news' })).toBe(
      false,
    );
  });
  ...
  it('includes order in the comparison', () => {
    expect(
      isPostViewActive(at('?type=draft'), 'posts', { type: 'draft', order: 'published_at asc' }),
    ).toBe(false);
    expect(
      isPostViewActive(at('?type=draft&order=published_at+asc'), 'posts', {
        type: 'draft',
  ...
  it('ignores params that are not part of a view', () => {
    expect(isPostViewActive(at('?type=draft&somethingElse=x'), 'posts', { type: 'draft' })).toBe(
      true,
    );
  });
});
...
describe('getDefaultPostViews', () => {
  it('offers Drafts, Scheduled and Published, in that order', () => {
    expect(getDefaultPostViews(false).map((view) => view.name)).toEqual([
      'Drafts',
      'Scheduled',
      'Published',
    ]);
  ...
  it('matches the Ember filters exactly', () => {
    expect(POST_DEFAULT_VIEWS.map((view) => view.filter)).toEqual([
      { type: 'draft' },
      { type: 'scheduled' },
      { type: 'published' },
    ]);
