...
describe('commentFields', () => {
  ...
  describe('commentDateCodec', () => {
    ...
    it('parses date comparators back to local dates', () => {
      expect(
        commentFields.created_at.codec.parse(
  ...
  describe('reportedCodec', () => {
    it('serializes reported yes/no state', () => {
      expect(
        commentFields.reported.codec.serialize(
          {
            id: '1',
            field: 'reported',
            operator: 'is',
    ...
    it('parses count-based report filters into boolean values', () => {
      expect(
        commentFields.reported.codec.parse(nql.parse('count.reports:>0') as never, reportedContext),
      ).toEqual({
        field: 'reported',
        operator: 'is',
        values: ['true'],
  ...
  describe('mapped shared codecs', () => {
    it('uses the shared text codec with the html field override', () => {
      expect(
        commentFields.body.codec.serialize(
          {
            id: '1',
            field: 'body',
