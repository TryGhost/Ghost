...
export function CommentHeader({
  ...
  return (
    <div className={cn('flex items-center gap-2', className)}>
      <div className={cn('flex min-w-0 items-center gap-x-1', isHidden && 'opacity-50')}>
        <div className="whitespace-nowrap">
          {memberId && onAuthorClick ? (
            ...
            <span className="block truncate font-semibold">{memberName || 'Unknown'}</span>
          )}
        </div>
        {canComment === false && (
          <Tooltip>
          ...
          <>
            ...
            <div className="min-w-0 truncate">
              {onPostClick ? (
                ...
                <span className="font-medium text-gray-800 dark:text-gray-400">{postTitle}</span>
              )}
            </div>
          </>
        )}
