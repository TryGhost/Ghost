...
function CommentLikesModal({
  ...
  const { data: likesData, isLoading: likesLoading } = useBrowseCommentLikes(comment.id, {
    enabled: open,
  });
  const { data: dislikesData, isLoading: dislikesLoading } = useBrowseCommentDislikes(comment.id, {
    enabled: open && dislikesEnabled,
  });
      ...
      <DialogContent aria-describedby={undefined}>
        <DialogHeader>
          <DialogTitle>
            {dislikesEnabled ? (
              <>
                {formatNumber(likeCount)} {likeCount === 1 ? 'like' : 'likes'} and{' '}
                {formatNumber(dislikeCount)} {dislikeCount === 1 ? 'dislike' : 'dislikes'}
              </>
            ) : (
              <>
            ...
            <TabsContent className="mt-4" value="likes">
              <div className="-mx-1 max-h-64 overflow-y-auto px-1">
                {likesLoading ? (
                  ...
                  <div className="py-4 text-center text-sm text-muted-foreground">No likes yet</div>
                ) : (
