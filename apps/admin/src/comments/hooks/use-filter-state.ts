...
import {
  hasTimezoneSensitiveCommentFilter,
  parseCommentFilter,
  serializeCommentFilters,
} from '@/comments/comment-filter-query';
import {
  parseLegacyCommentFilters,
  removeLegacyCommentFilterParams,
} from '@/comments/legacy-comment-filter-query';
...
export function shouldDelayCommentDateFilterHydration(
  ...
  return (
    ...
    isSettingsLoading &&
    !hasResolvedTimezone &&
    hasTimezoneSensitiveCommentFilter(filterParam)
  );
}
...
export function useFilterState(timezone: string): UseFilterStateReturn {
  ...
  const clearFilters = useCallback(
    ({ replace = true }: SetFiltersOptions = {}) => {
