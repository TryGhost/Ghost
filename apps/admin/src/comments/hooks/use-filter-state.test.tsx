...
import {
  shouldDelayCommentDateFilterHydration,
  useFilterState,
} from '@/comments/hooks/use-filter-state';
...
describe('use-filter-state', () => {
  describe('shouldDelayCommentDateFilterHydration', () => {
    ...
    it('does not wait for non-date filters', () => {
      expect(
        shouldDelayCommentDateFilterHydration('status:published+count.reports:>0', false, true),
      ).toBe(false);
    });
  });
  ...
  describe('useFilterState', () => {
    ...
    it('migrates legacy per-field filter params to canonical filters', () => {
      const { result } = renderHook(
        ...
        },
        {
          wrapper: createWrapper(
            '/?status=is:hidden&body=not_contains:spam&author=is_not:member_123&created_at=before:2024-02-01',
          ),
        },
      );
      ...
      expect(result.current.query).toBe(
        'filter=created_at%3A%3C%272024-02-01T00%3A00%3A00.000Z%27%2Bhtml%3A-%7E%27spam%27%2Bmember_id%3A-member_123%2Bstatus%3Ahidden',
      );
    });
    ...
    it('removes only the canonical filter param when clearing filters', () => {
      const { result } = renderHook(
        ...
        {
          wrapper: createWrapper(
            '/?filter=status:published&id=is:comment_123&thread=is:comment_456',
          ),
        },
      );
