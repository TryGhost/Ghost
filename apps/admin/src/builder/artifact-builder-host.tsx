...
class ArtifactBuilderLoadBoundary extends Component<
...
export const ArtifactBuilderHost = ({ Experience }: ArtifactBuilderHostProps) => {
  ...
  useEffect(
    () =>
      subscribeOpenArtifactBuilder((nextRequest) => {
        const bridgeEnabled = readEmberFeatureFlag('designBuilder');
        ...
        if (requestRef.current) {
          respondToArtifactBuilder({
            requestId: nextRequest.requestId,
            status: 'error',
            message: 'Finish the open Artifact before editing another one.',
          });
          return;
        }
  ...
  return (
    <Dialog open={true} onOpenChange={() => {}}>
      ...
      >
        <DialogTitle className="sr-only" id="artifact-builder-title">
          Artifact editor
        </DialogTitle>
        <DialogDescription className="sr-only" id="artifact-builder-description">
          Create or edit an interactive embed for this post.
        </DialogDescription>
