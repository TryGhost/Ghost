...
import {
  BuilderModelPreferenceStore,
  initialBuilderModelSelection,
} from '@/builder/models/model-preference-store';
import { createBuilderCredentialStore } from '@/builder/models/session-credential-store';

import type {
  AgentRuntime,
  BuilderRuntimeEvent,
  BuilderRuntimePromptMessage,
  BuilderTool,
} from '@/builder/core/agent-runtime';
import type {
  BuilderConversationMessage,
  BuilderModelTurnRequest,
  BuilderStreamEvent,
  ModelAccessAdapter,
  ...
  try {
    ...
    return text.length <= limit
      ...
      : {
          text: JSON.stringify({
            truncated: true,
            preview: text.slice(0, Math.max(0, limit - 40)),
          }),
          truncated: true,
        };
  } catch {
...
export function assembleBuilderSystemPrompt(
  request: Pick<BuilderModelTurnRequest, 'workspace' | 'tools'>,
): string {
  const tools = request.tools
    .map((tool) => `- ${tool.name}: ${tool.description.slice(0, 240)}`)
    .join('\n');
  const selection = request.workspace.selection
    ? stringifyBounded(request.workspace.selection).text
    : 'none';
  ...
  const workspaceGuidance =
    request.workspace.kind === 'artifact'
...
export function assembleBuilderUserPrompt(
  message: string,
  attachments: BuilderModelTurnRequest['workspace']['attachments'],
): string {
...
function modelToolText(result: BuilderToolResult<unknown>): string {
  const serializable =
    result.ok && result.attachments
      ? {
          ...result,
          attachments: result.attachments.map(({ type, mediaType }) => ({
            type,
            mediaType,
            data: '[attached image]',
          })),
...
function boundedTurn(
  turn: readonly BuilderRuntimePromptMessage[],
  limit: number,
): BuilderRuntimePromptMessage[] {
  let remaining = limit;
...
export function projectBuilderConversation(
  messages: readonly BuilderConversationMessage[],
  maxCharacters = defaultHistoryCharacterLimit,
): BuilderRuntimePromptMessage[] {
    ...
    execute: async (input, signal) => {
      ...
      return {
        ...
        attachments: result.ok
          ? result.attachments?.map((attachment: BuilderToolAttachment) => ({
...
export class BrowserPiModelAccess implements ModelAccessAdapter {
  ...
  constructor({
    credentialStore = createBuilderCredentialStore(),
    modelPreferenceStore = new BuilderModelPreferenceStore(),
    getApiKey,
    provider,
    modelId,
    fetch,
    maxMessages = 80,
    maxHistoryCharacters = defaultHistoryCharacterLimit,
  }: BrowserPiModelAccessOptions = {}) {
    const initial =
      provider && modelId
        ? { provider, modelId }
        : initialBuilderModelSelection(credentialStore, modelPreferenceStore);
  ...
  createRuntime({
    ...
    onEvent = () => {},
    ...options
  }: CreateRuntimeOptions): AgentRuntime {
    ...
    return createAgentRuntime({
      ...
      streamFn: (activeModel, context, streamOptions) =>
        providerApi.streamSimple(activeModel, context, {
          ...streamOptions,
          fetch: this.fetch,
      ...
      onEvent: (event) => {
        if (event.type === 'run-error') {
          onEvent({
            ...event,
            message: redact(event.message, [apiKey, this.getApiKey(provider) ?? '']),
          });
        } else {
          onEvent(event);
        }
  ...
  async runTurn(request: BuilderModelTurnRequest): Promise<void> {
    ...
    const runtime = this.createRuntime({
      ...
      initialMessages: projectBuilderConversation(
    ...
    try {
      await runtime.prompt(
        assembleBuilderUserPrompt(latestMessage.text, request.workspace.attachments),
      );
    } catch (error) {
