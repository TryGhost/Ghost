...
export function initialBuilderModelSelection(credentials: BuilderCredentialStore, preferences: BuilderModelPreferenceStore): BuilderModelSelection {
    const preferred = preferences.get();
    if (preferred && credentials.get(preferred.provider)) {
        return preferred;
    }
    const credentialProvider = CURATED_MODELS
        .map(model => model.provider)
        .find((provider, index, providers) => providers.indexOf(provider) === index && credentials.get(provider));
