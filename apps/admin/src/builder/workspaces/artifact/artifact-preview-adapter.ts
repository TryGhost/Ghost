...
function diagnostic(error: unknown): WorkspaceDiagnostic {
    return {code: 'artifact_render_failed', message: error instanceof Error ? error.message : String(error), severity: 'error'};
}
...
export class ArtifactPreviewAdapter implements BuilderPreviewAdapter {
    ...
    constructor({surface}: {surface: PreviewDocumentSurface}) {
        ...
        this.unsubscribeDiagnostic = surface.onDiagnostic((runtimeDiagnostic) => {
            const diagnostics = [...this.currentState.diagnostics, runtimeDiagnostic].slice(-maxDiagnostics);
    ...
    renderCandidate(draft: ArtifactDraft, signal: AbortSignal): Promise<ValidationResult> {
        return this.enqueue(signal, operationSignal => this.replaceWithRollback(draft, operationSignal));
    }
    ...
    inspectPage(signal: AbortSignal): Promise<PreviewPageInspection> {
        return this.enqueue(signal, operationSignal => this.surface.inspectPage(artifactPreviewUrl, operationSignal));
    }

    inspectElement(target: PreviewElementTarget, signal: AbortSignal): Promise<PreviewElementInspection> {
        return this.enqueue(signal, operationSignal => this.surface.inspectElement(target, operationSignal));
    }

    screenshot(request: ScreenshotRequest, signal: AbortSignal): Promise<ScreenshotResult> {
        return this.enqueue(signal, operationSignal => this.surface.screenshot(request, operationSignal));
    }
    ...
    private enqueue<T>(signal: AbortSignal, operation: (signal: AbortSignal) => Promise<T>): Promise<T> {
        if (this.destroyed) {
    ...
    private async replace(draft: ArtifactDraft, signal: AbortSignal, preserveSelection = false): Promise<ValidationResult> {
        try {
            const remappedSelection = await this.surface.replaceDocument({
                revision: draft.revision,
                url: artifactPreviewUrl,
                html: draft.html
            }, draft.selection, signal);
    ...
    private async replaceWithRollback(draft: ArtifactDraft, signal: AbortSignal): Promise<ValidationResult> {
    ...
    private openExternal(url: string): void {
        ...
        try {
            ...
            this.addDiagnostic({code: 'artifact_link_invalid', message: 'The embed tried to open an invalid link.', severity: 'error'});
            return;
        }
        if (!['http:', 'https:'].includes(destination.protocol)) {
            this.addDiagnostic({code: 'artifact_link_protocol_blocked', message: 'The embed can only open HTTP or HTTPS links from the preview.', severity: 'error'});
            return;
        }
