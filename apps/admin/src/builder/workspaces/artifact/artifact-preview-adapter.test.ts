...
import type {PreviewDocument, PreviewDocumentSurface} from '@/builder/workspaces/theme/preview/preview-document';
...
class FakeSurface implements PreviewDocumentSurface {
    ...
    replaceDocument(document: PreviewDocument, selection: BuilderSelectionContext | null): Promise<BuilderSelectionContext | null> {
        this.documents.push(structuredClone(document));
        if (this.failNext) {
            this.failNext = false;
            return Promise.reject(new Error('Document failed'));
        }
        return Promise.resolve(this.remappedSelection === undefined ? selection : this.remappedSelection);
    }

    inspectPage(url: string) {
        return Promise.resolve({url, title: 'Artifact', viewport: {width: 800, height: 600, scrollX: 0, scrollY: 0}, outline: [], text: 'Artifact', truncated: {outline: false, text: false, source: false}});
    }

    inspectElement() {
        return Promise.resolve({tag: 'h1', role: 'heading', accessibleName: 'Artifact', attributes: {}, box: {x: 0, y: 0, width: 100, height: 40}, styles: {}, text: 'Artifact', source: null, truncated: {text: false, source: false}});
    }
...
async function draft(html = '<!doctype html><html><head><title>Artifact</title></head><body><main><h1 id="title">Artifact</h1></main></body></html>') {
    return createArtifactDraft({id: 'artifact-1', artifactVersion: 1, title: 'Artifact', description: '', html});
}
...
describe('ArtifactPreviewAdapter', () => {
    ...
    it('preserves authored document policy that must also apply after Save', () => {
        const html = '<!doctype html><html><head><base href="https://cdn.example.com/widgets/"><meta http-equiv="content-security-policy" content="default-src https:"><meta http-equiv="refresh" content="60"><script src="https://example.com/ghost/assets/portal.min.js"></script></head><body></body></html>';
        const srcdoc = createPreviewDocument({revision: 'revision', url: 'https://artifact.builder.invalid/', html}, 'channel', null, 'revision', () => null, false, {}, false, true, true);
        ...
        expect(parsed.querySelector('meta[http-equiv]')?.getAttribute('http-equiv')).toBe('content-security-policy');
        expect(parsed.querySelector('script[src]')?.getAttribute('src')).toBe('https://example.com/ghost/assets/portal.min.js');
    });
    ...
    it('opens only HTTP or HTTPS links emitted from the artifact preview', async () => {
        ...
        expect(preview.state.diagnostics).toEqual([expect.objectContaining({code: 'artifact_link_protocol_blocked'})]);
    });
    ...
    it('restores the last valid document when a candidate fails to load', async () => {
        ...
        const preview = new ArtifactPreviewAdapter({surface});
        const initial = await draft();
        const changed = await draft('<!doctype html><html><head><title>Changed</title></head><body>Changed</body></html>');
        await preview.start(initial, new AbortController().signal);
        surface.failNext = true;

        const result = await preview.renderCandidate(changed, new AbortController().signal);

        expect(result).toMatchObject({valid: false, diagnostics: [{code: 'artifact_render_failed'}]});
    ...
    it('does not run queued preview work after teardown', async () => {
        ...
        surface.inspectPage = vi.fn((_url: string) => new Promise<Awaited<ReturnType<FakeSurface['inspectPage']>>>((resolve) => {
            releaseInspection = () => resolve({url: 'https://artifact.builder.invalid/', title: 'Artifact', viewport: {width: 800, height: 600, scrollX: 0, scrollY: 0}, outline: [], text: 'Artifact', truncated: {outline: false, text: false, source: false}});
        }));
        const inspecting = preview.inspectPage(new AbortController().signal);
        const rendering = preview.renderCandidate(await draft('<!doctype html><html><head><title>Queued</title></head><body>Queued</body></html>'), new AbortController().signal);
