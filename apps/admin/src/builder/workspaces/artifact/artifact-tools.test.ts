...
import {
  ARTIFACT_HTML_LIMITS,
  findInArtifactHtml,
  readArtifactHtml,
  replaceInArtifactHtml,
  writeArtifactHtml,
} from './artifact-tools';
...
describe('Artifact HTML tools', () => {
  it('boots an empty Koenig card as a complete portable document', async () => {
    const payload = artifactBuilderPayload({
      id: 'artifact-1',
      artifactVersion: 1,
      title: '',
      description: '',
  ...
  it('rejects stale, incomplete, and oversized writes without creating a candidate', async () => {
    ...
    await expect(
      writeArtifactHtml(draft, { revision: 'stale', html: draft.html }),
    ).resolves.toMatchObject({ ok: false, error: { code: 'stale_revision' } });
    await expect(
      writeArtifactHtml(draft, { revision: draft.revision, html: '<div>fragment</div>' }),
    ).resolves.toMatchObject({ ok: false, error: { code: 'incomplete_artifact_html' } });
    await expect(
      writeArtifactHtml(draft, {
        revision: draft.revision,
        html: `<!doctype html><html><head><title>Large</title></head><body>${'A'.repeat(ARTIFACT_HTML_LIMITS.maxBytes)}</body></html>`,
      }),
    ).resolves.toMatchObject({ ok: false, error: { code: 'artifact_html_too_large' } });
  });
