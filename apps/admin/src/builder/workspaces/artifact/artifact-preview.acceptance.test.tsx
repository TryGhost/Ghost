...
describe('Artifact preview', () => {
  it('runs interactive HTML, selects rendered context, and applies agent CSS edits in the live opaque preview', async () => {
    ...
    await expect(
      preview.inspectElement({ selector: '#isolation' }, new AbortController().signal),
    ).resolves.toMatchObject({ text: 'opaque origin' });
    await frame.getByRole('button', { name: 'Count 0' }).click();
    await expect(
      preview.inspectElement({ selector: '#count' }, new AbortController().signal),
    ...
    await expect(workspace.restore(counterCheckpoint)).resolves.toMatchObject({
      valid: true,
      revision: counterCheckpoint.revision,
    });
    await expect(
      preview.inspectElement({ marker: counterSelection?.id }, new AbortController().signal),
    ...
    await expect(
      preview.inspectElement({ marker: selectedHeading?.id }, new AbortController().signal),
    ).resolves.toMatchObject({ tag: 'h1', text: 'Interactive chart' });
    ...
    await expect(
      preview.inspectElement({ marker: secondPoint?.id }, new AbortController().signal),
    ).resolves.toMatchObject({ tag: 'li', text: 'Point' });
    ...
    await expect(
      preview.inspectElement({ selector: '#chart' }, new AbortController().signal),
    ...
    await expect(workspace.restore(checkpoint)).resolves.toMatchObject({
      valid: true,
    ...
    await expect
      .poll(async () => {
        try {
          return (
            await preview.inspectElement(
              { marker: delayedSelection?.id },
              new AbortController().signal,
            )
          ).text;
        } catch {
    ...
    await expect(
      preview.inspectElement({ selector: '#chart' }, new AbortController().signal),
    ).resolves.toMatchObject({ text: 'Interactive chart' });
    expect(preview.state.diagnostics).not.toEqual(
      expect.arrayContaining([expect.objectContaining({ code: 'preview_navigation_bypassed' })]),
    );
  });
});
