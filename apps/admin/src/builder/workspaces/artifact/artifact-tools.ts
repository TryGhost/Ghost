import {ARTIFACT_HTML_MAX_BYTES, ArtifactPayloadValidationError, cloneArtifactDraft, normalizeArtifactPayload, withArtifactRevision} from './artifact-state';
...
function failure(draft: ArtifactDraft, code: string, message: string, retryable = false, details?: unknown): Extract<BuilderToolResult<never>, {ok: false}> {
...
function currentRevision(draft: ArtifactDraft, revision: unknown): BuilderToolResult<null> {
    return typeof revision === 'string' && revision === draft.revision
        ...
        : failure(draft, 'stale_revision', 'The artifact changed since this tool call was prepared. Read the latest HTML and retry.', true, {currentRevision: draft.revision});
}

function completeDocument(draft: ArtifactDraft, html: unknown): BuilderToolResult<{html: string; title: string; description: string}> {
    try {
        const normalized = normalizeArtifactPayload({...draft, html});
        return {ok: true, revision: draft.revision, data: {html: normalized.html, title: normalized.title, description: normalized.description}};
    } catch (error) {
...
export function validateArtifactDraft(draft: ArtifactDraft): BuilderToolResult<{html: string; title: string; description: string}> {
    return completeDocument(draft, draft.html);
}

async function candidateWithHtml(draft: ArtifactDraft, html: string): Promise<ArtifactCandidateResult<{title: string; description: string; sizeBytes: number}>> {
    ...
    return {
        ...
        candidate: revised,
        data: {title: revised.title, description: revised.description, sizeBytes: encoder.encode(revised.html).byteLength}
    };
}

export function readArtifactHtml(draft: ArtifactDraft, input: Record<string, unknown>): BuilderToolResult<unknown> {
    ...
    const requestedEnd = integer(input.endLine, Math.min(lines.length, startLine + ARTIFACT_HTML_LIMITS.maxReadLines - 1), startLine, lines.length);
    ...
    return {ok: true, revision: draft.revision, data: {content: chunks.join(''), startLine, startColumn, endLine, endColumn, truncated: Boolean(next), next}};
}

export function findInArtifactHtml(draft: ArtifactDraft, input: Record<string, unknown>): BuilderToolResult<unknown> {
    if (typeof input.query !== 'string' || !input.query || input.query.length > ARTIFACT_HTML_LIMITS.maxQueryCharacters) {
        return failure(draft, 'invalid_html_query', `Use a non-empty literal query no longer than ${ARTIFACT_HTML_LIMITS.maxQueryCharacters} characters.`);
    }
    ...
    draft.html.split('\n').forEach((line, index) => {
        ...
        while (column >= 0) {
            ...
            if (matches.length < ARTIFACT_HTML_LIMITS.maxMatches) {
                matches.push({line: index + 1, column: column + 1, text: line.slice(0, ARTIFACT_HTML_LIMITS.maxExcerptCharacters)});
            }
            column = lower.indexOf(query, column + Math.max(1, query.length));
        }
    });
    return {ok: true, revision: draft.revision, data: {query: input.query, matches, totalMatches, truncated: totalMatches > matches.length}};
}

export async function replaceInArtifactHtml(draft: ArtifactDraft, input: Record<string, unknown>): Promise<ArtifactCandidateResult<unknown>> {
    ...
    if (typeof input.oldText !== 'string' || !input.oldText || typeof input.newText !== 'string') {
        return failure(draft, 'invalid_html_replacement', 'Provide non-empty oldText and string newText values.');
    }
    ...
    if (!occurrences) {
        return failure(draft, 'html_text_not_found', 'The requested HTML text was not found. Read the latest HTML and retry.', true);
    }
    if (occurrences > 1 && input.replaceAll !== true) {
        return failure(draft, 'html_text_not_unique', 'The requested HTML text appears more than once. Provide more context or set replaceAll to true.', true, {occurrences});
    }
    if (input.replaceAll === true && occurrences > ARTIFACT_HTML_LIMITS.maxReplacements) {
        return failure(draft, 'too_many_html_replacements', `One call can replace at most ${ARTIFACT_HTML_LIMITS.maxReplacements} matches. Use a more specific match or write the complete HTML document.`, true);
    }
    const replacementCount = input.replaceAll === true ? occurrences : 1;
    const projectedSizeBytes = encoder.encode(draft.html).byteLength
    ...
    if (projectedSizeBytes > ARTIFACT_HTML_LIMITS.maxBytes) {
        return failure(draft, 'artifact_html_too_large', `Artifact HTML must stay under ${ARTIFACT_HTML_LIMITS.maxBytes} bytes.`, false, {sizeBytes: projectedSizeBytes});
    }
    let html: string;
    ...
    return result.ok ? {...result, data: {...result.data, replacements: replacementCount}} : result;
}

export async function writeArtifactHtml(draft: ArtifactDraft, input: Record<string, unknown>): Promise<ArtifactCandidateResult<unknown>> {
    const revision = currentRevision(draft, input.revision);
    if (!revision.ok) {
        return revision;
    }
