...
function isCheckpointPayload(value: unknown): value is ArtifactCheckpointPayload {
  ...
  const validDraft = (draft: unknown): draft is ArtifactDraft =>
    Boolean(
      draft &&
      typeof draft === 'object' &&
  ...
  return (
    validDraft(payload.promoted) && (payload.candidate === null || validDraft(payload.candidate))
  );
}
...
function screenshotAttachment(result: ScreenshotResult): BuilderToolResult<unknown> {
  ...
  if (!result.dataUrl.startsWith(prefix)) {
    return {
      ...
      revision: '',
      error: {
        code: 'preview_screenshot_failed',
        message: 'The preview returned an invalid screenshot.',
        retryable: true,
      },
    };
  }
  return {
    ...
    attachments: [
      { type: 'image', mediaType: 'image/png', data: result.dataUrl.slice(prefix.length) },
    ],
  };
}
...
export class ArtifactWorkspace implements BuilderWorkspace {
  ...
  get hasPromotedChanges(): boolean {
    return Boolean(
      this.currentDraft && this.baseline && !sameSavedPayload(this.currentDraft, this.baseline),
    );
  }
    ...
    if (!validation.valid || validation.revision !== active.revision) {
      return validation.valid
        ? {
            ...
            diagnostics: [
              {
                code: 'artifact_restore_mismatch',
                message: 'The restored preview did not match the checkpoint.',
                severity: 'error',
              },
            ],
          }
    ...
    return [
      {
        ...
        execute: (input, signal) =>
          this.executeRead(signal, () => readArtifactHtml(this.activeDraft(), input)),
      },
      {
        name: 'find_in_html',
        ...
        execute: (input, signal) =>
          this.executeRead(signal, () => findInArtifactHtml(this.activeDraft(), input)),
      },
      {
        name: 'replace_in_html',
        description:
          'Replace exact text in the complete artifact HTML and rerender the preview automatically.',
        inputSchema: {
          type: 'object',
          properties: {
          ...
          required: ['revision', 'oldText', 'newText'],
          additionalProperties: false,
        },
        execute: (input, signal) =>
          this.enqueueMutation(signal, (draft) => replaceInArtifactHtml(draft, input)),
      },
      {
        name: 'write_html',
        description:
          'Replace the complete artifact HTML document and rerender the preview automatically.',
        inputSchema: {
          type: 'object',
          properties: { revision: { type: 'string' }, html: { type: 'string' } },
          required: ['revision', 'html'],
          additionalProperties: false,
        },
        execute: (input, signal) =>
          this.enqueueMutation(signal, (draft) => writeArtifactHtml(draft, input)),
      },
      {
        name: 'inspect',
        description:
          'Inspect the rendered artifact page or one element selected by marker or CSS selector.',
        inputSchema: {
          type: 'object',
          properties: {
  ...
  async publish(signal: AbortSignal): Promise<PublishResult> {
    ...
    if (!this.saveDraft) {
      return {
        ...
        error: {
          code: 'artifact_save_unavailable',
          message: 'Artifact saving is not connected yet.',
          retryable: false,
        },
      };
    }
  ...
  private async executeRead(
    signal: AbortSignal,
    read: () => BuilderToolResult<unknown>,
  ...
  private enqueueMutation(
    signal: AbortSignal,
    mutate: (draft: ArtifactDraft) => Promise<ArtifactCandidateResult<unknown>>,
  ): Promise<BuilderToolResult<unknown>> {
    const operation = this.mutationTail.then(async () => {
      ...
      const rendered =
        this.preview.draft && sameSavedPayload(this.preview.draft, result.candidate)
          ? this.preview.draft
          : result.candidate;
      ...
      this.setState({
        ...
        dirty: this.isDirty(),
        validation: { ...validation, revision: rendered.revision },
      });
      return {
        ...result,
        revision: rendered.revision,
        data: { ...(result.data as object), render: { valid: true } },
      };
    });
  ...
  private async executeInspect(
    ...
    if (hasMarker && hasSelector) {
      return {
        ...
        error: {
          ...
          message: 'Inspect by marker or selector, not both.',
          retryable: false,
        },
      };
    }
    try {
      const data =
        hasMarker || hasSelector
          ? await this.preview.inspectElement(
              hasMarker
  ...
  private async executeScreenshot(
    ...
    await this.flush(signal);
    const hasMarker =
      request.kind === 'element' && typeof request.marker === 'string' && request.marker.length > 0;
    const hasSelector =
      request.kind === 'element' &&
      typeof request.selector === 'string' &&
      request.selector.length > 0;
    if (
      !['viewport', 'full_page', 'element'].includes(request.kind) ||
    ...
    try {
      ...
      return result.ok
        ? { ...result, revision: this.activeDraft().revision }
        : { ...result, revision: this.activeDraft().revision };
    } catch (error) {
      if (isAbortError(error)) {
        throw error;
  ...
  private previewFailure(error: unknown): BuilderToolResult<never> {
    ...
    return {
      ok: false,
      revision: this.activeDraft().revision,
      error: { code: diagnostic.code, message: diagnostic.message, retryable: true },
    };
  }
