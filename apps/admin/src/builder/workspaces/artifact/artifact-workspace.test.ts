...
describe('ArtifactWorkspace', () => {
  it('automatically rerenders HTML mutations, promotes successful work, and saves the complete payload', async () => {
    ...
    const save = vi.fn<(payload: ArtifactPayload, signal: AbortSignal) => Promise<void>>(() =>
      Promise.resolve(),
    );
    ...
    const result = await write?.execute(
      { revision, oldText: '<h1>Revenue</h1>', newText: '<h1>Revenue by region</h1>' },
  ...
  it('restores promoted and interrupted candidate state together at a checkpoint', async () => {
    ...
    await write?.execute(
      {
        ...
        html: '<!doctype html><html><head><title>First candidate</title></head><body>First candidate</body></html>',
      },
      new AbortController().signal,
    );
    ...
    await write?.execute(
      {
        revision: workspace.state.revision,
        html: '<!doctype html><html><head><title>Second candidate</title></head><body>Second candidate</body></html>',
      },
      new AbortController().signal,
    );
  ...
  it('saves only promoted work while an interrupted candidate remains recoverable', async () => {
    ...
    await write?.execute(
      {
        revision: workspace.state.revision,
        html: '<!doctype html><html><head><title>Interrupted</title></head><body>Interrupted candidate</body></html>',
      },
      new AbortController().signal,
    );
  ...
  it('retains promoted dirty state when an interrupted candidate matches the baseline', async () => {
    ...
    await write?.execute(
      {
        revision: workspace.state.revision,
        html: '<!doctype html><html><head><title>Promoted</title></head><body>Promoted</body></html>',
      },
      new AbortController().signal,
    );
    await workspace.promoteCandidate(new AbortController().signal);
    await write?.execute(
      { revision: workspace.state.revision, html: baselineHtml },
      new AbortController().signal,
    );
  ...
  it('does not report a successful save when the editor bridge is unavailable', async () => {
    const workspace = new ArtifactWorkspace({
      ...
      title: 'Artifact Builder',
      load: initialDraft,
      preview: new FakeArtifactPreview(),
    });
    await workspace.load(new AbortController().signal);
  ...
  it('rejects malformed card payloads at the workspace ingress boundary', async () => {
    const malformed: ArtifactDraft = {
      id: 'artifact-1',
      artifactVersion: 1,
      title: 'Broken',
      description: '',
      html: '<div>fragment</div>',
      revision: 'artifact-invalid',
      selection: null,
    };
    const workspace = new ArtifactWorkspace({
      id: 'artifact-1',
    ...
    await expect(workspace.load(new AbortController().signal)).rejects.toThrow(
  ...
  it('exposes one inspect tool, screenshots, and no theme navigation or file tools', async () => {
    ...
    await expect(
      tools.find((tool) => tool.name === 'inspect')?.execute({}, new AbortController().signal),
    ).resolves.toMatchObject({ ok: true, data: { title: 'Artifact' } });
    await expect(
      tools
        .find((tool) => tool.name === 'inspect')
        ?.execute({ selector: 'h1' }, new AbortController().signal),
    ).resolves.toMatchObject({ ok: true, data: { tag: 'h1' } });
    await expect(
      tools
        .find((tool) => tool.name === 'screenshot')
        ?.execute({ kind: 'viewport' }, new AbortController().signal),
