...
async function archive(files: Record<string, string | Uint8Array>, order = Object.keys(files)): Promise<ArrayBuffer> {
    const zip = new JSZip();
...
function replaceByteOccurrences(buffer: ArrayBuffer, from: string, to: string, occurrences: number[]): ArrayBuffer {
...
function setCentralUncompressedSize(buffer: ArrayBuffer, path: string, byteLength: number): ArrayBuffer {
    const bytes = new Uint8Array(buffer.slice(0));
...
function input(themeArchive: ArrayBuffer): ThemeLoadInput {
    return {
        ...
        customSettings: [
            {id: 'layout', key: 'layout', type: 'select', value: 'Grid', default: 'List', options: ['List', 'Grid']},
            {id: 'show-author', key: 'show_author', type: 'boolean', value: false, default: true},
            {id: 'author-label', key: 'author_label', type: 'text', value: 'By', default: 'By', visibility: 'show_author:true'}
        ],
        site: {
            url: 'https://example.com/',
            contentApiKey: '0123456789abcdef',
            liveHtml: '<html><head><script defer src="https://cdn.example.com/portal.js" data-i18n="true"></script><script defer src="https://cdn.example.com/search.js" data-key="0123456789abcdef" data-styles="https://cdn.example.com/search.css" data-sodo-search="true"></script><link rel="stylesheet" href="/assets/built/screen.css?v=abc123"></head></html>'
        },
...
describe('loadThemeDraft', () => {
    it('loads normalized text, preserved binaries, settings, renderer config, and an initial revision', async () => {
        ...
        expect(draft.theme).toEqual({name: 'demo', version: '1.2.3', builtIn: false, rootPrefix: 'demo/'});
        expect(draft.files['index.hbs']).toMatchObject({kind: 'text', content: '<main>{{@site.title}}</main>'});
        expect(draft.files['assets/logo.png']).toMatchObject({kind: 'binary'});
        expect(Array.from(draft.files['assets/logo.png'].binary ?? [])).toEqual([137, 80, 78, 71, 0, 255]);
        ...
        expect(Object.keys(visibleThemeCustomSettings(draft.customSettings))).toEqual(['layout', 'show_author']);
        ...
        expect(Object.keys(visibleThemeCustomSettings(enabledSettings))).toEqual(['layout', 'show_author', 'author_label']);
        expect(draft.renderer).toMatchObject({
            ...
            config: {
                ...
                sodoSearch: {url: 'https://cdn.example.com/search.js', styles: 'https://cdn.example.com/search.css'}
            }
        });
    ...
    it('rejects an unsafe common root and Windows traversal separators', async () => {
        ...
        await expect(loadThemeDraft(input(traversalRoot))).rejects.toMatchObject({code: 'unsafe_path'});
    ...
    it('rejects duplicate normalized archive paths', async () => {
        ...
        await expect(loadThemeDraft(input(duplicate))).rejects.toMatchObject({code: 'duplicate_path'});
    });
    ...
    it('rejects unsupported text encodings', async () => {
        ...
        await expect(loadThemeDraft(input(themeArchive))).rejects.toMatchObject({code: 'unsupported_encoding'});
    });
    ...
    it('maps theme editor archive limits into a stable loader error', async () => {
        ...
        await expect(loadThemeDraft(input(await archive(files)))).rejects.toMatchObject({code: 'archive_limit'});
    });
    ...
    it('bounds inflation even when a compressed entry forges a small declared size', async () => {
        ...
        zip.file('demo/assets/huge.bin', new Uint8Array(THEME_EDITOR_ARCHIVE_LIMITS.maxExtractedBytes + 1), {
            binary: true,
            compression: 'DEFLATE',
        ...
        await expect(loadThemeDraft(input(themeArchive))).rejects.toMatchObject({code: 'archive_limit'});
    }, 15_000);

    it('rejects missing or invalid theme package metadata', async () => {
        const missing = await archive({'demo/index.hbs': '<main></main>'});
        const invalid = await archive({'demo/package.json': '{bad json}', 'demo/index.hbs': '<main></main>'});

        await expect(loadThemeDraft(input(missing))).rejects.toMatchObject({code: 'missing_metadata'});
        await expect(loadThemeDraft(input(invalid))).rejects.toMatchObject({code: 'invalid_metadata'});
    });
});
