...
const globalSettings: Array<{key: keyof ThemeGlobalSettings; type: 'color' | 'text' | 'image'; writable: boolean}> = [
...
function failure(draft: ThemeDraft, code: string, message: string, retryable = false, details?: unknown): ToolFailure {
    return {ok: false, revision: draft.revision, error: {code, message, retryable, ...(details === undefined ? {} : {details})}};
}
...
function customDescriptor(setting: ThemeCustomSetting, baseline?: ThemeCustomSetting): DesignSettingDescriptor {
    return {
...
export function listDesignSettings(draft: ThemeDraft, baseline = draft): BuilderToolResult<{settings: DesignSettingDescriptor[]}> {
...
function validateCustomValue(draft: ThemeDraft, setting: ThemeCustomSetting, value: unknown): BuilderToolResult<null> {
    if (setting.type === 'image') {
        return failure(draft, 'setting_read_only', `theme.${setting.key} is an image setting and is read-only in Builder v1.`);
    }
    if (setting.type === 'boolean' && typeof value !== 'boolean') {
        return failure(draft, 'invalid_setting_value', `theme.${setting.key} requires a boolean value.`);
    }
    if (setting.type === 'text' && value !== null && typeof value !== 'string') {
        return failure(draft, 'invalid_setting_value', `theme.${setting.key} requires a text or null value.`);
    }
    if (setting.type === 'color' && !validColor(value)) {
        return failure(draft, 'invalid_color', `theme.${setting.key} requires a six-digit hexadecimal color such as #AABBCC.`);
    }
    if (setting.type === 'select' && (typeof value !== 'string' || !setting.options.includes(value))) {
        return failure(draft, 'invalid_option', `theme.${setting.key} must be one of its listed choices.`, false, {choices: setting.options});
    }
    return {ok: true, revision: draft.revision, data: null};
}

export async function updateDesignSettings(draft: ThemeDraft, input: {revision: unknown; values: unknown}): Promise<ThemeCandidateResult<{updated: string[]}>> {
    if (typeof input.revision !== 'string' || input.revision !== draft.revision) {
        return failure(draft, 'stale_revision', 'The theme changed since this tool call was prepared. List the latest settings and retry.', true, {currentRevision: draft.revision});
    }
    if (!input.values || typeof input.values !== 'object' || Array.isArray(input.values) || Object.keys(input.values).length === 0) {
        return failure(draft, 'invalid_settings_update', 'Provide at least one setting identifier and value.');
    }
    ...
    for (const [identifier, value] of Object.entries(values)) {
        if (identifier.startsWith('global.')) {
            ...
            if (!definition.writable) {
                return failure(draft, 'setting_read_only', `${identifier} is an image setting and is read-only in Builder v1.`);
            }
            if (key === 'accent_color' && !validColor(value)) {
                return failure(draft, 'invalid_color', `${identifier} requires a six-digit hexadecimal color such as #AABBCC.`);
            }
            if (key !== 'accent_color' && value !== null && typeof value !== 'string') {
                return failure(draft, 'invalid_setting_value', `${identifier} requires a text or null value.`);
            }
            continue;
        }
        ...
        const setting = Object.hasOwn(draft.customSettings, key) ? draft.customSettings[key] : undefined;
        if (!setting) {
            return failure(draft, 'setting_not_found', `No design setting exists at ${identifier}.`);
        }
        if (!Object.hasOwn(visible, key)) {
            return failure(draft, 'setting_hidden', `${identifier} is currently hidden by the theme's visibility rules.`);
        }
    ...
    return {ok: true, revision: revised.revision, data: {updated: Object.keys(values)}, candidate: revised};
}
