  ...
  if (value && typeof value === 'object') {
    return Object.fromEntries(
      Object.entries(value)
        ...
        .flatMap(([key, item]) => (item === undefined ? [] : [[key, stableValue(item)]])),
    );
  }
  return value;
}
...
function publishMetadata(draft: ThemeDraft): unknown {
  return Object.fromEntries(
    Object.entries(revisionMetadata(draft)).filter(
      ([key]) => !['renderer', 'selection', 'virtualUrl'].includes(key),
    ),
  );
}
