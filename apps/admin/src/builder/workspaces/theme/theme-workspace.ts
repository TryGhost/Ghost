import type {
  ...
  BuilderToolResult,
  WorkspaceDiagnostic,
} from '@/builder/core/tool-types';
import type {
  BuilderAttachmentSnapshot,
  BuilderAttachmentSummary,
  BuilderAttachments,
} from '@/builder/core/attachments';
import type {
...
} from './preview/preview-inspection';
...
type ThemeMutationPreview = BuilderPreviewAdapter & {
  ...
  inspectPage?: (
    ...
    PreviewPageInspection & { diagnostics: WorkspaceDiagnostic[]; diagnosticsTruncated: boolean }
  >;
  inspectElement?: (
...
export class ThemeWorkspace implements BuilderWorkspace {
  ...
  async restore(snapshot: WorkspaceSnapshot): Promise<ValidationResult> {
    ...
    if (!isThemeDraft(promotedPayload) || !isThemeDraft(activePayload)) {
      return this.invalidSnapshot(
        'snapshot_invalid',
        'The Builder checkpoint is not a theme draft.',
      );
    }
    const promoted = await withThemeRevision(promotedPayload);
    const revised = checkpoint?.candidate
      ? await withThemeRevision(checkpoint.candidate)
      : promoted;
    const revisionsMatch =
      promoted.revision === promotedPayload.revision &&
      revised.revision === snapshot.revision &&
      activePayload.revision === snapshot.revision;
    if (!revisionsMatch) {
      return this.invalidSnapshot(
        'snapshot_revision_mismatch',
        'The Builder checkpoint revision does not match its contents.',
      );
    }
    let adopted = revised;
    ...
    if (restorePreview) {
      ...
      if (validation.revision !== snapshot.revision) {
        return this.invalidSnapshot(
          'snapshot_preview_revision_mismatch',
          'The preview could not restore the exact checkpoint revision.',
        );
      }
      ...
      if (adopted.revision !== validation.revision) {
        return this.invalidSnapshot(
          'snapshot_preview_revision_mismatch',
          'The preview restored a different theme revision.',
        );
      }
    }
    ...
    this.setState({
      ...
      dirty: publishRevision !== this.baselinePublishRevision,
      validation,
    });
    return validation;
  }
    ...
    return [
      {
        name: 'list_files',
        description:
          'List all files in the current theme with text or binary classification and byte size.',
      ...
      },
      {
        ...
        description:
          'Replace one exact, unambiguous text occurrence in a revision-checked theme file. For CSS, edit the stylesheet actually linked by the rendered theme; follow uncompiled_theme_asset.details.renderedPaths when provided.',
        inputSchema: mutationSchema({ oldText: { type: 'string' }, newText: { type: 'string' } }, [
          'oldText',
          'newText',
        ]),
        execute: (input, signal) =>
          this.enqueueMutation(signal, (draft) =>
            replaceInThemeFile(draft, {
      ...
      },
      {
        name: 'write_file',
        description:
          'Create or replace one text theme file at a safe revision-checked path. Theme-specific build scripts do not run, so write rendered assets rather than authoring-only sources.',
        inputSchema: mutationSchema({ content: { type: 'string' } }, ['content']),
        execute: (input, signal) =>
          this.enqueueMutation(signal, (draft) =>
            writeThemeFile(draft, {
              revision: input.revision as string,
              path: input.path as string,
      ...
      },
      {
        ...
        description:
          'List readable global and visible theme settings, including staged values and writability.',
        inputSchema: { type: 'object', properties: {}, additionalProperties: false },
        execute: () =>
          Promise.resolve(
            listDesignSettings(
              this.activeDraftForRead(),
              this.baselineDraft ?? this.requireDraft(),
            ),
          ),
      },
      {
        ...
        description:
          'Stage one or more writable global or visible theme settings against the current revision.',
        inputSchema: {
          type: 'object',
          properties: {
        ...
        execute: (input, signal) =>
          this.enqueueMutation(signal, (draft) =>
            updateDesignSettings(draft, { revision: input.revision, values: input.values }),
          ),
      },
      {
        name: 'inspect_page',
        description:
          'Inspect the current preview URL, title, accessibility outline, bounded page text, viewport, and diagnostics.',
        inputSchema: { type: 'object', properties: {}, additionalProperties: false },
        execute: (_input, signal) =>
          this.executePreviewRead(signal, 'inspect_page', (preview) =>
            preview.inspectPage?.(signal),
          ),
      },
      {
        name: 'inspect_element',
        description:
          'Inspect one accessible preview element by source marker or selector with bounded attributes, styles, text, box, and source origin.',
        inputSchema: {
          type: 'object',
          properties: {
        ...
        execute: (input, signal) =>
          this.executePreviewRead(signal, 'inspect_element', (preview) =>
            preview.inspectElement?.(
              {
                marker: input.marker as string | undefined,
      ...
      },
      {
        name: 'screenshot',
        description:
          'Capture a bounded viewport, full-page, or source-marker/selector element screenshot of the current preview.',
        inputSchema: {
          oneOf: [
            {
              type: 'object',
              properties: { kind: { type: 'string', const: 'viewport' } },
              required: ['kind'],
              additionalProperties: false,
            },
            {
              type: 'object',
              properties: { kind: { type: 'string', const: 'full_page' } },
              required: ['kind'],
              additionalProperties: false,
            },
            {
  ...
  applyInlineImageEdit(
    ...
    BuilderToolResult<{ path: string; marker: string; assetPath: string } & MutationRenderData>
  > {
    const revision = this.activeDraftForRead().revision;
    return this.enqueueMutation(
      signal,
      (draft) => editThemeImageAtMarker(draft, { ...input, revision }),
      { promote: true, requirePromotedSource: true },
    );
  }
  ...
  async publish(signal: AbortSignal): Promise<PublishResult> {
    ...
    if (!this.publishDraft) {
      return {
        ...
        error: {
          ...
          message: 'Theme publishing is not connected yet.',
          retryable: false,
        },
      };
    }
    ...
    if (result.ok) {
      ...
      if (publishedDraft.revision !== result.revision) {
        return {
          ...
          error: {
            ...
            message:
              'The published theme does not match the returned revision. Reload the Builder before continuing.',
            retryable: true,
          },
        };
      }
      ...
      this.setState({
        revision: activeDraft.revision,
        dirty: activePublishRevision !== this.baselinePublishRevision,
        validation,
      });
  ...
  private async executePreviewRead<T>(
    ...
    try {
      ...
      if (!pending) {
        return this.previewToolFailure(
          revision,
          'preview_unavailable',
          `The preview does not support ${tool}.`,
          true,
        );
      }
  ...
  private async executeNavigation(
    target: unknown,
    signal: AbortSignal,
  ): Promise<BuilderToolResult<{ url: string; status: number }>> {
    const revision = this.activeDraftForRead().revision;
    if (typeof target !== 'string' || !target || target.length > 8_192) {
      return this.previewToolFailure(
        revision,
        'invalid_navigation_target',
        'Provide a non-empty preview URL under 8192 characters.',
        false,
      );
    }
    const result = await this.executePreviewRead(signal, 'navigate', (preview) =>
      preview.navigate?.(target, signal),
    );
    if (!result.ok) {
      return result;
    }
    if (result.data.kind !== 'virtual') {
      ...
      return this.previewToolFailure(
        result.revision,
        diagnostic?.code ?? 'preview_navigation_blocked',
        diagnostic?.message ?? 'Agent navigation must stay on this site.',
        false,
        { url: result.data.url },
      );
    }
    return {
      ...
      data: { url: result.data.url, status: result.data.status },
    };
  }
  ...
  private async executeScreenshot(
    ...
    signal: AbortSignal,
  ): Promise<BuilderToolResult<Omit<ScreenshotResult, 'dataUrl'>>> {
    const revision = this.activeDraftForRead().revision;
    if (!['viewport', 'full_page', 'element'].includes(String(input.kind))) {
      return this.previewToolFailure(
        revision,
        'invalid_screenshot_request',
        'Screenshot kind must be viewport, full_page, or element.',
        false,
      );
    }
    ...
    if (
      (input.kind === 'element' && hasMarker === hasSelector) ||
      (input.kind !== 'element' && (input.marker !== undefined || input.selector !== undefined))
    ) {
      return this.previewToolFailure(
        revision,
        'invalid_screenshot_request',
        'Element screenshots require exactly one marker or selector; other screenshot kinds accept no target.',
        false,
      );
    }
    const request =
    ...
    const result = await this.executePreviewRead(signal, 'screenshot', (preview) =>
      preview.screenshot?.(request, signal),
    );
    if (!result.ok) {
      return result;
    }
    ...
    if (!result.data.dataUrl.startsWith(prefix)) {
      return this.previewToolFailure(
        result.revision,
        'preview_screenshot_failed',
        'The preview returned an invalid screenshot image.',
        true,
      );
    }
    return {
      ...
      attachments: [
        { type: 'image', mediaType: 'image/png', data: result.data.dataUrl.slice(prefix.length) },
      ],
    };
  }

  private previewToolFailure(
    revision: string,
    code: string,
    message: string,
    retryable: boolean,
  ...
  private enqueueMutation<T extends Record<string, unknown>>(
    ...
    const queued = this.mutationTail.then(async () => {
      ...
      if (options.requirePromotedSource && this.lastValidCandidate) {
        return {
          ...
          error: {
            ...
            message:
              'Inline editing is unavailable while interrupted agent changes are pending. Return to a checkpoint or continue the chat before editing the preview.',
            retryable: false,
          },
        };
      }
      ...
      if (!this.preview.renderCandidate) {
        return {
          ...
          error: {
            ...
            message:
              'The preview is not ready to validate theme changes. Reopen Builder and retry.',
            retryable: true,
          },
        };
      }
      ...
      try {
        ...
        return {
          ...
          error: {
            code: 'preview_failed',
            message:
              'The preview could not validate this change. Retry after the preview reconnects.',
            retryable: true,
      ...
      if (!validation.valid) {
        ...
        return {
          ...
          error: {
            code: 'render_invalid',
            message:
              'The change did not produce a valid preview. Repair the reported diagnostics and retry.',
            retryable: true,
      ...
      const previewDraft = this.preview.draft;
      const adopted =
        previewDraft?.revision === validation.revision ? previewDraft : result.candidate;
      if (adopted.revision !== validation.revision) {
        return {
          ok: false as const,
          revision: source.revision,
          error: {
            ...
            message:
              'The preview validated a different theme revision. Reload Builder before continuing.',
            retryable: true,
          },
        };
      }
