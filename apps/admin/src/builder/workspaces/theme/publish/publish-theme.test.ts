...
async function draft({builtIn = false, name = 'edition'}: {builtIn?: boolean; name?: string} = {}): Promise<ThemeDraft> {
    return withThemeRevision({
        revision: '',
        theme: {name, version: '1.2.3', builtIn, rootPrefix: `${name}/`},
        files: {
            'package.json': {path: 'package.json', kind: 'text', content: JSON.stringify({name, version: '1.2.3'}), binary: null, unixPermissions: null, dosPermissions: 0},
            'index.hbs': {path: 'index.hbs', kind: 'text', content: '<main>Changed</main>', binary: null, unixPermissions: null, dosPermissions: 0},
...
async function archiveFor(current: ThemeDraft, overrides: Record<string, string> = {}): Promise<ArrayBuffer> {
    const zip = new JSZip();
    for (const [path, file] of Object.entries(current.files)) {
        if (Object.hasOwn(overrides, path) || file.kind === 'text') {
            zip.file(`${current.theme.rootPrefix}${path}`, Object.hasOwn(overrides, path) ? overrides[path] : file.content ?? '', {
...
describe('ThemePublisher', () => {
    it('uploads an editable custom theme in place before applying only changed settings', async () => {
        ...
        expect(api.updateGlobalSettings).toHaveBeenCalledWith([{key: 'accent_color', value: '#123456'}], expect.any(AbortSignal));
        expect(api.updateCustomSettings).toHaveBeenCalledWith([{key: 'layout', value: 'Grid'}], expect.any(AbortSignal));
        expect(vi.mocked(api.upload).mock.invocationCallOrder[0]).toBeLessThan(vi.mocked(api.updateGlobalSettings).mock.invocationCallOrder[0] ?? 0);
        expect(vi.mocked(api.updateGlobalSettings).mock.invocationCallOrder[0]).toBeLessThan(vi.mocked(api.updateCustomSettings).mock.invocationCallOrder[0] ?? 0);
        ...
        expect(publisher.state).toMatchObject({status: 'complete', stage: 'complete', targetName: 'edition'});
    });
    ...
    it('validates, uploads, and activates a built-in theme copy before applying settings', async () => {
        ...
        const result = await publisher.publish(current, {copyName: 'source-edited'}, new AbortController().signal);

        expect(result.ok).toBe(true);
        if (!result.ok) {
            throw new Error('Expected publishing to succeed');
        }
        expect(result.draft).toMatchObject({theme: {name: 'source-edited', builtIn: false, rootPrefix: 'source-edited/'}});
        if (!result.draft) {
            throw new Error('Expected the published draft');
        }
        expect(result.revision).toBe(result.draft.revision);
        expect(api.upload).toHaveBeenCalledWith(expect.objectContaining({name: 'source-edited', copySettingsFrom: 'source'}), expect.any(AbortSignal));
        expect(api.activate).toHaveBeenCalledWith('source-edited', expect.any(AbortSignal));
        expect(vi.mocked(api.upload).mock.invocationCallOrder[0]).toBeLessThan(vi.mocked(api.activate).mock.invocationCallOrder[0] ?? 0);
    ...
    it.each([
        ...
        'contains spaces',
        `${'a'.repeat(65)}`
    ])('rejects invalid built-in copy name %j before upload', async (copyName) => {
        const current = await draft({builtIn: true, name: 'source'});
        const api = transport();
        const publisher = new ThemePublisher({baseline: current, transport: api});
        ...
        expect(result).toMatchObject({ok: false, error: {code: 'publish_validation_failed', retryable: false, details: {stage: 'validation'}}});
    ...
    it('reports a local archive validation failure without attempting an upload', async () => {
        ...
        expect(result).toMatchObject({ok: false, revision: current.revision, error: {code: 'publish_validation_failed', retryable: false, details: {stage: 'validation'}}});
        expect(api.upload).not.toHaveBeenCalled();
    });
    ...
    it('rejects a built-in copy name that would replace an installed custom theme', async () => {
        const current = await draft({builtIn: true, name: 'source'});
        const api = transport();
        const publisher = new ThemePublisher({baseline: current, installedThemeNames: ['Edition', 'source-edited'], transport: api});

        const result = await publisher.publish(current, {copyName: 'SOURCE-EDITED'}, new AbortController().signal);

        expect(result).toMatchObject({ok: false, error: {code: 'publish_validation_failed', retryable: false}});
        expect(api.upload).not.toHaveBeenCalled();
    });

    it('refuses to overwrite a custom theme changed on the server after Builder loaded', async () => {
        const current = await draft();
        const installedArchive = await archiveFor(current, {'index.hbs': '<main>Changed elsewhere</main>'});
        ...
        expect(result).toMatchObject({ok: false, error: {code: 'publish_conflict', retryable: false, details: {stage: 'validation'}}});
    ...
    it('retries from upload without mutating the retained draft', async () => {
        ...
        expect(failed).toMatchObject({ok: false, error: {code: 'publish_upload_failed', retryable: true, details: {stage: 'upload'}}});
    ...
    it('rechecks the installed custom theme before retrying a failed upload', async () => {
        ...
        const externalArchive = await archiveFor(current, {'index.hbs': '<main>Changed elsewhere</main>'});
        ...
        expect(failed).toMatchObject({ok: false, error: {code: 'publish_upload_failed', retryable: true}});
        expect(conflicted).toMatchObject({ok: false, error: {code: 'publish_conflict', retryable: false}});
    ...
    it('treats Ghost upload validation errors as non-retryable validation failures', async () => {
        const current = await draft({builtIn: true, name: 'source'});
        const api = transport({upload: () => Promise.reject(new ThemePublishRequestError(422, 'Theme validation failed'))});
        const publisher = new ThemePublisher({baseline: current, transport: api});

        const result = await publisher.publish(current, {copyName: 'source-edited'}, new AbortController().signal);

        expect(result).toMatchObject({ok: false, error: {code: 'publish_validation_failed', message: 'Theme validation failed', retryable: false}});
        expect(publisher.state).toMatchObject({status: 'failed', stage: 'validation', retryable: false});
    });
    ...
    it('treats Ghost settings validation errors as non-retryable settings failures', async () => {
        ...
        const baseline = structuredClone(current);
        baseline.customSettings.layout.value = 'List';
        const api = transport({updateCustomSettings: () => Promise.reject(new ThemePublishRequestError(422, 'Layout is not supported by this theme'))});
        const publisher = new ThemePublisher({baseline, transport: api});

        const result = await publisher.publish(current, {}, new AbortController().signal);

        expect(result).toMatchObject({ok: false, error: {code: 'publish_settings_failed', message: 'Layout is not supported by this theme', retryable: false}});
        expect(publisher.state).toMatchObject({status: 'failed', stage: 'settings', retryable: false});
    });
    ...
    it('resumes a built-in retry at activation without uploading twice', async () => {
        ...
        const failed = await publisher.publish(current, {copyName: 'source-edited'}, new AbortController().signal);
        const retried = await publisher.publish(current, {copyName: 'source-edited'}, new AbortController().signal);

        expect(failed).toMatchObject({ok: false, error: {code: 'publish_activation_failed', retryable: true, details: {stage: 'activation'}}});
        expect(retried.ok).toBe(true);
        expect(api.upload).toHaveBeenCalledOnce();
    ...
    it('resumes partial settings retries without repeating theme or completed setting stages', async () => {
        ...
        baseline.globalSettings.accent_color = '#000000';
        baseline.customSettings.layout.value = 'List';
        const updateCustomSettings = vi.fn()
        ...
        expect(failed).toMatchObject({ok: false, error: {code: 'publish_settings_failed', retryable: true, details: {stage: 'settings'}}});
        expect(retried.ok).toBe(true);
        expect(api.upload).toHaveBeenCalledOnce();
    ...
    it('reasserts an ambiguously failed global setting update after the draft changes', async () => {
        ...
        expect(failed).toMatchObject({ok: false, error: {code: 'publish_settings_failed', retryable: true}});
        expect(retried.ok).toBe(true);
        expect(updateGlobalSettings).toHaveBeenCalledTimes(2);
        expect(updateGlobalSettings).toHaveBeenNthCalledWith(1, [{key: 'accent_color', value: '#123456'}], expect.any(AbortSignal));
        expect(updateGlobalSettings).toHaveBeenNthCalledWith(2, [{key: 'accent_color', value: '#123456'}], expect.any(AbortSignal));
    });
    ...
    it('reasserts an ambiguously failed custom setting update after the draft changes', async () => {
        ...
        expect(retried.ok).toBe(true);
        expect(updateCustomSettings).toHaveBeenCalledTimes(2);
        expect(updateCustomSettings).toHaveBeenNthCalledWith(1, [{key: 'layout', value: 'Grid'}], expect.any(AbortSignal));
        expect(updateCustomSettings).toHaveBeenNthCalledWith(2, [{key: 'layout', value: 'Grid'}], expect.any(AbortSignal));
    });
});
...
describe('validateThemeCopyName', () => {
    it('normalizes a valid name and supplies the built-in default', () => {
        ...
        expect(validateThemeCopyName('Existing', 'source', ['existing'])).toEqual({ok: false, message: 'A theme named “existing” is already installed. Choose a different name for this copy.'});
    });
});
