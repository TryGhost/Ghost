import { ThemePublishRequestError } from '@tryghost/admin-x-framework/api/themes';
import type {
  ThemePublishTransport,
  ThemeSettingUpdate,
} from '@tryghost/admin-x-framework/api/themes';
export {
  createAdminThemePublishTransport,
  ThemePublishRequestError,
} from '@tryghost/admin-x-framework/api/themes';
export type {
  ThemePublishTransport,
  ThemeSettingUpdate,
...
import type {
  ...
  ThemeDraft,
  ThemeGlobalSettings,
} from '@/builder/workspaces/theme/theme-state';

export type ThemePublishStage =
  | 'idle'
  | 'validation'
  | 'upload'
  | 'activation'
...
export function validateThemeCopyName(
  ...
  if (!themeNamePattern.test(name)) {
    return {
      ok: false,
      message:
        'Use 1–64 characters, starting with a letter or number. Only letters, numbers, dashes, and underscores are allowed.',
    };
  }
  if (isDefaultThemeName(name)) {
    return {
      ok: false,
      message: 'Built-in themes cannot be overwritten. Choose a different name for the copy.',
    };
  }
  if (installedThemeNames.some((installed) => installed.toLowerCase() === name)) {
    return {
      ok: false,
      message: `A theme named “${name}” is already installed. Choose a different name for this copy.`,
    };
  }
  return { ok: true, name };
}
...
function editorFiles(draft: ThemeDraft): Record<string, ThemeEditorFile> {
  return Object.fromEntries(
    Object.entries(draft.files).map(([path, file]) => [
      path,
      {
        path,
...
function validatePackageFile(draft: ThemeDraft): void {
  const packageFile = Object.hasOwn(draft.files, 'package.json')
    ? draft.files['package.json']
    : undefined;
  if (!packageFile || packageFile.kind !== 'text' || packageFile.content === null) {
    throw new Error(
      'The theme must contain a readable package.json file before it can be published.',
    );
  }
  let parsed: unknown;
  try {
...
function changedGlobalSettings(
  current: ThemeGlobalSettings,
  baseline: ThemeGlobalSettings,
): ThemeSettingUpdate[] {
  return (Object.keys(current) as Array<keyof ThemeGlobalSettings>).flatMap((key) =>
    Object.is(current[key], baseline[key]) ? [] : [{ key, value: current[key] }],
  );
}

function changedCustomSettings(
  current: Record<string, ThemeCustomSetting>,
  baseline: Record<string, ThemeCustomSetting>,
): ThemeSettingUpdate[] {
  return Object.keys(current)
    ...
    .flatMap((key) => {
      ...
      return previous && Object.is(setting.value, previous.value)
        ? []
        : [{ key: setting.key, value: setting.value }];
    });
}

function globalSettingsForAttempt(
  current: ThemeGlobalSettings,
  baseline: ThemeGlobalSettings,
...
function customSettingsForAttempt(
  current: Record<string, ThemeCustomSetting>,
  baseline: Record<string, ThemeCustomSetting>,
  uncertainKeys: ReadonlySet<string>,
): ThemeSettingUpdate[] {
...
function archiveMatchesDraft(
  snapshot: Awaited<ReturnType<typeof extractThemeArchive>>,
  draft: ThemeDraft,
): boolean {
  const paths = Object.keys(draft.files).sort();
  const archivePaths = Object.keys(snapshot.files).sort();
  return (
    paths.length === archivePaths.length &&
...
export class ThemePublisher {
  ...
  constructor({
    ...
    baseline: ThemeDraft;
    transport: ThemePublishTransport;
    installedThemeNames?: readonly string[];
  ...
  async publish(
    ...
    const target = draft.theme.builtIn
      ? validateThemeCopyName(options.copyName, draft.theme.name, this.installedThemeNames)
      : { ok: true as const, name: draft.theme.name };
    if (!target.ok) {
    ...
    try {
      if (!attempt.validated) {
        ...
        const rootPrefix =
          attempt.draft.theme.builtIn && attempt.draft.theme.rootPrefix
            ? `${target.name}/`
            : attempt.draft.theme.rootPrefix;
      ...
      if (!attempt.uploaded) {
        if (!attempt.draft.theme.builtIn && this.transport.download) {
          ...
          const matchesLastAttempt =
            lastAttemptedUpload?.name === attempt.draft.theme.name &&
            archiveMatchesDraft(installed, lastAttemptedUpload.draft);
          if (archiveMatchesDraft(installed, this.serverBaseline)) {
            ...
            return this.failure(
              draft,
              'validation',
              'The installed theme changed after Builder was opened. Reload Builder before publishing so newer theme edits are not overwritten.',
              false,
              'publish_conflict',
            );
          }
      ...
      if (!attempt.uploaded) {
        ...
        if (!attempt.draft.theme.builtIn) {
          this.lastAttemptedUpload = {
            name: attempt.draft.theme.name,
            draft: cloneThemeDraft(attempt.draft),
          };
        }
      ...
      attempt.globalSettingsUpdates ??= globalSettingsForAttempt(
        attempt.draft.globalSettings,
        this.serverGlobalSettings,
        this.uncertainGlobalSettingKeys,
      );
      attempt.customSettingsUpdates ??= customSettingsForAttempt(
        attempt.draft.customSettings,
        this.serverCustomSettings,
        this.uncertainCustomSettingKeys,
      );
      if (!attempt.globalSettingsUpdated) {
        ...
        if (attempt.globalSettingsUpdates.length > 0) {
          this.onServerMutation();
          attempt.globalSettingsUpdates.forEach((update) =>
            this.uncertainGlobalSettingKeys.add(update.key),
          );
          ...
          abortIfNeeded(signal);
          attempt.globalSettingsUpdates.forEach((update) =>
            this.uncertainGlobalSettingKeys.delete(update.key),
          );
        }
      ...
      if (!attempt.customSettingsUpdated) {
        ...
        if (attempt.customSettingsUpdates.length > 0) {
          this.onServerMutation();
          attempt.customSettingsUpdates.forEach((update) =>
            this.uncertainCustomSettingKeys.add(update.key),
          );
          ...
          abortIfNeeded(signal);
          attempt.customSettingsUpdates.forEach((update) =>
            this.uncertainCustomSettingKeys.delete(update.key),
          );
        }
  ...
  private failure(
    draft: ThemeDraft,
    stage: Exclude<ThemePublishStage, 'idle' | 'complete'>,
    message: string,
    retryable: boolean,
    code = `publish_${stage === 'activation' ? 'activation' : stage}_failed`,
  ): ThemePublishResult {
    this.setState({
      status: 'failed',
      stage,
      targetName: this.currentAttempt?.targetName,
      error: message,
      retryable,
    });
    return {
