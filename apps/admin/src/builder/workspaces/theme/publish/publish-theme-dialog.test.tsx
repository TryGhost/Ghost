...
describe('PublishThemeDialog', () => {
    it('validates and confirms a built-in theme copy explicitly', async () => {
        ...
        expect(await screen.findByRole('alert')).toHaveTextContent('Built-in themes cannot be overwritten');
    ...
    it('retains an editable-theme failure and offers a retry', async () => {
        ...
        rerender(
            <PublishThemeDialog
                builtIn={false}
                dirty={true}
                publishState={{status: 'failed', stage: 'settings', targetName: 'edition', error: 'Settings unavailable'}}
                sessionStatus='ready'
                themeName='edition'
                onPublish={onPublish}
            />
    ...
    it('does not mark a valid copy name invalid for publish failures and closes non-retryable failures for editing', async () => {
        const onPublish = vi.fn(() => Promise.resolve({
            ok: false as const,
            revision: 'draft',
            error: {code: 'publish_conflict', message: 'The installed theme changed.', retryable: false}
        }));
        const {rerender} = render(
            <PublishThemeDialog
                builtIn={true}
        ...
        rerender(
            <PublishThemeDialog
                builtIn={true}
                dirty={true}
                publishState={{status: 'failed', stage: 'validation', error: 'The installed theme changed.', retryable: false}}
                sessionStatus='ready'
                themeName='source'
                onPublish={onPublish}
            />
    ...
    it('locks an uploaded copy name while retrying activation', async () => {
        const onPublish = vi.fn(() => Promise.resolve({
            ok: false as const,
            revision: 'draft',
            error: {code: 'publish_activation_failed', message: 'Activation unavailable', retryable: true}
        }));
        const {rerender} = render(
            <PublishThemeDialog
                builtIn={true}
        ...
        rerender(
            <PublishThemeDialog
                builtIn={true}
                dirty={true}
                publishState={{status: 'failed', stage: 'activation', error: 'Activation unavailable', retryable: true}}
                sessionStatus='ready'
                themeName='source'
                onPublish={onPublish}
            />
    ...
    it('keeps an uploaded copy name locked after a non-retryable settings failure', () => {
        render(
            <PublishThemeDialog
                builtIn={true}
                dirty={true}
                publishState={{status: 'failed', stage: 'settings', error: 'The setting is invalid', retryable: false}}
                sessionStatus='ready'
                themeName='source'
                onPublish={vi.fn()}
            />
