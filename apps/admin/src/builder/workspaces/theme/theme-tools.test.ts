...
describe('theme file tools', () => {
  it('rejects edits to authored CSS when the theme renders a compiled stylesheet', async () => {
    ...
    source.files['assets/css/screen.css'] = {
      ...
      kind: 'text',
      content: '@IMPORT url(global.css);\n.hero { color: red; }',
      binary: null,
      unixPermissions: null,
      dosPermissions: null,
    };
    source.files['assets/css/global.css'] = {
    ...
    source.files['assets/built/screen.css'] = {
      path: 'assets/built/screen.css',
      kind: 'text',
      content: 'body{color:#000}.hero{color:red}',
      binary: null,
      unixPermissions: null,
      dosPermissions: null,
    };
    ...
    await expect(
      ...
      error: {
        ...
        retryable: false,
        details: {
          sourcePath: 'assets/css/screen.css',
          renderedPaths: ['assets/built/screen.css'],
        },
      },
    });

    await expect(
      writeThemeFile(revised, {
        revision: revised.revision,
        path: 'assets/css/global.css',
      ...
      error: {
        code: 'uncompiled_theme_asset',
        details: {
          sourcePath: 'assets/css/global.css',
          renderedPaths: ['assets/built/screen.css'],
        },
      },
    });
  ...
  it('allows CSS edits when the authored stylesheet is loaded directly', async () => {
    ...
    source.files['default.hbs'] = {
      path: 'default.hbs',
      kind: 'text',
      content: '<link rel="stylesheet" href="{{asset "css/screen.css"}}">',
      binary: null,
      unixPermissions: null,
      dosPermissions: null,
    };
  ...
  it('applies an anchor-verified inline text edit to a new immutable candidate', async () => {
    ...
    expect(result).toMatchObject({
      ok: true,
      data: { path: 'index.hbs', marker: 'index.hbs:1:1' },
    });
    if (!result.ok) {
  ...
  it('rejects stale, unparseable, and mismatched inline text markers', async () => {
    ...
    ).resolves.toMatchObject({ ok: false, error: { code: 'invalid_source_marker' } });
    await expect(
      editThemeTextAtMarker(source, {
        revision: source.revision,
        marker: 'index.hbs:1:1',
    ...
    ).resolves.toMatchObject({ ok: false, error: { code: 'inline_edit_unavailable' } });
  });

  it('adds a bounded image asset and anchor-edits src while clearing responsive sources', async () => {
    const source = await draft();
    source.files['index.hbs'].content =
      '<img src="/old.png" srcset="/old-2x.png 2x" sizes="100vw">';
    const revised = await withThemeRevision(source);
    ...
    expect(result).toMatchObject({
      ok: true,
      data: { path: 'index.hbs', assetPath: 'assets/images/builder/new-hero.png' },
    });
    if (!result.ok) {
      throw new Error('Expected the inline image edit to succeed');
    }
    expect(result.candidate.files['index.hbs'].content).toBe(
      '<img src="{{asset "images/builder/new-hero.png"}}">',
    );
    expect(
      Array.from(result.candidate.files['assets/images/builder/new-hero.png'].binary ?? []).slice(
        0,
        8,
      ),
  ...
  it('searches literal and regular-expression queries with bounded line matches', async () => {
    ...
    expect(searchThemeFiles(source, { query: '[', regex: true })).toMatchObject({
      ok: false,
      error: { code: 'invalid_search_pattern' },
    });
  });
  ...
  it('reads bounded text ranges with line numbers and rejects binary reads', async () => {
    ...
    expect(readThemeFile(source, { path: 'index.hbs', startLine: 2, endLine: 2 })).toMatchObject({
      ...
      data: {
        ...
        endLine: 2,
        totalLines: 2,
        content: '2: <p>Welcome home</p>',
        truncated: false,
        next: null,
      },
    });
    expect(readThemeFile(source, { path: 'assets/logo.png' })).toMatchObject({
      ok: false,
  ...
  it('bounds a single long line and returns a resumable cursor', async () => {
    ...
    expect(
      readThemeFile(revised, {
        path: 'index.hbs',
        startLine: first.data.next.line,
        startColumn: first.data.next.column,
      }),
  ...
  it('returns a cursor when an explicit line range exceeds the read limit', async () => {
    const source = await draft();
    source.files['index.hbs'].content = Array.from(
      { length: THEME_TEXT_LIMITS.maxReadLines + 2 },
      (_, index) => `line ${index + 1}`,
    ).join('\n');
    const revised = await withThemeRevision(source);

    const result = readThemeFile(revised, {
      path: 'index.hbs',
      startLine: 1,
      endLine: THEME_TEXT_LIMITS.maxReadLines + 2,
    });

    expect(result).toMatchObject({
      ok: true,
      data: {
        endLine: THEME_TEXT_LIMITS.maxReadLines,
        truncated: true,
        next: { line: THEME_TEXT_LIMITS.maxReadLines + 1, column: 1 },
      },
    });
  });
  ...
  it('rejects regex shapes that can cause catastrophic backtracking', async () => {
    ...
    expect(searchThemeFiles(source, { query: '(a+)+$', regex: true })).toMatchObject({
      ok: false,
      error: { code: 'unsafe_search_pattern' },
    });
    expect(searchThemeFiles(source, { query: `${'a?'.repeat(5)}b`, regex: true })).toMatchObject({
      ok: false,
      error: { code: 'unsafe_search_pattern' },
    });
  });
  ...
  it.each([
    ...
    'bad\\path.hbs',
    'bad\0path.hbs',
  ])('rejects unsafe mutation path %s', async (path) => {
    const source = await draft();
    const result = await writeThemeFile(source, {
      revision: source.revision,
      path,
      content: 'unsafe',
    });
  ...
  it('replaces one exact occurrence in a new immutable candidate', async () => {
    const source = await draft();
    const result = await replaceInThemeFile(source, {
      revision: source.revision,
      path: 'index.hbs',
      oldText: 'Welcome home',
      newText: 'Hello',
    });
  ...
  it('rejects stale mutations and binary writes', async () => {
    ...
    await expect(
      writeThemeFile(source, { revision: 'theme-stale', path: 'new.hbs', content: 'new' }),
    ).resolves.toMatchObject({ ok: false, error: { code: 'stale_revision' } });
    await expect(
      writeThemeFile(source, {
        revision: source.revision,
        path: 'assets/logo.png',
        content: 'not an image',
      }),
    ).resolves.toMatchObject({ ok: false, error: { code: 'binary_file' } });
    await expect(
      writeThemeFile(source, {
        revision: source.revision,
        path: 'assets/new.png',
    ...
    await expect(
      deleteThemeFile(source, { revision: source.revision, path: 'constructor' }),
    ).resolves.toMatchObject({ ok: false, error: { code: 'file_not_found' } });
  });

  it('creates and replaces text files while preserving file metadata', async () => {
    const source = await draft();
    const created = await writeThemeFile(source, {
      revision: source.revision,
    ...
    const replaced = await writeThemeFile(created.candidate, {
  ...
  it('enforces per-file and total text size limits', async () => {
    ...
    await expect(
      writeThemeFile(revised, { revision: revised.revision, path: 'extra.txt', content: 'xx' }),
    ).resolves.toMatchObject({ ok: false, error: { code: 'theme_text_too_large' } });
  });
  ...
  it('deletes a file from a new candidate and reports missing files', async () => {
    ...
    await expect(
      deleteThemeFile(source, { revision: source.revision, path: 'missing.hbs' }),
    ).resolves.toMatchObject({ ok: false, error: { code: 'file_not_found' } });
  });
});
