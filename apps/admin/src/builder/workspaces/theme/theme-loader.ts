  ...
  while (offset < extra.byteLength) {
    if (offset + 4 > extra.byteLength) {
      throw new ThemeLoadError(
        'invalid_archive',
        'Failed to read the theme archive path metadata.',
      );
    }
    ...
    if (id === 0x7075) {
      if (foundPathField) {
        throw new ThemeLoadError(
          'invalid_archive',
          'Theme archive entries must not contain duplicate Unicode path metadata.',
        );
      }
      foundPathField = true;
      if (
        length >= 5 &&
        extra[valueOffset] === 1 &&
        view.getUint32(valueOffset + 1, true) === crc32(rawPath)
      ) {
        try {
          ...
          throw new ThemeLoadError(
            'unsupported_encoding',
            'Theme archive paths must use UTF-8 encoding.',
          );
        }
      }
    }
...
function validateArchivePaths(themeArchive: ArrayBuffer): void {
  ...
  if (files.length > THEME_EDITOR_ARCHIVE_LIMITS.maxFiles) {
    throw new ThemeLoadError(
      'archive_limit',
      `This theme archive contains too many files for the browser editor (${files.length}/${THEME_EDITOR_ARCHIVE_LIMITS.maxFiles}).`,
    );
  }
  ...
  if (declaredBytes > THEME_EDITOR_ARCHIVE_LIMITS.maxExtractedBytes) {
    throw new ThemeLoadError(
      'archive_limit',
      'This theme archive is too large to open in the browser editor.',
    );
  }
}
...
async function validateInflatedSize(
  themeArchive: ArrayBuffer,
  signal?: AbortSignal,
): Promise<void> {
  let zip: JSZip;
  try {
  ...
  try {
    for (const entry of Object.values(zip.files)) {
      ...
      if (!entry.dir) {
        extractedBytes += await streamedEntrySize(
          entry,
          THEME_EDITOR_ARCHIVE_LIMITS.maxExtractedBytes - extractedBytes,
          signal,
        );
      }
    }
...
function extractedFiles(
  files: Awaited<ReturnType<typeof extractThemeArchive>>['files'],
): Record<string, ThemeFile> {
  return Object.fromEntries(
    Object.entries(files).map(([path, file]) => {
      if (file.editable && file.content?.includes('\uFFFD')) {
        throw new ThemeLoadError(
          'unsupported_encoding',
          `Theme text file is not valid UTF-8: ${path}`,
        );
      }
      return [
        path,
...
function packageMetadata(files: Record<string, ThemeFile>): { name: string; version: string } {
  ...
  if (!packageFile || packageFile.kind !== 'text' || packageFile.content === null) {
    throw new ThemeLoadError(
      'missing_metadata',
      'The theme archive does not contain package.json metadata.',
    );
  }
  try {
    ...
    return {
      name: metadata.name,
      version:
        typeof metadata.version === 'string' && metadata.version ? metadata.version : '0.0.0',
    };
  } catch {
...
export function visibleThemeCustomSettings(
  settings: Record<string, ThemeCustomSetting>,
): Record<string, ThemeCustomSetting> {
  const definitions = Object.values(settings);
  const values = Object.fromEntries(
    definitions.map((setting) => [setting.key, setting.value]),
  ) as unknown as Record<string, string>;
  return Object.fromEntries(
    definitions
      .filter((setting) => isCustomThemeSettingVisible(setting, values))
      .map((setting) => [setting.key, structuredClone(setting)]),
  );
}

export async function loadThemeDraft(
  input: ThemeLoadInput,
  signal?: AbortSignal,
): Promise<ThemeDraft> {
  abortIfNeeded(signal);
