...
export function createThemeRendererWorkerHandler({
    ...
    const render = async (url: string): Promise<ThemeRenderResult> => {
        ...
        for (let hop = 0; hop <= 3; hop += 1) {
            ...
            return {status: response.status, html: await response.text(), url: currentUrl, diagnostics: []};
        }
        ...
        try {
            ...
            if (!cancelled.delete(request.id)) {
                postMessage({
                    ...
                    ok: false,
                    error: {code: 'theme_renderer_error', message: error instanceof Error ? error.message : String(error)}
                });
            }
        }
    };
...
if (isWorkerScope) {
    const handleRequest = createThemeRendererWorkerHandler({postMessage: response => self.postMessage(response)});
