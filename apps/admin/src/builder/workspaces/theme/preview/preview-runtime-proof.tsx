...
import type {ThemeRendererClient, ThemeRendererInitialization, ThemeRenderResult} from './preview-bridge';
...
async function proofDraft(content: string): Promise<ThemeDraft> {
    return withThemeRevision({
        revision: '',
        theme: {name: 'preview-proof', version: '1.0.0', builtIn: false, rootPrefix: 'preview-proof/'},
        files: {
            'package.json': {path: 'package.json', kind: 'text', content: '{"name":"preview-proof"}', binary: null, unixPermissions: null, dosPermissions: 0},
        ...
        customSettings: {},
        renderer: {siteUrl: 'https://example.com/', contentApiKey: 'proof-key', config: {}, missing: []},
...
const PreviewRuntimeProof = () => {
    ...
    useEffect(() => {
        ...
        const adapter = new ThemePreviewAdapter({rendererFactory: () => Promise.resolve(new ProofRenderer()), surface});
        adapterRef.current = adapter;
        let active = true;
        void proofDraft('<p>Initial</p>').then(initial => adapter.start(initial, new AbortController().signal)).then(() => {
            if (active) {
    ...
    const bypassAfterReady = async () => {
        ...
        const validation = await adapter.renderCandidate(bypassing, new AbortController().signal);
        setResult(`${validation.valid ? 'late-bypass-unexpected' : 'late-bypass-blocked'}:${adapter.state.url}`);
    };

    return (
        <Box className='size-full bg-background' padding='lg'>
            <Stack gap='md'>
                <Text as='h1' size='xl' weight='semibold'>Theme preview browser proof</Text>
                <Inline gap='sm'>
                    <Button type='button' onClick={() => void navigate()}>Navigate preview</Button>
                    <Button type='button' variant='outline' onClick={() => void breakPreview()}>Break candidate</Button>
                    <Button type='button' variant='outline' onClick={() => void repairPreview()}>Repair candidate</Button>
                    <Button type='button' variant='outline' onClick={() => void bypassBridge()}>Bypass bridge</Button>
                    <Button type='button' variant='outline' onClick={() => void bypassAfterReady()}>Bypass after ready</Button>
                </Inline>
                <Text data-testid='preview-proof-result'>{result}</Text>
                <iframe ref={iframeRef} className='h-96 w-full rounded-md border border-border' data-testid='preview-proof-frame' title='Theme preview proof' />
            </Stack>
        </Box>
    );
};
