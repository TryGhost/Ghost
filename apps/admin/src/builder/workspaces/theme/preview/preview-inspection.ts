...
function sourceLocation(element: Element): {
  source: PreviewSourceLocation | null;
  truncated: boolean;
} {
  ...
  return {
    source: {
      path: marker.file.slice(0, PREVIEW_INSPECTION_LIMITS.maxTargetCharacters),
      line: marker.line,
...
function elementStyles(element: Element): CSSStyleDeclaration | null {
  return (
    elementWindow(element)?.getComputedStyle(element) ?? (element as HTMLElement).style ?? null
  );
}
...
export function resolvePreviewElement(document: Document, target: PreviewElementTarget): Element {
  ...
  if (value.length > PREVIEW_INSPECTION_LIMITS.maxTargetCharacters) {
    throw new PreviewInspectionError(
      ...
      `Preview targets must stay under ${PREVIEW_INSPECTION_LIMITS.maxTargetCharacters} characters.`,
    );
  }
  ...
  if (hasMarker) {
    ...
    try {
      ...
      throw new PreviewInspectionError(
        'invalid_preview_selector',
        'The preview selector is invalid.',
      );
    }
  }
  if (!element) {
    throw new PreviewInspectionError(
      'preview_element_not_found',
      `No preview element matches the ${hasMarker ? 'source marker' : 'selector'}.`,
    );
  }
...
export function inspectPreviewPage(document: Document, url: string): PreviewPageInspection {
  const candidates = Array.from(document.querySelectorAll(outlineSelector)).filter(
    (element) => !isInaccessible(element),
  );
  const visible = candidates.slice(0, PREVIEW_INSPECTION_LIMITS.maxOutlineItems);
  const pageText = boundedText(
    document.body ? visibleText(document.body) : '',
    PREVIEW_INSPECTION_LIMITS.maxTextCharacters,
  );
