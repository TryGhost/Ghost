...
async function draft(content: string, assets: Record<string, string> = {}): Promise<ThemeDraft> {
    return withThemeRevision({
        ...
        files: {
            ...
            ...Object.fromEntries(Object.entries(assets).map(([path, assetContent]) => [path, {path, kind: 'text' as const, content: assetContent, binary: null, unixPermissions: null, dosPermissions: null}]))
        },
...
describe('production preview interaction', () => {
    it('edits source-mapped text in place and rerenders the workspace candidate', async () => {
        ...
        renderer.render.mockImplementation(() => Promise.resolve(rendered(
        ...
        await expect(adapter.inspectElement({marker: 'index.hbs:1:1'}, new AbortController().signal)).resolves.toMatchObject({text: 'Edited inline'});
    });
    ...
    it('replaces a source-mapped image and serves the new candidate asset', async () => {
        ...
        await frame.getByTestId('builder-inline-image-input').upload(new File([bytes], 'replacement.png', {type: 'image/png'}));

        await expect.poll(() => workspace?.draft.files['index.hbs'].content).toContain('{{asset "images/builder/replacement.png"}}');
        expect(Array.from(workspace.draft.files['assets/images/builder/replacement.png'].binary ?? []).slice(0, 8)).toEqual([137, 80, 78, 71, 13, 10, 26, 10]);
        await expect.poll(async () => (await adapter.inspectElement({marker: 'index.hbs:1:1'}, new AbortController().signal)).accessibleName).toBe('Hero loaded 1');
    });
    ...
    it('loads candidate theme assets and refreshes them after an agent mutation', async () => {
        ...
        iframes.push(iframe);
        const renderer = new BrowserRenderer();
        const html = '<html><head><link rel="stylesheet" href="/assets/built/screen.css?v=live"></head><body><main id="asset-target" data-edit="index.hbs:1:1">Fallback</main><script type="module" src="/assets/built/candidate.js?v=live"></script></body></html>';
        ...
        await adapter.start(await draft('<main>Initial</main>', {
            'assets/built/screen.css': '#asset-target { color: rgb(255, 0, 0); background-image: url("/content/images/fallback.png"); }',
            'assets/built/candidate.js': 'import {label} from "./chunk.js"; export const prefix = "Initial"; const target = document.querySelector("#asset-target"); target.textContent = `${label()}|${getComputedStyle(target).backgroundImage}`; window.addEventListener("load", () => target.click(), {once: true});',
        ...
        const validation = await adapter.renderCandidate(await draft('<main>Updated</main>', {
            'assets/built/screen.css': '#asset-target { color: rgb(0, 0, 255); background-image: url("/content/images/fallback.png"); }',
            'assets/built/candidate.js': 'import {label} from "./chunk.js"; export const prefix = "Updated"; const target = document.querySelector("#asset-target"); target.textContent = `${label()}|${getComputedStyle(target).backgroundImage}`;',
            'assets/built/chunk.js': 'import {prefix} from "./candidate.js"; export const label = () => `${prefix} candidate`;'
        }), new AbortController().signal);
