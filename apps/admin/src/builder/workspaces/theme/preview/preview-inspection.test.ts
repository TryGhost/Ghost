...
describe('preview inspection', () => {
  ...
  it('rejects missing markers, invalid selectors, and inaccessible elements with stable codes', () => {
    const document = previewDocument(
      '<main><p id="hidden" style="display:none" data-edit="index.hbs:1:1">Hidden</p><section inert><button id="inert" data-edit="index.hbs:2:1">Inactive</button></section></main>',
    );

    expect(() => inspectPreviewElement(document, { marker: 'missing.hbs:1:1' })).toThrowError(
      expect.objectContaining<Partial<PreviewInspectionError>>({
        code: 'preview_element_not_found',
      }),
    );
    expect(() => inspectPreviewElement(document, { selector: '[' })).toThrowError(
      expect.objectContaining<Partial<PreviewInspectionError>>({
        code: 'invalid_preview_selector',
      }),
    );
    expect(() => inspectPreviewElement(document, { selector: '#hidden' })).toThrowError(
      expect.objectContaining<Partial<PreviewInspectionError>>({
        code: 'preview_element_inaccessible',
      }),
    );
    expect(() => inspectPreviewElement(document, { selector: '#inert' })).toThrowError(
      expect.objectContaining<Partial<PreviewInspectionError>>({
        code: 'preview_element_inaccessible',
      }),
    );
  ...
  it('omits hidden, aria-hidden, and inert descendant text', () => {
    const document = previewDocument(
      '<main>Visible <span hidden>Hidden</span><span aria-hidden="true">Aria hidden</span><span inert>Inert</span></main>',
    );
  ...
  it('uses native control roles and associated labels', () => {
    const document = previewDocument(
      '<label for="updates">Email updates</label><input id="updates" type="checkbox" data-edit="index.hbs:1:40">',
    );
  ...
  it('uses visible descendant alternatives for control names', () => {
    const document = previewDocument(
      '<button data-edit="index.hbs:1:1"><span aria-hidden="true">Ignore</span><img alt="Search"></button>',
    );
  ...
  it('bounds explicit roles and source paths while reporting the truncation', () => {
    ...
    const document = previewDocument(
      `<main role="${role}" data-edit="${sourcePath}:2:3">Bounded</main>`,
    );

    const result = inspectPreviewPage(document, 'https://example.com/');

    expect(result.outline[0]?.role).toHaveLength(64);
    expect(result.outline[0]?.source?.path).toHaveLength(
      PREVIEW_INSPECTION_LIMITS.maxTargetCharacters,
    );
  ...
  it('rejects non-finite source locations', () => {
    const document = previewDocument(
      `<main data-edit="index.hbs:${'9'.repeat(400)}:1">Invalid marker</main>`,
    );
  ...
  it('requires exactly one bounded marker or selector target', () => {
    ...
    expect(() => inspectPreviewElement(document, {})).toThrowError(
      expect.objectContaining<Partial<PreviewInspectionError>>({ code: 'invalid_preview_target' }),
    );
    expect(() =>
      inspectPreviewElement(document, { marker: 'index.hbs:1:1', selector: 'main' }),
    ).toThrowError(
      expect.objectContaining<Partial<PreviewInspectionError>>({ code: 'invalid_preview_target' }),
    );
    expect(() => inspectPreviewElement(document, { selector: tooLong })).toThrowError(
      expect.objectContaining<Partial<PreviewInspectionError>>({
