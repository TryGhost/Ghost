...
function backgroundImageUrls(element: Element, document: Document): URL[] {
  const value =
    document.defaultView?.getComputedStyle(element).backgroundImage ??
...
export async function captureDocumentScreenshot(
  ...
  if (
    area.width > SCREENSHOT_LIMITS.maxDimension ||
    area.height > SCREENSHOT_LIMITS.maxDimension ||
    area.width * area.height > SCREENSHOT_LIMITS.maxPixels
  ) {
    throw new Error(
      `The screenshot target exceeds the ${SCREENSHOT_LIMITS.maxDimension}px or ${SCREENSHOT_LIMITS.maxPixels}-pixel size limit.`,
    );
  }
...
export function capturePreviewScreenshot(
  ...
  return captureDocumentScreenshot(
    document,
    {
