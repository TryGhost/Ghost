...
export class ThemePreviewAdapter implements BuilderPreviewAdapter {
  ...
  private currentState: ThemePreviewState = {
    revision: '',
    url: '',
    status: null,
    diagnostics: [],
  ...
  constructor({
    ...
    rendererFactory: ThemeRendererClientFactory;
    surface: PreviewDocumentSurface;
    onInlineEdit?: (
    ...
    this.unsubscribeNavigate = surface.onNavigate((url) => {
      ...
      void this.enqueue(signal, (operationSignal) =>
        this.navigateNow(url, operationSignal, true),
      ).catch(() => {});
    });
    ...
    this.unsubscribeDiagnostic = surface.onDiagnostic((runtimeDiagnostic) => {
      ...
      this.setState({
        ...this.currentState,
        diagnostics: diagnostics.slice(-MAX_PREVIEW_DIAGNOSTICS),
      });
    });
    this.unsubscribeInlineEdit =
      onInlineEdit && surface.onInlineEdit ? surface.onInlineEdit(onInlineEdit) : () => {};
  }
  ...
  rebaseDraft(draft: ThemeDraft): void {
    ...
    if (
      previous.virtualUrl !== draft.virtualUrl ||
      previous.selection?.id !== draft.selection?.id
    ) {
  ...
  > {
    return this.enqueue(signal, (operationSignal) => {
      ...
      return this.surface.inspectPage(this.currentState.url, operationSignal).then((result) => ({
        ...result,
        diagnostics: structuredClone(this.currentState.diagnostics),
  ...
  private async startNow(draft: ThemeDraft, signal: AbortSignal): Promise<ValidationResult> {
    for (let attempt = 0; attempt < 2; attempt += 1) {
      ...
      try {
        ...
        if (
          !(error instanceof ThemeRendererTransportError) ||
          attempt === 1 ||
          signal.aborted ||
          this.destroyed
        ) {
  ...
  private async renderCandidateNow(
    ...
    try {
      ...
      await this.commit(renderedDraft, result, signal, {
        preserveSelection: expectedRevision !== undefined,
        expectedRevision,
      });
      return {
        valid: true,
  ...
  private async navigateNow(
    ...
    if (!['http:', 'https:', 'mailto:', 'tel:'].includes(resolved.protocol)) {
      return this.blockNavigation(
        resolved.href,
        `The preview cannot open ${resolved.protocol} links.`,
      );
    }
    ...
    try {
      const result = await this.withRestart(
        (client) => client.render(resolved.href, this.rendererRevision, signal),
        signal,
      );
      this.assertRenderable(result);
      let resultUrl: URL;
      ...
      if (!isSameSiteDestination(draft, resultUrl)) {
        return this.blockNavigation(
          result.url,
          'The rendered preview redirected outside this site.',
        );
      }
      const navigated = {
        ...draft,
        virtualUrl: result.url,
  ...
  private async withRestart<T>(
    operation: (client: ThemeRendererClient) => Promise<T>,
    signal: AbortSignal,
  ): Promise<T> {
  ...
  private async restoreRenderer(
    ...
    try {
      ...
      await this.requireRenderer().setTheme(
        textTheme(draft),
        draft.revision,
        signal,
        rendererSettings(draft),
  ...
  private async commit(
    ...
    signal: AbortSignal,
    {
      ...
      expectedRevision,
    }: { preserveSelection?: boolean; expectedRevision?: string } = {},
  ): Promise<void> {
    this.throwIfUnavailable(signal);
    ...
    this.throwIfUnavailable(signal);
    const selection =
      preserveSelection && remappedSelection?.id === draft.selection?.id
        ? draft.selection
        : remappedSelection;
    const selectionDiagnostic: WorkspaceDiagnostic[] =
      draft.selection && !selection
        ? [
            {
              code: 'preview_selection_cleared',
              message:
                'The selected source element no longer exists in the rendered page, so the selection was cleared.',
              severity: 'info',
            },
          ]
        : [];
  ...
  private blockNavigation(url: string, message: string): ThemeNavigationResult {
    const diagnostics = [
      { code: 'preview_navigation_blocked', message, severity: 'error' as const },
    ];
  ...
  private async adoptSelection(selection: BuilderSelectionContext | null): Promise<void> {
    ...
    if (
      ...
      this.lastValidDraft?.revision !== previousRevision
    ) {
      return;
    }
    this.lastValidDraft = revised;
