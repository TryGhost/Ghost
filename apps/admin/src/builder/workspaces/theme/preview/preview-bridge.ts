...
export type ThemeRendererCandidateSettings = Pick<
  ThemeRendererInitialization,
  'settingsPayload' | 'customThemeSettings'
>;
...
export type ThemeRendererWorkerRequest =
  | {
      ...
      type: 'initialize';
      revision: string;
      payload: Omit<ThemeRendererInitialization, 'revision'>;
    }
  | {
      id: number;
      type: 'set-theme';
      revision: string;
      payload: { theme: Record<string, string> } & ThemeRendererCandidateSettings;
    }
...
export function createThemeRendererClient({
  ...
  worker.onmessage = (event) => {
    ...
    if (response.revision !== request.revision) {
      request.reject(
        new ThemeRendererTransportError('The renderer returned a response for the wrong revision.'),
      );
    } else if (response.ok) {
  ...
  worker.onerror = (event) =>
    rejectAll(
      new ThemeRendererTransportError(
        `Theme renderer worker crashed: ${event.message || 'unknown error'}`,
      ),
    );
  worker.onmessageerror = () =>
    rejectAll(
      new ThemeRendererTransportError('Theme renderer worker returned an unreadable response.'),
    );

  const call = (
    request: WithoutId<ThemeRendererWorkerRequest>,
    signal: AbortSignal,
  ): Promise<ThemeRenderResult | undefined> => {
    if (destroyed) {
      return Promise.reject(
        new ThemeRendererTransportError('Theme renderer worker has been destroyed.'),
      );
    }
    if (signal.aborted) {
    ...
    return new Promise((resolve, reject) => {
      const handleAbort = () => {
        ...
        } satisfies ThemeRendererWorkerRequest);
      ...
      const timeout = setTimeout(() => {
        ...
        reject(
          new ThemeRendererTransportError(
            `Theme renderer worker timed out during ${request.type}.`,
          ),
        );
      }, timeoutMs);
  ...
  return {
    ...
    async render(url, revision, signal) {
      const result = await call(
        { type: 'render', revision, payload: { url, markers: true } },
        signal,
      );
      if (!result) {
