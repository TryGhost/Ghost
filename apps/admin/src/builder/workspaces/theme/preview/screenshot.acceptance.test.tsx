...
function waitForCaptureFrame(): Promise<HTMLIFrameElement> {
    return new Promise((resolve, reject) => {
        ...
        const observer = new MutationObserver(() => {
            const frame = document.querySelector<HTMLIFrameElement>('iframe[sandbox="allow-same-origin"]');
            if (!frame) {
                return;
            }
            observer.disconnect();
...
async function centerPixel(dataUrl: string): Promise<[number, number, number, number]> {
    ...
    return Array.from(context?.getImageData(Math.floor(image.width / 2), Math.floor(image.height / 2), 1, 1).data ?? [0, 0, 0, 0]) as [number, number, number, number];
}
...
describe('preview screenshot proof', () => {
    ...
    it('captures through the production opaque-origin preview bridge', async () => {
        ...
        await replacing;
        const screenshot = await surface.screenshot({kind: 'element', marker: 'index.hbs:1:1'}, new AbortController().signal);
    ...
    it('replaces an unreadable runtime canvas with a warning', async () => {
        ...
        expect(screenshot.dataUrl).toMatch(/^data:image\/png;base64,/);
        expect(screenshot.warnings).toEqual(['1 canvas was replaced in the screenshot because its pixels could not be read.']);
        surface.destroy();
    });
    ...
    it('rejects an oversized capture before allocating a canvas', async () => {
        ...
        await expect(capturePreviewScreenshot(iframe, {kind: 'element', selector: '#target'})).rejects.toThrow('size limit');
    });
});
