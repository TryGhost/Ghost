...
import {
  ...
  ThemePublisher,
} from './workspaces/theme/publish/publish-theme';
...
export default function PublishRuntimeProof() {
    ...
    void proofDraft(builtIn).then(async (initial) => {
      ...
      const workspace = new ThemeWorkspace({
        ...
        publish: (draft, signal) =>
          publisher.publish(draft, { copyName: copyNameRef.current }, signal),
      });
      ...
      const fileResult = await write!.execute(
        {
  ...
  return (
    <Stack className="fixed inset-0 items-center justify-center bg-background p-6" gap="md">
      ...
      <Text data-testid="publish-proof-result">
        Result:{' '}
        {result
          ? result.ok
