...
import type {ThemeRendererCandidateSettings, ThemeRendererClient, ThemeRendererInitialization, ThemeRenderResult} from './workspaces/theme/preview/preview-bridge';
...
const steps = [
    {oldText: 'Initial', newText: 'First', color: '#111111', url: 'https://example.com/first/', selection: 'First hero'},
    {oldText: 'First', newText: 'Second', color: '#222222', url: 'https://example.com/second/', selection: 'Second hero'},
    {oldText: 'Initial', newText: 'Branched', color: '#333333', url: 'https://example.com/branch/', selection: 'Branch hero'}
] as const;
...
class RewindProofRenderer implements ThemeRendererClient {
    ...
    setTheme(theme: Record<string, string>, _revision: string, signal: AbortSignal, settings: ThemeRendererCandidateSettings = {}): Promise<void> {
        if (signal.aborted) {
    ...
    render(url: string, _revision: string, signal: AbortSignal): Promise<ThemeRenderResult> {
        ...
        const accent = typeof this.settings.settingsPayload?.accent_color === 'string' ? this.settings.settingsPayload.accent_color : 'none';
        return Promise.resolve({
            status: 200,
            url,
            diagnostics: [],
...
async function executeTool(request: BuilderModelTurnRequest, name: string, input: Record<string, unknown>): Promise<BuilderToolResult<unknown>> {
    ...
    async runTurn(request: BuilderModelTurnRequest): Promise<void> {
        ...
        const selected = await withThemeRevision({
            ...
            selection: {
                ...
                data: {tagName: 'main', marker: 'index.hbs:1:1', source: {path: 'index.hbs', line: 1, column: 1}}
            }
        });
        ...
        request.onEvent({type: 'assistant-text-delta', text: `Applied ${step.newText.toLowerCase()} state at ${setting.revision}.`});
...
export default function RewindRuntimeProof() {
    ...
    const [phase, setPhase] = useState<'initial' | 'first' | 'second' | 'rewound' | 'continued'>('initial');
    ...
    useEffect(() => {
        ...
        const preview = new ThemePreviewAdapter({rendererFactory: () => Promise.resolve(new RewindProofRenderer()), surface});
        ...
        void initialDraft().then(async (draft) => {
            ...
            nextSession = new BuilderSession({workspace: nextWorkspace, modelAccess: new RewindProofModelAccess(preview)});
    ...
    const preview = (
        <Stack className='size-full p-6' gap='md'>
            <Text as='h2' size='lg' weight='semibold'>Theme rewind proof</Text>
            <Button disabled={phase !== 'initial' || state.status !== 'ready'} type='button' onClick={() => start('First edit', 'first')}>Run first edit</Button>
            <Button disabled={phase !== 'first' || state.status !== 'ready'} type='button' onClick={() => start('Second edit', 'second')}>Run second edit</Button>
            <Button disabled={phase !== 'rewound' || state.status !== 'ready'} type='button' onClick={() => start('Continue on a new branch', 'continued')}>Continue after rewind</Button>
            ...
            <Text data-testid='rewind-proof-selection'>Selection: {previewState.selection?.label ?? 'none'}</Text>
            <iframe ref={setIframe} className='min-h-64 w-full flex-1 rounded-md border border-border' title='Rewind proof preview' />
        </Stack>
    );
    ...
    return (
        <BuilderShell
            ...
            onRewind={(messageId) => session?.rewind(messageId).then(async () => {
                await refreshPreviewText();
                setPhase('rewound');
            ...
            onSelectModel={() => {}}
            onStop={() => session?.stop()}
            onSubmit={message => session?.startTurn(message) ?? Promise.reject(new Error('The rewind proof is not ready.'))}
        />
    );
}
