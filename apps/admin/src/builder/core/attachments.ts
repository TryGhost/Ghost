...
export type BuilderAttachmentError = {
    name: string;
    code: 'attachment_too_large' | 'attachment_limit_reached' | 'attachment_session_limit_reached' | 'unsupported_attachment_type' | 'attachment_upload_failed';
    message: string;
};

type TextAttachmentSnapshot = Readonly<BuilderAttachmentSummary & {kind: 'text'; content: string}>;
type ImageAttachmentSnapshot = Readonly<BuilderAttachmentSummary & {kind: 'image'; url: string; data: string; source?: string}>;
...
function attachmentType(file: File): {kind: 'text' | 'image'; mediaType: string} | null {
    ...
    if (imageTypes.has(file.type) || imageExtensions.has(fileExtension)) {
        return {kind: 'image', mediaType: imageTypes.has(file.type) ? file.type : imageExtensions.get(fileExtension)!};
    }
    if (textTypes.has(file.type) || textExtensions.has(fileExtension)) {
        return {kind: 'text', mediaType: textTypes.has(file.type) ? file.type : textExtensions.get(fileExtension)!};
    }
    return null;
}
...
export class BuilderAttachments {
    ...
    private readonly listeners = new Set<(attachments: readonly BuilderAttachmentSummary[]) => void>();
    private sequence = 0;
    private retainedBytes = 0;

    constructor({uploadImage, maxFileBytes = defaultMaxFileBytes, maxAttachments = defaultMaxAttachments, maxRetainedBytes}: BuilderAttachmentsOptions) {
        this.uploadImage = uploadImage;
        this.maxFileBytes = Math.max(1, Math.floor(maxFileBytes));
        this.maxAttachments = Math.max(1, Math.floor(maxAttachments));
        this.maxRetainedBytes = Math.max(this.maxFileBytes, Math.floor(maxRetainedBytes ?? this.maxFileBytes * this.maxAttachments));
    }
    ...
    snapshot(): BuilderAttachmentSnapshot {
        return Object.freeze({version: 1, attachments: Object.freeze(Array.from(this.attachments.values()))});
    }

    restore(snapshot: BuilderAttachmentSnapshot): void {
        if (!isRecord(snapshot) || snapshot.version !== 1 || !Array.isArray(snapshot.attachments) || snapshot.attachments.length > this.maxAttachments) {
        ...
        for (const value of snapshot.attachments) {
            ...
            if (value.kind === 'text') {
                ...
                restored.set(value.id, Object.freeze({
                    id: value.id,
                    name: value.name,
                    kind: 'text',
                ...
                if (typeof value.url !== 'string'
                    ...
                    || (value.source !== undefined && (value.mediaType !== 'image/svg+xml' || typeof value.source !== 'string' || value.source.length > this.maxFileBytes))) {
                    throw new Error('The Builder attachment checkpoint is invalid.');
                }
                restored.set(value.id, Object.freeze({
                    id: value.id,
    ...
    async add(files: readonly File[], signal = new AbortController().signal): Promise<{added: BuilderAttachmentSummary[]; errors: BuilderAttachmentError[]}> {
        ...
        for (const file of files) {
            ...
            if (this.attachments.size >= this.maxAttachments) {
                errors.push({name: file.name, code: 'attachment_limit_reached', message: `Builder supports up to ${this.maxAttachments} attachments in one session.`});
                continue;
            }
            if (file.size > this.maxFileBytes) {
                errors.push({name: file.name, code: 'attachment_too_large', message: `${file.name} is larger than the ${Math.floor(this.maxFileBytes / (1024 * 1024)) || 1} MB attachment limit.`});
                continue;
            }
            if (this.retainedBytes + file.size > this.maxRetainedBytes) {
                errors.push({name: file.name, code: 'attachment_session_limit_reached', message: 'This Builder session has reached its attachment storage limit. Reopen Builder to start with a fresh attachment set.'});
                continue;
            }
            const type = attachmentType(file);
            if (!type) {
                errors.push({name: file.name, code: 'unsupported_attachment_type', message: 'Builder supports CSV, JSON, text, Markdown, GIF, JPEG, PNG, SVG, and WebP files.'});
                continue;
            }
            ...
            try {
                ...
                if (type.kind === 'text') {
                    ...
                    abortIfNeeded(signal);
                    stored = Object.freeze({id, name: file.name, kind: 'text', mediaType: type.mediaType, size: file.size, preview: content.slice(0, previewCharacters), content});
                } else {
                    const bytes = new Uint8Array(await file.arrayBuffer());
                    const source = type.mediaType === 'image/svg+xml' ? new TextDecoder().decode(bytes) : undefined;
                    ...
                    stored = Object.freeze({id, name: file.name, kind: 'image', mediaType: type.mediaType, size: file.size, url, data, source});
                }
                ...
                errors.push({name: file.name, code: 'attachment_upload_failed', message: error instanceof Error ? error.message : 'Builder could not add this attachment.'});
            }
        }
        ...
        return [
            {
                ...
                description: 'Read bounded text lines from a user-provided attachment, or retrieve an uploaded image URL and metadata.',
                inputSchema: {
                    type: 'object',
                    properties: {id: {type: 'string'}, startLine: {type: 'integer', minimum: 1}, startColumn: {type: 'integer', minimum: 1}, endLine: {type: 'integer', minimum: 1}},
                    required: ['id'],
            ...
            },
            {
                ...
                description: 'Search one text attachment for a bounded literal query and return matching line excerpts.',
                inputSchema: {
                    type: 'object',
                    properties: {id: {type: 'string'}, query: {type: 'string', minLength: 1, maxLength: maxSearchQueryCharacters}},
                    required: ['id', 'query'],
                    additionalProperties: false
                },
    ...
    private read(input: Record<string, unknown>, signal: AbortSignal, revision: string): BuilderToolResult<unknown> {
        abortIfNeeded(signal);
        const attachment = typeof input.id === 'string' ? this.attachments.get(input.id) : undefined;
        if (!attachment) {
            return this.failure(revision, 'attachment_not_found', 'The requested attachment is no longer available.');
        }
            ...
            if (attachment.mediaType === 'image/svg+xml') {
                ...
                return {
                    ...
                    data: {
                        ...
                        modelPreview: 'SVG source is provided as text because model vision accepts raster images only.'
                    }
                };
            }
            return {
        ...
        const startColumn = boundedInteger(input.startColumn, 1, 1, (lines[startLine - 1]?.length ?? 0) + 1);
        const requestedEnd = boundedInteger(input.endLine, Math.min(lines.length, startLine + maxReadLines - 1), startLine, lines.length);
    ...
    private search(input: Record<string, unknown>, signal: AbortSignal, revision: string): BuilderToolResult<unknown> {
        abortIfNeeded(signal);
        const attachment = typeof input.id === 'string' ? this.attachments.get(input.id) : undefined;
        if (!attachment) {
            return this.failure(revision, 'attachment_not_found', 'The requested attachment is no longer available.');
        ...
        if (attachment.kind !== 'text') {
            return this.failure(revision, 'attachment_not_text', `${attachment.name} is an image and cannot be searched as text.`);
        }
        if (typeof input.query !== 'string' || !input.query || input.query.length > maxSearchQueryCharacters) {
            return this.failure(revision, 'invalid_attachment_query', `Use a non-empty search query no longer than ${maxSearchQueryCharacters} characters.`);
        }
        ...
        return {ok: true, revision, data: {id: attachment.id, name: attachment.name, query: input.query, matches, totalMatches, truncated: totalMatches > matches.length}};
    }
