...
describe('BuilderAttachments', () => {
  it('keeps bounded text context and stable uploaded image URLs in one shared store', async () => {
    ...
    expect(attachments.list()).toEqual([
      ...
      expect.objectContaining({
        ...
        name: 'chart.png',
        mediaType: 'image/png',
        url: 'https://example.com/content/images/chart.png',
      }),
    ]);
  });
  ...
  it('reads bounded line ranges and searches text without exposing the whole attachment', async () => {
    ...
    const content = Array.from(
      { length: 700 },
      (_, index) => `row ${index + 1},${index % 7 === 0 ? 'match' : 'other'}`,
    ).join('\n');
    const {
      added: [attachment],
    } = await attachments.add([new File([content], 'large.csv', { type: 'text/csv' })]);
    if (!attachment) {
    ...
    await expect(
      read?.execute({ id: attachment.id, startLine: 490, endLine: 620 }, signal),
  ...
  it('bounds attachment bytes retained by earlier turn checkpoints across the session', async () => {
    const attachments = new BuilderAttachments({
      uploadImage: vi.fn(),
      maxFileBytes: 8,
      maxAttachments: 2,
      maxRetainedBytes: 10,
    ...
    const {
      added: [first],
    } = await attachments.add([new File(['123456'], 'first.txt', { type: 'text/plain' })]);
    if (!first) {
    ...
    const result = await attachments.add([
      new File(['12345'], 'second.txt', { type: 'text/plain' }),
    ]);
  ...
  it('returns a canonical error when searching an image', async () => {
    const attachments = new BuilderAttachments({
      uploadImage: vi.fn().mockResolvedValue('https://example.com/image.webp'),
    });
    const {
      added: [image],
    } = await attachments.add([new File(['image'], 'image.webp', { type: 'image/webp' })]);
    if (!image) {
      throw new Error('Expected the image attachment to be added.');
    }
    const search = attachments
      .getTools(() => 'revision-2')
      .find((tool) => tool.name === 'search_attachment');

    await expect(
      search?.execute({ id: image.id, query: 'anything' }, new AbortController().signal),
    ).resolves.toEqual({
      ok: false,
      revision: 'revision-2',
      error: {
  ...
  it('returns uploaded image bytes as model-visible tool content', async () => {
    ...
    const {
      added: [image],
    } = await attachments.add([new File(['image-bytes'], 'image.png', { type: 'image/png' })]);
    if (!image) {
      throw new Error('Expected the image attachment to be added.');
    }
    const read = attachments
      .getTools(() => 'revision-2')
      .find((tool) => tool.name === 'read_attachment');

    await expect(
      read?.execute({ id: image.id }, new AbortController().signal),
    ).resolves.toMatchObject({
      ok: true,
      revision: 'revision-2',
  ...
  it('returns SVG source as text and rejects compressed SVGZ instead of sending unsupported vision content', async () => {
    const attachments = new BuilderAttachments({
      uploadImage: vi.fn().mockResolvedValue('https://example.com/icon.svg'),
    });
    ...
    const read = attachments
      .getTools(() => 'revision-svg')
      .find((tool) => tool.name === 'read_attachment');
    const readResult = await read?.execute({ id: image.id }, new AbortController().signal);

    expect(result.errors).toEqual([
      expect.objectContaining({ name: 'icon.svgz', code: 'unsupported_attachment_type' }),
    ]);
  ...
  it('snapshots and restores attachment contents without re-uploading images', async () => {
    ...
    const read = attachments
      .getTools(() => 'revision-restored')
      .find((tool) => tool.name === 'read_attachment');
    await expect(
      read?.execute({ id: text.id }, new AbortController().signal),
    ).resolves.toMatchObject({
      ok: true,
  ...
  it('paginates one very long line by column as well as line', async () => {
    ...
    const {
      ...
      new File(['A'.repeat(40_000)], 'data.json', { type: 'application/json' }),
    ]);
    if (!attachment) {
