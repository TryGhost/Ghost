...
function boundedToolInput(input: Record<string, unknown>): Record<string, unknown> {
  ...
  if (
    ...
    if (input.values && typeof input.values === 'object' && !Array.isArray(input.values)) {
      identity.values = Object.fromEntries(
  ...
  return {
    ...
    error: {
      ...
      ...(result.error.details === undefined
        ? {}
        : { details: boundedValue(result.error.details) }),
    },
  };
}
...
export class BuilderSession {
  ...
  async startTurn(input: string): Promise<BuilderTurnResult> {
    ...
    const userMessage: BuilderConversationMessage = {
      id: userMessageId,
      role: 'user',
      text,
      status: 'complete',
    };
    const assistantMessage: BuilderConversationMessage = {
    ...
    try {
      ...
      await this.modelAccess.runTurn({
        ...
        tools: this.workspace
          .getTools()
          .filter((tool) => !userOnlyToolNames.has(tool.name.toLowerCase())),
        workspace: {
    ...
    if (!checkpointCreated) {
      ...
      this.setState({
        ...this.currentState,
        status:
          activeTurn.stopRequested || activeTurn.streamAborted ? previousStatus : 'interrupted',
    ...
    const interruptedBeforePromotion =
      activeTurn.stopRequested || activeTurn.streamAborted || Boolean(activeTurn.streamError);
    ...
    const interrupted =
      activeTurn.stopRequested || activeTurn.streamAborted || Boolean(activeTurn.streamError);
    const finalMessages = this.currentState.messages.map((message) =>
  ...
  private handleEvent(turn: ActiveTurn, event: BuilderStreamEvent): void {
    ...
    if (event.type === 'assistant-text-delta') {
      ...
      this.updateMessage(turn.assistantMessageId, (message) => ({
        ...message,
        toolCalls: (message.toolCalls ?? []).map((toolCall) =>
          toolCall.id === event.callId
            ? {
                ...toolCall,
                name: event.name,
                status:
                  turn.stopRequested || turn.controller.signal.aborted || turn.streamAborted
                    ? 'interrupted'
                    : 'complete',
  ...
  private updateMessage(
    id: string,
    update: (message: BuilderConversationMessage) => BuilderConversationMessage,
  ): void {
    this.setState({
      ...this.currentState,
