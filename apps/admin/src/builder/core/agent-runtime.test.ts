...
describe('agent runtime', () => {
  it('translates Pi events and executes tool calls sequentially', async () => {
    ...
    const streamFn: StreamFn = vi.fn(
      (_model: Model<Api>, context: Context, _options?: SimpleStreamOptions) => {
          ...
          execute: () =>
            Promise.resolve({
              ...
              details: {
                ok: false,
                revision: 'revision-0',
                error: { code: 'render_failed', message: 'Invalid template', retryable: true },
              },
            }),
        },
      ],
  ...
  it('compacts an active tool cycle when image payloads exceed the request budget', async () => {
    ...
    expect(JSON.stringify(providerCalls[1]?.messages)).toContain(
      'image omitted from compacted context',
    );
  ...
  it('propagates abort after the provider requests a tool', async () => {
    ...
    const streamFn: StreamFn = (_model, _context, options) => {
      ...
      return completedStream(
        message([{ type: 'toolCall', id: 'waiting-tool', name: 'wait', arguments: {} }], 'toolUse'),
      );
    };
    const runtime = createAgentRuntime({
      model,
  ...
  it('compacts an oversized current tool cycle without sending orphaned tool results', async () => {
    ...
    expect(JSON.stringify(contexts[1]?.messages)).toContain(
      'Earlier Builder tool activity compacted',
    );
  ...
  it('preserves the complete current user request when compacting older tool cycles', async () => {
    ...
    const responses = [
      message(
        [{ type: 'toolCall', id: 'first', name: 'record', arguments: { value: 'first' } }],
        'toolUse',
      ),
      message(
        [{ type: 'toolCall', id: 'second', name: 'record', arguments: { value: 'second' } }],
        'toolUse',
      ),
      message([{ type: 'text', text: 'Finished' }], 'stop'),
    ];
    ...
    expect(
      JSON.stringify(compactedUser && 'content' in compactedUser ? compactedUser.content : ''),
    ).toContain(currentRequest);
  });
});
