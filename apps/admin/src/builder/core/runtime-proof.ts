      ...
      ? [
          ...
          [
            'content_block_delta',
            {
              ...
              index: 0,
              delta: { type: 'input_json_delta', partial_json: '{"value":"anthropic"}' },
            },
          ],
      ...
      : [
          ...
          [
            'content_block_delta',
            {
              type: 'content_block_delta',
              index: 0,
              delta: { type: 'text_delta', text: 'Proof complete' },
            },
          ],
          ['content_block_stop', { type: 'content_block_stop', index: 0 }],
          [
...
export async function runBrowserRuntimeProof(provider: BuilderProvider): Promise<string> {
  ...
  if (
    providerCalls !== 2 ||
    toolCalls.join(',') !== provider ||
    runtime.messages.at(-1)?.text !== 'Proof complete'
  ) {
