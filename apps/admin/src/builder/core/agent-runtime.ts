...
function messageImageCharacters(message: AgentMessage): number {
  ...
  return message.content.reduce(
    (total, item) => (item.type === 'image' ? total + item.data.length : total),
    0,
  );
}
...
function contextFits(
  ...
  return (
    ...
    messages.reduce((total, message) => total + messageImageCharacters(message), 0) <=
      maxImageCharacters
  );
}
    ...
    .map((message) => {
      ...
      const prefix =
        message.role === 'toolResult'
          ? `Tool ${message.toolName} (${message.isError ? 'error' : 'complete'}): `
          : `${message.role}: `;
      if (typeof message.content === 'string') {
...
function compactCurrentTurn(
  ...
  const selectedCharacters = selectedMessages.reduce(
    (total, message) => total + messageCharacters(message),
    0,
  );
  const userCharacters = messageCharacters(userMessage);
  if (
    !contextFits([userMessage, ...selectedMessages], maxMessages, maxCharacters, maxImageCharacters)
  ) {
    const original =
      typeof userMessage.content === 'string'
        ? userMessage.content
    ...
    return [
      {
        ...userMessage,
        content: original.slice(0, Math.max(1, maxCharacters - selectedCharacters)),
      },
      ...selectedMessages,
    ];
  }
  ...
  const summary =
    dropped.length <= summaryAllowance
      ? dropped
      : dropped.slice(Math.max(0, dropped.length - summaryAllowance));
  const compactedSummary = `\n\n${marker}\n${summary}`;
  const compactedUser: AgentMessage = {
    ...userMessage,
    content:
      typeof userMessage.content === 'string'
        ? `${userMessage.content}${compactedSummary}`
        : [...userMessage.content, { type: 'text', text: compactedSummary }],
  };
  return [compactedUser, ...selectedMessages];
}

function truncateAtTurnBoundary(
  messages: AgentMessage[],
  maxMessages: number,
  maxCharacters: number,
  maxImageCharacters: number,
  ...
  const characterLimit = Number.isFinite(maxCharacters)
    ? Math.max(1, Math.floor(maxCharacters))
    : 100_000;
  const imageLimit = Number.isFinite(maxImageCharacters)
    ? Math.max(1, Math.floor(maxImageCharacters))
    : 5_000_000;
  const fits = (start: number) =>
    contextFits(messages.slice(start), limit, characterLimit, imageLimit);
  if (fits(0)) {
    return messages;
  }

  const userTurnStarts = messages.flatMap((message, index) =>
...
function toPiHistoryMessage(
  message: BuilderRuntimePromptMessage,
  model: Model<Api>,
  timestamp: number,
): AgentMessage {
  if (message.role === 'user') {
...
function emitBuilderEvent(event: AgentEvent, emit: (event: BuilderRuntimeEvent) => void): void {
  if (event.type === 'message_update' && event.assistantMessageEvent.type === 'text_delta') {
    ...
    if (stopReason === 'aborted') {
      ...
      emit({
        type: 'run-error',
        message: (event.message as AssistantMessage).errorMessage ?? 'The model request failed.',
      });
    }
...
export function createAgentRuntime({
  ...
  const agent = new Agent({
    ...
    afterToolCall: (context) =>
      Promise.resolve(
        isFailedBuilderEnvelope(context.result.details) ? { isError: true } : undefined,
      ),
    transformContext: (messages) =>
      Promise.resolve(
        truncateAtTurnBoundary(messages, maxMessages, maxCharacters, maxImageCharacters),
      ),
    initialState: {
      model,
      systemPrompt,
      ...
      messages: initialMessages.map((message, index) =>
        toPiHistoryMessage(message, model, historyTimestamp + index),
      ),
    },
  });
  ...
  return {
    ...
    get messages() {
      return agent.state.messages
        .map(toBuilderMessage)
        .filter((message): message is BuilderRuntimeMessage => Boolean(message));
    },
  };
}
