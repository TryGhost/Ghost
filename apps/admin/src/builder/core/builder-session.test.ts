...
class FakeArtifactWorkspace implements BuilderWorkspace {
  ...
  promoteCandidate(signal: AbortSignal): Promise<ValidationResult> {
    ...
    return Promise.resolve(
      this.workspaceState.validation ?? {
        valid: true,
        diagnostics: [],
              ...
              diagnostics: [
                {
                  ...
                  message: 'The artifact is invalid.',
  ...
  restoreAttachments(snapshot: BuilderAttachmentSnapshot): void {
    this.attachments = snapshot.attachments.map((attachment) =>
      attachment.kind === 'text'
        ? {
            id: attachment.id,
...
describe('BuilderSession', () => {
  it('loads a generic workspace and exposes its preview without theme assumptions', async () => {
    ...
    expect(session.state).toMatchObject({
      status: 'ready',
      workspace: { revision: 'revision-0', dirty: false },
    });
  ...
  it('passes transport-neutral workspace context and records canonical tool lifecycle on the assistant message', async () => {
    ...
    workspace.selection = {
      id: 'marker-42',
      label: 'Revenue heading',
      data: { source: 'index.hbs:12:3' },
    };
  ...
  it('stores a bounded conversation projection instead of full tool payloads', async () => {
    ...
    modelAccess.enqueue((request) => {
      request.onEvent({
        ...
        callId: 'call-1',
        name: 'screenshot',
        input: { path: 'index.hbs', selector: oversized },
      });
      request.onEvent({
        type: 'tool-end',
        callId: 'call-1',
    ...
    expect(session.state.messages.at(-1)?.toolCalls?.[0]?.input).toMatchObject({
      path: 'index.hbs',
      truncated: true,
    });
    expect(session.state.messages.at(-1)?.toolCalls?.[0]?.result).toMatchObject({
      ok: true,
      revision: 'revision-1',
    });
  ...
  it('retains the last valid candidate when a later mutation is invalid', async () => {
    ...
    expect(session.state.workspace.validation).toMatchObject({
      valid: true,
      revision: 'revision-1',
    });
  });
  ...
  it('stops an active turn and retains a valid candidate', async () => {
    ...
    expect(workspace.snapshot()).toEqual({ revision: 'revision-0', payload: { value: 'initial' } });
    expect(workspace.candidateSnapshot).toEqual({
      revision: 'revision-1',
      payload: { value: 'before-stop' },
    });
  ...
  it('rolls back a provisional turn when pre-turn workspace flushing is stopped', async () => {
    const workspace = new FakeArtifactWorkspace();
    workspace.flushImplementation = (signal) =>
  ...
  it('rewinds conversation and discards the later branch before continuing', async () => {
    ...
    ).toEqual(['First turn', 'Replacement turn']);
  ...
  it('restores the attachment context owned by an earlier turn checkpoint', async () => {
    ...
    expect(workspace.attachments).toEqual([
      expect.objectContaining({ id: 'attachment-a', name: 'a.csv' }),
    ]);
  });
  ...
  it('rejects a user prompt that exceeds the bounded turn input', async () => {
    ...
    await expect(session.startTurn('x'.repeat(32_001))).rejects.toThrow(
      '32,000 characters or fewer',
    );
    expect(session.state.messages).toEqual([]);
  });
