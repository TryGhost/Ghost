...
describe('ToolGroup', () => {
  it('keeps the latest step shimmering while the turn is still in progress between tool calls', () => {
    ...
    expect(container.querySelector('.builder-shimmer')).toHaveTextContent(
  ...
  it('presents failed work without exposing internal tool details', () => {
        ...
        toolCalls={[
          {
            ...
            result: {
              ok: false,
              revision: 'revision-1',
              error: { code: 'render_failed', message: 'Template error', retryable: true },
            },
          },
        ]}
      />,
    ...
    fireEvent.click(
      screen.getByLabelText('Editing the template. Some changes need attention. Failed'),
    );
  ...
  it('does not claim changes completed when the containing turn was interrupted', () => {
    ...
    expect(
      screen.getByLabelText('Editing the template. Work stopped. Interrupted'),
    ).toBeInTheDocument();
  });
        ...
        toolCalls={[
          {
            ...
            result: {
              ok: false,
              revision: 'revision-1',
              error: { code: 'render_failed', message: 'Try again', retryable: true },
            },
          },
          {
            id: 'repaired-tool',
  ...
  it('does not treat a later successful inspection as repairing a failed change', () => {
    ...
    expect(
      screen.getByLabelText('Inspecting the preview. Some changes need attention. Failed'),
    ).toBeInTheDocument();
  });
