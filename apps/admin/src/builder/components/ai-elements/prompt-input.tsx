    ...
    return (
        <form className='rounded-xl bg-surface-elevated p-2 shadow-sm' onSubmit={submit}>
            <Stack gap='sm'>
                ...
                {attachments.length > 0 && (
                    <Inline className='flex-wrap' gap='xs'>
                        {attachments.map(attachment => (
                            <Button key={attachment.id} aria-label={`Remove ${attachment.name}`} disabled={disabled || isRunning || addingAttachments} size='sm' type='button' variant='outline' onClick={() => onRemoveAttachment?.(attachment.id)}>
                                {attachment.kind === 'image' ? <LucideIcon.Image aria-hidden='true' /> : <LucideIcon.FileText aria-hidden='true' />}
                ...
                />
                {(submissionError || attachmentError) && <Text className='text-destructive' role='alert' size='sm'>{submissionError || attachmentError}</Text>}
