  ...
  const statusIndicator = (label: string, variant: 'warning' | 'info', active = false) => (
    <Tooltip>
      <TooltipTrigger asChild>
        <span
          aria-label={label}
          className="rounded-full p-1 outline-hidden focus-visible:ring-2 focus-visible:ring-focus-ring"
          role="status"
          tabIndex={0}
        >
    ...
    <header className="z-10 h-16 shrink-0 bg-preview-canvas px-4">
      <Inline align="center" className="size-full" gap="md" justify="between">
        <Inline align="center" className="min-w-0 flex-1" gap="md">
          ...
          <Text as="h1" className="min-w-0 truncate" weight="medium">
            {title}
          </Text>
        ...
        {publishAction && (
          <Box className="shrink-0 [&>button]:min-w-24 [&>button]:rounded-full [&>button]:px-6">
            {publishAction}
          </Box>
        )}
      </Inline>
