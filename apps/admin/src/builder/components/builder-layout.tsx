...
function availableMaxWidth(viewportWidth = window.innerWidth): number {
  return Math.max(
    minWidth,
    Math.min(absoluteMaxWidth, viewportWidth - minPreviewWidth - dividerWidth),
  );
}
...
export const BuilderLayout = ({
  ...
  chat: ReactNode;
  preview: BuilderLayoutSlot;
  header?: BuilderLayoutSlot;
}) => {
  const [width, setWidth] = useState(initialWidth);
  const widthRef = useRef(width);
  const preferredWidthRef = useRef(
    Number(sessionStorage.getItem(builderChatWidthStorageKey)) || defaultWidth,
  );
      ...
      if (crossingBreakpoint) {
        ...
        if (focusLabel) {
          requestAnimationFrame(() => {
            ...
            const match = [...(root?.querySelectorAll<HTMLElement>('[aria-label]') ?? [])].find(
              (element) => element.getAttribute('aria-label') === focusLabel,
            );
            match?.focus();
          });
        }
      ...
      <Box className="flex min-h-0 flex-1 flex-col bg-preview-canvas">
        {header && (
          <Box className="shrink-0 bg-preview-canvas min-[768px]:flex min-[768px]:h-16">
            <Box
              className={isNarrow ? 'h-16 w-full' : 'h-16 shrink-0'}
            ...
            <Box
              ref={registerToolbarHost}
              className="h-16 min-w-0 flex-1 bg-preview-canvas"
              data-testid="builder-toolbar-host"
            />
          </Box>
        )}
        {isNarrow && (
          <Inline
            ...
            className="bg-preview-canvas p-2"
            gap="sm"
            role="tablist"
          >
            <Button
        ...
        <Box className="relative flex min-h-0 flex-1 overflow-hidden min-[768px]:overflow-visible">
          <section
            ...
            className={
              isNarrow
                ? `absolute inset-0 min-w-0 bg-preview-canvas transition-transform motion-reduce:transition-none ${activeView === 'chat' ? 'translate-x-0' : 'pointer-events-none -translate-x-full'}`
                : 'min-w-0 shrink-0 bg-preview-canvas'
            }
          ...
          <section
            ...
            className={
              isNarrow
                ? `absolute inset-0 min-w-0 bg-preview-canvas transition-transform motion-reduce:transition-none ${activeView === 'preview' ? 'translate-x-0' : 'pointer-events-none translate-x-full'}`
                : 'relative z-10 min-w-0 flex-1 bg-preview-canvas'
            }
