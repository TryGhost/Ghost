...
export const PreviewPanel = ({
    ...
    if (target && target !== url) {
      ...
      void Promise.resolve(onNavigate?.(target)).then((outcome) => {
        if (outcome !== true) {
          setAddress(url);
          setNavigationError(
            typeof outcome === 'string' ? outcome : 'Builder could not open that preview address.',
          );
        }
      });
    }
      ...
      {responsive && !isNarrow && (
        <Inline align="center" gap="xs">
          <WebPreviewNavigationButton
            ...
            className={
              viewport === 'desktop'
                ? 'builder-raised-surface bg-surface-elevated hover:bg-surface-elevated'
                : undefined
            }
            disabled={disabled}
  ...
  const hostNavigation = (
    <WebPreviewNavigation aria-label="Builder actions" className="h-16 w-full">
      {isNarrow ? (
        ...
        <Inline align="center" className="min-w-0 flex-1" data-preview-controls="" gap="xs">
          {previewControls}
        </Inline>
      )}
      {action && (
        <Box className="builder-raised-action ml-2 shrink-0 [&>button]:min-w-24 [&>button]:rounded-full [&>button]:px-6">
          {action}
        </Box>
      )}
    </WebPreviewNavigation>
  ...
  return (
    <WebPreview>
      ...
      {navigationError && (
        <Text
          className="border-b border-border-default px-3 py-2 text-destructive"
          role="alert"
          size="sm"
        >
          {navigationError}
      ...
      <WebPreviewBody
        className={`flex justify-center overflow-auto p-4 pt-0 min-[768px]:overflow-visible min-[768px]:pl-0 ${disabled ? 'pointer-events-none' : ''}`}
        inert={disabled}
      >
        <Box
          className="builder-raised-surface-strong h-full max-w-full overflow-hidden rounded-xl bg-background transition-[width]"
          style={responsive ? { width: viewportWidths[viewport] } : { width: '100%' }}
        >
          {children}
        </Box>
      </WebPreviewBody>
