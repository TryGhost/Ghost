...
describe('ProviderSetup', () => {
  it('clears an unsaved credential when the provider changes', () => {
    const { rerender } = render(
      <ProviderSetup connected={false} provider="openai" onForget={vi.fn()} onSave={vi.fn()} />,
    );
    ...
    rerender(
      <ProviderSetup connected={false} provider="anthropic" onForget={vi.fn()} onSave={vi.fn()} />,
    );
  ...
  it('imports Codex auth JSON and reports invalid files without clearing them', () => {
    ...
    fireEvent.change(input, {
      target: {
        value: '{"tokens":{"access_token":"session-access","refresh_token":"never-store"}}',
      },
    });
