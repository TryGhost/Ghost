...
import {
  ...
  artifactPayload,
  createArtifactDraft,
} from './workspaces/artifact/artifact-state';
...
const ArtifactBuilderExperience = ({
  request,
  onCancel,
  onSave,
}: ArtifactBuilderExperienceProps) => {
  ...
  const [selection, setSelection] =
    useState<ReturnType<ArtifactWorkspaceInstance['getSelectionContext']>>(null);
  ...
  useEffect(() => {
    if (previewMode !== 'browse' && !['ready', 'interrupted'].includes(state.status)) {
      ...
      void previewRef.current
        ?.setInteractionMode('browse', new AbortController().signal)
        .catch(() => {});
    }
  ...
  const shouldGuardNavigation =
    hasUnsavedWork || state.status === 'running' || state.status === 'publishing';
  ...
  const setPreviewInteractionMode = (mode: PreviewInteractionMode) => {
    ...
    void previewRef.current
      ?.setInteractionMode(mode, new AbortController().signal)
  ...
  return (
    <>
      <BuilderShell
        ...
        backAction={
          <Button
            aria-label="Cancel Artifact Builder"
            size="icon"
            type="button"
            variant="ghost"
            onClick={cancel}
