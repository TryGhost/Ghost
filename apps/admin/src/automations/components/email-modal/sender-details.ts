...
type AutomatedEmailSenderFields = Pick<
  AutomatedEmail,
  'slug' | 'sender_name' | 'sender_email' | 'sender_reply_to'
>;
...
export const resolveEmailSenderDetails = ({
  ...
  const defaultNewsletterSenderEmail = newsletter
    ? trimValue(renderSenderEmail(newsletter, config, defaultEmailAddress || undefined))
    : '';
  const defaultNewsletterReplyTo = newsletter
    ? trimValue(
        renderReplyToEmailPlaceholder(
          newsletter,
          config,
  ...
  const senderNamePlaceholder =
    defaultNewsletterSenderName || trimValue(siteTitle) || 'Your site name';
  const senderEmailPlaceholder = defaultNewsletterSenderEmail || trimValue(defaultEmailAddress);
  const replyToEmailPlaceholder =
    defaultNewsletterReplyTo || trimValue(supportEmailAddress) || trimValue(defaultEmailAddress);
  ...
  const hasDistinctReplyTo =
    resolvedReplyToEmail !== '' && resolvedReplyToEmail !== resolvedSenderEmail;
