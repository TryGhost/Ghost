...
const HelpLinkCard: React.FC<HelpLinkCardProps> = ({
  url,
  className,
  title,
  description,
...
const HelpButtonCard: React.FC<HelpButtonCardProps> = ({
  className,
  title,
  description,
  children,
  ...
  <button
    className={cn(cardClass, 'cursor-pointer appearance-none', className)}
    type="button"
    onClick={onClick}
    onFocus={onFocus}
...
const FEEDBACK_DESCRIPTION =
  'Tell us what’s working and what’s missing — your input shapes what we build next.';
  ...
  return (
    <div className="mt-auto grid grid-cols-1 gap-6 pt-10 lg:grid-cols-2">
      ...
      {canUseFeaturebase ? (
        ...
        <HelpLinkCard description={FEEDBACK_DESCRIPTION} title={FEEDBACK_TITLE} url={FORUM_URL}>
          <FeedbackIconTile />
        </HelpLinkCard>
      )}
    </div>
