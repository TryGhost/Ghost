...
describe('Automations list', () => {
  ...
  it('lists the welcome automations', async () => {
    ...
    await expect.element(row).toMatchTextContent('Greet new free members.');
    await expect.element(row).toMatchTextContent('1,432');
    await expect.element(row).toMatchTextContent('118');
    await expect.element(row).toMatchTextContent('Live');
