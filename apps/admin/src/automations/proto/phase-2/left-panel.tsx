...
export const LeftPanel: React.FC<LeftPanelProps> = ({
  ...
  // filter matching nothing, and it reads differently everywhere it shows.
  const noData = runs.length === 0;
  ...
  const searchField = (
    <InputGroup className="h-9 min-w-0 flex-1">
      ...
      {/* autoFocus, and it's the mount that does it: the field appears because
                you pressed the magnifier, so focus is the point. */}
      <InputGroupInput
            ...
            <div className="overflow-hidden">
              <Box className="mb-4 rounded-lg border border-border-default px-4 py-3">
                <Stack gap="sm">
                  ...
                  {/* With no data at all, a flat line pinned to zero reads as a
                            measurement — as though the automation ran and nothing
                            happened. An empty plot area says the truthful thing
                            instead: there is nothing to draw yet. No copy in it; the
                            table below carries the one explanation, so the pane says it
                            once rather than three times. */}
                  {noData ? (
                    <div
                      className={cn(
                        CHART_HEIGHT,
                        'w-full rounded-md border border-dashed border-border-default',
                      )}
                    />
                  ) : (
                    <GhAreaChart
