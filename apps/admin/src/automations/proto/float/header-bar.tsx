...
//
// Three zones on one row: the pane toggle left, identity centre (with an
// Automations breadcrumb as the way back), action right. The title is centred
// rather than left-aligned so it stays put as the pane collapses, where a
// left-aligned one would shift with its neighbours.
//
...
export const HeaderBar: React.FC<HeaderBarProps> = ({
    ...
    <header className="relative z-30 flex h-16 shrink-0 items-center justify-between border-b border-border-default bg-surface-elevated px-4">
        ...
        {/* pointer-events-none so the centred block never swallows a click aimed at
            the zones behind it; only the Automations crumb opts back in — it took
            over navigation from the old back arrow, so the left zone is purely
            toggles and exit lives in the breadcrumb where it reads as navigation. */}
        <Inline align="center" className="pointer-events-none absolute inset-x-0 justify-center" gap="sm">
            <button className="pointer-events-auto text-md text-muted-foreground transition-colors hover:text-foreground" type="button" onClick={onBack}>Automations</button>
            <span className="text-md text-muted-foreground" aria-hidden>/</span>
