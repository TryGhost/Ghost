...
const AutomationFloat: React.FC = () => {
  ...
  // Both the commit button and the leave guards hang off hasChanges — the draft
  ...
  const chromeActions = (
    <>
      {/* The commit button, borderless and always there — the post editor's header,
                ...
                edits to the thing that's already running", and it's a promise a stopped
                automation can't keep. Off, there's no live version for edits to diverge
                from, so the same press is just "Save". The word only started mattering
                when the button stopped being permanently disabled while off — see the
                gate above.
      ...
      <Button disabled={!hasChanges} variant="ghost" onClick={handlePublishClick}>
