...
const ZoomControls: React.FC = () => {
    ...
    return (
        <div className="flex items-center gap-1 rounded-lg border bg-background p-1 shadow-sm">
            <Button aria-label="Zoom out" size="icon" variant="ghost" onClick={() => void zoomOut()}>
                <LucideIcon.Minus />
            </Button>
            <button
            ...
            <Button aria-label="Zoom in" size="icon" variant="ghost" onClick={() => void zoomIn()}>
                <LucideIcon.Plus />
            </Button>
        </div>
    );
...
const EmailEditorModal: React.FC<{onClose: () => void}> = ({onClose}) => (
    <div className="fixed inset-0 z-[60] flex animate-in flex-col bg-background duration-200 fade-in-0">
        ...
        <div className="bg-grey-75 flex-1 overflow-y-auto">
                ...
                <>
                    ...
                    {meta.value !== undefined && (
                        <SidebarField label="Value">
                            <div className="bg-grey-75 rounded-md border px-3 py-2">{meta.value}</div>
                        </SidebarField>
                    )}
                </>
            )}
...
const RunItem: React.FC<{run: Run; selected: boolean; onSelect: (id: string) => void}> = ({run, selected, onSelect}) => (
    <li className={`bg-grey-75 rounded-md border ${selected ? 'ring-2 ring-blue-500' : ''}`}>
        <button
        ...
        <div className="fixed inset-0 z-50 flex flex-col bg-background">
            ...
            {view === 'workflow' ? (
                <div className="relative flex min-h-0 flex-1 overflow-hidden">
                    <div ref={workflowCanvasRef} className="bg-grey-75 flex-1">
                        <ReactFlow
                ...
                <div className="flex min-h-0 flex-1">
                    ...
                    <div className="bg-grey-75 flex flex-1 flex-col">
