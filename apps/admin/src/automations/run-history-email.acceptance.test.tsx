...
describe('Historical email cards', () => {
  ...
  it('shows updated send and delivery evidence when reopening a run', async () => {
    setup();
    ...
    const times = email().element().querySelectorAll('time');
    expect(times).toHaveLength(2);
    expect(times[0].textContent).toMatch(/\d+:\d+/);
    expect(times[0].textContent).not.toContain('est.');
    expect(times[1].textContent).toBe(
      `est. ${new Date(data.steps[0].ready_at).toLocaleDateString(undefined, { month: 'short', day: 'numeric' })}`,
    );
  ...
  it('shows a successful send alongside a later execution failure', async () => {
    ...
    await open();
    await select();
    await expect.element(email()).not.toHaveTextContent('Failed');
    const event = canvas().getByRole('article', { name: 'Email step failed', exact: true });
    await expect.element(event).toBeVisible();
    expect(event.element().querySelector('time')?.dateTime).toBe('2026-09-14T12:00:06.000Z');
    expect(
      Array.from(email().element().querySelectorAll('time')).map((time) => time.dateTime),
    ).toEqual(['2026-09-14T12:00:02.000Z']);
  ...
  it('shows a pending email with its estimated date, including when an attempt has started', async () => {
    setup();
    ...
    await expect.element(email('Send email')).toHaveTextContent('est. ');
    expect(email('Send email').element().querySelector('time')?.dateTime).toBe(
      data.steps[0].ready_at,
    );
    expect(email('Send email').element().querySelector('time')?.textContent).toBe(
      `est. ${new Date(data.steps[0].ready_at).toLocaleDateString(undefined, { month: 'short', day: 'numeric' })}`,
    );
  ...
  it('retains historical content on an exited email without claiming it was sent', async () => {
    ...
    await expect
      .element(email('Send email'))
      .not.toHaveTextContent('Member unavailable or unsubscribed');
    await expect
      .element(
        canvas().getByRole('article', { name: 'Member unavailable or unsubscribed', exact: true }),
      )
      .toBeVisible();
  ...
  it.each([
    [null, null, null, null],
    ['', '', null, null],
    ['   ', lexical('Saved body'), null, 'Saved body'],
    ['Saved subject', '{malformed', 'Saved subject', null],
    [
      'Media only',
    ...
    'omits missing subject %s or content %s without placeholder messages',
    async (subjectText, content, expectedSubject, expectedContent) => {
      ...
      await expect
        .element(email())
        .not.toHaveTextContent(/unavailable|No subject|content is empty|No text preview/);
    },
  );
