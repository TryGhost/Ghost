  ...
  useEffect(() => {
    ...
    const queuePersistScrollPosition = () => {
      ...
      const hasLargePositionChange =
        Math.abs(latestScrollPositionRef.current - lastPersistedPositionRef.current) >=
        IMMEDIATE_SCROLL_POSITION_DELTA_PX;

      if (
        hasLargePositionChange ||
        now - lastPersistedAtRef.current >= SCROLL_POSITION_PERSIST_INTERVAL_MS
      ) {
        clearPendingPersist();
        persistScrollPosition(latestScrollPositionRef.current);
        return;
