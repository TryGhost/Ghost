...
const post = (overrides: Record<string, unknown> = {}) => ({
    id: 'p1', status: 'published', visibility: 'paid', ...overrides
});
...
describe('canCopyGiftLink', () => {
    ...
    describe('what may be shared', () => {
        it('allows a published post behind a paywall', () => {
            expect(canCopyGiftLink({user: user(['Owner']), post: post({visibility: 'paid'})})).toBe(true);
        });

        it('allows a members-only post', () => {
            expect(canCopyGiftLink({user: user(['Owner']), post: post({visibility: 'members'})})).toBe(true);
        });

        // Nothing to gift: anyone can already read it.
        it('refuses a public post', () => {
            expect(canCopyGiftLink({user: user(['Owner']), post: post({visibility: 'public'})})).toBe(false);
        });

        it('refuses a draft, which has nothing to share yet', () => {
            expect(canCopyGiftLink({user: user(['Owner']), post: post({status: 'draft'})})).toBe(false);
        });

        it('refuses a scheduled post', () => {
            expect(canCopyGiftLink({user: user(['Owner']), post: post({status: 'scheduled'})})).toBe(false);
        });

        it('refuses a post with no visibility set at all', () => {
            expect(canCopyGiftLink({user: user(['Owner']), post: post({visibility: undefined})})).toBe(false);
        });
    });
