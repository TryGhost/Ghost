...
const mockTrackFilterApplications = vi.mocked(
  await import('@tryghost/admin-x-framework'),
).trackFilterApplications;
...
describe('useFilterParams', () => {
  ...
  it('round-trips values containing commas', () => {
    ...
    act(() => {
      result.current.setFilters([
        { id: '1', field: 'utm_campaign', operator: 'is', values: ['summer,sale,2024'] },
      ]);
    });
    expect(result.current.filters).toEqual([
  ...
  it('preserves non-filter params when setting and clearing filters', () => {
    ...
    act(() => {
      result.current.setFilters([
        { id: '1', field: 'device', operator: 'is', values: ['desktop'] },
      ]);
    });
  ...
  it('supports functional updates', () => {
    ...
    act(() => {
      result.current.setFilters((prev) => [
        ...prev,
        { id: '2', field: 'device', operator: 'is', values: ['desktop'] },
      ]);
    });
    expect(result.current.filters).toEqual([
      expect.objectContaining({ field: 'source', values: ['google.com'] }),
  ...
  describe('post analytics parameterization', () => {
    ...
    it('supports the rest of the shared fields', () => {
      const { result } = renderFilterParams(
        postOptions,
        '/?audience=free&gift_link=true&utm_source=newsletter',
      );
