...
export const POST_ANALYTICS_FILTER_FIELDS = ANALYTICS_FILTER_FIELDS.filter(
  (field) => field !== 'post',
);
...
function filtersToSearchParams(
  filters: Filter[],
  supportedFieldsSet: ReadonlySet<string>,
): URLSearchParams {
...
function getStableFilterId(field: string): string {
  if (!filterIdCache.has(field)) {
    filterIdCache.set(
      field,
      `url-${field}-${Date.now()}-${Math.random().toString(36).substring(2, 11)}`,
    );
  }
...
export function useFilterParams({
  ...
  const supportedFieldsSet: ReadonlySet<string> = useMemo(
