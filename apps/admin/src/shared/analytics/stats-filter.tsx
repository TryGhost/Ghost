...
interface StatsFilterProps extends Omit<
...
const VisitCountBadge = ({ visits }: { visits: number }) => (
  <span className="order-2 font-mono text-xs text-muted-foreground">{formatNumber(visits)}</span>
);
  ...
  const options = useMemo(() => {
    ...
    return (stats || [])
      .filter((item) => {
        ...
        const hasValidPostUuid =
          item.post_uuid && item.post_uuid !== '' && item.post_uuid !== 'undefined';
...
function StatsFilter({
    ...
    const options = [
      {
        ...
        icon: <LucideIcon.Globe className="text-gray-700" />,
      },
  ...
  const { options: utmSourceOptions, loading: utmSourceLoading } = useTinybirdFilterOptions(
    'utm_source',
    filters,
    range,
    postUuid,
    { enabled: shouldFetchOptions('utm_source') },
  );
  const { options: utmMediumOptions, loading: utmMediumLoading } = useTinybirdFilterOptions(
    'utm_medium',
    filters,
    range,
    postUuid,
    { enabled: shouldFetchOptions('utm_medium') },
  );
  const { options: utmCampaignOptions, loading: utmCampaignLoading } = useTinybirdFilterOptions(
    'utm_campaign',
    filters,
    range,
    postUuid,
    { enabled: shouldFetchOptions('utm_campaign') },
  );
  const { options: utmContentOptions, loading: utmContentLoading } = useTinybirdFilterOptions(
    'utm_content',
    filters,
    range,
    postUuid,
    { enabled: shouldFetchOptions('utm_content') },
  );
  const { options: utmTermOptions, loading: utmTermLoading } = useTinybirdFilterOptions(
    'utm_term',
    filters,
    range,
    postUuid,
    { enabled: shouldFetchOptions('utm_term') },
  );
  const { options: sourceOptions, loading: sourceLoading } = useTinybirdFilterOptions(
    'source',
    filters,
    range,
    postUuid,
    { enabled: shouldFetchOptions('source') },
  );
  const { options: deviceOptions, loading: deviceLoading } = useTinybirdFilterOptions(
    'device',
    filters,
    range,
    postUuid,
    { enabled: shouldFetchOptions('device') },
  );
  const { options: locationOptions, loading: locationLoading } = useTinybirdFilterOptions(
    'location',
    filters,
    range,
    postUuid,
    { enabled: shouldFetchOptions('location') },
  );
  ...
  const { options: postOptions, loading: postLoading } = usePostOptions(filters, range, {
    enabled: showPostField && shouldFetchOptions('post'),
  });
