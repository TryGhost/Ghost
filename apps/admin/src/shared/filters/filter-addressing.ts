...
import type {ClauseGroup, EqualityClause, SemanticValue, SerializedValue, ValueComparator, ValueSemantics} from './semantics';
...
function toClauseGroup(node: AstNode): ClauseGroup | null {
    for (const join of ['and', 'or'] as const) {
        ...
        if (children) {
            const clauses = children.map(child => toEqualityClause(child)).filter(clause => clause !== null);
...
export function composeCodec<TOperator extends string>(
    ...
    return {
        ...
        serialize(predicate, ctx) {
            if (addressing.presenceOperators && listsOperator(addressing.presenceOperators, predicate.operator)) {
