...
import {
  RELATIVE_PAST_OPERATOR,
  formatRelativeDateTooltip,
  isRelativeDateOperator,
} from './filter-relative-date';
import {
  Tooltip,
  TooltipContent,
  TooltipProvider,
...
export const RelativeDateFilter: React.FC<RelativeDateFilterProps> = ({
  ...
  const amount =
    typeof rawAmount === 'number' && Number.isSafeInteger(rawAmount) && rawAmount > 0
      ...
      : DEFAULT_AMOUNT;
  const dateValue =
    typeof rawAmount === 'string' && DATE_PATTERN.test(rawAmount) ? rawAmount : fallbackDate;
  ...
  if (isRelative) {
    ...
    const tooltipDate = formatRelativeDateTooltip(
      fallbackDate,
      operator === RELATIVE_PAST_OPERATOR ? -amount : amount,
    );
          ...
          <TooltipTrigger asChild>
            <div className="flex w-full items-center gap-2" data-slot="filters-input-wrapper">
              <input
                ...
                className={cn(
                  inputClass,
                  '[field-sizing:content] min-w-[1ch] [appearance:textfield] tabular-nums [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none',
                )}
