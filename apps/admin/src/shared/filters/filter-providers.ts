...
type WritableBy<TType extends FilterTypeId> = ReturnType<
...
export interface DomainFieldDescriptor<
...
export function domainField<const TKey extends string, TOperator extends string>(
  descriptor:
...
export type FieldDescriptor = TypedFieldDescriptor | DomainFieldDescriptor<string>;
...
export function describeField(descriptor: FieldDescriptor): FilterField {
  ...
  const registered: FilterTypeFacts | undefined = descriptor.type
    ? FILTER_TYPES[descriptor.type]
    : undefined;
  ...
  const chosen =
    descriptor.operators?.filter((operator) => encodable.includes(operator)) ?? offered;
...
export function catalogCanRead(
  ...
  return providers.every(
    (provider) =>
      provider.resolved || !(provider.claims ?? []).some((claim) => filterNamesKey(filter, claim)),
  );
}
