import {
  type FilterOption,
  type ValueSource,
  type ValueSourceParams,
  type ValueSourceState,
} from '@tryghost/shade/patterns';
import { type ValueSourceHook, type ValueSourceHookOptions } from './create-remote-value-source';
...
export function createCombinedValueSource<T = string>(
  ...
  return function useCombinedValueSource(options?: ValueSourceHookOptions): ValueSource<T> {
    ...
    const useOptions = useCallback(
      ({ query, selectedValues }: ValueSourceParams<T>): ValueSourceState<T> => {
        ...
        const fallbackOptions = getMissingSelectedOption
