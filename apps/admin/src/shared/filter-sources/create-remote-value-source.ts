...
export type RemoteValueSourceHook<T = string, Item = unknown> = (
  options?: ValueSourceHookOptions,
) => RemoteValueSource<T, Item>;
  ...
  return function useRemoteValueSource(options = {}): RemoteValueSource<T, Item> {
    ...
    const useOptions = ({ query, selectedValues }: ValueSourceParams<T>): ValueSourceState<T> => {
      ...
      const fallbackOptions = useMemo(() => {
        return buildFallbackOptions(selectedValues, mergedOptions, config.getMissingSelectedOption);
