...
const getCountryName = (label: string) => {
  return (
    STATS_LABEL_MAPPINGS[label as keyof typeof STATS_LABEL_MAPPINGS] ||
    countries.getName(label, 'en', { select: 'alias' }) ||
    'Unknown'
  );
};
...
interface LocationsCardProps {
  data:
    | Array<{ location?: string | number; visits?: number; [key: string]: unknown }>
    | null
    | undefined;
  isLoading: boolean;
  range: number;
...
const LocationsCard: React.FC<LocationsCardProps> = ({
  ...
  const totalVisits = useMemo(
    () => data?.reduce((sum, row) => sum + Number(row.visits), 0) || 0,
    [data],
  );

  const processedData = useMemo<ProcessedLocationData[]>(() => {
    // First calculate total visits for known locations only
    const knownTotalVisits =
      data?.reduce(
        (sum, row) =>
          UNKNOWN_LOCATION_VALUES.includes(String(row.location)) ? sum : sum + Number(row.visits),
        0,
      ) || 0;
    ...
    const processed =
      data?.map((row) => ({
        ...
        relativeValue:
          UNKNOWN_LOCATION_VALUES.includes(String(row.location)) || knownTotalVisits === 0
            ? 0
            : Math.min(
                100,
                Math.max(10, Math.ceil(((Number(row.visits) / knownTotalVisits) * 100) / 10) * 10),
              ),
  ...
  const getLocationClassName = (location: { id: string; name: string }) => {
    ...
    const currentData = processedData.find(
      (item) => normalizeCountryCode(item.location) === countryCode,
    );
  ...
  return (
    <Card className="p-0">
      <CardHeader className="border-b">
        ...
        <CardDescription>
          A geographic breakdown of your readers {getPeriodText(range)}
        </CardDescription>
      </CardHeader>
      <CardContent className="p-0">
        <div className="flex flex-col lg:grid lg:grid-cols-2 lg:items-stretch">
            ...
            {tooltipData && (
              ...
              >
                <div className="flex items-center gap-2">
                  <Flag
                    countryCode={`${normalizeCountryCode(tooltipData.countryCode)}`}
                    height="12px"
                    width="20px"
                  />
          ...
          <div
            className="group/datalist flex flex-col justify-between overflow-hidden px-6"
                        ...
                        >
                          ...
                          <DataListItemContent className="group-hover/data:max-w-[calc(100%-140px)]">
                            <div className="flex items-center space-x-3 overflow-hidden">
                              <Flag
                                countryCode={`${normalizeCountryCode(row.location)}`}
                              ...
                              />
                              <div className="truncate font-medium" data-testid="country-name">
                                {countryName}
                              </div>
                            </div>
                          </DataListItemContent>
