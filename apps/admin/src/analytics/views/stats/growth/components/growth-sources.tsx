...
const GrowthSourcesTableBody: React.FC<GrowthSourcesTableProps> = ({
  data,
  currencySymbol,
  limit,
        ...
        <TableRow key={row.source} className="last:border-none">
          <TableCell className="font-medium">
            <div className="flex items-center gap-2">
              ...
              />
              {row.linkUrl ? (
                <a className="hover:underline" href={row.linkUrl} rel="noreferrer" target="_blank">
                  {row.displayName}
                </a>
              ) : (
          ...
          {appSettings?.paidMembersEnabled && (
            <>
              ...
              <TableCell className="text-right font-mono text-sm">
                +{currencySymbol}
                {formatNumber(centsToDollars(row.mrr))}
              </TableCell>
            </>
          )}
        </TableRow>
...
const GrowthSources: React.FC<GrowthSourcesProps> = ({
  ...
  const backendOrderBy = sortBy
    .replace('free_members', 'signups')
    .replace('paid_members', 'paid_conversions');
    ...
    <>
      ...
      {showViewAll && processedData.length > limit && (
        <TableFooter className="border-none bg-transparent hover:bg-transparent!">
          <TableRow>
            <TableCell
              className="border-none bg-transparent px-0 pb-0 hover:bg-transparent!"
              colSpan={4}
            >
              <Sheet>
