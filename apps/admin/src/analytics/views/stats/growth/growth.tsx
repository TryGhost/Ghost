...
const Growth: React.FC = () => {
  ...
  const transformedTopPosts = useMemo<UnifiedGrowthContentData[]>(() => {
    ...
    const uniqueData = growthData.reduce(
      (acc: Map<string, TopPostStatItem>, item: TopPostStatItem) => {
        const key =
          item.post_id ||
          (item.title && item.title.trim() !== '' ? item.title : item.attribution_url);
        if (!key) {
    ...
    const totalFreeMembers = filteredData.reduce(
      (sum: number, item: TopPostStatItem) => sum + item.free_members,
      0,
    );
    const totalPaidMembers = filteredData.reduce(
      (sum: number, item: TopPostStatItem) => sum + item.paid_members,
      0,
    );
  ...
  return (
    <StatsLayout>
      ...
      <StatsView
        data={isPageLoading ? undefined : chartData}
        isLoading={false}
        loadingComponent={<></>}
      >
        ...
        <Card className="w-full overflow-x-auto" data-testid="top-content-card">
          <CardHeader>
            ...
            <CardDescription>
              {getGrowthContentDescription(selectedContentType, range, getPeriodText)}
            </CardDescription>
          </CardHeader>
          <CardContent>
            <Table>
              <TableHeader>
                <TableRow className="[&>th]:h-auto [&>th]:pt-0 [&>th]:pb-2">
                  <TableHead className="min-w-[320px] pl-0">
                    ...
                    >
                      <TabsList>
                        <TabsTrigger value={CONTENT_TYPES.POSTS_AND_PAGES}>
                          Posts & pages
                        </TabsTrigger>
                ...
                <TableBody>
                  {isTableLoading ? (
                    ...
                    transformedTopPosts.map((post, index) => (
                      <TableRow
                        key={`${selectedContentType}-${post.post_id || `${post.title}-${index}`}`}
                      ...
                      >
                        <TableCell>
                          <div className="group/link inline-flex flex-col items-start gap-px">
                            {post.post_id && post.attribution_type === 'post' ? (
                              <Button
                                ...
                                onClick={getClickHandler(
                                  post.attribution_url,
                                  post.post_id,
                                  site.url || '',
                                  navigate,
                                  post.attribution_type,
                                )}
                              >
                                {post.title}
                              </Button>
                            ...
                            {post.published_at &&
                              formatDisplayDate &&
                              new Date(post.published_at).getTime() > 0 && (
                                <span className="text-muted-foreground">
                                  Published on {formatDisplayDate(post.published_at, siteTimezone)}
                                </span>
                              )}
                          </div>
                        </TableCell>
                        ...
                        {appSettings?.paidMembersEnabled && (
                          <>
                            ...
                            <TableCell className="text-right font-mono text-sm">
                              {post.mrr > 0 && '+'}
                              {currencySymbol}
                              {formatNumber(centsToDollars(post.mrr))}
                            </TableCell>
                          </>
                        )}
                      </TableRow>
