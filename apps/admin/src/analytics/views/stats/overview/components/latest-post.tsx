...
const LatestPost: React.FC<LatestPostProps> = ({ latestPostStats, isLoading }) => {
  ...
  const metricsToShow = latestPostStats
    ? getPostMetricsToDisplay(latestPostStats as Post, {
        membersTrackSources,
      })
    : null;
  ...
  const postDestination = getPostDestination({
    postId: latestPostStats?.id,
    hasEmailData,
          ...
          <>
            <div className="flex flex-col gap-6 px-6 transition-all md:flex-row md:items-start xl:col-span-2">
              {latestPostStats.feature_image && (
                <div
                  className="aspect-[16/10] w-full min-w-[100px] rounded-sm bg-cover bg-center sm:max-w-[170px] lg:max-w-[170px] xl:max-w-[232px]"
                  style={{
                ...
                <div className="mt-1 text-muted-foreground">
                  {latestPostStats.authors && latestPostStats.authors.length > 0 && (
                    <div>
                      ...
                      {formatDisplayDate(latestPostStats.published_at, siteTimezone)}
                    </div>
                  )}
                  <div className="mt-0.5">{getPostStatusText(latestPostStats)}</div>
                </div>
                <div className="mt-6 flex items-center gap-2">
                  {!latestPostStats.email_only && (
                    <PostShareModal
                      author={
                        latestPostStats.authors?.map((author) => author.name).join(', ') || ''
                      }
                      description=""
                ...
                {metricsToShow.showWebMetrics && webAnalytics && (
                  ...
                  >
                    <div className="flex items-center gap-1.5 font-medium text-muted-foreground transition-all group-hover:text-foreground">
                      ...
                      <span className="hidden md:visible! md:block!">Visitors</span>
                    </div>
                ...
                {metricsToShow.showMemberGrowth && (
                  <div
                    className={cn(
                      ...
                      metricsToShow.showEmailMetrics &&
                        (!metricsToShow.showWebMetrics || !webAnalytics) &&
                        'col-[1/2] row-[2/3]',
                    )}
                    ...
                    {emailTrackOpensEnabled && (
                      ...
                      >
                        <div className="flex items-center gap-1.5 font-medium text-muted-foreground transition-all group-hover:text-foreground">
                          ...
                          <span className="hidden whitespace-nowrap md:visible! md:block!">
                            Opens
                          </span>
                        </div>
                        <span className="text-[2.2rem] leading-none font-semibold tracking-tighter">
                          {latestPostStats.email.email_count
                            ? formatPercentage(
                                (latestPostStats.email.opened_count || 0) /
                                  latestPostStats.email.email_count,
                              )
                            : '0%'}
                        </span>
                      ...
                      >
                        ...
                        <span className="text-[2.2rem] leading-none font-semibold tracking-tighter">
                          {latestPostStats.email.email_count && latestPostStats.count?.clicks
                            ? formatPercentage(
                                (latestPostStats.count.clicks || 0) /
                                  latestPostStats.email.email_count,
                              )
                            : '0%'}
                        </span>
