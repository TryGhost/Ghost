...
The opt-out belongs to whichever component starts a fetch, not to the cache entry: a query key shared with a screen outside the flow is refetched with the options of whoever triggered that fetch. Every editor component reading a shared key therefore opts out too — the status line's count and settings reads, the card config's boot reads, and the preview modal's — so that a refetch one of them initiates cannot navigate away mid-edit. Those reads keep the global error handler, but a session-expiry error is one it deliberately swallows, so a 401 on one of them is silent: it neither navigates nor says anything. The editor reports the expiry from the save engine instead.

The opt-out does not reach every request an editor component causes, and cannot yet. `createQuery` and `createInfiniteQuery` call `usePermission` on every query, and `usePermission` reads `useCurrentUser()` with no options — so each framework query hook mounts a redirect-capable observer of the current user, and even an opted-out query can redirect through that inner read. `usePaidMembersEnabled`, `useNewslettersEnabled` and `usePinturaConfig` read settings and config with no options of their own, and the editor screen still calls `useCurrentUser()` twice untouched. What keeps those quiet today is only order — an opted-out caller usually reaches the same key first. Closing the gap means threading request options through `usePermission`, a framework change of its own.
