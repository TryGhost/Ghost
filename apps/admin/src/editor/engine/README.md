...
`contentChanged()` registers the current live document without starting a save.
`getPendingSave()` returns its version and eligibility reason, independently of
`getState()` (the active request/queue). `onPendingSaveChange` reports changes to
this metadata; ordinary state subscribers still receive deduplicated activity
transitions. Pending content is read from the live snapshot, not a second copy
of the post.

Reasons distinguish `field-commit`, `debounce`, `update`, `saving`, `queued`,
`validation`, `reauth`, `conflict`, and `error`. An edit can await a commit while
an older version is saving. Content awaiting a commit, Update, or local
validation occupies no runnable slot and no unresolved save promise; the next eligible dispatch builds from the current
whole document. Acknowledgements preserve newer edits and release pending work
only when the reconciled document is clean.

Local validation in `prepare` completes a background attempt as `blocked` and
retains its content with a validation reason. Explicit and status commands
complete as `failed` and surface an error, retaining content but no automatic
replay of their status/email target. Background retries of the unchanged invalid
version are suppressed. Every trigger uses this same path, including timers and
commands queued behind another save. Transport failures keep their existing
...
Three documents: **saved** (last persisted state, from load/refetch/acknowledged save), **baseline** (the hidden instance's post-load serialization — the document after Lexical's load-time transforms), **live** (the visible editor). Body verdict: dirty ⇔ live differs from saved **and** from baseline. Baseline readiness is separate from its value: `pending` (not reported yet), `ready` (a known document; `null`, `''`, and an empty root are all known-empty), `failed`. A live edit while pending is dirty (`BASELINE_PENDING`, fail closed); a failed baseline falls back to live-vs-saved (`BASELINE_FAILED`) and never disables body protection. Title, the ordered tag list, and the editable attributes contribute their own dirty bits. Stable reason codes identify each cause: `POST_HAS_ERROR`, `POST_TAGS_DIVERGED`, `POST_TITLE_DIVERGED`, `SCRATCH_DIVERGED_FROM_SECONDARY`, `NEW_POST_HAS_CHANGED_ATTRIBUTES`, `POST_HAS_DIRTY_ATTRIBUTES`, `BASELINE_PENDING`, `BASELINE_FAILED`, `LEXICAL_PARSE_FAILED` (malformed or structurally invalid Lexical is dirty, never a thrown route blocker).
...
| `verdict({includeDiff?})`                                        | `{dirty, reasons}`; `includeDiff` adds a human-readable body diff, and only where the baseline is ready and the live body diverged from it                                                                                                                                                                                                                                                                                  |
