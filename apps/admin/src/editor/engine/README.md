...
| `field`                           | a title, feature-image or settings commit the session's save policy gate passes   | none                                | no              | never; drafts only. On a published/scheduled/sent post it is dropped with reason `not-draft`                      |
