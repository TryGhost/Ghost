...
vi.mock('@/posts/analytics/hooks/use-gift-link-usage', () => ({
  useGiftLinkUsage: (...args: unknown[]) =>
    mockUseGiftLinkUsage(...args) as ReturnType<
      typeof import('@/posts/analytics/hooks/use-gift-link-usage').useGiftLinkUsage
    >,
}));

vi.mock('@/posts/analytics/hooks/use-post-details', () => ({
  usePostDetails: (...args: unknown[]) =>
    mockUsePostDetails(...args) as ReturnType<
      typeof import('@/posts/analytics/hooks/use-post-details').usePostDetails
    >,
}));
...
describe('GiftLinkModal', () => {
  ...
  it('ensures the gift link once when opened under StrictMode', async () => {
    ...
    expect(
      await screen.findByText('https://example.com/test-post/?gift=gift-token'),
    ).toBeInTheDocument();
  });
  ...
  it('ensures again after the modal is closed and reopened', async () => {
    ...
    rerender(
      <GiftLinkModal open={false} postId="post-id" source="share-modal" onOpenChange={vi.fn()} />,
    );
  ...
  it('ensures again when the open modal receives a different post id', async () => {
    ...
    rerender(
      <GiftLinkModal postId="other-post-id" source="share-modal" open onOpenChange={vi.fn()} />,
    );
