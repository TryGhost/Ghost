...
import {
  useMrrHistory,
  usePostGrowthStats as usePostGrowthStatsAPI,
  usePostReferrers as usePostReferrersAPI,
} from '@tryghost/admin-x-framework/api/stats';

export const usePostReferrers = (postId: string) => {
  const { data: postReferrerResponse, isLoading: isPostReferrersLoading } =
    usePostReferrersAPI(postId);
  // API doesn't support date_from yet, so we fetch all data and filter on the client for now
  const { data: postGrowthStatsResponse, isLoading: isPostGrowthStatsLoading } =
    usePostGrowthStatsAPI(postId);
  ...
  const isLoading = useMemo(
    () => isPostReferrersLoading || isPostGrowthStatsLoading || isMrrLoading,
    [isPostReferrersLoading, isPostGrowthStatsLoading, isMrrLoading],
  );
