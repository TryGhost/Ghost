...
const mockUseTinybirdQuery = vi.mocked(
  await import('@tryghost/admin-x-framework'),
).useTinybirdQuery;
const mockUseBrowseConfig = vi.mocked(
  await import('@tryghost/admin-x-framework/api/config'),
).useBrowseConfig;
const mockUseBrowseSettings = vi.mocked(
  await import('@tryghost/admin-x-framework/api/settings'),
).useBrowseSettings;
...
describe('useGiftLinkUsage', () => {
  beforeEach(() => {
    ...
    mockUseBrowseConfig.mockReturnValue({
      data: { config: { stats: { id: 'site-uuid' } } },
    } as unknown as ReturnType<typeof mockUseBrowseConfig>);
    mockUseBrowseSettings.mockReturnValue({
      data: { settings: [{ key: 'web_analytics_enabled', value: true }] },
    } as unknown as ReturnType<typeof mockUseBrowseSettings>);
  ...
  it('returns undefined usage when web analytics is disabled', () => {
    mockUseBrowseSettings.mockReturnValue({
      data: { settings: [{ key: 'web_analytics_enabled', value: false }] },
    } as unknown as ReturnType<typeof mockUseBrowseSettings>);
  ...
  it('returns undefined usage when there is no token yet', () => {
    const { result } = renderHook(() =>
      useGiftLinkUsage({ postUuid: 'post-uuid', token: undefined }),
    );
  ...
  it('reports loading while the active gift-link lookup is still resolving', () => {
    ...
    const { result } = renderHook(() =>
      useGiftLinkUsage({ postUuid: 'post-uuid', token: undefined, tokenLoading: true }),
    );
  ...
  it('surfaces an active gift-link lookup error so the count stays hidden', () => {
    ...
    const { result } = renderHook(() =>
      useGiftLinkUsage({ postUuid: 'post-uuid', token: undefined, tokenError: new Error('boom') }),
    );
