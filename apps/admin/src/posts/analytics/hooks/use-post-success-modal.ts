    ...
    const getDescription = () => {
      ...
      if (post.email?.email_count) {
        ...
        if (post.newsletter?.name) {
          parts.push(' of ');
          parts.push(
            React.createElement('strong', { key: 'newsletter-name' }, post.newsletter.name),
          );
        }
        parts.push(',');
      }
        ...
        if (isToday) {
          ...
          parts.push(
            React.createElement(
              'strong',
              { key: 'date' },
              publishedDate.toLocaleDateString('en-US', { month: 'long', day: 'numeric' }),
            ),
          );
        }
        parts.push(' at ');
        parts.push(
          React.createElement('strong', { key: 'time' }, formatPublicationTime(post.published_at)),
        );
      }
  ...
  useEffect(() => {
    const checkForPublishedPost = () => {
      try {
        ...
        if (storedData) {
          const parsedData = JSON.parse(storedData) as PublishedPostData;
