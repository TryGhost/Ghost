...
const WebOverview: React.FC<WebOverviewProps> = ({
  chartData,
  range,
  isLoading,
  visitors,
  ...
  return (
    <>
      <Card
        className={`group/datalist overflow-hidden ${!isNewsletterShown && 'col-span-2'}`}
        data-testid="web-performance"
      >
        ...
        <CardContent>
          <div>
            <KpiCardHeader
              className="group relative flex grow flex-row items-start justify-between gap-5 border-none px-0 pt-0"
              data-testid="unique-visitors"
            >
              <div className="flex grow flex-col gap-1.5 border-none pb-0">
                <KpiCardHeaderLabel color="var(--chart-blue)">Unique visitors</KpiCardHeaderLabel>
                <KpiCardHeaderValue value={formatNumber(visitors)} />
              </div>
            </KpiCardHeader>
            <Separator />
