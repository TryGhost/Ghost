...
export const processAndGroupTopLinks = (
  clicksResponse: LinkResponseType | undefined,
): CleanedLink[] => {
  // Convert clicksResponse to links format
  const links =
    clicksResponse?.links.map((link) => ({
      link: link.link,
  ...
  const linksByTitle = cleanedLinks.reduce(
    (acc: Record<string, CleanedLink>, link: CleanedLink) => {
