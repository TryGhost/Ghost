        ...
        <TableHeader>
          <TableRow>
            <TableHead
              ...
              variant={headerStyle === 'table' ? 'default' : 'cardhead'}
            >
              {children}
            </TableHead>
              ...
              <TableRow key={row.source} className="last:border-none">
                ...
                {appSettings?.paidMembersEnabled && (
                  <>
                    ...
                    <TableCell className="text-right font-mono text-sm">
                      +${formatNumber(centsToDollars(row.mrr || 0))}
                    </TableCell>
                  </>
                )}
              </TableRow>
...
const GrowthSources: React.FC<SourcesCardProps> = ({
  ...
  return (
    <Card
      className={cn(
        'group/datalist w-full max-w-[calc(100vw-64px)] overflow-x-auto sidebar:max-w-[calc(100vw-64px-280px)]',
        className,
      )}
      data-testid="top-sources-card"
    >
      {topSources.length <= 0 && (
        <CardHeader>
        ...
        {mode === 'growth' && !appSettings?.analytics.membersTrackSources ? (
          ...
          <div className="py-20 text-center text-sm text-gray-700">
            <EmptyIndicator
              className="h-full"
              description={
                mode === 'growth' && `Once someone signs up on this post, sources will show here`
              }
