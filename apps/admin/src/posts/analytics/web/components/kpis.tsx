...
const Kpis: React.FC<KpisProps> = ({ data, range }) => {
  ...
  const chartData = sanitizeChartData<KpiDataItem>(
    (data as KpiDataItem[]) || [],
    range,
  ...
  const chartRange = getEffectiveChartRange(range, data || [], {
      ...
      <CardContent>
        <Tabs defaultValue={currentTab} variant="kpis">
          <TabsList className="-mx-6 hidden grid-cols-2 md:visible! md:grid!">
            ...
            >
              <KpiTabValue
                color={KPI_METRICS.visits.color}
            ...
            >
              <KpiTabValue
                color={KPI_METRICS.views.color}
                label="Total views"
                value={kpiValues.views}
              />
            </KpiTabTrigger>
          </TabsList>
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-56">
              ...
              <DropdownMenuItem onClick={() => setCurrentTab('views')}>
                Total views
              </DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
