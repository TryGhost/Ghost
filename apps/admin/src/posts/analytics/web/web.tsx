...
import {
  ...
  useFilterParams,
} from '@/shared/analytics/use-filter-params';
...
const Web: React.FC = () => {
  ...
  const handleLocationClick = useCallback(
    (location: string) => handleFilterClick('location', location),
    [handleFilterClick],
  );
  const handleSourceClick = useCallback(
    (source: string) => handleFilterClick('source', source),
    [handleFilterClick],
  );
  ...
  const totalVisits = useMemo(
    () => locationsData?.reduce((sum, row) => sum + Number(row.visits), 0) || 0,
    [locationsData],
  );
  ...
  const isPageLoading =
    isConfigLoading || isPostLoading || isKpisLoading || isLocationsLoading || isSourcesLoading;
    ...
    <>
      ...
      <PostAnalyticsContent ref={containerRef}>
        {isPageLoading ? (
          ...
          <>
            <Kpis data={kpiData as KpiDataItem[] | null} range={chartRange} />
