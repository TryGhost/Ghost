...
describe('getCelebrationCopy', () => {
    ...
    it('says so when the post was email-only', () => {
        expect(getCelebrationCopy({wasPublished: true, type: 'post', emailOnly: true, postCount: 9}).secondary)
            .toBe('Your email has been sent.');
    });

    it('has its own line for a page, whatever the count says', () => {
        expect(getCelebrationCopy({wasPublished: true, type: 'page', postCount: 9}).secondary)
            .toBe('Your page is published.');
    });

    it('says "post" rather than "posts" for the first one', () => {
        expect(getCelebrationCopy({wasPublished: true, type: 'post', postCount: 1}).secondary)
            .toBe('That’s 1 post published.');
    });

    it('groups the digits of a large count', () => {
        expect(getCelebrationCopy({wasPublished: true, type: 'post', postCount: 1234}).secondary)
    ...
    it('ignores everything else when scheduled', () => {
        expect(getCelebrationCopy({wasPublished: false, type: 'page', emailOnly: true, postCount: 9}))
