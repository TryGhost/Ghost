...
describe('getPostSelectionFilter', () => {
    ...
    it('lists the ids of an ordinary selection', () => {
        expect(getPostSelectionFilter(selection({selectedIds: new Set(['a', 'b'])}), ''))
            .toBe('id:[\'a\',\'b\']');
    });
    ...
    it('ignores the active filter for an ordinary selection', () => {
        expect(getPostSelectionFilter(selection({selectedIds: new Set(['a'])}), 'status:draft'))
            .toBe('id:[\'a\']');
    });
    ...
    describe('after Select All', () => {
        ...
        it('matches the list filter when one is active', () => {
            expect(getPostSelectionFilter(selection({inverted: true}), 'status:draft'))
                .toBe('status:draft');
        });
        ...
        it('subtracts deselected ids with no filter to intersect', () => {
            expect(getPostSelectionFilter(
                selection({inverted: true, selectedIds: new Set(['a'])}),
                ''
            )).toBe('id:-[\'a\']');
        });
    });
