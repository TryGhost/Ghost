...
const emailed = (over: Partial<PostListItem> = {}) =>
  post({
    email: {
      status: 'submitted',
      email_count: 100,
...
describe('the Visitors column', () => {
  ...
  it('hides for a draft', () => {
    expect(
      keys(post({ status: 'draft' }), settings({ webAnalyticsEnabled: true }), 'posts'),
  ...
  it('hides for an email-only post', () => {
    expect(
      keys(post({ status: 'sent' }), settings({ webAnalyticsEnabled: true }), 'posts'),
    ).not.toContain('visitors');
  });
});
...
describe('the email columns', () => {
  ...
  it('shows nothing email-related for a post that was never emailed', () => {
    const shown = keys(
      post(),
      settings({ emailTrackOpens: true, emailTrackClicks: true }),
      'posts',
    );
...
describe('the Members column', () => {
  ...
  it('hides for an unpublished post', () => {
    expect(
      keys(post({ status: 'draft' }), settings({ membersTrackSources: true }), 'posts'),
...
describe('column order', () => {
  it('reads visitors, opens, clicks, then members', () => {
    ...
    ).toEqual(['visitors', 'opens', 'clicks', 'members']);
  });
});
...
describe('hasPostAnalyticsPage', () => {
  ...
  it('is false when only the Visitors column shows', () => {
    ...
    expect(getPostMetricColumns(webOnlyPost, onlyWeb, 'posts').map((column) => column.key)).toEqual(
      ['visitors'],
    );
  ...
  it('is true on the attribution path alone, with no email at all', () => {
    ...
    expect(
      hasPostAnalyticsPage(published, settings({ membersTrackSources: true }), 'posts', true),
    ).toBe(true);
  });
  ...
  it('is false when the only email column is the Sent fallback', () => {
    ...
    expect(
      getPostMetricColumns(emailedPost, untracked, 'posts').map((column) => column.key),
    ).toEqual(['sent']);
