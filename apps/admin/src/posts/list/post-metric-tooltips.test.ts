...
describe('getPostMetricTooltip', () => {
  ...
  it('lists only what is being tracked', () => {
    expect(
      getPostMetricTooltip('sent', emailed, { showOpens: false, showClicks: false }).rows,
    ).toEqual([{ label: 'Sent', value: 200, icon: 'sent' }]);
  });

  it('breaks new members into free and paid', () => {
    expect(
      getPostMetricTooltip('members', post(), {
        ...base,
        freeMembers: 69,
        paidMembers: 19,
      ...
      title: 'New members',
      rows: [
        { label: 'Free', value: 69, icon: 'free' },
        { label: 'Paid', value: 19, icon: 'paid' },
      ],
    });
  });
  ...
  it('omits paid when paid members are off', () => {
    ...
    ).toEqual([{ label: 'Free', value: 69, icon: 'free' }]);
  });
});
