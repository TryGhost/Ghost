...
export const POST_VIEW_COLORS = [
    'midgrey', 'blue', 'green', 'red', 'teal', 'purple', 'yellow', 'orange', 'pink'
] as const;
...
export function canSavePostView({
    isAdmin, resource, params, isDefaultView
...
export function findActivePostView(
    ...
    return views.find(view => POST_VIEW_PARAMS.every(
        param => (view.filter[param] ?? null) === (current[param] ?? null)
    ));
}
