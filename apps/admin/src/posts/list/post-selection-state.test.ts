...
function run(
  actions: PostSelectionAction[],
  from: PostSelectionState = initialPostSelection,
): PostSelectionState {
...
describe('postSelectionReducer', () => {
  ...
  describe('shift', () => {
    ...
    it('selects the range from the anchor', () => {
      const state = run([
        { type: 'toggle', id: 'b' },
        { type: 'shift', id: 'd', orderedIds: ids },
      ]);
...
describe('isSinglePostSelected', () => {
  ...
  it('is false when inverted, whatever the id count', () => {
    expect(isSinglePostSelected(run([{ type: 'selectAll' }, { type: 'toggle', id: 'b' }]))).toBe(
      false,
    );
  });
});
...
describe('transient selection', () => {
  ...
  it('leaves an inverted selection alone, since every row is in it', () => {
    const state = run([{ type: 'selectAll' }, { type: 'contextMenu', id: 'c' }]);
  ...
  it('drops a transient selection when the menu closes', () => {
    const state = run([{ type: 'contextMenu', id: 'd' }, { type: 'closeContextMenu' }]);
