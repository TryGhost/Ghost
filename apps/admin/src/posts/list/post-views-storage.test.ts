...
const postsView = (name: string, filter: Record<string, string>): SharedView => ({
    name, route: 'posts', color: 'blue', filter
});
...
describe('applyPostViewSave', () => {
    it('appends a new view', () => {
        ...
        expect(result).toEqual([{name: 'News', route: 'posts', color: 'blue', filter: {tag: 'news'}}]);
    });

    it('leaves other screens\' views exactly as they were', () => {
        const existing = JSON.stringify([{name: 'VIPs', route: 'members', filter: {filter: 'label:vip'}}]);
    ...
    it('replaces the original when editing, in place', () => {
        ...
        const existing = JSON.stringify([{name: 'VIPs', route: 'members', filter: {filter: 'x'}}, original]);
        const result = parse(applyPostViewSave(existing, 'Renamed', {tag: 'other'}, 'blue', original));
...
describe('applyPostViewDelete', () => {
    it('removes only the target', () => {
        ...
        const existing = JSON.stringify([{name: 'VIPs', route: 'members', filter: {filter: 'x'}}, target]);
    ...
    it('throws when the view is already gone', () => {
        expect(() => applyPostViewDelete('[]', postsView('News', {tag: 'news'})))
            .toThrow(/could not be found/i);
    });

    it('refuses to write when the stored value is unreadable', () => {
        expect(() => applyPostViewDelete('nonsense', postsView('News', {tag: 'news'})))
            .toThrow(/could not be read/i);
    });
});
