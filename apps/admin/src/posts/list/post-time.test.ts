...
describe('formatPostTime', () => {
  describe('within 12 hours either way', () => {
    ...
    it('reads as relative for a recent past time', () => {
      expect(formatPostTime('2026-08-04T10:00:00.000Z', { timezone: TZ, now: NOW })).toBe(
        '2 hours ago',
      );
    });

    it('reads as relative for a near-future time', () => {
      expect(formatPostTime('2026-08-04T14:00:00.000Z', { timezone: TZ, now: NOW })).toBe(
        'in 2 hours',
      );
    });
  });
  ...
  describe('same day, more than 12 hours away', () => {
    ...
    it('shows the time and "Today"', () => {
      expect(formatPostTime('2026-08-04T23:30:00.000Z', { timezone: TZ, now: EARLY })).toBe(
        '23:30 (UTC) Today',
      );
    });
  ...
  describe('yesterday', () => {
    it('shows the time and "yesterday"', () => {
      expect(
        formatPostTime('2026-08-03T09:00:00.000Z', { timezone: TZ, now: NOW, absolute: true }),
      ).toBe('09:00 (UTC) yesterday');
    });

    it('drops the time in short form', () => {
      expect(
        formatPostTime('2026-08-03T09:00:00.000Z', {
          timezone: TZ,
          now: NOW,
          absolute: true,
          short: true,
  ...
  it('shows the time and "tomorrow" when scheduled for tomorrow', () => {
    expect(
      formatPostTime('2026-08-05T09:00:00.000Z', { timezone: TZ, now: NOW, scheduled: true }),
  ...
  describe('further away', () => {
    it('shows just the date in short form', () => {
      expect(
        formatPostTime('2026-07-01T09:00:00.000Z', {
          timezone: TZ,
          now: NOW,
          absolute: true,
          short: true,
    ...
    it('shows time and date in long form', () => {
      expect(
        formatPostTime('2026-07-01T09:00:00.000Z', { timezone: TZ, now: NOW, absolute: true }),
      ).toBe('09:00 (UTC) 01 Jul 2026');
    });

    it('reads as a sentence when scheduled', () => {
      expect(
        formatPostTime('2026-09-01T09:00:00.000Z', { timezone: TZ, now: NOW, scheduled: true }),
  ...
  describe('timezone offsets', () => {
    it('writes a bare (UTC) with no offset', () => {
      expect(
        formatPostTime('2026-07-01T09:00:00.000Z', { timezone: 'UTC', now: NOW, absolute: true }),
      ).toContain('(UTC)');
    });
