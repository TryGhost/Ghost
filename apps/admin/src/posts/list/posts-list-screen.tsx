...
export function PostsListScreen({ resource }: { resource: PostResource }) {
  ...
  const isOnDefaultView = POST_DEFAULT_VIEWS.some((view) =>
    findActivePostView(
      [
        {
          ...view,
  ...
  const bucketFilters = useMemo(
    () =>
      getActiveBuckets(params).map((bucket) =>
        buildBucketFilter(bucket, params, { ownAuthorSlug }),
      ),
