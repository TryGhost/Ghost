...
describe('getBulkConfirmCopy', () => {
    ...
    it('reverts rather than deletes when unpublishing', () => {
        expect(getBulkConfirmCopy('unpublish', {count: 1, resource: 'posts', title: 'Live one'})).toEqual({
    ...
    it('says page on the pages screen', () => {
        expect(getBulkConfirmCopy('delete', {count: 2, resource: 'pages'}).title)
            .toBe('Are you sure you want to delete these pages?');
    });
    ...
    it('stays plural for an inverted selection of one', () => {
        const copy = getBulkConfirmCopy('delete', {
            count: 1, resource: 'posts', title: 'Only one', isSingle: false
        });
