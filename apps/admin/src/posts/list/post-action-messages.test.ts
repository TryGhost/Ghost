...
describe('getPostActionMessage', () => {
    ...
    describe('several posts', () => {
        ...
        it('accessUpdated leads with the type, not the count', () => {
            expect(getPostActionMessage('accessUpdated', {count: 3, resource: 'posts'}))
                .toBe('Post access updated for 3 posts');
        });
    });
    ...
    describe('pages', () => {
        it('uses the page noun, capitalised where the string leads with it', () => {
            ...
            expect(getPostActionMessage('deleted', {count: 4, resource: 'pages'})).toBe('4 pages deleted');
        });
        ...
        it('still says "Post link copied" on a page, as Ember does', () => {
            expect(getPostActionMessage('copiedPostUrl', {count: 1, resource: 'pages'}))
                .toBe('Post link copied');
            expect(getPostActionMessage('copiedPreviewUrl', {count: 1, resource: 'pages'}))
                .toBe('Preview link copied');
        });
    });
    ...
    it('keeps the copy messages singular whatever the count', () => {
        expect(getPostActionMessage('copiedPostUrl', {count: 9, resource: 'posts'}))
            .toBe('Post link copied');
    });
});
