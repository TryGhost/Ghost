...
describe('buildAllFilter', () => {
  ...
  it('omits blank params rather than emitting empty clauses', () => {
    expect(buildAllFilter({ type: null, visibility: null, author: null, tag: null })).toBe(
  ...
  it('treats a whitespace-only value as blank, like Ember', () => {
    expect(buildAllFilter({ tag: '   ', author: ' ' })).toBe(
      'status:[draft,scheduled,published,sent]',
    );
  });
  ...
  it('orders clauses tag, visibility, status, featured, authors', () => {
    expect(
      ...
      'tag:news+visibility:paid+status:[draft,scheduled,published,sent]+featured:true+authors:jo',
    );
  });

  it('adds featured:true only for type=featured', () => {
    expect(buildAllFilter({ type: 'featured' })).toBe(
      'status:[draft,scheduled,published,sent]+featured:true',
    );
  ...
  it('passes the paid+tiers visibility value through untouched', () => {
    ...
    expect(buildAllFilter({ visibility: '[paid,tiers]' })).toBe(
      'visibility:[paid,tiers]+status:[draft,scheduled,published,sent]',
    );
  });

  it('narrows to the current user for authors and contributors', () => {
    expect(buildAllFilter({ author: 'someone-else' }, { ownAuthorSlug: 'me' })).toBe(
      'status:[draft,scheduled,published,sent]+authors:me',
    );
  });

  it('uses the author param when the user is not scoped to their own posts', () => {
    expect(buildAllFilter({ author: 'jo' }, {})).toBe(
      'status:[draft,scheduled,published,sent]+authors:jo',
    );
  });
});
...
describe('buildBucketFilter', () => {
  it('replaces the status clause in place, keeping clause order', () => {
    expect(buildBucketFilter('scheduled', { tag: 'news', author: 'jo' })).toBe(
      'tag:news+status:scheduled+authors:jo',
    );
  });
