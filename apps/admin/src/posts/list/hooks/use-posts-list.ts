import {BUCKET_ORDER, getActiveBuckets, getBucketSearchParams} from '@/posts/list/post-query-params';
import {composePostBuckets, type ComposedPostList, type PostBucketResult} from '@/posts/list/compose-post-buckets';
...
function useBucketQuery(
    ...
    const items: PostListItem[] = (resource === 'pages'
...
export function usePostsList(options: UsePostsListOptions): UsePostsListReturn {
    ...
    const publishedAndSent = useBucketQuery('publishedAndSent', options, activeBuckets.includes('publishedAndSent'));
    ...
    return composePostBuckets(
        BUCKET_ORDER.filter(bucket => activeBuckets.includes(bucket)).map(bucket => byBucket[bucket])
    );
}
