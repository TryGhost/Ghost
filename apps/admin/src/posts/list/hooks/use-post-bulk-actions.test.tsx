...
describe('usePostBulkActions cache patching', () => {
  ...
  const bucketUrl = (filter: string) =>
    `/ghost/api/admin/posts/?filter=${encodeURIComponent(filter)}&order=published_at%20desc&limit=30`;

  const listPost = (id: string, status: string): PostListItem =>
    ({ id, status, title: id }) as PostListItem;
  ...
  const seedBucket = (
    ...
    queryClient.setQueryData(['PostsResponseType', bucketUrl(filter)], {
      ...
      pages: [
        {
          posts,
          meta: { pagination: { total, page: 1, limit: 30, pages: 1, next: null, prev: null } },
        },
      ],
    });
  };
  ...
  it('removes deleted rows and decrements the total', async () => {
    seedBucket(
      queryClient,
      PUBLISHED_FILTER,
      [listPost('p1', 'published'), listPost('p2', 'published')],
      40,
    );
  ...
  it('leaves a list whose filter merely extends the bucket filter alone', async () => {
    ...
    expect(
      readBucket(queryClient, `${DRAFT_FILTER}+featured:true`)?.pages[0].posts.map(
        (post) => post.id,
      ),
    ).toEqual(['p1']);
  });
