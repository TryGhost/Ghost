...
export function usePostBulkActions({ resource, onDeleted, onEdited }: UsePostBulkActionsOptions) {
  ...
  const patchCaches = useCallback(
    (snapshot: BulkActionSnapshot, action: PostContextMenuKey): Set<string> => {
          ...
          (cached) => {
            ...
            return {
              ...
              pages: pages.map((page) =>
                typeof page.meta?.pagination?.total === 'number'
                  ? {
                      ...page,
                      meta: {
                        ...page.meta,
  ...
  const run = useCallback(
    async (key: PostContextMenuKey, snapshot: BulkActionSnapshot) => {
      setIsRunning(true);
      ...
      try {
        ...
        const payload = {
          ...
          action: { type: bulk.action } as PostBulkAction,
        };
        ...
        if (bulk.message) {
          toast.success(
            getPostActionMessage(bulk.message, {
              count: snapshot.count,
              resource,
              isSingle: snapshot.isSingle,
            }),
