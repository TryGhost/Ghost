...
const view = (name: string, filter: Record<string, string | null>): SharedView => ({
    name, route: 'posts', filter, color: 'blue'
});
...
describe('canSavePostView', () => {
    ...
    it('allows an admin with a filter set', () => {
        expect(canSavePostView({
            isAdmin: true, resource: 'posts', params: {type: 'draft'}, isDefaultView: false
        })).toBe(true);
    });

    it('refuses a non-admin', () => {
        expect(canSavePostView({
            isAdmin: false, resource: 'posts', params: {type: 'draft'}, isDefaultView: false
        })).toBe(false);
    });
    ...
    it('refuses on the pages screen', () => {
        expect(canSavePostView({
            isAdmin: true, resource: 'pages', params: {type: 'draft'}, isDefaultView: false
        })).toBe(false);
    });

    it('refuses with nothing filtered', () => {
        expect(canSavePostView({
            isAdmin: true, resource: 'posts', params: {}, isDefaultView: false
        })).toBe(false);
    });
    ...
    it('allows a sort on its own', () => {
        expect(canSavePostView({
            isAdmin: true, resource: 'posts', params: {order: 'published_at asc'}, isDefaultView: false
        })).toBe(true);
    });

    it('refuses while a default view is active', () => {
        expect(canSavePostView({
            isAdmin: true, resource: 'posts', params: {type: 'draft'}, isDefaultView: true
        })).toBe(false);
    });
});
...
describe('buildPostView', () => {
    ...
    it('drops empty params so the filter compares equal to a clean URL', () => {
        expect(buildPostView('News', {type: 'draft', tag: null, order: ''}, 'blue').filter)
            .toEqual({type: 'draft'});
    });
...
describe('buildPostViewsForSave', () => {
    ...
    it('rejects a duplicate name on the same route', () => {
        expect(() => buildPostViewsForSave(
            [view('News', {type: 'draft'})], 'News', {tag: 'other'}, 'blue'
    ...
    it('replaces the original when editing', () => {
        ...
        const result = buildPostViewsForSave([original], 'Renamed', {type: 'published'}, 'red', original);
    ...
    it('leaves other views untouched when editing', () => {
        ...
        const result = buildPostViewsForSave([other, original], 'Renamed', {type: 'published'}, 'red', original);
...
describe('buildPostViewsForDelete', () => {
    it('removes the view', () => {
        ...
        expect(buildPostViewsForDelete([target, view('Other', {tag: 'x'})], target))
            .toEqual([view('Other', {tag: 'x'})]);
    });

    it('throws when the view is gone', () => {
        expect(() => buildPostViewsForDelete([], view('News', {type: 'draft'})))
...
describe('findActivePostView', () => {
    ...
    it('finds nothing when no view matches', () => {
        expect(findActivePostView([view('Drafts', {type: 'draft'})], {tag: 'news'})).toBeUndefined();
    });
});
