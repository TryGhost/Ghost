...
export function OnboardingStepItem({
  ...
  return (
    ...
    >
      <span
        className={`flex min-w-0 items-center ${complete ? 'opacity-20 group-hover:opacity-25' : 'group-hover:opacity-90'}`}
      >
        <Icon className="mr-4 size-5 shrink-0 text-purple" />
        <span className="min-w-0 text-left">
          <span className="block truncate pr-8 text-[16px] leading-[1.3] font-bold text-foreground">
            {step.title}
          </span>
          {isNext && (
            <span className="mt-1 block pr-8 text-[15px] leading-[1.4] text-muted-foreground">
              {step.description}
            </span>
          )}
        </span>
      ...
      <span
        className={`flex shrink-0 items-center transition-transform group-hover:translate-x-[5px] ${complete ? 'text-green' : 'text-purple'}`}
      >
        {complete ? (
          <LucideIcon.Check className="size-5" />
        ) : (
          <LucideIcon.ArrowRight className="size-3.5" />
        )}
      </span>
    </button>
  );
