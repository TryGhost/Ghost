...
export function OnboardingChecklist({
  ...
  return (
    <main className="relative flex min-h-screen flex-col items-center justify-center px-6 py-8">
      <section
        className="mt-[-48px] flex w-full flex-col items-center"
        data-test-dashboard="onboarding-checklist"
        data-testid="onboarding-checklist"
      >
        <div className="mb-8 flex flex-col items-center text-center">
          <OnboardingLogoVideo />
          {allStepsCompleted ? (
            <h1 className="text-[32px] leading-[1.15] font-bold tracking-normal text-foreground max-[480px]:text-[24px]">
              You&apos;re all set.
            </h1>
          ) : (
            <>
              ...
              <p className="mt-2 mb-0 text-[15px] text-muted-foreground max-[480px]:m-0 max-[480px]:text-[14px]">
        ...
        <div className="w-full max-w-[540px] rounded-md border border-border bg-background px-6 pt-4 pb-1">
          <div
            className={`mt-[-12px] flex items-center justify-between py-6 ${nextStep === 'customize-design' ? 'border-b-0' : 'border-b border-border'}`}
          >
        ...
        <p className="mt-8 mb-0 text-[15px] text-muted-foreground max-[480px]:text-[14px]">
          ...
          <Button
            className="h-auto p-0 align-baseline text-[15px] text-green hover:text-green/90 max-[480px]:text-[14px]"
            variant="link"
            asChild
          >
            <a
              href="https://ghost.org/help?utm_source=admin&utm_campaign=onboarding"
              id="ob-help-center"
              rel="noreferrer"
              target="_blank"
            >
