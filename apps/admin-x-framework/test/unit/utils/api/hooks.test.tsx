...
describe('API hooks', () => {
  ...
  describe('createQuery', () => {
    ...
    it('can override default query params', async () => {
      await withMockFetch({}, async (mock) => {
        ...
        const { result } = renderHook(() => useTestQuery({ searchParams: { b: '1' } }), {
          wrapper,
        });
  ...
  describe('createSuspenseQuery', () => {
    ...
    const suspenseQueryClient = new QueryClient({
      defaultOptions: {
        queries: {
          retry: false,
          staleTime: Infinity,
    ...
    const suspenseWrapper =
      (onError: (error: unknown) => void = () => {}): React.FC<{ children: ReactNode }> =>
      ({ children }) => (
        <FrameworkProvider
          externalNavigate={() => {}}
          ghostVersion="5.x"
    ...
    it('throws request errors to the nearest error boundary', async () => {
      await withMockFetch(
        {
          json: { errors: [{ message: 'Everything is on fire' }] },
        ...
        async () => {
          ...
          try {
            ...
            // The hook never resolved to a value; the boundary took over
    ...
    it('computes path, body, searchParams', async () => {
      await withMockFetch(
        ...
        async (mock) => {
          ...
          expect(mock.calls.length).toBe(1);
          expect(mock.calls[0]).toEqual([
            'http://localhost:3000/ghost/api/admin/test/hello/?a=hello',
            {
              credentials: 'include',
              headers: {
                'app-pragma': 'no-cache',
          ...
          const useTestMutation = createMutation({
            ...
            updateQueries: {
              ...
              update: (newData, currentData) => {
                return {
                  test: (newData as { test: number }).test + (currentData as { test: number }).test,
                };
              },
            },
          });
