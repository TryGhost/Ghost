  ...
  describe('APIError', () => {
    ...
    it('handles admin API URL with underscores', () => {
      ...
      expect(error.message).toBe(
        'Something went wrong while loading email previews, please try again.',
      );
    });
  ...
  describe('getErrorMessage', () => {
    ...
    it('falls back to the error message when context is empty', () => {
      ...
      expect(getErrorMessage(error, 'Fallback message')).toBe(
        'Validation error, cannot edit label.',
      );
    });
    ...
    it('returns the context of any error carrying an API body, not only a validation one', () => {
      ...
      expect(getErrorMessage(error, 'Fallback message')).toBe(
        'Custom fields are limited to 20 per site.',
      );
    });

    it('falls back to the API message for a non-validation error with no context', () => {
      expect(getErrorMessage(makeHostLimitError(null), 'Fallback message')).toBe(
        'Cannot create custom field.',
      );
    });
