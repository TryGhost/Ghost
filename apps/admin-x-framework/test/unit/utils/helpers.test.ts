...
describe('helpers utils', () => {
  ...
  describe('blobDownload', () => {
    ...
    it('fetches the URL and triggers a download', async () => {
      ...
      expect(globalThis.fetch).toHaveBeenCalledWith('https://example.com/export.csv', {
        method: 'GET',
      });
    ...
    it('names the file from the Content-Disposition header', async () => {
      ...
      globalThis.fetch = vi.fn().mockResolvedValue({
        ok: true,
        headers: new Headers({
          'content-disposition': 'Attachment; filename="my-site.ghost.members.2026-02-17.csv"',
        }),
        blob: () => Promise.resolve(mockBlob),
      });
    ...
    it('throws on non-ok response', async () => {
      ...
      await expect(blobDownload('https://example.com/fail', 'test.csv')).rejects.toThrow(
  ...
  describe('blobDownloadFromEndpoint', () => {
    ...
    it('constructs the full URL from apiRoot and path', async () => {
      ...
      expect(globalThis.fetch).toHaveBeenCalledWith('/ghost/api/admin/members/upload/?limit=all', {
        method: 'GET',
      });
    });
  ...
  describe('getFilenameFromContentDisposition', () => {
    ...
    it('parses a quoted filename', () => {
      expect(
        getFilenameFromContentDisposition(
          'Attachment; filename="my-site.ghost.members.2026-02-17.csv"',
        ),
      ).toBe('my-site.ghost.members.2026-02-17.csv');
    });

    it('parses an unquoted filename', () => {
      expect(getFilenameFromContentDisposition('attachment; filename=members.csv')).toBe(
        'members.csv',
      );
    });
    ...
    it('prefers the RFC 5987 extended (filename*) form and decodes it', () => {
      expect(
        getFilenameFromContentDisposition(
          'attachment; filename="fallback.csv"; filename*=UTF-8\'\'caf%C3%A9.members.csv',
        ),
    ...
    it('handles a non-empty language tag and non-UTF-8 charset in the extended form', () => {
      expect(
        getFilenameFromContentDisposition("attachment; filename*=UTF-8'en'caf%C3%A9.csv"),
      ).toBe('café.csv');
      expect(getFilenameFromContentDisposition("attachment; filename*=ISO-8859-1''file.csv")).toBe(
        'file.csv',
      );
    });
