...
describe('stats-config utils', () => {
  ...
  describe('getStatEndpointUrl', () => {
    ...
    it('constructs URL with production endpoint when local is disabled', () => {
      ...
      expect(getStatEndpointUrl(config, 'analytics')).toBe(
        'https://api.example.com/v0/pipes/analytics.json?',
      );
    });
    ...
    it('constructs URL with local endpoint when local is enabled', () => {
      ...
      expect(getStatEndpointUrl(config, 'analytics')).toBe(
        'http://localhost:8000/v0/pipes/analytics.json?',
      );
    });
    ...
    it('appends custom parameters', () => {
      ...
      expect(getStatEndpointUrl(config, 'analytics', 'foo=bar&baz=qux')).toBe(
        'https://api.example.com/v0/pipes/analytics.json?foo=bar&baz=qux',
      );
    });
    ...
    it('handles undefined endpoint parameter', () => {
      ...
      expect(getStatEndpointUrl(config, undefined)).toBe(
        'https://api.example.com/v0/pipes/undefined.json?',
      );
    });
    ...
    it('prefers endpointBrowser over endpoint when both are present', () => {
      ...
      expect(getStatEndpointUrl(config, 'analytics')).toBe(
        'https://browser-api.example.com/v0/pipes/analytics.json?',
      );
    });
  });
});
