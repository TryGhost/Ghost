...
describe('members api', () => {
  function expectQueryInvalidation(
    queryClient: ReturnType<typeof createTestQueryClient>,
    dataType: string,
    ...
    const queries = queryClient
      .getQueryCache()
      .getAll()
      .filter((q) => q.queryKey[0] === dataType);
  ...
  it('fetches the member count with the dedicated count hook', async () => {
    ...
    await withMockFetch(
      ...
      async (mockFetch) => {
        ...
        expect(mockFetch.calls[0][0].toString()).toBe(
          'http://localhost:3000/ghost/api/admin/members/?limit=1',
        );
      },
    );
  });
  ...
  it('syncs the sidebar member count from an unfiltered members list query', async () => {
    ...
    const memberDetailKey = [
      'MembersResponseType',
      'http://localhost:3000/ghost/api/admin/members/member-1/',
    ];
    ...
    expect(
      queryClient.getQueryData<MembersResponseType>(memberCountKey)?.meta?.pagination.total,
    ).toBe(83427);
    expect(queryClient.getQueryData<MembersResponseType>(memberDetailKey)?.members).toEqual([
      { id: 'member-1' },
    ]);
  });
  ...
  describe('member detail operations', () => {
    ...
    it('reads a member signin url for impersonation', async () => {
      ...
      await withMockFetch(
        {
          json: {
            member_signin_urls: [{ member_id: 'member-1', url: 'https://example.com/magic' }],
          },
        },
        async (mock) => {
    ...
    it('cancels a subscription at period end', async () => {
      ...
      await withMockFetch({ json: { members: [{ id: 'member-1' }] } }, async (mock) => {
        const { result } = renderHookWithProviders(() => useEditMemberSubscription(), {
          queryClient,
        });
        ...
        expect(mock.calls[0][0].toString()).toBe(
          `${apiRoot}/members/member-1/subscriptions/sub_1/`,
        );
        expect(mock.calls[0][1].method).toBe('PUT');
    ...
    it('removes a member from the email suppression list', async () => {
      ...
      await withMockFetch({ json: {} }, async (mock) => {
        const { result } = renderHookWithProviders(() => useRemoveMemberEmailSuppression(), {
          queryClient,
        });
    ...
    it('fetches a member activity feed filtered by member id', async () => {
      ...
      await withMockFetch({ json: { events } }, async (mock) => {
        const { result } = renderHookWithProviders(() => useMemberActivityFeed('member-1'), {
          queryClient,
        });
    ...
    it('advances the activity feed cursor from the last event', async () => {
      ...
      await withMockFetch({ json: { events } }, async (mock) => {
        ...
        expect(nextUrl.searchParams.get('filter')).toBe(
          "data.created_at:<'2024-01-02 10:00:19'+data.member_id:'member-1'",
        );
      });
    });
  });
