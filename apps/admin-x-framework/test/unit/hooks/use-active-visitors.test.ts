...
describe('useActiveVisitors', () => {
  ...
  it('handles statsConfig changes correctly', () => {
    ...
    const { rerender } = renderHook(
      ({ statsConfig: currentStatsConfig }) =>
        useActiveVisitors({ statsConfig: currentStatsConfig, enabled: true }),
  ...
  it('resets interval when enabled state changes', () => {
    ...
    const { rerender } = renderHook(({ enabled }) => useActiveVisitors({ enabled }), {
  ...
  it('transitions from undefined to valid token correctly', () => {
    ...
    // Render the hook
    const { rerender } = renderHook(() => useActiveVisitors({ statsConfig, enabled: true }), {
      wrapper,
    });
