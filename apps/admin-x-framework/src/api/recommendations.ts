...
export type EditOrAddRecommendation = Omit<Recommendation, 'id' | 'created_at' | 'updated_at'> & {
  id?: string;
};
...
export const useEditRecommendation = createMutation<
  RecommendationEditResponseType,
  Partial<Recommendation> & { id: string }
>({
  method: 'PUT',
  path: (recommendation) => `/recommendations/${recommendation.id}/`,
...
export const useBrowseIncomingRecommendations =
  createInfiniteQuery<IncomingRecommendationResponseType>({
    dataType,
