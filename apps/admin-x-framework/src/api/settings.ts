...
export const useBrowseSettings = createQuery<SettingsResponseType>({
  ...
  defaultSearchParams: {
    group:
      'site,theme,private,members,portal,newsletter,email,labs,slack,unsplash,views,firstpromoter,editor,comments,analytics,announcement,pintura,donations,security,social_web,explore,transistor,addons',
  },
});
...
export function getSettingValues<ValueType = SettingValue>(
  settings: Setting[] | null,
  keys: string[],
): Array<ValueType | undefined> {
  return keys.map((key) => settings?.find((setting) => setting.key === key)?.value) as ValueType[];
}

export function getSettingValue<ValueType = SettingValue>(
...
export function isSettingReadOnly(
  settings: Setting[] | null | undefined,
  key: string,
): boolean | undefined {
  if (!settings) {
