...
export const useBrowseActions = createInfiniteQuery<ActionsList>({
  ...
  returnData: (originalData) => {
    ...
    let actions = pages.flatMap((page) =>
      page.actions.map(({ context, ...action }) => ({ ...action, context: JSON.parse(context) })),
    );
...
export const isBulkAction = (action: Action) =>
  typeof action.context?.count === 'number' && action.context?.count > 1;
