...
export const usePreviewAutomationEmail = createMutation<
  AutomationEmailPreviewResponseType,
  { id: string; subject: string; lexical: string }
>({
  method: 'POST',
...
export const useSendTestAutomationEmail = createMutation<
  unknown,
  { id: string; email: string; subject: string; lexical: string }
>({
  method: 'POST',
...
const hasEdge = (
  ...
  detail.edges.some(
    (edge) => edge.source_action_id === sourceActionId && edge.target_action_id === targetActionId,
  );
...
const spliceAction = ({ detail, action, anchor }: SpliceActionArgs): AutomationDetail => {
  ...
  if (
    ...
    !hasEdge(detail, previousActionId, nextActionId)
  ) {
    throw new Error(
      `spliceAction: anchor edge "${previousActionId}" -> "${nextActionId}" does not exist`,
    );
  }
  if (
    previousActionId !== undefined &&
    nextActionId === undefined &&
    detail.edges.some((edge) => edge.source_action_id === previousActionId)
  ) {
    throw new Error(
      `spliceAction: anchor previousActionId "${previousActionId}" is not the tail action`,
    );
  }
  if (
    previousActionId === undefined &&
    nextActionId !== undefined &&
    detail.edges.some((edge) => edge.target_action_id === nextActionId)
  ) {
    throw new Error(`spliceAction: anchor nextActionId "${nextActionId}" is not the head action`);
  }
  const actions = [...detail.actions, action];
  const newEdges = detail.edges.filter(
    (edge) =>
...
export const updateWaitAction = ({
  detail,
  actionId,
  waitHours,
}: UpdateWaitActionArgs): AutomationDetail => {
  if (!Number.isSafeInteger(waitHours) || waitHours <= 0) {
    throw new Error(
      `updateWaitAction: waitHours must be a safe positive integer, received "${waitHours}"`,
    );
  }
...
export const updateSendEmailAction = ({
  detail,
  actionId,
  emailSubject,
  emailLexical,
}: UpdateSendEmailActionArgs): AutomationDetail => {
  let hasUpdated = false;
  const actions = detail.actions.map((action) => {
    if (action.id === actionId) {
