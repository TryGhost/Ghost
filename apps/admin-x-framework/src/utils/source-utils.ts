...
export const getFaviconDomain = (
  ...
  siteUrl?: string,
): { domain: string | null; isDirectTraffic: boolean } => {
...
export function processSources({
  ...
  const hasDirectTraffic =
    mode === 'visits'
      ? directTrafficTotal! > 0
      : directTrafficData &&
        (directTrafficData.free_members > 0 ||
          directTrafficData.paid_members > 0 ||
          directTrafficData.mrr > 0);
