...
async function loadMoreComments({
  ...
  const dedupedComments = updatedComments.filter(
...
async function addReply({
  ...
  state: EditableAppContext;
  api: GhostApi;
  data: { reply: AddComment; parent: Comment };
}) {
  ...
  const replies = allReplies.comments.some(
    (replyComment: Comment) => replyComment.id === newComment.id,
  )
    ? allReplies.comments
...
async function updateCommentLikeState({
  ...
  data: {
    ...
    liked: boolean;
    wasDisliked?: boolean;
    restoreDisliked?: boolean;
    expectedLiked?: boolean;
    expectedDisliked?: boolean;
    expectedVotePending?: boolean;
    votePending?: boolean;
  };
}) {
  return {
    comments: state.comments.map((c) => {
      const replies = c.replies.map((r) => {
        if (r.id === comment.id) {
          ...
          if (
            comment.expectedDisliked !== undefined &&
            Boolean(r.disliked) !== comment.expectedDisliked
          ) {
            return r;
          }
          if (
            comment.expectedVotePending !== undefined &&
            Boolean(r.votePending) !== comment.expectedVotePending
          ) {
            return r;
          }
      ...
      if (c.id === comment.id) {
        ...
        if (
          comment.expectedDisliked !== undefined &&
          Boolean(c.disliked) !== comment.expectedDisliked
        ) {
          return {
            ...c,
            replies,
        ...
        if (
          comment.expectedVotePending !== undefined &&
          Boolean(c.votePending) !== comment.expectedVotePending
        ) {
          return {
            ...c,
            replies,
...
async function updateCommentDislikeState({
  state,
  data: comment,
}: {
  state: EditableAppContext;
...
async function updateCommentVotePendingState({
  state,
  data: comment,
}: {
  state: EditableAppContext;
  data: { id: string; votePending: boolean; expectedVotePending?: boolean };
}) {
  return {
    comments: state.comments.map((c) => {
      const replies = c.replies.map((r) => {
...
async function likeComment({
  ...
  dispatchAction('updateCommentLikeState', {
    id: comment.id,
    liked: true,
    wasDisliked,
    votePending: true,
  ...
  try {
    ...
    dispatchAction('updateCommentVotePendingState', {
      id: comment.id,
      votePending: false,
      expectedVotePending: true,
    });
    return {};
  } catch {
    dispatchAction('updateCommentLikeState', {
...
async function dislikeComment({
  ...
  dispatchAction('updateCommentDislikeState', {
    id: comment.id,
    disliked: true,
    wasLiked,
    votePending: true,
  ...
  try {
    ...
    dispatchAction('updateCommentDislikeState', {
      id: comment.id,
      disliked: false,
      restoreLiked: wasLiked,
      expectedDisliked: true,
      expectedVotePending: true,
      votePending: false,
    });
  }
...
async function undislikeComment({
  state,
  api,
  data: comment,
  dispatchAction,
  ...
  return {
    comments: state.comments
      .map((topLevelComment) => {
        ...
        const keepTombstone = replyToDelete
          ? hasDescendantReply(topLevelComment.replies, replyToDelete.id)
          : false;
        ...
        if (
          hasDeletedReply &&
          replyToDelete &&
          !['hidden', 'deleted'].includes(replyToDelete.status)
        ) {
          const wasDirectReply =
            !replyToDelete.in_reply_to_id || replyToDelete.in_reply_to_id === topLevelComment.id;

          updatedTopLevelComment.count = {
            ...updatedTopLevelComment.count,
            replies: decrementCount(updatedTopLevelComment.count?.replies),
            direct_replies: wasDirectReply
              ? decrementCount(updatedTopLevelComment.count?.direct_replies)
              : updatedTopLevelComment.count?.direct_replies,
          };
        }
...
type SyncActionPayloadMap = {
  [T in SyncActionType]: Parameters<(typeof SyncActions)[T]>[0] extends { data: infer Data }
    ? Data
    : never;
};
...
export async function ActionHandler<T extends ActionType>({
  action,
  data,
  state,
  api,
  ...
  const handler = Actions[action] as (
    args: Parameters<(typeof Actions)[T]>[0],
  ) => ReturnType<(typeof Actions)[T]>;
  if (handler) {
    return (
      (await handler({ data, state, api, adminApi, options, dispatchAction } as Parameters<
        (typeof Actions)[T]
      >[0])) || {}
    );
  }
...
/** Handle actions in the App, returns updated state */
export function SyncActionHandler<T extends SyncActionType>({
  action,
  data,
  state,
  api,
  ...
  action: T;
  data: SyncActionPayload<T>;
  state: EditableAppContext;
  options: CommentsOptions;
  api: GhostApi;
  adminApi: AdminApi;
}): Partial<EditableAppContext> {
  const handler = SyncActions[action] as (
    args: Parameters<(typeof SyncActions)[T]>[0],
  ) => ReturnType<(typeof SyncActions)[T]>;
  if (handler) {
    // Do not await here
    return (
      handler({ data, state, api, adminApi, options } as Parameters<(typeof SyncActions)[T]>[0]) ||
      {}
    );
  }
  return {};
