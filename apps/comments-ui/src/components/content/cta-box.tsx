...
const CTABox: React.FC<Props> = ({ isFirst, isPaid, commentId }) => {
  ...
  return (
    <>
      <h1
        className={`mb-[8px] text-center font-sans text-2xl tracking-tight  text-black dark:text-[rgba(255,255,255,0.85)] ${isFirst ? 'font-semibold' : 'font-bold'}`}
      >
        {titleText}
      </h1>
      ...
      <button
        className="mb-[12px] inline-block rounded px-5 py-[14px] font-sans text-md font-medium leading-none text-white transition-all hover:opacity-90"
      ...
      {!member && (
        <p className="text-center font-sans text-md text-[rgba(0,0,0,0.4)] dark:text-[rgba(255,255,255,0.5)]">
