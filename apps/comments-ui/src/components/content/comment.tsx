...
const AuthorName: React.FC<{comment: Comment}> = ({comment}) => {
    ...
    const blueskyHandle = comment.member?.bluesky_handle;
    return (
        <div className="flex items-center gap-1">
            <h4 className="font-sans text-base font-bold leading-snug text-neutral-900 sm:text-sm dark:text-white/85">
                {name}
            </h4>
            {blueskyHandle && <BlueskyBadge handle={blueskyHandle} />}
        </div>
    );
};

const BlueskyBadge: React.FC<{handle: string}> = ({handle}) => {
    return (
        <a
            className="inline-flex items-center gap-0.5 rounded-sm px-1 py-0.5 font-sans text-xs leading-none text-blue-500 no-underline hover:text-blue-600 dark:text-blue-400 dark:hover:text-blue-300"
            href={`https://bsky.app/profile/${handle}`}
            rel="noopener noreferrer"
            target="_blank"
            title={`@${handle} on Bluesky`}
        >
            <svg className="h-3 w-3" fill="currentColor" viewBox="0 0 600 530" xmlns="http://www.w3.org/2000/svg">
