 ...
 */
function onIframeResize(iframe: HTMLIFrameElement, callback: () => void): () => void {
...
type NavigationHelpers = {
  ...
  scrollToComment: (
    element: HTMLElement,
    commentId: string,
    options?: { preserveScrollTarget?: boolean },
  ) => void;
...
function useNavigationHelpers(containerRef: RefObject<HTMLDivElement>): NavigationHelpers {
  ...
  const scrollToComment = useCallback(
    (element: HTMLElement, commentId: string, options: { preserveScrollTarget?: boolean } = {}) => {
  ...
  const scrollToCommentInstantly = useCallback(
    (commentId: string) => {
      afterNextLayoutFrame(() => {
...
function useScrollToHashTarget(
  ...
  const { comments, commentsIsLoading, commentIdFromHash, commentIdToScrollTo, dispatchAction } =
    useAppContext();
    ...
    if (iframe) {
      ...
      return onIframeResize(iframe, () => {
        scrollToComment(element, commentIdToScrollTo, {
          preserveScrollTarget: commentIdToScrollTo === commentIdFromHash,
        });
      });
    }
...
function useBackToParentScroll(
  ...
  useEffect(() => {
    ...
    if (
      !commentIdFromHash ||
      focusedThread ||
      instantScrollCommentId.current !== commentIdFromHash
    ) {
      return;
    }
...
export function useCommentNavigation({
  ...
  useHashChangeNavigation(
    containerRef,
    focusedThreadViewCommentId,
    instantScrollCommentId,
    helpers,
  );
