    ...
    if (editor && openForm) {
      const checkContent = () => {
        const hasUnsavedChanges =
          comment && openForm.type === 'edit' ? editor.getHTML() !== comment.html : !editor.isEmpty;
  ...
  return (
    <>
      ...
      <div className="absolute bottom-1 right-1 flex space-x-4 transition-[opacity] duration-150 sm:bottom-2 sm:right-2">
        {close && (
          <button
            className="ml-2.5 font-sans text-sm font-medium text-neutral-900/50 outline-0 transition-all hover:text-neutral-900/70 dark:text-white/60 dark:hover:text-white/75"
            type="button"
            onClick={close}
          >
            {t('Cancel')}
    ...
    >
      <div className="flex flex-wrap">
        ...
        <div className="flex items-baseline justify-start">
          ...
          >
            <span>
              <span className="mx-[0.3em] hidden sm:inline">·</span>
              {expertise ? expertise : t('Add your expertise')}
            </span>
            {expertise && (
              <EditIcon className="ml-1 size-[12px] translate-x-[-6px] stroke-black/50 opacity-0 transition-all duration-100 ease-out group-hover:translate-x-0 group-hover:stroke-black/75 group-hover:opacity-100 dark:stroke-white/60 dark:group-hover:stroke-white/75" />
            )}
          </button>
        </div>
      </div>
      {isReplyingToReply && (
        <div
          className="mt-0.5 line-clamp-1 font-sans text-base leading-snug text-neutral-900/50 dark:text-white/60 sm:text-sm"
          data-testid="replying-to"
        >
          <span>{t('Reply to')}:</span>&nbsp;
          <span className="font-semibold text-neutral-900/60 dark:text-white/70">
            {replyingToText}
          </span>
        </div>
      )}
...
const FormWrapper: React.FC<FormWrapperProps> = ({
  ...
  if (isOpen) {
    ...
    openStyles = isReplyToReply
      ? 'pl-[1px] pt-[68px] sm:pl-[44px] sm:pt-[56px]'
      : 'pl-[1px] pt-[48px] sm:pl-[44px] sm:pt-[40px]';
  }
  ...
  return (
    <div
      className={`${isReplyLayout ? '' : '-mx-2 mt-[-10px]'} rounded-md transition duration-200 ${isOpen ? 'cursor-default' : 'cursor-pointer'}`}
    >
      <div className="relative w-full" onClick={focusEditor}>
        <div className="pr-[1px] font-sans leading-normal dark:text-neutral-300">
          <div
            className={`relative mb-7 w-full pl-[40px] transition-[padding] delay-100 duration-150 sm:pl-[44px] ${reduced && 'pl-0'} ${openStyles}`}
          >
            {children}
          </div>
        </div>
        <div
          className={`absolute left-0 flex h-11 w-full items-start justify-start sm:h-12 ${isReplyLayout ? 'top-0' : 'top-1'}`}
        >
