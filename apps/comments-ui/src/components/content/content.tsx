...
import {
  DESKTOP_MAX_THREAD_DEPTH,
  MOBILE_BREAKPOINT,
  MOBILE_MAX_THREAD_DEPTH,
} from '../../utils/helpers';
...
const Content = () => {
  ...
  const content = focusedThread ? (
    <>
      ...
      <div
        ref={containerRef}
        className={`z-10 transition-opacity duration-100 ${commentsIsLoading ? 'opacity-50' : ''}`}
        data-testid="comment-elements"
      >
    ...
    <>
      ...
      {showMissingCommentNotice && (
        <div
          className="mb-6 rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 font-sans text-sm text-amber-900 dark:border-amber-500/40 dark:bg-amber-500/15 dark:text-amber-100"
          data-testid="missing-comment-notice"
        >
      ...
      <div>
        ...
        {showDisabledBox && (
          <section
            className="flex flex-col items-center py-6 sm:px-8 sm:py-10"
            data-testid="commenting-disabled-box"
          >
      ...
      <Pagination />
      {labs?.testFlag ? (
        <div data-testid="this-comes-from-a-flag" style={{ display: 'none' }}></div>
      ) : null}
    </>
  );
  ...
  return (
    <NavActionsContext.Provider value={navActions}>
      <ThreadingContext.Provider value={threadingContext}>{content}</ThreadingContext.Provider>
