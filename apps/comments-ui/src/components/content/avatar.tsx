...
export const BlankAvatar = () => {
  ...
  return (
    <figure className={`relative ${dimensionClasses}`} data-testid="blank-avatar">
      <div
        className={`flex items-center justify-center rounded-full bg-black/5 text-neutral-900/25 dark:bg-white/15 dark:text-white/30 ${dimensionClasses}`}
      >
        <AvatarIcon className="size-7 opacity-80" />
      </div>
    </figure>
  );
};
...
export const Avatar: React.FC<AvatarProps> = ({ member }) => {
  ...
  const generateHSL = (): [number, number, number] => {
    ...
    const saturation =
      avatarSaturation === undefined || isNaN(avatarSaturation) ? 50 : avatarSaturation;
  ...
  const avatarEl = (
    <>
      {memberName ? (
        <div
          className={`flex items-center justify-center rounded-full ${dimensionClasses}`}
        ...
        <div
          className={`flex items-center justify-center rounded-full bg-neutral-900 dark:bg-white/70 ${dimensionClasses}`}
          data-testid="avatar-background"
        >
      ...
      {member?.avatar_image && (
        <img
          alt="Avatar"
          className={`absolute left-0 top-0 rounded-full ${dimensionClasses}`}
