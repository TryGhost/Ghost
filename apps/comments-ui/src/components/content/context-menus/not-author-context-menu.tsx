...
const NotAuthorContextMenu: React.FC<Props> = ({ comment, close }) => {
  ...
  const itemClassName =
    'flex w-full items-center gap-3 rounded px-3 py-2 text-left text-[14px] leading-5 text-neutral-900 transition-colors hover:bg-neutral-100 dark:text-white dark:hover:bg-neutral-700';
  ...
  return (
    <div className="flex w-full flex-col gap-0.5">
      <button className={itemClassName} type="button" onClick={openModal}>
        ...
        <span className="hidden sm:inline">{t('Report comment')}</span>
