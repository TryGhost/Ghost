...
const AddDetailsPopup = (props: Props) => {
      ...
      return (
        ...
        >
          <div className="flex flex-row items-center justify-start gap-3 pr-4">
            <div
              className="size-10 shrink-0 rounded-full border-2 border-white bg-cover bg-no-repeat"
    ...
    const exampleProfiles = [
      {
        ...
        name: 'James Fletcher',
        expertise: t('Full-time parent'),
      },
      {
        avatar: 'https://randomuser.me/api/portraits/women/30.jpg',
        name: 'Naomi Schiff',
        expertise: t('Founder @ Acme Inc'),
      },
      {
        avatar: 'https://randomuser.me/api/portraits/men/4.jpg',
        name: 'Franz Tost',
        expertise: t('Neurosurgeon'),
      },
      {
        avatar: 'https://randomuser.me/api/portraits/women/51.jpg',
        name: 'Katrina Klosp',
        expertise: t('Local resident'),
      },
    ];
  ...
  return (
    <div
      className="rounded-none relative h-screen w-screen overflow-hidden bg-white p-[28px] text-center shadow-modal sm:h-auto sm:w-[720px] sm:rounded-xl sm:p-0"
      data-testid="profile-modal"
      onMouseDown={(event) => event.stopPropagation()}
    >
      <div className="flex">
        <div
          className={`hidden w-[50%] flex-col items-center justify-center bg-neutral-800 sm:block sm:p-8`}
        >
          <div className="mt-[-1px] flex flex-col gap-9 text-left">{renderExampleProfiles()}</div>
        </div>
        <div className={`p-0 sm:p-8`}>
          <h1 className="mb-1 text-center font-sans text-2xl font-bold tracking-tight text-black sm:text-left">
            {t('Complete your profile')}
            <span className="hidden sm:inline">.</span>
          </h1>
          <p className="pr-0 text-center font-sans text-md leading-snug text-neutral-500 sm:pr-10 sm:text-left">
            {t(
          ...
          <section className="mt-8 text-left">
            <div className="mb-2 flex flex-row justify-between">
              <label className="font-sans text-base font-semibold" htmlFor="comments-name">
                {t('Name')}
              </label>
              <Transition
            ...
            <div className="mb-2 mt-6 flex flex-row justify-between">
              ...
              <div
                className={`font-sans text-base text-neutral-400 ${expertiseCharsLeft === 0 && 'text-red-500'}`}
              >
                {charsText}
              </div>
            </div>
