...
const App: React.FC<AppProps> = ({ scriptTag, initialCommentId }) => {
  ...
  const setState = useCallback(
    (
      newState:
        ...
        | ((state: EditableAppContext) => Partial<EditableAppContext>),
    ) => {
      setFullState((state) => {
  ...
  const dispatchAction = useCallback(
    ...
    [api, options],
  ); // Do not add state or context as a dependency here -> infinite render loop
    ...
    try {
      ...
      try {
        ...
        if (admin) {
          ...
          const adminComments = await adminApi.browse({
            page: 1,
  ...
  const loadScrollTarget = async (
    ...
    const { comments: paginatedComments, pagination } = await paginateToComment(
      targetId,
      parentId,
      initialComments,
      initialPagination,
    ...
    if (parentId && !isCommentLoaded(comments, targetId)) {
      const { comments: allReplies } = await api.comments.replies({
        commentId: parentId,
        limit: 'all',
      });
  ...
  const initSetup = async () => {
    try {
      ...
      } = await fetchComments(NET_SCORE_ORDER);
