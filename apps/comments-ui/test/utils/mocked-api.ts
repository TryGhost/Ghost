...
export class MockedApi {
  ...
  browseComments({
    ...
    if (setOrder === 'count__net_score desc, created_at desc') {
      ...
      this.comments.sort((a, b) => {
        const likesDiff =
          b.count.likes - (b.count.dislikes ?? 0) - (a.count.likes - (a.count.dislikes ?? 0));
        if (likesDiff !== 0) {
          return likesDiff;
        } // Prioritize by likes
        ...
        return comments
          .filter(
            (comment) =>
              ...
              comment.replies?.some((r) => r.status === 'published'),
          )
  ...
  requestHandlers = {
    ...
    async getOrDeleteComment(route) {
      ...
      if (
        ...
        JSON.parse(route.request().postData()).comments?.[0]?.status === 'deleted'
      ) {
    ...
    async getCommentCounts(route) {
      ...
      await route.fulfill({
        status: 200,
        body: JSON.stringify(this.commentsCounts()),
      });
    },
  ...
  async listen({ page, path }: { page: any; path: string }) {
    ...
    await page.route(
      `${path}/members/api/comments/post/*/*`,
      this.requestHandlers.browseComments.bind(this),
    );
    await page.route(
      `${path}/members/api/comments/*/`,
      this.requestHandlers.getOrDeleteComment.bind(this),
    );
    await page.route(
      `${path}/members/api/comments/*/like/`,
      this.requestHandlers.likeComment.bind(this),
    );
    await page.route(
      `${path}/members/api/comments/*/dislike/`,
      this.requestHandlers.dislikeComment.bind(this),
    );
    await page.route(
      `${path}/members/api/comments/*/replies/*`,
      this.requestHandlers.getReplies.bind(this),
    );
    await page.route(
      `${path}/members/api/comments/counts/*`,
      this.requestHandlers.getCommentCounts.bind(this),
    );
    ...
    await page.route(
      ...
      this.adminRequestHandlers.getUser.bind(this),
    );
    await page.route(
      `${path}/ghost/api/admin/comments/post/*/*`,
      this.adminRequestHandlers.browseComments.bind(this),
    );
    await page.route(
      `${path}/ghost/api/admin/comments/*/*`,
      this.adminRequestHandlers.getOrUpdateComment.bind(this),
    );
    await page.route(
      `${path}/ghost/api/admin/comments/*/replies/*`,
      this.adminRequestHandlers.getReplies.bind(this),
    );
  }
}
