...
describe('buildCommentPermalink', function () {
  it('builds a fragment-only permalink so the browser resolves it against the live page URL', function () {
    expect(helpers.buildCommentPermalink('abc123')).toEqual('#ghost-comments-abc123');
  });
});

describe('buildCommentsRootPermalink', function () {
  it('builds a fragment-only permalink for the comments root', function () {
    expect(helpers.buildCommentsRootPermalink()).toEqual('#ghost-comments');
  });
});

describe('parseCommentIdFromHash', function () {
  it('extracts comment ID from valid hash', function () {
    expect(helpers.parseCommentIdFromHash('#ghost-comments-abc123')).toEqual('abc123');
  });

  it('handles uppercase hex characters', function () {
    expect(helpers.parseCommentIdFromHash('#ghost-comments-ABC123DEF')).toEqual('ABC123DEF');
  });

  it('returns null for hash without prefix', function () {
    expect(helpers.parseCommentIdFromHash('#some-other-hash')).toBeNull();
  });

  it('returns null for empty hash', function () {
    expect(helpers.parseCommentIdFromHash('')).toBeNull();
  });

  it('returns null for hash with only prefix', function () {
    expect(helpers.parseCommentIdFromHash('#ghost-comments-')).toBeNull();
  });

  it('returns null for hash with non-hex characters in ID', function () {
    expect(helpers.parseCommentIdFromHash('#ghost-comments-xyz123')).toBeNull();
  });

  it('returns null when prefix is not at start', function () {
    expect(helpers.parseCommentIdFromHash('#other-ghost-comments-abc123')).toBeNull();
  });
});
...
describe('formatRelativeTime', () => {
  ...
  const testCases: Array<[string, DurationInputObject, string, Date?]> = [
    ...
    [
      'yesterday > 1hr < 2hrs ago',
      { minutes: 65 },
      'One hour ago',
      new Date('2024-02-15T00:00:10.000Z'),
    ],
...
describe('getCommentInReplyToSnippet', function () {
  ...
  it('trims whitespace', function () {
    testGetSnippet(
      { html: '<p>  Test  <br />New line</p>\n<p>New paragraph</p>' },
      'Test New line New paragraph',
    );
  });

  it('strips blockquotes', function () {
    testGetSnippet(
      { html: '<blockquote>Previous comment</blockquote>\n<p>My reply to quote</p>' },
      'My reply to quote',
    );
  });
  ...
  it('ignores image alt text', function () {
    testGetSnippet(
      { html: '<img alt="Image alt text" src="image.jpg" />\n<p>Test comment</p>' },
      'Test comment',
    );
  });
