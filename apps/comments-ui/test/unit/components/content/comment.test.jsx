...
describe('<CommentComponent>', function () {
  ...
  it('renders threaded reply forms directly below the comment being replied to', function () {
    ...
    expect(
      firstReply.compareDocumentPosition(replyForm) & Node.DOCUMENT_POSITION_FOLLOWING,
    ).toBeTruthy();
    expect(
      replyForm.compareDocumentPosition(secondReply) & Node.DOCUMENT_POSITION_FOLLOWING,
    ).toBeTruthy();
  });
  ...
  it('renders pinned badge inline after the timestamp', function () {
    ...
    expect(label.parentElement?.parentElement?.querySelector('a')).toHaveAttribute(
      'href',
      `#ghost-comments-${comment.id}`,
    );
  });
