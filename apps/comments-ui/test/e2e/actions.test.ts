...
test.describe('Actions', async () => {
    ...
    async function testReplyToReply(page) {
        ...
        // The reply form should appear under the reply comment being replied to
        const editor = replyComment.getByTestId('form-editor');
        await expect(editor).toBeVisible();
        await waitEditorFocused(editor);
        ...
        const submitButton = replyComment.getByTestId('submit-form-button');
        await submitButton.click();
