...
test.describe('Editor', async () => {
  test('Editor placeholder shows Start the conversation when no existing comments ', async ({
    page,
  }) => {
  ...
  test('Start the conversation changes to Join the Discussion if more 0 comments. ', async ({
    page,
  }) => {
    const mockedApi = new MockedApi({});
    mockedApi.setMember({});
  ...
  test.describe('Markdown', () => {
    test('Can use > to type a quote', async ({ page }) => {
      ...
      expect(mockedApi.comments[0].html).toBe(
        '<blockquote><p>This is a quote</p><p>This is a new line</p></blockquote><p>This is a new paragraph<br></p>',
      );
    });
    ...
    test('Can paste an URL to create a link', async ({ page }) => {
      ...
      expect(mockedApi.comments).toHaveLength(1);
      expect(mockedApi.comments[0].html).toBe(
        '<p>Click <a target="_blank" rel="noopener noreferrer nofollow" href="https://www.google.com">here</a> to go to a new page</p>',
      );
    });
    ...
    test('Can do a soft line break', async ({ page }) => {
      ...
      expect(mockedApi.comments).toHaveLength(1);
      expect(mockedApi.comments[0].html).toBe(
        '<p>This is line 1<br>This is line 2</p><p>This is a new paragraph</p>',
      );
    });
  });
});
