...
test.describe('CTA', async () => {
  ...
  test('Shows CTA when logged in, but not a paid member and comments are paid only', async ({
    page,
  }) => {
    const mockedApi = new MockedApi({});
    mockedApi.addComments(2);
    ...
    await expect(ctaBox).toContainText('Join the discussion');
    await expect(ctaBox).toContainText(
      'Become a paid member of Publisher Weekly to start commenting',
    );
