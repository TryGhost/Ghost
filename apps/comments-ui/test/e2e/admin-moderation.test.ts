...
test.describe('Admin moderation', async () => {
  ...
  test('has no admin options when not signed in to Ghost admin but signed in as member', async ({
    page,
  }) => {
    mockedApi.addComment({ html: '<p>This is comment 1</p>' });
  ...
  test('has admin options when signed in to Ghost admin but not signed in as member', async ({
    page,
  }) => {
    mockedApi.addComment({ html: `<p>This is comment 1</p>` });
  ...
  test('has pin option when signed in to Ghost admin and viewing another member comment', async ({
    page,
  }) => {
    mockedApi.addComment({ html: `<p>This is comment 1</p>` });
    const { frame } = await initializeTest(page);
  ...
  test('admin can see comments hidden from members even when all comments are hidden', async ({
    page,
  }) => {
  ...
  test.describe('View in admin link', () => {
    test('shows View in admin link for admins', async ({ page }) => {
      ...
      await expect(viewInAdminLink).toHaveAttribute(
        'href',
        `${admin}#/comments/?id=is:test-comment-id`,
      );
