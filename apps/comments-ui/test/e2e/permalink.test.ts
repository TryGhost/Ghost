...
const TALL_BODY_HTML =
  '<html><head><meta charset="UTF-8" /></head><body><div style="width: 100%; height: 1500px;"></div></body></html>';
...
test.describe('Comment Permalinks', async () => {
  test('timestamp is a link with correct permalink href', async ({ page }) => {
    ...
    await expect(timestampLink).toHaveAttribute(
      'href',
      new RegExp(`#ghost-comments-${commentId}$`),
    );
  });
  ...
  test('shows highlight mark inside comment content when navigating via permalink', async ({
    page,
  }) => {
    const mockedApi = new MockedApi({});
    mockedApi.setMember({});
  ...
  test('handles invalid comment ID gracefully', async ({ page }) => {
    ...
    const commentsFrame = await setupPermalinkTest(
      page,
      mockedApi,
      '#ghost-comments-dead00000000000000000000',
      TALL_BODY_HTML,
    );
    ...
    await expect(commentsFrame.getByTestId('missing-comment-notice')).toContainText(
  ...
  test('hash without ghost-comments prefix does not trigger permalink behavior', async ({
    page,
  }) => {
    const mockedApi = new MockedApi({});
    mockedApi.setMember({});
  ...
  test('shows missing-comment notice for hidden permalink and scrolls to comments area', async ({
    page,
  }) => {
    const mockedApi = new MockedApi({});
    mockedApi.setMember({});
    ...
    const commentsFrame = await setupPermalinkTest(
      page,
      mockedApi,
      `#ghost-comments-${hiddenCommentId}`,
      TALL_BODY_HTML,
    );
  ...
  test('shows missing-comment notice for deleted permalink and scrolls to comments area', async ({
    page,
  }) => {
    const mockedApi = new MockedApi({});
    mockedApi.setMember({});
    ...
    const commentsFrame = await setupPermalinkTest(
      page,
      mockedApi,
      `#ghost-comments-${deletedCommentId}`,
      TALL_BODY_HTML,
    );
  ...
  test('highlights nested reply instead of top-level parent', async ({ page }) => {
    ...
    const parentContent = commentsFrame
      .locator(`[id="${parentId}"]`)
      .getByTestId('comment-content')
      .first();
    const replyContent = commentsFrame
      .locator(`[id="${targetReplyId}"]`)
      .getByTestId('comment-content')
      .first();
  ...
  test('highlights target reply instead of focused ancestor in the focused view', async ({
    page,
  }) => {
    const mockedApi = new MockedApi({});
    mockedApi.setMember({});
    ...
    const targetReplyContent = commentsFrame
      .locator(`[id="${targetReplyId}"]`)
      .getByTestId('comment-content')
      .first();
  ...
  test('opens the focused view when target reply is not in the initial comments response', async ({
    page,
  }) => {
    const mockedApi = new MockedApi({});
    mockedApi.setMember({});
  ...
  test('loads reply via server fetch when permalink target is not in partial API response', async ({
    page,
  }) => {
    const mockedApi = new MockedApi({});
    mockedApi.setMember({});
  ...
  test('admin auth does not overwrite paginated comments loaded for permalink', async ({
    page,
  }) => {
    const mockedApi = new MockedApi({});
