import BrandSettings, {
    BrandSettingValues,
} from "./designAndBranding/BrandSettings";
    ...
    const tabs: Tab[] = [
        {
            ...
            contents: (
                <BrandSettings
                    updateSetting={updateBrandSetting}
        ...
        <div className="flex h-full flex-col justify-between">
            ...
            <StickyFooter height={74}>
                <div className="w-full px-7">
                    <button
                        className="group flex w-full items-center justify-between text-sm font-medium opacity-80 transition-all hover:opacity-100"
                    ...
                    >
                        <div className="text-left">
                            ...
                            <div className="font-sm text-grey-700">
                                Current theme: {activeTheme?.name} - v
                                {activeTheme?.package.version}
                            </div>
                        </div>
                        <Icon
                            className="mr-2 transition-all group-hover:translate-x-2 dark:text-white"
                            name="chevron-right"
                            size="sm"
                        />
                    </button>
...
const DesignModal: React.FC = () => {
    ...
    const { formState, saveState, handleSave, updateForm, setFormState } =
        useForm({
            initialState: {
                settings: settings as Array<Setting & { dirty?: boolean }>,
                themeSettings: themeSettings
                    ? (themeSettings.custom_theme_settings as Array<
                          CustomThemeSetting & { dirty?: boolean }
                      >)
                    : undefined,
            },
            onSave: async () => {
                if (formState.themeSettings?.some((setting) => setting.dirty)) {
                    const response = await editThemeSettings(
                        formState.themeSettings
                    );
                    updateForm((state) => ({
                        ...state,
                        themeSettings: response.custom_theme_settings,
                    }));
                }
    ...
    const [description, accentColor, icon, logo, coverImage] = getSettingValues(
        formState.settings,
        ["description", "accent_color", "icon", "logo", "cover_image"]
    ) as string[];

    const themeSettingGroups = (formState.themeSettings || []).reduce(
        (groups, setting) => {
            const group =
                setting.group === "homepage" || setting.group === "post"
                    ? setting.group
                    : "site-wide";
    ...
    const themeSettingSections = Object.entries(themeSettingGroups).map(
        ([id, group]) => ({
            id,
            settings: group || [],
            title:
