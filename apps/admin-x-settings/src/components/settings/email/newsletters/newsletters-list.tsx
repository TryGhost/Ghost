...
const NewsletterItemContainer: React.FC<Partial<SortableItemContainerProps>> = ({
    ...
    const container = (
        <TableRow ref={setRef}
            ...
            onClick={showDetails}
        >
            {(props.dragHandleAttributes || isDragging) && <TableCell className='align-middle! w-10' >
        ...
        <>
            <TableCell className='w-full' onClick={showDetails}>
                <div className={`flex grow flex-col`}>
                    ...
                    <span className='text-grey-700 mt-0.5 text-xs leading-tight'>{newsletter.description || 'No description'}</span>
                </div>
            </TableCell>
            <TableCell className='md:visible! md:table-cell! hidden md:min-w-[11rem]' onClick={showDetails}>
                <div className={`flex grow flex-col`}>
                    <span>{numberWithCommas(newsletter.count?.active_members || 0) }</span>
                    <span className='text-grey-700 mt-0.5 whitespace-nowrap text-xs leading-tight'>Subscribers</span>
                </div>
            </TableCell>
            <TableCell className='md:visible! md:table-cell! hidden md:min-w-[11rem]' onClick={showDetails}>
                <div className={`flex grow flex-col`}>
                    <span>{numberWithCommas(newsletter.count?.posts || 0)}</span>
                    <span className='text-grey-700 mt-0.5 whitespace-nowrap text-xs leading-tight'>Delivered</span>
                </div>
            </TableCell>
        </>
    );
