...
const NewsletterPreview: React.FC<{newsletter: Newsletter}> = ({newsletter}) => {
    ...
    const backgroundColorIsDark = textColorForBackgroundColor(backgroundColor()).hex().toLowerCase() === '#ffffff';
        ...
        <div className="relative flex grow flex-col">
            <div className="absolute inset-0 m-5 flex items-center justify-center">
                <div className="mx-auto my-0 flex max-h-full w-full max-w-[700px] flex-col overflow-hidden rounded-[4px] text-black shadow-sm">
                    ...
                    {/* Email content */}
                    <div className="overflow-y-auto p-4 text-sm" style={{backgroundColor: backgroundColor()}}>
                        <div className="border border-transparent px-16" style={{borderColor: borderColor()}}>
                            ...
                            {showHeader && (
                                <div className="border-b py-12" style={{borderColor: secondaryBorderColor}}>
                                    {(newsletter.show_header_icon && icon) && <img alt="" className="mx-auto mb-2 h-10 w-10" role="presentation" src={icon} />}
                                    {headerTitle && <h4 className="mb-1 text-center text-[1.6rem] font-bold uppercase leading-tight tracking-tight" style={{color: textColor}}>{headerTitle}</h4>}
                                    {headerSubtitle && <h5 className="mb-1 text-center text-[1.4rem] font-normal leading-tight" style={{color: secondaryTextColor}}>{headerSubtitle}</h5>}
                                </div>
                            )}
                            {newsletter.show_post_title_section && (
                                <div className={clsx('flex flex-col pb-10 pt-12', newsletter.title_alignment === 'center' ? 'items-center' : 'items-start')}>
                                    <h2 className={clsx(
                                        'pb-4 text-5xl font-bold leading-supertight',
                                    ...
                                    <div className={clsx(
                                        'flex w-full justify-between text-center text-sm leading-none tracking-[0.1px]',
                            ...
                            {newsletter.show_feature_image && (
                                <>
                                    ...
                                    <div className="mt-1 w-full max-w-[600px] pb-[30px] text-center text-[1.3rem]" style={{color: secondaryTextColor}}>Feature image caption</div>
                                </>
                            )}

                            <div className={clsx('max-w-[600px] border-b py-5 text-[1.6rem] leading-[1.7] text-black', newsletter.body_font_category === 'serif' && 'font-serif')} style={{borderColor: secondaryBorderColor}}>
                            ...
                            {(showFeedback || showCommentCta) && (
                                <div className="grid gap-5 border-b px-6 py-5" style={{borderColor: secondaryBorderColor}}>
                            ...
                            {newsletter.show_latest_posts && (
                                <div className="border-b py-6" style={{borderColor: secondaryBorderColor}}>
                            ...
                            {newsletter.show_subscription_details && (
                                <div className="border-b py-8" style={{borderColor: secondaryBorderColor}}>
                            ...
                            <div className="flex flex-col items-center pt-10">
                                <div dangerouslySetInnerHTML={{__html: newsletter.footer_content || ''}} className="text break-words px-8 py-3 text-center text-[1.3rem] leading-base" style={{color: secondaryTextColor}} />

                                <div className="px-8 pb-14 pt-3 text-center text-[1.3rem]">
                                    <span style={{color: secondaryTextColor}}>{title} © {currentYear} &mdash; </span>
