...
const AutomationPreview: React.FC<AutomationPreviewProps> = ({model}) => {
    ...
    if (isManagedEmail(config)) {
        emailHeader = <><p className="leading-normal"><span className="font-semibold text-grey-900">From: </span><span>{senderName} ({renderedSenderEmail})</span></p>
            <p className="leading-normal">
                <span className="font-semibold text-grey-900">Reply-to: </span>{renderedReplyTo ? renderedReplyTo : renderedSenderEmail}
            </p>
        </>;
    } else {
        emailHeader = <><p className="leading-normal"><span className="font-semibold text-grey-900">{senderName}</span><span> {renderedSenderEmail}</span></p>
                ...
                <div className="mx-auto my-0 flex max-h-full w-full max-w-[700px] flex-col overflow-hidden rounded-[4px] text-black shadow-sm">
                    ...
                    <div className="overflow-y-auto text-sm" style={{backgroundColor: colors.backgroundColor}}>
                        ...
                        <div className='px-[7rem] pt-8'>
                            ...
                            >
                                <h1
                                    className={clsx('mb-6 text-4xl leading-supertight', titleFontClass, titleWeightClass(model.title_font_weight))}
                                ...
                                <h2
                                    className={clsx('mb-[13px] text-[2.6rem] leading-supertight', titleFontClass, titleWeightClass(model.title_font_weight))}
                                    style={{color: colors.headingColor}}
                                >
                                    Need inspiration?
                                </h2>
