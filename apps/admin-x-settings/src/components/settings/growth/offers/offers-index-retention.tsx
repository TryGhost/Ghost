                ...
                {filteredSignupOffers.map((offer) => {
                    ...
                    return (
                        <tr key={offer.id} className={`group relative scale-100 border-b border-b-grey-200 dark:border-grey-800 ${archived ? 'opacity-60' : ''}`} data-testid="offer-item">
                            ...
                            <td className='whitespace-nowrap p-0 text-sm'><a className={`block cursor-pointer p-5 ${offer.redemption_count === 0 ? '' : 'hover:underline'}`} href={offer.redemption_count > 0 ? createRedemptionFilterUrl(offer.id ? offer.id : '') : undefined} onClick={offer.redemption_count === 0 ? () => handleOfferEdit(offer?.id ? offer.id : '') : () => {}}>{offer.redemption_count}</a></td>
                            <td className='whitespace-nowrap p-0 text-sm'>
