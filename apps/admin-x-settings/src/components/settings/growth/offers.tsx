...
const OfferContainer: React.FC<{offerTitle: string, tier: Tier, cadence: string, redemptions: number, type: string, amount: number, currency: string, offerId: string, offerCode: string, goToOfferEdit: (offerId: string) => void}> = (
    ...
    return <div className='group flex h-full cursor-pointer flex-col justify-between gap-4 rounded-sm border border-transparent bg-grey-100 p-5 break-words transition-all hover:border-grey-100 hover:bg-grey-75 hover:shadow-sm min-[900px]:min-h-[187px] dark:bg-grey-950 dark:hover:border-grey-800' onClick={() => goToOfferEdit(offerId)}>
        <span className='text-[1.65rem] leading-tight font-bold tracking-tight text-black dark:text-white'>{offerTitle}</span>
