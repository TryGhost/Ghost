            ...
            <TableCell className='md:visible! md:table-cell! hidden w-[1%] whitespace-nowrap' valign="middle" onClick={showDetails}>
                {(signups === 0) ? (null) : (
                    <div className='-mt-px text-left'>
                        <span className='text-grey-700 -mb-px inline-block min-w-[60px] whitespace-nowrap text-left text-sm lowercase'>{freeMembersLabel}</span>
                    </div>
                )}
            </TableCell>
            {incomingRecommendation.recommending_back && <TableCell className='w-[1%] whitespace-nowrap group-hover/table-row:visible md:invisible'><div className='text-grey-700 mt-1 whitespace-nowrap text-right text-sm'>Recommending</div></TableCell>}
        </TableRow>
    );
};
