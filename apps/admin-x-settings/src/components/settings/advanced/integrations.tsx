...
const IntegrationItem: React.FC<IntegrationItemProps> = ({
    ...
    return <ListItem
        ...
        testId={testId}
        title={active ? <span className='inline-flex items-center gap-1'>{title} <span className='text-2xs text-green inline-flex items-center rounded-full bg-[rgba(48,207,67,0.15)] px-1.5 py-0.5 font-semibold uppercase tracking-wide'>Active</span></span> : title}
        onClick={handleClick}
    />;
};
...
const Integrations: React.FC<{ keywords: string[] }> = ({keywords}) => {
    ...
    const buttons = (
        <Button
            className='text-grey-900 hover:bg-grey-200 dark:hover:bg-grey-900 mt-[-5px] inline-flex h-7 cursor-pointer items-center justify-center whitespace-nowrap rounded px-3 text-sm font-semibold transition dark:text-white [&:hover]:text-black'
            color='clear'
        ...
        <TopLevelGroup
            ...
            customHeader={
                <div className='sm:-mt-5 md:-mt-7'>
                    <div className='border-grey-200 dark:border-grey-800 -mx-5 overflow-hidden rounded-t-xl border-b md:-mx-7'>
                    ...
                    <div className=' z-10 mt-6 flex items-start justify-between'>
                        ...
                        {
                            <Button className='text-grey-900 hover:bg-grey-200 dark:hover:bg-grey-900 mt-[-5px] inline-flex h-7 cursor-pointer items-center justify-center whitespace-nowrap rounded px-3 text-sm font-semibold transition dark:text-white [&:hover]:text-black' color='clear' label='Add custom integration' link onClick={() => {
                                updateRoute('integrations/new');
                                setSelectedTab('custom');
                            }} />
                        }
