...
export const TrialDaysLabel: React.FC<{size?: 'sm' | 'md'; trialDays: number;}> = ({size = 'md', trialDays}) => {
    ...
    const containerClassName = clsx(
        ...
        'text-grey-900 relative -mr-1 -mt-1 whitespace-nowrap rounded-full font-semibold leading-none tracking-wide'
    );

    return (
        <span className={containerClassName}>
            <span className="bg-ghostaccent dark:bg-pink absolute inset-0 block rounded-full opacity-20"></span>
...
const TierBenefits: React.FC<{benefits: string[]}> = ({benefits}) => {
    if (!benefits?.length) {
        return (
            <div className="text-md text-grey-900 mt-4 w-full leading-snug opacity-30">
        ...
        <>
            {
                benefits.map((benefit) => {
                    return (
                        <div key={benefit} className="text-md text-grey-900 mt-4 w-full leading-snug">
                            <div className="mb-2.5 flex items-start">
                                <Icon className="h-3.5! w-3.5! stroke-[3px]! mr-[10px] mt-[3px] min-w-[14px] overflow-visible" name='check' />
                                <div>{benefit}</div>
                            </div>
...
const DiscountLabel: React.FC<{discount: number}> = ({discount}) => {
    ...
    return (
        <span className="text-pink mt-1 text-sm font-semibold leading-none">{discount}% discount</span>
    );
};
...
const TierDetailPreview: React.FC<TierDetailPreviewProps> = ({tier, isFreeTier}) => {
    ...
    return (
        <div data-testid="tier-preview">
            ...
            <div className='border-grey-200 rounded-sm border bg-white dark:border-transparent'>
                <div className="flex-column relative flex min-h-[200px] w-full max-w-[420px] scale-90 items-start justify-stretch rounded bg-white p-4">
                    <div className="min-h-[56px] w-full">
                        ...
                        <div className="mt-4 flex w-full flex-row flex-wrap items-end justify-between gap-x-1 gap-y-[10px]">
                            <div className={`flex flex-wrap text-black ${((showingYearly && tier?.yearly_price === undefined) || (!showingYearly && tier?.monthly_price === undefined)) && !isFreeTier ? 'opacity-30' : ''}`}>
                                ...
                                {!isFreeTier && <span className="text-grey-800 ml-1 self-end text-[1.5rem] leading-snug">/{showingYearly ? 'year' : 'month'}</span>}
                            </div>
                    ...
                    <div className="flex-column flex w-full flex-1">
                        <div className="flex-1">
                            <div className={`text-grey-900 mt-4 w-full text-[1.55rem] font-semibold leading-snug ${!description && 'opacity-30'}`}>{description || (isFreeTier ? `Free preview` : 'Full access to premium content')}</div>
