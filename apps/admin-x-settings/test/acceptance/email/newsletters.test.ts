...
test.describe('Newsletter settings', async () => {
    ...
    test('Supports archiving newsletters', async ({page}) => {
        ...
        await archivedNewsletterModal.getByRole('button', {name: 'Close'}).last().click({force: true});
        ...
        await activeNewsletterModal.getByRole('button', {name: 'Close'}).last().click({force: true});
