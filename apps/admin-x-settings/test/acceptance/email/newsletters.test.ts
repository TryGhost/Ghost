...
test.describe('Newsletter settings', async () => {
    ...
    test('Supports archiving newsletters', async ({page}) => {
        ...
        await archivedNewsletterModal.getByRole('button', {name: 'Close'}).first().click({force: true});
        ...
        await activeNewsletterModal.getByRole('button', {name: 'Close'}).first().click({force: true});
