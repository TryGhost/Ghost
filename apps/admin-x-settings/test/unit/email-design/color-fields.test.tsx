...
const renderField = (field: ReactNode, settings = DEFAULT_EMAIL_DESIGN, accentColor = '#ff0088', onSettingsChange = () => {}) => {
    return render(
        <EmailDesignProvider
...
describe('Email design color fields', function () {
    it('renders accent-backed button colors using the resolved accent color', function () {
        ...
        const swatch = trigger.querySelector('.inset-\\[3px\\]');
    ...
    it('shows semantic swatches for each configured welcome-email color field', function () {
        ...
        assert.ok(screen.getByRole('button', {name: 'Transparent'}));
        header.unmount();
    ...
    it('stores semantic values when selecting accent and auto swatches', function () {
        ...
        assert.equal(updates[updates.length - 1]?.button_color, 'accent');

        openPicker('Button color');
        fireEvent.click(screen.getByRole('button', {name: 'Auto'}));
        assert.equal(updates[updates.length - 1]?.button_color, null);
    });
    ...
    it('stores semantic values for light and transparent swatches', function () {
        ...
        assert.equal(updates[updates.length - 1]?.background_color, 'light');

        openPicker('Header background color');
        fireEvent.click(screen.getByRole('button', {name: 'Transparent'}));
        assert.equal(updates[updates.length - 1]?.header_background_color, 'transparent');
    });
    ...
    it('stores null when selecting divider Light swatch', function () {
        ...
        assert.equal(updates[updates.length - 1]?.divider_color, null);
    });
