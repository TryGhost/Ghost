...
test.describe('Default recipient settings', async () => {
    ...
    test('Supports selecting specific tiers, labels and offers', async ({page}) => {
        const {lastApiRequests} = await mockApi({page, requests: {
            ...
            browseLabels: {method: 'GET', path: '/labels/?limit=20', response: responseFixtures.labels},
            browseOffers: {method: 'GET', path: '/offers/?limit=20', response: responseFixtures.offers},
            editSettings: {method: 'PUT', path: '/settings/', response: updatedSettingsResponse([
                {
