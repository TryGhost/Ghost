...
const TextField: React.FC<TextFieldProps> = ({
    ...
    const textFieldClasses = !unstyled && clsx(
        'peer z-[1] order-2 h-9 w-full bg-transparent px-3 py-1.5 placeholder:text-grey-500 md:h-[38px] md:py-2 dark:placeholder:text-grey-700',
    ...
    const rightPlaceholderClasses = !unstyled && clsx(
        ...
        (rightPlaceholder ?
            ((typeof (rightPlaceholder) === 'string') ? 'flex h-8 items-center py-1 pr-3 text-right text-grey-500 md:h-9' : 'h-9 pr-1')
            : 'pr-2')
    );
