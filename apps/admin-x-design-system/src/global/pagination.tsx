...
const Pagination: React.FC<PaginationProps> = ({page, limit, total, prevPage, nextPage}) => {
    ...
    if (total && limit < total) {
        return (
            <div className={`mt-1 flex items-center gap-2 text-sm text-grey-700`}>Showing {startIndex}-{endIndex} of {total}
    ...
    /* If there is more than X items total, where X is the number of items per page that we want to show */
        ...
        return (
            <div className={`mt-1 flex items-center gap-2 text-sm text-grey-700`}>Showing {total ?? '?'} in total
            </div>
        );
    }
};
